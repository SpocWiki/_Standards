---
aliases:
- Bản đồ sao
- carta celeste
- Carta estelar
- Carta estellara
- carte du ciel
- Csillagtérkép
- Harta cerului
- Himmelskarte
- hviezdna mapa
- hvězdná mapa
- Izar-mapa
- Karta kabintun-on
- mahere whetū
- mapa nieba
- Nebeska karta
- Nebovid
- Peta bintang
- Ramani ya nyota
- siart sêr
- star chart
- stela mapo
- sterrenkaart
- stjernekart
- stjärnkarta
- Stärekaart
- Taevakaart
- tähtikartta
- Yulduz xaritalari
- yıldız haritası
- Zvaigžņu karte
- zvezdna karta
- žvaigždėlapis
- ѕвездена карта
- жұлдыз картасы
- Жұлдыздық карталар
- Звездана мапа
- Звездна карта
- йолдызлы күк йөзе картасы
- Йондоҙҙар картаһы
- карта звёздного неба
- карта зоряного неба
- мапа зорнага неба
- աստղային քարտեզագիրք
- מפת כוכבים
- خريطة نجوم
- نقشه آسمان
- 星図
- 星图
- 星圖
- 성도
has_id_wikidata: Q842617
described_by_source:
- '[[../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817]]'
part_of: '[[_Standards/WikiData/WD~star_atlas,11025270]]'
instance_of: '[[_Standards/WikiData/WD~type_of_map,41362222]]'
studied_by: '[[_Standards/WikiData/WD~celestial_cartography,60198411]]'
subclass_of: '[[_Standards/WikiData/WD~map,4006]]'
Commons_category: Celestial maps
image: http://commons.wikimedia.org/wiki/Special:FilePath/Planisph%C3%A6ri%20c%C5%93leste.jpg
Iconclass_notation: 49E12
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Star_chart,842617|WD~Star_chart,842617]]'
  - '[[/_Standards/Astronomy/Astrometry/Star_Chart|Star_Chart]]'
  - '[[/_public/Astronomy/Astrometry/Star_Chart.public|Star_Chart.public]]'
  - '[[/_internal/Astronomy/Astrometry/Star_Chart.internal|Star_Chart.internal]]'
  - '[[/_protect/Astronomy/Astrometry/Star_Chart.protect|Star_Chart.protect]]'
  - '[[/_private/Astronomy/Astrometry/Star_Chart.private|Star_Chart.private]]'
  - '[[/_personal/Astronomy/Astrometry/Star_Chart.personal|Star_Chart.personal]]'
  - '[[/_secret/Astronomy/Astrometry/Star_Chart.secret|Star_Chart.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Star_chart,842617|WD~Star_chart,842617]]'
- '[[/_Standards/Astronomy/Astrometry/Star_Chart|Star_Chart]]'
- '[[/_public/Astronomy/Astrometry/Star_Chart.public|Star_Chart.public]]'
- '[[/_internal/Astronomy/Astrometry/Star_Chart.internal|Star_Chart.internal]]'
- '[[/_protect/Astronomy/Astrometry/Star_Chart.protect|Star_Chart.protect]]'
- '[[/_private/Astronomy/Astrometry/Star_Chart.private|Star_Chart.private]]'
- '[[/_personal/Astronomy/Astrometry/Star_Chart.personal|Star_Chart.personal]]'
- '[[/_secret/Astronomy/Astrometry/Star_Chart.secret|Star_Chart.secret]]'
dv_has_:
  name_:
    ar: خريطة نجوم
    ast: Carta estelar
    ba: Йондоҙҙар картаһы
    be_tarask: мапа зорнага неба
    bg: Звездна карта
    ca: carta celeste
    cs: hvězdná mapa
    cy: siart sêr
    de: Himmelskarte
    en: star chart
    eo: stela mapo
    es: carta celeste
    et: Taevakaart
    eu: Izar-mapa
    fa: نقشه آسمان
    fi: tähtikartta
    fr: carte du ciel
    gl: Carta celeste
    he: מפת כוכבים
    hr: Nebovid
    hu: Csillagtérkép
    hy: աստղային քարտեզագիրք
    id: Peta bintang
    it: carta celeste
    ja: 星図
    kk: Жұлдыздық карталар
    kk_cyrl: жұлдыз картасы
    ko: 성도
    lb: Stärekaart
    lt: žvaigždėlapis
    lv: Zvaigžņu karte
    mi: mahere whetū
    mk: ѕвездена карта
    nb: stjernekart
    nl: sterrenkaart
    nn: Stjernekart
    oc: Carta estellara
    pl: mapa nieba
    pt: carta celeste
    ro: Harta cerului
    ru: карта звёздного неба
    sh: Nebeska karta
    sk: hviezdna mapa
    sl: zvezdna karta
    sr: Звездана мапа
    sv: stjärnkarta
    sw: Ramani ya nyota
    tr: yıldız haritası
    tt: йолдызлы күк йөзе картасы
    uk: карта зоряного неба
    uz: Yulduz xaritalari
    vi: Bản đồ sao
    war: Karta kabintun-on
    wuu: 星图
    yue: 星圖
    zh: 星圖
    zh_hans: 星图
    zh_hant: 星圖
    zh_hk: 星圖
---

# [[Star_Chart]] 

#is_/same_as :: [[../../WikiData/WD~Star_chart,842617|WD~Star_chart,842617]]  

## #has_/text_of_/abstract 

> A star chart is a celestial map of the night sky 
> with astronomical objects laid out on a grid system. 
> 
> They are used to identify and locate constellations, stars, nebulae, galaxies, and planets. 
> 
> They have been used for human navigation since time immemorial. 
> Note that a star chart differs from an astronomical catalog, 
> which is a listing or tabulation of astronomical objects for a particular purpose. 
> 
> Tools using a star chart include the astrolabe and planisphere.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Star%20chart) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Astrometry/Star_Chart|Star_Chart]] 

### #is_/same_as :: [[/_public/Astronomy/Astrometry/Star_Chart.public|Star_Chart.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Astrometry/Star_Chart.internal|Star_Chart.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Astrometry/Star_Chart.protect|Star_Chart.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Astrometry/Star_Chart.private|Star_Chart.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Astrometry/Star_Chart.personal|Star_Chart.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Astrometry/Star_Chart.secret|Star_Chart.secret]] 

