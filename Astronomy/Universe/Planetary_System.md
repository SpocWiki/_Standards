---
aliases:
- anagraw amtiwgan
- Blanetesystem
- bolygórendszer
- córas pláinéadach
- gezegenler sistemi
- hệ hành tinh
- kulupu mun
- mbyjajerety
- plaaneetkodde
- planedsistemo
- planeedisüsteem
- planeettakunta
- planehtagoddi
- planeta-sistema
- planetarni sistem
- planetarni sustav
- planetary seestem
- Planetary System
- Planetary_System
- planetenstelsel
- Planetensystem
- Planetestelsel
- planetni sistem
- planetsystem
- planetsystemer
- planetárna sústava
- planetární soustava
- planeʹttkåʹdd
- Planéitesystem
- sistem keplanetan
- sistem planet
- sistem planetar
- sistema planetari
- sistema planetario
- sistema planetariu
- sistema planetário
- sistema planitariu
- sistèma planetari
- solsystem
- system blanedol
- systema planetari
- système planétaire
- sólkerfi
- układ planetarny
- Usoro mbara ala
- veivuravura ivakarau
- πλανητικό σύστημα
- планеталар системасы
- планетарен систем
- планетарна система
- планетарни систем
- планетна система
- планетная сістэма
- планетная система
- плянэтная сыстэма
- մոլորակային համակարգ
- מערכת כוכבי לכת
- سامانه سیارهای
- سيستم ڤلانيت
- سیستمی ھەسارەیی
- نظام كوكبي
- پاندھی پربندھ
- ߖߊ߯ߓߊߟߌ߫ ߞߊ߲ߞߋ
- ग्रहीय मण्डल
- গ্রহ ব্যবস্থা
- கோள் தொகுதி
- ระบบดาวเคราะห์
- ဂြိုဟ်စု
- პლანეტური სისტემა
- 惑星系
- 行星系
- 행성계
has_id_wikidata: Q206717
studied_by: '[[_Standards/WikiData/WD~planetary_science,104499|WD~planetary_science,104499]]'
different_from: '[[_Standards/WikiData/WD~star_system,595871|WD~star_system,595871]]'
part_of: '[[_Standards/WikiData/WD~star_system,595871|WD~star_system,595871]]'
has_part_s_:
- '[[_Standards/WikiData/WD~minor_planet,1022867|WD~minor_planet,1022867]]'
- '[[_Standards/WikiData/WD~star,523|WD~star,523]]'
- '[[_Standards/WikiData/WD~planet,634|WD~planet,634]]'
- '[[_Standards/WikiData/WD~comet,3559|WD~comet,3559]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Infobox_planetary_system,14385354|WD~Template_Infobox_planetary_system,14385354]]'
subclass_of: '[[_Standards/WikiData/WD~gravitationally_bound_system,15053549|WD~gravitationally_bound_system,15053549]]'
instance_of: '[[_Standards/WikiData/WD~astronomical_object_type,17444909|WD~astronomical_object_type,17444909]]'
does_not_have_part: '[[_Standards/WikiData/WD~star,523|WD~star,523]]'
Colon_Classification: B97
DataTrek_identifier: Q102
Dewey_Decimal_Classification: 523.2
image: http://commons.wikimedia.org/wiki/Special:FilePath/Artist%20Concept%20Planetary%20System.jpg
Commons_category: Planetary systems
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Planetary_system,206717|WD~Planetary_system,206717]]'
  - '[[/_Standards/Astronomy/Universe/Planetary_System|Planetary_System]]'
  - '[[/_public/Astronomy/Universe/Planetary_System.public|Planetary_System.public]]'
  - '[[/_internal/Astronomy/Universe/Planetary_System.internal|Planetary_System.internal]]'
  - '[[/_protect/Astronomy/Universe/Planetary_System.protect|Planetary_System.protect]]'
  - '[[/_private/Astronomy/Universe/Planetary_System.private|Planetary_System.private]]'
  - '[[/_personal/Astronomy/Universe/Planetary_System.personal|Planetary_System.personal]]'
  - '[[/_secret/Astronomy/Universe/Planetary_System.secret|Planetary_System.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Planetary_system,206717|WD~Planetary_system,206717]]'
- '[[/_Standards/Astronomy/Universe/Planetary_System|Planetary_System]]'
- '[[/_public/Astronomy/Universe/Planetary_System.public|Planetary_System.public]]'
- '[[/_internal/Astronomy/Universe/Planetary_System.internal|Planetary_System.internal]]'
- '[[/_protect/Astronomy/Universe/Planetary_System.protect|Planetary_System.protect]]'
- '[[/_private/Astronomy/Universe/Planetary_System.private|Planetary_System.private]]'
- '[[/_personal/Astronomy/Universe/Planetary_System.personal|Planetary_System.personal]]'
- '[[/_secret/Astronomy/Universe/Planetary_System.secret|Planetary_System.secret]]'
dv_has_:
  name_:
    ar: نظام كوكبي
    ast: sistema planetariu
    be: планетная сістэма
    be_tarask: плянэтная сыстэма
    bg: планетарна система
    bn: গ্রহ ব্যবস্থা
    bs: Planetarni sistem
    ca: sistema planetari
    ckb: سیستمی ھەسارەیی
    cs: planetární soustava
    cy: system blanedol
    da: planetsystemer
    de: Planetensystem
    el: πλανητικό σύστημα
    en: planetary system
    en_ca: planetary system
    en_gb: planetary system
    eo: planedsistemo
    es: sistema planetario
    et: planeedisüsteem
    eu: planeta-sistema
    fa: سامانه سیارهای
    fi: planeettakunta
    fj: veivuravura ivakarau
    fr: système planétaire
    fy: Planetestelsel
    ga: córas pláinéadach
    gl: sistema planetario
    gn: mbyjajerety
    gsw: Blanetesystem
    he: מערכת כוכבי לכת
    hi: ग्रहीय मण्डल
    hr: planetarni sustav
    hu: bolygórendszer
    hy: մոլորակային համակարգ
    ia: systema planetari
    id: sistem keplanetan
    ig: Usoro mbara ala
    is: sólkerfi
    it: sistema planetario
    ja: 惑星系
    ka: პლანეტური სისტემა
    kab: anagraw amtiwgan
    ko: 행성계
    lb: Planéitesystem
    mk: планетарен систем
    ms: sistem planet
    ms_arab: سيستم ڤلانيت
    my: ဂြိုဟ်စု
    nb: planetsystem
    nl: planetenstelsel
    nn: planetsystem
    nqo: ߖߊ߯ߓߊߟߌ߫ ߞߊ߲ߞߋ
    oc: sistèma planetari
    pl: układ planetarny
    pnb: پاندھی پربندھ
    pt: sistema planetário
    pt_br: sistema planetário
    ro: sistem planetar
    ru: планетная система
    scn: sistema planitariu
    sco: planetary seestem
    se: planehtagoddi
    sk: planetárna sústava
    sl: planetni sistem
    smn: plaaneetkodde
    sms: planeʹttkåʹdd
    sq: sistem planetar
    sr: планетарни систем
    sr_ec: планетарни систем
    sr_el: planetarni sistem
    sv: solsystem
    ta: கோள் தொகுதி
    th: ระบบดาวเคราะห์
    tok: kulupu mun
    tr: gezegenler sistemi
    tt: планеталар системасы
    uk: планетна система
    vi: hệ hành tinh
    wuu: 行星系
    yue: 行星系
    zh: 行星系
    zh_cn: 行星系
    zh_hans: 行星系
    zh_hant: 行星系
    zh_hk: 行星系
    zh_sg: 行星系
    zh_tw: 行星系
---

# [[Planetary_System]] 

#is_/same_as :: [[_Standards/WikiData/WD~Planetary_system,206717]] 

## #has_/text_of_/abstract 

> A planetary system consists of a set of non-stellar bodies 
> which are gravitationally bound to and in orbit of a star or star system. 
> 
> Generally speaking, such systems will include planets, and may include other objects such as dwarf planets, asteroids, natural satellites, meteoroids, comets, planetesimals, and circumstellar disks. The Solar System is an example of a planetary system, in which Earth, seven other planets, and other celestial objects are bound to and revolve around the Sun. The term exoplanetary system is sometimes used in reference to planetary systems other than the Solar System. By convention planetary systems are named after their host, or parent, star, as is the case with the Solar System being named after "Sol" (Latin for sun).
>
> As of 29 July 2025, there are 6,032 confirmed exoplanets in 4,530 planetary systems, with 989 systems having more than one planet. Debris disks are known to be common while other objects are more difficult to observe.
>
> Of particular interest to astrobiology is the habitable zone of planetary systems where planets could have surface liquid water, and thus, the capacity to support Earth-like life.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Planetary%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Universe/Planetary_System|Planetary_System]] 

### #is_/same_as :: [[/_public/Astronomy/Universe/Planetary_System.public|Planetary_System.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Universe/Planetary_System.internal|Planetary_System.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Universe/Planetary_System.protect|Planetary_System.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Universe/Planetary_System.private|Planetary_System.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Universe/Planetary_System.personal|Planetary_System.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Universe/Planetary_System.secret|Planetary_System.secret]] 

