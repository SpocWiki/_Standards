---
aliases:
- Annuz (tasnallunt)
- declinació
- declinación
- declinasion
- Declinason
- declinatie
- Declinatio
- declination
- declinazione
- declinação
- Declinație
- deklinaatio
- deklinace
- deklinacija
- deklinacio
- deklinacja
- deklinasi
- Deklinasie
- deklinasjon
- Deklinasyon
- Deklination
- Deklinatioun
- Deklinazio
- Deklinácia
- deklináció
- Deklinācija
- diallas
- dik açıklık
- déclinaison
- Déklinasi
- gogwyddiad
- Keserongan
- Kääne
- Mbelata
- meyl
- Stjörnubreidd
- Xích vĩ
- απόκλιση
- Авышлык
- деклинација
- Деклинация
- Еңісу
- склонение
- схіленне
- схіленьне
- схилення
- хамшавӣ
- թեքում
- נטייה
- اسمانی پئی لیک
- تنزل
- ميل
- میل
- झुकाव
- डेक्लिनेशन
- বিষুবলম্ব
- சரிவு
- డిక్లనేషన్
- അവനമനം
- เดคลิเนชัน
- დახრილობა
- 赤緯
- 赤纬
- 적위
has_id_wikidata: Q76287
part_of: '[[_Standards/WikiData/WD~equatorial_coordinate_system,208617]]'
described_by_source:
- '[[../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- "[[_Standards/WikiData/WD~Otto's_encyclopedia,2041543]]"
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Desktop_Encyclopedic_Dictionary,63284758]]'
subclass_of: '[[_Standards/WikiData/WD~angular_measure,1357788]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_DEC,5652918]]'
Wikidata_property: declination
opposite_of: '[[_Standards/WikiData/WD~right_ascension,13442]]'
lower_limit: -90
upper_limit: 90
ISQ_dimension: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mn>1</mn></mstyle></mrow></math>
quantity_symbol_LaTeX_: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>&#x03B4;</mi></mstyle></mrow></math>
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Declination,76287|WD~Declination,76287]]'
  - '[[/_Standards/Astronomy/Astronomical_Coordinate_System/Declination|Declination]]'
  - '[[/_public/Astronomy/Astronomical_Coordinate_System/Declination.public|Declination.public]]'
  - '[[/_internal/Astronomy/Astronomical_Coordinate_System/Declination.internal|Declination.internal]]'
  - '[[/_protect/Astronomy/Astronomical_Coordinate_System/Declination.protect|Declination.protect]]'
  - '[[/_private/Astronomy/Astronomical_Coordinate_System/Declination.private|Declination.private]]'
  - '[[/_personal/Astronomy/Astronomical_Coordinate_System/Declination.personal|Declination.personal]]'
  - '[[/_secret/Astronomy/Astronomical_Coordinate_System/Declination.secret|Declination.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Declination,76287|WD~Declination,76287]]'
- '[[/_Standards/Astronomy/Astronomical_Coordinate_System/Declination|Declination]]'
- '[[/_public/Astronomy/Astronomical_Coordinate_System/Declination.public|Declination.public]]'
- '[[/_internal/Astronomy/Astronomical_Coordinate_System/Declination.internal|Declination.internal]]'
- '[[/_protect/Astronomy/Astronomical_Coordinate_System/Declination.protect|Declination.protect]]'
- '[[/_private/Astronomy/Astronomical_Coordinate_System/Declination.private|Declination.private]]'
- '[[/_personal/Astronomy/Astronomical_Coordinate_System/Declination.personal|Declination.personal]]'
- '[[/_secret/Astronomy/Astronomical_Coordinate_System/Declination.secret|Declination.secret]]'
dv_has_:
  name_:
    af: Deklinasie
    ar: ميل
    ast: Declinación
    az: meyl
    be: схіленне
    be_tarask: схіленьне
    bg: Деклинация
    bn: বিষুবলম্ব
    bs: Deklinacija
    ca: declinació
    cs: deklinace
    cy: gogwyddiad
    da: Deklination
    de: Deklination
    el: απόκλιση
    en: declination
    eo: deklinacio
    es: declinación
    et: Kääne
    eu: Deklinazio
    fa: میل
    fi: deklinaatio
    fr: déclinaison
    frr: deklination
    ga: diallas
    gl: Declinación
    he: נטייה
    hi: झुकाव
    hr: Deklinacija
    hu: deklináció
    hy: թեքում
    id: deklinasi
    ig: Mbelata
    is: Stjörnubreidd
    it: declinazione
    ja: 赤緯
    jv: Déklinasi
    ka: დახრილობა
    kab: Annuz (tasnallunt)
    kk: Еңісу
    ko: 적위
    la: Declinatio
    lb: Deklinatioun
    lt: deklinacija
    lv: Deklinācija
    mk: деклинација
    ml: അവനമനം
    mr: डेक्लिनेशन
    ms: Keserongan
    nb: deklinasjon
    nl: declinatie
    nn: deklinasjon
    oc: Declinason
    pl: deklinacja
    pnb: اسمانی پئی لیک
    pt: declinação
    ro: Declinație
    ru: склонение
    sh: Deklinacija
    sk: Deklinácia
    sl: deklinacija
    sr: деклинација
    sv: deklination
    ta: சரிவு
    te: డిక్లనేషన్
    tg: хамшавӣ
    th: เดคลิเนชัน
    tl: Deklinasyon
    tr: dik açıklık
    tt: Авышлык
    uk: схилення
    ur: تنزل
    vec: declinasion
    vi: Xích vĩ
    wuu: 赤纬
    yue: 赤緯
    zh: 赤纬
---

# [[Declination]] 

#is_/same_as :: [[../../WikiData/WD~Declination,76287|WD~Declination,76287]] 
#is_/similar_to :: 

## #has_/text_of_/abstract 

> In astronomy, **Declination** (abbreviated dec; symbol δ) is one of the two angles 
> that locate a point on the celestial sphere in the equatorial coordinate system, 
> the other being hour angle. 
> 
> The declination angle is measured north (positive) or south (negative) of the celestial equator, 
> along the hour circle passing through the point in question. 
>
> The root of the word declination (Latin, declinatio) means "a bending away" or "a bending down". 
> It comes from the same root as the words incline ("bend forward") and recline ("bend backward").
>
> In some 18th and 19th century astronomical texts, declination is given as North Pole Distance (N.P.D.), 
> which is equivalent to 90 – (declination). 
> For instance an object marked as declination -5 would have an N.P.D. of 95, 
> and a declination of -90 (the south celestial pole) would have an N.P.D. of 180.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Declination) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Astronomical_Coordinate_System/Declination|Declination]] 

### #is_/same_as :: [[/_public/Astronomy/Astronomical_Coordinate_System/Declination.public|Declination.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Astronomical_Coordinate_System/Declination.internal|Declination.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Astronomical_Coordinate_System/Declination.protect|Declination.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Astronomical_Coordinate_System/Declination.private|Declination.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Astronomical_Coordinate_System/Declination.personal|Declination.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Astronomical_Coordinate_System/Declination.secret|Declination.secret]] 

