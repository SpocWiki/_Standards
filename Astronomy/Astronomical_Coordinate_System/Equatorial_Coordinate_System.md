---
aliases:
- Astronomiska koordinater
- coordenadas ecuatoriales
- coordenades equatorials
- Coordenaes ecuatoriales
- Ekuatore-koordenatuak
- Ekvaatorilised koordinaadid
- ekvaattorijärjestelmä
- ekvator koordinat sistemi
- ekvatora koordinatsistemo
- Ekvatorial koordinatalar tizimi
- ekvatorialkoordinater
- ekvatoriális koordináta-rendszer
- ekvatorski koordinatni sistem
- Ekvatorsystemet
- Equatoriaalt Koordinatesystem
- equatorial coordinate system
- equatoriale coördinatenstelsel
- Equatorial_Coordinate_System
- hệ tọa độ xích đạo
- Miðbaugshnit
- Nebeski ekvatorijalni koordinatni sistem
- Nebeski ekvatorski koordinatni sustav
- pusiaujinė koordinačių sistema
- Rovníková sústava súradníc
- rovníkové souřadnice
- sistem de coordonate ecuatoriale
- Sistem koordinat ekuator
- Sistem koordinat khatulistiwa
- sistema di coordinate equatoriali
- sistema equatorial de coordenadas
- sistemang ekwatoryal ng mga koordinado
- système de coordonnées équatoriales
- układ współrzędnych równikowych
- Usoro nhazi nke equatorial
- äquatoriales Koordinatensystem
- екваторіальна система координат
- Екваториална координатна система
- Екваторски координатен систем
- небески екваторијални координатни систем
- Першая экватарыяльная сістэма каардынат
- экватарыяльная сыстэма каардынатаў
- экваториальная система координат
- Հասարակածային կոորդինատային համակարգ
- מערכת הקואורדינטות המשוונית
- استوائی متناسق نظام
- دستگاه مختصات استوایی
- نظام الإحداثيات الاستوائية
- भूमध्यीय निर्देशांक प्रणाली
- భూమధ్యరేఖీయ నిర్దేశాంక వ్యవస్థ
- ระบบพิกัดศูนย์สูตร
- 赤道坐標系統
- 赤道座标系统
- 赤道座標
- 赤道座標系統
- 적도좌표계
has_id_wikidata: Q208617
subclass_of: '[[_Standards/WikiData/WD~astronomical_coordinate_system,86394|WD~astronomical_coordinate_system,86394]]'
different_from:
- '[[_Standards/WikiData/WD~first_equatorial_coordinate_system,9364775|WD~first_equatorial_coordinate_system,9364775]]'
- '[[_Standards/WikiData/WD~second_equatorial_coordinate_system,9364777|WD~second_equatorial_coordinate_system,9364777]]'
described_by_source:
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~Small_Soviet_Encyclopedia,66386517|WD~Small_Soviet_Encyclopedia,66386517]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q143%20%28epo%29-Nealmcb-ekvatora%20koordinatsistemo.wav
Commons_category: Equatorial coordinate system
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Equatorial_coordinate_system,208617|WD~Equatorial_coordinate_system,208617]]'
  - '[[/_Standards/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System|Equatorial_Coordinate_System]]'
  - '[[/_public/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.public|Equatorial_Coordinate_System.public]]'
  - '[[/_internal/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.internal|Equatorial_Coordinate_System.internal]]'
  - '[[/_protect/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.protect|Equatorial_Coordinate_System.protect]]'
  - '[[/_private/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.private|Equatorial_Coordinate_System.private]]'
  - '[[/_personal/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.personal|Equatorial_Coordinate_System.personal]]'
  - '[[/_secret/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.secret|Equatorial_Coordinate_System.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Equatorial_coordinate_system,208617|WD~Equatorial_coordinate_system,208617]]'
- '[[/_Standards/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System|Equatorial_Coordinate_System]]'
- '[[/_public/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.public|Equatorial_Coordinate_System.public]]'
- '[[/_internal/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.internal|Equatorial_Coordinate_System.internal]]'
- '[[/_protect/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.protect|Equatorial_Coordinate_System.protect]]'
- '[[/_private/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.private|Equatorial_Coordinate_System.private]]'
- '[[/_personal/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.personal|Equatorial_Coordinate_System.personal]]'
- '[[/_secret/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.secret|Equatorial_Coordinate_System.secret]]'
dv_has_:
  name_:
    ar: نظام الإحداثيات الاستوائية
    ast: Coordenaes ecuatoriales
    be: Першая экватарыяльная сістэма каардынат
    be_tarask: экватарыяльная сыстэма каардынатаў
    bg: Екваториална координатна система
    ca: coordenades equatorials
    cs: rovníkové souřadnice
    de: äquatoriales Koordinatensystem
    de_ch: Äquatoriales Koordinatensystem
    en: equatorial coordinate system
    en_ca: Equatorial coordinate system
    en_gb: Equatorial coordinate system
    eo: ekvatora koordinatsistemo
    es: coordenadas ecuatoriales
    et: Ekvaatorilised koordinaadid
    eu: Ekuatore-koordenatuak
    fa: دستگاه مختصات استوایی
    fi: ekvaattorijärjestelmä
    fr: système de coordonnées équatoriales
    he: מערכת הקואורדינטות המשוונית
    hi: भूमध्यीय निर्देशांक प्रणाली
    hr: Nebeski ekvatorski koordinatni sustav
    hu: ekvatoriális koordináta-rendszer
    hy: Հասարակածային կոորդինատային համակարգ
    id: Sistem koordinat ekuator
    ig: Usoro nhazi nke equatorial
    is: Miðbaugshnit
    it: sistema di coordinate equatoriali
    ja: 赤道座標
    ko: 적도좌표계
    lb: Equatoriaalt Koordinatesystem
    lt: pusiaujinė koordinačių sistema
    mk: Екваторски координатен систем
    ms: Sistem koordinat khatulistiwa
    nb: ekvatorialkoordinater
    nl: equatoriale coördinatenstelsel
    nn: Ekvatorsystemet
    pl: układ współrzędnych równikowych
    pt: sistema equatorial de coordenadas
    pt_br: Sistema equatorial de coordenadas
    ro: sistem de coordonate ecuatoriale
    ru: экваториальная система координат
    sh: Nebeski ekvatorijalni koordinatni sistem
    sk: Rovníková sústava súradníc
    sl: ekvatorski koordinatni sistem
    sr: небески екваторијални координатни систем
    sr_ec: Небески екваторијални координатни систем
    sr_el: Nebeski ekvatorijalni koordinatni sistem
    sv: Astronomiska koordinater
    te: భూమధ్యరేఖీయ నిర్దేశాంక వ్యవస్థ
    th: ระบบพิกัดศูนย์สูตร
    tl: sistemang ekwatoryal ng mga koordinado
    tr: ekvator koordinat sistemi
    uk: екваторіальна система координат
    ur: استوائی متناسق نظام
    uz: Ekvatorial koordinatalar tizimi
    vi: hệ tọa độ xích đạo
    yue: 赤道坐標系統
    zh: 赤道座標系統
    zh_cn: 赤道座标系统
    zh_hans: 赤道座标系统
    zh_hant: 赤道座標系統
    zh_hk: 赤道座標系統
    zh_sg: 赤道座标系统
    zh_tw: 赤道座標系統
---

# [[Equatorial_Coordinate_System]] 

#is_/same_as :: [[../../WikiData/WD~Equatorial_coordinate_system,208617|WD~Equatorial_coordinate_system,208617]] 

## #has_/text_of_/abstract 

> The equatorial coordinate system is a celestial coordinate system 
> widely used to specify the positions of celestial objects. 
> 
> It may be implemented in spherical or rectangular coordinates, both defined by 
> - an origin at the centre of Earth, 
> - a fundamental plane consisting of the projection of Earth's equator onto the celestial sphere 
>   (forming the celestial equator), 
> - a fixed primary direction towards the March equinox, and 
> - a right-handed convention.
>
> The origin at the centre of Earth means the coordinates are geocentric, 
> that is, as seen from the centre of Earth as if it were transparent. 
> 
> The fundamental plane and the fixed primary direction mean that the coordinate system, 
> while aligned with Earth's equator and pole, **does not rotate with the Earth**, 
> but remains relatively fixed against the background stars. 
> 
> A right-handed convention means that coordinates increase northward from 
> and eastward around the fundamental plane.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Equatorial%20coordinate%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System|Equatorial_Coordinate_System]] 

### #is_/same_as :: [[/_public/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.public|Equatorial_Coordinate_System.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.internal|Equatorial_Coordinate_System.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.protect|Equatorial_Coordinate_System.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.private|Equatorial_Coordinate_System.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.personal|Equatorial_Coordinate_System.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Astronomical_Coordinate_System/Equatorial_Coordinate_System.secret|Equatorial_Coordinate_System.secret]] 

