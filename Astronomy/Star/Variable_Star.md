---
aliases:
- bintang berubah
- bintang variabel
- değişen yıldız
- dəyişən ulduz
- estareyo vurneyer
- estela variabla
- estrela variável
- estrella variable
- gwiazda zmienna
- izar aldakor
- kintamoji žvaigždė
- maiņzvaigzne
- muutlik täht
- muuttuva tähti
- nyota geugeu
- oʻzgaruvchan yulduzlar
- premenná hviezda
- promenljiva zvezda
- promjenljiva zvijezda
- proměnná hvězda
- réalta athraitheach
- sao biến quang
- seren newidiol
- spremenljivka
- stea variabilă
- stella variabile
- stella variabilis
- steła variàbiłe
- varia stelo
- variabel stjerne
- variabel stjärna
- variable star
- variabli stele
- varianta stelo
- veranderlijke ster
- veranderlike ster
- veränderlicher Stern
- Verännerlech Stären
- változócsillag
- étoile variable
- μεταβλητος αστέρας
- айнымалы жұлдыз
- алмаш йолдыз
- вошъяськись кизили
- змінна зоря
- зьменная зорка
- ивгæ стъалы
- пераменная зорка
- переменная звезда
- променљива звезда
- променлива ѕвезда
- променлива звезда
- ситораи мутағйир
- улшăнуллă çăлтăр
- өзгөрмө жылдыз
- փոփոխական աստղեր
- כוכב משתנה
- ئەستێرەی گۆڕاو
- ستاره متغیر
- نجم متغير
- परिवर्ती तारा
- বিষমতারা
- மாறுபடும் விண்மீன்
- ചരനക്ഷത്രം
- ดาวแปรแสง
- მათირუ მურიცხი
- 变星
- 后髮座FK型變星
- 変光星
- 變星
- 변광성
has_id_wikidata: Q6243
described_by_source:
- '[[../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
has_characteristic: '[[_Standards/WikiData/WD~change,1150070]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Variable_star_topics,14337390]]'
instance_of: '[[_Standards/WikiData/WD~astronomical_object_type,17444909]]'
subclass_of: '[[_Standards/WikiData/WD~star,523]]'
main_Wikidata_property: type of variable star
Commons_category: Variable stars
image: http://commons.wikimedia.org/wiki/Special:FilePath/Eso2003c.jpg
video: http://commons.wikimedia.org/wiki/Special:FilePath/Evolution%20of%20the%20light%20echo%20around%20V838%20Monocerotis%20%28Heic0617a%29.ogv
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q188%20%28deu%29-Michael%20Schoenitzer%20%28MichaelSchoenitzer%29-ver%C3%A4nderlicher%20Stern.wav
Krugosvet_article_archived_: nauka_i_tehnika/astronomiya/PEREMENNIE_ZVEZDI.html
Stack_Exchange_tag: https://astronomy.stackexchange.com/tags/variable-star
Dewey_Decimal_Classification: 523.844
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Variable_star,6243|WD~Variable_star,6243]]'
  - '[[/_Standards/Astronomy/Star/Variable_Star|Variable_Star]]'
  - '[[/_public/Astronomy/Star/Variable_Star.public|Variable_Star.public]]'
  - '[[/_internal/Astronomy/Star/Variable_Star.internal|Variable_Star.internal]]'
  - '[[/_protect/Astronomy/Star/Variable_Star.protect|Variable_Star.protect]]'
  - '[[/_private/Astronomy/Star/Variable_Star.private|Variable_Star.private]]'
  - '[[/_personal/Astronomy/Star/Variable_Star.personal|Variable_Star.personal]]'
  - '[[/_secret/Astronomy/Star/Variable_Star.secret|Variable_Star.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Variable_star,6243|WD~Variable_star,6243]]'
- '[[/_Standards/Astronomy/Star/Variable_Star|Variable_Star]]'
- '[[/_public/Astronomy/Star/Variable_Star.public|Variable_Star.public]]'
- '[[/_internal/Astronomy/Star/Variable_Star.internal|Variable_Star.internal]]'
- '[[/_protect/Astronomy/Star/Variable_Star.protect|Variable_Star.protect]]'
- '[[/_private/Astronomy/Star/Variable_Star.private|Variable_Star.private]]'
- '[[/_personal/Astronomy/Star/Variable_Star.personal|Variable_Star.personal]]'
- '[[/_secret/Astronomy/Star/Variable_Star.secret|Variable_Star.secret]]'
dv_has_:
  name_:
    af: veranderlike ster
    ar: نجم متغير
    ast: estrella variable
    az: dəyişən ulduz
    be: пераменная зорка
    be_tarask: зьменная зорка
    bg: променлива звезда
    bn: বিষমতারা
    bs: promjenljiva zvijezda
    ca: estrella variable
    ckb: ئەستێرەی گۆڕاو
    cs: proměnná hvězda
    cv: улшăнуллă çăлтăр
    cy: seren newidiol
    da: variabel stjerne
    de: veränderlicher Stern
    diq: estareyo vurneyer
    el: μεταβλητος αστέρας
    en: variable star
    en_ca: variable star
    en_gb: variable star
    eo: varia stelo
    es: estrella variable
    et: muutlik täht
    eu: izar aldakor
    fa: ستاره متغیر
    fi: muuttuva tähti
    fr: étoile variable
    ga: réalta athraitheach
    he: כוכב משתנה
    hi: परिवर्ती तारा
    hr: promjenljiva zvijezda
    hu: változócsillag
    hy: փոփոխական աստղեր
    ia: stella variabile
    id: bintang variabel
    io: varianta stelo
    it: stella variabile
    ja: 変光星
    kk: айнымалы жұлдыз
    ko: 변광성
    ky: өзгөрмө жылдыз
    la: stella variabilis
    lb: Verännerlech Stären
    lt: kintamoji žvaigždė
    lv: maiņzvaigzne
    mk: променлива ѕвезда
    ml: ചരനക്ഷത്രം
    ms: bintang berubah
    nb: variabel stjerne
    nl: veranderlijke ster
    nn: variabel stjerne
    nov: variabli stele
    oc: estela variabla
    os: ивгæ стъалы
    pl: gwiazda zmienna
    pt: estrela variável
    pt_br: estrela variável
    ro: stea variabilă
    ru: переменная звезда
    sh: promenljiva zvezda
    sk: premenná hviezda
    sl: spremenljivka
    sr: променљива звезда
    sr_ec: променљива звезда
    sr_el: promenljiva zvezda
    sv: variabel stjärna
    sw: nyota geugeu
    ta: மாறுபடும் விண்மீன்
    tg: ситораи мутағйир
    th: ดาวแปรแสง
    tr: değişen yıldız
    tt: алмаш йолдыз
    udm: вошъяськись кизили
    uk: змінна зоря
    uz: oʻzgaruvchan yulduzlar
    vec: steła variàbiłe
    vi: sao biến quang
    wuu: 变星
    xmf: მათირუ მურიცხი
    yue: 變星
    zh: 變星
    zh_hans: 后髮座FK型變星
    zh_hant: 變星
---
# [[Variable_Star]] 

#is_/same_as :: [[../../WikiData/WD~Variable_star,6243|WD~Variable_star,6243]]  

## #has_/text_of_/abstract 

> A variable star is a star whose brightness as seen from Earth 
> (its apparent magnitude) changes systematically with time. 
> 
> This variation may be caused by a change in emitted light 
> or by something partly blocking the light, so variable stars are classified as either:
> - Intrinsic variables, whose inherent luminosity changes; for example, because the star swells and shrinks.
> - Extrinsic variables, whose apparent changes in brightness are due to changes in the amount of their light that can reach Earth; 
>   for example, because the star has an orbiting companion that sometimes eclipses it.
>
> Depending on the type of star system, this variation can include cyclical, irregular, fluctuating, or transient behavior. 
> 
> Changes can occur on time scales that range from under an hour to multiple years. 
> Many, possibly most, stars exhibit at least some oscillation in luminosity: 
> the energy output of the Sun, for example, varies by about 0.1% over an 11-year solar cycle. 
> 
> At the opposite extreme, a supernova event can briefly outshine an entire galaxy. 
> Of the 58,200 variable stars that have been catalogued as of 2023, 
> the most common type are pulsating variables with just under 30,000, 
> followed by eclipsing variables with over 10,000.
>
> Variable stars have been observed since the dawn of human history. The first documented periodic variable was the eclipsing binary Algol. The periodic variable Omicron Ceti, later named Mira, was discovered in the 17th century, followed by Chi Cygni then R Hydrae. By 1786, ten had been documented. Variable star discovery increased rapidly with the advent of photographic plates. When Cepheid variables were shown to have a period-luminosity relationship in 1912, this allowed them to be used for distance measurement. As a result, it was demonstrated that spiral nebulae are galaxies outside the Milky Way. Variable stars now form several methods for the cosmic distance ladder that is used to determine the scale of the visible universe. The periods of eclipsing binaries allowed for a more precise determination of the mass and radii of their component stars, which proved especially useful for modelling stellar evolution.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Variable%20star) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Star/Variable_Star|Variable_Star]] 

### #is_/same_as :: [[/_public/Astronomy/Star/Variable_Star.public|Variable_Star.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Star/Variable_Star.internal|Variable_Star.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Star/Variable_Star.protect|Variable_Star.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Star/Variable_Star.private|Variable_Star.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Star/Variable_Star.personal|Variable_Star.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Star/Variable_Star.secret|Variable_Star.secret]] 

