---
aliases:
- Binarii sdar
- Binary Star
- binary star system
- Binary_Star
- Bintang biner
- Bintang ganda
- Béntang ganda
- dobbeltstjerne
- dobelstäär
- Doppelstern
- Dubbelsteern
- dubbelster
- dubbelstjärna
- Duebelstär
- duopa stelo
- Duopla stelo
- duopli stele
- dvinarė žvaigždė
- Dvojhviezda
- dvojhvězda
- dvojna zvezda
- Dvojna zvijezda
- Dvojne i višestruke zvijezde
- dvostruka zvijezda
- déréalta
- Estela doble
- estrela binaria
- estrela binária
- estrella binaria
- estrella binària
- fizikālā dubultzvaigzne
- fizična dvojna zvezda
- fysisk dobbelstjerne
- fysisk dobbeltstjerne
- gwiazda podwójna
- Izar bitar
- Kaksiktäht
- kaksoistähti
- kettőscsillag
- Lintang gandha
- Nyota pacha
- physischer Doppelstern
- qoşa ulduz
- Qoʻshaloq yulduzlar
- Sao đôi
- seren ddwbl
- stea binară
- stella binaria
- Tvístirni
- çift yıldız
- étoile binaire
- Étwal binè
- διπλός αστέρας
- двојна ѕвезда
- двојна звезда
- Двойна звезда
- двойная звезда
- дывæр стъалы
- Иккĕлле çăлтăр
- Кош жылдыз
- Куш йолдыз
- падвойная зорка
- падвойныя зоркі
- подвійна зоря
- Қос жұлдыз
- կրկնակի աստղ
- כוכב כפול
- دوہرا تارہ
- ستاره دوتایی
- نجم ثنائي
- द्वितारा
- যুগ্ম তারা
- ਦੋਹਰਾ ਤਾਰਾ
- દ્વિસંગી તારો
- இரட்டை விண்மீன்
- ಅವಳಿ ನಕ್ಷತ್ರಗಳು
- ദ്വന്ദ്വനക്ഷത്രം
- ระบบดาวคู่
- ဒွိကြယ်
- ორჯერადი ვარსკვლავი
- ჟირმურიცხი
- 联星
- 聯星
- 連星
- 雙子星
- 쌍성
has_id_wikidata: Q50053
subclass_of:
- '[[_Standards/WikiData/WD~star_system,595871|WD~star_system,595871]]'
- '[[_Standards/WikiData/WD~binary_system,978297|WD~binary_system,978297]]'
- '[[_Standards/WikiData/WD~double_star,13890|WD~double_star,13890]]'
different_from:
- '[[_Standards/WikiData/WD~optical_double,3037794|WD~optical_double,3037794]]'
- '[[_Standards/WikiData/WD~double_star,13890|WD~double_star,13890]]'
instance_of: '[[_Standards/WikiData/WD~astronomical_object_type,17444909|WD~astronomical_object_type,17444909]]'
described_by_source:
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632|WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632]]'
topic_s_main_template: '[[_Standards/WikiData/WD~Template_Infobox_double_star,25812973|WD~Template_Infobox_double_star,25812973]]'
has_part_s_: '[[_Standards/WikiData/WD~star,523|WD~star,523]]'
OmegaWiki_Defined_Meaning: 430493
Commons_category: Binary stars
video: http://commons.wikimedia.org/wiki/Special:FilePath/Artist%27s%20impression%20of%20the%20evolution%20of%20a%20hot%20high-mass%20binary%20star.ogv
image: http://commons.wikimedia.org/wiki/Special:FilePath/Eclipsing%20binary%20star%20animation%203.gif
P8189: 987007531742305200
Commons_gallery: Binary Star
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Binary_star,50053|WD~Binary_star,50053]]'
  - '[[/_Standards/Astronomy/Star/Binary_Star|Binary_Star]]'
  - '[[/_public/Astronomy/Star/Binary_Star.public|Binary_Star.public]]'
  - '[[/_internal/Astronomy/Star/Binary_Star.internal|Binary_Star.internal]]'
  - '[[/_protect/Astronomy/Star/Binary_Star.protect|Binary_Star.protect]]'
  - '[[/_private/Astronomy/Star/Binary_Star.private|Binary_Star.private]]'
  - '[[/_personal/Astronomy/Star/Binary_Star.personal|Binary_Star.personal]]'
  - '[[/_secret/Astronomy/Star/Binary_Star.secret|Binary_Star.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Binary_star,50053|WD~Binary_star,50053]]'
- '[[/_Standards/Astronomy/Star/Binary_Star|Binary_Star]]'
- '[[/_public/Astronomy/Star/Binary_Star.public|Binary_Star.public]]'
- '[[/_internal/Astronomy/Star/Binary_Star.internal|Binary_Star.internal]]'
- '[[/_protect/Astronomy/Star/Binary_Star.protect|Binary_Star.protect]]'
- '[[/_private/Astronomy/Star/Binary_Star.private|Binary_Star.private]]'
- '[[/_personal/Astronomy/Star/Binary_Star.personal|Binary_Star.personal]]'
- '[[/_secret/Astronomy/Star/Binary_Star.secret|Binary_Star.secret]]'
dv_has_:
  name_:
    af: dubbelster
    ar: نجم ثنائي
    ast: estrella binaria
    az: qoşa ulduz
    be: падвойныя зоркі
    be_tarask: падвойная зорка
    bg: Двойна звезда
    bn: যুগ্ম তারা
    bs: dvostruka zvijezda
    ca: estrella binària
    cs: dvojhvězda
    cv: Иккĕлле çăлтăр
    cy: seren ddwbl
    da: dobbeltstjerne
    de: physischer Doppelstern
    de_ch: Doppelstern
    el: διπλός αστέρας
    en: binary star
    en_ca: Binary star
    en_gb: binary star
    en-us: binary star
    eo: duopa stelo
    es: estrella binaria
    et: Kaksiktäht
    eu: Izar bitar
    fa: ستاره دوتایی
    fi: kaksoistähti
    fr: étoile binaire
    frr: dobelstäär
    ga: déréalta
    gl: estrela binaria
    gu: દ્વિસંગી તારો
    he: כוכב כפול
    hi: द्वितारा
    hr: Dvojna zvijezda
    ht: Étwal binè
    hu: kettőscsillag
    hy: կրկնակի աստղ
    id: Bintang biner
    io: Duopla stelo
    is: Tvístirni
    it: stella binaria
    ja: 連星
    jv: Lintang gandha
    ka: ორჯერადი ვარსკვლავი
    kk: Қос жұлдыз
    kn: ಅವಳಿ ನಕ್ಷತ್ರಗಳು
    ko: 쌍성
    ky: Кош жылдыз
    la: Stella binaria
    lb: Duebelstär
    lt: dvinarė žvaigždė
    lv: fizikālā dubultzvaigzne
    mk: двојна ѕвезда
    ml: ദ്വന്ദ്വനക്ഷത്രം
    ms: Bintang ganda
    my: ဒွိကြယ်
    nb: fysisk dobbeltstjerne
    nds: Dubbelsteern
    nl: dubbelster
    nn: fysisk dobbelstjerne
    nov: duopli stele
    oc: Estela doble
    os: дывæр стъалы
    pa: ਦੋਹਰਾ ਤਾਰਾ
    pih: Binarii sdar
    pl: gwiazda podwójna
    pnb: دوہرا تارہ
    pt: estrela binária
    pt_br: estrela binária
    ro: stea binară
    ru: двойная звезда
    sh: Dvojne i višestruke zvijezde
    sk: Dvojhviezda
    sl: fizična dvojna zvezda
    sr: двојна звезда
    sr_ec: двојна звезда
    sr_el: dvojna zvezda
    su: Béntang ganda
    sv: dubbelstjärna
    sw: Nyota pacha
    ta: இரட்டை விண்மீன்
    th: ระบบดาวคู่
    tr: çift yıldız
    tt: Куш йолдыз
    uk: подвійна зоря
    uz: Qoʻshaloq yulduzlar
    vi: Sao đôi
    wuu: 联星
    xmf: ჟირმურიცხი
    yue: 雙子星
    zh: 聯星
---

# [[Binary_Star]] 

#is_/same_as :: [[../../WikiData/WD~Binary_star,50053|WD~Binary_star,50053]] 

## #has_/text_of_/abstract 

> A binary star or binary star system is a system of two stars 
> that are gravitationally bound to and in orbit around each other. 
> 
> Binary stars in the night sky that are seen as a single object to the naked eye 
> are often resolved as separate stars using a telescope, 
> in which case they are called visual binaries. 
> 
> Many visual binaries have long orbital periods of several centuries or millennia 
> and therefore have orbits which are uncertain or poorly known. 
> They may also be detected by indirect techniques, 
> such as spectroscopy (spectroscopic binaries) or astrometry (astrometric binaries). 
> 
> If a binary star happens to orbit in a plane along our line of sight, 
> its components will eclipse and transit each other; these pairs are called eclipsing binaries, 
> or, together with other binaries that change brightness as they orbit, photometric binaries.
>
> If components in binary star systems are close enough, they can gravitationally distort each other's outer stellar atmospheres. In some cases, these close binary systems can exchange mass, which may bring their evolution to stages that single stars cannot attain. Examples of binaries are Sirius, and Cygnus X-1 (Cygnus X-1 being a well-known black hole). Binary stars are also common as the nuclei of many planetary nebulae, and are the progenitors of both novae and type Ia supernovae.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Binary%20star) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Star/Binary_Star|Binary_Star]] 

### #is_/same_as :: [[/_public/Astronomy/Star/Binary_Star.public|Binary_Star.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Star/Binary_Star.internal|Binary_Star.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Star/Binary_Star.protect|Binary_Star.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Star/Binary_Star.private|Binary_Star.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Star/Binary_Star.personal|Binary_Star.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Star/Binary_Star.secret|Binary_Star.secret]] 

