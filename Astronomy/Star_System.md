---
aliases:
- anagraw itriklan
- csillagrendszer
- córas réaltaí
- hviezdna sústava
- hvězdná soustava
- hệ sao
- izar-sistema
- kulupu pi mun seli
- sistem bintang
- sistem lintang
- sistem najam
- sistem stelar
- sistema d'estrelles
- sistema estelar
- sistema stellare
- sistemi yjor
- star system
- starn seestem
- stelarni sistem
- stelosistemo
- Sternsystem
- stersysteem
- stjernesystem
- stjärnsystem
- Stäresystem
- system serol
- système stellaire
- sólkerfi
- tähesüsteem
- tähtijärjestelmä
- układ gwiezdny
- vairākkārtīga zvaigžņu sistēma
- veelvoudige ster
- yulduz tizimi
- yıldız sistemi
- zvezdani sistem
- zvezdni sistem
- zvjezdani sistem
- zvjezdani sustav
- žvaigždžių sistema
- αστρικό σύστημα
- ѕвезден систем
- жұлдыздар жүйесі
- звёздная система
- звездани систем
- звездна система
- зорная сістэма
- зорная сыстэма
- зоряна система
- йолдыз системасы
- աստղային համակարգ
- מערכת מרובת כוכבים
- تارکَن ہُنٛد نِظام
- تارہ پربندھ
- سامانه ستارهای
- نظام نجمي
- तारा प्रणाली
- ระบบดาว
- 恆星系統
- 恒星系
- 恒星系统
- 항성계
has_id_wikidata: Q595871
different_from: '[[_Standards/WikiData/WD~planetary_system,206717|WD~planetary_system,206717]]'
topic_has_template:
- '[[_Standards/WikiData/WD~Template_Infobox_star,6259679|WD~Template_Infobox_star,6259679]]'
- '[[_Standards/WikiData/WD~Template_Stellar_system,14336000|WD~Template_Stellar_system,14336000]]'
subclass_of:
- '[[_Standards/WikiData/WD~gravitationally_bound_system,15053549|WD~gravitationally_bound_system,15053549]]'
- '[[_Standards/WikiData/WD~astronomical_object,6999|WD~astronomical_object,6999]]'
instance_of: '[[_Standards/WikiData/WD~astronomical_object_type,17444909|WD~astronomical_object_type,17444909]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817|WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817]]'
part_of: '[[_Standards/WikiData/WD~galaxy,318|WD~galaxy,318]]'
has_part_s_: '[[_Standards/WikiData/WD~star,523|WD~star,523]]'
child_astronomical_body: '[[_Standards/WikiData/WD~planet,634|WD~planet,634]]'
Commons_category: Multiple star systems
image: http://commons.wikimedia.org/wiki/Special:FilePath/Sirius%20A%20and%20B%20Hubble%20photo.jpg
Dewey_Decimal_Classification: 523.841
dv_is_:
  same_as:
  - '[[../WikiData/WD~Star_system,595871|WD~Star_system,595871]]'
  - '[[/_Standards/Astronomy/Star_System|Star_System]]'
  - '[[/_public/Astronomy/Star_System.public|Star_System.public]]'
  - '[[/_internal/Astronomy/Star_System.internal|Star_System.internal]]'
  - '[[/_protect/Astronomy/Star_System.protect|Star_System.protect]]'
  - '[[/_private/Astronomy/Star_System.private|Star_System.private]]'
  - '[[/_personal/Astronomy/Star_System.personal|Star_System.personal]]'
  - '[[/_secret/Astronomy/Star_System.secret|Star_System.secret]]'
dv_is_same_as:
- '[[../WikiData/WD~Star_system,595871|WD~Star_system,595871]]'
- '[[/_Standards/Astronomy/Star_System|Star_System]]'
- '[[/_public/Astronomy/Star_System.public|Star_System.public]]'
- '[[/_internal/Astronomy/Star_System.internal|Star_System.internal]]'
- '[[/_protect/Astronomy/Star_System.protect|Star_System.protect]]'
- '[[/_private/Astronomy/Star_System.private|Star_System.private]]'
- '[[/_personal/Astronomy/Star_System.personal|Star_System.personal]]'
- '[[/_secret/Astronomy/Star_System.secret|Star_System.secret]]'
dv_has_:
  name_:
    af: veelvoudige ster
    ar: نظام نجمي
    ast: Sistema estelar
    be: зорная сістэма
    be_tarask: зорная сыстэма
    bg: звездна система
    bs: zvjezdani sistem
    ca: sistema d'estrelles
    cs: hvězdná soustava
    cy: system serol
    de: Sternsystem
    el: αστρικό σύστημα
    en: star system
    eo: stelosistemo
    es: sistema estelar
    et: tähesüsteem
    eu: izar-sistema
    fa: سامانه ستارهای
    fi: tähtijärjestelmä
    fr: système stellaire
    ga: córas réaltaí
    gl: sistema estelar
    he: מערכת מרובת כוכבים
    hi: तारा प्रणाली
    hr: zvjezdani sustav
    hu: csillagrendszer
    hy: աստղային համակարգ
    id: sistem bintang
    is: sólkerfi
    it: sistema stellare
    ja: 恒星系
    jv: sistem lintang
    kab: anagraw itriklan
    kk: жұлдыздар жүйесі
    ko: 항성계
    ks: تارکَن ہُنٛد نِظام
    lb: Stäresystem
    lt: žvaigždžių sistema
    lv: vairākkārtīga zvaigžņu sistēma
    mk: ѕвезден систем
    ms: sistem najam
    nb: stjernesystem
    nl: stersysteem
    nn: stjernesystem
    pl: układ gwiezdny
    pnb: تارہ پربندھ
    pt: sistema estelar
    ro: sistem stelar
    ru: звёздная система
    sco: starn seestem
    sh: stelarni sistem
    sk: hviezdna sústava
    sl: zvezdni sistem
    sq: sistemi yjor
    sr: звездани систем
    sr_ec: звездани систем
    sr_el: zvezdani sistem
    sv: stjärnsystem
    th: ระบบดาว
    tok: kulupu pi mun seli
    tr: yıldız sistemi
    tt: йолдыз системасы
    uk: зоряна система
    uz: yulduz tizimi
    vi: hệ sao
    wuu: 恒星系统
    yue: 恆星系統
    zh: 恆星系統
    zh_cn: 恒星系统
    zh_hk: 恆星系統
    zh_tw: 恆星系統
---
# [[Star_System]] 

#is_/same_as :: [[../WikiData/WD~Star_system,595871|WD~Star_system,595871]]  

## #has_/text_of_/abstract 

> A star system or stellar system is a small number of stars that orbit each other, 
> bound by gravitational attraction. 
> 
> It may sometimes be used to refer to a single star. 
> 
> A large group of stars bound by gravitation 
> is generally called a star cluster or galaxy, 
> although, broadly speaking, they are also star systems. 
> 
> Star systems are not to be confused with planetary systems, 
> which include planets and similar bodies (such as comets).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Star%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Star_System|Star_System]] 

### #is_/same_as :: [[/_public/Astronomy/Star_System.public|Star_System.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Star_System.internal|Star_System.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Star_System.protect|Star_System.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Star_System.private|Star_System.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Star_System.personal|Star_System.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Star_System.secret|Star_System.secret]] 

