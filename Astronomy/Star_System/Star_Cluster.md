---
aliases:
- aglomerado estelar
- amas stellaire
- amasso stellar
- ammasso stellare
- ammassu stiddari
- amàs estellar
- baron
- braisle réaltaí
- Chhiⁿ-thôan
- clwstwr sêr
- csillaghalmaz
- cumulus stellarum
- cúmul estel·lar
- cúmulo estelar
- cúmulu estelar
- fungunyota
- gromada gwiazd
- gugus bintang
- hviezdokopa
- hvězdokupa
- Izar-mulko
- Kachibwayila ka nyenyezi
- kelompok bintang
- kumey esteran
- nástečoahkki
- pundok biton-on
- quần tinh
- roi stelar
- Star Cluster
- Star_Cluster
- Steernkluuster
- stel-grupo
- stelamaso
- sterhoop
- Sternhaufen
- sterreswerm
- Sterrnhoop
- stjernehob
- stjernehop
- stjärnhop
- Stärekoup
- täheparv
- tähtijoukko
- täsnijuávkku
- täʹsnnjoukk
- ulduz topaları
- yıldız kümesi
- zvaigžņu kopa
- zvezdano jato
- zvezdna kopica
- zvjezdana skupina
- zvjezdano jato
- žvaigždžių spiečius
- αστρικό σμήνος
- ѕвездено јато
- жылдыздар чогулмалары
- звёздное скопление
- звездано јато
- звезден куп
- зорнае скопішча
- зорная скупнасьць
- зоряне скупчення
- Йолдызлар тупланышы
- кечет
- кизили люкаськем
- ҫăлтăр кĕпĕрленчĕкĕ
- үйіржұлдыз
- աստղակույտ
- צביר כוכבים
- تارہ گچھا
- خوشه ستارهای
- عنقود نجمي
- ھێشووی ئەستێرەیی
- तारा गुच्छ
- তারা গুচ্ছ
- ਤਾਰਾ ਖਿੱਤੀ
- விண்மீன் கொத்துகள்
- กระจุกดาว
- ကြယ်အစုအဝေး
- ვარსკვლავთგროვა
- მურიცხეფიშ გვართა
- 星团
- 星団
- 星團
- 성단
has_id_wikidata: Q168845
different_from: '[[_Standards/WikiData/WD~galaxy_cluster,204107|WD~galaxy_cluster,204107]]'
subclass_of: '[[_Standards/WikiData/WD~deep_sky_object,249389|WD~deep_sky_object,249389]]'
described_by_source:
- "[[_Standards/WikiData/WD~Otto's_encyclopedia,2041543|WD~Otto's_encyclopedia,2041543]]"
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817|WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Infobox_cluster,6266096|WD~Template_Infobox_cluster,6266096]]'
instance_of: '[[_Standards/WikiData/WD~astronomical_object_type,17444909|WD~astronomical_object_type,17444909]]'
part_of: '[[_Standards/WikiData/WD~galaxy,318|WD~galaxy,318]]'
OmegaWiki_Defined_Meaning: 160089
Dewey_Decimal_Classification: 523.85
image: http://commons.wikimedia.org/wiki/Special:FilePath/M92%20arp%20750pix.jpg
Commons_gallery: Star cluster
Commons_category: Star clusters
Wolfram_Language_entity_type: StarCluster
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Star_cluster,168845|WD~Star_cluster,168845]]'
  - '[[/_Standards/Astronomy/Star_System/Star_Cluster|Star_Cluster]]'
  - '[[/_public/Astronomy/Star_System/Star_Cluster.public|Star_Cluster.public]]'
  - '[[/_internal/Astronomy/Star_System/Star_Cluster.internal|Star_Cluster.internal]]'
  - '[[/_protect/Astronomy/Star_System/Star_Cluster.protect|Star_Cluster.protect]]'
  - '[[/_private/Astronomy/Star_System/Star_Cluster.private|Star_Cluster.private]]'
  - '[[/_personal/Astronomy/Star_System/Star_Cluster.personal|Star_Cluster.personal]]'
  - '[[/_secret/Astronomy/Star_System/Star_Cluster.secret|Star_Cluster.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Star_cluster,168845|WD~Star_cluster,168845]]'
- '[[/_Standards/Astronomy/Star_System/Star_Cluster|Star_Cluster]]'
- '[[/_public/Astronomy/Star_System/Star_Cluster.public|Star_Cluster.public]]'
- '[[/_internal/Astronomy/Star_System/Star_Cluster.internal|Star_Cluster.internal]]'
- '[[/_protect/Astronomy/Star_System/Star_Cluster.protect|Star_Cluster.protect]]'
- '[[/_private/Astronomy/Star_System/Star_Cluster.private|Star_Cluster.private]]'
- '[[/_personal/Astronomy/Star_System/Star_Cluster.personal|Star_Cluster.personal]]'
- '[[/_secret/Astronomy/Star_System/Star_Cluster.secret|Star_Cluster.secret]]'
dv_has_:
  name_:
    af: sterreswerm
    ar: عنقود نجمي
    ast: cúmulu estelar
    az: ulduz topaları
    be: зорнае скопішча
    be_tarask: зорная скупнасьць
    bg: звезден куп
    bn: তারা গুচ্ছ
    bs: zvjezdano jato
    ca: cúmul estel·lar
    ckb: ھێشووی ئەستێرەیی
    cs: hvězdokupa
    cv: ҫăлтăр кĕпĕрленчĕкĕ
    cy: clwstwr sêr
    da: stjernehob
    de: Sternhaufen
    diq: kumey esteran
    el: αστρικό σμήνος
    en: star cluster
    en_ca: star cluster
    en_gb: star cluster
    eo: stelamaso
    es: cúmulo estelar
    et: täheparv
    eu: Izar-mulko
    fa: خوشه ستارهای
    fi: tähtijoukko
    fr: amas stellaire
    ga: braisle réaltaí
    gl: cúmulo estelar
    he: צביר כוכבים
    hi: तारा गुच्छ
    hr: zvjezdana skupina
    hu: csillaghalmaz
    hy: աստղակույտ
    ia: amasso stellar
    id: gugus bintang
    io: stel-grupo
    it: ammasso stellare
    ja: 星団
    ka: ვარსკვლავთგროვა
    kk: үйіржұлдыз
    ko: 성단
    krc: кечет
    ky: жылдыздар чогулмалары
    la: cumulus stellarum
    lb: Stärekoup
    lt: žvaigždžių spiečius
    lv: zvaigžņu kopa
    mk: ѕвездено јато
    ms: kelompok bintang
    my: ကြယ်အစုအဝေး
    nan: Chhiⁿ-thôan
    nb: stjernehop
    nds: Steernkluuster
    nl: sterhoop
    nn: stjernehop
    oc: amàs estellar
    pa: ਤਾਰਾ ਖਿੱਤੀ
    pl: gromada gwiazd
    pms: baron
    pnb: تارہ گچھا
    pt: aglomerado estelar
    pt_br: aglomerado estelar
    ro: roi stelar
    ru: звёздное скопление
    scn: ammassu stiddari
    se: nástečoahkki
    sh: zvezdano jato
    sk: hviezdokopa
    sl: zvezdna kopica
    smn: täsnijuávkku
    sms: täʹsnnjoukk
    sr: звездано јато
    sr_ec: звездано јато
    sr_el: zvezdano jato
    sv: stjärnhop
    sw: fungunyota
    ta: விண்மீன் கொத்துகள்
    th: กระจุกดาว
    tr: yıldız kümesi
    tt: Йолдызлар тупланышы
    tum: Kachibwayila ka nyenyezi
    udm: кизили люкаськем
    uk: зоряне скупчення
    vi: quần tinh
    vls: Sterrnhoop
    war: pundok biton-on
    wuu: 星团
    xmf: მურიცხეფიშ გვართა
    yue: 星團
    zh: 星团
    zh_cn: 星团
    zh_hans: 星团
    zh_hant: 星團
    zh_hk: 星團
    zh_sg: 星团
    zh_tw: 星團
---

# [[Star_Cluster]] 

#is_/same_as :: [[../../WikiData/WD~Star_cluster,168845|WD~Star_cluster,168845]]  

## #has_/text_of_/abstract 

> A **Star cluster** is a group of stars held together by self-gravitation. 
> Two main types of star clusters can be distinguished: 
> - globular clusters, tight groups of ten thousand to millions of old stars which are gravitationally bound; 
> - and open clusters, less tight groups of stars, generally containing fewer than a few hundred members.
>
> As they move through the galaxy, over time, open clusters become disrupted by the gravitational influence of giant molecular clouds, so that the clusters we observe are often young. Even though they are no longer gravitationally bound, they will continue to move in broadly the same direction through space and are then known as stellar associations, sometimes referred to as moving groups. Globular clusters, with more members and more mass, remain intact for far longer and the globular clusters observed are usually billions of years old.
>
> Star clusters visible to the naked eye include the Pleiades and Hyades open clusters, and the globular cluster 47 Tucanae.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Star%20cluster) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Star_System/Star_Cluster|Star_Cluster]] 

### #is_/same_as :: [[/_public/Astronomy/Star_System/Star_Cluster.public|Star_Cluster.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Star_System/Star_Cluster.internal|Star_Cluster.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Star_System/Star_Cluster.protect|Star_Cluster.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Star_System/Star_Cluster.private|Star_Cluster.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Star_System/Star_Cluster.personal|Star_Cluster.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Star_System/Star_Cluster.secret|Star_Cluster.secret]] 

