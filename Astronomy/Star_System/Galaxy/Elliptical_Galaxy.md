---
aliases:
- elipsa galaksio
- Elipsinė galaktika
- Eliptická galaxia
- eliptická galaxie
- Eliptik galaksi
- Eliptička galaksija
- eliptična galaksija
- eliptična galaktika
- Elliptesch Galaxie
- elliptical galaxy
- Elliptical_Galaxy
- elliptiese sterrestelsel
- Elliptik galaktika
- elliptikus galaxis
- Elliptiline galaktika
- Elliptinen galaksi
- elliptisch sterrenstelsel
- elliptische Galaxie
- elliptisk galakse
- elliptisk galax
- galaeth eliptig
- Galaksi elips
- Galaksi eliptis
- Galaksiyo eliptik
- galaktyka eliptyczna
- galassia ellittica
- Galaxa elíptica
- Galaxia eliptiko
- Galaxia elliptica
- galaxia elíptica
- galaxie eliptică
- galaxie elliptique
- galàxia el·líptica
- galáxia elíptica
- réaltra éilipseach
- thiên hà elip
- Thó-îⁿ gîn-hô
- Umthalakazi omakhophelana
- Ελλειπτικός γαλαξίας
- еліптична галактика
- елиптичка галаксија
- Елиптична галаксија
- Елиптична галактика
- Эліптычная галактыка
- эліптычная галяктыка
- Эллипстик галактиктер
- эллиптик галактика
- Эллиптикалық галактика
- эллиптическая галактика
- էլիպսաձև գալակտիկա
- גלקסיה אליפטית
- آنڈیورگا تارہ سمندر
- مجرة إهليلجية
- کهکشان بیضوی
- گەلەستێرەی ھێلکەیی
- अंडाकार गैलेक्सी
- लंबवर्तुळाकार दीर्घिका
- ดาราจักรรี
- ဘဲဥပုံစံ ဂလက်ဆီ
- ელიპტიკური გალაქტიკა
- 椭圆星系
- 楕円銀河
- 橢圓星系
- 타원 은하
has_id_wikidata: Q184348
follows: '[[_Standards/WikiData/WD~lenticular_galaxy,192078]]'
subclass_of: '[[_Standards/WikiData/WD~galaxy,318]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Abell%20S740%2C%20cropped%20to%20ESO%20325-G004.jpg
Commons_category: Elliptical galaxies
dv_is_:
  same_as:
  - '[[../../../WikiData/WD~Elliptical_galaxy,184348|WD~Elliptical_galaxy,184348]]'
  - '[[/_Standards/Astronomy/Star_System/Galaxy/Elliptical_Galaxy|Elliptical_Galaxy]]'
  - '[[/_public/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.public|Elliptical_Galaxy.public]]'
  - '[[/_internal/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.internal|Elliptical_Galaxy.internal]]'
  - '[[/_protect/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.protect|Elliptical_Galaxy.protect]]'
  - '[[/_private/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.private|Elliptical_Galaxy.private]]'
  - '[[/_personal/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.personal|Elliptical_Galaxy.personal]]'
  - '[[/_secret/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.secret|Elliptical_Galaxy.secret]]'
dv_is_same_as:
- '[[../../../WikiData/WD~Elliptical_galaxy,184348|WD~Elliptical_galaxy,184348]]'
- '[[/_Standards/Astronomy/Star_System/Galaxy/Elliptical_Galaxy|Elliptical_Galaxy]]'
- '[[/_public/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.public|Elliptical_Galaxy.public]]'
- '[[/_internal/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.internal|Elliptical_Galaxy.internal]]'
- '[[/_protect/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.protect|Elliptical_Galaxy.protect]]'
- '[[/_private/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.private|Elliptical_Galaxy.private]]'
- '[[/_personal/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.personal|Elliptical_Galaxy.personal]]'
- '[[/_secret/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.secret|Elliptical_Galaxy.secret]]'
dv_has_:
  name_:
    af: elliptiese sterrestelsel
    ar: مجرة إهليلجية
    ast: Galaxa elíptica
    be: Эліптычная галактыка
    be_tarask: эліптычная галяктыка
    bg: Елиптична галактика
    bs: eliptična galaksija
    ca: galàxia el·líptica
    ckb: گەلەستێرەی ھێلکەیی
    cs: eliptická galaxie
    cy: galaeth eliptig
    de: elliptische Galaxie
    diq: Galaksiyo eliptik
    el: Ελλειπτικός γαλαξίας
    en: elliptical galaxy
    eo: elipsa galaksio
    es: galaxia elíptica
    et: Elliptiline galaktika
    eu: Galaxia eliptiko
    fa: کهکشان بیضوی
    fi: Elliptinen galaksi
    fr: galaxie elliptique
    ga: réaltra éilipseach
    gl: galaxia elíptica
    he: גלקסיה אליפטית
    hi: अंडाकार गैलेक्सी
    hr: eliptična galaktika
    hu: elliptikus galaxis
    hy: էլիպսաձև գալակտիկա
    id: Galaksi eliptis
    it: galassia ellittica
    ja: 楕円銀河
    kk: Эллиптикалық галактика
    ko: 타원 은하
    ky: Эллипстик галактиктер
    lb: Elliptesch Galaxie
    lt: Elipsinė galaktika
    mk: Елиптична галаксија
    mr: लंबवर्तुळाकार दीर्घिका
    ms: Galaksi elips
    my: ဘဲဥပုံစံ ဂလက်ဆီ
    nan: Thó-îⁿ gîn-hô
    nb: elliptisk galakse
    nl: elliptisch sterrenstelsel
    nn: elliptisk galakse
    oc: Galaxia elliptica
    pl: galaktyka eliptyczna
    pnb: آنڈیورگا تارہ سمندر
    pt: galáxia elíptica
    ro: galaxie eliptică
    ru: эллиптическая галактика
    sh: Eliptička galaksija
    sk: Eliptická galaxia
    sl: eliptična galaksija
    sr: елиптична галаксија
    sr_ec: елиптичка галаксија
    sr_el: eliptička galaksija
    sv: elliptisk galax
    th: ดาราจักรรี
    tr: Eliptik galaksi
    tt: эллиптик галактика
    uk: еліптична галактика
    uz: Elliptik galaktika
    vi: thiên hà elip
    wuu: 椭圆星系
    xmf: ელიპტიკური გალაქტიკა
    yue: 橢圓星系
    zh: 椭圆星系
    zh_cn: 椭圆星系
    zh_hans: 椭圆星系
    zh_hant: 橢圓星系
    zh_hk: 橢圓星系
    zh_sg: 椭圆星系
    zh_tw: 橢圓星系
    zu: Umthalakazi omakhophelana
---
# [[Elliptical_Galaxy]] 

#is_/same_as :: [[../../../WikiData/WD~Elliptical_galaxy,184348|WD~Elliptical_galaxy,184348]] 

## #has_/text_of_/abstract 

> An elliptical galaxy is a type of galaxy with an approximately ellipsoidal shape 
> and a smooth, nearly featureless image. 
> They are one of the three main classes of galaxy described by Edwin Hubble in his Hubble sequence 
> and 1936 work The Realm of the Nebulae, along with spiral and lenticular galaxies. 
>
> Elliptical (E) galaxies are, together with lenticular galaxies (S0) with their large-scale disks, 
> and ES galaxies with their intermediate scale disks, a subset of the "early-type" galaxy population.
>
> Most elliptical galaxies are composed of older, low-mass stars, with a sparse interstellar medium, and they tend to be surrounded by large numbers of globular clusters. Star formation activity in elliptical galaxies is typically minimal; they may, however, undergo brief periods of star formation when merging with other galaxies. Elliptical galaxies are believed to make up approximately 10–15% of galaxies in the Virgo Supercluster, and they are not the dominant type of galaxy in the universe overall. They are preferentially found close to the centers of galaxy clusters.
>
> Elliptical galaxies range in size from dwarf ellipticals with tens of millions of stars, to supergiants of over one hundred trillion stars that dominate their galaxy clusters. Originally, Edwin Hubble hypothesized that elliptical galaxies evolved into spiral galaxies, which was later discovered to be false, although the accretion of gas and smaller galaxies may build a disk around a pre-existing ellipsoidal structure.
>
> Stars found inside of elliptical galaxies are on average much older than stars found in spiral galaxies.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Elliptical%20galaxy) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Star_System/Galaxy/Elliptical_Galaxy|Elliptical_Galaxy]] 

### #is_/same_as :: [[/_public/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.public|Elliptical_Galaxy.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.internal|Elliptical_Galaxy.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.protect|Elliptical_Galaxy.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.private|Elliptical_Galaxy.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.personal|Elliptical_Galaxy.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Star_System/Galaxy/Elliptical_Galaxy.secret|Elliptical_Galaxy.secret]] 

