---
aliases:
- cruthú agus éabhlóid na réaltraí
- Entstehung und Entwicklung von Galaxien
- formace a vznik galaxií
- formacija i evolucija galaksija
- formació i evolució de les galàxies
- formación e evolución das galaxias
- formación y evolución de las galaxias
- formación y evolución de les galaxes
- formasion e evołusion gałàtega
- formation et évolution des galaxies
- formazione ed evoluzione galattica
- formação e evolução de galáxias
- formiranje i evolucija galaktike
- galaksi oluşumu ve evrimi
- galaksien muodostuminen ja evoluutio
- galaxien eraketa eta eboluzioa
- galaxiskeletkezés
- galaxy formation and evolution
- Galaxy_Formation_and_Evolution
- nastanek in razvoj galaksij
- ontstaan en evolutie van sterrenstelsels
- pembentukan dan evolusi galaksi
- powstawanie galaktyk
- sự hình thành và tiến hóa thiên hà
- vznik a vývoj galaxií
- возникновение и эволюция галактик
- галактикăсен эволюцийĕ
- галактикалар барлыкка килүе һәм эволюциясе
- настанак и еволуција галаксија
- настанок и развој на галаксиите
- утварэньне і эвалюцыя галяктык
- утворення та еволюція галактик
- գալակտիկաների էվոլյուցիա
- היווצרות גלקסיות והתפתחותן
- تشكل وتطور المجرات
- د کهکشان جوړښت او تکامل
- شکلگیری و تکامل کهکشانها
- กำเนิดและวิวัฒนาการของดาราจักร
- ọdịdị ụyọkọ kpakpando na evolushọn
- 星系嘅形成同演化
- 星系的形成和演化
- 銀河の形成と進化
- 은하의 형성 및 진화
has_id_wikidata: Q1995140
instance_of:
- '[[_Standards/WikiData/WD~academic_major,4671286|WD~academic_major,4671286]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829|WD~academic_discipline,11862829]]'
is_the_study_of: '[[_Standards/WikiData/WD~galaxy,318|WD~galaxy,318]]'
subclass_of: '[[_Standards/WikiData/WD~astrophysics,37547|WD~astrophysics,37547]]'
video: http://commons.wikimedia.org/wiki/Special:FilePath/Simulation%20%E2%80%93%20A%20Disk%20Galaxy%20Forms.webm
dv_is_:
  same_as:
  - '[[../../../WikiData/WD~Galaxy_formation_and_evolution,1995140|WD~Galaxy_formation_and_evolution,1995140]]'
  - '[[/_Standards/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution|Galaxy_Formation_and_Evolution]]'
  - '[[/_public/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.public|Galaxy_Formation_and_Evolution.public]]'
  - '[[/_internal/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.internal|Galaxy_Formation_and_Evolution.internal]]'
  - '[[/_protect/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.protect|Galaxy_Formation_and_Evolution.protect]]'
  - '[[/_private/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.private|Galaxy_Formation_and_Evolution.private]]'
  - '[[/_personal/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.personal|Galaxy_Formation_and_Evolution.personal]]'
  - '[[/_secret/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.secret|Galaxy_Formation_and_Evolution.secret]]'
dv_is_same_as:
- '[[../../../WikiData/WD~Galaxy_formation_and_evolution,1995140|WD~Galaxy_formation_and_evolution,1995140]]'
- '[[/_Standards/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution|Galaxy_Formation_and_Evolution]]'
- '[[/_public/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.public|Galaxy_Formation_and_Evolution.public]]'
- '[[/_internal/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.internal|Galaxy_Formation_and_Evolution.internal]]'
- '[[/_protect/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.protect|Galaxy_Formation_and_Evolution.protect]]'
- '[[/_private/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.private|Galaxy_Formation_and_Evolution.private]]'
- '[[/_personal/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.personal|Galaxy_Formation_and_Evolution.personal]]'
- '[[/_secret/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.secret|Galaxy_Formation_and_Evolution.secret]]'
dv_has_:
  name_:
    ar: تشكل وتطور المجرات
    ast: formación y evolución de les galaxes
    be_tarask: утварэньне і эвалюцыя галяктык
    bs: formacija i evolucija galaksija
    ca: formació i evolució de les galàxies
    cs: formace a vznik galaxií
    cv: галактикăсен эволюцийĕ
    de: Entstehung und Entwicklung von Galaxien
    en: galaxy formation and evolution
    en_ca: galaxy formation and evolution
    en_gb: galaxy formation and evolution
    es: formación y evolución de las galaxias
    eu: galaxien eraketa eta eboluzioa
    fa: شکلگیری و تکامل کهکشانها
    fi: galaksien muodostuminen ja evoluutio
    fr: formation et évolution des galaxies
    ga: cruthú agus éabhlóid na réaltraí
    gl: formación e evolución das galaxias
    he: היווצרות גלקסיות והתפתחותן
    hu: galaxiskeletkezés
    hy: գալակտիկաների էվոլյուցիա
    id: pembentukan dan evolusi galaksi
    ig: ọdịdị ụyọkọ kpakpando na evolushọn
    it: formazione ed evoluzione galattica
    ja: 銀河の形成と進化
    ko: 은하의 형성 및 진화
    mk: настанок и развој на галаксиите
    ms: Pembentukan dan evolusi galaksi
    nl: ontstaan en evolutie van sterrenstelsels
    pl: powstawanie galaktyk
    ps: د کهکشان جوړښت او تکامل
    pt: formação e evolução de galáxias
    pt_br: formação e evolução de galáxias
    ru: возникновение и эволюция галактик
    sh: formiranje i evolucija galaktike
    sk: vznik a vývoj galaxií
    sl: nastanek in razvoj galaksij
    sr: настанак и еволуција галаксија
    th: กำเนิดและวิวัฒนาการของดาราจักร
    tr: galaksi oluşumu ve evrimi
    tt: галактикалар барлыкка килүе һәм эволюциясе
    uk: утворення та еволюція галактик
    vec: formasion e evołusion gałàtega
    vi: sự hình thành và tiến hóa thiên hà
    yue: 星系嘅形成同演化
    zh: 星系的形成和演化
    zh_hans: 星系的形成和演化
    zh_hant: 星系的形成和演化
    zh_hk: 星系的形成和演化
---

# [[Galaxy_Formation_and_Evolution]] 

#is_/same_as :: [[../../../WikiData/WD~Galaxy_formation_and_evolution,1995140|WD~Galaxy_formation_and_evolution,1995140]] 

## #has_/text_of_/abstract 

> In cosmology, the study of galaxy formation and evolution 
> is concerned with the processes that formed a heterogeneous universe from a homogeneous beginning, 
> the formation of the first galaxies, the way galaxies change over time, 
> and the processes that have generated the variety of structures observed in nearby galaxies. 
> 
> Galaxy formation is hypothesized to occur from structure formation theories, as a result of tiny quantum fluctuations in the aftermath of the Big Bang. The simplest model in general agreement with observed phenomena is the Lambda-CDM model—that is, clustering and merging allows galaxies to accumulate mass, determining both their shape and structure. Hydrodynamics simulation, which simulates both baryons and dark matter, is widely used to study galaxy formation and evolution.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Galaxy%20formation%20and%20evolution) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution|Galaxy_Formation_and_Evolution]] 

### #is_/same_as :: [[/_public/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.public|Galaxy_Formation_and_Evolution.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.internal|Galaxy_Formation_and_Evolution.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.protect|Galaxy_Formation_and_Evolution.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.private|Galaxy_Formation_and_Evolution.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.personal|Galaxy_Formation_and_Evolution.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Star_System/Galaxy/Galaxy_Formation_and_Evolution.secret|Galaxy_Formation_and_Evolution.secret]] 

