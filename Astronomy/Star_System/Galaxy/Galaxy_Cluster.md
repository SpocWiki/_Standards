---
aliases:
- aglomerado de galáxias
- agrupación galáctica
- amas de galaxies
- ammasso di galassie
- ammassu di galassî
- Amàs de galaxias
- braisle réaltraí
- cluster
- cluster of galaxies
- clusters of galaxies
- clwstwr o alaethau
- cúmul de galàxies
- cúmulo de galaxias
- cụm thiên hà
- galaksehob
- galaksehop
- galaksi grubu
- galaksiamaso
- galaksijoukko
- Galaktikaparv
- galaktikoiden ühtmuz
- Galaktiku grupas un kopas
- Galaktikų spiečius
- galaktički skupovi
- Galaktičko jato
- galaxhop
- galaxia kumulu
- Galaxiala grupo
- Galaxienhaufen
- galaxishalmaz
- Galaxiëkoup
- galaxy cluster
- galaxy clusters
- Galaxy groups and clusters
- Galaxy_Cluster
- gromada galaktyk
- grupo de gałàsie
- Gugusan galaksi
- jata galaksij
- Kumey galaksi
- Kumpulan dan gugusan galaksi
- kupa galaxií
- Qalaktika topası
- Roi de galaxii
- Skupina galaxií
- sterrestelselswerm
- σμήνος γαλαξιών
- Галактикăсен кĕпĕрленĕвĕ
- Галактикалар тупланмаһы
- Галактикалар тупланышы
- Галактикалық шоғырлар
- Галактикийн бөөгнөрөл
- Галактическо струпване
- галактичко јато
- галяктычнае скопішча
- скопішча галактык
- скопление галактик
- скупчення галактик
- գալակտիկաների կուտակում
- צביר גלקסיות
- خوشه کهکشانی
- د کهکشان کلستر
- عناقيد المجرات
- ھێشووی گەلەستێرەیی
- मंदाकिनी गुच्छ
- ছায়াপথ স্তবক
- ਗਲੈਕਸੀ ਕਲੱਸਟਰ
- ഗ്യാലക്സി ക്ലസ്റ്റർ
- กลุ่มและกระจุกดาราจักร
- ဂလက်ဆီ အစုအပြုံ
- გალაქტიკათგროვა
- გალაქტიკეფიშ გვართა
- 星系团
- 星系團
- 銀河団
- 은하단
has_id_wikidata: Q204107
different_from:
- '[[_Standards/WikiData/WD~star_cluster,168845|WD~star_cluster,168845]]'
- '[[_Standards/WikiData/WD~galaxy_group,1491746|WD~galaxy_group,1491746]]'
subclass_of:
- '[[_Standards/WikiData/WD~deep_sky_object,249389|WD~deep_sky_object,249389]]'
- '[[_Standards/WikiData/WD~galaxy_group_or_cluster,14515149|WD~galaxy_group_or_cluster,14515149]]'
has_part_s_: '[[_Standards/WikiData/WD~brightest_cluster_galaxy,1151284|WD~brightest_cluster_galaxy,1151284]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Infobox_galaxy_cluster,11167589|WD~Template_Infobox_galaxy_cluster,11167589]]'
instance_of: '[[_Standards/WikiData/WD~astronomical_object_type,17444909|WD~astronomical_object_type,17444909]]'
has_part_s_of_the_class: '[[_Standards/WikiData/WD~galaxy,318|WD~galaxy,318]]'
part_of: '[[_Standards/WikiData/WD~supercluster,27521|WD~supercluster,27521]]'
OmegaWiki_Defined_Meaning: 639453
Commons_category: Galaxy clusters
video: http://commons.wikimedia.org/wiki/Special:FilePath/Artist%E2%80%99s%20impression%20of%20a%20protocluster%20forming%20in%20the%20early%20Universe.ogv
image: http://commons.wikimedia.org/wiki/Special:FilePath/Galaxienhaufen%20von%20SMACS%200723.jpg
Stack_Exchange_tag:
- https://astronomy.stackexchange.com/tags/galaxy-cluster
- https://physics.stackexchange.com/tags/galaxy-clusters
dv_is_:
  same_as:
  - '[[../../../WikiData/WD~Galaxy_cluster,204107|WD~Galaxy_cluster,204107]]'
  - '[[/_Standards/Astronomy/Star_System/Galaxy/Galaxy_Cluster|Galaxy_Cluster]]'
  - '[[/_public/Astronomy/Star_System/Galaxy/Galaxy_Cluster.public|Galaxy_Cluster.public]]'
  - '[[/_internal/Astronomy/Star_System/Galaxy/Galaxy_Cluster.internal|Galaxy_Cluster.internal]]'
  - '[[/_protect/Astronomy/Star_System/Galaxy/Galaxy_Cluster.protect|Galaxy_Cluster.protect]]'
  - '[[/_private/Astronomy/Star_System/Galaxy/Galaxy_Cluster.private|Galaxy_Cluster.private]]'
  - '[[/_personal/Astronomy/Star_System/Galaxy/Galaxy_Cluster.personal|Galaxy_Cluster.personal]]'
  - '[[/_secret/Astronomy/Star_System/Galaxy/Galaxy_Cluster.secret|Galaxy_Cluster.secret]]'
dv_is_same_as:
- '[[../../../WikiData/WD~Galaxy_cluster,204107|WD~Galaxy_cluster,204107]]'
- '[[/_Standards/Astronomy/Star_System/Galaxy/Galaxy_Cluster|Galaxy_Cluster]]'
- '[[/_public/Astronomy/Star_System/Galaxy/Galaxy_Cluster.public|Galaxy_Cluster.public]]'
- '[[/_internal/Astronomy/Star_System/Galaxy/Galaxy_Cluster.internal|Galaxy_Cluster.internal]]'
- '[[/_protect/Astronomy/Star_System/Galaxy/Galaxy_Cluster.protect|Galaxy_Cluster.protect]]'
- '[[/_private/Astronomy/Star_System/Galaxy/Galaxy_Cluster.private|Galaxy_Cluster.private]]'
- '[[/_personal/Astronomy/Star_System/Galaxy/Galaxy_Cluster.personal|Galaxy_Cluster.personal]]'
- '[[/_secret/Astronomy/Star_System/Galaxy/Galaxy_Cluster.secret|Galaxy_Cluster.secret]]'
dv_has_:
  name_:
    af: sterrestelselswerm
    ar: عناقيد المجرات
    az: Qalaktika topası
    ba: Галактикалар тупланмаһы
    be: скопішча галактык
    be_tarask: галяктычнае скопішча
    bg: Галактическо струпване
    bn: ছায়াপথ স্তবক
    bs: Galaktičko jato
    ca: cúmul de galàxies
    ckb: ھێشووی گەلەستێرەیی
    cs: kupa galaxií
    cv: Галактикăсен кĕпĕрленĕвĕ
    cy: clwstwr o alaethau
    da: galaksehob
    de: Galaxienhaufen
    de_ch: Galaxienhaufen
    diq: Kumey galaksi
    el: σμήνος γαλαξιών
    en: galaxy cluster
    en_ca: Galaxy groups and clusters
    en_gb: galaxy cluster
    eo: galaksiamaso
    es: agrupación galáctica
    et: Galaktikaparv
    eu: galaxia kumulu
    fa: خوشه کهکشانی
    fi: galaksijoukko
    fr: amas de galaxies
    ga: braisle réaltraí
    gl: cúmulo de galaxias
    he: צביר גלקסיות
    hi: मंदाकिनी गुच्छ
    hr: galaktički skupovi
    hu: galaxishalmaz
    hy: գալակտիկաների կուտակում
    id: Gugusan galaksi
    io: Galaxiala grupo
    it: ammasso di galassie
    ja: 銀河団
    ka: გალაქტიკათგროვა
    kk: Галактикалық шоғырлар
    ko: 은하단
    lb: Galaxiëkoup
    lt: Galaktikų spiečius
    lv: Galaktiku grupas un kopas
    mk: галактичко јато
    ml: ഗ്യാലക്സി ക്ലസ്റ്റർ
    mn: Галактикийн бөөгнөрөл
    ms: Kumpulan dan gugusan galaksi
    my: ဂလက်ဆီ အစုအပြုံ
    nb: galaksehop
    nl: cluster
    nn: galaksehop
    oc: Amàs de galaxias
    pa: ਗਲੈਕਸੀ ਕਲੱਸਟਰ
    pl: gromada galaktyk
    ps: د کهکشان کلستر
    pt: aglomerado de galáxias
    pt_br: aglomerado de galáxias
    ro: Roi de galaxii
    ru: скопление галактик
    scn: ammassu di galassî
    sk: Skupina galaxií
    sl: jata galaksij
    sr: галактичко јато
    sr_ec: галактичко јато
    sr_el: galaktičko jato
    sv: galaxhop
    th: กลุ่มและกระจุกดาราจักร
    tr: galaksi grubu
    tt: Галактикалар тупланышы
    uk: скупчення галактик
    vec: grupo de gałàsie
    vep: galaktikoiden ühtmuz
    vi: cụm thiên hà
    wuu: 星系团
    xmf: გალაქტიკეფიშ გვართა
    yue: 星系團
    zh: 星系团
    zh_cn: 星系团
    zh_hans: 星系团
    zh_hant: 星系團
    zh_hk: 星系團
    zh_sg: 星系团
    zh_tw: 星系團
---

# [[Galaxy_Cluster]] 

#is_/same_as :: [[../../../WikiData/WD~Galaxy_cluster,204107|WD~Galaxy_cluster,204107]] 

## #has_/text_of_/abstract 

> A galaxy cluster, or a cluster of galaxies, is a structure that consists of 
> anywhere from hundreds to thousands of galaxies that are bound together by gravity,  
> with typical masses ranging from 10^14 to 10^15 solar masses. 
> 
> Clusters consist of galaxies, heated gas, and dark matter. 
> They are the second-largest known gravitationally bound structures in the universe after superclusters. 
> They were believed to be the largest known structures in the universe until the 1980s, 
> when superclusters were discovered.  
> 
> Small aggregates of galaxies are referred to as galaxy groups rather than clusters of galaxies. 
> Together, galaxy groups and clusters form superclusters.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Galaxy%20cluster) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Star_System/Galaxy/Galaxy_Cluster|Galaxy_Cluster]] 

### #is_/same_as :: [[/_public/Astronomy/Star_System/Galaxy/Galaxy_Cluster.public|Galaxy_Cluster.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Star_System/Galaxy/Galaxy_Cluster.internal|Galaxy_Cluster.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Star_System/Galaxy/Galaxy_Cluster.protect|Galaxy_Cluster.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Star_System/Galaxy/Galaxy_Cluster.private|Galaxy_Cluster.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Star_System/Galaxy/Galaxy_Cluster.personal|Galaxy_Cluster.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Star_System/Galaxy/Galaxy_Cluster.secret|Galaxy_Cluster.secret]] 

