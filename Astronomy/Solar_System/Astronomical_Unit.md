---
aliases:
- aonad réalteolaíoch
- astronomesch Eenheet
- astronomia unuo
- astronomiala unajo
- Astronomical unit
- Astronomical_unit
- astronomická jednotka
- astronomiese eenheid
- astronomik birim
- Astronomik birlik
- astronomik vahid
- astronomikal a yunit
- astronominen yksikkö
- astronominis vienetas
- astronomische eenheid
- astronomische einheid
- astronomische Einheit
- astronomischi Einheit
- astronomisk eining
- astronomisk enhed
- astronomisk enhet
- astronomiske Eenhaid
- Astronomiske Eynheid
- astronomiskā vienība
- astronomska enota
- astronomska jedinica
- astronoomiline ühik
- astronoomisk ianhaid
- astronoomsch Eenheit
- AU
- csillagászati egység
- Hijian Astronomi
- jednostka astronomiczna
- kizio astronomia
- lecʉte anū anɛ nu
- otu ihe na-enyocha mbara igwe
- ounidade astronómica
- satuan astronomi
- stjarnfræðieining
- thian-bûn tan-ūi
- tigget tasnullant
- UA
- unanenn steredoniel
- uned seryddol
- unia astronomial
- unidad astronomico
- unidad astronómica
- unidade astronómica
- unidade(s) astronômica(s)
- unidadi astronòmica
- unidat astronomica
- unidat astronómiko
- unidá astronómica
- unit astronomi
- unitas astronomica
- unitat astronomica
- unitat astronòmica
- unitate astronomiko
- unità astronomica
- unità astronòmica
- unità astrunòmica
- unité astronomique
- unnid rollageydagh
- yekeya astronomîk
- yunit astronomiko
- yunit na pang-astronomiya
- đơn vị thiên văn
- αστρονομική μονάδα
- астранамічная адзінка
- астроном кээмэйэ
- астрономічна одиниця
- астрономик берәмлек
- астрономилле пĕрчĕ
- астрономин дакъа
- астрономическая единица
- астрономической единица
- астрономична единка
- астрономиялық бірлік
- Астрономнуг хемчеглер
- астрономон иуæг
- астрономска јединица
- астрономска единица
- воҳиди нуҷумӣ
- одон орны нэгж
- աստղագիտական միավոր
- אסטראנאמישע איינס
- יחידה אסטרונומית
- آسترونومیک واحید
- آسٽروناميڪل يونٽ
- اسمانی میل
- اونيت استرونومي
- عبرة فلكية
- فلکیاتی اکائی
- نجومي واحد
- واحد نجومی
- وحدة فلكية
- وحده فلكيه
- یەکەی ئەستێرەناسانە
- ߛߊ߲ߡߊߛߓߍ ߘߊ߲ߕߊ
- खगोलीय इकाई
- खगोलीय एकक
- জ্যোতির্বৈজ্ঞানিক একক
- ਖਗੋਲੀ ਇਕਾਈ
- வானியல் அலகு
- ಖಗೋಳ ಮಾನ
- സൗരദൂരം
- หน่วยดาราศาสตร์
- ასტრონომიული ართული
- ასტრონომიული ერთეული
- 天文单位
- 天文単位
- 天文單位
- 천문단위
has_id_wikidata: Q1811
different_from:
- '[[_Standards/WikiData/WD~AE,227699|WD~AE,227699]]'
- '[[_Standards/WikiData/WD~AU,61750950|WD~AU,61750950]]'
- '[[_Standards/WikiData/WD~African_Union,7159|WD~African_Union,7159]]'
opposite_of: '[[_Standards/WikiData/WD~ECEF,743590|WD~ECEF,743590]]'
instance_of:
- '[[_Standards/WikiData/WD~unit_of_length,1978718|WD~unit_of_length,1978718]]'
- '[[_Standards/WikiData/WD~non_SI_unit_mentioned_in_and_accepted_with_the_SI,3268848|WD~non_SI_unit_mentioned_in_and_accepted_with_the_SI,3268848]]'
- '[[_Standards/WikiData/WD~UCUM_derived_unit,82047057|WD~UCUM_derived_unit,82047057]]'
- '[[_Standards/WikiData/WD~SI_accepted_non_SI_unit,106839753|WD~SI_accepted_non_SI_unit,106839753]]'
unit_symbol:
- و.ف
- а.и.
- AU
- au
- а. а.
- ua
- ua
- au
- CsE
- а.е.
- а.б.
short_name:
- و.ف
- а.и.
- au
- аа
- ua
- au
- а.е.
- astronomiska enheter
- AE
described_by_source:
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~ISO_80000_3_2006_Quantities_and_units_Part_3_Space_and_time,26711932|WD~ISO_80000_3_2006_Quantities_and_units_Part_3_Space_and_time,26711932]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817|WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817]]'
UMLS_CUI: C1551067
conversion_to_SI_unit: 149597870700
measured_physical_quantity: '[[_Standards/WikiData/WD~length,36253|WD~length,36253]]'
OmegaWiki_Defined_Meaning: 665245
UCUM_code: AU
UN_CEFACT_Common_Code: A12
image: http://commons.wikimedia.org/wiki/Special:FilePath/Astronomical%20unit.svg
Wolfram_Language_unit_code: '"AstronomicalUnit"'
Wolfram_Language_entity_code: Entity["PhysicalConstant", "AstronomicalUnit"]
Wikidata_SPARQL_query_equivalent: wd:Q1811 p:P2370/psn:P2370 [wikibase:quantityAmount ?source; wikibase:quantityUnit ?base]. ?item p:P2370/psn:P2370 [wikibase:quantityAmount ?target; wikibase:quantityUnit ?base]. BIND(?source / ?target as ?value)
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Astronomical_unit,1811|WD~Astronomical_unit,1811]]'
  - '[[/_Standards/Astronomy/Solar_System/Astronomical_Unit|Astronomical_Unit]]'
  - '[[/_public/Astronomy/Solar_System/Astronomical_Unit.public|Astronomical_Unit.public]]'
  - '[[/_internal/Astronomy/Solar_System/Astronomical_Unit.internal|Astronomical_Unit.internal]]'
  - '[[/_protect/Astronomy/Solar_System/Astronomical_Unit.protect|Astronomical_Unit.protect]]'
  - '[[/_private/Astronomy/Solar_System/Astronomical_Unit.private|Astronomical_Unit.private]]'
  - '[[/_personal/Astronomy/Solar_System/Astronomical_Unit.personal|Astronomical_Unit.personal]]'
  - '[[/_secret/Astronomy/Solar_System/Astronomical_Unit.secret|Astronomical_Unit.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Astronomical_unit,1811|WD~Astronomical_unit,1811]]'
- '[[/_Standards/Astronomy/Solar_System/Astronomical_Unit|Astronomical_Unit]]'
- '[[/_public/Astronomy/Solar_System/Astronomical_Unit.public|Astronomical_Unit.public]]'
- '[[/_internal/Astronomy/Solar_System/Astronomical_Unit.internal|Astronomical_Unit.internal]]'
- '[[/_protect/Astronomy/Solar_System/Astronomical_Unit.protect|Astronomical_Unit.protect]]'
- '[[/_private/Astronomy/Solar_System/Astronomical_Unit.private|Astronomical_Unit.private]]'
- '[[/_personal/Astronomy/Solar_System/Astronomical_Unit.personal|Astronomical_Unit.personal]]'
- '[[/_secret/Astronomy/Solar_System/Astronomical_Unit.secret|Astronomical_Unit.secret]]'
dv_has_:
  name_:
    af: astronomiese eenheid
    an: unidat astronomica
    ar: وحدة فلكية
    ary: عبرة فلكية
    arz: وحده فلكيه
    ast: unidá astronómica
    az: astronomik vahid
    azb: آسترونومیک واحید
    be: астранамічная адзінка
    be_tarask: астранамічная адзінка
    bg: AU
    bn: জ্যোতির্বৈজ্ঞানিক একক
    br: unanenn steredoniel
    bs: astronomska jedinica
    ca: unitat astronòmica
    ce: астрономин дакъа
    ckb: یەکەی ئەستێرەناسانە
    cs: astronomická jednotka
    cv: астрономилле пĕрчĕ
    cy: uned seryddol
    da: astronomisk enhed
    de: astronomische Einheit
    de_ch: astronomische Einheit
    el: αστρονομική μονάδα
    en: astronomical unit
    en_ca: astronomical unit
    en_gb: astronomical unit
    eo: astronomia unuo
    es: unidad astronómica
    et: astronoomiline ühik
    eu: unitate astronomiko
    fa: واحد نجومی
    fi: astronominen yksikkö
    fr: unité astronomique
    frr: astronoomisk ianhaid
    ga: aonad réalteolaíoch
    gl: unidade astronómica
    gn: unidad astronómica
    gsw: astronomischi Einheit
    gv: unnid rollageydagh
    he: יחידה אסטרונומית
    hi: खगोलीय इकाई
    hr: astronomska jedinica
    hu: csillagászati egység
    hy: աստղագիտական միավոր
    id: satuan astronomi
    ig: otu ihe na-enyocha mbara igwe
    ilo: astronomikal a yunit
    io: astronomiala unajo
    is: stjarnfræðieining
    it: unità astronomica
    ja: 天文単位
    jv: satuan astronomi
    ka: ასტრონომიული ერთეული
    kab: tigget tasnullant
    kk: астрономиялық бірлік
    kn: ಖಗೋಳ ಮಾನ
    ko: 천문단위
    ku: yekeya astronomîk
    la: unitas astronomica
    lb: astronomesch Eenheet
    lfn: unia astronomial
    li: astronomische einheid
    lmo: unità astronomica
    lt: astronominis vienetas
    lv: astronomiskā vienība
    mk: астрономска единица
    ml: സൗരദൂരം
    mn: одон орны нэгж
    mr: खगोलीय एकक
    ms: unit astronomi
    ms_arab: اونيت استرونومي
    mwl: ounidade astronómica
    my: astronomical unit
    nan: thian-bûn tan-ūi
    nb: astronomisk enhet
    nds: astronoomsch Eenheit
    nds_nl: Astronomiske Eynheid
    nl: astronomische eenheid
    nn: astronomisk eining
    nqo: ߛߊ߲ߡߊߛߓߍ ߘߊ߲ߕߊ
    oc: unitat astronomica
    os: астрономон иуæг
    pa: ਖਗੋਲੀ ਇਕਾਈ
    pap: unidat astronómiko
    pap-aw: unidad astronomico
    pl: jednostka astronomiczna
    pms: unità astronòmica
    pnb: اسمانی میل
    ps: نجومي واحد
    pt: unidade astronómica
    pt_br: unidade(s) astronômica(s)
    ro: UA
    ru: астрономическая единица
    rue: астрономична единка
    sah: астроном кээмэйэ
    sc: unidadi astronòmica
    scn: unità astrunòmica
    sco: astronomical unit
    sd: آسٽروناميڪل يونٽ
    sh: astronomska jedinica
    sk: astronomická jednotka
    sl: astronomska enota
    so: astronomical unit
    sr: астрономска јединица
    stq: astronomiske Eenhaid
    su: Hijian Astronomi
    sv: astronomisk enhet
    sw: kizio astronomia
    ta: வானியல் அலகு
    tg: воҳиди нуҷумӣ
    th: หน่วยดาราศาสตร์
    tk: Astronomik birlik
    tl: yunit na pang-astronomiya
    tr: astronomik birim
    tt: астрономик берәмлек
    tt_cyrl: астрономик берәмлек
    tyv: Астрономнуг хемчеглер
    udm: астрономической единица
    uk: астрономічна одиниця
    ur: فلکیاتی اکائی
    uz: astronomik birlik
    vi: đơn vị thiên văn
    war: yunit astronomiko
    wuu: 天文单位
    xmf: ასტრონომიული ართული
    ybb: lecʉte anū anɛ nu
    yi: אסטראנאמישע איינס
    yue: 天文單位
    zh: 天文單位
    zh_cn: 天文单位
    zh_hans: 天文单位
    zh_hant: 天文單位
    zh_hk: 天文單位
    zh_mo: 天文單位
    zh-my: 天文单位
    zh_sg: 天文单位
    zh_tw: 天文單位
---

# [[Astronomical_Unit]] 

#is_/same_as :: [[../../WikiData/WD~Astronomical_unit,1811|WD~Astronomical_unit,1811]] 

## #has_/text_of_/abstract 

> The astronomical unit (symbol: au or AU) is a unit of length 
> defined to be exactly equal to 149597870700 m. 
> 
> Historically, the astronomical unit was conceived as the **average Earth-Sun distance** 
> (the average of Earth's aphelion and perihelion), before its modern redefinition in 2012.
>
> The astronomical unit is used primarily for measuring distances 
> within the Solar System or around other stars. 
> It is also a fundamental component in the definition of another unit of astronomical length, 
> the parsec. One au is approximately equivalent to 499 light-seconds.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Astronomical%20unit) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Astronomy/Solar_System/Astronomical_Unit|Astronomical_Unit]] 

### #is_/same_as :: [[/_public/Astronomy/Solar_System/Astronomical_Unit.public|Astronomical_Unit.public]] 

### #is_/same_as :: [[/_internal/Astronomy/Solar_System/Astronomical_Unit.internal|Astronomical_Unit.internal]] 

### #is_/same_as :: [[/_protect/Astronomy/Solar_System/Astronomical_Unit.protect|Astronomical_Unit.protect]] 

### #is_/same_as :: [[/_private/Astronomy/Solar_System/Astronomical_Unit.private|Astronomical_Unit.private]] 

### #is_/same_as :: [[/_personal/Astronomy/Solar_System/Astronomical_Unit.personal|Astronomical_Unit.personal]] 

### #is_/same_as :: [[/_secret/Astronomy/Solar_System/Astronomical_Unit.secret|Astronomical_Unit.secret]] 

