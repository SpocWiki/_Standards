---
aliases:
- Cruthú Gödel
- Các định lý bất toàn của Gödel
- Gedelove teoreme o nepotpunosti
- Godel-teorio
- Gödel eksiklik teoremi
- Gödel első nemteljességi tétele
- Gödel's Incompleteness Theorems
- Gödel's_incompleteness_theorems
- Gödelen ez-osotasunaren teoremak
- Gödeli mittetäielikkuse teoreem
- Gödelin epätäydellisyyslause
- Gödelova veta o neúplnosti
- Gödelove teoreme nepotpunosti
- Gödelovi teoremi nepotpunosti
- Gödelovy věty o neúplnosti
- Gödels ofullständighetssats
- Gödels ufuldstændighedssætning
- Gödels ufullstendighetsteoremer
- Gödelscher Unvollständigkeitssatz
- Gödlova izreka o nepopolnosti
- onvolledigheidsstellingen van Gödel
- teorema d'incompletesa de Gödel
- teorema da incompletude de Gödel
- Teorema ketaklengkapan Gödel
- teoremas da incompletude de Gödel
- teoremas de incompletitud de Gödel
- Teoreme de Gödel
- teoremele de incompletitudine ale lui Gödel
- teoremes d'incompletitú de Gödel
- teoremi di incompletezza di Gödel
- teoremoj de nekompleteco
- Theoremata Gödel de imperfectione
- théorèmes d'incomplétude de Gödel
- Tiurema d'incumplitizza di Gödel
- Twierdzenie Gödla
- Ufullstendigheitsteorema
- Θεωρήματα μη πληρότητας του Γκέντελ
- Бүрэн бус байдлын тухай Гёдэлын теоремууд
- Гёделĕн туллимарлăх теореми
- Геделове теореме о непотпуности
- Геделови теореми за непотполноста
- Теорема Гёделя о неполноте
- Теорема на Гьодел за непълнота
- Теореми Геделя про неповноту
- тэарэмы Гёдэля аб няпоўнасьці
- משפטי האי-שלמות של גדל
- قضایای ناتمامیت گودل
- مبرهنات عدم الاكتمال لغودل
- गाडेल का अपूर्णता प्रमेय
- ਗੋਇਡਲ ਦੀਆਂ ਅਪੂਰਨਤਾ ਦੀਆਂ ਥਿਊਰਮਾਂ
- கோடெலின் முழுமையற்ற தேற்றங்கள்
- గాడెల్ అసంపూర్ణత సిద్ధాంతం
- ಗೋಡೆಲ್ರ ಅಪೂರ್ಣತೆಯ ಪ್ರಮೇಯ
- ทฤษฎีบทความไม่บริบูรณ์ของเกอเดล
- ဂူဒယ်၏ မပြည့်စုံခြင်းသီအိုရမ်များ
- გოდელის არასრულობის თეორემები
- ゲーデルの不完全性定理
- 哥德唔完備定理
- 哥德尔不完备定理
- 哥德爾不完備定理
- 불완전성 정리
has_id_wikidata: Q200787
instance_of: '[[_Standards/WikiData/WD~theorem,65943|WD~theorem,65943]]'
studied_by: '[[_Standards/WikiData/WD~foundations_of_mathematics,833585|WD~foundations_of_mathematics,833585]]'
different_from: "[[_Standards/WikiData/WD~Gödel's_completeness_theorem,902052|WD~Gödel's_completeness_theorem,902052]]"
part_of: '[[_Standards/WikiData/WD~list_of_theorems,944443|WD~list_of_theorems,944443]]'
has_part_s_:
- "[[_Standards/WikiData/WD~Gödel's_second_incompleteness_theorem,1031460|WD~Gödel's_second_incompleteness_theorem,1031460]]"
- "[[_Standards/WikiData/WD~Godel's_first_incompleteness_theorem,62091930|WD~Godel's_first_incompleteness_theorem,62091930]]"
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
proved_by: '[[_Standards/WikiData/WD~Kurt_Gödel,41390|WD~Kurt_Gödel,41390]]'
named_after: '[[_Standards/WikiData/WD~Kurt_Gödel,41390|WD~Kurt_Gödel,41390]]'
Dewey_Decimal_Classification: 511.3
dv_has_:
  name_:
    ar: مبرهنات عدم الاكتمال لغودل
    ast: teoremes d'incompletitú de Gödel
    be_tarask: тэарэмы Гёдэля аб няпоўнасьці
    bg: Теорема на Гьодел за непълнота
    bs: Gödelove teoreme nepotpunosti
    ca: teorema d'incompletesa de Gödel
    cs: Gödelovy věty o neúplnosti
    cv: Гёделĕн туллимарлăх теореми
    da: Gödels ufuldstændighedssætning
    de: Gödelscher Unvollständigkeitssatz
    el: Θεωρήματα μη πληρότητας του Γκέντελ
    en: Gödel's incompleteness theorems
    eo: teoremoj de nekompleteco
    es: teoremas de incompletitud de Gödel
    et: Gödeli mittetäielikkuse teoreem
    eu: Gödelen ez-osotasunaren teoremak
    fa: قضایای ناتمامیت گودل
    fi: Gödelin epätäydellisyyslause
    fr: théorèmes d'incomplétude de Gödel
    ga: Cruthú Gödel
    gl: teorema da incompletude de Gödel
    gsw: Gödelscher Unvollständigkeitssatz
    he: משפטי האי-שלמות של גדל
    hi: गाडेल का अपूर्णता प्रमेय
    hr: Gödelovi teoremi nepotpunosti
    hu: Gödel első nemteljességi tétele
    id: Teorema ketaklengkapan Gödel
    io: Godel-teorio
    it: teoremi di incompletezza di Gödel
    ja: ゲーデルの不完全性定理
    ka: გოდელის არასრულობის თეორემები
    kn: ಗೋಡೆಲ್ರ ಅಪೂರ್ಣತೆಯ ಪ್ರಮೇಯ
    ko: 불완전성 정리
    la: Theoremata Gödel de imperfectione
    mk: Геделови теореми за непотполноста
    mn: Бүрэн бус байдлын тухай Гёдэлын теоремууд
    my: ဂူဒယ်၏ မပြည့်စုံခြင်းသီအိုရမ်များ
    nb: Gödels ufullstendighetsteoremer
    nl: onvolledigheidsstellingen van Gödel
    nn: Ufullstendigheitsteorema
    nov: Teoreme de Gödel
    pa: ਗੋਇਡਲ ਦੀਆਂ ਅਪੂਰਨਤਾ ਦੀਆਂ ਥਿਊਰਮਾਂ
    pl: Twierdzenie Gödla
    pt: teoremas da incompletude de Gödel
    pt_br: teoremas da incompletude de Gödel
    ro: teoremele de incompletitudine ale lui Gödel
    ru: Теорема Гёделя о неполноте
    scn: Tiurema d'incumplitizza di Gödel
    sk: Gödelova veta o neúplnosti
    sl: Gödlova izreka o nepopolnosti
    sr: Геделове теореме о непотпуности
    sr_ec: Геделове теореме о непотпуности
    sr_el: Gedelove teoreme o nepotpunosti
    sv: Gödels ofullständighetssats
    ta: கோடெலின் முழுமையற்ற தேற்றங்கள்
    te: గాడెల్ అసంపూర్ణత సిద్ధాంతం
    th: ทฤษฎีบทความไม่บริบูรณ์ของเกอเดล
    tr: Gödel eksiklik teoremi
    uk: Теореми Геделя про неповноту
    ur: Gödel's incompleteness theorems
    vi: Các định lý bất toàn của Gödel
    yue: 哥德唔完備定理
    zh: 哥德尔不完备定理
    zh_cn: 哥德尔不完备定理
    zh_hans: 哥德尔不完备定理
    zh_hant: 哥德爾不完備定理
    zh_hk: 哥德爾不完備定理
    zh_sg: 哥德尔不完备定理
    zh_tw: 哥德爾不完備定理
---

# [[Gödel's_Incompleteness_Theorems]] 

#is_/same_as :: [[../../../../WikiData/WD~Gödel's_incompleteness_theorems,200787|WD~Gödel's_incompleteness_theorems,200787]]  
## #has_/text_of_/abstract 

> Gödel's incompleteness theorems are two theorems of mathematical logic 
> that are concerned with the limits of provability in formal axiomatic theories. 
> 
> These results, published by Kurt Gödel in 1931, 
> are important both in mathematical logic and in the philosophy of mathematics. 
> The theorems are interpreted as showing that Hilbert's program 
> to find a complete and consistent set of axioms for all mathematics is impossible.
>
> The first incompleteness theorem states that no consistent system of axioms 
> whose theorems can be listed by an effective procedure (i.e. an algorithm) 
> is capable of proving all truths about the arithmetic of natural numbers. 
> 
> For any such consistent formal system, 
> there will always be statements about natural numbers that are true, 
> but that are unprovable within the system. 
> Equivalently, there will always be statements about natural numbers that are false, 
> but that are unprovably false within the system.
>
> The second incompleteness theorem, an extension of the first, shows that the system cannot demonstrate its own consistency.
>
> Employing a diagonal argument, Gödel's incompleteness theorems were among the first of 
> several closely related theorems on the limitations of formal systems. 
> They were followed by Tarski's undefinability theorem on the formal undefinability of truth, 
> Church's proof that Hilbert's Entscheidungsproblem is unsolvable, 
> and Turing's theorem that there is no algorithm to solve the halting problem.
>
> [Wikipedia](https://en.wikipedia.org/wiki/G%C3%B6del's%20incompleteness%20theorems) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Mathematician/Modern_Mathematician/Gödel,Kurt/Gödel's_Incompleteness_Theorems|Gödel's_Incompleteness_Theorems]] 

### #is_/same_as :: [[/_public/Mathematics/Mathematician/Modern_Mathematician/Gödel,Kurt/Gödel's_Incompleteness_Theorems.public|Gödel's_Incompleteness_Theorems.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Mathematician/Modern_Mathematician/Gödel,Kurt/Gödel's_Incompleteness_Theorems.internal|Gödel's_Incompleteness_Theorems.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Mathematician/Modern_Mathematician/Gödel,Kurt/Gödel's_Incompleteness_Theorems.protect|Gödel's_Incompleteness_Theorems.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Mathematician/Modern_Mathematician/Gödel,Kurt/Gödel's_Incompleteness_Theorems.private|Gödel's_Incompleteness_Theorems.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Mathematician/Modern_Mathematician/Gödel,Kurt/Gödel's_Incompleteness_Theorems.personal|Gödel's_Incompleteness_Theorems.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Mathematician/Modern_Mathematician/Gödel,Kurt/Gödel's_Incompleteness_Theorems.secret|Gödel's_Incompleteness_Theorems.secret]] 

