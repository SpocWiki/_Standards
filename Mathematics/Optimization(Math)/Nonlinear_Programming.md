---
aliases:
- icke-linjär optimering
- nelineara optimumigo
- nelinearno programiranje
- Nelineární programování
- nemlineáris programozás
- nichtlineare Optimierung
- nonlinear programming
- optimisation non linéaire
- programació no lineal
- programación no lineal
- Programación non linear
- programación non llinial
- programare neliniară
- Programazio ez lineal
- Programação não linear
- programmazione non-lineare
- programowanie nieliniowe
- μη Γραμμικός Προγραμματισμός
- нелінійне програмування
- нелінейнае праграмаваньне
- нелинейное программирование
- ոչ գծային ծրագրավորում
- תכנון לא-לינארי
- برمجة غير خطية
- برنامهسازی غیرخطی
- अरैखिक प्रोग्रामन
- 非線形計画法
- 非線性規劃
- 非线性规划
- 비선형 계획법
has_id_wikidata: Q769909
subclass_of:
- '[[_Standards/WikiData/WD~optimization_problem,984063|WD~optimization_problem,984063]]'
- '[[_Standards/WikiData/WD~mathematical_programming,10578306|WD~mathematical_programming,10578306]]'
Stack_Exchange_tag: https://or.stackexchange.com/tags/nonlinear-programming
dv_has_:
  name_:
    ar: برمجة غير خطية
    ast: programación non llinial
    be_tarask: нелінейнае праграмаваньне
    ca: programació no lineal
    cs: Nelineární programování
    de: nichtlineare Optimierung
    el: μη Γραμμικός Προγραμματισμός
    en: nonlinear programming
    eo: nelineara optimumigo
    es: programación no lineal
    eu: Programazio ez lineal
    fa: برنامهسازی غیرخطی
    fr: optimisation non linéaire
    gl: Programación non linear
    he: תכנון לא-לינארי
    hi: अरैखिक प्रोग्रामन
    hu: nemlineáris programozás
    hy: ոչ գծային ծրագրավորում
    it: programmazione non-lineare
    ja: 非線形計画法
    ko: 비선형 계획법
    pl: programowanie nieliniowe
    pt: Programação não linear
    pt_br: programação não linear
    ro: programare neliniară
    ru: нелинейное программирование
    sl: nelinearno programiranje
    sr: Nelinearno programiranje
    sv: icke-linjär optimering
    uk: нелінійне програмування
    yue: 非線性規劃
    zh: 非线性规划
---

# [[Nonlinear_Programming]] 

#is_/similar_to :: [[Linear_Programming|Linear_Programming]] 
#is_/same_as :: [[_Standards/WikiData/WD~Nonlinear_programming,769909|WD~Nonlinear_programming,769909]] 

## #has_/text_of_/abstract 

> In mathematics, nonlinear programming (NLP) 
> is the process of solving an optimization problem 
> where some of the constraints are not linear equalities 
> or the objective function is not a linear function. 
> 
> An optimization problem is one of calculation of the extrema 
> (maxima, minima or stationary points) of an objective function 
> over a set of unknown real variables 
> and conditional to the satisfaction of a system of equalities and inequalities, 
> collectively termed constraints. 
> 
> It is the sub-field of mathematical optimization that deals with problems that are not linear.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Nonlinear%20programming) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Optimization(Math)/Nonlinear_Programming|Nonlinear_Programming]] 

### #is_/same_as :: [[/_public/Mathematics/Optimization(Math)/Nonlinear_Programming.public|Nonlinear_Programming.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Optimization(Math)/Nonlinear_Programming.internal|Nonlinear_Programming.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Optimization(Math)/Nonlinear_Programming.protect|Nonlinear_Programming.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Optimization(Math)/Nonlinear_Programming.private|Nonlinear_Programming.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Optimization(Math)/Nonlinear_Programming.personal|Nonlinear_Programming.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Optimization(Math)/Nonlinear_Programming.secret|Nonlinear_Programming.secret]] 

