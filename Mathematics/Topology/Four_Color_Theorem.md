---
aliases:
- 4색 정리
- dörd boya problemi
- Dört renk teoremi
- Firfargeproblemet
- firfarveproblemet
- four color map theorem
- four color theorem
- four colour theorem
- Four_Color_Theorem
- fyrfärgssatsen
- izrek štirih barv
- Keturių spalvų teorema
- kvarkolormapa teoremo
- Lau koloreen teorema
- Neliväriongelma
- Neljavärviprobleem
- négyszín-tétel
- Poučak o četiri boje
- Problemo di quar kolori
- problém čtyř barev
- pôs y pedwar lliw
- Schauer-klöören-teoreem
- teoirim na gceithre dhath
- Teorema celor patru culori
- teorema das catro cores
- teorema das quatro cores
- teorema de los cuatro colores
- teorema dei quattro colori
- Teorema dels quatre colors
- Teorema dij quatr color
- Teorema e 4 ngjyrave
- teorema empat warna
- Teorema četiri boje
- théorème des quatre couleurs
- Tiurema dî quattru culura
- twierdzenie o czterech barwach
- Vier-Farben-Satz
- vierkleurenstelling
- Četru krāsu teorēma
- Định lý bốn màu
- Θεώρημα των τεσσάρων χρωμάτων
- задача за четирите цвята
- проблема четырёх красок
- проблема чотирьох фарб
- тэарэма аб чатырох фарбах
- չորս գույների թեորեմ
- משפט ארבעת הצבעים
- تیۆرمی چوار ڕەنگ
- قضیه چهاررنگ
- مبرهنة الألوان الأربعة
- چار رنگی مسئلہ
- चार रंग की प्रमेय
- চার বর্ণ উপপাদ্য
- நான்கு நிறத் தேற்றம்
- നാലുനിറസിദ്ധാന്തം
- ทฤษฎีบทสี่สี
- ოთხი ფერის პრობლემა
- 四色定理
has_id_wikidata: Q184410
instance_of:
- '[[_Standards/WikiData/WD~theorem,65943|WD~theorem,65943]]'
- '[[_Standards/WikiData/WD~computer_assisted_proof,1150675|WD~computer_assisted_proof,1150675]]'
- '[[_Standards/WikiData/WD~proof_by_exhaustion,9299950|WD~proof_by_exhaustion,9299950]]'
studied_by: '[[_Standards/WikiData/WD~graph_theory,131476|WD~graph_theory,131476]]'
proved_by:
- '[[_Standards/WikiData/WD~Kenneth_Appel,224787|WD~Kenneth_Appel,224787]]'
- '[[_Standards/WikiData/WD~Wolfgang_Haken,65082|WD~Wolfgang_Haken,65082]]'
statement_describes:
- '[[_Standards/WikiData/WD~graph_coloring,504843|WD~graph_coloring,504843]]'
- '[[_Standards/WikiData/WD~planar_graph,547823|WD~planar_graph,547823]]'
part_of: '[[_Standards/WikiData/WD~list_of_theorems,944443|WD~list_of_theorems,944443]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
uses: '[[_Standards/WikiData/WD~proof_assistant,11387554|WD~proof_assistant,11387554]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Th%C3%A9or%C3%A8me%20des%20quatre%20couleurs.jpg
Commons_category: Four-color theorem
nickname: Guthrie's problem
dv_has_:
  name_:
    ar: مبرهنة الألوان الأربعة
    ast: teorema de los cuatro colores
    az: dörd boya problemi
    be_tarask: тэарэма аб чатырох фарбах
    bg: задача за четирите цвята
    bn: চার বর্ণ উপপাদ্য
    ca: Teorema dels quatre colors
    ckb: تیۆرمی چوار ڕەنگ
    cs: problém čtyř barev
    cy: pôs y pedwar lliw
    da: firfarveproblemet
    de: Vier-Farben-Satz
    de_ch: Vier-Farben-Satz
    el: Θεώρημα των τεσσάρων χρωμάτων
    en: four color theorem
    en_ca: four colour theorem
    en_gb: four colour theorem
    eo: kvarkolormapa teoremo
    es: teorema de los cuatro colores
    et: Neljavärviprobleem
    eu: Lau koloreen teorema
    fa: قضیه چهاررنگ
    fi: Neliväriongelma
    fr: théorème des quatre couleurs
    frr: Schauer-klöören-teoreem
    ga: teoirim na gceithre dhath
    gl: teorema das catro cores
    he: משפט ארבעת הצבעים
    hi: चार रंग की प्रमेय
    hr: Poučak o četiri boje
    hu: négyszín-tétel
    hy: չորս գույների թեորեմ
    id: teorema empat warna
    io: Problemo di quar kolori
    it: teorema dei quattro colori
    ja: 四色定理
    ka: ოთხი ფერის პრობლემა
    ko: 4색 정리
    lt: Keturių spalvų teorema
    lv: Četru krāsu teorēma
    ml: നാലുനിറസിദ്ധാന്തം
    nl: vierkleurenstelling
    nn: Firfargeproblemet
    pl: twierdzenie o czterech barwach
    pms: Teorema dij quatr color
    pt: teorema das quatro cores
    pt_br: teorema das quatro cores
    ro: Teorema celor patru culori
    ru: проблема четырёх красок
    scn: Tiurema dî quattru culura
    sl: izrek štirih barv
    sq: Teorema e 4 ngjyrave
    sr: Teorema četiri boje
    sv: fyrfärgssatsen
    ta: நான்கு நிறத் தேற்றம்
    th: ทฤษฎีบทสี่สี
    tr: Dört renk teoremi
    uk: проблема чотирьох фарб
    ur: چار رنگی مسئلہ
    vi: Định lý bốn màu
    wuu: 四色定理
    yue: 四色定理
    zh: 四色定理
    zh_cn: 四色定理
    zh_hans: 四色定理
    zh_hant: 四色定理
    zh_hk: 四色定理
    zh_sg: 四色定理
    zh_tw: 四色定理
---

# [[Four_Color_Theorem]] 

#is_/same_as :: [[../../WikiData/WD~Four_color_theorem,184410|WD~Four_color_theorem,184410]] 

## #has_/text_of_/abstract 

> In mathematics, the four color theorem, or the four color map theorem, states that 
> no more than four colors are required to color the regions of any map 
> so that no two adjacent regions have the same color. 
> 
> Adjacent means that two regions share a common boundary of non-zero length 
> (i.e., not merely a corner where three or more regions meet). 
> It was the first major theorem to be proved using a computer. 
> Initially, this proof was not accepted by all mathematicians 
> because the computer-assisted proof was infeasible for a human to check by hand. 
> The proof has gained wide acceptance since then, although some doubts remain.
>
> The theorem is a stronger version of the five color theorem, 
> which can be shown using a significantly simpler argument. 
> Although the weaker five color theorem was proven already in the 1800s, 
> the four color theorem resisted until 1976 
> when it was proven by Kenneth Appel and Wolfgang Haken in a computer-aided proof. 
> This came after many false proofs and mistaken counterexamples in the preceding decades.
>
> The Appel–Haken proof proceeds by analyzing a very large number of reducible configurations. 
> This was improved upon in 1997 by Robertson, Sanders, Seymour, and Thomas, 
> who have managed to decrease the number of such configurations to 633 – 
> still an extremely long case analysis. 
> In 2005, the theorem was verified by Georges Gonthier using a general-purpose theorem-proving software.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Four%20color%20theorem) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Topology/Four_Color_Theorem|Four_Color_Theorem]] 

### #is_/same_as :: [[/_public/Mathematics/Topology/Four_Color_Theorem.public|Four_Color_Theorem.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Topology/Four_Color_Theorem.internal|Four_Color_Theorem.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Topology/Four_Color_Theorem.protect|Four_Color_Theorem.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Topology/Four_Color_Theorem.private|Four_Color_Theorem.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Topology/Four_Color_Theorem.personal|Four_Color_Theorem.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Topology/Four_Color_Theorem.secret|Four_Color_Theorem.secret]] 

