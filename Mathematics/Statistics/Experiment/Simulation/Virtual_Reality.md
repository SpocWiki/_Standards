---
aliases:
- Errealitate birtual
- Hi-gí si̍t-kéng
- Indwalazo
- Kasunyatan amurta
- navidezna resničnost
- realidad virtual
- Realidade virtual
- Realidat virtual
- Realidá virtual
- realitas virtual
- realitas virtualis
- realitat virtual
- Realitat virtuala
- Realitate virtuală
- Realiteti virtual
- realiti maya
- Realtà virtual
- realtà virtuale
- Reyalidad birtuwal
- rhithwir
- rzeczywistość wirtualna
- réalité virtuelle
- réaltacht fhíorúil
- sanal gerçeklik
- Sirma sullull kay
- skynwerklikheid
- Sýndarveruleiki
- thực tế ảo
- virtuaalitodellisuus
- virtuaalreaalsus
- virtuaalʼlaž tuõttvuõtt
- virtual reality
- Virtual realitá
- Virtual reallik
- Virtual reallıq
- virtuala realeco
- Virtuala Realeso (VR)
- virtuali realybė
- virtualna stvarnost
- Virtualni svijet
- virtuele warkelikheid
- virtuell verklighet
- virtuell virkelighet
- virtuelle Realität
- Virtuelna stvarnost
- virtuála duođalašvuohta
- virtuális valóság
- virtuálna realita
- virtuální realita
- virtuālā realitāte
- Vïrtwaldı şındıq
- Εικονική πραγματικότητα
- Віртуална реалность
- віртуальна реальність
- віртуальная рэальнасць
- віртуальная рэальнасьць
- Виртуал бодит байдал
- Виртуалдуу аныктык
- Виртуалды шындық
- Виртуална реалност
- виртуальная реальность
- Виртуелна реалност
- Виртуелна стварност
- Վիրտուալ իրականություն
- מציאות מדומה
- رياليتي ماي
- مجازی حقیقت
- مجازی واقعیت
- مەۋھۇم رېئاللىق
- واقع افتراضي
- واقعیت مجازی
- ڕاستیی خەیاڵی
- ۆىيرتۋالدى شىندىق
- आभासी यथार्थ
- অপ্রকৃত বাস্তবতা
- ਆਭਾਸੀ ਅਸਲੀਅਤ
- தோற்ற மெய்மை
- వర్చువల్ రియాలిటీ
- ವರ್ಚುಯಲ್ ರಿಯಾಲಿಟಿ
- വെർച്വൽ റിയാലിറ്റി
- ความเป็นจริงเสมือน
- རྟོག་བཟོས་དངོས་ཡོད་ཁོར་ཡུག་ཅེས་པ་
- အသွင်ယောင် ပကတိ
- ვირტუალური რეალობა
- バーチャルリアリティ
- 虚拟实境
- 虚拟现实
- 虛擬實境
- 가상 현실
has_id_wikidata: Q170519
ArchWiki_article: Virtual_reality
ComputerLanguage_com_definition: virtual+reality
Dewey_Decimal_Classification: 006.8
has_use:
- '[[_Standards/WikiData/WD~entertainment,173799]]'
- '[[_Standards/WikiData/WD~training,918385]]'
- '[[_Standards/WikiData/WD~new_product_development,1395034]]'
- '[[_Standards/WikiData/WD~science,336]]'
- '[[_Standards/WikiData/WD~art,735]]'
- '[[_Standards/WikiData/WD~video_game,7889]]'
- '[[_Standards/WikiData/WD~military,8473]]'
facet_of: '[[_Standards/WikiData/WD~human_computer_interaction,207434]]'
different_from:
- '[[_Standards/WikiData/WD~augmented_reality,254183]]'
- '[[_Standards/WikiData/WD~simulated_reality,785290]]'
- '[[_Standards/WikiData/WD~virtual_environment,55950686]]'
opposite_of: '[[_Standards/WikiData/WD~real_life,1058876]]'
has_characteristic: '[[_Standards/WikiData/WD~immersion,1660060]]'
subclass_of:
- '[[_Standards/WikiData/WD~extended_reality,25052165]]'
- '[[_Standards/WikiData/WD~virtual_place,107307154]]'
- '[[../../../../WikiData/WD~simulation,45045]]'
part_of: '[[_Standards/WikiData/WD~virtual_universe,107307169]]'
described_by_source: '[[_Standards/WikiData/WD~Virtual_Reality,133280563]]'
IAB_code:
- 639
- 1029
GitHub_topic:
- vr
- virtual-reality
hashtag: vr
OmegaWiki_Defined_Meaning: 689916
IMDb_keyword: virtual-reality
PhilPapers_topic: virtual-reality
subreddit: virtualreality
short_name: VR
Commons_category: Virtual reality
ACM_Classification_Code_2012_: 10010866
U_S_National_Archives_Identifier: 10675494
image: http://commons.wikimedia.org/wiki/Special:FilePath/Reality%20check%20ESA384313.jpg
spherical_panorama_image: http://commons.wikimedia.org/wiki/Special:FilePath/Santo%20Ecce-Homo%20-%20Crucero.jpg
Stack_Exchange_tag: https://stackoverflow.com/tags/virtual-reality
IEV_number: 171-09-13
UMLS_CUI: C0871582
MeSH_tree_code:
- L01.224.160.875
- L01.296.555
Krugosvet_article_archived_: nauka_i_tehnika/transport_i_svyaz/VIRTUALNAYA_REALNOST.html
dv_has_:
  name_:
    af: skynwerklikheid
    an: Realidat virtual
    ar: واقع افتراضي
    ast: Realidá virtual
    az: Virtual reallıq
    azb: مجازی واقعیت
    be: віртуальная рэальнасць
    be_tarask: віртуальная рэальнасьць
    bg: Виртуална реалност
    bn: অপ্রকৃত বাস্তবতা
    bo: རྟོག་བཟོས་དངོས་ཡོད་ཁོར་ཡུག་ཅེས་པ་
    bs: virtualna stvarnost
    ca: realitat virtual
    ckb: ڕاستیی خەیاڵی
    cs: virtuální realita
    cy: rhithwir
    da: virtual reality
    de: virtuelle Realität
    el: Εικονική πραγματικότητα
    en: virtual reality
    en_gb: virtual reality
    eo: virtuala realeco
    es: realidad virtual
    et: virtuaalreaalsus
    eu: Errealitate birtual
    fa: واقعیت مجازی
    fi: virtuaalitodellisuus
    fr: réalité virtuelle
    ga: réaltacht fhíorúil
    gl: Realidade virtual
    haw: Virtual reality
    he: מציאות מדומה
    hi: आभासी यथार्थ
    hr: Virtualni svijet
    hu: virtuális valóság
    hy: Վիրտուալ իրականություն
    id: realitas virtual
    ie: Virtual realitá
    io: Virtuala Realeso (VR)
    is: Sýndarveruleiki
    it: realtà virtuale
    ja: バーチャルリアリティ
    jv: Kasunyatan amurta
    ka: ვირტუალური რეალობა
    kk: Виртуалды шындық
    kk_arab: ۆىيرتۋالدى شىندىق
    kk-cn: ۆىيرتۋالدى شىندىق
    kk_cyrl: Виртуалды шындық
    kk-kz: Виртуалды шындық
    kk_latn: Vïrtwaldı şındıq
    kk-tr: Vïrtwaldı şındıq
    kn: ವರ್ಚುಯಲ್ ರಿಯಾಲಿಟಿ
    ko: 가상 현실
    ky: Виртуалдуу аныктык
    la: realitas virtualis
    lt: virtuali realybė
    lv: virtuālā realitāte
    mk: Виртуелна реалност
    ml: വെർച്വൽ റിയാലിറ്റി
    mn: Виртуал бодит байдал
    ms: realiti maya
    ms_arab: رياليتي ماي
    my: အသွင်ယောင် ပကတိ
    nan: Hi-gí si̍t-kéng
    nb: virtuell virkelighet
    nds_nl: virtuele warkelikheid
    nl: virtual reality
    oc: Realitat virtuala
    pa: ਆਭਾਸੀ ਅਸਲੀਅਤ
    pl: rzeczywistość wirtualna
    pms: Realtà virtual
    pnb: مجازی حقیقت
    pt: realidade virtual
    qu: Sirma sullull kay
    ro: Realitate virtuală
    ru: виртуальная реальность
    rue: Віртуална реалность
    se: virtuála duođalašvuohta
    sh: Virtuelna stvarnost
    sk: virtuálna realita
    sl: navidezna resničnost
    sms: virtuaalʼlaž tuõttvuõtt
    sq: Realiteti virtual
    sr: виртуелна стварност
    sr_ec: Виртуелна стварност
    sr_el: Virtuelna stvarnost
    sv: virtuell verklighet
    ta: தோற்ற மெய்மை
    te: వర్చువల్ రియాలిటీ
    th: ความเป็นจริงเสมือน
    tr: sanal gerçeklik
    ug: مەۋھۇم رېئاللىق
    uk: віртуальна реальність
    ur: مجازی حقیقت
    uz: Virtual reallik
    vi: thực tế ảo
    war: Reyalidad birtuwal
    wuu: 虚拟现实
    yue: 虛擬實境
    zh: 虚拟现实
    zh_cn: 虚拟现实
    zh_hans: 虚拟现实
    zh_hant: 虛擬實境
    zh_hk: 虛擬實境
    zh-my: 虚拟实境
    zh_sg: 虚拟实境
    zh_tw: 虛擬實境
    zu: Indwalazo
---
# [[Virtual_Reality]] 

#is_/same_as :: [[../../../../WikiData/WD~Virtual_reality,170519|WD~Virtual_reality,170519]]  

## #has_/text_of_/abstract 

> Virtual reality (VR) is a simulated experience that employs 3D near-eye displays 
> and pose tracking to give the user an immersive feel of a virtual world. 
> 
> Applications of virtual reality include entertainment (particularly video games), 
> education (such as medical, safety, or military training), 
> research  and business (such as virtual meetings). 
> 
> VR is one of the key technologies in the reality-virtuality continuum. 
> As such, it is different from other digital visualization solutions, 
> such as augmented virtuality and augmented reality.
>
> Currently, standard virtual reality systems use either virtual reality headsets 
> or multi-projected environments to generate some realistic images, sounds, 
> and other sensations that simulate a user's physical presence in a virtual environment. 
> 
> A person using virtual reality equipment is able to look around the artificial world, 
> move around in it, and interact with virtual features or items. 
> The effect is commonly created by VR headsets consisting of 
> a head-mounted display with a small screen in front of the eyes but can also be 
> created through specially designed rooms with multiple large screens. 
> 
> Virtual reality typically incorporates auditory and video feedback 
> but may also allow other types of sensory 
> and force feedback through haptic technology.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Virtual%20reality) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Statistics/Experiment/Simulation/Virtual_Reality|Virtual_Reality]] 

### #is_/same_as :: [[/_public/Mathematics/Statistics/Experiment/Simulation/Virtual_Reality.public|Virtual_Reality.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Statistics/Experiment/Simulation/Virtual_Reality.internal|Virtual_Reality.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Statistics/Experiment/Simulation/Virtual_Reality.protect|Virtual_Reality.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Statistics/Experiment/Simulation/Virtual_Reality.private|Virtual_Reality.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Statistics/Experiment/Simulation/Virtual_Reality.personal|Virtual_Reality.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Statistics/Experiment/Simulation/Virtual_Reality.secret|Virtual_Reality.secret]] 

