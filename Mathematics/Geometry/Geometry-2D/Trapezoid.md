---
aliases:
- gonyoina sambambiri
- hình thang
- koor
- lichobežník
- lichoběžník
- ngapan-apan
- parallelltrapets
- puolisuunnikas
- putuq
- traipéisiam
- trapec
- trapece
- trapecija
- trapecio
- trapeciu
- Trapeets
- trapes
- trapesa
- trapesium
- trapesiwm
- trapesiya
- trapesiýa
- trapesoid
- trapesoyd
- trapessi
- trapets
- trapetsiya
- Trapez
- trapezi
- trapezio
- trapezium
- trapezo
- Trapezoid
- trapezoido
- trapisa
- trapèze
- trapése
- trapésium
- trapéz
- trapézio
- yamuk
- τραπέζιο
- трапез
- трапец
- трапеція
- трапеци
- трапеций
- трапеция
- трапеция в широком смысле
- трапецыя
- трапэцыя
- սեղան
- טראפעז
- טרפז
- ذوزنقه
- شبه منحرف
- شکل منحرف
- نیمچەلاتەریب
- समलंब चौकोन
- समलम्ब चतुर्भुज
- ট্রাপিজিয়াম
- ਸਮਲੰਬ ਚਤੁਰਭੁਜ
- સમાંતરબાજુ ચતુષ્કોણ
- ଟ୍ରାପିଜିଅମ
- சரிவகம்
- సమలంబ చతుర్భుజం
- ലംബകം
- รูปสี่เหลี่ยมคางหมู
- ຮູບຄາງໝູ
- སྐས་དབྱིབས་
- ტრაპეცია
- ចតុកោណព្នាយ
- 台形
- 梯形
- 사다리꼴
has_id_wikidata: Q46303
described_by_source: '[[_Standards/WikiData/WD~Q2041543,2041543|WD~Q2041543,2041543]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
different_from: '[[_Standards/WikiData/WD~Trapez,12777716|WD~Trapez,12777716]]'
disjoint_union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486|WD~list_of_values_as_qualifiers,23766486]]'
subclass_of: '[[_Standards/WikiData/WD~convex_quadrilateral,39447036|WD~convex_quadrilateral,39447036]]'
has_part_s_:
- '[[_Standards/WikiData/WD~leg,117009003|WD~leg,117009003]]'
- '[[_Standards/WikiData/WD~base,117009001|WD~base,117009001]]'
- '[[_Standards/WikiData/WD~edge,26382|WD~edge,26382]]'
studied_by: '[[_Standards/WikiData/WD~geometry,8087|WD~geometry,8087]]'
has_characteristic: '[[_Standards/WikiData/WD~area,11500|WD~area,11500]]'
has_facet_polytope: '[[_Standards/WikiData/WD~edge,26382|WD~edge,26382]]'
OmegaWiki_Defined_Meaning: 375310
image: http://commons.wikimedia.org/wiki/Special:FilePath/Trapezoid%20special%20cases.svg
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q1571%20%28mar%29-Vj18081991-%E0%A4%B8%E0%A4%AE%E0%A4%B2%E0%A4%82%E0%A4%AC%20%E0%A4%9A%E0%A5%8C%E0%A4%95%E0%A5%8B%E0%A4%A8.wav
Commons_category: Trapezoids
dv_has_:
  name_:
    af: trapesium
    ar: شبه منحرف
    ast: trapeciu
    az: trapesiya
    ba: трапеция
    bcl: trapesoyd
    be: трапецыя
    be_tarask: трапэцыя
    bg: трапец
    bn: ট্রাপিজিয়াম
    bo: སྐས་དབྱིབས་
    br: trapez
    bs: trapez
    ca: trapezi
    ckb: نیمچەلاتەریب
    cs: lichoběžník
    cv: трапеци
    cy: trapesiwm
    da: trapez
    de: Trapez
    el: τραπέζιο
    en: trapezoid
    en_gb: trapezium
    en-us: trapezoid
    eo: trapezo
    es: trapecio
    et: trapets
    eu: trapezio
    fa: ذوزنقه
    fi: puolisuunnikas
    fr: trapèze
    frr: Trapeets
    ga: traipéisiam
    gl: trapecio
    gu: સમાંતરબાજુ ચતુષ્કોણ
    he: טרפז
    hi: समलम्ब चतुर्भुज
    hr: trapez
    hsb: trapec
    hu: trapéz
    hy: սեղան
    ia: trapezio
    id: trapesium
    io: trapezoido
    is: trapisa
    it: trapezio
    ja: 台形
    jv: ngapan-apan
    ka: ტრაპეცია
    kk: трапеция
    km: ចតុកោណព្នាយ
    ko: 사다리꼴
    ky: трапеция
    la: trapezium
    lmo: trapése
    lo: ຮູບຄາງໝູ
    lt: trapecija
    lv: trapece
    mhr: трапеций
    mk: трапез
    ml: ലംബകം
    mn: трапец
    mr: समलंब चौकोन
    ms: trapezium
    nb: trapes
    nl: trapezium
    nn: trapes
    or: ଟ୍ରାପିଜିଅମ
    pa: ਸਮਲੰਬ ਚਤੁਰਭੁਜ
    pl: trapez
    pms: trapessi
    pt: trapézio
    pt_br: trapézio
    qu: putuq
    ro: trapez
    ru: трапеция в широком смысле
    se: trapesa
    sh: trapez
    sk: lichobežník
    sl: trapez
    sn: gonyoina sambambiri
    so: koor
    sr: трапез
    sr_el: trapez
    su: trapésium
    sv: parallelltrapets
    szl: trapez
    ta: சரிவகம்
    te: సమలంబ చతుర్భుజం
    th: รูปสี่เหลี่ยมคางหมู
    tk: trapesiýa
    tl: trapesoid
    tr: yamuk
    uk: трапеція
    ur: شکل منحرف
    uz: trapetsiya
    vi: hình thang
    vls: trapezium
    war: trapesoyd
    wuu: 梯形
    xmf: ტრაპეცია
    yi: טראפעז
    yue: 梯形
    zh: 梯形
    zh_hans: 梯形
    zh_hant: 梯形
---

# [[Trapezoid]] 

#is_/same_as :: [[../../../WikiData/WD~Trapezoid,46303|WD~Trapezoid,46303]]  

## #has_/text_of_/abstract 

> In geometry, a **Trapezoid** in North American English, or trapezium in British English, 
> is a quadrilateral that has at least one pair of parallel sides.
>
> The parallel sides are called the bases of the trapezoid. 
> The other two sides are called the legs or lateral sides. 
> If the trapezoid is a parallelogram, then the choice of bases and legs is arbitrary.
>
> A trapezoid is usually considered to be a convex quadrilateral in Euclidean geometry, 
> but there are also crossed cases. 
> 
> If shape ABCD is a convex trapezoid, then ABDC is a crossed trapezoid. 
> The formulas in this article apply only in convex trapezoids.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Trapezoid) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Geometry/Geometry-2D/Trapezoid|Trapezoid]] 

### #is_/same_as :: [[/_public/Mathematics/Geometry/Geometry-2D/Trapezoid.public|Trapezoid.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Geometry/Geometry-2D/Trapezoid.internal|Trapezoid.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Geometry/Geometry-2D/Trapezoid.protect|Trapezoid.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Geometry/Geometry-2D/Trapezoid.private|Trapezoid.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Geometry/Geometry-2D/Trapezoid.personal|Trapezoid.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Geometry/Geometry-2D/Trapezoid.secret|Trapezoid.secret]] 

