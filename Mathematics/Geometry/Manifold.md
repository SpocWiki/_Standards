---
aliases:
- Barietate (matematika)
- daugdara
- Durthi
- ilfhillteán
- mangfald
- mangfoldighed
- mangfoldighet
- Manifold
- Manipoldo
- Mannifalt
- Mannigfaltigkeit
- mnogostrukost
- mnogoterost
- monisto
- muutkond
- mångfald
- n-dimensional manifold, or n-manifold
- rozmaitość
- sokaság
- sternaĵo
- to-iūⁿ-thé
- variedad
- variedade
- varieta
- varietat
- varietate
- varietà
- varietà topològica
- variété
- variëteit
- Çokkatlı
- Çoxobrazlı
- đa tạp
- πολλαπλότητα
- многовид
- многообразие
- Многострукост
- многуобразие
- Нумайсăнарлăх
- разнастайнасьць
- Բազմաձևություն
- יריעה
- פלאכטע
- خمینه
- متنوعة
- مینیفولڈ
- বহুভাঁজ
- ਮੈਨੀਫੋਲਡ
- แมนิโฟลด์
- 多様体
- 流形
- 다양체
has_id_wikidata: Q203920
subclass_of:
- '[[_Standards/WikiData/WD~topological_space,179899|WD~topological_space,179899]]'
- '[[_Standards/WikiData/WD~Hausdorff_space,326908|WD~Hausdorff_space,326908]]'
- '[[_Standards/WikiData/WD~second_countable_space,1363919|WD~second_countable_space,1363919]]'
- '[[_Standards/WikiData/WD~non_Hausdorff_manifold,7048762|WD~non_Hausdorff_manifold,7048762]]'
has_characteristic:
- '[[_Standards/WikiData/WD~Euler_characteristic,852973|WD~Euler_characteristic,852973]]'
- '[[_Standards/WikiData/WD~dimension,4440864|WD~dimension,4440864]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
different_from: '[[_Standards/WikiData/WD~Q9323213,9323213|WD~Q9323213,9323213]]'
studied_by: '[[_Standards/WikiData/WD~manifold_theory,10380344|WD~manifold_theory,10380344]]'
described_by_source:
- '[[_Standards/WikiData/WD~New_Encyclopedic_Dictionary,19190511|WD~New_Encyclopedic_Dictionary,19190511]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
Commons_category: Manifolds
dv_has_:
  name_:
    ar: متنوعة
    az: Çoxobrazlı
    be_tarask: разнастайнасьць
    bg: многообразие
    bn: বহুভাঁজ
    ca: varietat
    cs: varieta
    cv: Нумайсăнарлăх
    da: mangfoldighed
    de: Mannigfaltigkeit
    el: πολλαπλότητα
    en: manifold
    en_ca: manifold
    en_gb: manifold
    eo: sternaĵo
    es: variedad
    et: muutkond
    eu: Barietate (matematika)
    fa: خمینه
    fi: monisto
    fr: variété
    ga: ilfhillteán
    gl: variedade
    gsw: Mannigfaltigkeit
    he: יריעה
    hr: mnogostrukost
    hu: sokaság
    hy: Բազմաձևություն
    id: manifold
    it: varietà
    ja: 多様体
    ko: 다양체
    lb: Mannifalt
    lt: daugdara
    mk: многуобразие
    nan: to-iūⁿ-thé
    nb: mangfoldighet
    nl: variëteit
    nn: mangfald
    oc: varietat
    pa: ਮੈਨੀਫੋਲਡ
    pl: rozmaitość
    pms: varietà topològica
    pnb: مینیفولڈ
    pt: variedade
    pt_br: variedade
    ro: varietate
    ru: многообразие
    sco: manifold
    sh: Mnogostrukost
    sk: Varieta
    sl: mnogoterost
    sq: Durthi
    sr: Многострукост
    sr_ec: Многострукост
    sr_el: Mnogostrukost
    sv: mångfald
    th: แมนิโฟลด์
    tl: Manipoldo
    tr: Çokkatlı
    uk: многовид
    ur: مینیفولڈ
    vi: đa tạp
    yi: פלאכטע
    yue: 流形
    zh: 流形
    zh_cn: 流形
    zh_hans: 流形
    zh_hant: 流形
    zh_hk: 流形
    zh_sg: 流形
    zh_tw: 流形
---

# [[Manifold]] 

#is_/same_as :: [[_Standards/WikiData/WD~Manifold,203920|WD~Manifold,203920]] 

## #has_/text_of_/abstract 

> In mathematics, a **Manifold** is a topological space 
> that locally resembles Euclidean space near each point. 
> 
> More precisely, an n-dimensional manifold, or n-manifold for short, 
> is a topological space with the property that 
> each point has a neighborhood 
> that is homeomorphic to an open subset of n-dimensional Euclidean space.
>
> One-dimensional manifolds include lines and circles, but not self-crossing curves such as a figure 8. Two-dimensional manifolds are also called surfaces. Examples include the plane, the sphere, and the torus, and also the Klein bottle and real projective plane.
>
> The concept of a manifold is central to many parts of geometry and modern mathematical physics because it allows complicated structures to be described in terms of well-understood topological properties of simpler spaces. Manifolds naturally arise as solution sets of systems of equations and as graphs of functions. The concept has applications in computer-graphics given the need to associate pictures with coordinates (e.g. CT scans).
>
> Manifolds can be equipped with additional structure. One important class of manifolds are differentiable manifolds; their differentiable structure allows calculus to be done. A Riemannian metric on a manifold allows distances and angles to be measured. Symplectic manifolds serve as the phase spaces in the Hamiltonian formalism of classical mechanics, while four-dimensional Lorentzian manifolds model spacetime in general relativity.
>
> The study of manifolds requires working knowledge of calculus and topology.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Manifold) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Geometry/Manifold|Manifold]] 

### #is_/same_as :: [[/_public/Mathematics/Geometry/Manifold.public|Manifold.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Geometry/Manifold.internal|Manifold.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Geometry/Manifold.protect|Manifold.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Geometry/Manifold.private|Manifold.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Geometry/Manifold.personal|Manifold.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Geometry/Manifold.secret|Manifold.secret]] 

