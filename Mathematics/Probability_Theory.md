---
aliases:
- damcaniaeth tebygolrwydd
- ehtimal nəzəriyyəsi
- Ehtimollar nazariyasi
- Fundo ya kusanda vya mwaŵi
- itimallıqlar teoriyası
- kansrekening
- Líkindafræði
- lý thuyết xác suất
- olasılık kuramı
- Probabilitas
- Probabilitate teoria
- probability calculus
- probability theory
- Probability_Theory
- probabloteorio
- sandsynlighedsregning
- sannolikhetsteori
- sannsynlighetsteori
- sannsynsrekning
- teoiric na dóchúlachta
- Teori kebarangkalian
- teori peluang
- teoria das probabilidades
- teoria de la probabilitat
- Teoria dei probabilitats
- teoria della probabilità
- Teoria e probabilitetit
- teoria prawdopodobieństwa
- Teoria probabilităților
- teorie pravděpodobnosti
- teorija vjerojatnosti
- teoría da probabilidade
- teoría de la probabilidad
- teoría de la probabilidá
- teória pravdepodobnosti
- theoria probabilitatum
- théorie des probabilités
- tikimybių teorija
- todennäköisyysteoria
- tuođânálásâšteoria
- Téori probabilitas
- tõenäosusteooria
- valószínűségszámítás
- varbūtību teorija
- verjetnostna teorija
- Waarskynlikheidsleer
- Wahrscheinlichkeitstheorie
- Warscheinlechkeetstheorie
- Ähtimallyk teoriýasy
- Θεωρία πιθανοτήτων
- вероятность теорий
- ихтималлылык теориясе
- Ихтималлыҡ теорияһы
- Магадлалын онол
- Назарияи эҳтимолият
- Пулаяслăхсен теорийĕ
- теорія ймовірностей
- теорија вероватноће
- теорија на веројатноста
- теория вероятностей
- Теория на вероятностите
- тэорыя імавернасцей
- тэорыя імавернасьцяў
- уæвæны теори
- Ыктымалдуулук теориясы
- Ықтималдық теориясы
- հավանականությունների տեսություն
- טעאריע פון משמעותדיקייט
- תורת ההסתברות
- احتیمال نظریهسی
- تیۆریی ئەگەر
- نظرية الاحتمال
- نظریهٔ احتمالات
- نظریۂ احتمال
- प्रायिकता सिद्धांत
- সম্ভবনা তত্ত্ব
- ਪ੍ਰੋਬੇਬਿਲਟੀ ਥਿਊਰੀ
- ସମ୍ଭାବ୍ୟତା ସିଦ୍ଧାନ୍ତ
- நிகழ்தகவுக் கோட்பாடு
- සම්භාවිතා වාදය
- ทฤษฎีความน่าจะเป็น
- ალბათობის თეორია
- 概率論
- 概率论
- 確率論
- 확률론
has_id_wikidata: Q5862903
described_by_source:
- '[[../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633|WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633]]'
instance_of:
- '[[_Standards/WikiData/WD~area_of_mathematics,1936384|WD~area_of_mathematics,1936384]]'
- '[[_Standards/WikiData/WD~mathematical_theory,20026918|WD~mathematical_theory,20026918]]'
part_of: '[[_Standards/WikiData/WD~measure_theory,15614122|WD~measure_theory,15614122]]'
topic_s_main_template: '[[_Standards/WikiData/WD~Template_Probability_fundamentals,15748423|WD~Template_Probability_fundamentals,15748423]]'
Stack_Exchange_tag:
- https://cstheory.stackexchange.com/tags/pr.probability
- https://or.stackexchange.com/tags/probability-theory
- https://math.stackexchange.com/tags/probability-theory
- https://mathoverflow.net/tags/pr.probability
described_at_URL:
- https://www.geeksforgeeks.org/maths/probability-theorems-theorems-and-examples/
- https://www.cuemath.com/data/probability/
- https://dec41.user.srcf.net/notes/IA_L/probability_thm_proof.pdf
- https://online.stat.psu.edu/stat414/lesson/2/2.6
practiced_by: '[[_Standards/WikiData/WD~probability_theorist,97016341|WD~probability_theorist,97016341]]'
subclass_of: '[[_Standards/WikiData/WD~mathematics,395|WD~mathematics,395]]'
is_the_study_of: '[[_Standards/WikiData/WD~probability,9492|WD~probability,9492]]'
OmegaWiki_Defined_Meaning: 797277
Basisklassifikation: 31.70
Commons_category: Probability theory
UMLS_CUI: C0033207
MeSH_tree_code: L01.906.394.622
Krugosvet_article: nauka_i_tehnika/matematika/VEROYATNOSTE_TEORIYA.html
GitHub_topic: probability-theory
subreddit: probabilitytheory
dv_has_:
  name_:
    af: Waarskynlikheidsleer
    ar: نظرية الاحتمال
    ast: teoría de la probabilidá
    az: ehtimal nəzəriyyəsi
    azb: احتیمال نظریهسی
    ba: Ихтималлыҡ теорияһы
    be: тэорыя імавернасцей
    be_tarask: тэорыя імавернасьцяў
    bg: Теория на вероятностите
    bn: সম্ভবনা তত্ত্ব
    btm: Probabilitas
    ca: teoria de la probabilitat
    ckb: تیۆریی ئەگەر
    cs: teorie pravděpodobnosti
    cv: Пулаяслăхсен теорийĕ
    cy: damcaniaeth tebygolrwydd
    da: sandsynlighedsregning
    de: Wahrscheinlichkeitstheorie
    de_ch: Wahrscheinlichkeitstheorie
    el: Θεωρία πιθανοτήτων
    en: probability theory
    en_ca: Probability theory
    en_gb: probability theory
    eo: probabloteorio
    es: teoría de la probabilidad
    et: tõenäosusteooria
    eu: Probabilitate teoria
    fa: نظریهٔ احتمالات
    fi: todennäköisyysteoria
    fr: théorie des probabilités
    ga: teoiric na dóchúlachta
    gl: teoría da probabilidade
    he: תורת ההסתברות
    hi: प्रायिकता सिद्धांत
    hr: teorija vjerojatnosti
    hu: valószínűségszámítás
    hy: հավանականությունների տեսություն
    id: teori peluang
    is: Líkindafræði
    it: teoria della probabilità
    ja: 確率論
    jv: Téori probabilitas
    ka: ალბათობის თეორია
    kaa: itimallıqlar teoriyası
    kk: Ықтималдық теориясы
    ko: 확률론
    ky: Ыктымалдуулук теориясы
    la: theoria probabilitatum
    lb: Warscheinlechkeetstheorie
    lt: tikimybių teorija
    lv: varbūtību teorija
    mhr: вероятность теорий
    mk: теорија на веројатноста
    mn: Магадлалын онол
    ms: Teori kebarangkalian
    nb: sannsynlighetsteori
    nl: kansrekening
    nn: sannsynsrekning
    oc: Teoria dei probabilitats
    or: ସମ୍ଭାବ୍ୟତା ସିଦ୍ଧାନ୍ତ
    os: уæвæны теори
    pa: ਪ੍ਰੋਬੇਬਿਲਟੀ ਥਿਊਰੀ
    pl: teoria prawdopodobieństwa
    pt: teoria das probabilidades
    pt_br: teoria das probabilidades
    ro: Teoria probabilităților
    ru: теория вероятностей
    sco: probability theory
    sh: Teorija vjerojatnosti
    si: සම්භාවිතා වාදය
    sk: teória pravdepodobnosti
    sl: verjetnostna teorija
    smn: tuođânálásâšteoria
    sq: Teoria e probabilitetit
    sr: теорија вероватноће
    sr_ec: теорија вероватноће
    su: Téori probabilitas
    sv: sannolikhetsteori
    ta: நிகழ்தகவுக் கோட்பாடு
    tg: Назарияи эҳтимолият
    th: ทฤษฎีความน่าจะเป็น
    tk: Ähtimallyk teoriýasy
    tl: probability theory
    tr: olasılık kuramı
    tt: ихтималлылык теориясе
    tum: Fundo ya kusanda vya mwaŵi
    uk: теорія ймовірностей
    ur: نظریۂ احتمال
    uz: Ehtimollar nazariyasi
    vi: lý thuyết xác suất
    wuu: 概率论
    yi: טעאריע פון משמעותדיקייט
    yue: 概率論
    zh: 概率论
---

# [[Probability_Theory]] 

#is_/same_as :: [[../WikiData/WD~Probability_theory,5862903|WD~Probability_theory,5862903]] 

## #has_/text_of_/abstract 

> **probability theory** or probability calculus 
> is the branch of mathematics concerned with probability. 
> 
> Although there are several different probability interpretations, 
> probability theory treats the concept in a rigorous mathematical manner 
> by expressing it through a set of axioms. 
> 
> Typically these axioms formalise probability in terms of a probability space, 
> which assigns a measure taking values between 0 and 1, termed the probability measure, 
> to a set of outcomes called the sample space. 
> Any specified subset of the sample space is called an event.
>
> Central subjects in probability theory include discrete and continuous random variables, 
> probability distributions, and stochastic processes (which provide mathematical abstractions of non-deterministic or uncertain processes or measured quantities that may either be single occurrences or evolve over time in a random fashion).
>
> Although it is not possible to perfectly predict random events, much can be said about their behavior. Two major results in probability theory describing such behaviour are the law of large numbers and the central limit theorem.
>
> As a mathematical foundation for statistics, probability theory is essential to many human activities that involve quantitative analysis of data. Methods of probability theory also apply to descriptions of complex systems given only partial knowledge of their state, as in statistical mechanics or sequential estimation. A great discovery of twentieth-century physics was the probabilistic nature of physical phenomena at atomic scales, described in quantum mechanics.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Probability%20theory) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Probability_Theory|Probability_Theory]] 

### #is_/same_as :: [[/_public/Mathematics/Probability_Theory.public|Probability_Theory.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Probability_Theory.internal|Probability_Theory.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Probability_Theory.protect|Probability_Theory.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Probability_Theory.private|Probability_Theory.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Probability_Theory.personal|Probability_Theory.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Probability_Theory.secret|Probability_Theory.secret]] 

