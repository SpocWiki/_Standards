---
aliases:
- Aragtida Baytagoras
- Asekkud n Pythagore
- Bondeko ya mpanzi-misáto
- den pytagoréiske læresetninga
- den pythagoræiske læresætning
- dudzirazivo raPythagoras
- Pifagor teoremasi
- Pifagor teoreması
- Pifagoran teorem
- Pifagornıñ teoreması
- Pifagoryň teoremasy
- Pifaqor teoremi
- Pisagor teoremi
- Pitagora teorēma
- Pitagorasen teorema
- Pitagorasnon nga teyorema
- Pitagorasz-tétel
- Pitagorin poučak
- Pitagorina teorema
- Pitagoro teorema
- Pitagorov izrek
- Pitágora remimoañeterã
- Pytagoras’ læresetning
- Pytagorova veta
- Pythagoraan lause
- Pythagoras sats
- Pythagoras se stelling
- Pythagoras sin reegel
- Pythagoras' theorem
- Pythagorasa cealkka
- Pythagorase teoreem
- Pythagorean Theorem
- Pythagorean tēng-lí
- Pythagorean_Theorem
- Pythagorova věta
- Pėtaguora teuorema
- Raikitr' Pytagore
- regla Pýþagórasar
- rumus Pythagoras
- Sada Pythagorasa
- sasusuan ni pitaguras
- Satz des Pythagoras
- Satz van Pythagoras
- stelling van Pythagoras
- Såtz vum Pythagoras
- teoirim Phíotagarásach
- teorem de Pitagora
- teorem Pythagoras
- Teorema de Pitagora
- teorema de Pitagoras
- teorema de Pitàgora
- Teorema de Pitàgores
- teorema de Pitágoras
- teorema di Pitagora
- teorema e Pitagorës
- teorema lui Pitagora
- teorema ni Pythagoras
- Teorema Pythagoras
- Teorema Pîtagoras
- teorema ëd Pitàgora
- teoremo de Pitagoro
- teoremo di Pitagoro
- teoremê Pisagori
- teorèma de Pitagòras
- theorem Pythagoras
- theorema de Pythagoras
- theorema Pythagorae
- théorème de Pythagore
- tiurema di Pitagora
- tiuréma 'd Pitàgora
- twierdzenie Pitagorasa
- twjerdzyńy Pitagorasa
- uhakiki wa Pythagoras
- àgbérò Pythagoras
- định lý Pythagoras
- Πυθαγόρειο θεώρημα
- Питагорина теорема
- Питагорова теорема
- Пифагор и теоремэ
- Пифагор теоремасы
- Пифагор теоремата
- Пифагор теоремаһы
- Пифагор теореми
- Пифагоры теоремæ
- Пифагорын теорем
- теорема Піфагора
- теорема Пифагора
- тэарэма Пітагора
- тэарэма Піфагора
- Պյութագորասի թեորեմ
- משפט פיתגורס
- פיטאגאראס פרינציפ
- تیۆرمی پیتاگۆرس
- رابطه فیثاغورس
- طيوريم ديال پيطاڭوراص
- فیثاغورث تئوریمی
- فیثاغورس قانون
- مبرهنة فيثاغورس
- مسئلہ فیثا غورث
- مسئلۂ فیثا غورث
- مسلہ فیساغورس
- पाइथागोरस प्रमेय
- पाइथागोरस साध्य
- पायथागोरसचा सिद्धान्त
- পাইথেগাৰাছৰ উপপাদ্য
- পিথাগোরাসের উপপাদ্য
- ਪਾਇਥਾਗੋਰਸ ਦਾ ਪ੍ਰਮੇਏ
- પાયથાગોરસનો પ્રમેય
- பித்தேகோரசு தேற்றம்
- పైథాగరస్ సిద్ధాంతం
- പൈതഗോറസ് സിദ്ധാന്തം
- පයිතගරස් ප් රමේයය
- ทฤษฎีบทพีทาโกรัส
- ທິດສະດີ ປີທາໂກຣຽນ
- ပိုက်သာဂိုရ သီအိုရမ်
- პითაგორას თეორემა
- ፓይታጎረስ እርጉጥ
- ទ្រឹស្តីបទពីតាករ
- ᱯᱟᱭᱛᱷᱳᱜᱳᱨᱚᱱ ᱛᱷᱤᱭᱚᱨᱚᱢ
- ピタゴラスの定理
- 勾股定理
- 畢氏定理
- 피타고라스 정리
has_id_wikidata: Q11518
Commons_category: Pythagorean theorem
defining_formula: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle c^{2}=a^{2}+b^{2}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <msup>
            <mi>c</mi>
            <mrow class="MJX-TeXAtom-ORD">
              <mn>2</mn>
            </mrow>
          </msup>
          <mo>=</mo>
          <msup>
            <mi>a</mi>
            <mrow class="MJX-TeXAtom-ORD">
              <mn>2</mn>
            </mrow>
          </msup>
          <mo>+</mo>
          <msup>
            <mi>b</mi>
            <mrow class="MJX-TeXAtom-ORD">
              <mn>2</mn>
            </mrow>
          </msup>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle c^{2}=a^{2}+b^{2}}</annotation>
    </semantics>
  </math>
described_at_URL: https://www.geogebra.org/m/ZFTGX57r
described_by_source: '[[_Standards/WikiData/WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752]]'
generalization_of:
- '[[_Standards/WikiData/WD~parallel_postulate,207014|WD~parallel_postulate,207014]]'
- '[[_Standards/WikiData/WD~Pythagorean_trigonometric_identity,2039117|WD~Pythagorean_trigonometric_identity,2039117]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Pythagorean.svg
instance_of: '[[_Standards/WikiData/WD~theorem,65943|WD~theorem,65943]]'
in_defining_formula:
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle a}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>a</mi>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle a}</annotation>
    </semantics>
  </math>
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle c}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>c</mi>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle c}</annotation>
    </semantics>
  </math>
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle b}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>b</mi>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle b}</annotation>
    </semantics>
  </math>
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
named_after: '[[_Standards/WikiData/WD~Pythagoras,10261|WD~Pythagoras,10261]]'
part_of: '[[_Standards/WikiData/WD~list_of_theorems,944443|WD~list_of_theorems,944443]]'
permanent_duplicated_item: '[[_Standards/WikiData/WD~Q110206185,110206185|WD~Q110206185,110206185]]'
pronunciation_audio:
- http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q150%20%28fra%29-WikiLucas00-th%C3%A9or%C3%A8me%20de%20Pythagore.wav
- http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q1571%20%28mar%29-Vj18081991-%E0%A4%AA%E0%A4%BE%E0%A4%AF%E0%A4%A5%E0%A4%BE%E0%A4%97%E0%A5%8B%E0%A4%B0%E0%A4%B8%E0%A4%9A%E0%A4%BE%20%E0%A4%B8%E0%A4%BF%E0%A4%A6%E0%A5%8D%E0%A4%A7%E0%A4%BE%E0%A4%A8%E0%A5%8D%E0%A4%A4.wav
statement_describes: '[[_Standards/WikiData/WD~right_triangle,158688|WD~right_triangle,158688]]'
video: http://commons.wikimedia.org/wiki/Special:FilePath/2.%D0%A2%D0%B5%D0%BE%D1%80%D0%B5%D0%BC%D0%B0%20%D0%9F%D0%B8%D1%84%D0%B0%D0%B3%D0%BE%D1%80%D0%B0.ogv
dv_has_:
  name_:
    af: Pythagoras se stelling
    am: ፓይታጎረስ እርጉጥ
    an: teorema de Pitagoras
    ar: مبرهنة فيثاغورس
    ary: طيوريم ديال پيطاڭوراص
    as: পাইথেগাৰাছৰ উপপাদ্য
    ast: teorema de Pitágoras
    awa: पाइथागोरस प्रमेय
    az: Pifaqor teoremi
    azb: فیثاغورث تئوریمی
    ba: Пифагор теоремаһы
    ban: rumus Pythagoras
    bar: Såtz vum Pythagoras
    bcl: teorema ni Pythagoras
    be: тэарэма Піфагора
    be_tarask: тэарэма Пітагора
    bg: Питагорова теорема
    bn: পিথাগোরাসের উপপাদ্য
    br: teorem Pythagoras
    bs: Pitagorina teorema
    ca: Teorema de Pitàgores
    ckb: تیۆرمی پیتاگۆرس
    co: tiurema di Pitagora
    crh: Pifagornıñ teoreması
    cs: Pythagorova věta
    cv: Пифагор теореми
    cy: theorem Pythagoras
    da: den pythagoræiske læresætning
    de: Satz des Pythagoras
    diq: teoremê Pisagori
    dtp: Teorem Pythagoras
    el: Πυθαγόρειο θεώρημα
    eml: tiuréma 'd Pitàgora
    en: Pythagorean theorem
    eo: teoremo de Pitagoro
    es: teorema de Pitágoras
    et: Pythagorase teoreem
    eu: Pitagorasen teorema
    fa: رابطه فیثاغورس
    fi: Pythagoraan lause
    fr: théorème de Pythagore
    frr: Pythagoras sin reegel
    ga: teoirim Phíotagarásach
    gl: teorema de Pitágoras
    gn: Pitágora remimoañeterã
    gsw: Satz des Pythagoras
    gu: પાયથાગોરસનો પ્રમેય
    he: משפט פיתגורס
    hi: पाइथागोरस प्रमेय
    hif: Pythagorean theorem
    hr: Pitagorin poučak
    hsb: Sada Pythagorasa
    hu: Pitagorasz-tétel
    hy: Պյութագորասի թեորեմ
    ia: theorema de Pythagoras
    iba: Teorem Pythagoras
    id: teorema Pythagoras
    io: teoremo di Pitagoro
    is: regla Pýþagórasar
    it: teorema di Pitagora
    ja: ピタゴラスの定理
    ka: პითაგორას თეორემა
    kab: Asekkud n Pythagore
    kbd: Пифагор и теоремэ
    kk: Пифагор теоремасы
    km: ទ្រឹស្តីបទពីតាករ
    ko: 피타고라스 정리
    ku: Teorema Pîtagoras
    ky: Пифагор теоремасы
    la: theorema Pythagorae
    lfn: teorem de Pitagora
    lmo: Teorema de Pitagora
    ln: Bondeko ya mpanzi-misáto
    lo: ທິດສະດີ ປີທາໂກຣຽນ
    lt: Pitagoro teorema
    lv: Pitagora teorēma
    lzh: 勾股定理
    mad: Teorema Pythagoras
    mag: पाइथागोरस प्रमेय
    mg: Raikitr' Pytagore
    mk: Питагорина теорема
    ml: പൈതഗോറസ് സിദ്ധാന്തം
    mn: Пифагорын теорем
    mr: पायथागोरसचा सिद्धान्त
    ms: teorem Pythagoras
    my: ပိုက်သာဂိုရ သီအိုရမ်
    mzn: فیثاغورس قانون
    nan: Pythagorean tēng-lí
    nb: Pytagoras’ læresetning
    nds: Satz van Pythagoras
    ne: पाइथागोरस साध्य
    nl: stelling van Pythagoras
    nn: den pytagoréiske læresetninga
    oc: teorèma de Pitagòras
    os: Пифагоры теоремæ
    pa: ਪਾਇਥਾਗੋਰਸ ਦਾ ਪ੍ਰਮੇਏ
    pl: twierdzenie Pitagorasa
    pms: teorema ëd Pitàgora
    pnb: مسلہ فیساغورس
    pt: teorema de Pitágoras
    pt_br: teorema de Pitágoras
    pwn: sasusuan ni pitaguras
    ro: teorema lui Pitagora
    ru: теорема Пифагора
    sah: Пифагор теоремата
    sat: ᱯᱟᱭᱛᱷᱳᱜᱳᱨᱚᱱ ᱛᱷᱤᱭᱚᱨᱚᱢ
    scn: tiurema di Pitagora
    se: Pythagorasa cealkka
    sgs: Pėtaguora teuorema
    sh: Pitagorina teorema
    si: පයිතගරස් ප් රමේයය
    sk: Pytagorova veta
    skr: مسئلہ فیثا غورث
    sl: Pitagorov izrek
    sn: dudzirazivo raPythagoras
    so: Aragtida Baytagoras
    sq: teorema e Pitagorës
    sr: Питагорина теорема
    sr_ec: Питагорина теорема
    sv: Pythagoras sats
    sw: uhakiki wa Pythagoras
    szl: twjerdzyńy Pitagorasa
    ta: பித்தேகோரசு தேற்றம்
    te: పైథాగరస్ సిద్ధాంతం
    th: ทฤษฎีบทพีทาโกรัส
    tk: Pifagoryň teoremasy
    tl: Pythagorean theorem
    tr: Pisagor teoremi
    tt: Pifagor teoreması
    uk: теорема Піфагора
    ur: مسئلۂ فیثا غورث
    uz: Pifagor teoremasi
    vec: teorema de Pitàgora
    vep: Pifagoran teorem
    vi: định lý Pythagoras
    war: Pitagorasnon nga teyorema
    wuu: 勾股定理
    yi: פיטאגאראס פרינציפ
    yo: àgbérò Pythagoras
    yue: 勾股定理
    zh: 勾股定理
    zh_cn: 勾股定理
    zh_hans: 勾股定理
    zh_hant: 勾股定理
    zh_hk: 畢氏定理
    zh_tw: 勾股定理
---

# [[Pythagorean_Theorem]] 

#is_/same_as :: [[../../WikiData/WD~Pythagorean_theorem,11518|WD~Pythagorean_theorem,11518]] 

## #has_/text_of_/abstract 

> In mathematics, the **Pythagorean theorem** or Pythagoras' theorem 
> is a fundamental relation in Euclidean geometry 
> between the three sides of a right triangle. 
> 
> It states that the area of the square whose side is the hypotenuse 
> (the side opposite the right angle) is equal to 
> the sum of the areas of the squares on the other two sides.
>
> The theorem can be written as an equation relating 
> the lengths of the sides a, b and the hypotenuse c, 
> sometimes called the Pythagorean equation: a² + b² = c²
>
> The theorem is named for the Greek philosopher Pythagoras, born around 570 BC.  
> The theorem has been proved numerous times by many different methods – 
> possibly the most for any mathematical theorem. 
> The proofs are diverse, including both geometric proofs and algebraic proofs, 
> with some dating back thousands of years.
>
> When Euclidean space is represented by a Cartesian coordinate system in analytic geometry, Euclidean distance satisfies the Pythagorean relation: 
> the squared distance between two points 
> equals the sum of squares of the difference in each coordinate between the points.
>
> The theorem can be generalized in various ways: 
> - to higher-dimensional spaces, 
> - to spaces that are not Euclidean, 
> - to objects that are not right triangles, and 
> - to objects that are not triangles at all but n-dimensional solids.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Pythagorean%20theorem) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Theorem/Pythagorean_Theorem|Pythagorean_Theorem]] 

### #is_/same_as :: [[/_public/Mathematics/Theorem/Pythagorean_Theorem.public|Pythagorean_Theorem.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Theorem/Pythagorean_Theorem.internal|Pythagorean_Theorem.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Theorem/Pythagorean_Theorem.protect|Pythagorean_Theorem.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Theorem/Pythagorean_Theorem.private|Pythagorean_Theorem.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Theorem/Pythagorean_Theorem.personal|Pythagorean_Theorem.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Theorem/Pythagorean_Theorem.secret|Pythagorean_Theorem.secret]] 

