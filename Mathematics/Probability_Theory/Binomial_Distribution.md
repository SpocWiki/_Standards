---
aliases:
- Banakuntza binomial
- Bernoulli valem
- Binomial Distribution
- binomial distributione
- Binomial taqsimot
- binomiale verdeling
- binomialfordeling
- binomialfördelning
- Binomialverteilung
- Binomial_Distribution
- Binomické rozdelenie
- binomické rozdělení
- Binomijakauma
- binominis skirstinys
- binomisk fordeling
- binomiális eloszlás
- binomiālais sadalījums
- Binomna raspodela
- binomska porazdelitev
- Bïnomdıq ülestirilw
- Distribució binomial
- Distribución binomial
- distribución binómica
- distribuição binomial
- Distribusi binomial
- Distribusyong binomial
- distribuzione binomiale
- Distribuția binomială
- ikiterimli dağılım
- loi binomiale
- Phân phối nhị thức
- Rozkład dwumianowy
- Sebaran binomial
- Shpërndarja binomiale
- Taburan binomial
- Διωνυμική κατανομή
- біноміальний розподіл
- Біномнае размеркаванне
- біномнае разьмеркаваньне
- Биномдық үлестірілу
- биномиальное распределение
- биномна распределба
- התפלגות בינומית
- بىينومدىق ۇلەستىرىلۋ
- توزيع احتمالي ثنائي
- توزیع دوجملهای
- دو رقمی توزیع
- द्विपद वितरण
- দ্বিপদী বন্টন
- ஈருறுப்புப் பரவல்
- การแจกแจงแบบทวินาม
- 二項分佈
- 二項分布
- 二项分布
- 이항 분포
has_id_wikidata: Q185547
subclass_of:
- '[[_Standards/WikiData/WD~Panjer_distribution,1090859]]'
- '[[_Standards/WikiData/WD~multinomial_distribution,1147928]]'
- '[[_Standards/WikiData/WD~Poisson_binomial_distribution,3258231]]'
- '[[_Standards/WikiData/WD~univariate_probability_distribution,7893853]]'
- '[[_Standards/WikiData/WD~discrete_probability_distribution,26898117]]'
in_defining_formula:
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle X}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>X</mi>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle X}</annotation>
    </semantics>
  </math>
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle P(X=x)}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>P</mi>
          <mo stretchy="false">(</mo>
          <mi>X</mi>
          <mo>=</mo>
          <mi>x</mi>
          <mo stretchy="false">)</mo>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle P(X=x)}</annotation>
    </semantics>
  </math>
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle {\binom {n}{x}}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mrow class="MJX-TeXAtom-ORD">
            <mrow>
              <mrow class="MJX-TeXAtom-OPEN">
                <mo maxsize="2.047em" minsize="2.047em">(</mo>
              </mrow>
              <mfrac linethickness="0">
                <mi>n</mi>
                <mi>x</mi>
              </mfrac>
              <mrow class="MJX-TeXAtom-CLOSE">
                <mo maxsize="2.047em" minsize="2.047em">)</mo>
              </mrow>
            </mrow>
          </mrow>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle {\binom {n}{x}}}</annotation>
    </semantics>
  </math>
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137]]'
instance_of: '[[_Standards/WikiData/WD~mathematical_concept,24034552]]'
probability_mass_function:
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle {\frac {n!}{x!(n-x)!}}p^{x}(1-p)^{n-x}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mrow class="MJX-TeXAtom-ORD">
            <mfrac>
              <mrow>
                <mi>n</mi>
                <mo>!</mo>
              </mrow>
              <mrow>
                <mi>x</mi>
                <mo>!</mo>
                <mo stretchy="false">(</mo>
                <mi>n</mi>
                <mo>&#x2212;<!-- - --></mo>
                <mi>x</mi>
                <mo stretchy="false">)</mo>
                <mo>!</mo>
              </mrow>
            </mfrac>
          </mrow>
          <msup>
            <mi>p</mi>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>x</mi>
            </mrow>
          </msup>
          <mo stretchy="false">(</mo>
          <mn>1</mn>
          <mo>&#x2212;<!-- - --></mo>
          <mi>p</mi>
          <msup>
            <mo stretchy="false">)</mo>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>n</mi>
              <mo>&#x2212;<!-- - --></mo>
              <mi>x</mi>
            </mrow>
          </msup>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle {\frac {n!}{x!(n-x)!}}p^{x}(1-p)^{n-x}}</annotation>
    </semantics>
  </math>
- >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle {\binom {n}{x}}p^{x}(1-p)^{n-x}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mrow class="MJX-TeXAtom-ORD">
            <mrow>
              <mrow class="MJX-TeXAtom-OPEN">
                <mo maxsize="2.047em" minsize="2.047em">(</mo>
              </mrow>
              <mfrac linethickness="0">
                <mi>n</mi>
                <mi>x</mi>
              </mfrac>
              <mrow class="MJX-TeXAtom-CLOSE">
                <mo maxsize="2.047em" minsize="2.047em">)</mo>
              </mrow>
            </mrow>
          </mrow>
          <msup>
            <mi>p</mi>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>x</mi>
            </mrow>
          </msup>
          <mo stretchy="false">(</mo>
          <mn>1</mn>
          <mo>&#x2212;<!-- - --></mo>
          <mi>p</mi>
          <msup>
            <mo stretchy="false">)</mo>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>n</mi>
              <mo>&#x2212;<!-- - --></mo>
              <mi>x</mi>
            </mrow>
          </msup>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle {\binom {n}{x}}p^{x}(1-p)^{n-x}}</annotation>
    </semantics>
  </math>
described_by_source: '[[_Standards/WikiData/WD~ISO_3534_1_2006(en)_Statistics_Vocabulary_and_symbols_Part_1_General_statistical_terms_and_terms_used_in_probability,111973641]]'
variance_of_a_probability_distribution: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle np(1-p)}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>n</mi>
          <mi>p</mi>
          <mo stretchy="false">(</mo>
          <mn>1</mn>
          <mo>&#x2212;<!-- - --></mo>
          <mi>p</mi>
          <mo stretchy="false">)</mo>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle np(1-p)}</annotation>
    </semantics>
  </math>
mean_of_a_probability_distribution: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle np}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>n</mi>
          <mi>p</mi>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle np}</annotation>
    </semantics>
  </math>
characteristic_function: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle \left(p\mathrm {e} ^{\mathrm {i} t}+1-p\right)^{n}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <msup>
            <mrow>
              <mo>(</mo>
              <mrow>
                <mi>p</mi>
                <msup>
                  <mrow class="MJX-TeXAtom-ORD">
                    <mi mathvariant="normal">e</mi>
                  </mrow>
                  <mrow class="MJX-TeXAtom-ORD">
                    <mrow class="MJX-TeXAtom-ORD">
                      <mi mathvariant="normal">i</mi>
                    </mrow>
                    <mi>t</mi>
                  </mrow>
                </msup>
                <mo>+</mo>
                <mn>1</mn>
                <mo>&#x2212;<!-- - --></mo>
                <mi>p</mi>
              </mrow>
              <mo>)</mo>
            </mrow>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>n</mi>
            </mrow>
          </msup>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle \left(p\mathrm {e} ^{\mathrm {i} t}+1-p\right)^{n}}</annotation>
    </semantics>
  </math>
support_of_a_function: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle x\in \{0,1,2,\ldots ,n\}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>x</mi>
          <mo>&#x2208;<!-- ∈ --></mo>
          <mo fence="false" stretchy="false">{</mo>
          <mn>0</mn>
          <mo>,</mo>
          <mn>1</mn>
          <mo>,</mo>
          <mn>2</mn>
          <mo>,</mo>
          <mo>&#x2026;<!-- … --></mo>
          <mo>,</mo>
          <mi>n</mi>
          <mo fence="false" stretchy="false">}</mo>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle x\in \{0,1,2,\ldots ,n\}}</annotation>
    </semantics>
  </math>
defining_formula: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle P(X=x)={\binom {n}{x}}p^{x}(1-p)^{n-x}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mi>P</mi>
          <mo stretchy="false">(</mo>
          <mi>X</mi>
          <mo>=</mo>
          <mi>x</mi>
          <mo stretchy="false">)</mo>
          <mo>=</mo>
          <mrow class="MJX-TeXAtom-ORD">
            <mrow>
              <mrow class="MJX-TeXAtom-OPEN">
                <mo maxsize="2.047em" minsize="2.047em">(</mo>
              </mrow>
              <mfrac linethickness="0">
                <mi>n</mi>
                <mi>x</mi>
              </mfrac>
              <mrow class="MJX-TeXAtom-CLOSE">
                <mo maxsize="2.047em" minsize="2.047em">)</mo>
              </mrow>
            </mrow>
          </mrow>
          <msup>
            <mi>p</mi>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>x</mi>
            </mrow>
          </msup>
          <mo stretchy="false">(</mo>
          <mn>1</mn>
          <mo>&#x2212;<!-- - --></mo>
          <mi>p</mi>
          <msup>
            <mo stretchy="false">)</mo>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>n</mi>
              <mo>&#x2212;<!-- - --></mo>
              <mi>x</mi>
            </mrow>
          </msup>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle P(X=x)={\binom {n}{x}}p^{x}(1-p)^{n-x}}</annotation>
    </semantics>
  </math>
skewness: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle {\frac {1-2p}{\sqrt {np(1-p)}}}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mrow class="MJX-TeXAtom-ORD">
            <mfrac>
              <mrow>
                <mn>1</mn>
                <mo>&#x2212;<!-- - --></mo>
                <mn>2</mn>
                <mi>p</mi>
              </mrow>
              <msqrt>
                <mi>n</mi>
                <mi>p</mi>
                <mo stretchy="false">(</mo>
                <mn>1</mn>
                <mo>&#x2212;<!-- - --></mo>
                <mi>p</mi>
                <mo stretchy="false">)</mo>
              </msqrt>
            </mfrac>
          </mrow>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle {\frac {1-2p}{\sqrt {np(1-p)}}}}</annotation>
    </semantics>
  </math>
image: http://commons.wikimedia.org/wiki/Special:FilePath/Binomial%20distribution%20PDF.svg
P8189: 987007282570105171
Commons_category: Binomial distributions
MeSH_tree_code:
- E05.318.740.994.250
- G17.820.250
- N05.715.360.750.750.150
- N06.850.520.830.994.250
excess_kurtosis: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle {\frac {1-6p(1-p)}{np(1-p)}}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <mrow class="MJX-TeXAtom-ORD">
            <mfrac>
              <mrow>
                <mn>1</mn>
                <mo>&#x2212;<!-- - --></mo>
                <mn>6</mn>
                <mi>p</mi>
                <mo stretchy="false">(</mo>
                <mn>1</mn>
                <mo>&#x2212;<!-- - --></mo>
                <mi>p</mi>
                <mo stretchy="false">)</mo>
              </mrow>
              <mrow>
                <mi>n</mi>
                <mi>p</mi>
                <mo stretchy="false">(</mo>
                <mn>1</mn>
                <mo>&#x2212;<!-- - --></mo>
                <mi>p</mi>
                <mo stretchy="false">)</mo>
              </mrow>
            </mfrac>
          </mrow>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle {\frac {1-6p(1-p)}{np(1-p)}}}</annotation>
    </semantics>
  </math>
moment_generating_function: >-
  <math xmlns="http://www.w3.org/1998/Math/MathML" display="block" alttext="{\displaystyle \left(p\left(\mathrm {e} ^{t}-1\right)+1\right)^{n}}">
    <semantics>
      <mrow class="MJX-TeXAtom-ORD">
        <mstyle displaystyle="true" scriptlevel="0">
          <msup>
            <mrow>
              <mo>(</mo>
              <mrow>
                <mi>p</mi>
                <mrow>
                  <mo>(</mo>
                  <mrow>
                    <msup>
                      <mrow class="MJX-TeXAtom-ORD">
                        <mi mathvariant="normal">e</mi>
                      </mrow>
                      <mrow class="MJX-TeXAtom-ORD">
                        <mi>t</mi>
                      </mrow>
                    </msup>
                    <mo>&#x2212;<!-- - --></mo>
                    <mn>1</mn>
                  </mrow>
                  <mo>)</mo>
                </mrow>
                <mo>+</mo>
                <mn>1</mn>
              </mrow>
              <mo>)</mo>
            </mrow>
            <mrow class="MJX-TeXAtom-ORD">
              <mi>n</mi>
            </mrow>
          </msup>
        </mstyle>
      </mrow>
      <annotation encoding="application/x-tex">{\displaystyle \left(p\left(\mathrm {e} ^{t}-1\right)+1\right)^{n}}</annotation>
    </semantics>
  </math>
dv_has_:
  name_:
    ar: توزيع احتمالي ثنائي
    ast: distribución binómica
    be: Біномнае размеркаванне
    be_tarask: біномнае разьмеркаваньне
    bn: দ্বিপদী বন্টন
    ca: Distribució binomial
    cs: binomické rozdělení
    da: binomialfordeling
    de: Binomialverteilung
    el: Διωνυμική κατανομή
    en: binomial distribution
    es: distribución binómica
    et: Bernoulli valem
    eu: Banakuntza binomial
    fa: توزیع دوجملهای
    fi: Binomijakauma
    fr: loi binomiale
    gl: Distribución binomial
    he: התפלגות בינומית
    hi: द्विपद वितरण
    hu: binomiális eloszlás
    id: Distribusi binomial
    it: distribuzione binomiale
    ja: 二項分布
    kk: Биномдық үлестірілу
    kk_arab: بىينومدىق ۇلەستىرىلۋ
    kk-cn: بىينومدىق ۇلەستىرىلۋ
    kk_cyrl: Биномдық үлестірілу
    kk-kz: Биномдық үлестірілу
    kk_latn: Bïnomdıq ülestirilw
    kk-tr: Bïnomdıq ülestirilw
    ko: 이항 분포
    lt: binominis skirstinys
    lv: binomiālais sadalījums
    mk: биномна распределба
    ms: Taburan binomial
    nb: binomisk fordeling
    nl: binomiale verdeling
    nn: binomisk fordeling
    nov: binomial distributione
    pl: Rozkład dwumianowy
    pt: distribuição binomial
    ro: Distribuția binomială
    ru: биномиальное распределение
    sk: Binomické rozdelenie
    sl: binomska porazdelitev
    sq: Shpërndarja binomiale
    sr: Binomna raspodela
    su: Sebaran binomial
    sv: binomialfördelning
    ta: ஈருறுப்புப் பரவல்
    th: การแจกแจงแบบทวินาม
    tl: Distribusyong binomial
    tr: ikiterimli dağılım
    uk: біноміальний розподіл
    ur: دو رقمی توزیع
    uz: Binomial taqsimot
    vi: Phân phối nhị thức
    yue: 二項分佈
    zh: 二項分佈
    zh_cn: 二项分布
    zh_hans: 二项分布
    zh_hant: 二項分佈
    zh_hk: 二項分佈
    zh_sg: 二项分布
    zh_tw: 二項分佈
---

# [[Binomial_Distribution]] 

#is_/same_as :: [[../../WikiData/WD~Binomial_distribution,185547|WD~Binomial_distribution,185547]] 

## #has_/text_of_/abstract 

> In probability theory and statistics, the binomial distribution with parameters n and p 
> is the discrete probability distribution of the number of successes 
> in a sequence of n independent experiments, each asking a yes–no question, 
> and each with its own Boolean-valued outcome: 
> - success (with probability p) or 
> - failure (with probability q = 1 - p). 
> 
> A single success/failure experiment is also called a Bernoulli trial or Bernoulli experiment, 
> and a sequence of outcomes is called a Bernoulli process; 
> for a single trial, i.e., n = 1, the binomial distribution is a Bernoulli distribution. 
> The binomial distribution is the basis for the binomial test of statistical significance.
>
> The binomial distribution is frequently used to model the number of successes in a sample of size n drawn with replacement from a population of size N. If the sampling is carried out without replacement, the draws are not independent and so the resulting distribution is a hypergeometric distribution, not a binomial one.  However, for N much larger than n, the binomial distribution remains a good approximation, and is widely used.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Binomial%20distribution) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Probability_Theory/Binomial_Distribution|Binomial_Distribution]] 

### #is_/same_as :: [[/_public/Mathematics/Probability_Theory/Binomial_Distribution.public|Binomial_Distribution.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Probability_Theory/Binomial_Distribution.internal|Binomial_Distribution.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Probability_Theory/Binomial_Distribution.protect|Binomial_Distribution.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Probability_Theory/Binomial_Distribution.private|Binomial_Distribution.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Probability_Theory/Binomial_Distribution.personal|Binomial_Distribution.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Probability_Theory/Binomial_Distribution.secret|Binomial_Distribution.secret]] 

