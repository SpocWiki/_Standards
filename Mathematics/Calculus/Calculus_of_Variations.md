---
aliases:
- Analiza e variacionit
- bariazioen kalkulu
- calcolo delle variazioni
- calcul des variations
- Calcul variațional
- Calculus of Variations
- Calculus_of_Variations
- calcwlws o amrywiadau
- Càlcol dle variassion
- càlcul de variacions
- cálculo de variaciones
- cálculo de variacións
- cálculo de variações
- Cálculu de variaciones
- kalkulu tal-varjazzjonijiet
- Phép tính biến phân
- rachunek wariacyjny
- variaatiolaskenta
- variacijski račun
- variada kalkulo
- variasjonsregning
- variasjonsrekning
- variatierekening
- variationskalkyl
- Variationsrechnung
- variationsregning
- Variatsioonarvutus
- variační počet
- Variačný počet
- Varijacijski račun
- variációszámítás
- Varïyacïalıq qïsap
- Λογισμός των μεταβολών
- варіаційне числення
- Вариациалы иҫәпләмә
- Вариацилле шутлав
- вариацион хынцынад
- Вариационно смятане
- вариационное исчисление
- Варияциалық қисап
- варыяцыйнае злічэнне
- варыяцыйнае зьлічэньне
- Конкременти
- ҳисобони тағйирот
- վարիացիոն հաշիվ
- חשבון וריאציות
- حساب التغيرات
- حسابان تغییرات
- ۆارىيياتسىيالىق قىيساپ
- विचरण-कलन
- ვარიაციული გამოთვლა
- 变分法
- 変分法
- 變分法
- 변분법
has_id_wikidata: Q216861
has_use:
- '[[_Standards/WikiData/WD~mathematical_optimization,141495|WD~mathematical_optimization,141495]]'
- '[[_Standards/WikiData/WD~physics,413|WD~physics,413]]'
instance_of: '[[_Standards/WikiData/WD~calculus,149972|WD~calculus,149972]]'
described_by_source:
- '[[../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
named_after: '[[_Standards/WikiData/WD~Q3658674,3658674|WD~Q3658674,3658674]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
part_of: '[[_Standards/WikiData/WD~calculus_of_variations,_systems_theory_and_control_theory,113208892|WD~calculus_of_variations,_systems_theory_and_control_theory,113208892]]'
subclass_of: '[[_Standards/WikiData/WD~mathematical_analysis,7754|WD~mathematical_analysis,7754]]'
P8189: 987007293785905171
Commons_category: Calculus of variations
Krugosvet_article: nauka_i_tehnika/matematika/VARIATSIONNOE_ISCHISLENIE.html
dv_has_:
  name_:
    ar: حساب التغيرات
    ast: Cálculu de variaciones
    ba: Вариациалы иҫәпләмә
    be: варыяцыйнае злічэнне
    be_tarask: варыяцыйнае зьлічэньне
    bg: Вариационно смятане
    ca: càlcul de variacions
    cs: variační počet
    cv: Вариацилле шутлав
    cy: calcwlws o amrywiadau
    da: variationsregning
    de: Variationsrechnung
    el: Λογισμός των μεταβολών
    en: calculus of variations
    eo: variada kalkulo
    es: cálculo de variaciones
    et: Variatsioonarvutus
    eu: bariazioen kalkulu
    fa: حسابان تغییرات
    fi: variaatiolaskenta
    fr: calcul des variations
    gl: cálculo de variacións
    he: חשבון וריאציות
    hi: विचरण-कलन
    hr: Varijacijski račun
    hu: variációszámítás
    hy: վարիացիոն հաշիվ
    it: calcolo delle variazioni
    ja: 変分法
    ka: ვარიაციული გამოთვლა
    kk: Варияциалық қисап
    kk_arab: ۆارىيياتسىيالىق قىيساپ
    kk-cn: ۆارىيياتسىيالىق قىيساپ
    kk_cyrl: Варияциалық қисап
    kk-kz: Варияциалық қисап
    kk_latn: Varïyacïalıq qïsap
    kk-tr: Varïyacïalıq qïsap
    ko: 변분법
    mt: kalkulu tal-varjazzjonijiet
    nb: variasjonsregning
    nl: variatierekening
    nn: variasjonsrekning
    os: вариацион хынцынад
    pl: rachunek wariacyjny
    pms: Càlcol dle variassion
    pt: cálculo de variações
    pt_br: cálculo de variações
    ro: Calcul variațional
    ru: вариационное исчисление
    sk: Variačný počet
    sl: variacijski račun
    sq: Analiza e variacionit
    sr: Конкременти
    sv: variationskalkyl
    tg: ҳисобони тағйирот
    uk: варіаційне числення
    ur: Calculus of variations
    vi: Phép tính biến phân
    wuu: 变分法
    yue: 變分法
    zh: 变分法
    zh_cn: 变分法
    zh_hans: 变分法
    zh_hant: 變分法
    zh_hk: 變分法
    zh_sg: 变分法
    zh_tw: 變分法
---

# [[Calculus_of_Variations]] 

#is_/same_as :: [[../../WikiData/WD~Calculus_of_variations,216861|WD~Calculus_of_variations,216861]] 

## #has_/text_of_/abstract 

> The calculus of variations (or variational calculus) is a field of mathematical analysis 
> that uses variations, which are small changes in functions and functionals, 
> to find maxima and minima of functionals: 
> mappings from a set of functions to the real numbers. 
> 
> Functionals are often expressed as definite integrals involving functions and their derivatives. 
> Functions that maximize or minimize functionals may be found 
> using the Euler–Lagrange equation of the calculus of variations.
>
> A simple example of such a problem is to find the curve of shortest length connecting two points. 
> If there are no constraints, the solution is a straight line between the points. 
> However, if the curve is constrained to lie on a surface in space, 
> then the solution is less obvious, and possibly many solutions may exist. 
> Such solutions are known as geodesics. 
> 
> A related problem is posed by Fermat's principle: 
> light follows the path of shortest optical length connecting two points, 
> which depends upon the material of the medium. 
> One corresponding concept in mechanics is the principle of least/stationary action.
>
> Many important problems involve functions of several variables. 
> Solutions of boundary value problems for the Laplace equation satisfy the Dirichlet's principle. 
> Plateau's problem requires finding a surface of minimal area 
> that spans a given contour in space: 
> a solution can often be found by dipping a frame in soapy water. 
> 
> Although such experiments are relatively easy to perform, 
> their mathematical formulation is far from simple: 
> there may be more than one locally minimizing surface, 
> and they may have non-trivial topology.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Calculus%20of%20variations) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Calculus/Calculus_of_Variations|Calculus_of_Variations]] 

### #is_/same_as :: [[/_public/Mathematics/Calculus/Calculus_of_Variations.public|Calculus_of_Variations.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Calculus/Calculus_of_Variations.internal|Calculus_of_Variations.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Calculus/Calculus_of_Variations.protect|Calculus_of_Variations.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Calculus/Calculus_of_Variations.private|Calculus_of_Variations.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Calculus/Calculus_of_Variations.personal|Calculus_of_Variations.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Calculus/Calculus_of_Variations.secret|Calculus_of_Variations.secret]] 

