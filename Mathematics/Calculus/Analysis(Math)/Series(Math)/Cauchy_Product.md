---
has_id_wikidata: Q1051404
dv_has_:
  name_:
    ca: Producte de Cauchy
    de: Cauchy-Produktformel
    el: Γινόμενο Κωσύ
    en: Cauchy product
    eo: produkto de Cauchy
    es: producto de Cauchy
    fr: produit de Cauchy
    gl: produto de Cauchy
    he: משפט קושי (תורת הטורים)
    hi: कौशी गुणनफल
    id: perkalian Cauchy
    it: prodotto di Cauchy
    ja: コーシー積
    ko: 코시 곱
    pms: Prodot ëd Cauchy
    pt: Produto de Cauchy
    ru: произведение Коши
    sl: Cauchyjev produkt
    th: ผลคูณโคชี
    tr: Cauchy çarpımı
    uk: Добуток Коші
    zh: 柯西乘积
aliases:
- Cauchy product
- Cauchy çarpımı
- Cauchy-Produktformel
- Cauchyjev produkt
- perkalian Cauchy
- Prodot ëd Cauchy
- prodotto di Cauchy
- Producte de Cauchy
- producto de Cauchy
- produit de Cauchy
- produkto de Cauchy
- produto de Cauchy
- Γινόμενο Κωσύ
- Добуток Коші
- произведение Коши
- משפט קושי (תורת הטורים)
- कौशी गुणनफल
- ผลคูณโคชี
- コーシー積
- 柯西乘积
- 코시 곱
---

# [[Cauchy_Product]] 

The Product of 2 Sums is calculated as the Sum of diagonal Products to ensure convergence. 

${\displaystyle \left(\sum _{i=0}^{\infty }a_{i}\right)\cdot \left(\sum _{j=0}^{\infty }b_{j}\right)=\sum _{k=0}^{\infty }c_{k}}$
with ${c_{k}=\sum _{l=0}^{k}a_{l}b_{k-l}}$ .

## #has_/text_of_/abstract 

> In mathematics, more specifically in mathematical analysis, 
> the **Cauchy product** is the discrete convolution of two infinite series. 
> 
> It is named after the French mathematician Augustin-Louis Cauchy.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Cauchy%20product) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Calculus/Analysis(Math)/Series(Math)/Cauchy_Product|Cauchy_Product]] 

### #is_/same_as :: [[/_public/Mathematics/Calculus/Analysis(Math)/Series(Math)/Cauchy_Product.public|Cauchy_Product.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Calculus/Analysis(Math)/Series(Math)/Cauchy_Product.internal|Cauchy_Product.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Calculus/Analysis(Math)/Series(Math)/Cauchy_Product.protect|Cauchy_Product.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Calculus/Analysis(Math)/Series(Math)/Cauchy_Product.private|Cauchy_Product.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Calculus/Analysis(Math)/Series(Math)/Cauchy_Product.personal|Cauchy_Product.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Calculus/Analysis(Math)/Series(Math)/Cauchy_Product.secret|Cauchy_Product.secret]] 

