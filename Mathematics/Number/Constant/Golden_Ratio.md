---
aliases:
- Altın oran
- Amḍan n wuṛeɣ
- aranymetszés
- cymedr euraid
- cóimheas órga
- Det gyldne snit
- det gyldne snitt
- Det gylne snittet
- Divina proportio
- Fi
- fi'u
- geuldene snede
- Goidner Schnitt
- golden ratio
- goldener Schnitt
- Golden_Ratio
- Gouden fyk
- goue snit
- gulden snede
- Gulden Snee
- Gulen Skeer
- Gullinsnið
- gyllene snittet
- Gëllene Schnëtt
- Gólde sni-j
- Kuldlõige
- Kultainen leikkaus
- Nisbah Keemasan
- Nombre d'aur
- nombre d'or
- Nonm di lò
- Nùmer d'òr
- Nùmmuru d'oru
- Númberu aureu
- número áureo
- Númiru aureu
- Nümar àuri
- Oltin kesim
- ora proporcio
- Prerja e artë
- proporção áurea
- Qızıl bölgü
- Rasio emas
- Ration auree
- secció àuria
- secțiunea de aur
- sezione aurea
- Tỷ lệ vàng
- urrezko zenbakia
- Zelta griezums
- zlati rez
- Zlatni rez
- Zlatý rez
- zlatý řez
- złoty podział
- Ôrinombe
- χρυσή τομή
- Алтын киселеш
- Алтын киҫелеш
- Алтын қатынас
- Залатое сечыва
- Залатое сячэнне
- Зарбуриш
- златен пресек
- златни пресек
- Златно сечение
- золотий перетин
- золотое сечение
- Ылттăн касăлу
- Ոսկե հատում
- יחס הזהב
- النسبه الدهبيه
- نسبة ذهبية
- نسبت طلایی
- وثق باب ریاضیات
- ڕێژەی زێڕین
- स्वर्णिमानुपात
- সোণালী অনুপাত
- সোনালী অনুপাত
- ਸੁਨਹਿਰੀ ਰਾਤਿਓ
- பொன் விகிதம்
- స్వర్ణ నిష్పత్తి
- സുവർണ്ണ അനുപാതം
- රන්මය අනුපාතය
- อัตราส่วนทอง
- ოქროს კვეთა
- 黃金分割
- 黃金比例
- 黄金分割率
- 黄金比
- 黄金比例
- 황금비
has_id_wikidata: Q41690
instance_of:
- '[[_Standards/WikiData/WD~algebraic_number,168817|WD~algebraic_number,168817]]'
- '[[_Standards/WikiData/WD~mathematical_constant,186509|WD~mathematical_constant,186509]]'
- '[[_Standards/WikiData/WD~irrational_number,607728|WD~irrational_number,607728]]'
- '[[_Standards/WikiData/WD~constructible_number,1321926|WD~constructible_number,1321926]]'
- '[[_Standards/WikiData/WD~Brjuno_number,4971764|WD~Brjuno_number,4971764]]'
- '[[_Standards/WikiData/WD~metallic_ratio,11634820|WD~metallic_ratio,11634820]]'
has_use:
- '[[_Standards/WikiData/WD~mathematics_and_art,2864650|WD~mathematics_and_art,2864650]]'
- '[[_Standards/WikiData/WD~mathematics_and_architecture,3535118|WD~mathematics_and_architecture,3535118]]'
described_by_source:
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138|WD~Granat_Encyclopedic_Dictionary,4532138]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._8,124737635|WD~Armenian_Soviet_Encyclopedia,_vol._8,124737635]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
different_from:
- '[[_Standards/WikiData/WD~Q9393496,9393496|WD~Q9393496,9393496]]'
- '[[_Standards/WikiData/WD~Goldens,50810258|WD~Goldens,50810258]]'
permanent_duplicated_item: '[[_Standards/WikiData/WD~Q96464922,96464922|WD~Q96464922,96464922]]'
has_part_s_: '[[_Standards/WikiData/WD~Golden_ratio_in_art,121911124|WD~Golden_ratio_in_art,121911124]]'
numeric_value: 1.618033988749895
defining_formula: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>&#x03C6;</mi><mo stretchy="false">=</mo><mrow data-mjx-texclass="ORD"><mfrac><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mn>1</mn><mo stretchy="false">+</mo><mrow data-mjx-texclass="ORD"><msqrt><mn>5</mn></msqrt></mrow></mrow></mrow><mrow data-mjx-texclass="ORD"><mn>2</mn></mrow></mfrac></mrow></mstyle></mrow></math>
notation: '[[_Standards/WikiData/WD~Φ_φ,10029|WD~Φ_φ,10029]]'
in_defining_formula:
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>&#x03C6;</mi></mstyle></mrow></math>
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mrow data-mjx-texclass="ORD"><msqrt><mn>5</mn></msqrt></mrow></mstyle></mrow></math>
Commons_gallery: Golden ratio
Commons_category: Golden ratio
Krugosvet_article: nauka_i_tehnika/matematika/ZOLOTOE_SECHENIE.html
dv_has_:
  name_:
    af: goue snit
    ar: نسبة ذهبية
    arz: النسبه الدهبيه
    as: সোণালী অনুপাত
    ast: Númberu aureu
    az: Qızıl bölgü
    ba: Алтын киҫелеш
    bar: Goidner Schnitt
    be: Залатое сячэнне
    be_tarask: Залатое сечыва
    bg: Златно сечение
    bn: সোনালী অনুপাত
    bs: Zlatni rez
    ca: secció àuria
    ckb: ڕێژەی زێڕین
    cs: zlatý řez
    cv: Ылттăн касăлу
    cy: cymedr euraid
    da: Det gyldne snit
    de: goldener Schnitt
    el: χρυσή τομή
    en: golden ratio
    eo: ora proporcio
    es: número áureo
    et: Kuldlõige
    eu: urrezko zenbakia
    ext: Númiru aureu
    fa: نسبت طلایی
    fi: Kultainen leikkaus
    fr: nombre d'or
    frr: Gulen Skeer
    fy: Gouden fyk
    ga: cóimheas órga
    gcr: Nonm di lò
    gl: número áureo
    gsw: Goldener Schnitt
    he: יחס הזהב
    hi: स्वर्णिमानुपात
    hr: Zlatni rez
    hu: aranymetszés
    hy: Ոսկե հատում
    ia: Ration auree
    id: Rasio emas
    is: Gullinsnið
    it: sezione aurea
    ja: 黄金比
    jbo: fi'u
    ka: ოქროს კვეთა
    kab: Amḍan n wuṛeɣ
    kk: Алтын қатынас
    ko: 황금비
    la: Divina proportio
    lb: Gëllene Schnëtt
    li: Gólde sni-j
    lmo: Nümar àuri
    lt: Fi
    lv: Zelta griezums
    lzh: 黃金分割
    mk: златен пресек
    ml: സുവർണ്ണ അനുപാതം
    ms: Nisbah Keemasan
    nb: det gyldne snitt
    nds_nl: geuldene snede
    nl: gulden snede
    nn: Det gylne snittet
    oc: Nombre d'aur
    pa: ਸੁਨਹਿਰੀ ਰਾਤਿਓ
    pl: złoty podział
    pms: Nùmer d'òr
    pt: proporção áurea
    pt_br: proporção áurea
    ro: secțiunea de aur
    ru: золотое сечение
    scn: Nùmmuru d'oru
    sh: Zlatni rez
    si: රන්මය අනුපාතය
    sk: Zlatý rez
    sl: zlati rez
    sq: Prerja e artë
    sr: златни пресек
    sr_ec: златни пресек
    sv: gyllene snittet
    ta: பொன் விகிதம்
    te: స్వర్ణ నిష్పత్తి
    tg: Зарбуриш
    th: อัตราส่วนทอง
    tr: Altın oran
    tt: Алтын киселеш
    uk: золотий перетин
    ur: وثق باب ریاضیات
    uz: Oltin kesim
    vi: Tỷ lệ vàng
    vls: Gulden Snee
    wa: Ôrinombe
    wuu: 黄金比例
    yue: 黃金比例
    zh: 黄金分割率
---

# [[Golden_Ratio]] 

#is_/same_as :: [[../../../WikiData/WD~Golden_ratio,41690|WD~Golden_ratio,41690]]
## #has_/text_of_/abstract 

> In mathematics, two quantities are in the golden ratio 
> if their ratio is the same as the ratio of their sum to the larger of the two quantities. 
> Expressed algebraically, for quantities ⁠a and ⁠b with ⁠a>b>0 
> ⁠ is in a golden ratio to b if
> {a + b}/a = a/b = φ
> where the Greek letter phi (⁠φ or ⁠ϕ⁠) denotes the golden ratio. 
> The constant ⁠φ⁠ satisfies the quadratic equation ⁠φ² = φ + 1
> ⁠ and is an irrational number with a value of
>
> The golden ratio was called the extreme and mean ratio by Euclid, 
> and the divine proportion by Luca Pacioli; it also goes by other names.
>
> Mathematicians have studied the golden ratio's properties since antiquity. 
> It is the ratio of a regular pentagon's diagonal to its side 
> and thus appears in the construction of the dodecahedron and icosahedron. 
> A golden rectangle—that is, a rectangle with an aspect ratio of φ
> ⁠—may be cut into a square and a smaller rectangle with the same aspect ratio. 
> The golden ratio has been used to analyze the proportions of natural objects 
> and artificial systems such as financial markets, in some cases based on dubious fits to data. 
> The golden ratio appears in some patterns in nature, 
> including the spiral arrangement of leaves and other parts of vegetation.
>
> Some 20th-century artists and architects, including Le Corbusier and Salvador Dalí, 
> have proportioned their works to approximate the golden ratio, 
> believing it to be aesthetically pleasing. 
> These uses often appear in the form of a golden rectangle.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Golden%20ratio) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Number/Constant/Golden_Ratio|Golden_Ratio]] 

### #is_/same_as :: [[/_public/Mathematics/Number/Constant/Golden_Ratio.public|Golden_Ratio.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Number/Constant/Golden_Ratio.internal|Golden_Ratio.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Number/Constant/Golden_Ratio.protect|Golden_Ratio.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Number/Constant/Golden_Ratio.private|Golden_Ratio.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Number/Constant/Golden_Ratio.personal|Golden_Ratio.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Number/Constant/Golden_Ratio.secret|Golden_Ratio.secret]] 

