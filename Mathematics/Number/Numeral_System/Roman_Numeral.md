---
aliases:
- Angka Romawi
- Angka Rumi
- Chiffes romaines
- cifre romane
- cyfry rzymskie
- earrooyn Raueagh
- Fuainumera o Roma
- Hejmarên romî
- Izwal n Yiṛmmuyn
- Lebadi la roma
- Lô-má sò͘-jī
- Namba za Kiroma
- nasin nanpa Lasina
- Niveradur roman
- numarasion romana
- Numberación romana
- Numeracion romana
- numeració romana
- numeración romana
- Numeration roman
- Numeraçon romana
- numeração romana
- numeri romani
- Numeros roman
- Numrat romakë
- numération romaine
- Nùmer romàṅ
- nùmmura rumani
- Papaha Rómapegua
- Reqemê Romenan
- Rim cifrleri
- Rim raqamlari
- rimske števke
- Rimski brojevi
- romaj ciferoj
- Roman numeral
- Roman numerals
- Romana cifri
- Romano nga ihap
- Romanong numero
- romartal
- Romeins cijfer
- Romeinse syfers
- Romeinske sifers
- Romen rakamları
- Romerska talsystemet
- romertal
- romertall
- Romiešu skaitļi
- Romėniški skaičiai
- Rooma numbrid
- Roomalaiset numerot
- Rum rəqəmləri
- Réimesch Zuelen
- Rímske číslice
- római számírás
- Rómverskir tölustafir
- Römische Zahlschrift
- Röömsche Tallen
- Röömsk taal
- Scistema numerico roman
- System rhifolion Rhufeinig
- số La Mã
- Uimhir Rómhánach
- Wilangan Romawi
- Zenbaketa erromatar
- římské číslice
- Ρωμαϊκοί αριθμοί
- Рим сандары
- Рим цифралары
- рим цифрҙары
- Рим шутлав йĕрки
- Римски бројеви
- Римски бројки
- Римски цифри
- римские цифры
- римські цифри
- Ром цифр
- рымскія лічбы
- Հռոմեական թվեր
- Հռովմէական թուանշաններ
- ספרות רומיות
- רוימישע צאל
- أرقام رومانية
- ارقام رومانيه
- رومن اعداد
- رومَن گرَٛنٛد
- رومی اعداد
- رىم رەقەملىرى
- عددنویسی رومی
- रोमन अंक
- रोमन संख्या
- রোমান সংখ্যা
- ਰੋਮਨ ਅੰਕ
- ரோம எண்ணுருக்கள்
- ರೋಮನ್ ಅಂಕಿಗಳು
- റോമൻ സംഖ്യാസമ്പ്രദായം
- රෝමානු සංඛ්යා
- เลขโรมัน
- རོ་མའི་ཨང་ཀི།
- ရောမကိန်းဂဏန်းများ
- რომაული ციფრები
- ᎠᏂᎶᎻ ᎢᏧᏅᏁᏗ ᏗᏎᏍᏗ ᏓᏃᏪᎵᏍᎬᎢ
- ローマ数字
- 罗马数字
- 羅馬數字
- 로마 숫자
has_id_wikidata: Q38918
instance_of:
- '[[_Standards/WikiData/WD~numeric_writing_system,140774]]'
- '[[_Standards/WikiData/WD~sign_value_numeral_system,353240]]'
used_by: '[[_Standards/WikiData/WD~chronogram,161108]]'
subclass_of: '[[_Standards/WikiData/WD~notation,2001982]]'
different_from: '[[_Standards/WikiData/WD~Latin_numerals,2097357]]'
has_part_s_:
- '[[_Standards/WikiData/WD~_,2932176]]'
- '[[_Standards/WikiData/WD~_,3011516]]'
- '[[_Standards/WikiData/WD~_,3147025]]'
- '[[_Standards/WikiData/WD~_,3206231]]'
- '[[_Standards/WikiData/WD~_,3273364]]'
- '[[_Standards/WikiData/WD~_,3553034]]'
- '[[_Standards/WikiData/WD~_,3570499]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137]]'
uses: '[[_Standards/WikiData/WD~Latin_alphabet,41670]]'
OmegaWiki_Defined_Meaning: 815575
Commons_category: Roman numerals
image: http://commons.wikimedia.org/wiki/Special:FilePath/CuttySarkRomNum.jpg
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/Zenbaki%20erromatarren%20arauak.png
Stack_Exchange_tag:
- https://stackoverflow.com/tags/roman-numerals
- https://tex.stackexchange.com/tags/roman-numerals
IMDb_keyword: roman-numeral-in-title
dv_has_:
  name_:
    af: Romeinse syfers
    ar: أرقام رومانية
    arz: ارقام رومانيه
    ast: Numberación romana
    az: Rum rəqəmləri
    azb: رومی اعداد
    ba: рим цифрҙары
    ban: Wilangan Romawi
    be: рымскія лічбы
    be_tarask: рымскія лічбы
    bg: Римски цифри
    bn: রোমান সংখ্যা
    bo: རོ་མའི་ཨང་ཀི།
    br: Niveradur roman
    bs: Rimski brojevi
    ca: numeració romana
    chr: ᎠᏂᎶᎻ ᎢᏧᏅᏁᏗ ᏗᏎᏍᏗ ᏓᏃᏪᎵᏍᎬᎢ
    cs: římské číslice
    cv: Рим шутлав йĕрки
    cy: System rhifolion Rhufeinig
    da: romertal
    de: Römische Zahlschrift
    diq: Reqemê Romenan
    el: Ρωμαϊκοί αριθμοί
    eml: Nùmer romàṅ
    en: Roman numerals
    eo: romaj ciferoj
    es: numeración romana
    et: Rooma numbrid
    eu: Zenbaketa erromatar
    fa: عددنویسی رومی
    fi: Roomalaiset numerot
    fr: numération romaine
    frr: Röömsk taal
    fy: Romeinske sifers
    ga: Uimhir Rómhánach
    gl: numeración romana
    gn: Papaha Rómapegua
    gv: earrooyn Raueagh
    he: ספרות רומיות
    hi: रोमन अंक
    hr: rimski brojevi
    hu: római számírás
    hy: Հռոմեական թվեր
    hyw: Հռովմէական թուանշաններ
    ia: Numeration roman
    id: Angka Romawi
    io: Romana cifri
    is: Rómverskir tölustafir
    it: numeri romani
    ja: ローマ数字
    ka: რომაული ციფრები
    kaa: Rim cifrleri
    kk: Рим сандары
    kn: ರೋಮನ್ ಅಂಕಿಗಳು
    ko: 로마 숫자
    ks: رومَن گرَٛنٛد
    ku: Hejmarên romî
    ky: Рим цифралары
    la: Numeri Romani
    lb: Réimesch Zuelen
    lfn: Numeros roman
    lij: Scistema numerico roman
    lt: Romėniški skaičiai
    lv: Romiešu skaitļi
    lzh: 羅馬數字
    mad: Angka Romawi
    mk: Римски бројки
    ml: റോമൻ സംഖ്യാസമ്പ്രദായം
    mn: Ром цифр
    mr: रोमन अंक
    ms: Angka Rumi
    mwl: Numeraçon romana
    my: ရောမကိန်းဂဏန်းများ
    nan: Lô-má sò͘-jī
    nb: romertall
    nds: Röömsche Tallen
    ne: रोमन संख्या
    nl: Romeins cijfer
    nn: romartal
    nrm: Chiffes romaines
    nso: Lebadi la roma
    oc: Numeracion romana
    pa: ਰੋਮਨ ਅੰਕ
    pl: cyfry rzymskie
    pt: numeração romana
    pt_br: numeração romana
    ro: cifre romane
    ru: римские цифры
    scn: nùmmura rumani
    sco: Roman numeral
    sh: Rimski brojevi
    shi: Izwal n Yiṛmmuyn
    si: රෝමානු සංඛ්යා
    sk: Rímske číslice
    sl: rimske števke
    sm: Fuainumera o Roma
    sq: Numrat romakë
    sr: Римски бројеви
    sv: Romerska talsystemet
    sw: Namba za Kiroma
    ta: ரோம எண்ணுருக்கள்
    th: เลขโรมัน
    tl: Romanong numero
    tok: nasin nanpa Lasina
    tr: Romen rakamları
    ug: رىم رەقەملىرى
    uk: римські цифри
    ur: رومن اعداد
    uz: Rim raqamlari
    vec: numarasion romana
    vi: số La Mã
    war: Romano nga ihap
    wuu: 罗马数字
    yi: רוימישע צאל
    yue: 羅馬數字
    zh: 罗马数字
    zh_cn: 罗马数字
    zh_hans: 罗马数字
    zh_hant: 羅馬數字
    zh_hk: 羅馬數字
    zh_mo: 羅馬數字
    zh-my: 罗马数字
    zh_sg: 罗马数字
    zh_tw: 羅馬數字
---
# [[Roman_Numeral]] 

#is_/same_as :: [[../../../WikiData/WD~Roman_numerals,38918|WD~Roman_numerals,38918]]  

## #has_/text_of_/abstract 

> Roman numerals are a numeral system that originated in ancient Rome 
> and remained the usual way of writing numbers throughout Europe 
> well into the Late Middle Ages. 
> 
> Numbers are written with combinations of letters from the Latin alphabet, 
> each with a fixed integer value. 
> 
> The modern style uses only these seven:
>
> 
>
> The use of Roman numerals continued long after the decline of the Roman Empire. From the 14th century on, Roman numerals began to be replaced by Arabic numerals; however, this process was gradual, and the use of Roman numerals persisted in various places, including on clock faces. For instance, on the clock of Big Ben (designed in 1852), the hours from 1 to 12 are written as:
>
> 
>
> The notations IV and IX can be read as "one less than five" (4) and "one less than ten" (9), although there is a tradition favouring the representation of "4" as "IIII" on Roman numeral clocks.
>
> 
>
> Other common uses include year numbers on monuments and buildings and copyright dates on the title screens of films and television programmes. MCM, signifying "a thousand, and a hundred less than another thousand", means 1900, so 1912 is written MCMXII. For the years of the current (21st) century, MM indicates 2000; this year is MMXXV (2025).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Roman%20numerals) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Number/Numeral_System/Roman_Numeral|Roman_Numeral]] 

### #is_/same_as :: [[/_public/Mathematics/Number/Numeral_System/Roman_Numeral.public|Roman_Numeral.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Number/Numeral_System/Roman_Numeral.internal|Roman_Numeral.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Number/Numeral_System/Roman_Numeral.protect|Roman_Numeral.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Number/Numeral_System/Roman_Numeral.private|Roman_Numeral.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Number/Numeral_System/Roman_Numeral.personal|Roman_Numeral.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Number/Numeral_System/Roman_Numeral.secret|Roman_Numeral.secret]] 

