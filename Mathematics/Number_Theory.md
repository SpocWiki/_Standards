---
aliases:
- arvoteooria
- arvuteooria
- bîrdoza jimare
- damcaniaeth rhifau
- damkaniezh an niveroù
- Getalleteorie
- getaltheorie
- lohkoteoriija
- lohoteoria
- lukuteoria
- lååkkteoria
- lý thuyết số
- Naba icavacava
- nacycmaci
- Nadharia ya namba
- nomba tiori
- nombroteorio
- numateor
- Number theory
- Number-theory
- Number_Theory
- nummer theory
- Sanlar teoriýasy
- sayılar nazariyesi
- sayılar teorisi
- Skaitliu teuorėjė
- skaitļu teorija
- skaičių teorija
- sonlar nazariyasi
- számelmélet
- Sò͘-lūn
- Taalenteorii
- Tallentheorie
- tallteori
- talnafræði
- talteori
- teori bilangan
- teori e numrave
- teori nombor
- teoria de nombres
- teoria de números
- teoria dei numeri
- teoria dels nombres
- Teoria di numer
- teoria dos números
- teoria liczb
- teoria numerelor
- teorie čísel
- teorija brojeva
- teorija tan-numri
- teorija števil
- Teorio di nombri
- teoriya ng bilang
- Teorya nin bilang
- teorìa dij nùmer
- teoría de numers
- teoría de númberos
- teoría de números
- teyorya han ihap
- teória čísel
- theoria de numeros
- Theoria numerorum
- théorie des nombres
- tiurìa dî nùmmura
- Téori di sé nonm
- téori wilangan
- uimhirtheoiric
- Zahlentheorie
- zenbaki-teoria
- Zuelentheorie
- ədədlər nəzəriyyəsi
- θεωρία αριθμών
- лыдпус теори
- назарияи ададҳо
- нымæцты теори
- Саамай улахан уопсай түҥэтээччи
- сандар теориясы
- теорія чисел
- теорија бројева
- теорија на броевите
- теория на числата
- теория чисел
- тооны онол
- тэорыя лікаў
- хисепсен теорийĕ
- һандар теорияһы
- թվերի տեսություն
- נומערן טעאריע
- תורת המספרים
- تيوري نومبور
- تیۆریی ژمارە
- طيوري د لأعداد
- نظرية الأعداد
- نظريه الاعداد
- نظریه اعداد
- نظریۂ عدد
- نمبر تھیوری
- अंकशास्त्र
- ल्याः सिद्धान्त
- संख्या सिद्धान्त
- संख्याशास्त्रम्
- सङ्ख्या सिद्धान्त
- সংখ্যাতত্ত্ব
- ਸੰਖਿਆ ਸਿਧਾਂਤ
- સંખ્યા સિદ્ધાંત
- எண் கோட்பாடு
- ಸಂಖ್ಯಾಸಿದ್ಧಾಂತ
- സംഖ്യാസിദ്ധാന്തം
- ทฤษฎีจำนวน
- ကိန်းသီအိုရီ
- რიცხვთა თეორია
- რიცხუეფიშ თეორია
- 数論
- 数论
- 數論
- 정수론
has_id_wikidata: Q12479
different_from: '[[_Standards/WikiData/WD~numerology,140732|WD~numerology,140732]]'
part_of:
- '[[_Standards/WikiData/WD~pure_mathematics,837863|WD~pure_mathematics,837863]]'
- '[[_Standards/WikiData/WD~mathematics,395|WD~mathematics,395]]'
instance_of: '[[_Standards/WikiData/WD~area_of_mathematics,1936384|WD~area_of_mathematics,1936384]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
topic_s_main_Wikimedia_portal: '[[_Standards/WikiData/WD~Portal_Number_theory,14615430|WD~Portal_Number_theory,14615430]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Number_theory,16648951|WD~Template_Number_theory,16648951]]'
has_part_s_: '[[_Standards/WikiData/WD~elementary_number_theory,18450254|WD~elementary_number_theory,18450254]]'
practiced_by: '[[_Standards/WikiData/WD~number_theorist,115653130|WD~number_theorist,115653130]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._4,124737630|WD~Armenian_Soviet_Encyclopedia,_vol._4,124737630]]'
subclass_of: '[[_Standards/WikiData/WD~mathematics,395|WD~mathematics,395]]'
is_the_study_of: '[[_Standards/WikiData/WD~integer,12503|WD~integer,12503]]'
Universal_Decimal_Classification: 511
OmegaWiki_Defined_Meaning: 797305
Library_of_Congress_Classification: QA241-QA247.5
image: http://commons.wikimedia.org/wiki/Special:FilePath/Euclid%27s%20Orchard%20%28large%29.svg
Stack_Exchange_tag:
- https://hsm.stackexchange.com/tags/number-theory
- https://math.stackexchange.com/tags/number-theory
- https://matheducators.stackexchange.com/tags/number-theory
- https://mathematica.stackexchange.com/tags/number-theory
- https://mathoverflow.net/tags/nt.number-theory
- https://stackoverflow.com/tags/number-theory
Classification_of_Instructional_Programs_code: 27.0102
Basisklassifikation: 31.14
Dewey_Decimal_Classification: 512.7
Krugosvet_article_archived_: nauka_i_tehnika/matematika/CHISEL_TEORIYA.html
Commons_category: Number theory
GitHub_topic: number-theory
dv_has_:
  name_:
    af: Getalleteorie
    an: teoría de numers
    ar: نظرية الأعداد
    ary: طيوري د لأعداد
    arz: نظريه الاعداد
    as: সংখ্যাতত্ত্ব
    ast: teoría de númberos
    az: ədədlər nəzəriyyəsi
    ba: һандар теорияһы
    bcl: Teorya nin bilang
    be: тэорыя лікаў
    be_tarask: тэорыя лікаў
    bg: теория на числата
    bn: সংখ্যাতত্ত্ব
    br: damkaniezh an niveroù
    bs: teorija brojeva
    ca: teoria de nombres
    ckb: تیۆریی ژمارە
    crh: sayılar nazariyesi
    cs: teorie čísel
    cv: хисепсен теорийĕ
    cy: damcaniaeth rhifau
    da: talteori
    de: Zahlentheorie
    el: θεωρία αριθμών
    en: number theory
    en_gb: number theory
    eo: nombroteorio
    es: teoría de números
    et: arvuteooria
    eu: zenbaki-teoria
    fa: نظریه اعداد
    fi: lukuteoria
    fj: Naba icavacava
    fr: théorie des nombres
    frr: Taalenteorii
    ga: uimhirtheoiric
    gan: 數論
    gcr: Téori di sé nonm
    gl: teoría de números
    gsw: Zahlentheorie
    gu: સંખ્યા સિદ્ધાંત
    he: תורת המספרים
    hi: संख्या सिद्धान्त
    hif: Number theory
    hr: teorija brojeva
    hu: számelmélet
    hy: թվերի տեսություն
    ia: theoria de numeros
    id: teori bilangan
    io: Teorio di nombri
    is: talnafræði
    it: teoria dei numeri
    ja: 数論
    jam: nomba tiori
    jbo: nacycmaci
    jv: téori wilangan
    ka: რიცხვთა თეორია
    kk: сандар теориясы
    kn: ಸಂಖ್ಯಾಸಿದ್ಧಾಂತ
    ko: 정수론
    ku: bîrdoza jimare
    la: Theoria numerorum
    lb: Zuelentheorie
    lmo: Teoria di numer
    lt: skaičių teorija
    lv: skaitļu teorija
    mag: सङ्ख्या सिद्धान्त
    mk: теорија на броевите
    ml: സംഖ്യാസിദ്ധാന്തം
    mn: тооны онол
    mr: अंकशास्त्र
    ms: teori nombor
    ms_arab: تيوري نومبور
    mt: teorija tan-numri
    my: ကိန်းသီအိုရီ
    nan: Sò͘-lūn
    nb: tallteori
    nds: Tallentheorie
    new: ल्याः सिद्धान्त
    nl: getaltheorie
    nn: talteori
    oc: teoria dels nombres
    os: нымæцты теори
    pa: ਸੰਖਿਆ ਸਿਧਾਂਤ
    pl: teoria liczb
    pms: teorìa dij nùmer
    pnb: نمبر تھیوری
    pt: teoria de números
    pt_br: teoria dos números
    rki: ကိန်းသီအိုရီ
    ro: teoria numerelor
    ru: теория чисел
    sa: संख्याशास्त्रम्
    sah: Саамай улахан уопсай түҥэтээччи
    scn: tiurìa dî nùmmura
    sco: nummer theory
    se: lohkoteoriija
    sgs: Skaitliu teuorėjė
    sh: teorija brojeva
    sk: teória čísel
    sl: teorija števil
    smn: lohoteoria
    sms: lååkkteoria
    sq: teori e numrave
    sr: теорија бројева
    sv: talteori
    sw: Nadharia ya namba
    ta: எண் கோட்பாடு
    tg: назарияи ададҳо
    th: ทฤษฎีจำนวน
    tk: Sanlar teoriýasy
    tl: teoriya ng bilang
    tr: sayılar teorisi
    udm: лыдпус теори
    uk: теорія чисел
    ur: نظریۂ عدد
    uz: sonlar nazariyasi
    vi: lý thuyết số
    vo: numateor
    vro: arvoteooria
    war: teyorya han ihap
    wuu: 数论
    xmf: რიცხუეფიშ თეორია
    yi: נומערן טעאריע
    yue: 數論
    zh: 数论
    zh_hans: 数论
---

# [[Number_Theory]] 

#is_/same_as :: [[../WikiData/WD~Number_theory,12479|WD~Number_theory,12479]] 

## #has_/text_of_/abstract 

> **Number theory** is a branch of pure mathematics 
> devoted primarily to the study of the integers and arithmetic functions. 
> 
> German mathematician Carl Friedrich Gauss (1777–1855) said, 
> "Mathematics is the queen of the sciences—and number theory is the queen of mathematics." 
> 
> Number theorists study prime numbers 
> as well as the properties of mathematical objects constructed from integers 
> (for example, rational numbers), 
> or defined as generalizations of the integers (for example, algebraic integers).
>
> Integers can be considered either in themselves or as solutions to equations (Diophantine geometry). 
> Questions in number theory are often best understood through the study of analytical objects 
> (for example, the Riemann zeta function) 
> that encode properties of the integers, primes or other number-theoretic objects in some fashion 
> (analytic number theory). 
> One may also study real numbers in relation to rational numbers; 
> for example, as approximated by the latter (Diophantine approximation).
>
> The older term for number theory is arithmetic. 
> By the early twentieth century, it had been superseded by number theory. 
> (The word arithmetic is used by the general public to mean elementary calculations; 
> it has also acquired other meanings in mathematical logic, as in Peano arithmetic, 
> and computer science, as in floating-point arithmetic.) 
> 
> The use of the term arithmetic for number theory regained some ground 
> in the second half of the twentieth century, arguably in part due to French influence. 
> In particular, arithmetical is commonly preferred as an adjective to number-theoretic.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Number%20theory) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Number_Theory|Number_Theory]] 

### #is_/same_as :: [[/_public/Mathematics/Number_Theory.public|Number_Theory.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Number_Theory.internal|Number_Theory.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Number_Theory.protect|Number_Theory.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Number_Theory.private|Number_Theory.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Number_Theory.personal|Number_Theory.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Number_Theory.secret|Number_Theory.secret]] 

