---
aliases:
- analytisk funktion
- foncion olomòrfa
- fonction holomorphe
- funció holomorfa
- función holomorfa
- functio holomorphica
- function holomorphe
- funcție olomorfă
- funkcja holomorficzna
- funksioni holomorfik
- funtzio holomorfiko
- funzione olomorfa
- funzioni olomorfa
- funziú ulumorfa
- função holomorfa
- holomorf fonksiyon
- holomorf funksjon
- holomorf funktion
- holomorf függvény
- holomorfa funciono
- holomorfa funkcio
- holomorfe functie
- holomorfinen funktio
- holomorfinė funkcija
- holomorfna funkcija
- holomorfná funkcia
- holomorfní funkce
- Holomorfo funcion
- holomorphe Funktion
- holomorphic function
- hàm chỉnh hình
- regulaarne funktsioon
- ολομορφική συνάρτηση
- галяморфная функцыя
- голоморфлы функция
- голоморфна функція
- голоморфная функция
- холоморфна функција
- холоморфна функция
- հոլոմորֆ ֆունկցիա
- פונקציה הולומורפית
- تابع هولومورفیک
- دالة تحليلية تامة التشكل
- होलोमार्फिक फलन
- முற்றுருவச் சார்பியம்
- ჰოლომორფული ფუნქცია
- 全純函數
- 全纯函数
- 正則関数
- 정칙 함수
has_id_wikidata: Q207476
studied_by: '[[_Standards/WikiData/WD~complex_analysis,193756|WD~complex_analysis,193756]]'
subclass_of:
- '[[_Standards/WikiData/WD~meromorphic_function,217616|WD~meromorphic_function,217616]]'
- '[[_Standards/WikiData/WD~function_of_a_complex_variable,56312591|WD~function_of_a_complex_variable,56312591]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
described_by_source: '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
related_image: http://commons.wikimedia.org/wiki/Special:FilePath/Conformal%20map.svg
dv_has_:
  name_:
    ar: دالة تحليلية تامة التشكل
    ast: función holomorfa
    ba: голоморфлы функция
    be_tarask: галяморфная функцыя
    bg: холоморфна функция
    ca: funció holomorfa
    cbk_zam: Holomorfo funcion
    cs: holomorfní funkce
    da: holomorf funktion
    de: holomorphe Funktion
    el: ολομορφική συνάρτηση
    en: holomorphic function
    eo: holomorfa funkcio
    es: función holomorfa
    et: regulaarne funktsioon
    eu: funtzio holomorfiko
    fa: تابع هولومورفیک
    fi: holomorfinen funktio
    fr: fonction holomorphe
    gl: función holomorfa
    he: פונקציה הולומורפית
    hi: होलोमार्फिक फलन
    hu: holomorf függvény
    hy: հոլոմորֆ ֆունկցիա
    ia: function holomorphe
    io: holomorfa funciono
    it: funzione olomorfa
    ja: 正則関数
    ka: ჰოლომორფული ფუნქცია
    ko: 정칙 함수
    la: functio holomorphica
    lmo: funziú ulumorfa
    lt: holomorfinė funkcija
    nb: holomorf funksjon
    nl: holomorfe functie
    nn: holomorf funksjon
    oc: foncion olomòrfa
    pl: funkcja holomorficzna
    pt: função holomorfa
    pt_br: função holomorfa
    ro: funcție olomorfă
    ru: голоморфная функция
    scn: funzioni olomorfa
    sk: holomorfná funkcia
    sl: holomorfna funkcija
    sq: funksioni holomorfik
    sr: холоморфна функција
    sr_ec: холоморфна функција
    sr_el: holomorfna funkcija
    sv: analytisk funktion
    ta: முற்றுருவச் சார்பியம்
    tr: holomorf fonksiyon
    uk: голоморфна функція
    vi: hàm chỉnh hình
    yue: 全純函數
    zh: 全纯函数
    zh_cn: 全纯函数
    zh_hans: 全纯函数
    zh_hant: 全純函數
    zh_hk: 全純函數
    zh_sg: 全纯函数
    zh_tw: 全純函數
---

# [[Holomorphic_Function]] 

#is_/same_as :: [[_Standards/WikiData/WD~Holomorphic_function,207476|WD~Holomorphic_function,207476]] 

## #has_/text_of_/abstract 

> In mathematics, a holomorphic function is a complex-valued function 
> of one or more complex variables that is complex differentiable in a neighbourhood of 
> each point in a domain in complex coordinate space C^n⁠
>
> ⁠The existence of a complex derivative in a neighbourhood is a very strong condition: 
> It implies that a holomorphic function is infinitely differentiable 
> and locally equal to its own Taylor series (is analytic). 
> Holomorphic functions are the central objects of study in complex analysis.
>
> Though the term analytic function is often used interchangeably with "holomorphic function", 
> the word **"analytic" is defined in a broader sense** to denote 
> any function (real, complex, or of more general type) that can be written as a 
> convergent power series in a neighbourhood of each point in its domain. 
> 
> That **all holomorphic functions are complex analytic functions, and vice versa**, 
> is a major theorem in complex analysis.
>
> Holomorphic functions are also sometimes referred to as regular functions. 
> A holomorphic function whose domain is the whole complex plane 
> is called an entire function. 
> 
> The phrase "holomorphic at a point z0" means not just differentiable at ⁠z0⁠, 
> but differentiable everywhere within some close neighbourhood of ⁠z0⁠ in the complex plane.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Holomorphic%20function) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Function(Math)/smooth_Function/Holomorphic_Function|Holomorphic_Function]] 

### #is_/same_as :: [[/_public/Mathematics/Function(Math)/smooth_Function/Holomorphic_Function.public|Holomorphic_Function.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Function(Math)/smooth_Function/Holomorphic_Function.internal|Holomorphic_Function.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Function(Math)/smooth_Function/Holomorphic_Function.protect|Holomorphic_Function.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Function(Math)/smooth_Function/Holomorphic_Function.private|Holomorphic_Function.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Function(Math)/smooth_Function/Holomorphic_Function.personal|Holomorphic_Function.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Function(Math)/smooth_Function/Holomorphic_Function.secret|Holomorphic_Function.secret]] 

