---
has_id_wikidata: Q1524945
dv_has_:
  name_:
    af: ternêre bewerking
    ca: operació ternària
    cs: ternární operace
    de: dreistellige Verknüpfung
    en: ternary operation
    eo: triloka operacio
    es: operación ternaria
    et: Ternaarne tehe
    eu: eragiketa hirutar
    fr: opération ternaire
    he: פעולה טרנארית
    hu: ternáris művelet
    id: Operasi ternary
    it: operazione ternaria
    ja: 三項演算
    ko: 삼항 연산
    nn: ternær operasjon
    pl: działanie trójargumentowe
    pt: operação ternária
    ro: operație ternară
    ru: тернарная операция
    sl: tričlena operacija
    th: การดำเนินการไตรภาค
    uk: тернарна операція
    vec: oparasion ternària
    vi: phép toán ba ngôi
    zh: 三元運算
aliases:
- dreistellige Verknüpfung
- działanie trójargumentowe
- eragiketa hirutar
- oparasion ternària
- operació ternària
- operación ternaria
- Operasi ternary
- operazione ternaria
- operação ternária
- operație ternară
- opération ternaire
- phép toán ba ngôi
- Ternaarne tehe
- ternary operation
- ternáris művelet
- ternární operace
- ternær operasjon
- ternêre bewerking
- triloka operacio
- tričlena operacija
- тернарна операція
- тернарная операция
- פעולה טרנארית
- การดำเนินการไตรภาค
- 三元運算
- 三項演算
- 삼항 연산
---

# [[Function~ternary]] 

The most common ternary Function is the `?:` Operator or its equivalend `if/else` structure. 
It is not an Operator, because the first Argument is a [[../../schema-org/Class/is_a_/Data_Type/Boolean|Boolean]] Value. 

## #has_/text_of_/abstract 

> In mathematics, a **Ternary operation** is an n-ary operation with [[../../schema-org/Arity|Arity]] = 3. 
> A ternary operation on a set A takes any given three elements of A 
> and combines them to form a single element of A.
>
> In computer science, 
> a ternary operator is an operator that takes three arguments as input 
> and returns one output.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Ternary%20operation) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Function(Math)/Function~ternary|Function~ternary]] 

### #is_/same_as :: [[/_public/Mathematics/Function(Math)/Function~ternary.public|Function~ternary.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Function(Math)/Function~ternary.internal|Function~ternary.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Function(Math)/Function~ternary.protect|Function~ternary.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Function(Math)/Function~ternary.private|Function~ternary.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Function(Math)/Function~ternary.personal|Function~ternary.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Function(Math)/Function~ternary.secret|Function~ternary.secret]] 

