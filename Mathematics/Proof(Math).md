---
aliases:
- Beweis
- Bewies
- Bewis
- bukti matematik
- Chèng-bêng
- chứng minh toán học
- cipra
- Demonstratio mathematica
- demonstração
- demonstrație matematică
- Demostracion matematica
- demostració
- Demostración matematica
- Demostración matemática
- dimostrazione matematica
- Dimustrazzioni matimàtica
- Dokaz
- dowód
- Démonstrasyon
- démonstration mathématique
- Dôkaz
- Froga matematiko
- Karig-onan matematika
- matemaatiline tõestus
- Matemaattinen todistus
- matematický důkaz
- matematika pruvo
- matematikai bizonyítás
- matematiksel ispat
- matematinis įrodymas
- matematisk bevis
- matematiskt bevis
- Matematički dokaz
- matematični dokaz
- Matematėnis iruodėms
- Matemātisks pierādījums
- mathematesche Beweis
- Mathematical proof
- mathematischer Beweis
- Matimatikal pruuf
- Pang-matematikang patibay
- pembuktian matematika
- prawf mathemategol
- Preuva matemàtica
- Proba matemática
- Proof(Math)
- prova matemática
- prova mathematic
- Provat matematikore
- Riyazi sübut
- riyaziy isbat
- Stærðfræðileg sönnun
- Thibitisho la kihisabati
- wiskundig bewijs
- wiskundige bewys
- Ĕнентерӳ (математика)
- μαθηματική απόδειξη
- доведення
- Дәлелдеу
- Математічне доказательство
- математик исбатлау
- Математик иҫбатлау
- Математическо доказателство
- математическое доказательство
- математической доказательство
- Математички доказ
- матэматычны доказ
- Մաթեմաթիկական ապացոյց
- Մաթեմատիկական ապացույց
- הוכחה
- מאטעמאטישער דערווייז
- ئیسپات
- برهان
- برهان د لماط
- برهان رياضي
- بوقتي ماتماتيک
- ریاضیاتی ثبوت
- ریاضیکي ثبوت
- میتھمیٹیکل ثبوت
- गणितीय उपपत्ति
- गणितीय पुरावा
- গাণিতিক প্রমাণ
- গাণিতিক প্ৰমাণ
- ਗਣਿਤਕ ਸਬੂਤ
- கணித நிறுவல்
- ಗಣಿತದ ಪುರಾವೆ
- തെളിവ് ഗണിതശാസ്ത്രത്തിൽ
- ගණිතමය ඔප්පු කිරීම්
- การพิสูจน์เชิงคณิตศาสตร์
- მათემატიკური დამტკიცება
- 數學證明
- 証明
- 證明
- 证明
- 증명
has_id_wikidata: Q11538
Commons_category: Mathematical proof
Stack_Exchange_tag:
- https://math.stackexchange.com/tags/proofs
- https://matheducators.stackexchange.com/tags/proofs
different_from: '[[_Standards/WikiData/WD~Q19814025,19814025|WD~Q19814025,19814025]]'
follows: '[[_Standards/WikiData/WD~conjecture,319141|WD~conjecture,319141]]'
has_characteristic:
- '[[_Standards/WikiData/WD~proof,516021|WD~proof,516021]]'
- '[[_Standards/WikiData/WD~validity,1047000|WD~validity,1047000]]'
has_goal: '[[_Standards/WikiData/WD~theorem,65943|WD~theorem,65943]]'
instance_of:
- '[[_Standards/WikiData/WD~mathematical_concept,24034552|WD~mathematical_concept,24034552]]'
- '[[_Standards/WikiData/WD~mathematical_textform,114424400|WD~mathematical_textform,114424400]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
related_image:
- http://commons.wikimedia.org/wiki/Special:FilePath/Twocolumnproof.png
- http://commons.wikimedia.org/wiki/Special:FilePath/LK%20dem%20contraposition.png
- http://commons.wikimedia.org/wiki/Special:FilePath/P.%20Oxy.%20I%2029.jpg
said_to_be_the_same_as: '[[_Standards/WikiData/WD~formal_proof,2762418|WD~formal_proof,2762418]]'
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/Beispiel%20zum%20Aufbau%20eines%20Beweises.svg
studied_in: '[[_Standards/WikiData/WD~proof_theory,852732|WD~proof_theory,852732]]'
subclass_of:
- '[[_Standards/WikiData/WD~argument,186619|WD~argument,186619]]'
- '[[_Standards/WikiData/WD~mathematical_reasoning,122766417|WD~mathematical_reasoning,122766417]]'
uses:
- '[[_Standards/WikiData/WD~theorem,65943|WD~theorem,65943]]'
- '[[_Standards/WikiData/WD~logic,8078|WD~logic,8078]]'
- '[[_Standards/WikiData/WD~axiom,17736|WD~axiom,17736]]'
video:
- http://commons.wikimedia.org/wiki/Special:FilePath/Pythag%20anim.gif
- http://commons.wikimedia.org/wiki/Special:FilePath/%D0%94%D0%BE%D0%BA%D0%B0%D0%B7%D0%B0%D1%82%D0%B5%D0%BB%D1%8C%D1%81%D1%82%D0%B2%D0%BE.webm
- http://commons.wikimedia.org/wiki/Special:FilePath/Beweise%20-%20Quatematik.webm
dv_has_:
  name_:
    af: wiskundige bewys
    an: Demostración matematica
    ar: برهان رياضي
    ary: برهان د لماط
    as: গাণিতিক প্ৰমাণ
    ast: Demostración matemática
    az: Riyazi sübut
    ba: Математик иҫбатлау
    be: матэматычны доказ
    be_tarask: матэматычны доказ
    bg: Математическо доказателство
    bn: গাণিতিক প্রমাণ
    bs: Matematički dokaz
    ca: demostració
    ckb: ئیسپات
    crh: riyaziy isbat
    cs: matematický důkaz
    cv: Ĕнентерӳ (математика)
    cy: prawf mathemategol
    da: matematisk bevis
    de: mathematischer Beweis
    el: μαθηματική απόδειξη
    en: mathematical proof
    en_ca: Mathematical proof
    en_gb: mathematical proof
    eo: matematika pruvo
    es: demostración matemática
    et: matemaatiline tõestus
    eu: Froga matematiko
    fa: برهان
    fi: Matemaattinen todistus
    fr: démonstration mathématique
    frr: Bewis
    gan: 數學證明
    gcr: Démonstrasyon
    gl: Proba matemática
    gsw: Beweis
    he: הוכחה
    hi: गणितीय उपपत्ति
    hif: mathematical proof
    hr: Matematički dokaz
    hu: matematikai bizonyítás
    hy: Մաթեմատիկական ապացույց
    hyw: Մաթեմաթիկական ապացոյց
    ia: prova mathematic
    id: pembuktian matematika
    is: Stærðfræðileg sönnun
    it: dimostrazione matematica
    ja: 証明
    jam: Matimatikal pruuf
    jbo: cipra
    ka: მათემატიკური დამტკიცება
    kk: Дәлелдеу
    kn: ಗಣಿತದ ಪುರಾವೆ
    ko: 증명
    la: Demonstratio mathematica
    lb: mathematesche Beweis
    lt: matematinis įrodymas
    lv: Matemātisks pierādījums
    lzh: 證明
    mk: Математички доказ
    ml: തെളിവ് ഗണിതശാസ്ത്രത്തിൽ
    mr: गणितीय पुरावा
    ms: bukti matematik
    ms_arab: بوقتي ماتماتيک
    nan: Chèng-bêng
    nb: matematisk bevis
    nds: Bewies
    nl: wiskundig bewijs
    nn: matematisk bevis
    oc: Demostracion matematica
    pa: ਗਣਿਤਕ ਸਬੂਤ
    pl: dowód
    pms: Preuva matemàtica
    pnb: میتھمیٹیکل ثبوت
    ps: ریاضیکي ثبوت
    pt: prova matemática
    pt_br: demonstração
    ro: demonstrație matematică
    ru: математическое доказательство
    rue: Математічне доказательство
    scn: Dimustrazzioni matimàtica
    sgs: Matematėnis iruodėms
    sh: Dokaz
    si: ගණිතමය ඔප්පු කිරීම්
    sk: Dôkaz
    sl: matematični dokaz
    sq: Provat matematikore
    sr: математички доказ
    sr_ec: математички доказ
    sr_el: matematički dokaz
    sv: matematiskt bevis
    sw: Thibitisho la kihisabati
    ta: கணித நிறுவல்
    th: การพิสูจน์เชิงคณิตศาสตร์
    tl: Pang-matematikang patibay
    tr: matematiksel ispat
    tt: математик исбатлау
    udm: математической доказательство
    uk: доведення
    ur: ریاضیاتی ثبوت
    vi: chứng minh toán học
    war: Karig-onan matematika
    wuu: 证明
    yi: מאטעמאטישער דערווייז
    yue: 數學證明
    zh: 證明
    zh_cn: 证明
---

# [[Proof(Math)]] 

#is_/same_as :: [[../WikiData/WD~proof_technique,105530558|WD~proof_technique,105530558]]  
#is_/same_as :: [[../../WikiData/WD~Mathematical_proof,11538|WD~Mathematical_proof,11538]] 

## #has_/text_of_/abstract 

> A **Mathematical proof** is a deductive argument for a mathematical statement, 
> showing that the stated assumptions logically guarantee the conclusion. 
>
> The argument may use other previously established statements, such as theorems; 
> but every proof can, in principle, be constructed using only certain basic or original assumptions 
> known as axioms, along with the accepted rules of inference. 
> 
> Proofs are examples of exhaustive deductive reasoning that establish logical certainty, 
> to be distinguished from empirical arguments 
> or non-exhaustive inductive reasoning that establish "reasonable expectation". 
> 
> Presenting many cases in which the statement holds is not enough for a proof, 
> which must demonstrate that the statement is true in all possible cases. 
> 
> A proposition that has not been proved but is believed to be true is known as a conjecture, 
> or a hypothesis if frequently used as an assumption for further mathematical work.
>
> Proofs employ logic expressed in mathematical symbols, 
> along with natural language that usually admits some ambiguity. 
> 
> In most mathematical literature, proofs are written in terms of rigorous informal logic. 
> Purely formal proofs, written fully in symbolic language without the involvement of natural language, 
> are considered in proof theory. 
> 
> The distinction between formal and informal proofs 
> has led to much examination of current and historical mathematical practice, 
> quasi-empiricism in mathematics, and so-called folk mathematics, 
> oral traditions in the mainstream mathematical community or in other cultures. 
> 
> The philosophy of mathematics is concerned with the role of language and logic in proofs, 
> and mathematics as a language.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Mathematical%20proof) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Proof(Math)|Proof(Math)]] 

### #is_/same_as :: [[/_public/Mathematics/Proof(Math).public|Proof(Math).public]] 

### #is_/same_as :: [[/_internal/Mathematics/Proof(Math).internal|Proof(Math).internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Proof(Math).protect|Proof(Math).protect]] 

### #is_/same_as :: [[/_private/Mathematics/Proof(Math).private|Proof(Math).private]] 

### #is_/same_as :: [[/_personal/Mathematics/Proof(Math).personal|Proof(Math).personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Proof(Math).secret|Proof(Math).secret]] 

