---
aliases:
- bî-chek-hun
- calcalas
- calcolo infinitesimale
- calcul infinitesimal
- calcul infinitezimal
- calcul infinitésimal
- calculo
- calculo infinitesimal
- Calculus
- calcwlws
- càlcul infinitesimal
- cálculo
- cálculo infinitesimal
- cálculu
- cónto infiniteximałe
- diferenciálny a integrálny počet
- differentiaali- ja integraalilaskenta
- Infinitesimaalraekening
- infinitesimalkalkyl
- Infinitesimalrechnung
- infinitesimalregning
- infinitezimalni račun
- infinitezimální počet
- integralinis ir diferencialinis skaičiavimas
- kaalkulasii
- kakyula
- kal'kul
- kalkulo
- kalkulu
- kalkulus
- Kalkül
- kalkülüs
- kialkiulos
- mì-chit-fûn-ho̍k
- rachunek różniczkowy i całkowy
- rēķini
- Sidsimban
- vi tích phân
- wiskundige analyse
- yupaylliy
- örsmæðareikningur
- λογισμός
- диференциално и интегрално смятане
- зьлічэньне
- интеграл ба дифференциал тоолол
- инфинитезимални рачун
- инфинитезимално сметање
- математик тәхлил
- математикăлла анализ
- математическай анализ
- математический анализ
- матэматычны аналіз
- обчислення
- ҳисобон
- մաթեմատիկական անալիզ
- חשבון אינפיניטסימלי
- קאלקולוס
- تفاضل وتكامل
- جیاکاری و تەواوکاری
- حساب التفاضل والتكامل
- حساب دیفرانسیل و انتگرال
- حسابان
- علم احصا
- کلکولوس
- کیلکولس
- कलन
- क्याल्कुलस
- কলন গণিত
- ক্যালকুলাস
- ਹਿਸਾਬ ਦਿਫਰੇਨਸੀਆਲ ਅਤੇ ਇਨਤੇਗਰਾਲ
- કલન શાસ્ત્ર
- நுண்கணிதம்
- కలన గణితము
- കലനം
- කලනය
- แคลคูลัส
- ကဲကုလပ်
- ကဲးကုလတ်သ်
- კალკულუსი
- ካልኩለስ
- 微分積分学
- 微积分
- 微积分学
- 微積分
- 미적분학
has_id_wikidata: Q149972
based_on:
- "[[_Standards/WikiData/WD~Cavalieri's_principle,586686|WD~Cavalieri's_principle,586686]]"
- '[[_Standards/WikiData/WD~method_of_exhaustion,865019|WD~method_of_exhaustion,865019]]'
Colon_Classification: B32
Commons_category: Calculus
described_by_source:
- '[[../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~The_Nuttall_Encyclopædia,3181656|WD~The_Nuttall_Encyclopædia,3181656]]'
- '[[_Standards/WikiData/WD~Encyclopedic_Lexicon,4532135|WD~Encyclopedic_Lexicon,4532135]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633|WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633]]'
Dewey_Decimal_Classification: 515
different_from:
- '[[_Standards/WikiData/WD~Calculus,400405|WD~Calculus,400405]]'
- '[[_Standards/WikiData/WD~calculus,617079|WD~calculus,617079]]'
- '[[_Standards/WikiData/WD~multivariable_calculus,2035437|WD~multivariable_calculus,2035437]]'
- '[[_Standards/WikiData/WD~Q13538498,13538498|WD~Q13538498,13538498]]'
- '[[_Standards/WikiData/WD~Calculus,18092768|WD~Calculus,18092768]]'
discoverer_or_inventor:
- '[[_Standards/WikiData/WD~Isaac_Newton,935|WD~Isaac_Newton,935]]'
- '[[_Standards/WikiData/WD~Gottfried_Wilhelm_Leibniz,9047|WD~Gottfried_Wilhelm_Leibniz,9047]]'
has_part_s_:
- '[[_Standards/WikiData/WD~differential_calculus,149999|WD~differential_calculus,149999]]'
- '[[_Standards/WikiData/WD~integral_calculus,150008|WD~integral_calculus,150008]]'
history_of_topic: '[[_Standards/WikiData/WD~history_of_calculus,2338709|WD~history_of_calculus,2338709]]'
instance_of: '[[_Standards/WikiData/WD~area_of_mathematics,1936384|WD~area_of_mathematics,1936384]]'
Krugosvet_article: matematika/matematicheskiy-analiz
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
OmegaWiki_Defined_Meaning: 350515
replaces: '[[_Standards/WikiData/WD~sublime_calculation,47457983|WD~sublime_calculation,47457983]]'
social_media_followers: 78572
Stack_Exchange_tag:
- https://math.stackexchange.com/tags/calculus
- https://matheducators.stackexchange.com/tags/calculus
subclass_of:
- '[[_Standards/WikiData/WD~advanced_mathematics,4464|WD~advanced_mathematics,4464]]'
- '[[_Standards/WikiData/WD~mathematical_analysis,7754|WD~mathematical_analysis,7754]]'
subreddit: calculus
theorized_by:
- '[[_Standards/WikiData/WD~Isaac_Newton,935|WD~Isaac_Newton,935]]'
- '[[_Standards/WikiData/WD~Gottfried_Wilhelm_Leibniz,9047|WD~Gottfried_Wilhelm_Leibniz,9047]]'
topic_s_main_template:
- '[[_Standards/WikiData/WD~Template_Calculus,6740481|WD~Template_Calculus,6740481]]'
- '[[_Standards/WikiData/WD~Template_Calculus_topics,20326646|WD~Template_Calculus_topics,20326646]]'
Wikimedia_outline: '[[_Standards/WikiData/WD~outline_of_calculus,7112574|WD~outline_of_calculus,7112574]]'
dv_has_:
  name_:
    af: Calculus
    am: ካልኩለስ
    an: calculo
    ar: حساب التفاضل والتكامل
    arz: تفاضل وتكامل
    as: কলন গণিত
    ast: cálculu
    bcl: kalkulus
    be: матэматычны аналіз
    be_tarask: зьлічэньне
    bg: диференциално и интегрално смятане
    blk: ကဲးကုလတ်သ်
    bn: ক্যালকুলাস
    bs: kalkulus
    ca: càlcul infinitesimal
    ckb: جیاکاری و تەواوکاری
    cs: infinitezimální počet
    cv: математикăлла анализ
    cy: calcwlws
    da: infinitesimalregning
    de: Infinitesimalrechnung
    de_ch: Kalkül
    diq: kalkulus
    dtp: Sidsimban
    el: λογισμός
    en: calculus
    en_ca: calculus
    en_gb: calculus
    eo: kalkulo
    es: cálculo infinitesimal
    eu: kalkulu
    fa: حساب دیفرانسیل و انتگرال
    fi: differentiaali- ja integraalilaskenta
    fr: calcul infinitésimal
    ga: calcalas
    gan: 微積分
    gl: cálculo infinitesimal
    gu: કલન શાસ્ત્ર
    ha: Calculus
    hak: mì-chit-fûn-ho̍k
    he: חשבון אינפיניטסימלי
    hi: कलन
    hif: calculus
    hr: infinitezimalni račun
    hu: kalkulus
    hy: մաթեմատիկական անալիզ
    ia: calculo infinitesimal
    iba: Kalkulus
    id: kalkulus
    io: kalkulo
    is: örsmæðareikningur
    it: calcolo infinitesimale
    ja: 微分積分学
    jam: kialkiulos
    jv: kalkulus
    ka: კალკულუსი
    ko: 미적분학
    la: calculus
    lfn: calculo
    li: Infinitesimaalraekening
    lt: integralinis ir diferencialinis skaičiavimas
    lv: rēķini
    lzh: 微積分
    mad: kalkulus
    mag: कलन
    min: kalkulus
    mk: инфинитезимално сметање
    ml: കലനം
    mn: интеграл ба дифференциал тоолол
    mr: कलन
    ms: kalkulus
    ms_arab: کلکولوس
    my: ကဲကုလပ်
    nan: bî-chek-hun
    nb: kalkulus
    new: क्याल्कुलस
    nl: wiskundige analyse
    nn: kalkulus
    oc: calcul infinitesimal
    om: kaalkulasii
    pa: ਹਿਸਾਬ ਦਿਫਰੇਨਸੀਆਲ ਅਤੇ ਇਨਤੇਗਰਾਲ
    pl: rachunek różniczkowy i całkowy
    pnb: کیلکولس
    pt: cálculo
    pt_br: cálculo
    qu: yupaylliy
    rki: ကဲကုလပ်
    ro: calcul infinitezimal
    ru: математический анализ
    sah: математическай анализ
    sco: calculus
    sd: علم احصا
    sh: infinitezimalni račun
    si: කලනය
    sk: diferenciálny a integrálny počet
    sl: infinitezimalni račun
    sr: инфинитезимални рачун
    ss: calculus
    sv: infinitesimalkalkyl
    ta: நுண்கணிதம்
    te: కలన గణితము
    tg: ҳисобон
    th: แคลคูลัส
    tl: calculus
    tr: kalkülüs
    tt: математик тәхлил
    tum: kakyula
    uk: обчислення
    ur: حسابان
    vec: cónto infiniteximałe
    vep: kal'kul
    vi: vi tích phân
    war: kalkulo
    wuu: 微积分学
    yi: קאלקולוס
    yue: 微積分
    zh: 微积分学
    zh_hans: 微积分
---

# [[Calculus]] 

#is_/same_as :: [[../WikiData/WD~Calculus,149972|WD~Calculus,149972]] 

## #has_/text_of_/abstract 

> **Calculus** is the mathematical study of continuous change, in the same way that geometry is the study of shape, and algebra is the study of generalizations of arithmetic operations.
>
> Originally called infinitesimal calculus or "the calculus of infinitesimals", it has two major branches, differential calculus and integral calculus. The former concerns instantaneous rates of change, and the slopes of curves, while the latter concerns accumulation of quantities, and areas under or between curves. These two branches are related to each other by the fundamental theorem of calculus. They make use of the fundamental notions of convergence of infinite sequences and infinite series to a well-defined limit. It is the "mathematical backbone" for dealing with problems where variables change with time or another reference variable.
>
> Infinitesimal calculus was formulated separately in the late 17th century by Isaac Newton and Gottfried Wilhelm Leibniz. Later work, including codifying the idea of limits, put these developments on a more solid conceptual footing. Today, calculus is widely used in science, engineering, biology, and even has applications in social science and other branches of math.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Calculus) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Mathematics/Calculus|Calculus]] 

### #is_/same_as :: [[/_public/Mathematics/Calculus.public|Calculus.public]] 

### #is_/same_as :: [[/_internal/Mathematics/Calculus.internal|Calculus.internal]] 

### #is_/same_as :: [[/_protect/Mathematics/Calculus.protect|Calculus.protect]] 

### #is_/same_as :: [[/_private/Mathematics/Calculus.private|Calculus.private]] 

### #is_/same_as :: [[/_personal/Mathematics/Calculus.personal|Calculus.personal]] 

### #is_/same_as :: [[/_secret/Mathematics/Calculus.secret|Calculus.secret]] 

