---
aliases:
  - Anferensa
  - argumentatio
  - dôsledok
  - gevolgtrekking
  - Imperensiya
  - infearas
  - Inference
  - Inferencia
  - inferenco
  - inferens
  - inferensa
  - inferensi
  - inferentzia
  - Inferenz
  - inferenza
  - Inferență
  - infero
  - inferència
  - inferência
  - inférence
  - išvada
  - järeldus
  - következtetés
  - ni'icpa
  - Nəticəçıxarma
  - poellata
  - päättely
  - Schlussfolgerung
  - sklepanje
  - Sluss (Logik)
  - slutledning
  - slutning
  - sonuç çıkarma
  - suy luận
  - wnioskowanie
  - xulosa chiqarish
  - zaključak
  - zaključivanje
  - συμπέρασμα
  - висновок
  - выснова
  - закључак
  - закључивање
  - заклучок
  - Кăлару
  - логический вывод
  - ой тұжырымы
  - разсъждение
  - Մտահանգում
  - היסק
  - ئاکامگری
  - استدلال
  - استنباط
  - अनुमान
  - अनुमान (तर्क)
  - অনুমিতি
  - ਮੰਤਕੀ ਅਨੁਮਾਨ
  - அனுமானம்
  - การอนุมาน
  - 推理
  - 推論
  - 推论
  - 추론
has_id_wikidata: Q408386
model_item:
  - "[[_Standards/WikiData/WD~abductive_reasoning,308495|WD~abductive_reasoning,308495]]"
  - "[[_Standards/WikiData/WD~backward_chaining,1141183|WD~backward_chaining,1141183]]"
has_part_s_:
  - "[[_Standards/WikiData/WD~deductive_reasoning,484284|WD~deductive_reasoning,484284]]"
  - "[[_Standards/WikiData/WD~inductive_reasoning,484511|WD~inductive_reasoning,484511]]"
partially_coincident_with: "[[_Standards/WikiData/WD~reasoning,1156402|WD~reasoning,1156402]]"
subclass_of: "[[_Standards/WikiData/WD~process,3249551|WD~process,3249551]]"
different_from: "[[_Standards/WikiData/WD~Inferentzia,12259609|WD~Inferentzia,12259609]]"
instance_of:
  - "[[_Standards/WikiData/WD~sourcing_circumstance,104637420|WD~sourcing_circumstance,104637420]]"
  - "[[_Standards/WikiData/WD~type_of_process,116505632|WD~type_of_process,116505632]]"
has_characteristic: "[[_Standards/WikiData/WD~type_of_reasoning,126723190|WD~type_of_reasoning,126723190]]"
Wikidata_property: based on heuristic
Library_of_Congress_Classification: BC199.I47
PhilPapers_topic: inference
Stack_Exchange_tag: https://stackoverflow.com/tags/inference
dv_has_:
  name_:
    ar: استدلال
    ast: Inferencia
    az: Nəticəçıxarma
    be_tarask: выснова
    bg: разсъждение
    bn: অনুমিতি
    br: poellata
    bs: zaključak
    ca: inferència
    ckb: ئاکامگری
    cs: inference
    cv: Кăлару
    da: inferens
    de: Schlussfolgerung
    el: συμπέρασμα
    en: inference
    en_ca: inference
    en_gb: inference
    eo: inferenco
    es: inferencia
    et: järeldus
    eu: inferentzia
    fa: استنباط
    fi: päättely
    fr: inférence
    ga: infearas
    gl: inferencia
    he: היסק
    hi: अनुमान (तर्क)
    hr: zaključak
    hu: következtetés
    hy: Մտահանգում
    id: inferensi
    io: infero
    it: inferenza
    ja: 推論
    jbo: ni'icpa
    kk: ой тұжырымы
    ko: 추론
    la: argumentatio
    lt: išvada
    mk: заклучок
    mr: अनुमान
    ms: inferens
    nb: slutning
    nds: Sluss (Logik)
    new: अनुमान
    nl: gevolgtrekking
    pa: ਮੰਤਕੀ ਅਨੁਮਾਨ
    pl: wnioskowanie
    pms: Anferensa
    pnb: استدلال
    pt: inferência
    pt_br: inferência
    ro: Inferență
    ru: логический вывод
    sh: zaključivanje
    sk: dôsledok
    sl: sklepanje
    sr: закључак
    sr_ec: закључивање
    sv: slutledning
    ta: அனுமானம்
    th: การอนุมาน
    tl: Imperensiya
    tr: sonuç çıkarma
    uk: висновок
    ur: استدلال
    uz: xulosa chiqarish
    vec: inferensa
    vi: suy luận
    wuu: 推理
    yue: 推理
    zh: 推论
    zh_cn: 推理
---

# [[Inference]] 

#is_/same_as :: [[_Standards/WikiData/WD~Inference,408386|WD~Inference,408386]] 

| Mode          | Guarantees Truth? | Amplifies Content? | Direction                      |
| ------------- | ----------------- | ------------------ | ------------------------------ |
| **Deduction** | Yes               | No                 | General → Particular           |
| **Induction** | No                | Yes                | Particular → General           |
| **Abduction** | No                | Yes                | Observation → Best Explanation |
| **Bayesian**  | No                | Yes                | Particular → General           |

## #has_/text_of_/abstract 

> **Inference**s are steps in logical reasoning, moving from premises to logical consequences; 
> etymologically, the word infer means to "carry forward". 
> 
> Inference is theoretically traditionally divided into deduction and induction, 
> a distinction that in Europe dates at least to Aristotle (300s BC). 
> 
> **Deduction** is inference deriving logical conclusions 
> from premises known or assumed to be true, 
> with the laws of valid inference being studied in logic. 
> 
> **Induction** is inference from particular evidence to a universal conclusion. 
> 
> A third type of inference is sometimes distinguished, 
> notably by Charles Sanders Peirce, 
> contradistinguishing **abduction** from induction.
>
> Various fields study how inference is done in practice. 
> 
> Human inference (i.e. how humans draw conclusions) 
> is traditionally studied within the fields of logic, argumentation studies, 
> and cognitive psychology; 
> artificial intelligence researchers develop automated inference systems 
> to emulate human inference. 
> 
> Statistical inference uses mathematics 
> to draw conclusions in the presence of uncertainty. 
> This generalizes deterministic reasoning, 
> with the absence of uncertainty as a special case. 
> 
> Statistical inference uses quantitative or qualitative (categorical) data 
> which may be subject to random variations.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Inference) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Philosophy/Logic/Inference|Inference]] 

### #is_/same_as :: [[/_public/Philosophy/Logic/Inference.public|Inference.public]] 

### #is_/same_as :: [[/_internal/Philosophy/Logic/Inference.internal|Inference.internal]] 

### #is_/same_as :: [[/_protect/Philosophy/Logic/Inference.protect|Inference.protect]] 

### #is_/same_as :: [[/_private/Philosophy/Logic/Inference.private|Inference.private]] 

### #is_/same_as :: [[/_personal/Philosophy/Logic/Inference.personal|Inference.personal]] 

### #is_/same_as :: [[/_secret/Philosophy/Logic/Inference.secret|Inference.secret]] 

