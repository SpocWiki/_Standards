...
```mermaid
flowchart TD
Goal["Goal: Prove φ"] --> Type{Shape of φ}
Type -->|"φ = P → Q"| Direct["Direct Proof\n Assume P, derive Q"]
Type -->|"φ = P → Q"| Contra["Contrapositive\n Assume ¬Q, derive ¬P"]
Type -->|"φ = ¬P"| ByContrad["Proof by Contradiction\n Assume P, derive ⊥"]
Type -->|"φ = ∀n P(n)"| Induction["Mathematical Induction"]
Type -->|"φ = P ∨ Q"| Cases["Case Analysis\n Prove each case"]
Type -->|"φ = ∃x P(x)"| Witness["Constructive / Non-Constructive\n Exhibit a witness"]
```
...
PL -->|"adds ∀, ∃"| FOL
FOL -->|"quantifies over predicates"| SOL
PL -->|"adds □, ◇"| ML
PL -->|"formal derivation"| PT
PL -->|"truth in models"| MT
FOL --> PT
FOL --> MT
```
