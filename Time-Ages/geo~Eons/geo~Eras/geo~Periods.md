---
aliases:
- ajastu
- aldi
- cyfnod daearegol
- doba
- dómina xeolóxica
- dövr
- földtörténeti időszak
- geochronologická perioda
- geologia periodo
- geologisk period
- Geološki period
- geo~Periods
- jarðsöguleg tímabil
- kausi
- kỷ địa chất
- okres
- perioadă
- period
- periode
- periode (geologie)
- periodo
- periodus
- periòde
- peryòd
- perìodo
- període geològic
- período
- período geológico
- período xeolóxico
- période géologique
- sistema
- spys dororiethel
- System
- tréimhse gheolaíoch
- zaman
- ìgbà ayé
- ģeoloģiskais periods
- γεωλογική περίοδος
- асри ҷеолоҷӣ
- геалагічны перыяд
- геалягічны пэрыяд
- геологический период
- геологиялык мезгил
- геологиялық бөлім
- геолошки период
- період
- период
- Երկրաբանական ժամանակաշրջան
- תת-תור גאולוגי
- دوره
- زمین شناسی دوره ئون
- عصر
- भूवैज्ञानिक कल्प
- ยุค (ธรณีวิทยา)
- ခေတ်
- ၵၢပ်ႈ
- პერიოდი
- 紀
- 紀 (地質學)
- 纪
- 기
has_id_wikidata: Q392928
Commons_category: Geology by period
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138]]'
different_from:
- '[[_Standards/WikiData/WD~archaeological_period,15401633]]'
- '[[_Standards/WikiData/WD~system,403523]]'
equivalent_class: http://dbpedia.org/ontology/GeologicalPeriod
has_part_s_:
- '[[_Standards/WikiData/WD~epoch,754897]]'
- '[[_Standards/WikiData/WD~subperiod,2239359]]'
locator_map_image: http://commons.wikimedia.org/wiki/Special:FilePath/Geological%20time%20spiral.png
part_of: '[[_Standards/WikiData/WD~era,630830]]'
standards_body: '[[_Standards/WikiData/WD~International_Commission_on_Stratigraphy,402104]]'
studied_by: '[[_Standards/WikiData/WD~geochronology,213891]]'
subclass_of:
- '[[_Standards/WikiData/WD~geochronological_unit,4005761]]'
- '[[_Standards/WikiData/WD~unit_of_time,1790144]]'
topic_s_main_template: '[[_Standards/WikiData/WD~Template_Geological_period,10305208]]'
Wolfram_Language_entity_type: GeologicalPeriod
dv_is_:
  part_of: '[[../geo~Eras]]'
  same_as:
  - '[[/_Standards/Time-Ages/geo~Eons/geo~Eras/geo~Periods|geo~Periods]]'
  - '[[/_public/Time-Ages/geo~Eons/geo~Eras/geo~Periods.public|geo~Periods.public]]'
  - '[[/_internal/Time-Ages/geo~Eons/geo~Eras/geo~Periods.internal|geo~Periods.internal]]'
  - '[[/_protect/Time-Ages/geo~Eons/geo~Eras/geo~Periods.protect|geo~Periods.protect]]'
  - '[[/_private/Time-Ages/geo~Eons/geo~Eras/geo~Periods.private|geo~Periods.private]]'
  - '[[/_personal/Time-Ages/geo~Eons/geo~Eras/geo~Periods.personal|geo~Periods.personal]]'
  - '[[/_secret/Time-Ages/geo~Eons/geo~Eras/geo~Periods.secret|geo~Periods.secret]]'
dv_has_:
  name_:
    af: periode (geologie)
    ar: عصر
    ast: dómina xeolóxica
    az: dövr
    be: геалагічны перыяд
    be-tarask: геалягічны пэрыяд
    bg: период
    bs: Geološki period
    ca: període geològic
    cs: geochronologická perioda
    cy: cyfnod daearegol
    da: periode
    de: Periode
    el: γεωλογική περίοδος
    en: period
    en-gb: period
    eo: geologia periodo
    es: período
    et: ajastu
    eu: aldi
    fa: دوره
    fi: kausi
    fr: période géologique
    ga: tréimhse gheolaíoch
    gl: período xeolóxico
    he: תת-תור גאולוגי
    hi: भूवैज्ञानिक कल्प
    ht: peryòd
    hu: földtörténeti időszak
    hy: Երկրաբանական ժամանակաշրջան
    id: zaman
    is: jarðsöguleg tímabil
    it: periodo
    ja: 紀
    ka: პერიოდი
    kk: геологиялық бөлім
    ko: 기
    kw: spys dororiethel
    ky: геологиялык мезгил
    la: periodus
    lv: ģeoloģiskais periods
    mk: геолошки период
    ms: zaman
    my: ခေတ်
    mzn: زمین شناسی دوره ئون
    nb: periode
    nds: System
    nl: periode
    nn: periode
    nov: periode
    oc: periòde
    pl: okres
    pt: período geológico
    ro: perioadă
    ru: геологический период
    sah: период
    sh: geološki period
    shn: ၵၢပ်ႈ
    sl: doba
    sr: геолошки период
    sr-ec: геолошки период
    sr-el: geološki period
    sv: geologisk period
    tg: асри ҷеолоҷӣ
    th: ยุค (ธรณีวิทยา)
    uk: період
    uz: sistema
    vec: perìodo
    vi: kỷ địa chất
    yo: ìgbà ayé
    yue: 紀
    zh: 纪
    zh-hant: 紀 (地質學)
  parts: '[[geo~Periods/geo~Epoch]]'
dv_is_part_of: '[[../geo~Eras]]'
dv_has_parts: '[[geo~Periods/geo~Epoch]]'
dv_is_same_as:
- '[[/_Standards/Time-Ages/geo~Eons/geo~Eras/geo~Periods|geo~Periods]]'
- '[[/_public/Time-Ages/geo~Eons/geo~Eras/geo~Periods.public|geo~Periods.public]]'
- '[[/_internal/Time-Ages/geo~Eons/geo~Eras/geo~Periods.internal|geo~Periods.internal]]'
- '[[/_protect/Time-Ages/geo~Eons/geo~Eras/geo~Periods.protect|geo~Periods.protect]]'
- '[[/_private/Time-Ages/geo~Eons/geo~Eras/geo~Periods.private|geo~Periods.private]]'
- '[[/_personal/Time-Ages/geo~Eons/geo~Eras/geo~Periods.personal|geo~Periods.personal]]'
- '[[/_secret/Time-Ages/geo~Eons/geo~Eras/geo~Periods.secret|geo~Periods.secret]]'
---

# [[geo~Periods]]

is_part_of = `=this.dv_is_part_of` 
has_parts = `=this.dv_has_parts` 

A period is equivalent to a chronostratigraphic system. These are millions to tens of millions of years in length.

# [[geo~Periods]]

?A period is equivalent to a chronostratigraphic system. These are millions to tens of millions of years in length.

## All Contents

```folderv
```

```ccard
type: folder_brief_live
style : card
col: 2
briefMax: 128
noteOnly: true
```


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Time-Ages/geo~Eons/geo~Eras/geo~Periods|geo~Periods]] 

### #is_/same_as :: [[/_public/Time-Ages/geo~Eons/geo~Eras/geo~Periods.public|geo~Periods.public]] 

### #is_/same_as :: [[/_internal/Time-Ages/geo~Eons/geo~Eras/geo~Periods.internal|geo~Periods.internal]] 

### #is_/same_as :: [[/_protect/Time-Ages/geo~Eons/geo~Eras/geo~Periods.protect|geo~Periods.protect]] 

### #is_/same_as :: [[/_private/Time-Ages/geo~Eons/geo~Eras/geo~Periods.private|geo~Periods.private]] 

### #is_/same_as :: [[/_personal/Time-Ages/geo~Eons/geo~Eras/geo~Periods.personal|geo~Periods.personal]] 

### #is_/same_as :: [[/_secret/Time-Ages/geo~Eons/geo~Eras/geo~Periods.secret|geo~Periods.secret]] 

