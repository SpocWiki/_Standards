---
aliases:
- chhibavipnaski familiya
- dangin harshen
- dil aileleri
- dillərin genetik təsnifatı
- družina jezikov
- fameja lenguìstega
- famia di idioma
- famigghia linguìstica
- famiglia linguistica
- famije lenghìstiche
- familh yezhoù
- familha lingüistica
- famili bilong tokples
- familia de lenguas
- familia lengüística
- familia linguistic
- familia linguistica
- familia lingüistica
- familia lingüística
- familia llingüística
- familie de limbi
- familjet e gjuhëve
- famille de langues
- familya kang manga pulong
- famìggia lengoìstica
- famìlia limbìstica
- família linguística
- família lingüística
- fianakaviam-piteny
- fine teangacha
- giellabearaš
- gí-hē
- hizkuntza-familia
- jazyková rodina
- jezičke porodice i jezici
- jezična porodica
- jezične porodice i jezici
- kalbų šeimos
- keelkond
- keluarga bahasa
- keyey zıwanan
- kielikunta
- kielâpeerâ
- kiilkund
- kulupu mama toki
- kynney çhengey
- Language family
- Language_Family
- leid faimily
- linguaro
- lingva familio
- mag-anak ng mga wika
- malbata zimanî
- mhuri ye mutauro
- ngî-ne
- ngữ hệ
- nyelvcsalád
- pamilia ti pagsasao
- pamilya nin tataramon
- rimaykunap ayllun
- rodzina językowa
- rumpun bahasa
- rumpun basa
- rěčna swójba
- Schprachfamilie
- Spraakfamilie
- Sprachfamilie
- spraokfemielje
- Spriakfamile
- Sproachfômilie
- Sproakgruppe
- sprogfamilie
- Sproochefamill
- språkfamilie
- språkfamilj
- taalfamilie
- taalfamylje
- taelfemieljes
- tawacult n tutlayin
- tawja n tutlayin
- tel ğäiläse
- teulu ieithyddol
- tillar oilasi
- tiller toparı
- tungumálaætt
- tíl ayilesí
- vahhaeh
- valodu saime
- volūdu saime
- zyvonə xyjzon
- ñe'ẽ ogaygua
- ǩiõllkåʹdd
- γλωσσική οικογένεια
- οἰκία γλωσσῶν
- јазично семејство
- језичка породица
- бзэхэм я гинетикэ классификациэр
- генеалагічная класіфікацыя моў
- езиково семейство
- кылъёслэн семья
- мацӀазул хъизан
- моўная сям’я
- мовна сім’я
- оилаи забонҳо
- тілдердің топтары
- тел гаиләсе
- тел ғаиләһе
- тил бүлө
- языкова родина
- языковая семья
- լեզվաընտանիք
- משפחת שפות
- שפראך-פאמיליע
- أسرة لغات
- بولی ٹبر
- خانواده زبانی
- خێزانی زمانەکان
- د ژبې کورنۍ
- دیل عائیلهسی
- زوون خانواده
- لساني خاندان
- لسانی خاندان
- لَسٲنؠ خاندان
- کلوارݢ بهاس
- ߞߊ߲ ߝߊ߬ߓߏ߲
- भाय् माचाछि
- भाषा परिवार
- भाषा-परिवार
- भाषाकुटुम्बः
- भाषाकुळ
- भाषापरिबार
- ভাষা পরিবার
- ਭਾਸ਼ਾ ਪਰਿਵਾਰ
- ଭାଷା ପରିବାର
- மொழிகளும், மொழிக் குடும்பங்களும்
- భాషా కుటుంబము
- ಭಾಷಾ ವಂಶವೃಕ್ಷ
- ഭാഷാഗോത്രങ്ങൾ
- ตระกูลของภาษา
- ဘာသာစကား မိသားစု
- ენათა ოჯახები
- ნინეფიშ ფანიეფი
- ᱯᱟᱹᱨᱥᱤ ᱜᱷᱟᱨᱚᱸᱡᱽ
- 語族
- 語系
- 語言系屬分類
- 语系
- 语言系属分类
- 어족
- "\U000110A6\U000110B0\U0001108E\U000110B0\U00011086 \U000110A3\U000110A9\U000110B1\U000110AB\U000110B0\U000110A9"
has_id_wikidata: Q25295
studied_by:
- '[[_Standards/WikiData/WD~sociolinguistics,160845]]'
- '[[_Standards/WikiData/WD~linguistics,8162]]'
different_from: '[[_Standards/WikiData/WD~sprachbund,839463]]'
subclass_of:
- '[[_Standards/WikiData/WD~language_group,941501]]'
- '[[_Standards/WikiData/WD~languoid,17376908]]'
part_of:
- '[[_Standards/WikiData/WD~macrofamily,949344]]'
- '[[_Standards/WikiData/WD~genetic_classification,25521366]]'
has_part_s_: '[[_Standards/WikiData/WD~language_subfamily,11820611]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Language_families,13099669]]'
described_by_source:
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~Soziolinguistika_Hiztegia,124156602]]'
instance_of: '[[_Standards/WikiData/WD~languoid_class,28923954]]'
properties_for_this_type:
- Freebase ID
- Glottolog code
- WALS family code
- ISO 639-5 code
has_list: '[[_Standards/WikiData/WD~list_of_language_families,33898]]'
OmegaWiki_Defined_Meaning: 1282916
exact_match: https://ontology.londisizwe.org/mola#LanguageFamily
Wolfram_Language_entity_type: LanguageFamily
Commons_category: Languages by family
dv_has_:
  name_:
    af: taalfamilie
    an: familia lingüistica
    ar: أسرة لغات
    ary: أسرة لغات
    ast: familia llingüística
    av: мацӀазул хъизан
    az: dillərin genetik təsnifatı
    azb: دیل عائیلهسی
    ba: тел ғаиләһе
    bcl: pamilya nin tataramon
    be: генеалагічная класіфікацыя моў
    be_tarask: моўная сям’я
    bg: езиково семейство
    bho: "\U000110A6\U000110B0\U0001108E\U000110B0\U00011086 \U000110A3\U000110A9\U000110B1\U000110AB\U000110B0\U000110A9"
    bn: ভাষা পরিবার
    br: familh yezhoù
    bs: jezičke porodice i jezici
    ca: família lingüística
    ckb: خێزانی زمانەکان
    co: famiglia linguistica
    crh-ro: tíl ayilesí
    cs: jazyková rodina
    cy: teulu ieithyddol
    da: sprogfamilie
    de: Sprachfamilie
    diq: keyey zıwanan
    el: γλωσσική οικογένεια
    en: language family
    en-us: language family
    eo: lingva familio
    es: familia de lenguas
    et: keelkond
    eu: hizkuntza-familia
    ext: familia lengüística
    fa: خانواده زبانی
    fi: kielikunta
    fr: famille de langues
    frr: Spriakfamile
    fy: taalfamylje
    ga: fine teangacha
    gl: familia lingüística
    gn: ñe'ẽ ogaygua
    grc: οἰκία γλωσσῶν
    gsw: Schprachfamilie
    gv: kynney çhengey
    ha: dangin harshen
    hak: ngî-ne
    he: משפחת שפות
    hi: भाषा-परिवार
    hif: language family
    hr: jezična porodica
    hsb: rěčna swójba
    hu: nyelvcsalád
    hy: լեզվաընտանիք
    ia: familia linguistic
    id: rumpun bahasa
    ilo: pamilia ti pagsasao
    io: linguaro
    is: tungumálaætt
    it: famiglia linguistica
    ja: 語族
    jv: rumpun basa
    ka: ენათა ოჯახები
    kaa: tiller toparı
    kab: tawacult n tutlayin
    kbd: бзэхэм я гинетикэ классификациэр
    kk: тілдердің топтары
    kn: ಭಾಷಾ ವಂಶವೃಕ್ಷ
    ko: 어족
    krj: familya kang manga pulong
    ks: لَسٲنؠ خاندان
    ku: malbata zimanî
    ku_latn: malbata zimanî
    ky: тил бүлө
    la: familia linguistica
    lb: Sproochefamill
    li: spraokfemielje
    lij: famìggia lengoìstica
    lt: kalbų šeimos
    ltg: volūdu saime
    lv: valodu saime
    mag: भाषापरिबार
    mai: भाषा परिवार
    mg: fianakaviam-piteny
    mk: јазично семејство
    ml: ഭാഷാഗോത്രങ്ങൾ
    mr: भाषाकुळ
    ms: keluarga bahasa
    ms_arab: کلوارݢ بهاس
    my: ဘာသာစကား မိသားစု
    mzn: زوون خانواده
    nan: gí-hē
    nb: språkfamilie
    nds: Spraakfamilie
    ne: भाषा परिवार
    new: भाय् माचाछि
    nl: taalfamilie
    nn: språkfamilie
    nqo: ߞߊ߲ ߝߊ߬ߓߏ߲
    oc: familha lingüistica
    or: ଭାଷା ପରିବାର
    pa: ਭਾਸ਼ਾ ਪਰਿਵਾਰ
    pap: famia di idioma
    pl: rodzina językowa
    pms: famije lenghìstiche
    pnb: بولی ٹبر
    ps: د ژبې کورنۍ
    pt: família linguística
    pt_br: família linguística
    qu: rimaykunap ayllun
    rki: ဘာသာစကား မိသားစု
    rmy: chhibavipnaski familiya
    ro: familie de limbi
    ru: языковая семья
    rue: языкова родина
    sa: भाषाकुटुम्बः
    sat: ᱯᱟᱹᱨᱥᱤ ᱜᱷᱟᱨᱚᱸᱡᱽ
    sc: famìlia limbìstica
    scn: famigghia linguìstica
    sco: leid faimily
    sd: لساني خاندان
    se: giellabearaš
    sh: jezične porodice i jezici
    shi: tawja n tutlayin
    sk: jazyková rodina
    sl: družina jezikov
    smn: kielâpeerâ
    sms: ǩiõllkåʹdd
    sn: mhuri ye mutauro
    sq: familjet e gjuhëve
    sr: језичка породица
    sr_ec: језичка породица
    stq: Sproakgruppe
    sv: språkfamilj
    ta: மொழிகளும், மொழிக் குடும்பங்களும்
    te: భాషా కుటుంబము
    tg: оилаи забонҳо
    th: ตระกูลของภาษา
    tl: mag-anak ng mga wika
    tly: zyvonə xyjzon
    tok: kulupu mama toki
    tpi: famili bilong tokples
    tr: dil aileleri
    tt: тел гаиләсе
    tt_cyrl: тел гаиләсе
    tt_latn: tel ğäiläse
    udm: кылъёслэн семья
    uk: мовна сім’я
    ur: لسانی خاندان
    uz: tillar oilasi
    vec: fameja lenguìstega
    vi: ngữ hệ
    vmf: Sproachfômilie
    vro: kiilkund
    wuu: 语系
    xmf: ნინეფიშ ფანიეფი
    yi: שפראך-פאמיליע
    yo: Language family
    yue: 語系
    za: vahhaeh
    zea: taelfemieljes
    zh: 语言系属分类
    zh_cn: 语言系属分类
    zh_hans: 语言系属分类
    zh_hant: 語言系屬分類
    zh_hk: 語言系屬分類
    zh_mo: 語言系屬分類
    zh-my: 语言系属分类
    zh_sg: 语言系属分类
    zh_tw: 語言系屬分類
---
# [[Language_Family]] 

#is_/same_as :: [[../WikiData/WD~Language_family,25295|WD~Language_family,25295]] 

## #has_/text_of_/abstract 

> A **Language family** is a group of languages related through descent from a common ancestor, 
> called the proto-language of that family. 
> 
> The term family is a metaphor borrowed from biology, with the tree model used in historical linguistics analogous to a family tree, or to phylogenetic trees of taxa used in evolutionary taxonomy. Linguists thus describe the daughter languages within a language family as being genetically related. The divergence of a proto-language into daughter languages typically occurs through geographical separation, with different regional dialects of the proto-language undergoing different language changes and thus becoming distinct languages over time. 
>
> One well-known example of a language family is the Romance languages, including Spanish, French, Italian, Portuguese, Romanian, Catalan, and many others, all of which are descended from Vulgar Latin. The Romance family itself is part of the larger Indo-European family, which includes many other languages native to Europe and South Asia, all believed to have descended from a common ancestor known as Proto-Indo-European.
>
> A language family is usually said to contain at least two languages, although language isolates — languages that are not related to any other language — are occasionally referred to as families that contain one language. Inversely, there is no upper bound to the number of languages a family can contain. Some families, such as the Austronesian languages, contain over 1000.
>
> Language families can be identified from shared characteristics amongst languages. Sound changes are one of the strongest pieces of evidence that can be used to identify a genetic relationship because of their predictable and consistent nature, and through the comparative method can be used to reconstruct proto-languages. However, languages can also change through language contact which can falsely suggest genetic relationships. For example, the Mongolic, Tungusic, and Turkic languages share a great deal of similarities that lead several scholars to believe they were related. These supposed relationships were later discovered to be derived through language contact and thus they are not truly related. Eventually though, high amounts of language contact and inconsistent changes will render it essentially impossible to derive any more relationships; even the oldest demonstrable language family, Afroasiatic, is far younger than language itself.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Language%20family) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Language/Language_Family|Language_Family]] 

### #is_/same_as :: [[/_public/Language/Language_Family.public|Language_Family.public]] 

### #is_/same_as :: [[/_internal/Language/Language_Family.internal|Language_Family.internal]] 

### #is_/same_as :: [[/_protect/Language/Language_Family.protect|Language_Family.protect]] 

### #is_/same_as :: [[/_private/Language/Language_Family.private|Language_Family.private]] 

### #is_/same_as :: [[/_personal/Language/Language_Family.personal|Language_Family.personal]] 

### #is_/same_as :: [[/_secret/Language/Language_Family.secret|Language_Family.secret]] 

