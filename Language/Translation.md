---
aliases:
- aistriú
- ante toki
- anuwaad
- asuɣl
- Auersaatang
- awdarma
- cyfieithu
- dịch thuật
- Eadar-theangachadh
- Erterduccion
- Fa'aliliuga
- Fandikan-teny
- fassara
- Fo'ali
- fordítás
- Hoan-e̍k
- Hoiz
- Interpretatio
- itzulpengintza
- Iwwersetzung
- jorgaleapmi
- jurgâlem
- jåårǥlâttmõš
- Kushandura
- Kutafsiri
- kääntäminen
- Leregere
- lɛbigibu
- oersetting
- omsetjing
- oversettelse
- oversættelse
- Paghubad
- Pagsasalinwika
- panagipatarus
- Pargoldīms
- Pertalan
- preklad
- prevajanje
- prevođenje
- Prijevod
- përkthim
- překlad
- přełožowanje
- Ratournaedje
- Rimay t'ikray
- Tarjamahan
- Tarjima
- tarɩjɩma
- tercime
- Terjemahan
- Terjime
- Tradiksyon
- traduccion
- traducció
- traducción
- traducere
- tradución
- traduction
- traduko
- tradukshon
- tradusion
- Tradussion
- tradutzione
- Traduzion
- traduzione
- traduzzioni
- traduzzjoni
- tradução
- Translation
- Treiñ ha troidigezh
- Treylyans
- tulkošana
- tõlkimine
- tłumaczenie
- tərcümə
- Ukuhumusha
- vertaalkunde
- vertaling
- vertimas
- Vuhundzuluxeri
- werger
- Çeviri
- çhyndaays
- ñe'ẽ'asa
- Översetten
- översättning
- Übersetzung
- þýðing
- μετάφραση
- Ётафтомась
- Аударма
- Гоч
- кылысь кылэ берыктон
- Орчуулга
- Оршуулга
- пераклад
- перевод
- переклад
- преведување
- превођење
- превод
- тæлмац
- Тăлмач
- Тарҷума
- Тылбаас
- тәржемә
- тәрҗемә
- Ютавтома
- թարգմանութիւն
- թարգմանություն
- איבערזעצונג
- תרגום
- الترجمه
- ترجمة
- ترجمه
- ترجمهن
- ترجمو
- ترجمہ
- طرجامة
- وەرگێڕان
- ژباړه
- ߘߟߊߡߌߘߊߟߌ
- अनुबाद
- अनुवाद
- অনুবাদ
- ਤਰਜਮਾ
- ଅନୁବାଦ
- மொழிபெயர்ப்பு
- అనువాదం
- ಭಾಷಾಂತರ
- വിവർത്തനം
- භාෂා පරිවර්තනය
- การแปล
- ပွမကၠာဲဘာသာ
- თარგმანი
- მათანგალი
- ᱛᱚᱨᱡᱳᱢᱟ
- Ṱhalutshedzelo
- ⴰⵙⵓⵖⵍ
- 翻訳
- 翻譯
- 翻译
- 譯
- 번역
has_id_wikidata: Q7553
Baidu_Tieba_name: 翻译
Commons_category: Translation
described_by_source: '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
different_from:
- '[[_Standards/WikiData/WD~translated_edition,21112633|WD~translated_edition,21112633]]'
- '[[_Standards/WikiData/WD~translated_work,125309905|WD~translated_work,125309905]]'
icon: http://commons.wikimedia.org/wiki/Special:FilePath/Icon%20Translate%20to%20Gujarati.png
image: http://commons.wikimedia.org/wiki/Special:FilePath/Rosetta%20Stone.JPG
instance_of:
- '[[_Standards/WikiData/WD~craft,2207288|WD~craft,2207288]]'
- '[[_Standards/WikiData/WD~academic_major,4671286|WD~academic_major,4671286]]'
- '[[_Standards/WikiData/WD~service,7406919|WD~service,7406919]]'
- '[[_Standards/WikiData/WD~occupation,12737077|WD~occupation,12737077]]'
Krugosvet_article: gumanitarnye_nauki/lingvistika/PEREVOD.html
Libris_URI: gdsvtxz01ctbbm0
MeSH_tree_code: L01.462.500.682.920
OmegaWiki_Defined_Meaning: 7173
practiced_by:
- '[[_Standards/WikiData/WD~translator,333634|WD~translator,333634]]'
- '[[_Standards/WikiData/WD~exegete,21185790|WD~exegete,21185790]]'
product_material_or_service_produced_or_provided: '[[_Standards/WikiData/WD~translated_edition,21112633|WD~translated_edition,21112633]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/De-%C3%9Cbersetzung2.ogg
said_to_be_the_same_as: '[[_Standards/WikiData/WD~translating_activity,115160303|WD~translating_activity,115160303]]'
studied_in: '[[_Standards/WikiData/WD~translation_studies,501778|WD~translation_studies,501778]]'
subclass_of:
- '[[_Standards/WikiData/WD~human_communication,1066689|WD~human_communication,1066689]]'
- '[[_Standards/WikiData/WD~transformation,65757353|WD~transformation,65757353]]'
topic_s_main_template: '[[_Standards/WikiData/WD~Template_Translation_info,106979019|WD~Template_Translation_info,106979019]]'
U_S_National_Archives_Identifier: 10632699
Wikidata_property: translation
dv_has_:
  name_:
    af: vertaalkunde
    an: Traducción
    ar: ترجمة
    ary: طرجامة
    arz: الترجمه
    as: অনুবাদ
    ast: Traducción
    az: tərcümə
    ba: тәржемә
    be: пераклад
    be_tarask: пераклад
    bg: превод
    bho: अनुबाद
    bn: অনুবাদ
    br: Treiñ ha troidigezh
    bs: prevođenje
    btm: Tarjamahan
    bxr: Оршуулга
    ca: traducció
    ce: Гоч
    ckb: وەرگێڕان
    co: traduzione
    crh: tercime
    cs: překlad
    cv: Тăлмач
    cy: cyfieithu
    da: oversættelse
    dag: lɛbigibu
    de: Übersetzung
    de_ch: Übersetzung
    el: μετάφραση
    en: translation
    en_ca: translation
    en_gb: translation
    eo: traduko
    es: traducción
    et: tõlkimine
    eu: itzulpengintza
    fa: ترجمه
    fi: kääntäminen
    fr: traduction
    frr: Auersaatang
    fur: Traduzion
    fy: oersetting
    ga: aistriú
    gd: Eadar-theangachadh
    gl: tradución
    gn: ñe'ẽ'asa
    gsw: Übersetzung
    gur: Leregere
    gv: çhyndaays
    ha: fassara
    he: תרגום
    hi: अनुवाद
    hif: anuwaad
    hr: prevođenje
    hsb: přełožowanje
    ht: Tradiksyon
    hu: fordítás
    hy: թարգմանություն
    hyw: թարգմանութիւն
    id: Terjemahan
    ilo: panagipatarus
    is: þýðing
    it: traduzione
    ja: 翻訳
    ka: თარგმანი
    kaa: awdarma
    kk: Аударма
    kn: ಭಾಷಾಂತರ
    ko: 번역
    ku: werger
    ku_latn: werger
    kw: Treylyans
    la: Interpretatio
    lb: Iwwersetzung
    lmo: Traduzion
    lt: vertimas
    lv: tulkošana
    lzh: 譯
    mad: Pertalan
    mdf: Ётафтомась
    mg: Fandikan-teny
    mk: преведување
    ml: വിവർത്തനം
    mn: Орчуулга
    mnw: ပွမကၠာဲဘာသာ
    mr: अनुवाद
    ms: terjemahan
    ms_arab: ترجمهن
    mt: traduzzjoni
    myv: Ютавтома
    nan: Hoan-e̍k
    nb: oversettelse
    nds: Översetten
    ne: अनुवाद
    new: अनुवाद
    nia: Fo'ali
    nl: vertaling
    nn: omsetjing
    nqo: ߘߟߊߡߌߘߊߟߌ
    oc: traduccion
    or: ଅନୁବାଦ
    os: тæлмац
    pa: ਤਰਜਮਾ
    pap: tradukshon
    pcd: Erterduccion
    pl: tłumaczenie
    pms: Tradussion
    pnb: ترجمہ
    ps: ژباړه
    pt: tradução
    pt_br: tradução
    qu: Rimay t'ikray
    ro: traducere
    ru: перевод
    rue: переклад
    sah: Тылбаас
    sat: ᱛᱚᱨᱡᱳᱢᱟ
    sc: tradutzione
    scn: traduzzioni
    sco: translation
    sd: ترجمو
    se: jorgaleapmi
    sgs: Pargoldīms
    sh: Prijevod
    shi: asuɣl
    si: භාෂා පරිවර්තනය
    sk: preklad
    sl: prevajanje
    sm: Fa'aliliuga
    smn: jurgâlem
    sms: jåårǥlâttmõš
    sn: Kushandura
    sq: përkthim
    sr: превођење
    su: Terjemahan
    sv: översättning
    sw: Kutafsiri
    ta: மொழிபெயர்ப்பு
    te: అనువాదం
    tg: Тарҷума
    th: การแปล
    tk: Terjime
    tl: Pagsasalinwika
    tok: ante toki
    tr: Çeviri
    ts: Vuhundzuluxeri
    tt: тәрҗемә
    tt_cyrl: тәрҗемә
    udm: кылысь кылэ берыктон
    uk: переклад
    ur: ترجمہ
    uz: Tarjima
    ve: Ṱhalutshedzelo
    vec: tradusion
    vi: dịch thuật
    wa: Ratournaedje
    war: Paghubad
    wlx: tarɩjɩma
    wuu: 翻譯
    xmf: მათანგალი
    yi: איבערזעצונג
    yue: 翻譯
    za: Hoiz
    zgh: ⴰⵙⵓⵖⵍ
    zh: 翻譯
    zh_cn: 翻译
    zh_hans: 翻译
    zh_hant: 翻譯
    zh_hk: 翻譯
    zh_tw: 翻譯
    zu: Ukuhumusha
---

# [[Translation]] 

#is_/same_as :: [[../WikiData/WD~Translation,7553|WD~Translation,7553]] 

## #has_/text_of_/abstract 

> **Translation** is the communication of the meaning of a source-language text 
> by means of an equivalent target-language text. 
> 
> The English language draws a terminological distinction 
> (which does not exist in every language) between translating (a written text) 
> and interpreting (oral or signed communication between users of different languages); 
> under this distinction, translation can begin only after the appearance of writing within a language community.
>
> A translator always risks inadvertently introducing source-language words, grammar, or syntax into the target-language rendering. On the other hand, such "spill-overs" have sometimes imported useful source-language calques and loanwords that have enriched target languages. Translators, including early translators of sacred texts, have helped shape the very languages into which they have translated.
>
> Because of the laboriousness of the translation process, since the 1940s efforts have been made, with varying degrees of success, to automate translation or to mechanically aid the human translator. More recently, the rise of the Internet has fostered a world-wide market for translation services and has facilitated "language localisation".
>
> [Wikipedia](https://en.wikipedia.org/wiki/Translation) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Language/Translation|Translation]] 

### #is_/same_as :: [[/_public/Language/Translation.public|Translation.public]] 

### #is_/same_as :: [[/_internal/Language/Translation.internal|Translation.internal]] 

### #is_/same_as :: [[/_protect/Language/Translation.protect|Translation.protect]] 

### #is_/same_as :: [[/_private/Language/Translation.private|Translation.private]] 

### #is_/same_as :: [[/_personal/Language/Translation.personal|Translation.personal]] 

### #is_/same_as :: [[/_secret/Language/Translation.secret|Translation.secret]] 

