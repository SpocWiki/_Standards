---
aliases:
- ainmeolaíocht
- anvadurezh
- danh xưng học
- nafnfræði
- Namenforschung
- nammekunde
- nimistöntutkimus
- nomscienco
- Noomenkunde
- névtan
- onomasteg
- onomastica
- Onomastics
- onomastică
- onomastiek
- onomastiga
- Onomastik
- onomastika
- onomastikk
- Onomastiko
- onomastique
- onomastyka
- onomastïka
- Onomatologie
- onomatology
- onomàstica
- onomástica
- Vutivi bya ti-Onomastics
- ονοματολογία
- анамастыка
- номастика
- ономастикæ
- ономастика
- ономастике
- անվանաբանություն
- אונומסטיקה
- أصل الكلمة
- دانش کلمات
- ونوماستىيكا
- नामिकी
- অনোমাস্টিক্স
- பெயராய்வு
- ონომასტიკა
- 专名学
- 固有名詞学
- 專名學
- 고유명사학
has_id_wikidata: Q175156
is_the_study_of:
- '[[_Standards/WikiData/WD~name,82799]]'
- '[[_Standards/WikiData/WD~naming,25488743]]'
has_part_s_: '[[_Standards/WikiData/WD~toponymy,485762]]'
instance_of: '[[_Standards/WikiData/WD~branch_of_science,2465832]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Personal_names,11131773]]'
practiced_by: '[[_Standards/WikiData/WD~onomastician,15976327]]'
subclass_of: '[[_Standards/WikiData/WD~linguistics,8162]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Onoma.jpg
Krugosvet_article_archived_: gumanitarnye_nauki/lingvistika/ONOMASTIKA.html
Commons_category: Onomastics
dv_has_:
  name_:
    ar: أصل الكلمة
    az: onomastika
    be: анамастыка
    be_tarask: анамастыка
    bg: ономастика
    bn: অনোমাস্টিক্স
    br: anvadurezh
    ca: onomàstica
    cs: onomastika
    cy: onomasteg
    da: onomastik
    de: Namenforschung
    el: ονοματολογία
    en: onomastics
    en_ca: onomastics
    en_gb: onomastics
    eo: nomscienco
    es: onomástica
    et: onomastika
    eu: onomastika
    fa: دانش کلمات
    fi: nimistöntutkimus
    fr: onomastique
    fy: nammekunde
    ga: ainmeolaíocht
    gl: onomástica
    ha: onomastics
    he: אונומסטיקה
    hi: नामिकी
    hr: onomastika
    ht: onomastik
    hu: névtan
    hy: անվանաբանություն
    id: onomastika
    ig: onomastics
    io: Onomastiko
    is: nafnfræði
    it: onomastica
    ja: 固有名詞学
    ka: ონომასტიკა
    kk: ономастика
    kk_arab: ونوماستىيكا
    kk-cn: ونوماستىيكا
    kk_cyrl: ономастика
    kk-kz: ономастика
    kk_latn: onomastïka
    kk-tr: onomastïka
    ko: 고유명사학
    ky: ономастика
    la: onomastica
    lt: onomastika
    lv: onomastika
    mhr: ономастике
    mk: ономастика
    ms: onomastik
    nb: onomastikk
    nl: onomastiek
    nn: onomastikk
    oc: onomastica
    os: ономастикæ
    pl: onomastyka
    pt: onomástica
    pt_br: onomástica
    ro: onomastică
    ru: ономастика
    sah: ономастика
    sh: onomastika
    sk: onomastika
    sl: onomastika
    sq: onomastika
    sr: ономастика
    sr_ec: ономастика
    sr_el: onomastika
    stq: Noomenkunde
    sv: onomastik
    ta: பெயராய்வு
    tg_cyrl: номастика
    tk: onomastika
    tr: onomastik
    ts: Vutivi bya ti-Onomastics
    tt: ономастика
    uk: ономастика
    uz: onomastika
    vi: danh xưng học
    vro: onomastiga
    wuu: 专名学
    zh: 專名學
    zh_cn: 专名学
    zh_hans: 专名学
    zh_hant: 專名學
    zh_hk: 專名學
    zh_sg: 专名学
    zh_tw: 專名學
---
# [[Onomastics]] 

#is_/same_as :: [[_Standards/WikiData/WD~onomastics,175156]] 

## #has_/text_of_/abstract 

> **Onomastics** (or onomatology in older texts) 
> is the study of proper names, including their etymology, history, and use.
>
> An alethonym ('true name') or an orthonym ('real name') is the proper name of the object in question, 
> the object of onomastic study. Scholars studying onomastics are called onomasticians.
>
> Onomastics has applications in data mining, 
> with applications such as named-entity recognition, or recognition of the origin of names. 
> 
> It is a popular approach in historical research, 
> where it can be used to identify ethnic minorities within populations and for the purpose of prosopography.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Onomastics) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Language/Onomastics|Onomastics]] 

### #is_/same_as :: [[/_public/Language/Onomastics.public|Onomastics.public]] 

### #is_/same_as :: [[/_internal/Language/Onomastics.internal|Onomastics.internal]] 

### #is_/same_as :: [[/_protect/Language/Onomastics.protect|Onomastics.protect]] 

### #is_/same_as :: [[/_private/Language/Onomastics.private|Onomastics.private]] 

### #is_/same_as :: [[/_personal/Language/Onomastics.personal|Onomastics.personal]] 

### #is_/same_as :: [[/_secret/Language/Onomastics.secret|Onomastics.secret]] 

