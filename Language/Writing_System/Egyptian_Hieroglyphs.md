---
has_time_stopped: 0400-01-01
aliases:
- Chữ tượng hình Ai Cập
- Dealbh-sgrìobhadh
- Egipatski hijeroglifi
- Egipskie hieroglify
- egiptaj hieroglifoj
- Egiptiana hieroglifi
- Egiptiese hiërogliewe
- Egiptiečių hieroglifai
- Egiptoar idazkera hieroglifiko
- egiptuse hieroglüüfkiri
- egipčanski hieroglifi
- Egyptian hieroglyphs
- Egyptian_Hieroglyphs
- Egyptische hiëroglief
- egyptisk hieroglyf
- Egyptisk hieroglyphen
- egyptiska hieroglyfer
- Egyptské hieroglyfické písmo
- Egyptské hieroglyfy
- Egïpet jazwı
- geroglifico egizio
- Heroglipiko Ehiptonon
- Heroglipikong Ehipsiyo
- Herogríficu
- Hieroglif Mesir
- hieroglife egiptene
- Hieroglifoù Egipt
- hieroglyf
- hieroglyffau'r Aifft
- hieroglyfit
- hieroglyph
- Hieroglyphica Aegyptia
- hieroglyphs
- hieroglyyf
- hieróglifos egípcios
- Hiroglifi
- Hiyeroglifo Mısırki
- Hiyeroglîfên misrî
- hiéroglyphes égyptiens
- Hotunan Masar
- Híeróglýfur
- Iairiglifí Éigipteacha
- Ieroglifs egipcians
- Izifenqo zaseGibhithe
- Jalloo-ocklyn Egyptagh
- Jeroglífic egipci
- jeroglífico egipcio
- Jerôglifika Ejiptana
- Masar hieroglyphs
- Misir heroqlif yazısı
- Misr iyerogliflari
- Mısır hiyeroglifleri
- sitelen Keme
- Sèng-su-thé
- Séng-cṳ̆-tā̤
- xeroglíficu exipciu
- Xeróglifo exipcio
- Ägyptische Hieroglyphen
- Ägyptischi Hieroglyphe
- ēģiptiešu hieroglifi
- Ġeroglifiċi Eġizzjani
- Αιγυπτιακά ιερογλυφικά
- Єгипетські ієрогліфи
- Егіпецкія іерогліфы
- египатски хијероглифи
- Египет жазуы
- Египетски йероглиф
- египетски хиероглифи
- египетское иероглифическое письмо
- Иероглифтер теориясы
- эгіпецкія герогліфы
- եգիպտական հիերոգլիֆներ
- Հիերոկլիֆ
- כתב חרטומים
- حیروغلیفی
- حیەرۆگلیفێن مسری
- مِصری ہائروگلیفٕس
- هيروجليفى
- هيروغليفية مصرية
- هیروگلیف مصری
- هېروګليف
- ھیرۆگلیفی میسری
- ەگىيپەت جازۋى
- इजिप्टेली चित्रलिपि
- मिस्र के हाइरोग्लिफ
- मिस्री चित्रलिपि
- মিশরীয় চিত্রলিপি
- ਮਿਸਰੀ ਚਿੱਤਰ ਅੱਖਰ
- எகிப்திய மறைபொருட் குறியீடுகள்
- ഈജിപ്ഷ്യൻ ഹൈറോഗ്ലിഫ്
- ඊජිප්තු රූපාක්ෂර
- ไฮเออโรกลีฟอียิปต์
- ეგვიპტური იეროგლიფები
- ეგვიპტური იეროგლიფეფი
- ヒエログリフ
- 圣书体
- 聖書體
- 신성문자
has_id_wikidata: Q132659
subclass_of: '[[_Standards/WikiData/WD~hieroglyph,193762|WD~hieroglyph,193762]]'
described_by_source:
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
instance_of:
- '[[_Standards/WikiData/WD~logographic_writing_system,3953107|WD~logographic_writing_system,3953107]]'
- '[[_Standards/WikiData/WD~unicase_alphabet,4004706|WD~unicase_alphabet,4004706]]'
- '[[_Standards/WikiData/WD~natural_writing_system,29517555|WD~natural_writing_system,29517555]]'
- '[[_Standards/WikiData/WD~writing_system,8192|WD~writing_system,8192]]'
script_directionality:
- '[[_Standards/WikiData/WD~right_to_left,7333457|WD~right_to_left,7333457]]'
- '[[_Standards/WikiData/WD~left_to_right,17321844|WD~left_to_right,17321844]]'
- '[[_Standards/WikiData/WD~top_to_bottom,17321917|WD~top_to_bottom,17321917]]'
- '[[_Standards/WikiData/WD~bottom_to_top,17322076|WD~bottom_to_top,17322076]]'
end_time: 0400-01-01T00:00:00Z
inception: -3199-01-01T00:00:00Z
locations: '[[_Standards/WikiData/WD~Ancient_Egypt,11768|WD~Ancient_Egypt,11768]]'
language_of_work_or_name: '[[_Standards/WikiData/WD~Egyptian,50868|WD~Egyptian,50868]]'
ISO_15924_numeric_code: 050
name:
- hiéroglyphes égyptiens
- Egyptian Hieroglyphs
- Egyptian hieroglyphs
image:
- http://commons.wikimedia.org/wiki/Special:FilePath/Extract%20from%20the%20Tale%20of%20the%20Two%20Brothers.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Hieroglyphs%20from%20the%20tomb%20of%20Seti%20I.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Minnakht%2001.JPG
- http://commons.wikimedia.org/wiki/Special:FilePath/Papyrus%20Ani%20curs%20hiero.jpg
Unicode_range:
- 233hh hbsj2l2
- U+13430-1343F
Iconclass_notation: 49L21
ISO_15924_alpha_4_code: Egyp
Commons_category: Egyptian hieroglyphs
dv_has_:
  name_:
    af: Egiptiese hiërogliewe
    ar: هيروغليفية مصرية
    arz: هيروجليفى
    ast: xeroglíficu exipciu
    az: Misir heroqlif yazısı
    be: Егіпецкія іерогліфы
    be_tarask: эгіпецкія герогліфы
    bg: Египетски йероглиф
    bho: मिस्र के हाइरोग्लिफ
    bjn: Hieroglif Mesir
    bn: মিশরীয় চিত্রলিপি
    br: Hieroglifoù Egipt
    ca: Jeroglífic egipci
    cdo: Séng-cṳ̆-tā̤
    ckb: ھیرۆگلیفی میسری
    cs: Egyptské hieroglyfy
    cy: hieroglyffau'r Aifft
    da: egyptisk hieroglyf
    de: Ägyptische Hieroglyphen
    diq: Hiyeroglifo Mısırki
    el: Αιγυπτιακά ιερογλυφικά
    en: Egyptian hieroglyphs
    en_ca: Egyptian hieroglyphs
    en_gb: Egyptian hieroglyphs
    eo: egiptaj hieroglifoj
    es: jeroglífico egipcio
    et: egiptuse hieroglüüfkiri
    eu: Egiptoar idazkera hieroglifiko
    ext: Herogríficu
    fa: هیروگلیف مصری
    fi: hieroglyfit
    fr: hiéroglyphes égyptiens
    frr: Egyptisk hieroglyphen
    ga: Iairiglifí Éigipteacha
    gd: Dealbh-sgrìobhadh
    gl: Xeróglifo exipcio
    gsw: Ägyptischi Hieroglyphe
    gv: Jalloo-ocklyn Egyptagh
    ha: Hotunan Masar
    he: כתב חרטומים
    hi: मिस्री चित्रलिपि
    hr: Egipatski hijeroglifi
    hy: եգիպտական հիերոգլիֆներ
    hyw: Հիերոկլիֆ
    id: hieroglif Mesir
    io: Egiptiana hieroglifi
    is: Híeróglýfur
    it: geroglifico egizio
    ja: ヒエログリフ
    ka: ეგვიპტური იეროგლიფები
    kk: Египет жазуы
    kk_arab: ەگىيپەت جازۋى
    kk-cn: ەگىيپەت جازۋى
    kk_cyrl: Египет жазуы
    kk-kz: Египет жазуы
    kk_latn: Egïpet jazwı
    kk-tr: Egïpet jazwı
    ko: 신성문자
    ks: مِصری ہائروگلیفٕس
    ku: Hiyeroglîfên misrî
    ku_arab: حیەرۆگلیفێن مسری
    ku_latn: Hiyeroglîfên misrî
    ky: Иероглифтер теориясы
    la: Hieroglyphica Aegyptia
    lt: Egiptiečių hieroglifai
    lv: ēģiptiešu hieroglifi
    mg: Jerôglifika Ejiptana
    mk: египетски хиероглифи
    ml: ഈജിപ്ഷ്യൻ ഹൈറോഗ്ലിഫ്
    ms: Hieroglif Mesir
    mt: Ġeroglifiċi Eġizzjani
    nan: Sèng-su-thé
    nb: hieroglyf
    ne: इजिप्टेली चित्रलिपि
    nl: Egyptische hiëroglief
    nn: Hieroglyf
    oc: Ieroglifs egipcians
    pa: ਮਿਸਰੀ ਚਿੱਤਰ ਅੱਖਰ
    pl: Egipskie hieroglify
    ps: هېروګليف
    pt: hieróglifos egípcios
    ro: hieroglife egiptene
    ru: египетское иероглифическое письмо
    sco: Egyptian hieroglyphs
    sh: Egipatski hijeroglifi
    si: ඊජිප්තු රූපාක්ෂර
    sk: Egyptské hieroglyfické písmo
    sl: egipčanski hieroglifi
    smn: hieroglyf
    sms: hieroglyyf
    so: Masar hieroglyphs
    sr: египатски хијероглифи
    sr_ec: Египатски хијероглифи
    sr_el: Egipatski hijeroglifi
    sv: egyptiska hieroglyfer
    sw: Hiroglifi
    ta: எகிப்திய மறைபொருட் குறியீடுகள்
    th: ไฮเออโรกลีฟอียิปต์
    tl: Heroglipikong Ehipsiyo
    tok: sitelen Keme
    tr: Mısır hiyeroglifleri
    uk: Єгипетські ієрогліфи
    ur: حیروغلیفی
    uz: Misr iyerogliflari
    vi: Chữ tượng hình Ai Cập
    war: Heroglipiko Ehiptonon
    wuu: 圣书体
    xmf: ეგვიპტური იეროგლიფეფი
    yue: 聖書體
    zh: 聖書體
    zh_cn: 圣书体
    zh_hans: 圣书体
    zh_hant: 聖書體
    zh_hk: 聖書體
    zh_sg: 圣书体
    zh_tw: 聖書體
    zu: Izifenqo zaseGibhithe
---

# [[Egyptian_Hieroglyphs]] 

#is_/same_as :: [[../../WikiData/WD~Egyptian_hieroglyphs,132659|WD~Egyptian_hieroglyphs,132659]] 

## #has_/text_of_/abstract 

> Ancient Egyptian hieroglyphs ( HY-roh-glifs) 
> were the formal writing system used in Ancient Egypt for writing the Egyptian language. 
> Hieroglyphs combined ideographic, logographic, syllabic and alphabetic elements, 
> with more than 1,000 distinct characters. 
> 
> Cursive hieroglyphs were used for religious literature on papyrus and wood. 
> The later hieratic and demotic Egyptian scripts were derived from hieroglyphic writing, 
> as was the Proto-Sinaitic script that later evolved into the Phoenician alphabet. 
> 
> Egyptian hieroglyphs are the ultimate ancestor of the Phoenician alphabet, 
> the first widely adopted phonetic writing system. 
> Moreover, owing in large part to the Greek and Aramaic scripts that descended from Phoenician, 
> the majority of the world's living writing systems are descendants of Egyptian hieroglyphs—
> most prominently the Latin and Cyrillic scripts through Greek, 
> and the Arabic and Brahmic scripts through Aramaic.
>
> The use of hieroglyphic writing arose from proto-literate symbol systems in the Early Bronze Age 
> c. the 33rd century BC (Naqada III), 
> with the first decipherable sentence written in the Egyptian language dating to the 28th century BC (Second Dynasty). 
> 
> Ancient Egyptian hieroglyphs developed into a mature writing system 
> used for monumental inscription in the classical language of the Middle Kingdom period; 
> during this period, the system used about 900 distinct signs. 
> The use of this writing system continued through the New Kingdom and Late Period, 
> and on into the Persian and Ptolemaic periods. 
> 
> Late survivals of hieroglyphic use are found well into the Roman period, extending into the 4th century AD.
>
> During the 5th century, the permanent closing of pagan temples across Roman Egypt 
> ultimately resulted in the loss of fluent readers and writers in hieroglyphs. 
> 
> Despite attempts at decipherment, 
> the nature of the script remained unknown throughout the Middle Ages and the early modern period. 
> The decipherment of hieroglyphic writing was finally accomplished in the 1820s 
> by Jean-François Champollion, with the help of the Rosetta Stone.
>
> The entire Ancient Egyptian corpus, including both hieroglyphic and hieratic texts, 
> is approximately 5 million words in length; 
> if counting duplicates (such as the Book of the Dead and the Coffin Texts) as separate, 
> this figure is closer to 10 million. 
> 
> The most complete compendium of Ancient Egyptian, 
> the Wörterbuch der ägyptischen Sprache, contains 1.5–1.7 million words.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Egyptian%20hieroglyphs) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Language/Writing_System/Egyptian_Hieroglyphs|Egyptian_Hieroglyphs]] 

### #is_/same_as :: [[/_public/Language/Writing_System/Egyptian_Hieroglyphs.public|Egyptian_Hieroglyphs.public]] 

### #is_/same_as :: [[/_internal/Language/Writing_System/Egyptian_Hieroglyphs.internal|Egyptian_Hieroglyphs.internal]] 

### #is_/same_as :: [[/_protect/Language/Writing_System/Egyptian_Hieroglyphs.protect|Egyptian_Hieroglyphs.protect]] 

### #is_/same_as :: [[/_private/Language/Writing_System/Egyptian_Hieroglyphs.private|Egyptian_Hieroglyphs.private]] 

### #is_/same_as :: [[/_personal/Language/Writing_System/Egyptian_Hieroglyphs.personal|Egyptian_Hieroglyphs.personal]] 

### #is_/same_as :: [[/_secret/Language/Writing_System/Egyptian_Hieroglyphs.secret|Egyptian_Hieroglyphs.secret]] 

