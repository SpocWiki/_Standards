---
aliases:
- Abbyrlhit Romanagh
- abecedarium Latinum
- Abecee latin
- afabɛ́ ya latɛ́
- Agemmay alatini
- aibítir Laidineach
- aksara Latin
- alefbet latino
- alfabbetu latinu
- Alfabet Latin
- alfabet latén
- alfabet llatí
- Alfabeta latina
- alfabeti latin
- alfabeti ya Kilatini
- alfabeto latin
- alfabeto latino
- alfabeto latín
- Alfabetu latinu
- alfabetu llatín
- alfabetul latin
- Alfabey Latinki
- alfabeya latînî
- alfabèt latin
- alfabé laten
- Alfabét Latin
- alpabetong Latin
- Alphabet latin
- alphabeto latin
- Baƙaƙen boko
- bảng chữ cái Latinh
- det latinske alfabetet
- eskritúrang Latínu
- Farta laatiinka
- Iskriturang Latin
- Ladina kiri
- lafab latinik
- Laitin script alphabet
- Latainse alfabet
- lateinisches Schriftsystem
- lateinischs Oiphabet
- Latiens sjrif
- latiensch Alphabet
- Latiensk Alphabet
- latiinnalaš alfabehta
- latiinsk alfabeet
- Latijns alfabet
- Latin alfabesi
- Latin alpabet
- latin betűs írás
- Latin elifbesi
- Latin Script
- Latin siq'i llumpa
- latina alfabeto
- latinalaiset aakkoset
- Latinan kirjamišt
- Latinañe'ẽ achegety
- latindar alfabetoa
- latinica
- latinikano lekhipen
- latinka
- Latinmachiyōtlahtōltecpantiliztli
- latinska alfabetet
- latinske alfabet
- latinské písmo
- latmo lerfu
- latneskt stafróf
- latynischs Alphabet
- Latynse skrif
- Latynske alfabet
- latynə əlifbo
- laténgesche Schrëftsystem
- latínska stavraðið
- latīņu raksts
- Latın jazıwı
- latın əlifbası
- Linatin nga abakadahan
- litreadh Laidinneach
- Lizherenneg latin
- Lotin alifbosi
- lotynų abėcėlė
- Luotīnėšks rašts
- lyuut Lati̱n
- Lá-dĭng cê-mō̤
- látin yazîsî
- Lâ-tên sṳ-mû
- läättinlâš puustaveh
- Lô-má-jī
- Lǣden stæfrōf
- pismo łacińskie
- Roman akchhar
- Roman Script
- Romi Lipiecho Fuddar
- rumazi
- Script~Latn
- Scritura latina
- sinulatang Latinhon
- sinuratan a Latin
- sitelen Lasina
- Skrypt Latin
- Surat Latin
- Tarehin-tsoratra latina
- tulisan Rumi
- Urup Latin
- yr wyddor Ladin
- écriture latine
- Łacyńsczi alfabét
- Łaćińske abecadło
- łaćonski alfabet
- λατινική γραφή
- Алаҭын алфавит
- Алифбои лотинӣ
- Лата үзэг
- Латин алфавит
- Латин алфавиты
- Латин бичлт
- Латин тхыпкъылъэ
- Латин хъвай-хъвагӀи
- латин язуы
- Латин үсэг
- латинаг алфавит
- Латинан абат
- Латиний алапат
- латиница
- латиницась
- латинично писмо
- латинка
- латинське письмо
- Латиньска аꙁъбоукꙑ
- Латын алфавити
- Латын әліпбиі
- Латыын алпабыыта
- лацінскае пісьмо
- լատինական այբուբեն
- אלפבית לטיני
- לאטיינישער אלפאבעט
- ئەلفوبێی لاتین
- الفبا لاتین
- الفبای لاتین
- توليسن رومي
- حروف لاتينيه
- رومن اسڪرپٽ
- كتابة لاتينية
- لاتىن ئەلىڧبەسى
- لاتين ليکدود
- لاتین الیفباسی
- لاتین خط
- لاطینی حروفِ تہجی
- لاطینی لپی
- لٲطِن لِکھٲرؠ نِظام
- ܐܠܦܒܝܬ ܠܐܛܝܢܝܐ
- ߟߊ߬ߕߍ߲߬ ߛߓߍߛߎ߲
- रोमन लिपि
- लॅटिन वर्णमाला
- লাতিন লিপি
- লেটিন লিপি
- ਲਾਤੀਨੀ ਲਿੱਪੀ
- ରୋମାନ ଲିପି
- இலத்தீன் எழுத்துகள்
- ಲ್ಯಾಟಿನ್ ಲಿಪಿ
- ലത്തീൻ അക്ഷരമാല
- อักษรละติน
- ອັກສອນລາຕິນ
- လက်တင်ဘာသာအရေးအသား
- ლათინური დამწერლობა
- ლათინური ჭარალუა
- ላቲን ጽሕፈት
- እንግሊዝኛ ፊደል
- ᐃᓕᓴᐅᑎᒃ
- អក្សរឡាតាំង
- ᱞᱟᱛᱤᱱ ᱦᱚᱨᱚᱯ
- ⴰⴳⵎⵎⴰⵢ ⴰⵍⴰⵜⵉⵏⵉ
- ラテン文字
- 拉丁字母
- 拉丁文字
- ꠞꠥꠝ ꠘꠣꠉꠞꠤ ꠟꠤꠙꠤ
- ꯂꯦꯇꯤꯟ ꯃꯌꯦꯛ
- 로마자
- "\U000110AA\U000110B6\U00011097\U000110B1\U000110A2 \U000110AA\U000110B1\U0001108E\U000110B0\U00011086"
has_id_wikidata: Q8229
subclass_of: '[[_Standards/WikiData/WD~alphabetic_writing_system,2182919|WD~alphabetic_writing_system,2182919]]'
history_of_topic: '[[_Standards/WikiData/WD~history_of_the_Latin_alphabet,3772237|WD~history_of_the_Latin_alphabet,3772237]]'
replaces: '[[_Standards/WikiData/WD~Old_Italic,4891256|WD~Old_Italic,4891256]]'
based_on: '[[_Standards/WikiData/WD~Old_Italic,4891256|WD~Old_Italic,4891256]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Latin_script,15063213|WD~Template_Latin_script,15063213]]'
different_from:
- '[[_Standards/WikiData/WD~English_script,16929818|WD~English_script,16929818]]'
- '[[_Standards/WikiData/WD~Latin_script_alphabet,29575627|WD~Latin_script_alphabet,29575627]]'
- '[[_Standards/WikiData/WD~Latin_alphabet,41670|WD~Latin_alphabet,41670]]'
script_directionality: '[[_Standards/WikiData/WD~left_to_right,17321844|WD~left_to_right,17321844]]'
has_part_s_: '[[_Standards/WikiData/WD~Latin_script_letter,19776628|WD~Latin_script_letter,19776628]]'
described_by_source: '[[_Standards/WikiData/WD~Wikipesija,108929142|WD~Wikipesija,108929142]]'
inception: -0700-01-01T00:00:00Z
instance_of: '[[_Standards/WikiData/WD~writing_system,8192|WD~writing_system,8192]]'
locations: '[[_Standards/WikiData/WD~Italy,38|WD~Italy,38]]'
ISO_15924_numeric_code: 215
name:
- latin
- Latin
ISO_15924_alpha_4_code: Latn
Commons_gallery: Latin letters
Unicode_range:
- U+0100-017F
- U+0180-024F
- U+1E00-1EFF
- U+2C60-2C7F
- U+0041-005A,U+0061-006A
- U+00AA,U+00BA,U+00C0-00D6,U+00D7-00F6,U+00F8-00FF
- U+0250-02AF
- U+02B0-02E4
- U+10780-10785,U+10787-107B0,U+107B2-107BA
- U+1D00-1D25,U+1D2C-1D5C,U+1D62-1D65,U+1D6B-1D77,U+1D79-1DBE
- U+1D400-1D6A5
- U+2071,U+207F,U+2090-209C
- U+2102,U+2107,U+210A-2113,U+2115,U+2117-2125,U+2128,U+212A-2134,U+2139-213B,U+2141-214C,U+214E
- U+2160-2188
- U+A722-A787,U+A78B-A7CA,U+A7D0-A7D1,U+A7D3,U+A7D5-A7D9,U+A7F2-A7FF
- U+AB30-AB5A,U+AB5C-AB64,U+AB66-AB69
- U+FB00-FB06
- U+FF21-FF3A,U+FF41-FF5A
3D_model: http://commons.wikimedia.org/wiki/Special:FilePath/LATEINISCHES%20ALPHABET%20NOTO%20SANS.stl
Commons_category: Latin alphabet
image: http://commons.wikimedia.org/wiki/Special:FilePath/Abecedarium.svg
distribution_map: http://commons.wikimedia.org/wiki/Special:FilePath/Latin%20alphabet%20world%20distribution.svg
spoken_text_audio: http://commons.wikimedia.org/wiki/Special:FilePath/Nl-Latijnsalfabet.ogg
described_at_URL: https://wikipesija.org/wiki/sitelen_Lasina
dv_has_:
  name_:
    ab: Алаҭын алфавит
    af: Latynse skrif
    am: ላቲን ጽሕፈት
    an: alfabeto latín
    ang: Lǣden stæfrōf
    ar: كتابة لاتينية
    arc: ܐܠܦܒܝܬ ܠܐܛܝܢܝܐ
    ary: كتابة لاتينية
    arz: حروف لاتينيه
    as: লেটিন লিপি
    ast: alfabetu llatín
    av: Латин хъвай-хъвагӀи
    az: latın əlifbası
    azb: لاتین الیفباسی
    ba: Латин алфавиты
    ban: Aksara Latin
    bar: lateinischs Oiphabet
    bcl: Iskriturang Latin
    be: лацінскае пісьмо
    be_tarask: лацінскае пісьмо
    bg: латиница
    bho: "\U000110AA\U000110B6\U00011097\U000110B1\U000110A2 \U000110AA\U000110B1\U0001108E\U000110B0\U00011086"
    bn: লাতিন লিপি
    br: Lizherenneg latin
    bs: latinica
    btm: Surat Latin
    bxr: Лата үзэг
    ca: alfabet llatí
    cdo: Lá-dĭng cê-mō̤
    ce: Латинан абат
    ceb: sinulatang Latinhon
    ckb: ئەلفوبێی لاتین
    co: Alfabetu latinu
    crh: Latin elifbesi
    crh_latn: Latin elifbesi
    crh-ro: látin yazîsî
    cs: latinka
    csb: Łacyńsczi alfabét
    cu: Латиньска аꙁъбоукꙑ
    cy: yr wyddor Ladin
    da: latinske alfabet
    de: lateinisches Schriftsystem
    diq: Alfabey Latinki
    el: λατινική γραφή
    en: Latin script
    en_ca: Latin script
    en_gb: Latin script
    en-us: Latin script
    eo: latina alfabeto
    es: alfabeto latino
    et: ladina kiri
    eu: latindar alfabetoa
    ext: alfabetu latinu
    fa: الفبای لاتین
    fi: latinalaiset aakkoset
    fo: latínska stavraðið
    fr: écriture latine
    frc: alphabet latin
    frr: latiinsk alfabeet
    fur: Alfabet latin
    fy: Latynske alfabet
    ga: aibítir Laidineach
    gan: 拉丁字母
    gcr: alfabé laten
    gd: litreadh Laidinneach
    gl: alfabeto latino
    gn: Latinañe'ẽ achegety
    gom: Romi Lipiecho Fuddar
    gsw: latynischs Alphabet
    gv: Abbyrlhit Romanagh
    ha: Baƙaƙen boko
    hak: Lâ-tên sṳ-mû
    he: אלפבית לטיני
    hi: रोमन लिपि
    hif: Roman akchhar
    hr: latinica
    hsb: łaćonski alfabet
    hu: latin betűs írás
    hy: լատինական այբուբեն
    hyw: լատինական այբուբեն
    ia: alphabeto latin
    iba: Urup Latin
    id: aksara Latin
    ilo: sinuratan a Latin
    inh: Латиний алапат
    io: latina alfabeto
    is: latneskt stafróf
    it: alfabeto latino
    iu: ᐃᓕᓴᐅᑎᒃ
    ja: ラテン文字
    jbo: latmo lerfu
    jv: aksara Latin
    ka: ლათინური დამწერლობა
    kaa: Latın jazıwı
    kab: Agemmay alatini
    kbd: Латин тхыпкъылъэ
    kcg: lyuut Lati̱n
    kge: Alfabet Latin
    kk: Латын әліпбиі
    km: អក្សរឡាតាំង
    kn: ಲ್ಯಾಟಿನ್ ಲಿಪಿ
    ko: 로마자
    krc: Латин алфавит
    ks: لٲطِن لِکھٲرؠ نِظام
    ku: alfabeya latînî
    ku_latn: alfabeya latînî
    kw: Skrypt Latin
    ky: Латын алфавити
    la: abecedarium Latinum
    lad: alefbet latino
    lb: laténgesche Schrëftsystem
    lfn: Alfabeta latina
    li: Latiens sjrif
    lld: Scritura latina
    lmo: Abecee latin
    ln: afabɛ́ ya latɛ́
    lo: ອັກສອນລາຕິນ
    lrc: الفبا لاتین
    lt: lotynų abėcėlė
    lv: latīņu raksts
    lzh: 拉丁字母
    map_bms: Alfabet Latin
    mdf: латиницась
    mg: Tarehin-tsoratra latina
    mk: латиница
    ml: ലത്തീൻ അക്ഷരമാല
    mn: Латин үсэг
    mni: ꯂꯦꯇꯤꯟ ꯃꯌꯦꯛ
    mr: लॅटिन वर्णमाला
    ms: tulisan Rumi
    ms_arab: توليسن رومي
    my: လက်တင်ဘာသာအရေးအသား
    mzn: لاتین خط
    nah: Latinmachiyōtlahtōltecpantiliztli
    nan: Lô-má-jī
    nb: det latinske alfabetet
    nds: latiensch Alphabet
    nds_nl: Latainse alfabet
    ne: रोमन लिपि
    nl: Latijns alfabet
    nn: det latinske alfabetet
    nqo: ߟߊ߬ߕߍ߲߬ ߛߓߍߛߎ߲
    nrm: Alphabet latin
    oc: alfabet latin
    or: ରୋମାନ ଲିପି
    os: латинаг алфавит
    pa: ਲਾਤੀਨੀ ਲਿੱਪੀ
    pam: eskritúrang Latínu
    pap: alfabèt latin
    pcd: Alfabet latin
    pl: pismo łacińskie
    pms: Alfabet latin
    pnb: لاطینی لپی
    ps: لاتين ليکدود
    pt: alfabeto latino
    pt_br: alfabeto latino
    pwn: rumazi
    qu: Latin siq'i llumpa
    rmy: latinikano lekhipen
    ro: alfabetul latin
    ru: латиница
    rue: латинка
    sah: Латыын алпабыыта
    sat: ᱞᱟᱛᱤᱱ ᱦᱚᱨᱚᱯ
    sc: Alfabetu latinu
    scn: alfabbetu latinu
    sco: Laitin script alphabet
    sd: رومن اسڪرپٽ
    se: latiinnalaš alfabehta
    sgs: Luotīnėšks rašts
    sh: latinica
    sk: latinské písmo
    sl: latinica
    smn: läättinlâš puustaveh
    so: Farta laatiinka
    sq: alfabeti latin
    sr: латиница
    sr_ec: латинично писмо
    stq: Latiensk Alphabet
    su: Alfabét Latin
    sv: latinska alfabetet
    sw: alfabeti ya Kilatini
    syl: ꠞꠥꠝ ꠘꠣꠉꠞꠤ ꠟꠤꠙꠤ
    szl: Łaćińske abecadło
    ta: இலத்தீன் எழுத்துகள்
    tg: алифбои лотинӣ
    tg_cyrl: Алифбои лотинӣ
    th: อักษรละติน
    ti: እንግሊዝኛ ፊደል
    tl: alpabetong Latin
    tly: latynə əlifbo
    tok: sitelen Lasina
    tpi: Latin alpabet
    tr: Latin alfabesi
    tt: латин язуы
    tt_cyrl: латин язуы
    udm: латиница
    ug: لاتىن ئەلىڧبەسى
    uk: латинське письмо
    ur: لاطینی حروفِ تہجی
    uz: Lotin alifbosi
    vec: alfabeto latin
    vep: Latinan kirjamišt
    vi: bảng chữ cái Latinh
    vo: lafab latinik
    vro: Ladina kiri
    wa: alfabet latén
    war: Linatin nga abakadahan
    wuu: 拉丁字母
    xal: Латин бичлт
    xmf: ლათინური ჭარალუა
    yi: לאטיינישער אלפאבעט
    yue: 拉丁文字
    zgh: ⴰⴳⵎⵎⴰⵢ ⴰⵍⴰⵜⵉⵏⵉ
    zh: 拉丁字母
    zh_cn: 拉丁字母
    zh_hans: 拉丁文字
    zh_hant: 拉丁文字
    zh_hk: 拉丁文字
    zh_mo: 拉丁文字
    zh-my: 拉丁文字
    zh_sg: 拉丁文字
    zh_tw: 拉丁文字
---

# [[Script~Latn]] 

#is_/same_as :: [[_Standards/WikiData/WD~Latin_script,8229|WD~Latin_script,8229]] 

## #has_/text_of_/abstract 

> The Latin script, also known as the Roman script, 
> is a writing system based on the letters of the classical Latin alphabet, 
> derived from a form of the Greek alphabet 
> which was in use in the ancient Greek city of Cumae in Magna Graecia. 
> 
> The Greek alphabet was altered by the Etruscans, 
> and subsequently their alphabet was altered by the Ancient Romans. 
> 
> Several Latin-script alphabets exist, which differ in graphemes, collation 
> and phonetic values from the classical Latin alphabet.
>
> The Latin script is the basis of the International Phonetic Alphabet (IPA), 
> and the 26 most widespread letters are the letters of the ISO basic Latin alphabet, 
> which are the same letters as the English alphabet.
>
> Latin script is the basis for the largest number of alphabets of any writing system 
> and is the most widely adopted writing system in the world. 
> 
> Latin script is used as the standard method of writing the languages of 
> - Western and Central Europe, 
> - most of sub-Saharan Africa, 
> - the Americas, and Oceania, 
> as well as many languages in other parts of the world.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Latin%20script) 


## Romanization 

Used to indicate Latin Script in Web-Pages 
Many Languages have been Romanized and can therefor be written in 
either their native Script or a Latin Transcription: 

1. **Serbian:**
    
    - Latin Script: sr-Latn
    - Cyrillic Script: sr-Cyrl
2. **Bosnian:**
    
    - Latin Script: bs-Latn
    - Cyrillic Script: bs-Cyrl
3. **Montenegrin:**
    
    - Latin Script: sr-Latn (Montenegrin is closely related to Serbian, and Latin script is commonly used.)
    - Cyrillic Script: sr-Cyrl (Less common but still used in certain contexts.)
4. **Macedonian:**
    
    - Latin Script: mk-Latn
    - Cyrillic Script: mk-Cyrl
5. **Kazakh:**
    
    - Latin Script: kk-Latn
    - Cyrillic Script: kk-Cyrl
6. **Azerbaijani:**
    
    - Latin Script: az-Latn
    - Cyrillic Script: az-Cyrl (Historically used, but Latin script is now more common.)
7. **Ukrainian:**
    
    - Latin Script: uk-Latn (Less common and mainly for specific purposes)
    - Cyrillic Script: uk-Cyrl
8. **Belarusian:**
    
    - Latin Script: be-Latn (Less common and mainly for specific purposes)
    - Cyrillic Script: be-Cyrl
9. **Tatar:**
    
    - Latin Script: tt-Latn
    - Cyrillic Script: tt-Cyrl
    - 
10. **Arabic:**
    
    - Arabic Script: ar-Arab
    - Latin Script (Transcription): ar-Latn
11. **Chinese (Mainland China):**
    
    - Simplified Chinese Script: zh-Hans
    - Traditional Chinese Script: zh-Hant
12. **Hindi:**
    
    - Devanagari Script: hi-Deva
    - Latin Script (Transcription): hi-Latn
13. **Persian (Farsi):**
    
    - Persian Script: fa-Arab
    - Latin Script (Transcription): fa-Latn
14. **Urdu:**
    
    - Urdu Script: ur-Arab
    - Latin Script (Transcription): ur-Latn
15. **Japanese:**
    
    - Kanji Script: ja-Jpan
    - Hiragana Script: ja-Hira
    - Katakana Script: ja-Kana
    - Latin Script (Transcription): ja-Latn
16. **Korean:**
    
    - Hangul Script: ko-Hang
    - Hanja Script: ko-Hani (Hanja is less common and is used mainly in specific contexts.)
17. **Georgian:**
    
    - Georgian Script: ka-Geor
    - Latin Script (Transcription): ka-Latn
18. **Tibetan:**
    
    - Tibetan Script: bo-Tibt
    - Latin Script (Transcription): bo-Latn


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Language/Writing_System/Script~Latn|Script~Latn]] 

### #is_/same_as :: [[/_public/Language/Writing_System/Script~Latn.public|Script~Latn.public]] 

### #is_/same_as :: [[/_internal/Language/Writing_System/Script~Latn.internal|Script~Latn.internal]] 

### #is_/same_as :: [[/_protect/Language/Writing_System/Script~Latn.protect|Script~Latn.protect]] 

### #is_/same_as :: [[/_private/Language/Writing_System/Script~Latn.private|Script~Latn.private]] 

### #is_/same_as :: [[/_personal/Language/Writing_System/Script~Latn.personal|Script~Latn.personal]] 

### #is_/same_as :: [[/_secret/Language/Writing_System/Script~Latn.secret|Script~Latn.secret]] 

