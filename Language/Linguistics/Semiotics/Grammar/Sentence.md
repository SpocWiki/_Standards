---
aliases:
- abairt
- ahịrịokwu
- ayat
- brawddeg
- bûn-kù
- câu
- cümle
- cümlə
- dumanna
- faláze
- fehezanteny
- fjali
- frasa
- frase
- frasi
- fraze
- frazenn
- frazo
- fråze
- gap
- gbólóhùn
- gáp
- hawraar
- hevok
- hunt'a rimay
- jimla
- kalimah
- kalimat
- lause
- lengkara
- mening
- mondat
- málsgrein
- okara
- oración
- pahayag
- pamulong
- pangungusap
- pangurolay
- perpaus
- período
- phrase
- poved
- Prepozisyon
- propoziție
- rečenica
- rosg-rann
- sakinys
- Satz
- sentence
- sentensi
- sententia
- setning
- sin
- sætning
- tafyirt
- teikums
- ukara
- umusho
- veta
- věta
- yɛltɔɣili
- yɛlɛ
- zdanie
- zin
- ğömlä
- πρόταση
- изречение
- ойлончо
- предложение
- реченица
- речення
- сĕнӳ
- сёрникузя
- сказ
- сүйлөм
- сөйлем
- җөмлә
- ҷумла
- һөйләм
- өгүүлбэр
- նախադասություն
- זאץ
- משפט
- آیُت
- جملة
- جمله
- جملو
- جملہ
- جومله
- ڕستە
- खँपु
- वाक्य
- वाक्य और वाक्य के भेद
- वाक्यम्
- वाक्यरचना
- বাক্য
- ਵਾਕ
- વાક્ય
- ବାକ୍ୟ
- వాక్యం
- വാചകം
- ประโยค
- ປະໂຫຍກ
- တူၼ်ႈထႅဝ်ႈ
- ဝါကျ
- წინადადება
- ዓረፍተ-ነገር
- ᡤᡳᠰᡠᠨ
- ᱟ.ᱭᱟ.ᱛ
- 句
- 句子
- 文
- 문장
has_id_wikidata: Q41796
different_from:
- '[[_Standards/WikiData/WD~phrase,187931|WD~phrase,187931]]'
- '[[_Standards/WikiData/WD~sentence,348486|WD~sentence,348486]]'
- '[[_Standards/WikiData/WD~sentence,1763090|WD~sentence,1763090]]'
subclass_of:
- '[[_Standards/WikiData/WD~utterance,258403|WD~utterance,258403]]'
- '[[_Standards/WikiData/WD~Q12132146,12132146|WD~Q12132146,12132146]]'
- '[[_Standards/WikiData/WD~semantic_unit,15916540|WD~semantic_unit,15916540]]'
- '[[_Standards/WikiData/WD~form,115786086|WD~form,115786086]]'
described_by_source:
- '[[../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138|WD~Granat_Encyclopedic_Dictionary,4532138]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
has_part_s_of_the_class: '[[_Standards/WikiData/WD~constituent,1786828|WD~constituent,1786828]]'
opposite_of: '[[_Standards/WikiData/WD~Q9380011,9380011|WD~Q9380011,9380011]]'
Krugosvet_article_archived_: gumanitarnye_nauki/lingvistika/PREDLOZHENIE.html
Gujarati_Vishwakosh_entry: વાક્ય
dv_has_:
  name_:
    af: sin
    am: ዓረፍተ-ነገር
    ar: جملة
    ast: oración
    az: cümlə
    azb: جومله
    ba: һөйләм
    ban: lengkara
    bcl: pangurolay
    be: сказ
    be_tarask: сказ
    bg: изречение
    bho: वाक्य
    bn: বাক্য
    br: frazenn
    bs: rečenica
    ca: frase
    ckb: ڕستە
    cs: věta
    cv: сĕнӳ
    cy: brawddeg
    da: sætning
    dag: yɛltɔɣili
    de: Satz
    de_ch: Satz
    dga: yɛlɛ
    el: πρόταση
    en: sentence
    en_ca: sentence
    en_gb: sentence
    eo: frazo
    es: oración
    et: lause
    eu: perpaus
    ext: frasi
    fa: جمله
    fi: lause
    fr: phrase
    ga: abairt
    gd: rosg-rann
    gl: oración
    gu: વાક્ય
    ha: jimla
    he: משפט
    hi: वाक्य और वाक्य के भेद
    hr: rečenica
    ht: Prepozisyon
    hu: mondat
    hy: նախադասություն
    ia: phrase
    id: kalimat
    ig: ahịrịokwu
    io: frazo
    is: málsgrein
    it: frase
    ja: 文
    jv: ukara
    ka: წინადადება
    kaa: gáp
    kab: tafyirt
    kk: сөйлем
    ko: 문장
    koi: сёрникузя
    krj: pahayag
    ks: آیُت
    ku_latn: hevok
    ky: сүйлөм
    la: sententia
    ln: faláze
    lo: ປະໂຫຍກ
    lt: sakinys
    lv: teikums
    mad: okara
    mg: fehezanteny
    mhr: ойлончо
    mk: реченица
    ml: വാചകം
    mn: өгүүлбэр
    mnc: ᡤᡳᠰᡠᠨ
    mr: वाक्यरचना
    ms: ayat
    my: ဝါကျ
    nan: bûn-kù
    nb: setning
    new: खँपु
    nl: zin
    nn: setning
    oc: frasa
    or: ବାକ୍ୟ
    pa: ਵਾਕ
    pl: zdanie
    pnb: جملہ
    pt: período
    pt_br: período
    qu: hunt'a rimay
    ro: propoziție
    ru: предложение
    sa: वाक्यम्
    sat: ᱟ.ᱭᱟ.ᱛ
    scn: dumanna
    sco: sentence
    sd: جملو
    sh: rečenica
    shn: တူၼ်ႈထႅဝ်ႈ
    sk: veta
    sl: poved
    so: hawraar
    sq: fjali
    sr: реченица
    sr_ec: реченица
    su: kalimah
    sv: mening
    sw: sentensi
    te: వాక్యం
    tg: ҷумла
    tg_cyrl: ҷумла
    th: ประโยค
    tl: pangungusap
    tr: cümle
    tt: җөмлә
    tt_cyrl: җөмлә
    tt_latn: ğömlä
    uk: речення
    ur: جملہ
    uz: gap
    vec: fraze
    vi: câu
    wa: fråze
    war: pamulong
    wuu: 句子
    yi: זאץ
    yo: gbólóhùn
    yue: 句
    zh: 句子
    zh_hans: 句子
    zh_hant: 句子
    zh_tw: 句子
    zu: umusho
---

# [[Sentence]] 

#is_/same_as :: [[_Standards/WikiData/WD~Sentence,41796|WD~Sentence,41796]] 

## #has_/text_of_/abstract 

> In linguistics and grammar, a sentence is a linguistic expression, 
> such as the English example "The quick brown fox jumps over the lazy dog." 
> 
> In traditional grammar, 
> it is typically defined as a string of words that expresses a complete thought, 
> or as a unit consisting of a subject and predicate. 
> 
> In non-functional linguistics 
> it is typically defined as a maximal unit of syntactic structure such as a constituent. 
> 
> In functional linguistics, 
> it is defined as a unit of written texts delimited by graphological features 
> such as upper-case letters and markers such as periods, question marks, and exclamation marks. 
> 
> This notion contrasts with a curve, which is delimited by phonologic features such as 
> pitch and loudness and markers such as pauses; 
> and with a clause, 
> which is a sequence of words that represents some process going on throughout time.
>
> A sentence can include words grouped meaningfully 
> to express a statement, question, exclamation, request, command, or suggestion.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Sentence%20(linguistics)) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Language/Linguistics/Semiotics/Grammar/Sentence|Sentence]] 

### #is_/same_as :: [[/_public/Language/Linguistics/Semiotics/Grammar/Sentence.public|Sentence.public]] 

### #is_/same_as :: [[/_internal/Language/Linguistics/Semiotics/Grammar/Sentence.internal|Sentence.internal]] 

### #is_/same_as :: [[/_protect/Language/Linguistics/Semiotics/Grammar/Sentence.protect|Sentence.protect]] 

### #is_/same_as :: [[/_private/Language/Linguistics/Semiotics/Grammar/Sentence.private|Sentence.private]] 

### #is_/same_as :: [[/_personal/Language/Linguistics/Semiotics/Grammar/Sentence.personal|Sentence.personal]] 

### #is_/same_as :: [[/_secret/Language/Linguistics/Semiotics/Grammar/Sentence.secret|Sentence.secret]] 

