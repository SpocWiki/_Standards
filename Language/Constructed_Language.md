---
aliases:
- bahasa buatan
- Basa bikinan
- basa damelan
- Basa Guwayan
- constructed language
- constructit leid
- dirbtinė kalba
- edorer it eman
- fiteny foronina
- gjuha artificiale
- hizkuntza eraikia
- Iaith artiffisial
- idioma artifisial
- jîn-chō gí-giân
- język sztuczny
- keinotekoinen kieli
- keunsttaal
- konstruerat språk
- konstruiate Sproch
- konstruierte Sprache
- konstruita lingvo
- konstrukteti lingue
- konstruktita linguo
- kunsmatige taal
- kunstaol
- kunstsprog
- kunstspråk
- kunsttaal
- lang konstwi
- langue construite
- laungue construite
- lenga artificiala
- lenghe artifissiaj
- lengua construida
- limbă artificială
- lingua artificial
- lingua artificiale
- lingua artificiali
- lingua artificiosa
- lingua construida
- lingua contruída
- llengua construïda
- llingua construyida
- lo runbau
- luenga construita
- lugha ya kuundwa
- léngoa artifiçiâle
- língua artificial
- mesterséges nyelv emberek közötti kommunikációra
- mutauro wekuumba
- mākslīgā valoda
- ngôn ngữ được xây dựng
- planlingue
- planski jezici
- pük mekavik
- stilkene Sproake
- sunʼiy tillar
- sztuczno godka
- süni dil
- tahokielâ
- teanga chumtha
- tehiskeel
- tilbúið tungumál
- Tinugdas na tataramon
- toki sin
- umelý jazyk
- umetni jezik
- umjetni jezici
- umělý jazyk
- umětny jezyk
- wallpasqa simi
- wikang artipisiyal
- yapay dil
- yezh kalvezadel
- zimanê çêkirî
- łéngua artifisałe
- τεχνητή γλώσσα
- вештачки јазик
- жасалма тилдер
- жасанды тілдер
- забони фаросохта
- зохиомол хэл
- изграђен језик
- изкуствен език
- искусственный человеческий язык
- лэсьтэм кыл
- лӧсьӧдӧм кыв
- оҥоhуу тыл
- штучна мова
- штучная мова
- шутласа кăларнă чĕлхе
- ясалма тел
- արհեստական լեզու
- שפה מתוכננת
- بهاس بيناءن
- جوړه شوې ژبه
- زبان فراساخته
- زوون بساتهبیی
- ساختہ زبان
- سۈنئىي تىل
- لغة مصطنعة
- مصنوعی زبان
- ߞߊ߲߫ ߠߊߘߊ߲ߣߍ߲
- कृत्रिम भाषा
- নির্মিত ভাষা
- வடிவமைக்கப்பட்ட மொழிகள்
- ಕೃತಕ ಭಾಷೆಗಳು
- ภาษาประดิษฐ์
- ხელოვნური ენა
- 人工言語
- 人工語言
- 人工语言
- 人造語言
- 인공어
has_id_wikidata: Q33215
studied_by: '[[_Standards/WikiData/WD~interlinguistics,856574]]'
instance_of:
- '[[_Standards/WikiData/WD~language_group,941501]]'
- '[[_Standards/WikiData/WD~type_of_language,20829075]]'
different_from:
- '[[_Standards/WikiData/WD~language_inventor,1206912]]'
- '[[_Standards/WikiData/WD~artificial_language,3247505]]'
- '[[_Standards/WikiData/WD~online_conlanging_community,8349870]]'
- '[[_Standards/WikiData/WD~conlanging,19642588]]'
subclass_of: '[[_Standards/WikiData/WD~artificial_language,3247505]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Constructed_languages,6274104]]'
topic_s_main_Wikimedia_portal: '[[_Standards/WikiData/WD~Portal_Constructed_languages,14614960]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._2,124737604]]'
ABS_ASCL_2011_code:
- 96
- 9601
ISO_639_5_code: art
IETF_language_tag: art
ISO_639_2_code: art
OmegaWiki_Defined_Meaning: 344165
image: http://commons.wikimedia.org/wiki/Special:FilePath/Conlangflag.svg
exact_match: http://publications.europa.eu/resource/authority/language/ART
Stack_Exchange_site_URL: https://conlang.stackexchange.com
Stack_Exchange_tag:
- https://esperanto.stackexchange.com/tags/constructed-languages
- https://linguistics.stackexchange.com/tags/conlang
Dewey_Decimal_Classification: 499.99
Universal_Decimal_Classification: 811.92
subreddit: conlangs
Commons_category: Constructed languages
dv_has_:
  name_:
    af: kunsmatige taal
    an: luenga construita
    ar: لغة مصطنعة
    ast: llingua construyida
    az: süni dil
    bar: konstruiate Sproch
    bcl: Tinugdas na tataramon
    be: штучная мова
    be_tarask: штучная мова
    bew: Basa bikinan
    bg: изкуствен език
    bn: নির্মিত ভাষা
    br: yezh kalvezadel
    ca: llengua construïda
    co: lingua artificiale
    cs: umělý jazyk
    cv: шутласа кăларнă чĕлхе
    cy: Iaith artiffisial
    da: kunstsprog
    de: konstruierte Sprache
    el: τεχνητή γλώσσα
    en: constructed language
    en_gb: constructed language
    eo: konstruita lingvo
    es: lengua construida
    et: tehiskeel
    eu: hizkuntza eraikia
    fa: زبان فراساخته
    fi: keinotekoinen kieli
    fr: langue construite
    fy: keunsttaal
    ga: teanga chumtha
    gl: lingua contruída
    he: שפה מתוכננת
    hi: कृत्रिम भाषा
    hr: umjetni jezici
    ht: lang konstwi
    hu: mesterséges nyelv emberek közötti kommunikációra
    hy: արհեստական լեզու
    ia: lingua artificial
    id: bahasa buatan
    ie: planlingue
    ig: constructed language
    io: konstruktita linguo
    is: tilbúið tungumál
    isv_latn: umětny jezyk
    it: lingua artificiale
    ja: 人工言語
    jbo: lo runbau
    jv: basa damelan
    ka: ხელოვნური ენა
    kge: Basa Guwayan
    kk: жасанды тілдер
    kn: ಕೃತಕ ಭಾಷೆಗಳು
    ko: 인공어
    ku: zimanê çêkirî
    ku_latn: zimanê çêkirî
    kv: лӧсьӧдӧм кыв
    ky: жасалма тилдер
    la: lingua artificiosa
    lfn: lingua construida
    li: kunstaol
    lij: léngoa artifiçiâle
    lt: dirbtinė kalba
    lv: mākslīgā valoda
    lzh: 人工語言
    mg: fiteny foronina
    mk: вештачки јазик
    mn: зохиомол хэл
    mr: कृत्रिम भाषा
    ms: bahasa buatan
    ms_arab: بهاس بيناءن
    mzn: زوون بساتهبیی
    na: edorer it eman
    nan: jîn-chō gí-giân
    nb: kunstspråk
    nl: kunsttaal
    nn: kunstspråk
    nov: konstrukteti lingue
    nqo: ߞߊ߲߫ ߠߊߘߊ߲ߣߍ߲
    nrm: laungue construite
    oc: lenga artificiala
    pap: idioma artifisial
    pl: język sztuczny
    pms: lenghe artifissiaj
    pnb: مصنوعی زبان
    ps: جوړه شوې ژبه
    pt: língua artificial
    qu: wallpasqa simi
    ro: limbă artificială
    ru: искусственный человеческий язык
    sah: оҥоhуу тыл
    scn: lingua artificiali
    sco: constructit leid
    sh: planski jezici
    sk: umelý jazyk
    sl: umetni jezik
    smn: tahokielâ
    sn: mutauro wekuumba
    sq: gjuha artificiale
    sr: изграђен језик
    sr_ec: изграђен језик
    stq: stilkene Sproake
    sv: konstruerat språk
    sw: lugha ya kuundwa
    szl: sztuczno godka
    ta: வடிவமைக்கப்பட்ட மொழிகள்
    tg: забони фаросохта
    th: ภาษาประดิษฐ์
    tl: wikang artipisiyal
    tok: toki sin
    tr: yapay dil
    tt: ясалма тел
    tt_cyrl: ясалма тел
    udm: лэсьтэм кыл
    ug: سۈنئىي تىل
    uk: штучна мова
    ur: ساختہ زبان
    uz: sunʼiy tillar
    vec: łéngua artifisałe
    vi: ngôn ngữ được xây dựng
    vo: pük mekavik
    wuu: 人工语言
    yue: 人造語言
    zh: 人工語言
    zh_hans: 人工语言
    zh_hant: 人工語言
    zh_hk: 人工語言
    zh_tw: 人工語言
---

# [[Constructed_Language]] 

#is_/same_as :: [[../WikiData/WD~Constructed_language,33215|WD~Constructed_language,33215]] 

## #has_/text_of_/abstract 

> A constructed language is a language for communication between humans 
> (i.e. not with or between computers) but unlike a language that emerges from human interaction, is intentionally devised for a particular purpose. Constructed language is often shortened to conlang and is a relatively broad term that encompasses subcategories including: fictional, artificial, engineered, planned and invented. A constructed language may include natural language aspects including phonology, grammar, orthography, and vocabulary. Interlinguistics includes the study of constructed languages.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Constructed%20language) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Language/Constructed_Language|Constructed_Language]] 

### #is_/same_as :: [[/_public/Language/Constructed_Language.public|Constructed_Language.public]] 

### #is_/same_as :: [[/_internal/Language/Constructed_Language.internal|Constructed_Language.internal]] 

### #is_/same_as :: [[/_protect/Language/Constructed_Language.protect|Constructed_Language.protect]] 

### #is_/same_as :: [[/_private/Language/Constructed_Language.private|Constructed_Language.private]] 

### #is_/same_as :: [[/_personal/Language/Constructed_Language.personal|Constructed_Language.personal]] 

### #is_/same_as :: [[/_secret/Language/Constructed_Language.secret|Constructed_Language.secret]] 

