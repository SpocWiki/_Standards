---
aliases:
- abubuwan sunadarai
- bahan kimia
- cheemsch Stoff
- Chemical Substance
- chemical-substance
- ChemicalSubstance
- chemical_substance
- chemická látka
- cheminė medžiaga
- chemische sjtof
- chemische stof
- chemische stoffe
- chemischer Stoff
- chemiska maćizna
- gai kimiko
- gemyske stof
- hemijska supstanca
- hoà-ha̍k bu̍t-chit
- hóa chất
- Isiqa
- is_a_chemical_substance
- keemiline aine
- kemiala kompozo
- kemiallinen aine
- kemijska tvar
- kemikaal
- kemikali
- kemikalie
- kemikála
- kemisk stof
- kemična substanca
- kimiska lītne
- kimyasal madde
- kimyəvi maddə
- kjemikalie
- kjemisk stoff
- mushonga
- ohun àpòpọ̀ògùn
- ollus
- sibstans
- sostansa
- sostanza pura
- stoo kemmigagh
- substaint cheimiceach
- substance chimique
- substancia química
- substancja chemiczna
- substancë kimike
- substans gymyk
- substantia chemicalis
- substantia chimic
- substanță chimică
- substància quimica
- substància química
- substância química
- supstanca
- sustancia química
- sustansiyang kimikal
- sustanza chìmica
- sylwedd cemegol
- tanga takrurant
- vegyi anyag
- veika kemikali
- zat kimia
- ĥemiaĵo
- ķīmiskā viela
- χημική ουσία
- зат
- матдә
- модда
- рэчыва
- супстанција
- сэк
- хімічна речовина
- хемијска супстанца
- химийн бодис
- химикаттар
- химион буаргъæд
- химическое вещество
- химично вещество
- эмен
- япалалăх
- նիւթ
- նյութ
- כימיקל
- כעמישע סובסטאנץ
- باهن کيميا
- رساێن مَواد
- مادة كيماوية
- مادة كيميائية
- ماددەی کیمیایی
- مواد شیمیایی
- کیمیائی شے
- रासायनिक पदार्थ
- রাসায়নিক পদার্থ
- ৰাসায়নিক দ্ৰব্য
- ਰਸਾਇਣਕ ਪਦਾਰਥ
- ରାସାୟନିକ ପଦାର୍ଥ
- வேதிப்பொருள்
- രാസപദാർത്ഥം
- สารเคมี
- ნივთიერება
- សារធាតុគីមី
- 化学物質
- 化学物质
- 化學物質
- ꯀꯦꯃꯤꯀꯦꯜ
- 화학 물질
has_id_wikidata: Q79529
confidential: public
cssclasses:
- Type
draft: false
expiryDate: ''
isDeleted: false
isReadOnly: false
keywords:
- chemical_substance
layout: ''
license: CC BY-SA 4.0
linkTitle: is_a_chemical_substance Class
publish: true
publishDate: ''
tags:
- class/Class
- schema-org/Class
title: is_a_chemical_substance Class
type: Type
has_characteristic:
- '[[_Standards/WikiData/WD~cohesion,216828]]'
- '[[_Standards/WikiData/WD~group_or_class_of_chemical_substances,17339814]]'
different_from:
- '[[_Standards/WikiData/WD~Stoff,341867]]'
- '[[_Standards/WikiData/WD~chemical_substance,55661792]]'
- '[[_Standards/WikiData/WD~Stof,123360405]]'
model_item:
- '[[_Standards/WikiData/WD~pure_substance,578779]]'
- '[[_Standards/WikiData/WD~chemical_product,1069267]]'
has_part_s_of_the_class: '[[_Standards/WikiData/WD~molecular_entity,2393187]]'
subclass_of:
- '[[_Standards/WikiData/WD~substance,10683158]]'
- '[[_Standards/WikiData/WD~chemical_entity,43460564]]'
union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486]]'
studied_by:
- '[[_Standards/WikiData/WD~physics,413]]'
- '[[_Standards/WikiData/WD~chemistry,2329]]'
Dewey_Decimal_Classification:
- 501
- 547
- 546
OmegaWiki_Defined_Meaning: 557
P8189: 987007285071005200
Commons_category: Chemical substances
equivalent_class:
- http://bioschemas.org/ChemicalSubstance
- http://purl.obolibrary.org/obo/CHEBI_59999
- https://schema.org/ChemicalSubstance
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q8752%20%28eus%29-ElsaBornFree-gai%20kimiko.wav
narrower_external_class: http://id.nlm.nih.gov/mesh/vocab#SCR_Chemical
Iconclass_notation: 22A1
UMLS_CUI: C0220806
IPTC_NewsCode: mediatopic/20000217
dv_is_:
  part_of: https://pending.schema.org
  parent_class: '[[../../schema-org/Class/is_a_/Bio_Chem_Entity|bio_chem_entity]]'
  same_as:
  - '[[../../WikiData/WD~Chemical_substance,79529|WD~Chemical_substance,79529]]'
  - '[[/_Standards/Chemistry/Substance/Chemical_Substance|Chemical_Substance]]'
  - '[[/_public/Chemistry/Substance/Chemical_Substance.public|Chemical_Substance.public]]'
  - '[[/_internal/Chemistry/Substance/Chemical_Substance.internal|Chemical_Substance.internal]]'
  - '[[/_protect/Chemistry/Substance/Chemical_Substance.protect|Chemical_Substance.protect]]'
  - '[[/_private/Chemistry/Substance/Chemical_Substance.private|Chemical_Substance.private]]'
  - '[[/_personal/Chemistry/Substance/Chemical_Substance.personal|Chemical_Substance.personal]]'
  - '[[/_secret/Chemistry/Substance/Chemical_Substance.secret|Chemical_Substance.secret]]'
dv_has_:
  name_:
    ar: مادة كيميائية
    ary: مادة كيماوية
    as: ৰাসায়নিক দ্ৰব্য
    ast: sustancia química
    az: kimyəvi maddə
    ba: матдә
    be: рэчыва
    be-tarask: рэчыва
    bg: химично вещество
    bho: रासायनिक पदार्थ
    bn: রাসায়নিক পদার্থ
    bs: hemijska supstanca
    ca: substància química
    ckb: ماددەی کیمیایی
    cs: chemická látka
    cv: япалалăх
    cy: sylwedd cemegol
    da: kemisk stof
    de: chemischer Stoff
    de-ch: chemischer Stoff
    el: χημική ουσία
    en: chemical substance
    en-ca: chemical substance
    en-gb: chemical substance
    eo: ĥemiaĵo
    es: sustancia química
    et: keemiline aine
    eu: gai kimiko
    fa: مواد شیمیایی
    fi: kemiallinen aine
    fj: veika kemikali
    fr: substance chimique
    fy: gemyske stof
    ga: substaint cheimiceach
    gl: substancia química
    gv: stoo kemmigagh
    ha: abubuwan sunadarai
    he: כימיקל
    hr: kemijska tvar
    hsb: chemiska maćizna
    ht: sibstans
    hu: vegyi anyag
    hy: նյութ
    hyw: նիւթ
    ia: substantia chimic
    id: zat kimia
    io: kemiala kompozo
    it: sostanza pura
    ja: 化学物質
    jv: zat kimia
    ka: ნივთიერება
    kab: tanga takrurant
    kk: химикаттар
    km: សារធាតុគីមី
    ko: 화학 물질
    ks: رساێن مَواد
    kw: substans gymyk
    ky: зат
    la: substantia chemicalis
    li: chemische sjtof
    lt: cheminė medžiaga
    ltg: kimiska lītne
    lv: ķīmiskā viela
    mhr: эмен
    min: zat kimia
    mk: супстанција
    ml: രാസപദാർത്ഥം
    mn: химийн бодис
    mni: ꯀꯦꯃꯤꯀꯦꯜ
    mr: रासायनिक पदार्थ
    ms: bahan kimia
    ms-arab: باهن کيميا
    nan: hoà-ha̍k bu̍t-chit
    nb: kjemikalie
    nds: cheemsch Stoff
    ne: रासायनिक पदार्थ
    nl: chemische stof
    nn: kjemisk stoff
    oc: substància quimica
    or: ରାସାୟନିକ ପଦାର୍ଥ
    os: химион буаргъæд
    pa: ਰਸਾਇਣਕ ਪਦਾਰਥ
    pl: substancja chemiczna
    pnb: کیمیائی شے
    pt: substância química
    pt-br: substância química
    ro: substanță chimică
    ru: химическое вещество
    sah: сэк
    scn: sustanza chìmica
    sco: chemical substance
    se: kemikála
    sh: supstanca
    sk: chemická látka
    sl: kemična substanca
    smn: kemikaal
    sms: kemikaal
    sn: mushonga
    sq: substancë kimike
    sr: хемијска супстанца
    sr-ec: хемијска супстанца
    su: bahan kimia
    sv: kemikalie
    sw: kemikali
    ta: வேதிப்பொருள்
    tg: модда
    th: สารเคมี
    tl: sustansiyang kimikal
    tr: kimyasal madde
    tt: матдә
    tt-cyrl: матдә
    uk: хімічна речовина
    ur: کیمیائی شے
    vec: sostansa
    vi: hóa chất
    vls: chemische stoffe
    vro: ollus
    wuu: 化学物质
    yi: כעמישע סובסטאנץ
    yo: ohun àpòpọ̀ògùn
    yue: 化學物質
    zh: 化学物质
    zh-cn: 化学物质
    zh-hans: 化学物质
    zh-hant: 化學物質
    zh-hk: 化學物質
    zh-mo: 化學物質
    zh-my: 化学物质
    zh-sg: 化学物质
    zh-tw: 化學物質
    zu: Isiqa
  properties: '[ additionalType, alternateName, associatedDisease, bioChemInteraction, bioChemSimilarity, biologicalRole, chemicalComposition, chemicalRole, description, disambiguatingDescription, funding, hasBioChemEntityPart, hasMolecularFunction, hasRepresentation, identifier, image, isEncodedByBioChemEntity, isInvolvedInBiologicalProcess, isLocatedInSubcellularLocation, isPartOfBioChemEntity, mainEntityOfPage, name, potentialAction, potentialUse, sameAs, subjectOf, taxonomicRange, url ]'
dv_is_part_of: https://pending.schema.org
dv_has_parent_class: '[[../../schema-org/Class/is_a_/Bio_Chem_Entity|bio_chem_entity]]'
dv_has_properties: '[ additionalType, alternateName, associatedDisease, bioChemInteraction, bioChemSimilarity, biologicalRole, chemicalComposition, chemicalRole, description, disambiguatingDescription, funding, hasBioChemEntityPart, hasMolecularFunction, hasRepresentation, identifier, image, isEncodedByBioChemEntity, isInvolvedInBiologicalProcess, isLocatedInSubcellularLocation, isPartOfBioChemEntity, mainEntityOfPage, name, potentialAction, potentialUse, sameAs, subjectOf, taxonomicRange, url ]'
dv_is_same_as:
- '[[../../WikiData/WD~Chemical_substance,79529|WD~Chemical_substance,79529]]'
- '[[/_Standards/Chemistry/Substance/Chemical_Substance|Chemical_Substance]]'
- '[[/_public/Chemistry/Substance/Chemical_Substance.public|Chemical_Substance.public]]'
- '[[/_internal/Chemistry/Substance/Chemical_Substance.internal|Chemical_Substance.internal]]'
- '[[/_protect/Chemistry/Substance/Chemical_Substance.protect|Chemical_Substance.protect]]'
- '[[/_private/Chemistry/Substance/Chemical_Substance.private|Chemical_Substance.private]]'
- '[[/_personal/Chemistry/Substance/Chemical_Substance.personal|Chemical_Substance.personal]]'
- '[[/_secret/Chemistry/Substance/Chemical_Substance.secret|Chemical_Substance.secret]]'
---
# [[Chemical_Substance]] 

Class of all chemical_substances.

Tag Instances like this: 
#is_a_/chemical_substance

is_part_of = `=this.dv_is_part_of` 

A chemical substance is 'a portion of matter of constant composition, 
composed of molecular entities of the same type or of different types' (source: <a href="https://www.ebi.ac.uk/chebi/searchId.do?chebiId=59999">ChEBI:59999</a>).

has_parent_class = `=this.dv_has_parent_class` 

has_properties = `=this.dv_has_properties` 

#is_/same_as :: [[../../WikiData/WD~Chemical_substance,79529|WD~Chemical_substance,79529]] 

## #has_/text_of_/abstract 

> A **Chemical substance** is a unique form of matter with constant chemical composition and characteristic properties. 
> Chemical substances may take the form of a single element or chemical compounds. 
> If two or more chemical substances can be combined without reacting, they may form a mixture. 
> If a mixture is separated to isolate one chemical substance to a desired degree, 
> the resulting substance is said to be chemically pure.
>
> Chemical substances can exist in several different physical states or phases (e.g. solids, liquids, gases, or plasma) 
> without changing their chemical composition. 
> Substances transition between these phases of matter in response to changes in temperature or pressure. 
> Some chemical substances can be combined or converted into new substances by means of chemical reactions. 
> Chemicals that do not possess this ability are said to be inert.
>
> Pure water is an example of a chemical substance, 
> with a constant composition of two hydrogen atoms bonded to a single oxygen atom (i.e. H2O). 
> The atomic ratio of hydrogen to oxygen is always 2:1 in every molecule of water. 
> Pure water will tend to boil near 100 °C (212 °F), an example of one of the characteristic properties that define it. 
> Other notable chemical substances include diamond (a form of the element carbon), table salt (NaCl; an ionic compound), and refined sugar (C12H22O11; an organic compound).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Chemical%20substance) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Substance/Chemical_Substance|Chemical_Substance]] 

### #is_/same_as :: [[/_public/Chemistry/Substance/Chemical_Substance.public|Chemical_Substance.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Substance/Chemical_Substance.internal|Chemical_Substance.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Substance/Chemical_Substance.protect|Chemical_Substance.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Substance/Chemical_Substance.private|Chemical_Substance.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Substance/Chemical_Substance.personal|Chemical_Substance.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Substance/Chemical_Substance.secret|Chemical_Substance.secret]] 

