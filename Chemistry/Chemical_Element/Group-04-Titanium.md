---
aliases:
- 4. grupa
- 4. grupa hemijskih elemenata
- 4. Grupp vum Periodesystem
- 4. Hauptgruppe des Periodensystems
- 4. skupina
- 4. taldeko elementu
- 4. група хемијских елемената
- 4η ομάδα του περιοδικού πίνακα
- 4වන කාණ්ඩයේ මූල ද්රව්ය
- 4族元素
- 4족 원소
- bagas 4
- Datu golongan 4
- Dövri sistemin 4-cü qrupu
- elementi del gruppo 4
- elemento de grupo 4
- elementos del grupo 4
- Elementos del grupu 4
- Elementos do grupo 4
- Elfen Grŵp 4
- elimentu dû gruppu 4
- frumefni í flokki 4
- Grop 4 de la taula periodica
- group 4
- Group-04-Titanium
- groupe 4
- grup 4 de la taula periòdica
- Grupa Titanului
- Grupi i katërt i elementeve kimike
- grupo 04 (química)
- Grupo 4 d'a tabla periodica
- grupp 4
- Grupp 4 de la taula periòdica
- gruppe 4
- grúpa 4
- Guruubka 4
- nhóm nguyên tố 4
- Periodinės cheminių elementų sistemos IVB grupė
- possan 4
- Ryhmän 4 alkuaine
- titaangroep
- Titaanskööl
- Titanyum grubu
- tytanowce
- unsur golongan 4
- Unsur kumpulan 4
- Група 4 на периодичната система
- група 4 періодичної системи елементів
- Елементи од група 4
- падгрупа тытану
- подгруппа титана
- קבוצת יסודות 4
- عناصر المجموعة الرابعة
- گروه ۴ جدول تناوبی
- समूह ४ तत्व
- চতুর্থ শ্রেণীর মৌল
- ਗਰੁੱਪ 4 ਤੱਤ
- நெடுங்குழு 4 தனிமங்கள்
- గ్రూప్ 4 మూలకం
- ธาตุหมู่ 4
- 第4族元素
has_id_wikidata: Q189302
Commons_category: Periodic table group 4
Dewey_Decimal_Classification: 546.51
has_part_s_:
- '[[_Standards/WikiData/WD~titanium,716|WD~titanium,716]]'
- '[[_Standards/WikiData/WD~zirconium,1038|WD~zirconium,1038]]'
- '[[_Standards/WikiData/WD~hafnium,1119|WD~hafnium,1119]]'
- '[[_Standards/WikiData/WD~rutherfordium,1226|WD~rutherfordium,1226]]'
- '[[_Standards/WikiData/WD~chemical_element,11344|WD~chemical_element,11344]]'
instance_of: '[[_Standards/WikiData/WD~group,83306|WD~group,83306]]'
P8189: 987007538813205100
part_of:
- '[[_Standards/WikiData/WD~d_block,214503|WD~d_block,214503]]'
- '[[_Standards/WikiData/WD~periodic_table,10693|WD~periodic_table,10693]]'
type: ElementGroup
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Group_4,189302|WD~Group_4,189302]]'
  - '[[/_Standards/Chemistry/Chemical_Element/Group-04-Titanium|Group-04-Titanium]]'
  - '[[/_public/Chemistry/Chemical_Element/Group-04-Titanium.public|Group-04-Titanium.public]]'
  - '[[/_internal/Chemistry/Chemical_Element/Group-04-Titanium.internal|Group-04-Titanium.internal]]'
  - '[[/_protect/Chemistry/Chemical_Element/Group-04-Titanium.protect|Group-04-Titanium.protect]]'
  - '[[/_private/Chemistry/Chemical_Element/Group-04-Titanium.private|Group-04-Titanium.private]]'
  - '[[/_personal/Chemistry/Chemical_Element/Group-04-Titanium.personal|Group-04-Titanium.personal]]'
  - '[[/_secret/Chemistry/Chemical_Element/Group-04-Titanium.secret|Group-04-Titanium.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Group_4,189302|WD~Group_4,189302]]'
- '[[/_Standards/Chemistry/Chemical_Element/Group-04-Titanium|Group-04-Titanium]]'
- '[[/_public/Chemistry/Chemical_Element/Group-04-Titanium.public|Group-04-Titanium.public]]'
- '[[/_internal/Chemistry/Chemical_Element/Group-04-Titanium.internal|Group-04-Titanium.internal]]'
- '[[/_protect/Chemistry/Chemical_Element/Group-04-Titanium.protect|Group-04-Titanium.protect]]'
- '[[/_private/Chemistry/Chemical_Element/Group-04-Titanium.private|Group-04-Titanium.private]]'
- '[[/_personal/Chemistry/Chemical_Element/Group-04-Titanium.personal|Group-04-Titanium.personal]]'
- '[[/_secret/Chemistry/Chemical_Element/Group-04-Titanium.secret|Group-04-Titanium.secret]]'
dv_has_:
  name_:
    an: Grupo 4 d'a tabla periodica
    ar: عناصر المجموعة الرابعة
    ast: Elementos del grupu 4
    az: Dövri sistemin 4-cü qrupu
    ban: Datu golongan 4
    be_tarask: падгрупа тытану
    bg: Група 4 на периодичната система
    bn: চতুর্থ শ্রেণীর মৌল
    bs: 4. grupa hemijskih elemenata
    ca: grup 4 de la taula periòdica
    cs: 4. skupina
    cy: Elfen Grŵp 4
    da: gruppe 4
    de: 4. Hauptgruppe des Periodensystems
    el: 4η ομάδα του περιοδικού πίνακα
    en: group 4
    eo: elemento de grupo 4
    es: elementos del grupo 4
    eu: 4. taldeko elementu
    fa: گروه ۴ جدول تناوبی
    fi: Ryhmän 4 alkuaine
    fr: groupe 4
    frr: Titaanskööl
    ga: grúpa 4
    gl: Elementos do grupo 4
    gv: possan 4
    he: קבוצת יסודות 4
    hi: समूह ४ तत्व
    id: unsur golongan 4
    is: frumefni í flokki 4
    it: elementi del gruppo 4
    ja: 第4族元素
    ko: 4족 원소
    kw: bagas 4
    lb: 4. Grupp vum Periodesystem
    lmo: Grupp 4 de la taula periòdica
    lt: Periodinės cheminių elementų sistemos IVB grupė
    lv: 4. grupa
    mk: Елементи од група 4
    ms: Unsur kumpulan 4
    nl: titaangroep
    nn: gruppe 4
    oc: Grop 4 de la taula periodica
    pa: ਗਰੁੱਪ 4 ਤੱਤ
    pl: tytanowce
    pt: grupo 04 (química)
    ro: Grupa Titanului
    ru: подгруппа титана
    scn: elimentu dû gruppu 4
    sh: 4. grupa hemijskih elemenata
    si: 4වන කාණ්ඩයේ මූල ද්රව්ය
    sk: 4. skupina
    sl: 4. skupina
    so: Guruubka 4
    sq: Grupi i katërt i elementeve kimike
    sr: 4. група хемијских елемената
    sr_ec: 4. група хемијских елемената
    sr_el: 4. grupa hemijskih elemenata
    sv: grupp 4
    ta: நெடுங்குழு 4 தனிமங்கள்
    te: గ్రూప్ 4 మూలకం
    th: ธาตุหมู่ 4
    tr: Titanyum grubu
    uk: група 4 періодичної системи елементів
    vi: nhóm nguyên tố 4
    wuu: 4族元素
    yue: 4族元素
    zh: 4族元素
    zh_cn: 4族元素
    zh_hans: 4族元素
    zh_hant: 4族元素
    zh_hk: 4族元素
    zh_sg: 4族元素
    zh_tw: 4族元素
---

# [[Group-04-Titanium]] 

#chemic/Group 

4)[[Group-04-Titanium/Titanium|Titanium]]]
5)[[Group-04-Titanium/Zirconium|Zirconium]]]
6)[[Group-04-Titanium/Hafnium|Hafnium]]]
7)[[Group-04-Titanium/Rutherfordium|Rutherfordium]]]

#is_/same_as :: [[../../WikiData/WD~Group_4,189302|WD~Group_4,189302]] 

## #has_/text_of_/abstract 

> Group 4 is the second group of transition metals in the periodic table. It contains the four elements titanium (Ti), zirconium (Zr), hafnium (Hf), and rutherfordium (Rf). The group is also called the titanium group or titanium family after its lightest member.
>
> As is typical for early transition metals, zirconium and hafnium have only the group oxidation state of +4 as a major one, and are quite electropositive and have a less rich coordination chemistry. Due to the effects of the lanthanide contraction, they are very similar in properties. Titanium is somewhat distinct due to its smaller size: it has a well-defined +3 state as well (although +4 is more stable).
>
> All the **Group 4 element**s are hard, refractory metals. Their inherent reactivity is completely masked due to the formation of a dense oxide layer that protects them from corrosion, as well as attack by many acids and alkalis. The first three of them occur naturally. Rutherfordium is strongly radioactive: it does not occur naturally and must be produced by artificial synthesis, but its observed and theoretically predicted properties are consistent with it being a heavier homologue of hafnium. None of them have any biological role.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Group%204%20element) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Element/Group-04-Titanium|Group-04-Titanium]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Element/Group-04-Titanium.public|Group-04-Titanium.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Element/Group-04-Titanium.internal|Group-04-Titanium.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Element/Group-04-Titanium.protect|Group-04-Titanium.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Element/Group-04-Titanium.private|Group-04-Titanium.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Element/Group-04-Titanium.personal|Group-04-Titanium.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Element/Group-04-Titanium.secret|Group-04-Titanium.secret]] 

