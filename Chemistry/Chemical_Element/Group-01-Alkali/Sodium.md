---
aliases:
- ijo lili nanpa 11
- INaside
- konutai
- Na
- naatrium
- natiri
- natri
- natrii
- natrij
- Natrin
- natrio
- natris
- natriu
- Natrium
- natriumi
- natriy
- natriyu
- natriyûm
- natro
- natrín
- na̍p
- nátrium
- nātrijs
- năk
- nаtriy
- saadiyom
- sodi
- sodio
- sodiom
- sodiomu
- sodiu
- Sodium
- sodiuma
- sodiwm
- Sodiyamu
- Sodiyeemii
- sodju
- sodjum
- sodna
- sodu
- sodyo
- sodyum
- sodyòm
- sodík
- soodin
- sòdi
- sòdio
- sòdiu
- sòidium
- sód
- sódio
- sóidiam
- sôdgium
- Sôdiôma
- sɔdɩyɔm
- νάτριο
- νάτριον
- натар
- натрій
- Натри
- натријум
- натрий
- натрион
- натриум
- натрый
- Նաթրիոն
- նատրիում
- נאטריום
- נתרן
- سدیم
- سودیوم
- سوڈیم
- سوډيوم
- سوڊيئم
- سۆدیۆم
- صوداصر
- صوديوم
- ناترىي
- نتريوم
- ސޯޑިއަމް
- सोडियम
- সোডিয়াম
- ਸੋਡੀਅਮ
- સોડિયમ
- ସୋଡ଼ିଅମ
- சோடியம்
- సోడియమ్
- ಸೋಡಿಯಮ್
- സോഡിയം
- සෝඩියම්
- โซเดียม
- ໂຊດຽມ
- བུལ་རྫས།
- ဆိုဒီယမ်
- သိုဒီယမ်
- ნატრიუმი
- ሶዲየም
- សូដ្យូម
- ᱥᱚᱰᱤᱭᱟᱢ
- ⴰⵚⵓⴹⵢⵓⵎ
- ナトリウム
- 鈉
- 钠
- ꯁꯣꯗꯤꯌꯝ
- 나트륨
- 나트리움
has_id_wikidata: Q658
atomic_number: 11
canonical_SMILES: '[Na]'
CAS_Registry_Number: 7440-23-5
chemical_formula: Na
Colon_Classification: E111
Commons_category: Sodium
Commons_gallery: Sodium
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718]]'
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138]]'
- "[[_Standards/WikiData/WD~The_New_Student's_Reference_Work,16082057]]"
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~New_Encyclopedic_Dictionary,19190511]]'
- '[[_Standards/WikiData/WD~Sodium,84690714]]'
Dewey_Decimal_Classification: 546.382
different_from:
- '[[_Standards/WikiData/WD~sodium_in_biology,3337048]]'
- '[[_Standards/WikiData/WD~serum_sodium_level,3963915]]'
- '[[_Standards/WikiData/WD~sodium,114850397]]'
- '[[_Standards/WikiData/WD~table_salt,11254]]'
discoverer_or_inventor:
- '[[_Standards/WikiData/WD~Humphry_Davy,131761]]'
DSSTOX_compound_identifier: DTXCID00876449
EC_number: 231-132-9
electronegativity: 1
electron_configuration:
- '[Ne] 3s¹'
- 1s² 2s² 2p⁶ 3s¹
element_symbol: Na
follows: '[[_Standards/WikiData/WD~neon,654]]'
found_in_taxon:
- '[[_Standards/WikiData/WD~Phytelephas_aequatorialis,2709988]]'
- '[[_Standards/WikiData/WD~Montanoa_tomentosa,6904440]]'
- '[[_Standards/WikiData/WD~Montanoa_frutescens,15564776]]'
- '[[_Standards/WikiData/WD~Montanoa_grandiflora,15564965]]'
- '[[_Standards/WikiData/WD~Montanoa_leucantha,15565960]]'
- '[[_Standards/WikiData/WD~Chamomilla_recutita,50837010]]'
- '[[_Standards/WikiData/WD~Matricaria_chamomilla,28437]]'
- '[[_Standards/WikiData/WD~cacao,42385]]'
Gmelin_number: 16221
Gujarati_Vishwakosh_entry: સોડિયમ
image: http://commons.wikimedia.org/wiki/Special:FilePath/Sodium%20Atom.png
InChI: InChI=1S/Na
InChIKey: KEAYESYHFKHZAL-UHFFFAOYSA-N
instance_of:
- '[[_Standards/WikiData/WD~lithophile,1130479]]'
- '[[_Standards/WikiData/WD~chemical_element,11344]]'
ionic_radius:
- 0.99
- 1.02
- 1.18
- 1.24
- 1.39
ionization_energy:
- 5.14
- 495.8
Krugosvet_article: nauka_i_tehnika/himiya/NATRI.html
location_of_discovery:
- '[[_Standards/WikiData/WD~United_Kingdom,145]]'
made_from_material:
- '[[_Standards/WikiData/WD~albite,182264]]'
- '[[_Standards/WikiData/WD~seawater,184395]]'
- '[[_Standards/WikiData/WD~cryolite,193502]]'
- '[[_Standards/WikiData/WD~brine,214403]]'
- '[[_Standards/WikiData/WD~analcime,239589]]'
- '[[_Standards/WikiData/WD~riebeckite,256865]]'
- '[[_Standards/WikiData/WD~glaucophane,413272]]'
- '[[_Standards/WikiData/WD~natrolite,415318]]'
- '[[_Standards/WikiData/WD~natron,427329]]'
- '[[_Standards/WikiData/WD~clinoptilolite-Na,1116232]]'
- '[[_Standards/WikiData/WD~arfvedsonite,2008960]]'
- '[[_Standards/WikiData/WD~chabazite-Na,2947265]]'
- '[[_Standards/WikiData/WD~eckermannite,3718594]]'
- '[[_Standards/WikiData/WD~heulandite-Na,3785394]]'
- '[[_Standards/WikiData/WD~phillipsite-Na,3901554]]'
- '[[_Standards/WikiData/WD~stilbite-Na,3973529]]'
- '[[_Standards/WikiData/WD~halite,5314]]'
mass: 22.98976928
MeSH_tree_code:
- D01.268.549.750
- D01.268.557.650
- D01.552.528.850
- D01.552.547.725
named_after:
- '[[_Standards/WikiData/WD~sodium_hydroxide,102769]]'
- '[[_Standards/WikiData/WD~sodium_carbonate,190227]]'
- '[[_Standards/WikiData/WD~natron,427329]]'
- '[[_Standards/WikiData/WD~headache,86]]'
OmegaWiki_Defined_Meaning: 7248
oxidation_state:
- -1
- 0
- 1
part_of:
- '[[_Standards/WikiData/WD~period_3,211331]]'
- '[[_Standards/WikiData/WD~group_1,10801007]]'
- '[[_Standards/WikiData/WD~alkali_metal,19557]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/Sodium-ar.ogg
PubChem_CID: 5360545
radius:
- 180
- 190
- 154
- 227
SpocWebEntityId: 21913
standard_molar_entropy: 51.2
subclass_of:
- '[[_Standards/WikiData/WD~s-block,208106]]'
- '[[_Standards/WikiData/WD~alkali_metal,19557]]'
time_of_discovery_or_invention: 1807
type: Element
UNII: 9NEZ333N27
ZVG_number: 8080
dv_is_:
  same_as:
  - '[[../../../WikiData/WD~Sodium,658]]'
  - '[[/_Standards/Chemistry/Chemical_Element/Group-01-Alkali/Sodium|Sodium]]'
  - '[[/_public/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.public|Sodium.public]]'
  - '[[/_internal/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.internal|Sodium.internal]]'
  - '[[/_protect/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.protect|Sodium.protect]]'
  - '[[/_private/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.private|Sodium.private]]'
  - '[[/_personal/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.personal|Sodium.personal]]'
  - '[[/_secret/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.secret|Sodium.secret]]'
  part_of: '[[../Group-01-Alkali]]'
dv_has_:
  name_:
    abbreviated: Na
    af: natrium
    am: ሶዲየም
    an: sodio
    anp: सोडियम
    ar: صوديوم
    ary: صوديوم
    arz: صوديوم
    ast: sodiu
    awa: सोडियम
    az: natrium
    azb: سودیوم
    ban: natrium
    bcl: sodyo
    be: натрый
    be_tarask: натар
    bg: натрий
    bho: सोडियम
    blk: သိုဒီယမ်
    bn: সোডিয়াম
    bo: བུལ་རྫས།
    br: sodiom
    bs: natrij
    ca: sodi
    cdo: năk
    ceb: sodio
    ckb: سۆدیۆم
    co: sodiu
    cs: sodík
    cv: натри
    cy: sodiwm
    da: natrium
    de: Natrium
    de_ch: Natrium
    dv: ސޯޑިއަމް
    el: νάτριο
    en: sodium
    en_ca: sodium
    en_gb: sodium
    eo: natrio
    es: sodio
    et: naatrium
    eu: sodio
    ext: sodiu
    fa: سدیم
    fi: natrium
    fo: natrium
    fr: sodium
    frr: Natrium
    fur: sodi
    fy: natrium
    ga: sóidiam
    gan: 鈉
    gd: sòidium
    gl: sodio
    group: Group-01-Alkali
    gu: સોડિયમ
    gv: sodjum
    hak: na̍p
    haw: sodiuma
    he: נתרן
    hi: सोडियम
    hif: sodium
    hr: natrij
    ht: sodyòm
    hu: nátrium
    hy: նատրիում
    hyw: Նաթրիոն
    ia: sodium
    id: natrium
    io: natro
    is: natrín
    it: sodio
    ja: ナトリウム
    jbo: sodna
    jv: natrium
    ka: ნატრიუმი
    kaa: nаtriy
    kbp: sɔdɩyɔm
    ki: sodium
    kk: натрий
    km: សូដ្យូម
    kn: ಸೋಡಿಯಮ್
    ko: 나트륨
    ko-kp: 나트리움
    ku: natriyûm
    kv: натрий
    kw: sodiom
    ky: натрий
    la: natrium
    lb: Natrium
    lfn: sodio
    li: natrium
    lij: sodio
    lmo: sodio
    ln: sodu
    lo: ໂຊດຽມ
    lt: natris
    lv: nātrijs
    lzh: 鈉
    mdf: Натри
    mg: Sôdiôma
    mi: konutai
    min: natrium
    mk: натриум
    ml: സോഡിയം
    mn: натри
    mni: ꯁꯣꯗꯤꯌꯝ
    mr: सोडियम
    mrj: натрий
    ms: natrium
    ms_arab: نتريوم
    mt: sodju
    mul: Na
    my: ဆိုဒီယမ်
    nan: Na
    nb: natrium
    nds: Natrium
    ne: सोडियम
    new: सोडियम
    nl: natrium
    nn: natrium
    nov: natrium
    nrm: sôdgium
    nv: soodin
    oc: sòdi
    olo: natrii
    om: Sodiyeemii
    or: ସୋଡ଼ିଅମ
    os: натрий
    pa: ਸੋਡੀਅਮ
    pap: sodio
    pap-aw: sodio
    pi: सोडियम
    pl: sód
    pms: sòdio
    pnb: سوڈیم
    pnt: νάτριον
    ps: سوډيوم
    pt: sódio
    pt_br: sódio
    qu: natriyu
    ro: sodiu
    ru: натрий
    rup: natriu
    sa: सोडियम
    sah: натриум
    sat: ᱥᱚᱰᱤᱭᱟᱢ
    sc: sòdiu
    scn: sodiu
    sco: sodium
    sd: سوڊيئم
    se: natrium
    sh: natrij
    si: සෝඩියම්
    sk: sodík
    skr: سوڈیم
    sl: natrij
    smn: natrium
    so: saadiyom
    sq: natriumi
    sr: натријум
    stq: Natrium
    su: natrium
    sv: natrium
    sw: natiri
    ta: சோடியம்
    te: సోడియమ్
    tg: натрий
    th: โซเดียม
    tl: sodyo
    tok: ijo lili nanpa 11
    tr: sodyum
    tt: натрий
    tum: Sodiyamu
    ug: ناترىي
    uk: натрій
    ur: صوداصر
    uz: natriy
    vec: sòdio
    vep: natrii
    vi: natri
    vo: Natrin
    wa: sodiom
    war: sodyo
    wuu: 钠
    xal: натрион
    yi: נאטריום
    yo: sodiomu
    yue: 鈉
    zgh: ⴰⵚⵓⴹⵢⵓⵎ
    zh: 鈉
    zh_cn: 钠
    zh_hans: 钠
    zh_hant: 鈉
    zh_hk: 鈉
    zh_mo: 鈉
    zh-my: 钠
    zh_sg: 钠
    zh_tw: 鈉
    zu: INaside
  number_of_:
    period: 3
    electrons: 11
    group: 1
  value_for_:
    weight_:
      u: 22.9898
    radius: 2.23
    volume_:
      ccm_:
        mol: 23.7
    unknown2: 1.54
    radius_:
      nano_:
        m: 5.139
    electronegativity: 0.9
    density: 0.97
    temperature_:
      melt_:
        c: 97.81
      boil_:
        c: 882.9
    capacity_:
      heat: 1.23
    conductivity_:
      heat: 1.41
    enthalpy: 2.598
    energy_:
      form_:
        kj_:
          mol: 96.96
dv_is_same_as:
- '[[../../../WikiData/WD~Sodium,658]]'
- '[[/_Standards/Chemistry/Chemical_Element/Group-01-Alkali/Sodium|Sodium]]'
- '[[/_public/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.public|Sodium.public]]'
- '[[/_internal/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.internal|Sodium.internal]]'
- '[[/_protect/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.protect|Sodium.protect]]'
- '[[/_private/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.private|Sodium.private]]'
- '[[/_personal/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.personal|Sodium.personal]]'
- '[[/_secret/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.secret|Sodium.secret]]'
dv_has_name_group: Group-01-Alkali
dv_is_part_of: '[[../Group-01-Alkali]]'
dv_has_name_en: Sodium
dv_has_name_de: Natrium
dv_has_value_for_weight_u: 22.9898
dv_has_value_for_radius: 2.23
dv_has_value_for_volume_ccm_mol: 23.7
dv_has_value_for_unknown2: 1.54
dv_has_value_for_radius_nano_m: 5.139
dv_has_value_for_electronegativity: 0.9
dv_has_value_for_density: 0.97
dv_has_value_for_temperature_melt_c: 97.81
dv_has_value_for_temperature_boil_c: 882.9
dv_has_value_for_capacity_heat: 1.23
dv_has_value_for_conductivity_heat: 1.41
dv_has_value_for_enthalpy: 2.598
dv_has_value_for_energy_form_kj_mol: 96.96
dv_has_number_of_period: 3
dv_has_name_abbreviated: Na
dv_has_number_of_electrons: 11
dv_has_number_of_group: 1
Unicode_character: 鈉
---

# [[Sodium]] 鈉 



#chemic/Element 
#is_/same_as :: [[../../../WikiData/WD~Sodium,658]] 

has_name_group = `=this.dv_has_name_group` 
is_part_of = `=this.dv_is_part_of` 

has_name_en = `=this.dv_has_name_en` 

has_name_de = `=this.dv_has_name_de` 

has_value_for_weight_u = `=this.dv_has_value_for_weight_u` 

has_value_for_radius = `=this.dv_has_value_for_radius` 
has_value_for_volume_ccm_mol = `=this.dv_has_value_for_volume_ccm_mol` 
has_value_for_unknown2 = `=this.dv_has_value_for_unknown2` 

has_value_for_radius_nano_m = `=this.dv_has_value_for_radius_nano_m` 

has_value_for_electronegativity = `=this.dv_has_value_for_electronegativity` 
has_value_for_density = `=this.dv_has_value_for_density` 

has_value_for_temperature_melt_c = `=this.dv_has_value_for_temperature_melt_c` 

has_value_for_temperature_boil_c = `=this.dv_has_value_for_temperature_boil_c` 

has_value_for_capacity_heat = `=this.dv_has_value_for_capacity_heat` 
has_value_for_conductivity_heat = `=this.dv_has_value_for_conductivity_heat` 
has_value_for_enthalpy = `=this.dv_has_value_for_enthalpy` 
has_value_for_energy_form_kj_mol = `=this.dv_has_value_for_energy_form_kj_mol` 
has_number_of_period = `=this.dv_has_number_of_period` 

## #has_/text_of_/abstract 

> **Sodium** is a chemical element with the symbol has_name_abbreviated = `=this.dv_has_name_abbreviated`  (from Latin natrium) and atomic number has_number_of_electrons = `=this.dv_has_number_of_electrons` . 
> It is a soft, silvery-white, highly reactive metal. 
> 
> Sodium is an alkali metal, being in group has_number_of_group = `=this.dv_has_number_of_group`  of the periodic table. Its only stable isotope is 23Na. 
> The free metal does not occur in nature, and must be prepared from compounds. 
> Sodium is the sixth most abundant element in the Earth's crust 
> and exists in numerous minerals such as feldspars, sodalite, and halite (NaCl). 
> 
> Many salts of sodium are highly water-soluble: 
> sodium ions have been leached by the action of water from the Earth's minerals over eons, 
> and thus sodium and chlorine are the most common dissolved elements by weight in the oceans.
>
> Sodium was first isolated by Humphry Davy in 1807 by the electrolysis of sodium hydroxide. 
> Among many other useful sodium compounds, sodium hydroxide (lye) is used in soap manufacture, 
> and sodium chloride (edible salt) is a de-icing agent and a nutrient for animals including humans.
>
> Sodium is an essential element for all animals and some plants. 
> Sodium ions are the major cation in the extracellular fluid (ECF) 
> and as such are the major contributor to the ECF osmotic pressure and ECF compartment volume. 
> Loss of water from the ECF compartment increases the sodium concentration, 
> a condition called hypernatremia. 
> Isotonic loss of water and sodium from the ECF compartment 
> decreases the size of that compartment in a condition called ECF hypovolemia.
>
> By means of the sodium-potassium pump, 
> living human cells pump three sodium ions out of the cell 
> in exchange for two potassium ions pumped in; 
> comparing ion concentrations across the cell membrane, inside to outside, 
> potassium measures about 40:1, and sodium, about 1:10. 
> 
> In nerve cells, the electrical charge across the cell membrane 
> enables transmission of the nerve impulse—an action potential—
> when the charge is dissipated; sodium plays a key role in that activity.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Sodium) 


## Natrium-Batteries 

Natrium Ions are larger and less movable than [[Lithium]]. 
But they don't require rare Graphite and can work with amorphous [[../Group-14-Carbon/Carbon|Carbon]]. 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Element/Group-01-Alkali/Sodium|Sodium]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.public|Sodium.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.internal|Sodium.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.protect|Sodium.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.private|Sodium.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.personal|Sodium.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Element/Group-01-Alkali/Sodium.secret|Sodium.secret]] 

