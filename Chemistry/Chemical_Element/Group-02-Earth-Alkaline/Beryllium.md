---
aliases:
- Abirilyum
- Beirilium
- beirilliam
- Belilu
- Beriiliyaam
- Berili
- Berilie
- berilij
- berilijs
- Berilijum
- Berilin
- berilio
- beriliom
- berilis
- Beriliu
- Berilium
- Beriliumi
- beriliwm
- Beriliyamu
- Berilli
- Berillii
- Berillio
- Berilliu
- Berillium
- Berilliy
- berillju
- Berilyo
- Berilyu
- berilyum
- Berilyòm
- beril·li
- beryl
- beryli
- Berylium
- Beryllio
- Berylliom
- Beryllium
- beryllín
- Berìli
- berìlio
- Berìlliu
- berílio
- Berîlyûm
- berüllium
- berýlium
- Biriiliyeemii
- birilliu
- Bèrilium
- Bérilium
- béryllium
- Bɛrɩlɩyɔm
- IBehlezani
- ijo lili nanpa 4
- Iztactlāltepoztli
- jinmrberilo
- konuuku
- Pì
- βηρύλλιο
- Белүр
- Берилій
- берилијум
- берилий
- берилиум
- Берилли
- Бериллий
- Бериллиум
- Берылій
- бэрыль
- բերիլիում
- בעריליום
- בריליום
- بریلیم
- بریلیوم
- بلوصر
- بيريليئم
- بيريليوم
- بيرېليوم
- بیریلیم
- بێریلیۆم
- بېرىللىي
- बेरिलियम
- विडूर
- वैडूर्य
- বেরিলিয়াম
- বেৰিলিয়াম
- ਬੇਰਿਲੀਅਮ
- બેરિલિયમ
- ବେରିଲିୟମ
- பெரிலியம்
- బెరీలియం
- ಬೆರಿಲಿಯಮ್
- ബെറിലിയം
- බෙරිලියම්
- เบริลเลียม
- བེ་རི་ལིམ།
- ဘယ်ရီလီယမ်
- ბერილიუმი
- ቤሪሊየም
- ᏇᎵᎵᎥᎻ
- បេរីល្យូម
- ᱵᱮᱨᱤᱞᱤᱭᱚᱢ
- ⴰⴱⵉⵔⵉⵍⵢⵓⵎ
- ベリリウム
- 鈹
- 铍
- ꯕꯦꯔꯤꯂꯤꯌꯝ
- 베릴륨
- 베릴리움
has_id_wikidata: Q569
associated_hazard: '[[_Standards/WikiData/WD~acute_beryllium_poisoning,4896104]]'
atomic_number: 4
boiling_point:
- 2970
- 4532
canonical_SMILES: '[Be]'
CAS_Registry_Number: 7440-41-7
ceiling_exposure_limit: 0.0005
chemical_formula: Be
Commons_category: Beryllium
Commons_gallery: Beryllium
density: 1.85
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718]]'
- "[[_Standards/WikiData/WD~Collier's_New_Encyclopedia,_1921,19047539]]"
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
Dewey_Decimal_Classification:
- 546.391
- 669.724
different_from: '[[_Standards/WikiData/WD~Beryl,440624]]'
discoverer_or_inventor:
- '[[_Standards/WikiData/WD~Louis_Nicolas_Vauquelin,333438]]'
- '[[_Standards/WikiData/WD~Antoine_Bussy,585780]]'
- '[[_Standards/WikiData/WD~Friedrich_Wöhler,58575]]'
DSSTOX_compound_identifier: DTXCID90875623
EC_number: 231-150-7
electrical_conductivity: 25000000
electronegativity: 2
electron_configuration:
- '[He] 2s²'
- 1s² 2s²
element_symbol: Be
Gmelin_number: 16265
has_effect:
- '[[_Standards/WikiData/WD~berylliosis,684810]]'
- '[[_Standards/WikiData/WD~acute_beryllium_poisoning,4896104]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Beryllium%20nuggets%202.jpg
immediately_dangerous_to_life_or_health: 4
InChI: InChI=1S/Be
InChIKey: ATBAMAFKBVZNFJ-UHFFFAOYSA-N
instance_of:
- '[[_Standards/WikiData/WD~lithophile,1130479]]'
- '[[_Standards/WikiData/WD~simple_substance,2512777]]'
- '[[_Standards/WikiData/WD~combustible_powder,21073023]]'
- '[[_Standards/WikiData/WD~chemical_element,11344]]'
ionic_radius:
- 0.27
- 0.45
Krugosvet_article: nauka_i_tehnika/himiya/BERILLI.html
location_of_discovery: '[[_Standards/WikiData/WD~France,142]]'
made_from_material: '[[_Standards/WikiData/WD~beryl,103480]]'
mass: 9.0121831
melting_point:
- 1278
- 2349
MeSH_tree_code:
- D01.268.552.075
- D01.268.557.080
- D01.552.547.080
named_after: '[[_Standards/WikiData/WD~beryl,103480]]'
OmegaWiki_Defined_Meaning: 336
oxidation_state: 2
part_of:
- '[[_Standards/WikiData/WD~period_2,207712]]'
- '[[_Standards/WikiData/WD~alkaline_earth_metal,19563]]'
PubChem_CID: 5460467
RTECS_number: DS1750000
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/Electron%20shell%20de%20004%20Beryllium.svg
SpocWebEntityId: 21907 á à ô
subclass_of: '[[_Standards/WikiData/WD~metal,11426]]'
subject_has_role:
- '[[_Standards/WikiData/WD~carcinogen,187661]]'
- '[[_Standards/WikiData/WD~occupational_carcinogen,21074597]]'
time_of_discovery_or_invention: 1798
time_weighted_average_exposure_limit: 0.002
type: Element
UMLS_CUI: C0005139
UNII: OW5102UV6N
vapor_pressure: 0
ZVG_number: 8020
dv_is_:
  same_as:
  - '[[../../../WikiData/WD~Beryllium,569]]'
  - '[[/_Standards/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium|Beryllium]]'
  - '[[/_public/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.public|Beryllium.public]]'
  - '[[/_internal/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.internal|Beryllium.internal]]'
  - '[[/_protect/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.protect|Beryllium.protect]]'
  - '[[/_private/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.private|Beryllium.private]]'
  - '[[/_personal/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.personal|Beryllium.personal]]'
  - '[[/_secret/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.secret|Beryllium.secret]]'
  part_of: '[[../Group-02-Earth-Alkaline]]'
dv_has_:
  name_:
    abbreviated: Be
    af: berillium
    am: ቤሪሊየም
    an: Berilio
    anp: बेरिलियम
    ar: بيريليوم
    ary: بيريليوم
    arz: بيريليوم
    as: বেৰিলিয়াম
    ast: beriliu
    awa: बेरिलियम
    az: Berillium
    azb: بریلیوم
    ban: Bérilium
    bcl: Berilyo
    be: Берылій
    be_tarask: бэрыль
    bg: берилий
    bho: बेरिलियम
    blk: ဘယ်ရီလီယမ်
    bn: বেরিলিয়াম
    bo: བེ་རི་ལིམ།
    br: Beriliom
    bs: berilij
    ca: beril·li
    cdo: Pì
    ceb: Berilio
    chr: ᏇᎵᎵᎥᎻ
    ckb: بێریلیۆم
    co: Berilliu
    cs: beryllium
    cv: Берилли
    cy: beriliwm
    da: beryllium
    de: Beryllium
    de_ch: Beryllium
    egl: Berìli
    el: βηρύλλιο
    eml: Berìli
    en: beryllium
    en_ca: Beryllium
    en_gb: beryllium
    eo: berilio
    es: berilio
    et: berüllium
    eu: berilio
    fa: بریلیم
    fi: beryllium
    fo: Beryllium
    fr: béryllium
    frr: Beryllium
    fur: Berili
    fy: Beryllium
    ga: beirilliam
    gan: 鈹
    gd: Beirilium
    gl: berilio
    group: Group-02-Earth-Alkaline
    gu: બેરિલિયમ
    gv: beryllium
    ha: beryllium
    hak: Beryllium
    haw: Beryllium
    he: בריליום
    hi: विडूर
    hif: Beryllium
    hr: berilij
    hsb: Berylium
    ht: Berilyòm
    hu: berillium
    hy: բերիլիում
    ia: beryllium
    id: berilium
    io: berilio
    is: beryllín
    it: berillio
    ja: ベリリウム
    jbo: jinmrberilo
    jv: Bèrilium
    ka: ბერილიუმი
    kbp: Bɛrɩlɩyɔm
    ki: Beryllium
    kk: Бериллий
    km: បេរីល្យូម
    kn: ಬೆರಿಲಿಯಮ್
    knc: Beryllium
    ko: 베릴륨
    ko-kp: 베릴리움
    ku: Berîlyûm
    kv: Бериллий
    kw: Berylliom
    ky: Бериллий
    la: beryllium
    lb: Beryllium
    lfn: berilio
    li: Beryl
    lij: Berillio
    lld: Berilie
    lmo: Berilio
    ln: Belilu
    lt: berilis
    lv: berilijs
    lzh: 鈹
    mdf: Берилли
    mi: konuuku
    mk: берилиум
    ml: ബെറിലിയം
    mn: берилли
    mni: ꯕꯦꯔꯤꯂꯤꯌꯝ
    mr: बेरिलियम
    mrj: Бериллий
    ms: Berilium
    mt: berillju
    my: ဘယ်ရီလီယမ်
    nah: Iztactlāltepoztli
    nan: Beryllium
    nb: beryllium
    nds: Beryllium
    ne: बेरिलियम
    new: बेरिलियम
    nl: beryllium
    nn: beryllium
    nov: Berilium
    oc: Berilli
    olo: Berillium
    om: Biriiliyeemii
    or: ବେରିଲିୟମ
    os: бериллий
    pa: ਬੇਰਿਲੀਅਮ
    pap: Beryllio
    pi: बेरिलियम
    pl: beryl
    pms: Berili
    pnb: بیریلیم
    ps: بيرېليوم
    pt: berílio
    pt_br: berílio
    qu: Berilyu
    ro: beriliu
    ru: бериллий
    rue: Берилій
    rup: Beriliu
    sa: वैडूर्य
    sah: Бериллиум
    sat: ᱵᱮᱨᱤᱞᱤᱭᱚᱢ
    sc: Berìlliu
    scn: birilliu
    sco: beryllium
    sd: بيريليئم
    se: beryllium
    sh: Berilijum
    shi: Abirilyum
    si: බෙරිලියම්
    sk: berýlium
    sl: berilij
    smn: beryllium
    sn: Beryllium
    so: Beriiliyaam
    sq: Beriliumi
    sr: берилијум
    stq: Beryllium
    su: Bérilium
    sv: beryllium
    sw: Berili
    ta: பெரிலியம்
    te: బెరీలియం
    tg: Бериллий
    th: เบริลเลียม
    tl: Berilyo
    tok: ijo lili nanpa 4
    tr: berilyum
    tt: Бериллий
    tum: Beriliyamu
    udm: бериллий
    ug: بېرىللىي
    uk: берилій
    ur: بلوصر
    uz: Berilliy
    vec: berìlio
    vep: Berillii
    vi: beryli
    vls: beryllium
    vo: Berilin
    wa: beriliom
    war: Berilyo
    wuu: 铍
    xal: Белүр
    xh: Beryllium
    yi: בעריליום
    yo: Beryllium
    yue: 鈹
    zgh: ⴰⴱⵉⵔⵉⵍⵢⵓⵎ
    zh: 鈹
    zh_cn: 铍
    zh_hans: 铍
    zh_hant: 鈹
    zh_hk: 鈹
    zh_mo: 鈹
    zh-my: 铍
    zh_sg: 铍
    zh_tw: 鈹
    zu: IBehlezani
  number_of_:
    period: 2
    group: 2
    electrons: 4
  value_for_:
    weight_:
      u: 9.0122
    radius: 1.4
    volume_:
      ccm_:
        mol: 5.009
    unknown2: 0.9
    radius_:
      nano_:
        m: 9.322
    electronegativity: 1.6
    density: 1.85
    temperature_:
      melt_:
        c: 1278
      boil_:
        c: 2970
    capacity_:
      heat: 1.82
    conductivity_:
      heat: 2
    enthalpy: 12.2
    energy_:
      form_:
        kj_:
          mol: 292.4
dv_is_same_as:
- '[[../../../WikiData/WD~Beryllium,569]]'
- '[[/_Standards/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium|Beryllium]]'
- '[[/_public/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.public|Beryllium.public]]'
- '[[/_internal/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.internal|Beryllium.internal]]'
- '[[/_protect/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.protect|Beryllium.protect]]'
- '[[/_private/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.private|Beryllium.private]]'
- '[[/_personal/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.personal|Beryllium.personal]]'
- '[[/_secret/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.secret|Beryllium.secret]]'
dv_has_name_group: Group-02-Earth-Alkaline
dv_is_part_of: '[[../Group-02-Earth-Alkaline]]'
dv_has_name_de: Beryllium
dv_has_value_for_weight_u: 9.0122
dv_has_value_for_radius: 1.4
dv_has_value_for_volume_ccm_mol: 5.009
dv_has_value_for_unknown2: 0.9
dv_has_value_for_radius_nano_m: 9.322
dv_has_value_for_electronegativity: 1.6
dv_has_value_for_density: 1.85
dv_has_value_for_temperature_melt_c: 1278
dv_has_value_for_temperature_boil_c: 2970
dv_has_value_for_capacity_heat: 1.82
dv_has_value_for_conductivity_heat: 2
dv_has_value_for_enthalpy: 12.2
dv_has_value_for_energy_form_kj_mol: 292.4
dv_has_number_of_period: 2
dv_has_number_of_group: 2
dv_has_name_en: Beryllium
dv_has_name_abbreviated: Be
dv_has_number_of_electrons: 4
Unicode_character: 鈹
---
# [[Beryllium]] 鈹 

#chemic/Element 
#is_/same_as :: [[../../../WikiData/WD~Beryllium,569]] 

has_name_group = `=this.dv_has_name_group` 
is_part_of = `=this.dv_is_part_of` 

has_name_de = `=this.dv_has_name_de` 

has_value_for_weight_u = `=this.dv_has_value_for_weight_u` 

has_value_for_radius = `=this.dv_has_value_for_radius` 
has_value_for_volume_ccm_mol = `=this.dv_has_value_for_volume_ccm_mol` 
has_value_for_unknown2 = `=this.dv_has_value_for_unknown2` 

has_value_for_radius_nano_m = `=this.dv_has_value_for_radius_nano_m` 

has_value_for_electronegativity = `=this.dv_has_value_for_electronegativity` 
has_value_for_density = `=this.dv_has_value_for_density` 

has_value_for_temperature_melt_c = `=this.dv_has_value_for_temperature_melt_c` 

has_value_for_temperature_boil_c = `=this.dv_has_value_for_temperature_boil_c` 

has_value_for_capacity_heat = `=this.dv_has_value_for_capacity_heat` 
has_value_for_conductivity_heat = `=this.dv_has_value_for_conductivity_heat` 
has_value_for_enthalpy = `=this.dv_has_value_for_enthalpy` 
has_value_for_energy_form_kj_mol = `=this.dv_has_value_for_energy_form_kj_mol` 
has_number_of_period = `=this.dv_has_number_of_period` 
has_number_of_group = `=this.dv_has_number_of_group` 

## #has_/text_of_/abstract 

> **has_name_en = `=this.dv_has_name_en` ** is a chemical element with the symbol has_name_abbreviated = `=this.dv_has_name_abbreviated`  and atomic number has_number_of_electrons = `=this.dv_has_number_of_electrons` . 
> 
> It is a steel-gray, strong, lightweight and brittle alkaline earth metal. 
> 
> It is a divalent element that occurs naturally only in combination with other elements to form minerals. 
> Notable gemstones high in beryllium include beryl (aquamarine, emerald) and chrysoberyl. 
> 
> It is a relatively rare element in the universe, 
> usually occurring as a product of the spallation of larger atomic nuclei 
> that have collided with cosmic rays. 
> 
> Within the cores of stars, beryllium is depleted as it is fused into heavier elements. 
> Beryllium constitutes about 0.0004 percent by mass of Earth's crust. 
> The world's annual beryllium production of 220 tons 
> is usually manufactured by extraction from the mineral beryl, 
> a difficult process because beryllium bonds strongly to oxygen.
>
> In structural applications, the combination of high flexural rigidity, 
> thermal stability, thermal conductivity and low density (1.85 times that of water) 
> make beryllium metal a desirable aerospace material for aircraft components, 
> missiles, spacecraft, and satellites. 
> 
> Because of its low density and atomic mass, 
> beryllium is relatively transparent to X-rays and other forms of ionizing radiation; 
> therefore, it is the most common window material for X-ray equipment 
> and components of particle detectors. 
> 
> When added as an alloying element to aluminium, copper (notably the alloy beryllium copper), 
> iron, or nickel, beryllium improves many physical properties. 
> For example, tools and components made of beryllium copper alloys are strong and hard 
> and do not create sparks when they strike a steel surface. 
> 
> In air, the surface of beryllium oxidizes readily at room temperature to form a 
> passivation layer 1–10 nm thick that protects it from further oxidation and corrosion. 
> The metal oxidizes in bulk (beyond the passivation layer) when heated above 500 °C (932 °F), 
> and burns brilliantly when heated to about 2,500 °C (4,530 °F).
>
> The commercial use of beryllium requires the use of appropriate dust control equipment 
> and industrial controls at all times because of the toxicity of inhaled beryllium-containing dusts 
> that can cause a chronic life-threatening allergic disease in some people called berylliosis. 
> Berylliosis causes pneumonia and other associated respiratory illness.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Beryllium) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium|Beryllium]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.public|Beryllium.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.internal|Beryllium.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.protect|Beryllium.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.private|Beryllium.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.personal|Beryllium.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Element/Group-02-Earth-Alkaline/Beryllium.secret|Beryllium.secret]] 

