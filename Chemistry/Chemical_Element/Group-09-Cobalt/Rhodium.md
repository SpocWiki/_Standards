---
aliases:
- rhodi
- Rhodium
- Rhodiumi
- rhodiwm
- rod
- Rodi
- Rodii
- rodij
- rodijs
- Rodijum
- rodio
- Rodiom
- Rodis
- rodiu
- rodium
- Rodiy
- rodju
- Rodyo
- Rodyum
- Rodyòm
- Rodyûm
- Ronyadel
- Roodhiyaam
- Roodium
- roydjum
- Rutyu
- Ròdi
- Ròdio
- Ròdiu
- Ròdium
- ródio
- ródium
- ródín
- róidiam
- Rɔdɩyɔm
- ULundu
- zgujinme
- ρόδιο
- род
- родій
- Роди
- родијум
- родий
- Родиум
- Родый
- ռոդիում
- רהאדיום
- רודיום
- رودىي
- روديوم
- رودیم
- روڈیم
- روڊيم
- رہوڈیئم
- ڕۆدیۆم
- रोडियम
- र्होडियम
- রোডিয়াম
- ৰ’ডিয়াম
- ਰ੍ਹੋਡੀਅਮ
- રોડિયમ
- ରୋଡ଼ିଅମ
- ரோடியம்
- రోడియం
- ರೋಡಿಯಮ್
- റോഡിയം
- โรเดียม
- རོ་ཌིམ།
- ရိုဒီယမ်
- როდიუმი
- ሮዲየም
- ロジウム
- 銠
- 铑
- ꯔ꯭ꯍꯣꯗꯤꯌꯝ
- 로듐
has_id_wikidata: Q1087
associated_hazard: '[[_Standards/WikiData/WD~rhodium_exposure,21175091]]'
atomic_number: 45
boiling_point: 6741
canonical_SMILES: '[Rh]'
CAS_Registry_Number: 7440-16-6
chemical_formula: Rh
Commons_category: Rhodium
Commons_gallery: Rhodium
Common_Procurement_Vocabulary_code:
- 14744000
- 14744000-8
CPV_Supplementary_code: AA22
density: 12.41
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Explanatory_Dictionary_of_the_Living_Great_Russian_Language,_Second_Edition,23705356]]'
different_from: '[[_Standards/WikiData/WD~Rod,231105]]'
discoverer_or_inventor: '[[_Standards/WikiData/WD~William_Hyde_Wollaston,312975]]'
DSSTOX_compound_identifier: DTXCID0031053
EC_number: 231-125-0
electrical_conductivity: 22300000
electronegativity: 2.28
electron_configuration: '[Kr] 5s¹ 4d⁸'
element_symbol: Rh
has_effect: '[[_Standards/WikiData/WD~rhodium_exposure,21175091]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Rhodium%20powder%20pressed%20melted.jpg
immediately_dangerous_to_life_or_health: 100
InChI: InChI=1S/Rh
InChIKey: MHOVAHRLVXNVSD-UHFFFAOYSA-N
instance_of:
- '[[_Standards/WikiData/WD~combustible_powder,21073023]]'
- '[[_Standards/WikiData/WD~chemical_element,11344]]'
ionic_radius:
- 0.6
- 0.55
- 0.67
mass: 102.90549
melting_point: 3571
MeSH_tree_code:
- D01.268.556.793
- D01.268.956.781
- D01.552.544.793
named_after: '[[_Standards/WikiData/WD~rose,533047]]'
OmegaWiki_Defined_Meaning: 349380
oxidation_state: 3
part_of:
- '[[_Standards/WikiData/WD~group_9,208107]]'
- '[[_Standards/WikiData/WD~period_5,244982]]'
PubChem_CID: 23948
RTECS_number: VI9069000
SpocWebEntityId: 21943
subclass_of:
- '[[_Standards/WikiData/WD~platinum_group,223995]]'
- '[[_Standards/WikiData/WD~period_5,244982]]'
- '[[_Standards/WikiData/WD~transition_metal,19588]]'
tensile_modulus_of_elasticity: 275000
time_of_discovery_or_invention: 1803
time_weighted_average_exposure_limit: 0.1
type: Element
UMLS_CUI: C0035493
Unicode_character: 銠
UNII: DMK383DSAC
vapor_pressure: 0
video: http://commons.wikimedia.org/wiki/Special:FilePath/A%20very%20unique%20rhodium%20cake.webm
Wolfram_Language_entity_code: Entity["Mineral", "Rhodium"]
ZVG_number: 7450
dv_has_:
  name_:
    abbreviated: Rh
    af: rodium
    am: ሮዲየም
    an: Rodio
    anp: रोडियम
    ar: روديوم
    ary: روديوم
    arz: روديوم
    as: ৰ’ডিয়াম
    ast: rodiu
    avk: Ronyadel
    az: Rodium
    ban: Rhodium
    bcl: Rodyo
    be: Родый
    be-tarask: род
    bg: родий
    bho: रोडियम
    bn: রোডিয়াম
    bo: རོ་ཌིམ།
    br: Rodiom
    bs: rodij
    ca: rodi
    cdo: Rhodium
    ceb: Rodyo
    ckb: ڕۆدیۆم
    co: Rodiu
    cs: rhodium
    cv: Роди
    cy: rhodiwm
    da: rhodium
    de: Rhodium
    el: ρόδιο
    en: rhodium
    eo: rodio
    es: rodio
    et: Roodium
    eu: rodio
    fa: رودیم
    fi: rodium
    fr: rhodium
    frr: Roodium
    fur: Rodi
    ga: róidiam
    gd: Ròdium
    gl: rodio
    group: Group-09-Cobalt
    gu: રોડિયમ
    gv: roydjum
    hak: rhodium
    he: רודיום
    hi: रोडियम
    hif: Rhodium
    hr: Rodij
    ht: Rodyòm
    hu: ródium
    hy: ռոդիում
    ia: rhodium
    id: rodium
    io: rodio
    is: ródín
    it: rodio
    ja: ロジウム
    jbo: zgujinme
    jv: Rodium
    ka: როდიუმი
    kbp: Rɔdɩyɔm
    kk: Родий
    kn: ರೋಡಿಯಮ್
    ko: 로듐
    ku: Rodyûm
    kv: Родий
    kw: Rodiom
    ky: Родий
    la: rhodium
    lb: Rhodium
    lfn: rodio
    li: Rhodium
    lij: Rodio
    lmo: Rodio
    lt: Rodis
    lv: rodijs
    lzh: 銠
    mk: родиум
    ml: റോഡിയം
    mn: роди
    mni: ꯔ꯭ꯍꯣꯗꯤꯌꯝ
    mr: र्होडियम
    mrj: Родий
    ms: Rodium
    mt: rodju
    my: ရိုဒီယမ်
    nan: Rhodium
    nb: rhodium
    ne: रोडियम
    new: रोडियम
    nl: rodium
    nn: rhodium
    nov: Rodium
    oc: Ròdi
    olo: Rodii
    or: ରୋଡ଼ିଅମ
    os: родий
    pa: ਰ੍ਹੋਡੀਅਮ
    pi: रोडियम
    pl: rod
    pms: Ròdio
    pnb: روڈیم
    ps: روديوم
    pt: ródio
    pt-br: ródio
    qu: Rutyu
    ro: rodiu
    ru: родий
    rup: Rodiu
    sa: रोडियम
    sc: Ròdiu
    scn: rodiu
    sco: rhodium
    sd: روڊيم
    sh: Rodijum
    sk: ródium
    sl: rodij
    so: Roodhiyaam
    sq: Rhodiumi
    sr: родијум
    sr-ec: Родијум
    stq: Rhodium
    sv: rodium
    sw: Rodi
    ta: ரோடியம்
    te: రోడియం
    tg: родий
    th: โรเดียม
    tl: rhodium
    tr: Rodyum
    tt: Родий
    ug: رودىي
    uk: родій
    ur: رہوڈیئم
    uz: Rodiy
    vep: Rodii
    vi: rhodi
    war: Rodyo
    wuu: 铑
    xal: Родиум
    yi: רהאדיום
    yo: Rhodium
    yue: 銠
    zh: 铑
    zh-hans: 铑
    zh-hant: 銠
    zh-tw: 銠
    zu: ULundu
  number_of_:
    period: 5
    group: 9
    electrons: 45
  value_for_:
    weight_:
      u: 102.9055
    radius: 1.83
    volume_:
      ccm_:
        mol: 8.3
    unknown2: 1.25
    radius_:
      nano_:
        m: 7.46
    electronegativity: 2.3
    density: 12.4
    temperature_:
      melt_:
        c: 1966
      boil_:
        c: 3727
    capacity_:
      heat: 0.242
    conductivity_:
      heat: 1.5
    enthalpy: 21.5
    energy_:
      form_:
        kj_:
          mol: 493
dv_is_:
  part_of: '[[../Group-09-Cobalt]]'
  same_as:
  - '[[../../../WikiData/WD~Rhodium,1087]]'
  - '[[/_Standards/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium|Rhodium]]'
  - '[[/_public/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.public|Rhodium.public]]'
  - '[[/_internal/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.internal|Rhodium.internal]]'
  - '[[/_protect/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.protect|Rhodium.protect]]'
  - '[[/_private/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.private|Rhodium.private]]'
  - '[[/_personal/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.personal|Rhodium.personal]]'
  - '[[/_secret/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.secret|Rhodium.secret]]'
dv_has_name_group: Group-09-Cobalt
dv_is_part_of: '[[../Group-09-Cobalt]]'
dv_has_name_en: Rhodium
dv_has_name_de: Rhodium
dv_has_value_for_weight_u: 102.9055
dv_has_value_for_radius: 1.83
dv_has_value_for_volume_ccm_mol: 8.3
dv_has_value_for_unknown2: 1.25
dv_has_value_for_radius_nano_m: 7.46
dv_has_value_for_electronegativity: 2.3
dv_has_value_for_density: 12.4
dv_has_value_for_temperature_melt_c: 1966
dv_has_value_for_temperature_boil_c: 3727
dv_has_value_for_capacity_heat: 0.242
dv_has_value_for_conductivity_heat: 1.5
dv_has_value_for_enthalpy: 21.5
dv_has_value_for_energy_form_kj_mol: 493
dv_has_number_of_period: 5
dv_has_number_of_group: 9
dv_is_same_as:
- '[[../../../WikiData/WD~Rhodium,1087]]'
- '[[/_Standards/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium|Rhodium]]'
- '[[/_public/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.public|Rhodium.public]]'
- '[[/_internal/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.internal|Rhodium.internal]]'
- '[[/_protect/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.protect|Rhodium.protect]]'
- '[[/_private/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.private|Rhodium.private]]'
- '[[/_personal/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.personal|Rhodium.personal]]'
- '[[/_secret/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.secret|Rhodium.secret]]'
dv_has_name_abbreviated: Rh
dv_has_number_of_electrons: 45
---
# [[Rhodium]] 

#chemic/Element 

has_name_group = `=this.dv_has_name_group` 
is_part_of = `=this.dv_is_part_of` 

has_name_en = `=this.dv_has_name_en` 

has_name_de = `=this.dv_has_name_de` 

has_value_for_weight_u = `=this.dv_has_value_for_weight_u` 

has_value_for_radius = `=this.dv_has_value_for_radius` 
has_value_for_volume_ccm_mol = `=this.dv_has_value_for_volume_ccm_mol` 
has_value_for_unknown2 = `=this.dv_has_value_for_unknown2` 

has_value_for_radius_nano_m = `=this.dv_has_value_for_radius_nano_m` 

has_value_for_electronegativity = `=this.dv_has_value_for_electronegativity` 
has_value_for_density = `=this.dv_has_value_for_density` 

has_value_for_temperature_melt_c = `=this.dv_has_value_for_temperature_melt_c` 

has_value_for_temperature_boil_c = `=this.dv_has_value_for_temperature_boil_c` 

has_value_for_capacity_heat = `=this.dv_has_value_for_capacity_heat` 
has_value_for_conductivity_heat = `=this.dv_has_value_for_conductivity_heat` 
has_value_for_enthalpy = `=this.dv_has_value_for_enthalpy` 
has_value_for_energy_form_kj_mol = `=this.dv_has_value_for_energy_form_kj_mol` 
has_number_of_period = `=this.dv_has_number_of_period` 
has_number_of_group = `=this.dv_has_number_of_group` 
#is_/same_as :: [[../../../WikiData/WD~Rhodium,1087]] 


## #has_/text_of_/abstract 

> **Rhodium** is a chemical element with the symbol has_name_abbreviated = `=this.dv_has_name_abbreviated`  and atomic number has_number_of_electrons = `=this.dv_has_number_of_electrons` . 
> It is a very rare, silvery-white, hard, corrosion-resistant transition metal. 
> It is a noble metal and a member of the platinum group. 
> It has only one naturally occurring isotope: 103Rh. 
> 
> Naturally occurring rhodium is usually found as a free metal 
> or as an alloy with similar metals and rarely as a chemical compound 
> in minerals such as bowieite and rhodplumsite. 
> It is one of the rarest and most valuable precious metals.
>
> Rhodium is found in platinum or nickel ores with the other members of the platinum group metals. 
> It was discovered in 1803 by William Hyde Wollaston in one such ore, 
> and named for the rose color of one of its chlorine compounds.
>
> The element's major use (consuming about 80% of world rhodium production) is as 
> one of the catalysts in the three-way catalytic converters in automobiles. 
> Because rhodium metal is inert against corrosion and most aggressive chemicals, 
> and because of its rarity, rhodium is usually alloyed with platinum or palladium 
> and applied in high-temperature and corrosion-resistive coatings. 
> 
> White gold is often plated with a thin rhodium layer to improve its appearance, 
> while sterling silver is often rhodium-plated to resist tarnishing. 
> 
> Rhodium is sometimes used to cure silicones: a two-part silicone in which 
> one part containing a silicon hydride and 
> the other containing a vinyl-terminated silicone are mixed; 
> one of these liquids contains a rhodium complex.
> 
> Rhodium detectors are used in nuclear reactors to measure the neutron flux level. 
> Other uses of rhodium include asymmetric hydrogenation used to form drug precursors 
> and the processes for the production of acetic acid.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Rhodium) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium|Rhodium]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.public|Rhodium.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.internal|Rhodium.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.protect|Rhodium.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.private|Rhodium.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.personal|Rhodium.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Element/Group-09-Cobalt/Rhodium.secret|Rhodium.secret]] 

