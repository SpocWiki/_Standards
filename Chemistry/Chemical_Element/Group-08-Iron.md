---
aliases:
- 8-я група
- 8. grupa hemijskih elemenata
- 8. Grupp vum Periodesystem
- 8. Hauptgruppe des Periodensystems
- 8. skupina
- 8. taldeko elementu
- 8. група хемијских елемената
- 8η ομάδα του περιοδικού πίνακα
- 8族元素
- 8족 원소
- bagas 8
- datu golongan 8
- Dövri sistemin 8-ci qrupu
- elementi del gruppo 8
- elemento de grupo 8
- elementos del grupo 8
- Elementos del grupu 8
- elementê gruba 8ine
- Elfen Grŵp 8
- elimentu dû gruppu 8
- frumefni í flokki 8
- groep 8 element
- group 8
- Group-08-Iron
- groupe 8
- grup 8 de la taula periòdica
- Grupa Fierului
- Grupi i tetë i elementeve kimike
- grupo 08
- Grupo 8 d'a tabla periodica
- grupp 8
- Grupp 8 de la taula periòdica
- gruppe 8
- grúpa 8
- Guruubka 8
- Iisenskööl
- nhóm nguyên tố 8
- possan 8
- ryhmän 8 alkuaine
- unsur golongan 8
- Unsur kumpulan 8
- żelazowce
- Група 8 на периодичната система
- Група 8Б на периодниот систем
- Кшнинь алокурось
- подгруппа железа
- родина заліза
- тимер астөркеме
- קבוצת יסודות 8
- عناصر المجموعة الثامنة
- گروه ۸ جدول تناوبی
- समूह ८ तत्व
- অষ্টম শ্রেণীর মৌল
- ਗਰੁੱਪ 8 ਤੱਤ
- நெடுங்குழு 8 தனிமங்கள்
- గ్రూప్ 8 మూలకం
- ธาตุหมู่ 8
- 第8族元素
type: ElementGroup
dv_is_:
  same_as:
  - '[[/_Standards/Chemistry/Chemical_Element/Group-08-Iron|Group-08-Iron]]'
  - '[[/_public/Chemistry/Chemical_Element/Group-08-Iron.public|Group-08-Iron.public]]'
  - '[[/_internal/Chemistry/Chemical_Element/Group-08-Iron.internal|Group-08-Iron.internal]]'
  - '[[/_protect/Chemistry/Chemical_Element/Group-08-Iron.protect|Group-08-Iron.protect]]'
  - '[[/_private/Chemistry/Chemical_Element/Group-08-Iron.private|Group-08-Iron.private]]'
  - '[[/_personal/Chemistry/Chemical_Element/Group-08-Iron.personal|Group-08-Iron.personal]]'
  - '[[/_secret/Chemistry/Chemical_Element/Group-08-Iron.secret|Group-08-Iron.secret]]'
dv_is_same_as:
- '[[/_Standards/Chemistry/Chemical_Element/Group-08-Iron|Group-08-Iron]]'
- '[[/_public/Chemistry/Chemical_Element/Group-08-Iron.public|Group-08-Iron.public]]'
- '[[/_internal/Chemistry/Chemical_Element/Group-08-Iron.internal|Group-08-Iron.internal]]'
- '[[/_protect/Chemistry/Chemical_Element/Group-08-Iron.protect|Group-08-Iron.protect]]'
- '[[/_private/Chemistry/Chemical_Element/Group-08-Iron.private|Group-08-Iron.private]]'
- '[[/_personal/Chemistry/Chemical_Element/Group-08-Iron.personal|Group-08-Iron.personal]]'
- '[[/_secret/Chemistry/Chemical_Element/Group-08-Iron.secret|Group-08-Iron.secret]]'
has_id_wikidata: Q202224
dv_has_:
  name_:
    an: Grupo 8 d'a tabla periodica
    ar: عناصر المجموعة الثامنة
    ast: Elementos del grupu 8
    az: Dövri sistemin 8-ci qrupu
    ban: datu golongan 8
    be_tarask: 8-я група
    bg: Група 8 на периодичната система
    bn: অষ্টম শ্রেণীর মৌল
    bs: 8. grupa hemijskih elemenata
    ca: grup 8 de la taula periòdica
    cs: 8. skupina
    cy: Elfen Grŵp 8
    de: 8. Hauptgruppe des Periodensystems
    diq: elementê gruba 8ine
    el: 8η ομάδα του περιοδικού πίνακα
    en: group 8
    eo: elemento de grupo 8
    es: elementos del grupo 8
    eu: 8. taldeko elementu
    fa: گروه ۸ جدول تناوبی
    fi: ryhmän 8 alkuaine
    fr: groupe 8
    frr: Iisenskööl
    ga: grúpa 8
    gv: possan 8
    he: קבוצת יסודות 8
    hi: समूह ८ तत्व
    id: unsur golongan 8
    is: frumefni í flokki 8
    it: elementi del gruppo 8
    ja: 第8族元素
    ko: 8족 원소
    kw: bagas 8
    lb: 8. Grupp vum Periodesystem
    lmo: Grupp 8 de la taula periòdica
    mk: Група 8Б на периодниот систем
    ms: Unsur kumpulan 8
    myv: Кшнинь алокурось
    nl: groep 8 element
    nn: gruppe 8
    pa: ਗਰੁੱਪ 8 ਤੱਤ
    pl: żelazowce
    pt: grupo 08
    ro: Grupa Fierului
    ru: подгруппа железа
    scn: elimentu dû gruppu 8
    sh: 8. grupa hemijskih elemenata
    sk: 8. skupina
    sl: 8. skupina
    so: Guruubka 8
    sq: Grupi i tetë i elementeve kimike
    sr: 8. група хемијских елемената
    sr_ec: 8. група хемијских елемената
    sr_el: 8. grupa hemijskih elemenata
    sv: grupp 8
    ta: நெடுங்குழு 8 தனிமங்கள்
    te: గ్రూప్ 8 మూలకం
    th: ธาตุหมู่ 8
    tt: тимер астөркеме
    tt_cyrl: тимер астөркеме
    uk: родина заліза
    vi: nhóm nguyên tố 8
    wuu: 8族元素
    yue: 8族元素
    zh: 8族元素
    zh_cn: 8族元素
    zh_hans: 8族元素
    zh_hant: 8族元素
    zh_hk: 8族元素
    zh_sg: 8族元素
    zh_tw: 8族元素
---

# [[Group-08-Iron]]  

#chemic/Group 

4)[[Group-08-Iron/Iron|Iron]]]
5)[[Group-08-Iron/Osmium|Osmium]]]
6)[[Group-08-Iron/Ruthenium|Ruthenium]]]


> Group 8 is a group (column) of chemical elements in the periodic table. It  consists of iron (Fe), ruthenium (Ru), osmium (Os) and hassium (Hs). They are all transition metals.
>
> Like other groups, the members of this family show patterns in electron configuration, especially in the outermost shells, resulting in trends in chemical behavior.
>
> "Group 8" is the modern standard designation for this group, adopted by the IUPAC in 1990.In the older group naming systems, this group was combined with groups 9 and 10 and called group "VIIIB" in the Chemical Abstracts Service (CAS) "U.S. system", or "VIII" in the old IUPAC (pre-1990) "European system" (and in Mendeleev's original table).
>
> Group 8 (current IUPAC) should not be confused with "group VIIIA" in the CAS system, which is group 18 (current IUPAC), the noble gases.
>
> While groups (columns) of the periodic table are sometimes named after their lighter member (as in "the oxygen group" for group 16), the term iron group does not mean "group 8".  Most often, it means a set of adjacent elements on period (row) 4 of the table that includes iron, such as chromium, manganese, iron, cobalt, and nickel, or only the last three, or some other set, depending on the context.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Group%208%20element) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Element/Group-08-Iron|Group-08-Iron]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Element/Group-08-Iron.public|Group-08-Iron.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Element/Group-08-Iron.internal|Group-08-Iron.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Element/Group-08-Iron.protect|Group-08-Iron.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Element/Group-08-Iron.private|Group-08-Iron.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Element/Group-08-Iron.personal|Group-08-Iron.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Element/Group-08-Iron.secret|Group-08-Iron.secret]] 

