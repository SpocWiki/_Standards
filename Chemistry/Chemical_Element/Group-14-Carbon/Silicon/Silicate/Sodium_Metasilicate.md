---
aliases:
- Gloine uisce
- metakrzemian sodu
- metasilicat de sodi
- metassilicato de sódio
- Natri metasilicat
- natria metasilikato
- Natrij-silikat
- natrijev metasilikat
- Natrijev silikat
- Natrijum silikat
- natriummetasilikaat
- natriumsilicaat
- natriumsilikat
- Natriy metasilikat
- Natronwasserglas
- nátrium-szilikát
- nātrija silikāts
- silicate de sodium
- silicato de sódio
- silicato di sodio
- silicato sódico
- Silicatu sódicu
- Sodio metasilikato
- sodium metasilicate
- Sodium silicate
- Sodyum silikat
- vandglas
- Vesilasi
- Vodní sklo
- Водно стъкло
- метасиликат натрия
- мэтасылікат натру
- Натријум силикат
- Рідке скло
- Նատրիումի սիլիկատներ
- سيليكات الصوديوم
- سیلیکات سدیم
- سیلیکات سودیوم
- सोडियम सिलिकेट
- சோடியம் சிலிக்கேட்டு
- സോഡിയം സിലിക്കേറ്റ്
- ケイ酸ナトリウム
- 偏矽酸鈉
- 矽酸鈉
- 硅酸鈉
- 硅酸钠
- 규산 나트륨
has_id_wikidata: Q425397
safety_classification_and_labelling: '[[_Standards/WikiData/WD~NFPA_704_Standard_System_for_the_Identification_of_the_Hazards_of_Materials_for_Emergency_Response,51139288]]'
subclass_of: '[[_Standards/WikiData/WD~sodium_silicates,54910469]]'
different_from: '[[_Standards/WikiData/WD~water_glass,107385620]]'
instance_of: '[[_Standards/WikiData/WD~type_of_chemical_entity,113145171]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._8,124737635]]'
has_part_s_:
- '[[_Standards/WikiData/WD~oxygen,629]]'
- '[[_Standards/WikiData/WD~sodium,658]]'
- '[[_Standards/WikiData/WD~silicon,670]]'
ZVG_number: 2350
UN_number: 3253
PubChem_CID:
- 23266
- 5163954
CosIng_number: 79774
mass: 121.941
InChI: InChI=1S/2Na.O3Si/c;;1-4(2)3/q2*+1;-2
UNII: 052612U92L
InChIKey: NTHWMYGWWRZVTN-UHFFFAOYSA-N
RxNorm_CUI: 1733690
Reaxys_registry_number: 11343143
chemical_structure: http://commons.wikimedia.org/wiki/Special:FilePath/Sodium-metasilicate-repeating-unit-2D.png
canonical_SMILES: '[O-][Si](=O)[O-].[Na+].[Na+]'
EC_number: 229-912-9
CAS_Registry_Number: 6834-92-0
DSSTOX_compound_identifier: DTXCID509669
chemical_formula: Na₂O₃Si
Commons_category: Sodium metasilicate
RTECS_number: VV9275000
dv_is_:
  same_as:
  - '[[../../../../../WikiData/WD~Sodium_metasilicate,425397|WD~Sodium_metasilicate,425397]]'
  - '[[/_Standards/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate|Sodium_Metasilicate]]'
  - '[[/_public/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.public|Sodium_Metasilicate.public]]'
  - '[[/_internal/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.internal|Sodium_Metasilicate.internal]]'
  - '[[/_protect/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.protect|Sodium_Metasilicate.protect]]'
  - '[[/_private/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.private|Sodium_Metasilicate.private]]'
  - '[[/_personal/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.personal|Sodium_Metasilicate.personal]]'
  - '[[/_secret/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.secret|Sodium_Metasilicate.secret]]'
dv_is_same_as:
- '[[../../../../../WikiData/WD~Sodium_metasilicate,425397|WD~Sodium_metasilicate,425397]]'
- '[[/_Standards/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate|Sodium_Metasilicate]]'
- '[[/_public/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.public|Sodium_Metasilicate.public]]'
- '[[/_internal/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.internal|Sodium_Metasilicate.internal]]'
- '[[/_protect/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.protect|Sodium_Metasilicate.protect]]'
- '[[/_private/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.private|Sodium_Metasilicate.private]]'
- '[[/_personal/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.personal|Sodium_Metasilicate.personal]]'
- '[[/_secret/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.secret|Sodium_Metasilicate.secret]]'
dv_has_:
  name_:
    af: natriummetasilikaat
    ar: سيليكات الصوديوم
    ast: Silicatu sódicu
    azb: سیلیکات سودیوم
    be_tarask: мэтасылікат натру
    bg: Водно стъкло
    bs: Natrij-silikat
    ca: metasilicat de sodi
    cs: Vodní sklo
    da: vandglas
    de: Natronwasserglas
    en: sodium metasilicate
    eo: natria metasilikato
    es: silicato sódico
    eu: Sodio metasilikato
    fa: سیلیکات سدیم
    fi: Vesilasi
    fr: silicate de sodium
    ga: Gloine uisce
    hi: सोडियम सिलिकेट
    hr: Natrijev silikat
    hu: nátrium-szilikát
    hy: Նատրիումի սիլիկատներ
    it: silicato di sodio
    ja: ケイ酸ナトリウム
    ko: 규산 나트륨
    lv: nātrija silikāts
    ml: സോഡിയം സിലിക്കേറ്റ്
    nb: natriumsilikat
    nl: natriumsilicaat
    pl: metakrzemian sodu
    pt: silicato de sódio
    pt_br: metassilicato de sódio
    ru: метасиликат натрия
    sh: Natrijum silikat
    sl: natrijev metasilikat
    sr: Natrijum silikat
    sr_ec: Натријум силикат
    sr_el: Natrijum silikat
    sv: natriumsilikat
    ta: சோடியம் சிலிக்கேட்டு
    tr: Sodyum silikat
    uk: Рідке скло
    uz: Natriy metasilikat
    vi: Natri metasilicat
    war: Sodium silicate
    yue: 偏矽酸鈉
    zh: 硅酸鈉
    zh_cn: 硅酸钠
    zh_hans: 硅酸钠
    zh_hant: 硅酸鈉
    zh_hk: 硅酸鈉
    zh_sg: 硅酸钠
    zh_tw: 矽酸鈉
---
# [[Sodium_Metasilicate]] 

#is_/same_as :: [[../../../../../WikiData/WD~Sodium_metasilicate,425397|WD~Sodium_metasilicate,425397]]  

## #has_/text_of_/abstract 

> Sodium metasilicate is the chemical substance with formula Na2SiO3, 
> which is the main component of commercial sodium silicate solutions.  
> 
> It is an ionic compound consisting of sodium cations Na+ 
> and the polymeric metasilicate anions [–SiO2−3–]n.  
> 
> It is a colorless crystalline hygroscopic and deliquescent solid, 
> soluble in water (giving an alkaline solution) but not in alcohols.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Sodium%20metasilicate) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate|Sodium_Metasilicate]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.public|Sodium_Metasilicate.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.internal|Sodium_Metasilicate.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.protect|Sodium_Metasilicate.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.private|Sodium_Metasilicate.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.personal|Sodium_Metasilicate.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Element/Group-14-Carbon/Silicon/Silicate/Sodium_Metasilicate.secret|Sodium_Metasilicate.secret]] 

