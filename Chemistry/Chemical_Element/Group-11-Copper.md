---
aliases:
- 11-րդ խմբի տարրեր
- 11. grupa hemijskih elemenata
- 11. Grupp vum Periodesystem
- 11. Hauptgruppe des Periodensystems
- 11. skupina
- 11. taldeko elementu
- 11. група хемијских елемената
- 11η ομάδα του περιοδικού πίνακα
- 11族元素
- 11족 원소
- bagas 11
- Datu golongan 11
- Dövri sistemin 11-ci qrupu
- elementi del gruppo 11
- Elementi ya kundi la 11
- elemento de grupo 11
- elementos del grupo 11
- Elementos del grupu 11
- Elementos do grupo 11
- Elfen Grŵp 11
- elimentu dû gruppu 11
- frumefni í flokki 11
- group 11
- Group-11-Copper
- groupe 11
- Gruba 11ine
- grup 11 de la taula periòdica
- Grupa 11 din tabelul periodic
- Grupi i njëmbëdhjetë i elementeve kimike
- grupo 11
- Grupo 11 d'a tabla periodica
- Grupp 11 de la taula periòdica
- Gruppe 11
- grúpa 11
- Guruubka 11
- kopergroep
- koppargruppen
- miedziowce
- Müntmetal
- nhóm nguyên tố 11
- periodesystemets gruppe 11
- possan 11
- pūmotu rōpū 11
- ryhmän 11 alkuaine
- Unsur golongan 11
- Unsur kumpulan 11
- бакыр астөркеме
- група 11 на периодичната система
- Група 11 періодичної системи елементів
- елементи од група 11
- падгрупа медзі
- подгруппа меди
- קבוצת יסודות 11
- عناصر المجموعة الحادية عشر
- گروه ۱۱ جدول تناوبی
- समूह ११ तत्व
- একাদশ শ্রেণীর মৌল
- ਗਰੁੱਪ 11 ਤੱਤ
- நெடுங்குழு 11 தனிமங்கள்
- గ్రూప్ 11 మూలకం
- ธาตุหมู่ 11
- 第11族元素
has_id_wikidata: Q185870
Commons_category: Periodic table group 11
Dewey_Decimal_Classification: 546.65
has_part_s_:
- '[[_Standards/WikiData/WD~copper,753]]'
- '[[_Standards/WikiData/WD~gold,897]]'
- '[[_Standards/WikiData/WD~silver,1090]]'
- '[[_Standards/WikiData/WD~roentgenium,1272]]'
- '[[_Standards/WikiData/WD~chemical_element,11344]]'
image:
- http://commons.wikimedia.org/wiki/Special:FilePath/Silver%20crystal.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Gold-crystals.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Cu-Scheibe.JPG
instance_of: '[[_Standards/WikiData/WD~group,83306]]'
part_of:
- '[[_Standards/WikiData/WD~d_block,214503]]'
- '[[_Standards/WikiData/WD~periodic_table,10693]]'
type: ElementGroup
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Group_11,185870]]'
  - '[[/_Standards/Chemistry/Chemical_Element/Group-11-Copper|Group-11-Copper]]'
  - '[[/_public/Chemistry/Chemical_Element/Group-11-Copper.public|Group-11-Copper.public]]'
  - '[[/_internal/Chemistry/Chemical_Element/Group-11-Copper.internal|Group-11-Copper.internal]]'
  - '[[/_protect/Chemistry/Chemical_Element/Group-11-Copper.protect|Group-11-Copper.protect]]'
  - '[[/_private/Chemistry/Chemical_Element/Group-11-Copper.private|Group-11-Copper.private]]'
  - '[[/_personal/Chemistry/Chemical_Element/Group-11-Copper.personal|Group-11-Copper.personal]]'
  - '[[/_secret/Chemistry/Chemical_Element/Group-11-Copper.secret|Group-11-Copper.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Group_11,185870]]'
- '[[/_Standards/Chemistry/Chemical_Element/Group-11-Copper|Group-11-Copper]]'
- '[[/_public/Chemistry/Chemical_Element/Group-11-Copper.public|Group-11-Copper.public]]'
- '[[/_internal/Chemistry/Chemical_Element/Group-11-Copper.internal|Group-11-Copper.internal]]'
- '[[/_protect/Chemistry/Chemical_Element/Group-11-Copper.protect|Group-11-Copper.protect]]'
- '[[/_private/Chemistry/Chemical_Element/Group-11-Copper.private|Group-11-Copper.private]]'
- '[[/_personal/Chemistry/Chemical_Element/Group-11-Copper.personal|Group-11-Copper.personal]]'
- '[[/_secret/Chemistry/Chemical_Element/Group-11-Copper.secret|Group-11-Copper.secret]]'
dv_has_:
  name_:
    an: Grupo 11 d'a tabla periodica
    ar: عناصر المجموعة الحادية عشر
    ast: Elementos del grupu 11
    az: Dövri sistemin 11-ci qrupu
    ban: Datu golongan 11
    be_tarask: падгрупа медзі
    bg: група 11 на периодичната система
    bn: একাদশ শ্রেণীর মৌল
    bs: 11. grupa hemijskih elemenata
    ca: grup 11 de la taula periòdica
    cs: 11. skupina
    cy: Elfen Grŵp 11
    da: Gruppe 11
    de: 11. Hauptgruppe des Periodensystems
    diq: Gruba 11ine
    el: 11η ομάδα του περιοδικού πίνακα
    en: group 11
    eo: elemento de grupo 11
    es: elementos del grupo 11
    eu: 11. taldeko elementu
    fa: گروه ۱۱ جدول تناوبی
    fi: ryhmän 11 alkuaine
    fr: groupe 11
    frr: Müntmetal
    ga: grúpa 11
    gl: Elementos do grupo 11
    gv: possan 11
    he: קבוצת יסודות 11
    hi: समूह ११ तत्व
    hy: 11-րդ խմբի տարրեր
    id: Unsur golongan 11
    is: frumefni í flokki 11
    it: elementi del gruppo 11
    ja: 第11族元素
    ko: 11족 원소
    kw: bagas 11
    lb: 11. Grupp vum Periodesystem
    lmo: Grupp 11 de la taula periòdica
    mi: pūmotu rōpū 11
    mk: елементи од група 11
    ms: Unsur kumpulan 11
    nb: periodesystemets gruppe 11
    nl: kopergroep
    nn: gruppe 11
    pa: ਗਰੁੱਪ 11 ਤੱਤ
    pl: miedziowce
    pt: grupo 11
    ro: Grupa 11 din tabelul periodic
    ru: подгруппа меди
    scn: elimentu dû gruppu 11
    sk: 11. skupina
    sl: 11. skupina
    so: Guruubka 11
    sq: Grupi i njëmbëdhjetë i elementeve kimike
    sr: 11. група хемијских елемената
    sv: koppargruppen
    sw: Elementi ya kundi la 11
    ta: நெடுங்குழு 11 தனிமங்கள்
    te: గ్రూప్ 11 మూలకం
    th: ธาตุหมู่ 11
    tt: бакыр астөркеме
    tt_cyrl: бакыр астөркеме
    uk: Група 11 періодичної системи елементів
    vi: nhóm nguyên tố 11
    wuu: 11族元素
    yue: 11族元素
    zh: 11族元素
    zh_cn: 11族元素
    zh_hans: 11族元素
    zh_hant: 11族元素
    zh_hk: 11族元素
    zh_sg: 11族元素
    zh_tw: 11族元素
---
# [[Group-11-Copper]] 

#chemic/Group 
 
4)[[Group-11-Copper/Copper]]]
5)[[Group-11-Copper/Silver]]]
6)[[Group-11-Copper/Gold]]]

#is_/same_as :: [[../../WikiData/WD~Group_11,185870]] 
## #has_/text_of_/abstract 

> Group 11, by modern IUPAC numbering, is a group of chemical elements in the periodic table, consisting of copper (Cu), silver (Ag), and gold (Au), and roentgenium (Rg), although no chemical experiments have yet been carried out to confirm that roentgenium behaves like the heavier homologue to gold. Group 11 is also known as the coinage metals, due to their usage in minting coins—while the rise in metal prices mean that silver and gold are no longer used for circulating currency, remaining in use for bullion, copper remains a common metal in coins to date, either in the form of copper clad coinage or as part of the cupronickel alloy. They were most likely the first three elements discovered. Copper, silver, and gold all occur naturally in elemental form.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Group%2011%20element) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Element/Group-11-Copper|Group-11-Copper]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Element/Group-11-Copper.public|Group-11-Copper.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Element/Group-11-Copper.internal|Group-11-Copper.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Element/Group-11-Copper.protect|Group-11-Copper.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Element/Group-11-Copper.private|Group-11-Copper.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Element/Group-11-Copper.personal|Group-11-Copper.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Element/Group-11-Copper.secret|Group-11-Copper.secret]] 

