---
dv_is_:
  same_as:
  - '[[/_Standards/Chemistry/Organic_Chemistry/RIC|RIC]]'
  - '[[/_public/Chemistry/Organic_Chemistry/RIC.public|RIC.public]]'
  - '[[/_internal/Chemistry/Organic_Chemistry/RIC.internal|RIC.internal]]'
  - '[[/_protect/Chemistry/Organic_Chemistry/RIC.protect|RIC.protect]]'
  - '[[/_private/Chemistry/Organic_Chemistry/RIC.private|RIC.private]]'
  - '[[/_personal/Chemistry/Organic_Chemistry/RIC.personal|RIC.personal]]'
  - '[[/_secret/Chemistry/Organic_Chemistry/RIC.secret|RIC.secret]]'
dv_is_same_as:
- '[[/_Standards/Chemistry/Organic_Chemistry/RIC|RIC]]'
- '[[/_public/Chemistry/Organic_Chemistry/RIC.public|RIC.public]]'
- '[[/_internal/Chemistry/Organic_Chemistry/RIC.internal|RIC.internal]]'
- '[[/_protect/Chemistry/Organic_Chemistry/RIC.protect|RIC.protect]]'
- '[[/_private/Chemistry/Organic_Chemistry/RIC.private|RIC.private]]'
- '[[/_personal/Chemistry/Organic_Chemistry/RIC.personal|RIC.personal]]'
- '[[/_secret/Chemistry/Organic_Chemistry/RIC.secret|RIC.secret]]'
has_id_wikidata: Q5609621
dv_has_:
  name_:
    ar: رموز تمييز الراتنج
    bn: রজন শনাক্তকরণ কোড
    bs: Kodovi za identifikaciju plastičnih masa
    ca: codi d'identificació de resines
    en: resin identification code
    eo: rezino-identiga kodo
    es: Código identificador de resina
    fr: Code d'identification des résines
    ja: 樹脂識別コード
    ko: 수지 식별 코드
    ml: ഭക്ഷണം കഴിക്കാനുതകുന്ന പ്ലാസ്റ്റിക്കിന്റെ കോഡുകൾ
    mn: Хуванцар савны таних тэмдэглэгээ
    sh: Kodovi za identifikaciju plastičnih masa
    sl: identifikacijska oznaka smole
    sr: Kodovi za identifikaciju plastičnih masa
    sv: Plastklassningskod
    sw: Nambari ya kitambulisho cha resin
    uk: Код ідентифікації смоли
    vi: Mã số nhận diện nhựa cây
    zh: 塑膠分類標誌
    zh_hans: 塑胶分类标志
    zh_hant: 塑膠分類標誌
    zh_hk: 塑膠分類標誌
aliases:
- Code d'identification des résines
- codi d'identificació de resines
- Código identificador de resina
- identifikacijska oznaka smole
- Kodovi za identifikaciju plastičnih masa
- Mã số nhận diện nhựa cây
- Nambari ya kitambulisho cha resin
- Plastklassningskod
- resin identification code
- rezino-identiga kodo
- Код ідентифікації смоли
- Хуванцар савны таних тэмдэглэгээ
- رموز تمييز الراتنج
- রজন শনাক্তকরণ কোড
- ഭക്ഷണം കഴിക്കാനുതകുന്ന പ്ലാസ്റ്റിക്കിന്റെ കോഡുകൾ
- 塑胶分类标志
- 塑膠分類標誌
- 樹脂識別コード
- 수지 식별 코드
---
# [[RIC]]  

the RIC is aligned with the EU [[Recycling_codes]]. 

## #has_/text_of_/abstract 

> The **Resin identification code** (RIC) is a technical standard 
> with a set of symbols appearing on plastic products 
> that identify the plastic resin out of which the product is made. 
> 
> It was developed in 1988 by the Society of the Plastics Industry 
> (now the Plastics Industry Association) in the United States, 
> but since 2008 it has been administered by ASTM International, an international standards organization.
>
> Due to resemblance to the recycling symbol, RIC symbols are often mistaken for the former. 
> Subsequent revisions to the RIC have replaced the arrows with a solid triangle, 
> but the old symbols are still in common use.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Resin%20identification%20code) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Organic_Chemistry/RIC|RIC]] 

### #is_/same_as :: [[/_public/Chemistry/Organic_Chemistry/RIC.public|RIC.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Organic_Chemistry/RIC.internal|RIC.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Organic_Chemistry/RIC.protect|RIC.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Organic_Chemistry/RIC.private|RIC.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Organic_Chemistry/RIC.personal|RIC.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Organic_Chemistry/RIC.secret|RIC.secret]] 

