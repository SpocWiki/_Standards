---
aliases:
- amenorgamientu-oxidación
- erredox erreakzio
- hapetus-pelkistysreaktio
- ocsdí
- oksidacijos-redukcijos reakcija
- oksidasyon
- oksidləşmə-reduksiya reaksiyaları
- oksido-redukcija
- oksidēšanās-reducēšanās reakcijas
- ossidoriduzione
- Oxidatiouns-Reaktioun
- oxidačno-redukčná reakcia
- oxidorredución
- oxirredução
- oxun
- phản ứng oxid hóa khử
- reaccion d'oxidoreduccion
- reacció d'oxidació-reducció
- reakcja redoks
- reaksionet redoks
- reação redox
- redoks
- redoks reakcija
- redoksa reakcio
- redoksreaksie
- Redoksreaksje
- redoksreaksjon
- redoksreaktsioon
- Redox
- Redox Reaction
- Redox Reaktion
- redoxireakció
- redoxní reakce
- redoxo
- redoxreactie
- redoxreaktion
- reducción-oxidación
- reduction–oxidation or oxidation–reduction
- rhydocs
- réaction d'oxydoréduction
- sng-hòa hoân-goân hoán-èng
- tindak balas redoks
- uksiday
- οξειδοαναγωγή
- акісленне
- балқытпа қалдық
- затляняльна-аднаўленчая рэакцыя
- исэлдэлт
- окисланыу-ҡайтарылыу реакциялары
- окислително-редукционни процеси
- окислительно-восстановительные реакции
- окисно-відновна реакція
- оксидационо-редукциона реакција
- оксидлав-таврăнав реакцийĕ
- оксидлашу-кайтару реакцияләре
- оксидо-редукција
- чырсатӥсь-берыктӥсь воштӥськонъёс
- օքսիդացում և վերականգնում
- חמצון-חיזור
- ئۆکسان
- اکسایش و کاهش
- تخسید
- تفاعلات أكسدة-اختزال
- ريڊوڪس
- رڈاکس
- आक्सीकरण
- জারণ-বিজারণ
- ਰਿਡਾਕਸ
- ஒடுக்க ஏற்ற வேதி வினைகள்
- ಉತ್ಕರ್ಷಣ - ಅಪಕರ್ಷಣ
- റിഡോക്സ് പ്രവർത്തനം
- ปฏิกิริยารีดอกซ์
- ჟანგვა-აღდგენითი რეაქციები
- ᮇᮊ᮪ᮞᮤᮓᮞᮤ
- 氧化
- 氧化还原反应
- 氧化還原反應
- 酸化還元反応
- 산화·환원 반응
has_id_wikidata: Q82682
used_by:
- '[[_Standards/WikiData/WD~electrochemical_cell,80097]]'
- '[[_Standards/WikiData/WD~cellular_respiration,912751]]'
has_part_s_:
- '[[_Standards/WikiData/WD~reduction,912546]]'
- '[[_Standards/WikiData/WD~oxidation,1786087]]'
- '[[_Standards/WikiData/WD~half_reaction,2272343]]'
part_of: '[[_Standards/WikiData/WD~Systems_Biology_Ontology,2377160]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._12,124737664]]'
video: http://commons.wikimedia.org/wiki/Special:FilePath/16.%20%D0%A0%D0%B5%D0%B0%D0%BA%D1%86%D0%B8%D1%98%D0%B0%20%D0%BC%D0%B5%D1%93%D1%83%20%D1%81%D0%B8%D0%BB%D0%BD%D0%BE%20%D0%BE%D0%BA%D1%81%D0%B8%D0%B4%D0%B0%D1%86%D0%B8%D0%BE%D0%BD%D0%BE%20%D0%B8%20%D1%80%D0%B5%D0%B4%D1%83%D0%BA%D1%86%D0%B8%D0%BE%D0%BD%D0%BE%20%D1%81%D1%80%D0%B5%D0%B4%D1%81%D1%82%D0%B2%D0%BE.webm
studied_by: '[[_Standards/WikiData/WD~electrochemistry,7877]]'
subclass_of: '[[_Standards/WikiData/WD~chemical_reaction,36534]]'
Commons_category: Redox reactions
exact_match: http://biomodels.net/SBO/SBO_0000200
image: http://commons.wikimedia.org/wiki/Special:FilePath/RustyChainEdit1.jpg
UMLS_CUI: C0030012
MeSH_tree_code:
- G02.700
- G03.295.531
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Redox,82682]]'
  - '[[/_Standards/Chemistry/Chemical_Reaction/Redox|Redox]]'
  - '[[/_public/Chemistry/Chemical_Reaction/Redox.public|Redox.public]]'
  - '[[/_internal/Chemistry/Chemical_Reaction/Redox.internal|Redox.internal]]'
  - '[[/_protect/Chemistry/Chemical_Reaction/Redox.protect|Redox.protect]]'
  - '[[/_private/Chemistry/Chemical_Reaction/Redox.private|Redox.private]]'
  - '[[/_personal/Chemistry/Chemical_Reaction/Redox.personal|Redox.personal]]'
  - '[[/_secret/Chemistry/Chemical_Reaction/Redox.secret|Redox.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Redox,82682]]'
- '[[/_Standards/Chemistry/Chemical_Reaction/Redox|Redox]]'
- '[[/_public/Chemistry/Chemical_Reaction/Redox.public|Redox.public]]'
- '[[/_internal/Chemistry/Chemical_Reaction/Redox.internal|Redox.internal]]'
- '[[/_protect/Chemistry/Chemical_Reaction/Redox.protect|Redox.protect]]'
- '[[/_private/Chemistry/Chemical_Reaction/Redox.private|Redox.private]]'
- '[[/_personal/Chemistry/Chemical_Reaction/Redox.personal|Redox.personal]]'
- '[[/_secret/Chemistry/Chemical_Reaction/Redox.secret|Redox.secret]]'
dv_has_:
  name_:
    af: redoksreaksie
    ar: تفاعلات أكسدة-اختزال
    ast: amenorgamientu-oxidación
    az: oksidləşmə-reduksiya reaksiyaları
    ba: окисланыу-ҡайтарылыу реакциялары
    be: акісленне
    be_tarask: затляняльна-аднаўленчая рэакцыя
    bg: окислително-редукционни процеси
    bn: জারণ-বিজারণ
    bs: redoks reakcija
    ca: reacció d'oxidació-reducció
    ckb: ئۆکسان
    cs: redoxní reakce
    cv: оксидлав-таврăнав реакцийĕ
    cy: rhydocs
    da: redoxreaktion
    de: Redoxreaktion
    de_ch: Redoxreaktion
    el: οξειδοαναγωγή
    en: redox
    en_ca: redox
    en_gb: redox
    eo: redoksa reakcio
    es: reducción-oxidación
    et: redoksreaktsioon
    eu: erredox erreakzio
    fa: اکسایش و کاهش
    fi: hapetus-pelkistysreaktio
    fr: réaction d'oxydoréduction
    fy: Redoksreaksje
    ga: ocsdí
    gl: oxidorredución
    he: חמצון-חיזור
    hi: आक्सीकरण
    hr: redoks
    ht: oksidasyon
    hu: redoxireakció
    hy: օքսիդացում և վերականգնում
    ia: redox
    id: redoks
    io: redoxo
    is: oxun
    it: ossidoriduzione
    ja: 酸化還元反応
    ka: ჟანგვა-აღდგენითი რეაქციები
    kk: балқытпа қалдық
    kn: ಉತ್ಕರ್ಷಣ - ಅಪಕರ್ಷಣ
    ko: 산화·환원 반응
    lb: Oxidatiouns-Reaktioun
    lt: oksidacijos-redukcijos reakcija
    lv: oksidēšanās-reducēšanās reakcijas
    lzh: 氧化還原反應
    mk: оксидационо-редукциона реакција
    ml: റിഡോക്സ് പ്രവർത്തനം
    mn: исэлдэлт
    ms: tindak balas redoks
    nan: sng-hòa hoân-goân hoán-èng
    nb: redoksreaksjon
    nl: redoxreactie
    nn: redoksreaksjon
    oc: reaccion d'oxidoreduccion
    pa: ਰਿਡਾਕਸ
    pl: reakcja redoks
    pnb: رڈاکس
    pt: oxirredução
    pt_br: reação redox
    qu: uksiday
    ro: redox
    ru: окислительно-восстановительные реакции
    sd: ريڊوڪس
    sh: oksido-redukcija
    sk: oxidačno-redukčná reakcia
    sl: redoks reakcija
    sq: reaksionet redoks
    sr: оксидо-редукција
    su: ᮇᮊ᮪ᮞᮤᮓᮞᮤ
    sv: redox
    ta: ஒடுக்க ஏற்ற வேதி வினைகள்
    th: ปฏิกิริยารีดอกซ์
    tr: redoks
    tt: оксидлашу-кайтару реакцияләре
    udm: чырсатӥсь-берыктӥсь воштӥськонъёс
    uk: окисно-відновна реакція
    ur: تخسید
    vi: phản ứng oxid hóa khử
    wuu: 氧化还原反应
    yue: 氧化
    zh: 氧化还原反应
    zh_hans: 氧化还原反应
    zh_hant: 氧化還原反應
---

# [[Redox]] 

#is_/same_as :: [[WD~Redox,82682]] 

## #has_/text_of_/abstract 

> **Redox** (RED-oks,  REE-doks, reduction–oxidation or oxidation–reduction) is a type of chemical reaction in which the oxidation states of the reactants change. Oxidation is the loss of electrons or an increase in the oxidation state, while reduction is the gain of electrons or a decrease in the oxidation state. The oxidation and reduction processes occur simultaneously in the chemical reaction.
>
> There are two classes of redox reactions:
> - Atom transfer – An atom transfers from one substrate to another.  
>   In hydrogenation, bonds like C=C  are reduced by transfer of hydrogen atoms.
> - Electron-transfer – Only one (usually) electron flows 
>   from the atom, ion, or molecule being oxidized 
>   to the atom, ion, or molecule that is reduced. 
>   This type of redox reaction is often discussed in terms of redox couples and electrode potentials.
> 
> For example, in the rusting of iron, the oxidation state of iron atoms increases as the iron converts to an oxide, and simultaneously, the oxidation state of oxygen decreases as it accepts electrons released by the iron. 
> 
> Although oxidation reactions are commonly associated with forming oxides, 
> other chemical species can serve the same function. 
>
> [Wikipedia](https://en.wikipedia.org/wiki/Redox) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Chemical_Reaction/Redox|Redox]] 

### #is_/same_as :: [[/_public/Chemistry/Chemical_Reaction/Redox.public|Redox.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Chemical_Reaction/Redox.internal|Redox.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Chemical_Reaction/Redox.protect|Redox.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Chemical_Reaction/Redox.private|Redox.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Chemical_Reaction/Redox.personal|Redox.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Chemical_Reaction/Redox.secret|Redox.secret]] 

