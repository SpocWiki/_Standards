---
aliases:
- beşa kîmyayê
- brainse den cheimic
- branca de la química
- branca de ła chìmega
- branca del chimia
- branca della chimica
- branch of chemistry
- branche de la chimie
- cabang kimia
- cangen o fewn cemeg
- Deelgebitt vun der Chimie
- disciplina da química
- grein av kjemien
- gren af kemi
- gren av kjemien
- kemia sueʹrǧǧ
- kemia syergi
- kemian haara
- kemigren
- kemiija suorgi
- kimikaren adar
- kimya dalı
- kémia ága
- nauki chemiczne
- obor chemie
- področje kemije
- rama de la química
- ramo da química
- skorren kymygieth
- skourr ar gimiezh
- studfako de ĥemio
- tak van de chemie
- Teilgebiet der Chemie
- ķīmijas nozare
- κλάδος της χημείας
- гранка на хемијата
- област хемије
- раздел химии
- раздзел хіміі
- разьдзел хіміі
- розділ хімії
- химийы къабаз
- химия бүлеге
- քիմիայի բաժին
- תחום בכימיה
- فروع الكيمياء
- لقی کیمیا
- چابڠ کيميا
- کیمیا کی شاخ
- রসায়নের শাখা
- ರಸಾಯನಶಾಸ್ತ್ರದ ಶಾಖೆ
- ქიმიაშ გჷმნართი
- 化学の分野
- 化学分支领域
- 化學分支領域
- 화학 분야
has_id_wikidata: Q11790203
subclass_of: '[[_Standards/WikiData/WD~branch_of_science,2465832|WD~branch_of_science,2465832]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Branches_of_chemistry,8085158|WD~Template_Branches_of_chemistry,8085158]]'
instance_of: '[[_Standards/WikiData/WD~second_order_class,24017414|WD~second_order_class,24017414]]'
facet_of: '[[_Standards/WikiData/WD~chemistry,2329|WD~chemistry,2329]]'
part_of: '[[_Standards/WikiData/WD~chemistry,2329|WD~chemistry,2329]]'
described_at_URL: https://www.chemie.de/lexikon/Kategorie:Teilgebiet_der_Chemie.html
dv_is_:
  same_as:
  - '[[_Standards/WikiData/WD~branch_of_chemistry,11790203|WD~branch_of_chemistry,11790203]]'
  - '[[/_Standards/Chemistry/Branch_of_Chemistry|Branch_of_Chemistry]]'
  - '[[/_public/Chemistry/Branch_of_Chemistry.public|Branch_of_Chemistry.public]]'
  - '[[/_internal/Chemistry/Branch_of_Chemistry.internal|Branch_of_Chemistry.internal]]'
  - '[[/_protect/Chemistry/Branch_of_Chemistry.protect|Branch_of_Chemistry.protect]]'
  - '[[/_private/Chemistry/Branch_of_Chemistry.private|Branch_of_Chemistry.private]]'
  - '[[/_personal/Chemistry/Branch_of_Chemistry.personal|Branch_of_Chemistry.personal]]'
  - '[[/_secret/Chemistry/Branch_of_Chemistry.secret|Branch_of_Chemistry.secret]]'
dv_is_same_as:
- '[[_Standards/WikiData/WD~branch_of_chemistry,11790203|WD~branch_of_chemistry,11790203]]'
- '[[/_Standards/Chemistry/Branch_of_Chemistry|Branch_of_Chemistry]]'
- '[[/_public/Chemistry/Branch_of_Chemistry.public|Branch_of_Chemistry.public]]'
- '[[/_internal/Chemistry/Branch_of_Chemistry.internal|Branch_of_Chemistry.internal]]'
- '[[/_protect/Chemistry/Branch_of_Chemistry.protect|Branch_of_Chemistry.protect]]'
- '[[/_private/Chemistry/Branch_of_Chemistry.private|Branch_of_Chemistry.private]]'
- '[[/_personal/Chemistry/Branch_of_Chemistry.personal|Branch_of_Chemistry.personal]]'
- '[[/_secret/Chemistry/Branch_of_Chemistry.secret|Branch_of_Chemistry.secret]]'
dv_has_:
  name_:
    ar: فروع الكيمياء
    be: раздзел хіміі
    be_tarask: разьдзел хіміі
    bn: রসায়নের শাখা
    br: skourr ar gimiezh
    ca: branca de la química
    ckb: لقی کیمیا
    cs: obor chemie
    cy: cangen o fewn cemeg
    da: gren af kemi
    de: Teilgebiet der Chemie
    el: κλάδος της χημείας
    en: branch of chemistry
    eo: studfako de ĥemio
    es: rama de la química
    eu: kimikaren adar
    fi: kemian haara
    fr: branche de la chimie
    ga: brainse den cheimic
    gl: disciplina da química
    he: תחום בכימיה
    hu: kémia ága
    hy: քիմիայի բաժին
    ia: branca del chimia
    it: branca della chimica
    ja: 化学の分野
    kn: ರಸಾಯನಶಾಸ್ತ್ರದ ಶಾಖೆ
    ko: 화학 분야
    ku: beşa kîmyayê
    ku_latn: beşa kîmyayê
    kw: skorren kymygieth
    lb: Deelgebitt vun der Chimie
    lv: ķīmijas nozare
    mk: гранка на хемијата
    ms: cabang kimia
    ms_arab: چابڠ کيميا
    nb: gren av kjemien
    nl: tak van de chemie
    nn: grein av kjemien
    os: химийы къабаз
    pl: nauki chemiczne
    pt: ramo da química
    pt_br: ramo da química
    ru: раздел химии
    se: kemiija suorgi
    sl: področje kemije
    smn: kemia syergi
    sms: kemia sueʹrǧǧ
    sr: област хемије
    sv: kemigren
    tr: kimya dalı
    tt: химия бүлеге
    uk: розділ хімії
    ur: کیمیا کی شاخ
    vec: branca de ła chìmega
    xmf: ქიმიაშ გჷმნართი
    zh: 化学分支领域
    zh_cn: 化学分支领域
    zh_hant: 化學分支領域
    zh_tw: 化學分支領域
---

# [[Branch_of_Chemistry]] 

#is_/same_as :: [[_Standards/WikiData/WD~branch_of_chemistry,11790203|WD~branch_of_chemistry,11790203]] 

Just like [[../Chemistry|Chemistry]] is a Branch of [[../Science|Science]], its growing Size led to a split into different Specializations. 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Chemistry/Branch_of_Chemistry|Branch_of_Chemistry]] 

### #is_/same_as :: [[/_public/Chemistry/Branch_of_Chemistry.public|Branch_of_Chemistry.public]] 

### #is_/same_as :: [[/_internal/Chemistry/Branch_of_Chemistry.internal|Branch_of_Chemistry.internal]] 

### #is_/same_as :: [[/_protect/Chemistry/Branch_of_Chemistry.protect|Branch_of_Chemistry.protect]] 

### #is_/same_as :: [[/_private/Chemistry/Branch_of_Chemistry.private|Branch_of_Chemistry.private]] 

### #is_/same_as :: [[/_personal/Chemistry/Branch_of_Chemistry.personal|Branch_of_Chemistry.personal]] 

### #is_/same_as :: [[/_secret/Chemistry/Branch_of_Chemistry.secret|Branch_of_Chemistry.secret]] 

