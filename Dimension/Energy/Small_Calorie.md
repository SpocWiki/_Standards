---
aliases:
- Ca-lo
- Caloree
- Calori
- caloria
- caloria(s)
- calorie
- Calorìa
- Caloría
- calra
- Caluria
- Gă-lô-lī
- Hitaeining
- hoo'illaa
- Kalor
- kalori
- Kaloria
- kalorie
- Kalorii
- Kalorija
- kalorio
- kaloriya
- kalorïya
- kalória
- Khah-lō͘-lí
- small calorie
- θερμίδα
- калёрыя
- калорія
- Калорија
- калория
- калорыя
- կալորիա
- קאלערי
- קלוריה
- سعرة
- كالورىييا
- کالری
- کالۆری
- कैलोरी
- ক্যালরি
- ਕੈਲੋਰੀ
- கலோரி
- కెలోరి
- ಕೆಲೊರಿ
- แคลอรี
- ཁཱ།
- ကယ်လိုရီ
- ခယ်လိုရီ
- カロリー
- 卡路里
- 칼로리
has_id_wikidata: Q130964
instance_of:
- '[[_Standards/WikiData/WD~unit_of_energy,2916980]]'
- '[[_Standards/WikiData/WD~UCUM_derived_unit,82047057]]'
different_from:
- '[[_Standards/WikiData/WD~kilocalorie,25668616]]'
- '[[_Standards/WikiData/WD~calorie,87260855]]'
- '[[_Standards/WikiData/WD~mean_calorie,93811434]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632]]'
measured_physical_quantity: '[[_Standards/WikiData/WD~energy,11379]]'
conversion_to_SI_unit: 4.184
UCUM_code:
- cal
- cal_th
OmegaWiki_Defined_Meaning: 333116
UN_CEFACT_Common_Code: D35
unit_symbol:
- кал
- cal
- kal.
image: http://commons.wikimedia.org/wiki/Special:FilePath/Nutrition-label.jpg
Wolfram_Language_unit_code: '"ThermochemicalCalories"'
Wikidata_SPARQL_query_equivalent: wd:Q130964 p:P2370/psn:P2370 [wikibase:quantityAmount ?source; wikibase:quantityUnit ?base]. ?item p:P2370/psn:P2370 [wikibase:quantityAmount ?target; wikibase:quantityUnit ?base]. BIND(?source / ?target as ?value)
dv_has_:
  name_:
    ar: سعرة
    ast: Caloría
    az: Kalori
    be: калорыя
    be_tarask: калёрыя
    bg: Калория
    blk: ခယ်လိုရီ
    bn: ক্যালরি
    bo: ཁཱ།
    bs: Kalorija
    ca: caloria
    cdo: Gă-lô-lī
    ckb: کالۆری
    cs: kalorie
    cy: Calori
    da: kalorie
    de: Kalorie
    el: θερμίδα
    en: small calorie
    eo: kalorio
    es: caloría
    et: Kalor
    eu: Kaloria
    fa: کالری
    fi: Kalori
    fr: calorie
    frr: Kalorii
    ga: calra
    gl: Caloría
    gv: Caloree
    he: קלוריה
    hi: कैलोरी
    hr: Kalorija
    ht: Kalori
    hu: kalória
    hy: կալորիա
    id: Kalori
    io: Kalorio
    is: Hitaeining
    it: caloria
    ja: カロリー
    kk: Калория
    kk_arab: كالورىييا
    kk-cn: كالورىييا
    kk_cyrl: калория
    kk-kz: Калория
    kk_latn: kalorïya
    kk-tr: Kalorïya
    kn: ಕೆಲೊರಿ
    ko: 칼로리
    ky: Калория
    lb: Kalorie
    lmo: Caloria
    lt: Kalorija
    lv: kalorija
    mk: Калорија
    ms: Kalori
    my: ကယ်လိုရီ
    nan: Khah-lō͘-lí
    nb: kalori
    nl: calorie
    nn: kalori
    oc: Caloria
    om: hoo'illaa
    pa: ਕੈਲੋਰੀ
    pl: kaloria
    pms: Calorìa
    pt: caloria
    pt_br: caloria(s)
    ro: calorie
    ru: калория
    scn: Caluria
    sh: Kalorija
    sk: kalória
    sl: kalorija
    sq: Kaloria
    sr: калорија
    sr_ec: калорија
    sr_el: kalorija
    su: Kalori
    sv: kalori
    ta: கலோரி
    te: కెలోరి
    th: แคลอรี
    tl: kaloriya
    tr: kalori
    udm: калория
    uk: калорія
    uz: Kaloriya
    vi: Ca-lo
    wuu: 卡路里
    yi: קאלערי
    yue: 卡路里
    zh: 卡路里
    zh_cn: 卡路里
    zh_hans: 卡路里
    zh_hant: 卡路里
    zh_hk: 卡路里
    zh_sg: 卡路里
    zh_tw: 卡路里
---
# [[Small_Calorie]] 

#is_/same_as :: [[../../WikiData/WD~Small_calorie,130964|WD~Small_calorie,130964]]  

## #has_/text_of_/abstract 

> The calorie is a unit of energy that originated from the caloric theory of heat. 
> The large calorie, food calorie, dietary calorie,  or kilocalorie 
> is defined as the amount of heat needed to raise the temperature of one liter of water 
> by one degree Celsius (or one kelvin). 
> 
> The small calorie or gram calorie is defined as the amount of heat needed 
> to cause the same 1°K increase in one milliliter of water. 
> Thus, 1 large calorie is equal to 1,000 small calories.
>
> In nutrition and food science, the term calorie and the symbol cal 
> may refer to the large unit or to the small unit in different regions of the world. 
> 
> It is generally used in publications and package labels 
> to express the energy value of foods in per serving or per weight, 
> recommended dietary caloric intake, metabolic rates, etc. 
> 
> Some authors recommend the spelling Calorie and the symbol Cal (both with a capital C) 
> if the large calorie is meant, to avoid confusion; however, this convention is often ignored.
>
> In physics and chemistry, the word calorie and its symbol usually refer to the small unit, the large one being called kilocalorie (kcal). However, the kcal is not officially part of the International System of Units (SI), and is regarded as obsolete, having been replaced in many uses by the SI derived unit of energy, the joule (J), or the kilojoule (kJ) for 1000 joules.
>
> The precise equivalence between calories and joules has varied over the years, but in thermochemistry and nutrition it is now generally assumed that one (small) calorie (thermochemical calorie) is equal to exactly 4.184 J, and therefore one kilocalorie (one large calorie) is 4184 J or 4.184 kJ.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Calorie) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Dimension/Energy/Small_Calorie|Small_Calorie]] 

### #is_/same_as :: [[/_public/Dimension/Energy/Small_Calorie.public|Small_Calorie.public]] 

### #is_/same_as :: [[/_internal/Dimension/Energy/Small_Calorie.internal|Small_Calorie.internal]] 

### #is_/same_as :: [[/_protect/Dimension/Energy/Small_Calorie.protect|Small_Calorie.protect]] 

### #is_/same_as :: [[/_private/Dimension/Energy/Small_Calorie.private|Small_Calorie.private]] 

### #is_/same_as :: [[/_personal/Dimension/Energy/Small_Calorie.personal|Small_Calorie.personal]] 

### #is_/same_as :: [[/_secret/Dimension/Energy/Small_Calorie.secret|Small_Calorie.secret]] 

