---
aliases:
- batu
- dặm Anh
- eng-lí
- engelsk mil
- international mile
- jūdze
- Maali
- mail
- maili
- maily
- Mayl
- Meil
- Meile
- mejlo
- Miele
- migghiu
- miglio
- miil
- mij
- mijl
- mil
- mila
- mile
- milha
- milia
- Milja
- milla
- mille
- Mille passus
- milltir
- milya
- milă
- myet
- myl
- mylia
- máìlì
- mérföld
- mìlia
- míl
- míla
- míle
- míľa
- statute mile
- Unit~Mile
- Ĭng-lī
- μίλι
- Бээр
- міля
- миља
- милја
- миль
- миля
- մղոն
- מייל
- מיל
- باتو
- مایل
- ميل
- میل
- میل (اؤلچو بیریمی)
- میٖل
- މޭލު
- मील
- मैल
- মাইল
- ਮੀਲ
- માઇલ
- மைல்
- మైలు
- ಮೈಲಿ
- മൈൽ
- සැතපුම
- ไมล์
- တိုင်
- မိုင်
- მილი
- მილი (ართული)
- ማይል
- マイル
- 哩
- 英哩
- 英里
- 마일
has_id_wikidata: Q253276
is_part_of: '[[_Standards/WikiData/WD~Russian_system_of_units,435964|WD~Russian_system_of_units,435964]]'
different_from:
- '[[_Standards/WikiData/WD~Mell,1197183|WD~Mell,1197183]]'
- '[[_Standards/WikiData/WD~1593_English_statute_mile,2296171|WD~1593_English_statute_mile,2296171]]'
- '[[_Standards/WikiData/WD~mile_(US_survey),61749562|WD~mile_(US_survey),61749562]]'
- '[[_Standards/WikiData/WD~international_mile,61750260|WD~international_mile,61750260]]'
instance_of:
- '[[_Standards/WikiData/WD~unit_of_length,1978718|WD~unit_of_length,1978718]]'
- '[[_Standards/WikiData/WD~English_unit_of_measurement,16782623|WD~English_unit_of_measurement,16782623]]'
described_by_source:
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718|WD~Armenian_Soviet_Encyclopedia,2657718]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~ISO_80000_3_2006_Quantities_and_units_Part_3_Space_and_time,26711932|WD~ISO_80000_3_2006_Quantities_and_units_Part_3_Space_and_time,26711932]]'
conversion_to_standard_unit:
- 80
- 8
- 1760
- 5280
- 63360000000
- 63360
- 0.28966
- 0.33313
- 0.33333
- 0.86898
- 0.868421
- 1609344000
- 1609344
- 160934.4
- 16093.44
- 1609.344
- 1.609344
- 1.701078e-13
- 0.2894737
- 5.215529e-14
measured_physical_quantity: '[[_Standards/WikiData/WD~length,36253|WD~length,36253]]'
UN_CEFACT_Common_Code: SMI
conversion_to_SI_unit: 1609.344
unit_symbol:
- مایل
- миль
- mi
- മൈൽ
- miglia
- ميل
- mil
- mil
- মাইল
- mejloj
- miles
- millas
- mérföld
- милји
- mil
- milhas
- mile
- миль
- mìlia
- 英里
image: http://commons.wikimedia.org/wiki/Special:FilePath/Thimphu%20and%20Watsa%20milestone.jpg
Wolfram_Language_unit_code: '"Miles"'
Wikidata_SPARQL_query_equivalent: wd:Q253276 p:P2370/psn:P2370 [wikibase:quantityAmount ?source; wikibase:quantityUnit ?base]. ?item p:P2370/psn:P2370 [wikibase:quantityAmount ?target; wikibase:quantityUnit ?base]. BIND(?source / ?target as ?value)
dv_has_:
  name_:
    af: myl
    am: ማይል
    ar: ميل
    ast: milla
    az: mil
    azb: میل (اؤلچو بیریمی)
    ba: миль
    be: міля
    be_tarask: міля
    bg: миля
    bn: মাইল
    bs: milja
    ca: milla
    cdo: Ĭng-lī
    ckb: میل
    cs: míle
    cy: milltir
    da: mil
    dag: Maali
    de: Meile
    dv: މޭލު
    el: μίλι
    en: mile
    en_ca: mile
    en_gb: mile
    eo: mejlo
    es: milla
    et: miil
    eu: milia
    fa: مایل
    fi: maili
    fo: míl
    fr: mille
    frr: miil
    fy: myl
    ga: míle
    gl: milla
    gu: માઇલ
    ha: mil
    he: מיל
    hi: मील
    hr: milja
    hu: mérföld
    hy: մղոն
    id: mil
    ilo: milia
    is: míla
    it: miglio
    ja: マイル
    jam: mail
    jv: mil
    ka: მილი
    kcg: myet
    kk: миля
    kn: ಮೈಲಿ
    ko: 마일
    ks: میٖل
    la: Mille passus
    lb: Meil
    li: mijl
    lt: mylia
    lv: jūdze
    mg: maily
    mk: милја
    ml: മൈൽ
    mn: Бээр
    mnw: တိုင်
    mr: मैल
    ms: batu
    ms_arab: باتو
    mt: mil
    my: မိုင်
    nan: eng-lí
    nb: engelsk mil
    nl: mijl
    nn: mile
    pa: ਮੀਲ
    pl: mila
    pms: mij
    pnb: میل
    ps: مایل
    pt: milha
    pt_br: milha
    ro: milă
    ru: миля
    scn: migghiu
    sco: mile
    sh: milja
    si: සැතපුම
    sk: míľa
    sl: milja
    so: Mayl
    sq: Milja
    sr: миља
    stq: Miele
    sv: engelsk mil
    sw: maili
    ta: மைல்
    te: మైలు
    th: ไมล์
    tr: mil
    uk: миля
    ur: میل
    uz: milya
    vec: mìlia
    vi: dặm Anh
    vro: miil
    war: milya
    wuu: 英里
    xmf: მილი (ართული)
    yi: מייל
    yo: máìlì
    yue: 哩
    zh: 英里
    zh_cn: 英里
    zh_hans: 英里
    zh_hant: 英里
    zh_hk: 哩
    zh_mo: 哩
    zh-my: 哩
    zh_sg: 哩
    zh_tw: 英哩
---

# [[Unit~Mile]] 

#is_/same_as :: [[_Standards/WikiData/WD~Mile,253276]] 

## #has_/text_of_/abstract 

> The **Mile**, sometimes the international mile or statute mile to distinguish it from other miles, 
> is a British imperial unit and United States customary unit of length; 
> both are based on the older English unit of length equal to 5,280 English feet, or 1,760 yards. 
> 
> The statute mile was standardised between the Commonwealth of Nations and the United States 
> by an international agreement in 1959, when the yard was formally redefined with respect to SI units as 0.9144 metres, making the mile exactly 1609.344 metres (1.609344 kilometres). For everyday use, five miles equates roughly to eight kilometres.
>
> With qualifiers, mile is also used to describe or translate a wide range of units derived from or roughly equivalent to the Roman mile (roughly 1.48 km), such as the nautical mile (now 1.852 km exactly), the Italian mile (roughly 1.852 km), and the Chinese mile (now 500 m exactly). The Romans divided their mile into 5,000 pedes (lit. 'feet'), but the greater importance of furlongs in the Elizabethan-era England meant that the statute mile was made equivalent to 8 furlongs or 5,280 feet in 1593. This form of the mile then spread across the British Empire, some successor states of which continue to employ the mile. The US Geological Survey now employs the metre for official purposes, but legacy data from its 1927 geodetic datum has meant that a separate US survey mile (⁠6336/3937⁠ km) continues to see some use, although it was officially phased out in 2022. While most countries replaced the mile with the kilometre when switching to the International System of Units (SI), the international mile continues to be used in some countries, such as the United Kingdom, the United States, and a number of countries with fewer than one million inhabitants, most of which are UK or US territories or have close historical ties with the UK or US.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Mile) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[Unit~Mile|Unit~Mile]] 

### #is_/same_as :: [[/_public/Dimension/Dim~Length/Unit~Mile.public|Unit~Mile.public]] 

### #is_/same_as :: [[/_internal/Dimension/Dim~Length/Unit~Mile.internal|Unit~Mile.internal]] 

### #is_/same_as :: [[/_protect/Dimension/Dim~Length/Unit~Mile.protect|Unit~Mile.protect]] 

### #is_/same_as :: [[/_private/Dimension/Dim~Length/Unit~Mile.private|Unit~Mile.private]] 

### #is_/same_as :: [[/_personal/Dimension/Dim~Length/Unit~Mile.personal|Unit~Mile.personal]] 

### #is_/same_as :: [[/_secret/Dimension/Dim~Length/Unit~Mile.secret|Unit~Mile.secret]] 

