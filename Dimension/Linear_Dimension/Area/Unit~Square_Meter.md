---
aliases:
- fermetri
- fjouwerkante meter
- kuadrado metro
- kvadrat metr
- kvadrata metro
- kvadratinis metras
- kvadratmeter
- kvadratni metar
- kvadratni meter
- kvadratėnis metros
- kvadrātmetrs
- Kwadraatmeter
- kwadrootmeeter
- mete cwåré
- meter kuadra
- meter persegi
- meter quadrad
- metr karrez
- metr kwadratowy
- metr sgwâr
- metr čtvereční
- metre quadrat
- metre çargoşe
- metrekare
- metro cadrado
- metro cuadrado
- metro cuadro
- metro koadro
- metro quadrado
- metro quadrate
- metro quadrato
- metro(s) quadrado(s)
- metru cuadrau
- metru cuadráu
- metru kwadru
- metru pătrat
- metru quairau
- metru quatratu
- metrum quadratum
- metër katror
- mita ya mraba
- mitir labajibbaaran
- m²
- mètre carrat
- mètre carré
- méadar cearnach
- mét vuông
- méter pasagi
- neliömetri
- négyzetméter
- phiàng-fông kûng-tshak
- pêng-hong-kong-chhioh
- Quadratmeta
- Quadratmeter
- ruutmeeter
- skweya remita
- square meter
- square metre
- square mitar
- veerkante maeter
- vierkante meter
- ìlọ́poméjì mítà
- četvorni metar
- štvorcový meter
- τετραγωνικό μέτρο
- аквадраттә метр
- квадрат метр
- квадратан метр
- квадратен метар
- квадратен метър
- квадратни метар
- квадратний метр
- квадратны метр
- квадратны мэтар
- квадратный метр
- квадратон метр
- километри мураббаъ
- метри мураббаъ
- тăваткал метр
- шаршы метр
- քառակուսի մետր
- מטר רבוע
- متر مربع
- متر موربع
- متره مربع
- مربع میٹر
- مەتری چوارگۆشە
- घन मीटर
- वर्ग मीटर
- বর্গমিটার
- சதுர மீட்டர்
- చదరపు మీటరు
- ചതുരശ്ര മീറ്റർ
- වර්ග මීටර්
- ตารางเมตร
- စတုရန်း မီတာ
- კვადრატული მეტრი
- ម៉ែត្រការ៉េ
- ㎡
- 平方メートル
- 平方公尺
- 平方米
- 제곱미터
has_id_wikidata: Q25343
instance_of:
- '[[_Standards/WikiData/WD~unit_of_area,1371562]]'
- '[[_Standards/WikiData/WD~coherent_SI_unit,69197847]]'
described_by_source:
- "[[_Standards/WikiData/WD~Otto's_encyclopedia,2041543]]"
- '[[_Standards/WikiData/WD~ISO_80000_3_2006_Quantities_and_units_Part_3_Space_and_time,26711932]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~m_,6997234]]'
permanent_duplicated_item: '[[_Standards/WikiData/WD~Q25930092,25930092]]'
measured_physical_quantity:
- '[[_Standards/WikiData/WD~nuclear_quadrupole_moment,97921226]]'
- '[[_Standards/WikiData/WD~direction_distribution_of_cross_section,98266630]]'
- '[[_Standards/WikiData/WD~atomic_attenuation_coefficient,98592911]]'
- '[[_Standards/WikiData/WD~area,11500]]'
derived_from_base_unit: '[[_Standards/WikiData/WD~metre,11573]]'
conversion_to_standard_unit:
- 1000000
- 10000
- 0.0001
- 0.000001
- 1550
- 1.19599
- 10.76391
- 1550031000000000
- 0.0002471054
- 3.861022e-7
conversion_to_SI_unit: 1
unit_symbol:
- מ"ר
- م²
- m²
- m²
- м²
- m²
- m²
- m²
- m²
- m²
- m²
- m²
- m²
- ച.മീ
- м²
- m²
- м²
- m²
- м²
- m²
UCUM_code: m2
UN_CEFACT_Common_Code: MTK
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/M%C3%A8tre%20carr%C3%A9.png
image: http://commons.wikimedia.org/wiki/Special:FilePath/SquareMeterQuadrat.png
Wolfram_Language_unit_code: '"Meters"^2'
Unicode_character: ㎡
Commons_category: Square metre
Wikidata_SPARQL_query_equivalent: wd:Q25343 p:P2370/psn:P2370 [wikibase:quantityAmount ?source; wikibase:quantityUnit ?base]. ?item p:P2370/psn:P2370 [wikibase:quantityAmount ?target; wikibase:quantityUnit ?base]. BIND(?source / ?target as ?value)
dv_has_:
  name_:
    ab: аквадраттә метр
    af: vierkante meter
    ar: متر مربع
    arz: متر مربع
    ast: metru cuadráu
    az: kvadrat metr
    azb: متر موربع
    ba: квадрат метр
    bar: Quadratmeta
    be: квадратны метр
    be_tarask: квадратны мэтар
    bg: квадратен метър
    bn: বর্গমিটার
    br: metr karrez
    bs: kvadratni metar
    ca: metre quadrat
    ce: квадратан метр
    ckb: مەتری چوارگۆشە
    crh: kvadrat metr
    cs: metr čtvereční
    cv: тăваткал метр
    cy: metr sgwâr
    da: kvadratmeter
    de: Quadratmeter
    el: τετραγωνικό μέτρο
    en: square metre
    en_ca: square metre
    en_gb: square metre
    en-us: square meter
    eo: kvadrata metro
    es: metro cuadrado
    et: ruutmeeter
    eu: metro koadro
    ext: metru quairau
    fa: متر مربع
    fi: neliömetri
    fr: mètre carré
    frr: kwadrootmeeter
    fy: fjouwerkante meter
    ga: méadar cearnach
    gl: metro cadrado
    gsw: Quadratmeter
    ha: square mitar
    hak: phiàng-fông kûng-tshak
    he: מטר רבוע
    hi: घन मीटर
    hr: četvorni metar
    hu: négyzetméter
    hy: քառակուսի մետր
    ia: metro quadrate
    id: meter persegi
    ilo: kuadrado metro
    is: fermetri
    it: metro quadrato
    ja: 平方メートル
    ka: კვადრატული მეტრი
    kk: шаршы метр
    km: ម៉ែត្រការ៉េ
    ko: 제곱미터
    ksh: Kwadraatmeter
    ku: metre çargoşe
    ku_latn: metre çargoşe
    la: metrum quadratum
    lb: Quadratmeter
    li: veerkante maeter
    lmo: meter quadrad
    lt: kvadratinis metras
    lv: kvadrātmetrs
    mk: квадратен метар
    ml: ചതുരശ്ര മീറ്റർ
    mr: वर्ग मीटर
    ms: meter persegi
    mt: metru kwadru
    mul: ㎡
    my: စတုရန်း မီတာ
    nan: pêng-hong-kong-chhioh
    nb: kvadratmeter
    nds: Quadratmeter
    nl: vierkante meter
    nn: kvadratmeter
    oc: mètre carrat
    os: квадратон метр
    pap: meter kuadra
    pl: metr kwadratowy
    pnb: مربع میٹر
    ps: متره مربع
    pt: metro quadrado
    pt_br: metro(s) quadrado(s)
    rm: m²
    ro: metru pătrat
    ru: квадратный метр
    sc: metru cuadrau
    scn: metru quatratu
    sgs: kvadratėnis metros
    sh: kvadratni metar
    si: වර්ග මීටර්
    sk: štvorcový meter
    sl: kvadratni meter
    sn: skweya remita
    so: mitir labajibbaaran
    sq: metër katror
    sr: квадратни метар
    sr_ec: квадратни метар
    su: méter pasagi
    sv: kvadratmeter
    sw: mita ya mraba
    ta: சதுர மீட்டர்
    te: చదరపు మీటరు
    tg: метри мураббаъ
    tg_cyrl: километри мураббаъ
    th: ตารางเมตร
    tr: metrekare
    tt: квадрат метр
    tt_cyrl: квадрат метр
    uk: квадратний метр
    ur: مربع میٹر
    uz: kvadrat metr
    vec: metro cuadro
    vi: mét vuông
    wa: mete cwåré
    wuu: 平方米
    yo: ìlọ́poméjì mítà
    yue: 平方米
    zh: 平方米
    zh_cn: 平方米
    zh_hans: 平方米
    zh_hant: 平方公尺
    zh_hk: 平方米
    zh_mo: 平方米
    zh-my: 平方米
    zh_sg: 平方米
    zh_tw: 平方公尺
---

# [[Unit~Square_Meter]] ㎡ 


#is_/same_as :: [[../../../WikiData/WD~Square_metre,25343|WD~Square_metre,25343]]  

## #has_/text_of_/abstract 

> The square metre (international spelling by the International Bureau of Weights and Measures) 
> or square meter (American spelling) is the unit of area in the International System of Units (SI) 
> with symbol m². 
> It is the area of a square with sides one metre in length.
>
> Adding and subtracting SI prefixes creates multiples and submultiples; 
> however, as the unit is exponentiated, 
> the quantities grow exponentially by the corresponding power of 10. 
> For example, 1 kilometre is 10³ (one thousand) times the length of 1 metre, 
> but 1 square kilometre is (10³)² (10^6, one million) times the area of 1 square metre, 
> and 1 cubic kilometre is (103)3 (109, one billion) cubic metres.
>
> Its inverse is the reciprocal square metre (m^-2), often called "per square metre".
>
> [Wikipedia](https://en.wikipedia.org/wiki/Square%20metre) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Dimension/Linear_Dimension/Area/Unit~Square_Meter|Unit~Square_Meter]] 

### #is_/same_as :: [[/_public/Dimension/Linear_Dimension/Area/Unit~Square_Meter.public|Unit~Square_Meter.public]] 

### #is_/same_as :: [[/_internal/Dimension/Linear_Dimension/Area/Unit~Square_Meter.internal|Unit~Square_Meter.internal]] 

### #is_/same_as :: [[/_protect/Dimension/Linear_Dimension/Area/Unit~Square_Meter.protect|Unit~Square_Meter.protect]] 

### #is_/same_as :: [[/_private/Dimension/Linear_Dimension/Area/Unit~Square_Meter.private|Unit~Square_Meter.private]] 

### #is_/same_as :: [[/_personal/Dimension/Linear_Dimension/Area/Unit~Square_Meter.personal|Unit~Square_Meter.personal]] 

### #is_/same_as :: [[/_secret/Dimension/Linear_Dimension/Area/Unit~Square_Meter.secret|Unit~Square_Meter.secret]] 

