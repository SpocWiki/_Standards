---
aliases:
- Agbègbè àkókò
- Aiguvyöhyke
- aigvö
- aikavyöhyke
- ajavöönd
- Akud adigan
- amchrios
- Aovüü
- Ataran waktu
- beşa saetê
- Casowa cona
- Coisse d' eure
- Cryss hraa
- cylchfa amser
- Czasowo strefa
- estaya horaria
- Faritr'ora
- Feusé horaire
- fus horari
- Fus orar
- fus orari
- fuseau horaire
- Fuso horari
- fuso horario
- fuso horário
- Fuso orario
- fusu urariu
- Fûs orari
- fûzo oràrio
- Fûş orâri
- Füs urari
- geldialdi-ordutegi
- Godina sa'aatii
- horala zono
- horzono
- huso horario
- időzóna
- Ijet lag
- Indawo yesikhathi
- Izdi imiran
- Kanda muda
- laika josla
- laiko juosta
- lokacin yanki
- ma tenpo
- Mukaba ya ntangu
- múi giờ
- Pacha suyu
- parth dermyn
- rann-eur
- roinn-tìde
- saat dilimi
- saat qurşağı
- saat quşağı
- sizgiz
- Sona kan oras
- sona ng oras
- sona ti oras
- strefa czasowa
- Sî-khu
- Sṳ̀-khî
- tempe-sone
- tidjsoon
- tidssone
- tidszon
- tidszone
- Tiedzaone
- Tiedzone
- tiemsoen
- Tietrebeet
- tiidsône
- tijdzone
- time zone
- tydsone
- tímabelti
- Tíðarsonur
- Vaqt mintaqasi
- Vremenska zona
- Waktu banua
- Waqıt zonası
- Wewengkon galah
- Yvy Árapehẽ
- Zeitzone
- Ziitzone
- zon waktu
- Zona d'orar
- zona di tempu
- Zona hin oras
- Zona horaria
- zona kohore
- Zona temporalis
- Zona wakatu
- Zona waktu
- Zona wanci
- Zona wektu
- Zäitzon
- Zòna oraria
- áigeavádat
- äigistielâs
- äiʹǧǧvuʹvdd
- časovni pas
- časové pásmo
- Časowe pasmo
- Čiesa zuona
- żona tal-ħin
- ζώνη ώρας
- Асааҭтә зонақәа
- Вăхăт тăрăхĕ
- временска зона
- дырлэн зонаез
- Кэм зоната
- Минтақаҳои замонӣ
- пингонь каркс
- Сагъат бёлге
- СагӀтил рачел
- Сахьата оаса
- Сахьтан аса
- Сыхьэт зонэ
- Сятдин чӀул
- сәгать поясы
- Сәғәт бүлкәте
- Уақыт белдеуі
- Убакыт алкагы
- Цагийн бүс
- Час кытш
- часавы пас
- часавы пояс
- Часова зона
- часовен појас
- часовий пояс
- часовой пояс
- шагат ӱштӧ
- Үени хемчээри
- Ժամային գոտի
- אזור זמן
- צייט זאנע
- زون وقتو
- سیمه ییز وخت
- مجال زمنى
- منطقة زمانية
- منطقة زمنية
- منطقه زمانی
- منطقۂ وقت
- ناوچەی کاتی
- وقت پٽو
- ویلا تھاں
- چاغ بؤلگهسی
- کال مَنڈَل
- आंतरराष्ट्रीय कालविभाग
- टाइम जोन
- समय क्षेत्र
- समय मण्डल
- समयवलयः
- সময় অঞ্চল
- সময়র লয়া
- ਸਮਾਂ ਖੇਤਰ
- સમય વિસ્તાર
- ସମୟ ମଣ୍ଡଳ
- நேர வலயம்
- కాల మండలం
- ಸಮಯ ವಲಯ
- സമയമേഖല
- වේලා කලාපය
- เขตเวลา
- ເຂດເວລາ
- အခိန်ႏအွိုင်ꩻ
- အချိန်ဇုန်
- ၶၢဝ်းယၢမ်းၼႃႈလိၼ်
- ოსაათე ორტყაფუ
- სასაათო სარტყელი
- ሰዓት ክልል
- ᎠᏟᏯᎵᏙᎲ
- ល្វែងម៉ោង
- ᥑᥣᥝᥰ ᥕᥣᥛᥰ ᥘᥣᥲ ᥘᥤᥢ
- 时区
- 時區
- 標準時間帯
- 시간대
has_id_wikidata: Q12143
subclass_of:
- '[[_Standards/WikiData/WD~region,82794]]'
- '[[_Standards/WikiData/WD~time_standard,6313217]]'
- '[[_Standards/WikiData/WD~non_political_administrative_territorial_entity,15642566]]'
has_list: '[[_Standards/WikiData/WD~list_of_time_zones,3048055]]'
topic_has_template:
- '[[_Standards/WikiData/WD~Template_UTC_time_offsets,7474166]]'
- '[[_Standards/WikiData/WD~Template_Infobox_time_zone,11001401]]'
- '[[_Standards/WikiData/WD~Template_Infobox_time_zone_UTC,13520154]]'
properties_for_this_type:
- located in time zone
- UTC timezone offset
- instance of
main_Wikidata_property: located in time zone
image: http://commons.wikimedia.org/wiki/Special:FilePath/World%20Time%20Zones%20Map.png
page_banner: http://commons.wikimedia.org/wiki/Special:FilePath/HK%20Mongkok%20night%20banner.JPG
equivalent_class: http://www.w3.org/2006/time#TimeZone
described_at_URL: https://github.com/whosonfirst/whosonfirst-placetypes#timezone
Stack_Exchange_tag: https://stackoverflow.com/tags/timezone
Commons_category: Time zones
Commons_gallery: Timezone
Wolfram_Language_entity_type: TimeZone
OpenStreetMap_key: timezone
OpenStreetMap_tag: boundary=timezone
Dewey_Decimal_Classification: 389.17
dv_has_:
  name_:
    ab: Асааҭтә зонақәа
    af: tydsone
    am: ሰዓት ክልል
    an: Zona horaria
    ar: منطقة زمنية
    ary: منطقة زمانية
    arz: مجال زمنى
    as: সময় অঞ্চল
    ast: estaya horaria
    av: СагӀтил рачел
    awa: समय मण्डल
    az: saat qurşağı
    azb: چاغ بؤلگهسی
    ba: Сәғәт бүлкәте
    ban: Wewengkon galah
    bar: Zeitzone
    bcl: Sona kan oras
    be: часавы пояс
    be-tarask: часавы пас
    bg: Часова зона
    bho: टाइम जोन
    bjn: Waktu banua
    blk: အခိန်ႏအွိုင်ꩻ
    bn: সময় অঞ্চল
    bpy: সময়র লয়া
    br: rann-eur
    bs: vremenska zona
    btm: Zona waktu
    ca: fus horari
    ce: Сахьтан аса
    chr: ᎠᏟᏯᎵᏙᎲ
    ckb: ناوچەی کاتی
    crh: saat quşağı
    crh-latn: Saat quşağı
    cs: časové pásmo
    cv: Вăхăт тăрăхĕ
    cy: cylchfa amser
    da: tidszone
    de: Zeitzone
    de-ch: Zeitzone
    dsb: Casowa cona
    el: ζώνη ώρας
    eml: Fûş orâri
    en: time zone
    en-us: time zone
    eo: horzono
    es: huso horario
    et: ajavöönd
    eu: geldialdi-ordutegi
    fa: منطقه زمانی
    fi: aikavyöhyke
    fo: Tíðarsonur
    fr: fuseau horaire
    frr: tidjsoon
    fur: Fûs orari
    fy: tiidsône
    ga: amchrios
    gd: roinn-tìde
    gl: fuso horario
    gn: Yvy Árapehẽ
    gsw: Ziitzone
    gu: સમય વિસ્તાર
    gv: Cryss hraa
    ha: lokacin yanki
    hak: Sṳ̀-khî
    he: אזור זמן
    hi: समय मण्डल
    hif: Time zone
    hr: vremenska zona
    hsb: Časowe pasmo
    hu: időzóna
    hy: Ժամային գոտի
    ia: Fuso horari
    id: zona waktu
    ig: Time zone
    ilo: sona ti oras
    inh: Сахьата оаса
    io: horala zono
    is: tímabelti
    it: fuso orario
    ja: 標準時間帯
    jv: Zona wektu
    ka: სასაათო სარტყელი
    kaa: Waqıt zonası
    kab: Izdi imiran
    kbd: Сыхьэт зонэ
    kg: Mukaba ya ntangu
    kge: Ataran waktu
    kk: Уақыт белдеуі
    km: ល្វែងម៉ោង
    kn: ಸಮಯ ವಲಯ
    ko: 시간대
    krc: Сагъат бёлге
    ks: کال مَنڈَل
    ku: beşa saetê
    ku-latn: beşa saetê
    kv: Час кытш
    kw: parth dermyn
    ky: Убакыт алкагы
    la: Zona temporalis
    lb: Zäitzon
    lez: Сятдин чӀул
    li: Tiedzaone
    lij: fûzo oràrio
    lld: Zona d'orar
    lmo: Füs urari
    lo: ເຂດເວລາ
    lt: laiko juosta
    lv: laika josla
    lzh: 時區
    mag: समय मण्डल
    mai: समय क्षेत्र
    map-bms: Zona waktu
    mdf: пингонь каркс
    mg: Faritr'ora
    mhr: шагат ӱштӧ
    min: Zona wakatu
    mk: часовен појас
    ml: സമയമേഖല
    mn: Цагийн бүс
    mr: आंतरराष्ट्रीय कालविभाग
    ms: zon waktu
    ms-arab: زون وقتو
    mt: żona tal-ħin
    my: အချိန်ဇုန်
    nan: Sî-khu
    nb: tidssone
    nds: Tietrebeet
    ne: समय क्षेत्र
    nl: tijdzone
    nn: tidssone
    nov: tempe-sone
    nrm: Feusé horaire
    nso: Time zone
    oc: fus orari
    olo: Aiguvyöhyke
    om: Godina sa'aatii
    or: ସମୟ ମଣ୍ଡଳ
    pa: ਸਮਾਂ ਖੇਤਰ
    pap: zona di tempu
    pih: tiemsoen
    pl: strefa czasowa
    pms: Zòna oraria
    pnb: ویلا تھاں
    ps: سیمه ییز وخت
    pt: fuso horário
    pt-br: fuso horário
    qu: Pacha suyu
    ro: Fus orar
    ru: часовой пояс
    rue: Часова зона
    sa: समयवलयः
    sah: Кэм зоната
    scn: fusu urariu
    sco: time zone
    sd: وقت پٽو
    se: áigeavádat
    sgs: Čiesa zuona
    sh: Vremenska zona
    shi: Akud adigan
    shn: ၶၢဝ်းယၢမ်းၼႃႈလိၼ်
    si: වේලා කලාපය
    sk: časové pásmo
    sl: časovni pas
    smn: äigistielâs
    sms: äiʹǧǧvuʹvdd
    sq: zona kohore
    sr: временска зона
    sr-ec: временска зона
    stq: Tiedzone
    su: Zona wanci
    sv: tidszon
    sw: Kanda muda
    szl: Czasowo strefa
    ta: நேர வலயம்
    tdd: ᥑᥣᥝᥰ ᥕᥣᥛᥰ ᥘᥣᥲ ᥘᥤᥢ
    te: కాల మండలం
    tg: Минтақаҳои замонӣ
    th: เขตเวลา
    tl: sona ng oras
    tok: ma tenpo
    tr: saat dilimi
    tt: сәгать поясы
    tt-cyrl: сәгать поясы
    tyv: Үени хемчээри
    udm: дырлэн зонаез
    uk: часовий пояс
    ur: منطقۂ وقت
    uz: Vaqt mintaqasi
    vec: Fuso orario
    vep: aigvö
    vi: múi giờ
    vro: Aovüü
    wa: Coisse d' eure
    war: Zona hin oras
    wuu: 时区
    xh: Ijet lag
    xmf: ოსაათე ორტყაფუ
    yi: צייט זאנע
    yo: Agbègbè àkókò
    yue: 時區
    za: sizgiz
    zh: 时区
    zh-cn: 时区
    zh-hans: 时区
    zh-hant: 時區
    zh-hk: 時區
    zh-tw: 時區
    zu: Indawo yesikhathi
---
# [[Time_Zone]] 

#is_/same_as :: [[WD~Time_zone,12143]] 

## #has_/text_of_/abstract 

> A time zone is an area which observes a uniform 
> standard time for legal, commercial and social purposes. 
> 
> Time zones tend to follow the boundaries between countries and their subdivisions 
> instead of strictly following longitude, 
> because it is convenient for areas in frequent communication to keep the same time.
>
> Each time zone is defined by a standard offset from Coordinated Universal Time (UTC). 
> The offsets range from UTC−12:00 to UTC+14:00, and are usually a whole number of hours, 
> but a few zones are offset by an additional 30 or 45 minutes, such as in India and Nepal. 
> 
> Some areas in a time zone may use a different offset for part of the year, 
> typically one hour ahead during spring and summer, 
> a practice known as daylight saving time (DST).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Time%20zone) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Dimension/Dim~Time/Time_Zone|Time_Zone]] 

### #is_/same_as :: [[/_public/Dimension/Dim~Time/Time_Zone.public|Time_Zone.public]] 

### #is_/same_as :: [[/_internal/Dimension/Dim~Time/Time_Zone.internal|Time_Zone.internal]] 

### #is_/same_as :: [[/_protect/Dimension/Dim~Time/Time_Zone.protect|Time_Zone.protect]] 

### #is_/same_as :: [[/_private/Dimension/Dim~Time/Time_Zone.private|Time_Zone.private]] 

### #is_/same_as :: [[/_personal/Dimension/Dim~Time/Time_Zone.personal|Time_Zone.personal]] 

### #is_/same_as :: [[/_secret/Dimension/Dim~Time/Time_Zone.secret|Time_Zone.secret]] 

