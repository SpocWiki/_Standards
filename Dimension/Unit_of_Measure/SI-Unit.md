---
aliases:
- aalmugijkoskâsâš ohtâdâhsysteem
- Alþjóðlega einingakerfið
- Anagraw agraɣlan n tiynin
- Beynəlxalq vahidlər sistemi
- Córas Idirnáisiúnta na nAonad
- Fasal ngaltəram Dunyabe
- halqara birlemler sisteması
- Horgale
- Inkqubo yamazwe ngamazwe yeeyunithi zokulinganisa
- Intanashinal Sistim a Yuunit
- Internacia sistemo de unuoj
- Internasional a Sistema dagiti Yunit
- International System of Units
- Internationalen Eenheetesystem
- Internationales Einheitensystem
- Internationella måttenhetssystemet
- Internatschonal Eenheitensystem
- Kanasodnon Sistema han mga Yunit
- kansainvälinen yksikköjärjestelmä
- Kategoryjo:Ukłŏd SI
- Kevreyth Keswlasek Unsesow
- Kok-chè Tan-ūi Hē-thóng
- kulupu pi wan ilo pi ma ale
- mednarodni sistem enot
- Međunarodni sustav mjernih jedinica
- Millumtjóða eindarskipanin
- Nazioarteko Unitate Sistema
- Pandaigdigang Sistema ng mga Yunit
- Reizhiad unanennoù etrebroadel
- SI
- SI sistem
- SI tupuy
- SI-Basiseinheite
- SI-Einheitesystem
- SI-mértékegységrendszer
- SI-stelsel
- SI-systema
- SI-systemet
- SI-systèm
- SI-süsteem
- SI-Unit
- Siostam Eadar-nàiseanta nan Aonadan
- Sistem Hijian Internasional
- Sistem Internasional de Unias
- Sistem Pétungan Internasional
- Sistem Satuan Internasional
- Sistem Unit Antarabangsa
- Sistem Unit Antarabansa
- Sistema 'ntirnazziunali di unitati di misura
- Sistema Antërnassional ëd Misure
- sistema internacional d'unidaes
- Sistema Internacional d'Unidatz
- Sistema Internacional d'Unitats
- Sistema Internacional de Unidades
- Sistema Internasional
- Sistema International de Unités
- Sistema Internatzionale de Unidades de Medida
- Sistema internazional d'unitaa
- sistema internazionale di unità di misura
- Sistema Internazzjonali ta' Unitajiet
- sistema SI
- Sisteme 'Ndernazionale de aunità de mesure
- Sisteme internazionâl di unitâts
- Sistemi SI
- sistemul internațional de unități
- Sistemê Yewiya İnternasyonale
- Sistèm enternasyonal di inité
- Sistèm Entènasyonal
- Sistèma Internacionau d'Unitats
- Si̱tem Mai-nkyang Bibyin Swanta
- Soustava SI
- Starptautiskā mērvienību sistēma
- System Ryngwladol o Unedau
- Systema International de Unitates
- Systema Internationale
- Système international d'unités
- Sîstema Navneteweyî ya Yekeyan
- Ta'ãhakuéra Ypykatu Hetatetãgua
- treci'e
- układ SI
- Uluslararası Birimler Sistemi
- Unaj-internaciona sistemo
- Vipimo sanifu vya kimataifa
- Ynternasjonaal Systeem fan Ienheden
- Ìwọ̀n Ẹyọ Akáríayé SI
- διεθνές σύστημα μονάδων
- ЁС
- Интернационалдыг хемчеглер системазы
- Мєждоународьнъ мѣръ сѷстима
- Міжнародна система одиниць
- Міжнародная сыстэма СІ
- Међународни систем јединица
- Меѓународен систем на мерни единици
- Международна система единици
- Международная система единиц
- Мертэтъёсын калыккуспо сӧзнэт
- Олон улсын нэгжийн систем
- Омуктардааҕы биирдик тутумтата
- СІ
- СИ
- СИ (Виçев пĕрчисен тĕнчери тытăмĕ)
- Система SI
- Системаи байналмилалии воҳидҳо
- Уласхоорондын нэгэжын системэ
- Халыҡ-ара берәмектәр системаһы
- Эл аралык бирдиктер системасы
- Өлшем бірліктерінің халықаралық жүйесі
- Միավորների միջազգային համակարգ
- Միաւորներու Միջազգային Համակարգ
- אינטערנאציאנאלע סיסטעם פון איינסן
- מערכת היחידות הבין-לאומית
- النظام الدولى للوحدات
- اکائیاں دا عالمی نظام
- اڪائين جو بين الاقوامي نظام
- بین الاقوامی نظام اکائیات
- د واحدونو نړيوال سيستم
- دستگاه بینالمللی یکاها
- سيستم اونيت انتارابڠسا
- سيستيم عالمي د لعبرات
- سیستمی نێونەتەوەییی یەکەکان
- میلتلر آراسی بیریملر سیستمی
- نظام الوحدات الدولي
- अन्तर्राष्ट्रिय इकाई प्रणाली
- अन्तर्राष्ट्रीय इकाई प्रणाली
- एस.आय.
- एसआइ एकाइ
- एस् आइ
- আন্তর্জাতিক একক পদ্ধতি
- আন্তৰ্জাতিক একক পদ্ধতি
- ਅੰਤਰਦੇਸ਼ੀ ਇਕਾਈ ਪ੍ਰਣਾਲੀ
- அனைத்துலக முறை அலகுகள்
- అంతర్జాతీయ ప్రమాణాల పద్ధతి
- ಅಂತರರಾಷ್ಟ್ರೀಯ ಏಕಮಾನ ವ್ಯವಸ್ಥೆ
- അന്താരാഷ്ട്ര ഏകകവ്യവസ്ഥ
- අන්තර්ජාතික ඒකක පද්ධතිය
- ระบบหน่วยวัดระหว่างประเทศ
- အပြည်ပြည်ဆိုင်ရာ ယူနစ် စနစ်
- ერთეულთა საერთაშოირისო სისტემა
- ዓለምለኻዊ ስርዓት ኣሃዱታት መለክዒ
- ប្រព័ន្ធខ្នាតអន្តរជាតិ
- 国际单位制
- 国際単位系
- 國際單位制
- 國際單位系統
- 萬國公制
- 국제단위계
has_id_wikidata: Q12457
Commons_category: International System of Units
instance_of:
- '[[_Standards/WikiData/WD~metric_system,232405|WD~metric_system,232405]]'
- '[[_Standards/WikiData/WD~technical_standard,317623|WD~technical_standard,317623]]'
- '[[_Standards/WikiData/WD~international_standard,1334738|WD~international_standard,1334738]]'
- '[[_Standards/WikiData/WD~coherent_system_of_units,99734981|WD~coherent_system_of_units,99734981]]'
based_on: '[[_Standards/WikiData/WD~MKS_system_of_units,512417|WD~MKS_system_of_units,512417]]'
described_by_source:
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~SI_Brochure_(9th_edition),68977219|WD~SI_Brochure_(9th_edition),68977219]]'
- '[[_Standards/WikiData/WD~International_Vocabulary_of_Metrology_(3rd_edition,_2012),70257574|WD~International_Vocabulary_of_Metrology_(3rd_edition,_2012),70257574]]'
- '[[_Standards/WikiData/WD~DIN_1301,105424877|WD~DIN_1301,105424877]]'
short_name:
- SI
- СИ
OmegaWiki_Defined_Meaning: 983745
schematic:
- http://commons.wikimedia.org/wiki/Special:FilePath/%D0%A1%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%B0%20%D0%A1%D0%98.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Birim%201.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Infografik%20SI%20Einheiten%20Zusammenhang%20V2%201%20Schroffenegger%20Thomas.png
- http://commons.wikimedia.org/wiki/Special:FilePath/Mek-klas-mag.png
- http://commons.wikimedia.org/wiki/Special:FilePath/SI-systemet.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/SI%20base%20unit.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/SI%20Units%20definition%20dependencies.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/SImap.pdf
image: http://commons.wikimedia.org/wiki/Special:FilePath/International%20prototype%20of%20the%20kilogram%20aka%20Le%20Grand%20K.jpg
logo_image: http://commons.wikimedia.org/wiki/Special:FilePath/International%20System%20Of%20Units%20Logo.png
IEV_number: 112-02-02
UMLS_CUI: C0021791
MeSH_tree_code: E05.978.507
dv_has_:
  name_:
    ab: СИ
    af: SI-stelsel
    an: Sistema Internacional d'Unidatz
    ar: نظام الوحدات الدولي
    ary: سيستيم عالمي د لعبرات
    arz: النظام الدولى للوحدات
    as: আন্তৰ্জাতিক একক পদ্ধতি
    ast: sistema internacional d'unidaes
    az: Beynəlxalq vahidlər sistemi
    azb: میلتلر آراسی بیریملر سیستمی
    ba: Халыҡ-ара берәмектәр системаһы
    be: СІ
    be_tarask: Міжнародная сыстэма СІ
    bg: Международна система единици
    bn: আন্তর্জাতিক একক পদ্ধতি
    br: Reizhiad unanennoù etrebroadel
    bs: SI sistem
    bxr: Уласхоорондын нэгэжын системэ
    ca: Sistema Internacional d'Unitats
    ckb: سیستمی نێونەتەوەییی یەکەکان
    crh: halqara birlemler sisteması
    cs: Soustava SI
    cu: Мєждоународьнъ мѣръ сѷстима
    cv: СИ (Виçев пĕрчисен тĕнчери тытăмĕ)
    cy: System Ryngwladol o Unedau
    da: Système International d'Unités
    de: Internationales Einheitensystem
    diq: Sistemê Yewiya İnternasyonale
    el: διεθνές σύστημα μονάδων
    en: International System of Units
    en-us: International System of Units
    eo: Internacia sistemo de unuoj
    es: Sistema Internacional de Unidades
    et: SI-süsteem
    eu: Nazioarteko Unitate Sistema
    fa: دستگاه بینالمللی یکاها
    fi: kansainvälinen yksikköjärjestelmä
    fo: Millumtjóða eindarskipanin
    fr: Système international d'unités
    frr: SI-süsteem
    fur: Sisteme internazionâl di unitâts
    fy: Ynternasjonaal Systeem fan Ienheden
    ga: Córas Idirnáisiúnta na nAonad
    gan: 國際單位系統
    gcr: Sistèm enternasyonal di inité
    gd: Siostam Eadar-nàiseanta nan Aonadan
    gl: Sistema Internacional de Unidades
    gn: Ta'ãhakuéra Ypykatu Hetatetãgua
    gsw: SI-Einheitesystem
    he: מערכת היחידות הבין-לאומית
    hi: अन्तर्राष्ट्रीय इकाई प्रणाली
    hif: SI
    hr: Međunarodni sustav mjernih jedinica
    ht: Sistèm Entènasyonal
    hu: SI-mértékegységrendszer
    hy: Միավորների միջազգային համակարգ
    hyw: Միաւորներու Միջազգային Համակարգ
    ia: Systema International de Unitates
    iba: Sistem Unit Antarabansa
    id: Sistem Satuan Internasional
    ie: Sistema International de Unités
    ilo: Internasional a Sistema dagiti Yunit
    io: Unaj-internaciona sistemo
    is: Alþjóðlega einingakerfið
    it: sistema internazionale di unità di misura
    ja: 国際単位系
    jam: Intanashinal Sistim a Yuunit
    jbo: treci'e
    jv: Sistem Pétungan Internasional
    ka: ერთეულთა საერთაშოირისო სისტემა
    kab: Anagraw agraɣlan n tiynin
    kcg: Si̱tem Mai-nkyang Bibyin Swanta
    kk: Өлшем бірліктерінің халықаралық жүйесі
    km: ប្រព័ន្ធខ្នាតអន្តរជាតិ
    kn: ಅಂತರರಾಷ್ಟ್ರೀಯ ಏಕಮಾನ ವ್ಯವಸ್ಥೆ
    knc: Fasal ngaltəram Dunyabe
    ko: 국제단위계
    krc: ЁС
    ksh: SI-Basiseinheite
    ku: Sîstema Navneteweyî ya Yekeyan
    kw: Kevreyth Keswlasek Unsesow
    ky: Эл аралык бирдиктер системасы
    la: Systema Internationale
    lb: Internationalen Eenheetesystem
    lfn: Sistem Internasional de Unias
    li: SI
    lmo: Sistema internazional d'unitaa
    lt: SI
    lv: Starptautiskā mērvienību sistēma
    lzh: 萬國公制
    mag: अन्तर्राष्ट्रिय इकाई प्रणाली
    mk: Меѓународен систем на мерни единици
    ml: അന്താരാഷ്ട്ര ഏകകവ്യവസ്ഥ
    mn: Олон улсын нэгжийн систем
    mr: एस.आय.
    ms: Sistem Unit Antarabangsa
    ms_arab: سيستم اونيت انتارابڠسا
    mt: Sistema Internazzjonali ta' Unitajiet
    mul: SI
    my: အပြည်ပြည်ဆိုင်ရာ ယူနစ် စနစ်
    nan: Kok-chè Tan-ūi Hē-thóng
    nb: SI-systemet
    nds: Internatschonal Eenheitensystem
    ne: एसआइ एकाइ
    new: एस् आइ
    nl: SI-stelsel
    nn: SI-systemet
    nso: SI
    oc: Sistèma Internacionau d'Unitats
    pa: ਅੰਤਰਦੇਸ਼ੀ ਇਕਾਈ ਪ੍ਰਣਾਲੀ
    pap: sistema SI
    pcm: International System of Units
    pl: układ SI
    pms: Sistema Antërnassional ëd Misure
    pnb: اکائیاں دا عالمی نظام
    ps: د واحدونو نړيوال سيستم
    pt: Sistema Internacional de Unidades
    qu: SI tupuy
    ro: sistemul internațional de unități
    roa-tara: Sisteme 'Ndernazionale de aunità de mesure
    ru: Международная система единиц
    rue: Система SI
    sah: Омуктардааҕы биирдик тутумтата
    sc: Sistema Internatzionale de Unidades de Medida
    scn: Sistema 'ntirnazziunali di unitati di misura
    sco: SI
    sd: اڪائين جو بين الاقوامي نظام
    se: SI-systema
    sgs: SI
    sh: SI
    si: අන්තර්ජාතික ඒකක පද්ධතිය
    sk: SI
    sl: mednarodni sistem enot
    smn: aalmugijkoskâsâš ohtâdâhsysteem
    so: Horgale
    sq: Sistemi SI
    sr: Међународни систем јединица
    su: Sistem Hijian Internasional
    sv: Internationella måttenhetssystemet
    sw: Vipimo sanifu vya kimataifa
    szl: Kategoryjo:Ukłŏd SI
    ta: அனைத்துலக முறை அலகுகள்
    te: అంతర్జాతీయ ప్రమాణాల పద్ధతి
    tg: Системаи байналмилалии воҳидҳо
    th: ระบบหน่วยวัดระหว่างประเทศ
    ti: ዓለምለኻዊ ስርዓት ኣሃዱታት መለክዒ
    tl: Pandaigdigang Sistema ng mga Yunit
    tok: kulupu pi wan ilo pi ma ale
    tr: Uluslararası Birimler Sistemi
    tt: СИ
    tyv: Интернационалдыг хемчеглер системазы
    udm: Мертэтъёсын калыккуспо сӧзнэт
    uk: Міжнародна система одиниць
    ur: بین الاقوامی نظام اکائیات
    uz: SI
    vec: Sistema Internasional
    vi: SI
    vls: SI-systèm
    war: Kanasodnon Sistema han mga Yunit
    wuu: 國際單位制
    xh: Inkqubo yamazwe ngamazwe yeeyunithi zokulinganisa
    yi: אינטערנאציאנאלע סיסטעם פון איינסן
    yo: Ìwọ̀n Ẹyọ Akáríayé SI
    yue: 國際單位制
    zh: 國際單位制
    zh_cn: 国际单位制
    zh_hans: 国际单位制
    zh_hant: 國際單位制
    zh_hk: 國際單位制
    zh_mo: 國際單位制
    zh-my: 国际单位制
    zh_sg: 国际单位制
    zh_tw: 國際單位制
---

# [[SI-Units]] 

## Edit

SI-Units are defined since 1967 and standardized in ISO_80000. 

#is_/same_as :: [[WD~International_System_of_Units,12457]] 

## #has_/text_of_/abstract 

> The International System of Units, internationally known by the abbreviation SI 
> (from French Système international d'unités), is the modern form of the metric system 
> and the world's most widely used system of measurement. 
> 
> It is the only system of measurement with official status in nearly every country in the world, 
> employed in science, technology, industry, and everyday commerce. 
> The SI system is coordinated by the International Bureau of Weights and Measures, 
> which is abbreviated BIPM from French: Bureau international des poids et mesures.
>
> The SI comprises a coherent system of units of measurement starting with seven base units, 
> which are the second (symbol s, the unit of time), metre (m, length), kilogram (kg, mass), 
> ampere (A, electric current), kelvin (K, thermodynamic temperature), 
> mole (mol, amount of substance), and candela (cd, luminous intensity). 
> 
> The system can accommodate coherent units for an unlimited number of additional quantities.
>  These are called coherent derived units, 
>  which can always be represented as products of powers of the base units. 
>  Twenty-two coherent derived units have been provided with special names and symbols.
>
> The seven base units and the 22 coherent derived units with special names and symbols 
> may be used in combination to express other coherent derived units. 
>
> Since the sizes of coherent units will be convenient for only some applications and not for others, the SI provides twenty-four prefixes which, when added to the name and symbol of a coherent unit produce twenty-four additional (non-coherent) SI units for the same quantity; these non-coherent units are always decimal (i.e. power-of-ten) multiples and sub-multiples of the coherent unit.
>
> The current way of defining the SI is a result of a decades-long move 
> towards increasingly abstract and idealised formulation 
> in which the realisations of the units are separated conceptually from the definitions. 
> 
> A consequence is that as science and technologies develop, 
> new and superior realisations may be introduced without the need to redefine the unit. 
> One problem with artefacts is that they can be lost, damaged, or changed; 
> another is that they introduce uncertainties that cannot be reduced by advancements in science and technology.
>
> The original motivation for the development of the SI was the diversity of units 
> that had sprung up within the centimetre–gram–second (CGS) systems 
> (specifically inconsistencies between electrostatic and electromagnetic units) 
> and the lack of coordination between the various disciplines that used them. 
> 
> The [[SI-Unit/CGPM(General_Conference_on_Weights_and_Measures)|CGPM(General_Conference_on_Weights_and_Measures)]] (French: Conférence générale des poids et mesures – CGPM), which was established by the Metre Convention of 1875, 
> brought together many international organisations 
> to establish the definitions and standards of a new system 
> and to standardise the rules for writing and presenting measurements. 
> 
> The system was published in 1960 as a result of an initiative that began in 1948, and is based on the metre–kilogram–second system of units (MKS) combined with ideas from the development of the CGS system.
>
> [Wikipedia](https://en.wikipedia.org/wiki/International%20System%20of%20Units) 


## All Contents

```folderv
```

```ccard
type: folder_brief_live
```


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Dimension/Unit_of_Measurement/SI-Unit|SI-Unit]] 

### #is_/same_as :: [[/_public/Dimension/Unit_of_Measurement/SI-Unit.public|SI-Unit.public]] 

### #is_/same_as :: [[/_internal/Dimension/Unit_of_Measurement/SI-Unit.internal|SI-Unit.internal]] 

### #is_/same_as :: [[/_protect/Dimension/Unit_of_Measurement/SI-Unit.protect|SI-Unit.protect]] 

### #is_/same_as :: [[/_private/Dimension/Unit_of_Measurement/SI-Unit.private|SI-Unit.private]] 

### #is_/same_as :: [[/_personal/Dimension/Unit_of_Measurement/SI-Unit.personal|SI-Unit.personal]] 

### #is_/same_as :: [[/_secret/Dimension/Unit_of_Measurement/SI-Unit.secret|SI-Unit.secret]] 

