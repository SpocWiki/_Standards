---
aliases:
- daya gerak elektrik
- electromotive force
- elektr yurituvchi kuch
- elektriskais dzinējspēks
- elektrohərəkət qüvvəsi
- elektromotoorjõud
- elektromotor kuvvet
- elektromotorická sila
- elektromotorické napětí
- elektromotorische kracht
- elektromotorisk kraft
- elektromotorisk spenning
- elektromotorisk spänning
- elektromotorna sila
- elektrovara
- fonta elektra tensio
- forca elektromotore
- force électromotrice
- forrásfeszültség
- forsa elettromotrice
- forza electromotriz
- forza elettromotiva
- forza elettromotrice
- força electromotriu
- força eletromotriz
- forță electromotoare
- fuercia electromotriz
- Fòrça electromotritz
- fòs elektwomotris
- fórsa leictreaghluaisneach
- gaya gerak listrik
- grym electromotif
- Indar elektroeragile
- lực điện động
- napajalna napetost
- Quellenspannung
- siła elektromotoryczna
- source voltage
- sähkömotorinen voima
- tensión de fuente
- ηλεκτρεγερτική δύναμη
- електродвижеща сила
- електромоторна сила
- електрорушійна сила
- электр йөртү көче
- электр қозғаушы күш
- электрарухальная сіла
- электраруховая сіла
- электродвижущая сила
- электрокуçтару вăйĕ
- Էլեկտրաշարժ ուժ
- כוח אלקטרו מניע
- برحرکی قوت
- قوة محركة كهربائية
- نیروی محرک الکتریکی
- विद्युतवाहक बल
- তড়িচ্চালক শক্তি
- மின்னியக்கு விசை
- విద్యుచ్ఛాలక బలం
- แรงเคลื่อนไฟฟ้า
- အီလက်ထရွန်မိုးတစ်အား
- 电动势
- 起電力
- 電動勢
- 기전력
has_id_wikidata: Q185329
subclass_of:
- '[[_Standards/WikiData/WD~physical_quantity,107715|WD~physical_quantity,107715]]'
- '[[_Standards/WikiData/WD~scalar_quantity,181175|WD~scalar_quantity,181175]]'
- '[[_Standards/WikiData/WD~voltage,118309876|WD~voltage,118309876]]'
different_from: '[[_Standards/WikiData/WD~open_circuit_voltage,1812203|WD~open_circuit_voltage,1812203]]'
described_by_source:
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718|WD~Armenian_Soviet_Encyclopedia,2657718]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~IEC_80000_6_2008_Quantities_and_units_Part_6_Electromagnetism,26711936|WD~IEC_80000_6_2008_Quantities_and_units_Part_6_Electromagnetism,26711936]]'
- '[[_Standards/WikiData/WD~Small_Soviet_Encyclopedia,66386517|WD~Small_Soviet_Encyclopedia,66386517]]'
- '[[_Standards/WikiData/WD~IEC_80000_6_2022_Quantities_and_units_Part_6_Electromagnetism,117847945|WD~IEC_80000_6_2022_Quantities_and_units_Part_6_Electromagnetism,117847945]]'
recommended_unit_of_measurement:
- '[[_Standards/WikiData/WD~kilogram_square_metre_per_cubic_second_ampere,107299709|WD~kilogram_square_metre_per_cubic_second_ampere,107299709]]'
- '[[_Standards/WikiData/WD~volt,25250|WD~volt,25250]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~Q131438152,131438152|WD~Q131438152,131438152]]'
ISQ_dimension: "<math class=\"mwe-math-element mwe-math-element-inline\" xmlns=\"http://www.w3.org/1998/Math/MathML\"><mrow data-mjx-texclass=\"ORD\"><mstyle displaystyle=\"true\" scriptlevel=\"0\"><msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5AB</mi></mrow><mrow data-mjx-texclass=\"ORD\"><mn>2</mn></mrow></msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5AC</mi></mrow><msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5B3</mi></mrow><mrow data-mjx-texclass=\"ORD\"><mrow data-mjx-texclass=\"ORD\"><mo stretchy=\"false\">&#x2212;</mo><mn>3</mn></mrow></mrow></msup><msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5A8</mi></mrow><mrow data-mjx-texclass=\"ORD\"><mrow data-mjx-texclass=\"ORD\"><mo stretchy=\"false\">&#x2212;</mo><mn>1</mn></mrow></mrow></msup></mstyle></mrow></math>"
quantity_symbol_LaTeX_:
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>E</mi></mstyle></mrow></math>
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><msub><mi>U</mi><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mi mathvariant="normal">s</mi></mrow></mrow></msub></mstyle></mrow></math>
image: http://commons.wikimedia.org/wiki/Special:FilePath/Electromotive%20Force%20for%20an%20ideal%20voltage%20source.jpg
related_image: http://commons.wikimedia.org/wiki/Special:FilePath/Galvanic%20cell%20labeled.svg
IEV_number: 131-12-22
Commons_category: Electromotive force
short_name:
- emf
- ЭДС
dv_has_:
  name_:
    ar: قوة محركة كهربائية
    ast: fuercia electromotriz
    az: elektrohərəkət qüvvəsi
    be: электрарухальная сіла
    be_tarask: электраруховая сіла
    bg: електродвижеща сила
    bn: তড়িচ্চালক শক্তি
    bs: elektromotorna sila
    ca: força electromotriu
    cs: elektromotorické napětí
    cv: электрокуçтару вăйĕ
    cy: grym electromotif
    da: elektromotorisk kraft
    de: Quellenspannung
    el: ηλεκτρεγερτική δύναμη
    en: electromotive force
    en-us: source voltage
    eo: fonta elektra tensio
    es: tensión de fuente
    et: elektromotoorjõud
    eu: Indar elektroeragile
    fa: نیروی محرک الکتریکی
    fi: sähkömotorinen voima
    fr: force électromotrice
    ga: fórsa leictreaghluaisneach
    gl: forza electromotriz
    he: כוח אלקטרו מניע
    hi: विद्युतवाहक बल
    hr: elektromotorna sila
    ht: fòs elektwomotris
    hu: forrásfeszültség
    hy: Էլեկտրաշարժ ուժ
    id: gaya gerak listrik
    it: forza elettromotrice
    ja: 起電力
    kk: электр қозғаушы күш
    ko: 기전력
    lij: forsa elettromotrice
    lt: elektrovara
    lv: elektriskais dzinējspēks
    mag: विद्युतवाहक बल
    mk: електромоторна сила
    ms: daya gerak elektrik
    mt: forza elettromotiva
    my: အီလက်ထရွန်မိုးတစ်အား
    nb: elektromotorisk spenning
    ne: विद्युतवाहक बल
    nl: elektromotorische kracht
    nn: elektromotorisk spenning
    oc: Fòrça electromotritz
    pl: siła elektromotoryczna
    pt: força eletromotriz
    ro: forță electromotoare
    ru: электродвижущая сила
    sco: electromotive force
    sh: elektromotorna sila
    sk: elektromotorická sila
    sl: napajalna napetost
    sq: forca elektromotore
    sr: електромоторна сила
    sr_ec: електромоторна сила
    sr_el: elektromotorna sila
    sv: elektromotorisk spänning
    ta: மின்னியக்கு விசை
    te: విద్యుచ్ఛాలక బలం
    th: แรงเคลื่อนไฟฟ้า
    tr: elektromotor kuvvet
    tt: электр йөртү көче
    tt_cyrl: электр йөртү көче
    uk: електрорушійна сила
    ur: برحرکی قوت
    uz: elektr yurituvchi kuch
    vi: lực điện động
    wuu: 电动势
    yue: 電動勢
    zh: 電動勢
    zh_cn: 电动势
    zh_hans: 电动势
    zh_hant: 電動勢
    zh_hk: 電動勢
    zh_sg: 电动势
    zh_tw: 電動勢
---

# [[Source_Voltage]] 

#is_/same_as :: [[_Standards/WikiData/WD~Source_voltage,185329|WD~Source_voltage,185329]] 
#is_/similar_to :: [[Dim~Voltage|Dim~Voltage]] 

## #has_/text_of_/abstract 

> In electromagnetism and electronics, electromotive force 
> (also electromotance, abbreviated emf, denoted E)
> is an energy transfer to an electric circuit per unit of electric charge, measured in volts. 
> 
> Devices called electrical transducers provide an emf 
> by converting other forms of energy into electrical energy. 
> 
> Other types of electrical equipment also produce an emf, 
> such as batteries, which convert chemical energy, 
> and generators, which convert mechanical energy. 
> 
> This energy conversion is achieved 
> by physical forces applying physical work on electric charges. 
> 
> However, electromotive force itself is not a physical force, 
> and ISO/IEC standards have deprecated the term 
> in favor of source voltage or source tension instead (denoted U_s).
>
> An electronic–hydraulic analogy may 
> view emf as the mechanical work done to water by a pump, 
> which results in a pressure difference (analogous to voltage).
>
> In electromagnetic induction, emf can be defined around a closed loop of a conductor 
> as the electromagnetic work that would be done on an elementary electric charge 
> (such as an electron) if it travels once around the loop.
>
> For two-terminal devices modeled as a Thévenin equivalent circuit, 
> an equivalent emf can be measured as 
> the open-circuit voltage between the two terminals. 
> This emf can drive an electric current if an external circuit is attached to the terminals, 
> in which case the device becomes the voltage source of that circuit.
>
> Although an emf gives rise to a voltage and can be measured as a voltage 
> and may sometimes informally be called a "voltage", 
> they are not the same phenomenon 
>
> [Wikipedia](https://en.wikipedia.org/wiki/Electromotive%20force) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Dimension/Derived_Dimension/Source_Voltage|Source_Voltage]] 

### #is_/same_as :: [[/_public/Dimension/Derived_Dimension/Source_Voltage.public|Source_Voltage.public]] 

### #is_/same_as :: [[/_internal/Dimension/Derived_Dimension/Source_Voltage.internal|Source_Voltage.internal]] 

### #is_/same_as :: [[/_protect/Dimension/Derived_Dimension/Source_Voltage.protect|Source_Voltage.protect]] 

### #is_/same_as :: [[/_private/Dimension/Derived_Dimension/Source_Voltage.private|Source_Voltage.private]] 

### #is_/same_as :: [[/_personal/Dimension/Derived_Dimension/Source_Voltage.personal|Source_Voltage.personal]] 

### #is_/same_as :: [[/_secret/Dimension/Derived_Dimension/Source_Voltage.secret|Source_Voltage.secret]] 

