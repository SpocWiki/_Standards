---
aliases:
- auren mata da yawa
- fampirafesana
- Isithembu
- liespriediezh
- mitara
- permaduan
- polagamas
- poligami
- poligamia
- poligamie
- poligamii
- poligamija
- poligamio
- poligamiya
- Poligamiye
- poligamya
- poligàmia
- poligámia
- poligāmija
- poliqamiya
- polyandry
- polygami
- polygamia
- Polygamie
- polygamiija
- polygamija
- polygamije
- Polygamy
- polygyny
- polügaamia
- sitsembu
- đa phu thê
- πολυγαμία
- палігамія
- полігамія
- полигамија
- полигамия
- чандҳамсарӣ
- բազմակնություն
- פוליגמיה
- بوليجاميه
- تعدد ازواج
- زواج تعددي
- فرەھاوسەری
- پولیگامی
- څو ودونه کول
- چندهمسری
- ڤرمادوان
- बहुपत्नीकत्व
- बहुविवाह
- বহুগামিতা
- বহুবিবাহ
- ਬਹੁਵਿਆਹ
- பலதுணை மணம்
- బహుభార్యాత్వం
- პოლიგამია
- ịlụ karịrị otu nwanyị
- 多配偶制
- 複婚
- 복혼
has_id_wikidata: Q170585
has_part_s_of_the_class:
- '[[_Standards/WikiData/WD~polygynous_marriage,243688]]'
- '[[_Standards/WikiData/WD~polyandry,2102979]]'
different_from:
- '[[_Standards/WikiData/WD~polyamory,272026]]'
- '[[_Standards/WikiData/WD~Polygamia,10350931]]'
- '[[_Standards/WikiData/WD~consensual_non_monogamy,110515686]]'
described_by_source:
- '[[../../../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Explanatory_Dictionary_of_the_Living_Great_Russian_Language,1970746]]'
- "[[_Standards/WikiData/WD~Otto's_encyclopedia,2041543]]"
- '[[_Standards/WikiData/WD~Jewish_Encyclopedia_of_Brockhaus_and_Efron,4173137]]'
- '[[_Standards/WikiData/WD~The_American_Cyclopædia,19077875]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~New_Encyclopedic_Dictionary,19190511]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
has_part_s_: '[[_Standards/WikiData/WD~Polygamy_in_Islam,3300314]]'
subclass_of:
- '[[_Standards/WikiData/WD~polygamy_in_animals,3395570]]'
- '[[_Standards/WikiData/WD~non_monogamy,3436958]]'
- '[[_Standards/WikiData/WD~marriage,8445]]'
instance_of:
- '[[_Standards/WikiData/WD~mating_system,4584563]]'
- '[[_Standards/WikiData/WD~type_of_marriage,7860953]]'
OmegaWiki_Defined_Meaning: 1267421
image: http://commons.wikimedia.org/wiki/Special:FilePath/Chef%20de%20village%20et%20ses%20femmes%20%28Guin%C3%A9e%29.jpg
icon: http://commons.wikimedia.org/wiki/Special:FilePath/Polygyny%20%28bold%2C%20color%29.svg
Stack_Exchange_tag:
- https://christianity.stackexchange.com/tags/polygamy
- https://judaism.stackexchange.com/tags/polygamy
U_S_National_Archives_Identifier: 10665454
Iconclass_notation: 42Z1
subreddit: polygamy
booru_tag: polygamy
Commons_category: Polygamy
dv_has_:
  name_:
    af: poligamie
    an: poligamia
    ar: زواج تعددي
    arz: بوليجاميه
    as: বহুবিবাহ
    ast: poligamia
    az: poliqamiya
    ba: полигамия
    bcl: poligamiya
    be: палігамія
    be_tarask: палігамія
    bg: полигамия
    bn: বহুগামিতা
    br: liespriediezh
    bs: poligamija
    ca: poligàmia
    ckb: فرەھاوسەری
    cs: polygamie
    da: polygami
    de: Polygamie
    diq: Poligamiye
    el: πολυγαμία
    en: polygamy
    eo: poligamio
    es: poligamia
    et: polügaamia
    eu: poligamia
    fa: چندهمسری
    fi: polygamia
    fr: polygamie
    fy: Polygamy
    ga: polagamas
    gl: poligamia
    ha: auren mata da yawa
    he: פוליגמיה
    hi: बहुविवाह
    hr: poligamija
    hu: poligámia
    hy: բազմակնություն
    id: poligami
    ig: ịlụ karịrị otu nwanyị
    it: poligamia
    ja: 複婚
    jv: poligami
    ka: პოლიგამია
    kk: полигамия
    ko: 복혼
    ky: полигамия
    la: polygamia
    lb: Polygamie
    lt: poligamija
    lv: poligāmija
    mg: fampirafesana
    mk: полигамија
    mr: बहुपत्नीकत्व
    ms: permaduan
    ms_arab: ڤرمادوان
    mzn: پولیگامی
    nb: polygami
    ne: बहुविवाह
    nl: polygamie
    nn: polygami
    pa: ਬਹੁਵਿਆਹ
    pl: poligamia
    ps: څو ودونه کول
    pt: poligamia
    ro: poligamie
    ru: полигамия
    sah: полигамия
    se: polygamiija
    sh: poligamija
    sk: polygamia
    sl: poligamija
    sma: polygamije
    smj: polygamija
    smn: polygamia
    sms: polygamia
    sr: полигамија
    sr_ec: полигамија
    sr_el: poligamija
    ss: sitsembu
    sv: polygami
    sw: mitara
    ta: பலதுணை மணம்
    te: బహుభార్యాత్వం
    tg: чандҳамсарӣ
    tl: poligamiya
    tr: poligami
    uk: полігамія
    ur: تعدد ازواج
    uz: poligamiya
    vep: poligamii
    vi: đa phu thê
    war: poligamya
    wuu: 多配偶制
    yue: 多配偶制
    zh: 多配偶制
    zh_cn: 多配偶制
    zh_hans: 多配偶制
    zh_hant: 多配偶制
    zh_hk: 多配偶制
    zh_sg: 多配偶制
    zh_tw: 多配偶制
    zu: Isithembu
---
# [[Polygamy]] 

#is_/same_as :: [[WD~Polygamy,170585]] 

## #has_/text_of_/abstract 

> **Polygamy** (from Late Greek πολυγαμία polygamía, "state of marriage to many spouses") 
> is the practice of marrying multiple spouses. 
> When a man is married to more than one wife at the same time, it is called polygyny. 
> When a woman is married to more than one husband at the same time, it is called polyandry. 
> In contrast, in sociobiology and zoology, researchers use "polygamy" more broadly to refer to any form of multiple mating.
>
> In contrast to polygamy, monogamy is marriage consisting of only two parties. 
> Like "monogamy", the term "polygamy" is often used in a de facto sense, 
> applied regardless of whether a state recognizes the relationship.  In many countries, the law only recognises monogamous marriages (a person can only have one spouse, and bigamy is illegal), but adultery is not illegal, leading to a situation of de facto polygamy being allowed without legal recognition for non-official "spouses".
>
> Worldwide, different societies variously encourage, accept or outlaw polygamy. In societies which allow or tolerate polygamy, polygyny is the accepted form in the vast majority of cases. According to the Ethnographic Atlas Codebook, of 1,231 societies noted from 1960 to 1980, 588 had frequent polygyny, 453 had occasional polygyny, 186 were monogamous, and 4 had polyandry – although more recent research found some form of polyandry in 53 communities, which is more common than previously thought. In cultures which practice polygamy, its prevalence among that population often correlates with social class and socioeconomic status. Polygamy (taking the form of polygyny) is most common in a region known as the "polygamy belt" in West Africa and Central Africa, with the countries estimated to have the highest polygamy prevalence in the world being Burkina Faso, Mali, Gambia, Niger and Nigeria.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Polygamy) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Agent/Community/Clan/Family/Marriage/Polygamy|Polygamy]] 

### #is_/same_as :: [[/_public/Society/Agent/Community/Clan/Family/Marriage/Polygamy.public|Polygamy.public]] 

### #is_/same_as :: [[/_internal/Society/Agent/Community/Clan/Family/Marriage/Polygamy.internal|Polygamy.internal]] 

### #is_/same_as :: [[/_protect/Society/Agent/Community/Clan/Family/Marriage/Polygamy.protect|Polygamy.protect]] 

### #is_/same_as :: [[/_private/Society/Agent/Community/Clan/Family/Marriage/Polygamy.private|Polygamy.private]] 

### #is_/same_as :: [[/_personal/Society/Agent/Community/Clan/Family/Marriage/Polygamy.personal|Polygamy.personal]] 

### #is_/same_as :: [[/_secret/Society/Agent/Community/Clan/Family/Marriage/Polygamy.secret|Polygamy.secret]] 

