---
aliases:
- comhphobal fíorúil
- communauté virtuelle
- communitas virtualis
- comunautat virtuala
- comunidad virtual
- comunidade virtual
- comunidá virtual
- comunitat virtual
- comunitate virtuală
- comunità virtuale
- cộng đồng ảo
- komunitas maya
- komunitate birtual
- komuniti maya
- kumuniezh enlinenn
- navidezna skupnost
- społeczność internetowa
- topluluk siteleri
- virtuaaliyhteisö
- virtuaalne kogukond
- virtuaalsiärvádâh
- virtual commonty
- virtual community
- virtual cəmiyyət
- virtual hamjamiyat
- virtuala komunumo
- virtualna zajednica
- virtuele gemeenschap
- virtuell gemenskap
- virtuelle Gemeinschaft
- virtuelt fællesskab
- virtuelt samfunn
- virtuála servodat
- virtuális közösség
- virtuální komunita
- virtuāla kopiena
- vrtualna zajednica
- δυνητικές κοινότητες
- віртуальна спільнота
- віртуальная супольнасць
- віртуальная супольнасьць
- виртуална общност
- виртуаль җәмгыять
- виртуальное сообщество
- виртуелна заједница
- виртуелна заедница
- ҷомеъаи маҷозӣ
- համացանցային համայնք
- קהילה מקוונת
- مجتمع افتراضي
- ชุมชนเสมือน
- 仮想共同体
- 虚拟社区
- 虚拟社群
- 虛擬社群
- 가상 공동체
has_id_wikidata: Q210980
subclass_of:
- '[[_Standards/WikiData/WD~Web_2.0,131164]]'
- '[[_Standards/WikiData/WD~community,177634]]'
- '[[_Standards/WikiData/WD~website,35127]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~online_community,6576792]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/HD%40DH.nrw%20Online-Meeting.svg
Commons_category: Community websites
Dewey_Decimal_Classification: 302.30285
dv_has_:
  name_:
    ar: مجتمع افتراضي
    ast: comunidá virtual
    az: virtual cəmiyyət
    be: віртуальная супольнасць
    be_tarask: віртуальная супольнасьць
    bg: виртуална общност
    br: kumuniezh enlinenn
    bs: virtualna zajednica
    ca: comunitat virtual
    cs: virtuální komunita
    da: virtuelt fællesskab
    de: virtuelle Gemeinschaft
    el: δυνητικές κοινότητες
    en: virtual community
    eo: virtuala komunumo
    es: comunidad virtual
    et: virtuaalne kogukond
    eu: komunitate birtual
    fi: virtuaaliyhteisö
    fr: communauté virtuelle
    ga: comhphobal fíorúil
    gl: comunidade virtual
    he: קהילה מקוונת
    hr: vrtualna zajednica
    hu: virtuális közösség
    hy: համացանցային համայնք
    id: komunitas maya
    it: comunità virtuale
    ja: 仮想共同体
    ko: 가상 공동체
    la: communitas virtualis
    lv: virtuāla kopiena
    mk: виртуелна заедница
    ms: komuniti maya
    nb: virtuelt samfunn
    nl: virtuele gemeenschap
    nn: virtuelt samfunn
    oc: comunautat virtuala
    pl: społeczność internetowa
    pt: comunidade virtual
    ro: comunitate virtuală
    ru: виртуальное сообщество
    sco: virtual commonty
    se: virtuála servodat
    sh: virtualna zajednica
    sl: navidezna skupnost
    smn: virtuaalsiärvádâh
    sr: виртуелна заједница
    sr_ec: виртуелна заједница
    sv: virtuell gemenskap
    tg: ҷомеъаи маҷозӣ
    th: ชุมชนเสมือน
    tr: topluluk siteleri
    tt: виртуаль җәмгыять
    uk: віртуальна спільнота
    uz: virtual hamjamiyat
    vi: cộng đồng ảo
    zh: 虛擬社群
    zh_cn: 虚拟社区
    zh_hans: 虚拟社群
    zh_hant: 虛擬社群
    zh_hk: 虛擬社群
    zh_sg: 虚拟社区
    zh_tw: 虛擬社群
---
# [[Virtual_Community]] 

#is_/same_as :: [[../../../WikiData/WD~Virtual_community,210980|WD~Virtual_community,210980]]  

## #has_/text_of_/abstract 

> A virtual community is a social network of individuals 
> who connect through specific social media, 
> potentially crossing geographical and political boundaries 
> in order to pursue mutual interests or goals. 
> 
> Some of the most pervasive virtual communities are online communities 
> operating under social networking services.
>
> Howard Rheingold discussed virtual communities in his book, The Virtual Community, 
> published in 1993. 
> 
> The book's discussion ranges from Rheingold's adventures on The WELL, 
> computer-mediated communication, social groups and information science. 
> 
> Technologies cited include Usenet, MUDs (Multi-User Dungeon) 
> and their derivatives MUSHes and MOOs, Internet Relay Chat (IRC), 
> chat rooms and electronic mailing lists. 
> 
> Rheingold also points out the potential benefits for personal psychological well-being, 
> as well as for society at large, of belonging to a virtual community. 
> At the same time, it showed that job engagement 
> positively influences virtual communities of practice engagement.
>
> Virtual communities all encourage interaction, 
> sometimes focusing around a particular interest or just to communicate. 
> Some virtual communities do both. 
> 
> Community members are allowed to interact over a shared passion 
> through various means: message boards, chat rooms, 
> social networking World Wide Web sites, or virtual worlds. 
> 
> Members usually become attached to the community world, 
> logging in and out on sites all day every day, which can certainly become an addiction.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Virtual%20community) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Agent/Community/Virtual_Community|Virtual_Community]] 

### #is_/same_as :: [[/_public/Society/Agent/Community/Virtual_Community.public|Virtual_Community.public]] 

### #is_/same_as :: [[/_internal/Society/Agent/Community/Virtual_Community.internal|Virtual_Community.internal]] 

### #is_/same_as :: [[/_protect/Society/Agent/Community/Virtual_Community.protect|Virtual_Community.protect]] 

### #is_/same_as :: [[/_private/Society/Agent/Community/Virtual_Community.private|Virtual_Community.private]] 

### #is_/same_as :: [[/_personal/Society/Agent/Community/Virtual_Community.personal|Virtual_Community.personal]] 

### #is_/same_as :: [[/_secret/Society/Agent/Community/Virtual_Community.secret|Virtual_Community.secret]] 

