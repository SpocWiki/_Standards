---
aliases:
- Daftar pamenang bebungah Nobel miturut bidang
- Daftar penerima Nobel
- danh sách người đoạt giải Nobel
- Elenco dei vincitori del Premio Nobel
- ganadores del Premio Nobel
- laureados com o Nobel
- lijst van winnaars van de Nobelprijs
- list of Nobel laureates
- Lista de ganiores de la Premio Nobel
- lista de gañadores dos Premios Nobel
- lista e fituesve të Çmimit Nobel
- lista laureatów Nagrody Nobla
- lista laureaților Premiului Nobel
- lista över nobelpristagare
- Liste der Nobelpreisträger
- liste des lauréats du Prix Nobel
- liste over modtagere af Nobelprisen
- liste over nobelprisvinnere
- listo Nobel-palhâšume vyeittein
- llista de ganadores del Premiu Nobel
- llista de guardonats amb el Nobel
- luettelo Nobel-palkinnon saajista
- Lîsteya xelatgirên Nobelê
- Nobel laureate
- Nobel mukofoti sohiblari roʻyxati
- Nobel mükafatı laureatlarının siyahısı
- Nobel Ödülü sahipleri listesi
- Nobel-díjasok listája
- Nobela prēmiju laureāti
- Nobel_Laureate
- Senarai pemenang Hadiah Nobel
- seznam Nobelovih nagrajencev
- Seznam nositelů Nobelovy ceny
- Àkójọ àwọn ẹlẹ́bùn Nobel
- Κατάλογος βραβευμένων με Νόμπελ
- Листа нобелових лауреата
- Нобелевские премии по году вручения
- Нобелийн шагналтнуудын нэрс
- список Нобелівських лауреатів
- Списък на нобеловите лауреати
- Феҳристи барандагони ҷоизаи Нобел
- Նոբելյան մրցանակակիրների ցանկ
- الحاصلين على جايزة نوبل
- د نوبل جایزې ګټونکو نوملړ
- فهرست برندگان جایزه نوبل
- قائمة الحاصلين على جائزة نوبل
- نوبل انعام جتن والیاں دی لسٹ
- نوبل انعام حاصل کرنے والوں کی فہرست
- نوبل انعام حاصل ڪندڙن جي فهرست
- پێرستی وەرگرانی خەڵاتی نۆبێل
- नोबल पुरस्कार-विजेतानां सूची
- नोबेल पुरस्कार विजेताओं की सूची
- नोबेल पुरस्कार विजेताहरुको सूची
- नोबेल पुरस्कार विजेताहरूको सूची
- ন'বেল বঁটা (২০১২)
- নোবেল পুরস্কারপ্রাপ্তদের তালিকা
- ਨੋਬਲ ਇਨਾਮ ਜੇਤੂਆਂ ਦੀ ਸੂਚੀ
- નોબેલ પારિતોષિક વડે સન્માનીત મહાનુભાવો
- நோபல் பரிசு பெற்றவர்கள் பட்டியல்
- నోబెల్ బహుమతి గ్రహీతలు
- സാഹിത്യനോബൽ
- รายชื่อผู้ได้รับรางวัลโนเบล
- ნობელის პრემიის ლაურეატები
- ノーベル賞受賞者の一覧
- 諾貝爾獎得主列表
- 诺贝尔奖得主列表
- 노벨상 수상자 목록
has_id_wikidata: Q339619
dv_has_:
  name_:
    ar: قائمة الحاصلين على جائزة نوبل
    arz: الحاصلين على جايزة نوبل
    as: ন'বেল বঁটা (২০১২)
    ast: llista de ganadores del Premiu Nobel
    az: Nobel mükafatı laureatlarının siyahısı
    bg: Списък на нобеловите лауреати
    bn: নোবেল পুরস্কারপ্রাপ্তদের তালিকা
    ca: llista de guardonats amb el Nobel
    ckb: پێرستی وەرگرانی خەڵاتی نۆبێل
    cs: Seznam nositelů Nobelovy ceny
    da: liste over modtagere af Nobelprisen
    de: Liste der Nobelpreisträger
    dty: नोबेल पुरस्कार विजेताहरूको सूची
    el: Κατάλογος βραβευμένων με Νόμπελ
    en: list of Nobel laureates
    en_ca: List of Nobel laureates
    en_gb: list of Nobel laureates
    es: ganadores del Premio Nobel
    fa: فهرست برندگان جایزه نوبل
    fi: luettelo Nobel-palkinnon saajista
    fr: liste des lauréats du Prix Nobel
    gl: lista de gañadores dos Premios Nobel
    gu: નોબેલ પારિતોષિક વડે સન્માનીત મહાનુભાવો
    hi: नोबेल पुरस्कार विजेताओं की सूची
    hu: Nobel-díjasok listája
    hy: Նոբելյան մրցանակակիրների ցանկ
    id: Daftar penerima Nobel
    it: Elenco dei vincitori del Premio Nobel
    ja: ノーベル賞受賞者の一覧
    jv: Daftar pamenang bebungah Nobel miturut bidang
    ka: ნობელის პრემიის ლაურეატები
    ko: 노벨상 수상자 목록
    ku: Lîsteya xelatgirên Nobelê
    lfn: Lista de ganiores de la Premio Nobel
    lv: Nobela prēmiju laureāti
    ml: സാഹിത്യനോബൽ
    mn: Нобелийн шагналтнуудын нэрс
    ms: Senarai pemenang Hadiah Nobel
    nb: liste over nobelprisvinnere
    ne: नोबेल पुरस्कार विजेताहरुको सूची
    nl: lijst van winnaars van de Nobelprijs
    pa: ਨੋਬਲ ਇਨਾਮ ਜੇਤੂਆਂ ਦੀ ਸੂਚੀ
    pl: lista laureatów Nagrody Nobla
    pnb: نوبل انعام جتن والیاں دی لسٹ
    ps: د نوبل جایزې ګټونکو نوملړ
    pt: laureados com o Nobel
    pt_br: Laureados com o Nobel
    ro: lista laureaților Premiului Nobel
    ru: Нобелевские премии по году вручения
    sa: नोबल पुरस्कार-विजेतानां सूची
    sd: نوبل انعام حاصل ڪندڙن جي فهرست
    sl: seznam Nobelovih nagrajencev
    smn: listo Nobel-palhâšume vyeittein
    sq: lista e fituesve të Çmimit Nobel
    sr: Листа нобелових лауреата
    sv: lista över nobelpristagare
    ta: நோபல் பரிசு பெற்றவர்கள் பட்டியல்
    te: నోబెల్ బహుమతి గ్రహీతలు
    tg: Феҳристи барандагони ҷоизаи Нобел
    th: รายชื่อผู้ได้รับรางวัลโนเบล
    tr: Nobel Ödülü sahipleri listesi
    uk: список Нобелівських лауреатів
    ur: نوبل انعام حاصل کرنے والوں کی فہرست
    uz: Nobel mukofoti sohiblari roʻyxati
    vi: danh sách người đoạt giải Nobel
    yo: Àkójọ àwọn ẹlẹ́bùn Nobel
    zh: 諾貝爾獎得主列表
    zh_cn: 诺贝尔奖得主列表
    zh_hans: 诺贝尔奖得主列表
    zh_hant: 諾貝爾獎得主列表
    zh_hk: 諾貝爾獎得主列表
    zh_sg: 诺贝尔奖得主列表
    zh_tw: 諾貝爾獎得主列表
---
## #has_/text_of_/abstract 

> The Nobel Prizes (Swedish: Nobelpriset, Norwegian: Nobelprisen) 
> are awarded annually by the Royal Swedish Academy of Sciences, the Swedish Academy, 
> the Karolinska Institutet, and the Norwegian Nobel Committee 
> to individuals and organizations who make outstanding contributions in the fields of 
> - chemistry, 
> - physics, 
> - literature, 
> - peace, and 
> - physiology or 
> - medicine. 
> 
> They were established by the 1895 will of Alfred Nobel, which dictates that the awards should be administered by the Nobel Foundation. An additional prize in memory of Alfred Nobel was established in 1968 by Sveriges Riksbank (Sweden's central bank) for outstanding contributions to the field of economics. Each recipient, a Nobelist or laureate, receives a gold medal, a diploma, and a sum of money which is decided annually by the Nobel Foundation.
>
> [Wikipedia](https://en.wikipedia.org/wiki/List%20of%20Nobel%20laureates) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/Nobel_Prize/Nobel_Laureate|Nobel_Laureate]] 

### #is_/same_as :: [[/_public/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/Nobel_Prize/Nobel_Laureate.public|Nobel_Laureate.public]] 

### #is_/same_as :: [[/_internal/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/Nobel_Prize/Nobel_Laureate.internal|Nobel_Laureate.internal]] 

### #is_/same_as :: [[/_protect/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/Nobel_Prize/Nobel_Laureate.protect|Nobel_Laureate.protect]] 

### #is_/same_as :: [[/_private/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/Nobel_Prize/Nobel_Laureate.private|Nobel_Laureate.private]] 

### #is_/same_as :: [[/_personal/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/Nobel_Prize/Nobel_Laureate.personal|Nobel_Laureate.personal]] 

### #is_/same_as :: [[/_secret/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/Nobel_Prize/Nobel_Laureate.secret|Nobel_Laureate.secret]] 

