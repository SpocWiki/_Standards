---
aliases:
- biucinjvaq
- estandardització
- estandarización
- kuchepa
- normalisation technique
- normalización
- normalización técnica
- normalizacja
- normalizatio
- normalizazio
- normigo
- padronização
- pembakuan
- pemiawaian
- safoni
- standaardisatie
- standardimine
- standardisasi
- Standardisassion
- standardisation
- standardisering
- Standardisierung
- Standardiséierung
- standardizace
- standardizacija
- standardizare
- standardizasion
- standardizasyon
- Standardization
- Standardizimi
- standardizzazione
- standardointi
- standarisasi
- standartizacija
- standartizācija
- standartlaşdırma
- standarttaw
- szabványosítás
- tiêu chuẩn hóa
- προτυποποίηση
- стандардизација
- стандартизація
- стандартизация
- стандартлаштырыу
- стандарттау
- стандартызацыя
- ստանդարտացում
- תִּקְנוּן
- استانداردسازی
- توحيد المعايير
- ستاندارتتاۋ
- معیارسازی
- मानकीकरण
- মানকীকৰণ
- செந்தரமாக்கம்
- മാനകീകരണം
- การจัดมาตรฐาน
- ოერეფოშქაშე სტანდარტი
- საერთაშორისო სტანდარტი
- 标准化
- 標準化
- 표준화
has_id_wikidata: Q369577
has_effect: '[[_Standards/WikiData/WD~technical_standard,317623|WD~technical_standard,317623]]'
product_or_material_produced: '[[_Standards/WikiData/WD~technical_standard,317623|WD~technical_standard,317623]]'
subclass_of:
- '[[_Standards/WikiData/WD~quality_assurance,836575|WD~quality_assurance,836575]]'
- '[[_Standards/WikiData/WD~industry_classification,2976602|WD~industry_classification,2976602]]'
facet_of:
- '[[_Standards/WikiData/WD~norm,1087786|WD~norm,1087786]]'
- '[[_Standards/WikiData/WD~quality_process,3406372|WD~quality_process,3406372]]'
instance_of: '[[_Standards/WikiData/WD~type_of_process,116505632|WD~type_of_process,116505632]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._11,124355862|WD~Armenian_Soviet_Encyclopedia,_vol._11,124355862]]'
Dewey_Decimal_Classification: 389
Commons_category: Standardization
spoken_text_audio: http://commons.wikimedia.org/wiki/Special:FilePath/Wikipedia%20-%20Standardization%20%28spoken%20by%20AI%20voice%29.mp3
dv_has_:
  name_:
    ar: توحيد المعايير
    as: মানকীকৰণ
    ast: normalización
    az: standartlaşdırma
    ba: стандартлаштырыу
    be: стандартызацыя
    be_tarask: стандартызацыя
    ca: estandardització
    cs: standardizace
    cy: safoni
    da: standardisering
    de: Standardisierung
    el: προτυποποίηση
    en: standardization
    en_ca: standardization
    en_gb: standardisation
    eo: normigo
    es: normalización técnica
    et: standardimine
    eu: normalizazio
    fa: استانداردسازی
    fi: standardointi
    fr: normalisation technique
    gl: estandarización
    he: תִּקְנוּן
    hi: मानकीकरण
    hu: szabványosítás
    hy: ստանդարտացում
    id: pembakuan
    it: standardizzazione
    ja: 標準化
    jv: standarisasi
    ka: საერთაშორისო სტანდარტი
    kk: стандарттау
    kk_arab: ستاندارتتاۋ
    kk-cn: ستاندارتتاۋ
    kk_cyrl: стандарттау
    kk-kz: стандарттау
    kk_latn: standarttaw
    kk-tr: standarttaw
    ko: 표준화
    la: normalizatio
    lb: Standardiséierung
    lt: standartizacija
    lv: standartizācija
    mk: стандардизација
    ml: മാനകീകരണം
    ms: pemiawaian
    nb: standardisering
    nl: standaardisatie
    nn: standardisering
    pl: normalizacja
    pms: Standardisassion
    pt: padronização
    pt_br: padronização
    ro: standardizare
    ru: стандартизация
    sl: standardizacija
    sn: kuchepa
    sq: Standardizimi
    sr: стандардизација
    sr_ec: стандардизација
    sr_el: standardizacija
    su: standardisasi
    sv: standardisering
    ta: செந்தரமாக்கம்
    th: การจัดมาตรฐาน
    tr: standardizasyon
    uk: стандартизація
    ur: معیارسازی
    vec: standardizasion
    vi: tiêu chuẩn hóa
    wuu: 标准化
    xmf: ოერეფოშქაშე სტანდარტი
    yue: 標準化
    za: biucinjvaq
    zh: 标准化
    zh_cn: 标准化
    zh_hans: 标准化
    zh_hant: 標準化
    zh_hk: 標準化
    zh_sg: 标准化
    zh_tw: 標準化
---

# [[Standardization]] 

#is_/same_as :: [[../../../../../../../WikiData/WD~Standardization,369577|WD~Standardization,369577]]  

## #has_/text_of_/abstract 

> **Standardization** (American English) or standardisation (British English) 
> is the process of implementing and developing technical standards 
> based on the consensus of different parties that include 
> firms, users, interest groups, standards organizations and governments. 
> 
> Standardization can help maximize 
> compatibility, interoperability, safety, repeatability, efficiency, and quality. 
> 
> It can also facilitate a normalization of formerly custom processes. 
>
> In social sciences, including economics, 
> the idea of standardization is close to the solution for a coordination problem, 
> a situation in which all parties can realize mutual gains, 
> but only by making mutually consistent decisions. 
> 
> Divergent national, industrial or company standards impose costs on consumers 
> and can be a form of non-tariff trade barrier.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Standardization) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/ISO(International_Organization_for_Standardization)/Standardization|Standardization]] 

### #is_/same_as :: [[/_public/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/ISO(International_Organization_for_Standardization)/Standardization.public|Standardization.public]] 

### #is_/same_as :: [[/_internal/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/ISO(International_Organization_for_Standardization)/Standardization.internal|Standardization.internal]] 

### #is_/same_as :: [[/_protect/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/ISO(International_Organization_for_Standardization)/Standardization.protect|Standardization.protect]] 

### #is_/same_as :: [[/_private/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/ISO(International_Organization_for_Standardization)/Standardization.private|Standardization.private]] 

### #is_/same_as :: [[/_personal/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/ISO(International_Organization_for_Standardization)/Standardization.personal|Standardization.personal]] 

### #is_/same_as :: [[/_secret/Society/Agent/Community/Organization/International_Organization/NGO(non-governmental_organization)/ISO(International_Organization_for_Standardization)/Standardization.secret|Standardization.secret]] 

