---
aliases:
- Agham pang-aklatan at pang-impormasyon
- bibliotecoloxía
- biblioteconomia
- biblioteconomie
- biblioteconomía y documentación
- biblioteekkunde
- bibliotek- og informasjonsvitenskap
- bibliotek- og informasjonsvitskap
- bibliotekarstvo
- bibliotekarstvo in informacijska znanost
- bibliotekininkystė
- Bibliotekonomia
- bibliotekoznawstwo
- biblioteks- och informationsvetenskap
- bibliotekscienco
- biblioteksvidenskab
- bibliotheekwetenschap
- Bibliothekswissenschaft
- Bibliothekswüsseschaft
- Bibliothéikswëssenschaft
- bibliotēkzinātne
- Bókasafns- og upplýsingafræði
- eolaíocht leabharlainne
- girjerádjodieđa
- ilmu perpustakaan
- khoa học thư viện
- kirjastotiede
- kirjerájutieđâ
- kitabxanaşünaslıq
- kitobxonəšynosəti
- knihovní věda
- knihovníctvo
- knjižničarstvo
- Kutubxonashunoslik
- könyvtártudomány
- kütüphane bilimi
- Levraouegouriezh
- librar science
- Libraro ed informo cienco
- Library and information science
- Library science
- Library_Science
- Llyfrgellyddiaeth
- Ngèlmu kapustakan
- Oaylleeaght lioarlann
- Raamadukogondus
- raamatukoguteadus
- sains perpustakaan
- science des bibliothèques
- Sciéncias de l'informacion e de las bibliotècas
- shkencë e bibliotekës
- Syensya hin sulod-barasahan
- ǩeʹrjjpõrtt-tiođ
- βιβλιοθηκονομία
- бібліотекознавство
- бібліятэказнаўства
- бібліятэчная справа
- библиотекарство
- библиотековедение
- илми китобдонӣ
- китапханәчелек
- գրադարանագիտություն
- מידענות
- د کتابتون معلوماتو ساینس
- زانستی کتێبخانە
- ساءينس ڤرڤوستاکاءن
- علم المكتبات
- علم کتابداری
- کتب خانہ سائنس
- ग्रंथालयशास्त्र
- पुस्तकालय विज्ञान
- গ্রন্থাগার বিজ্ঞান
- গ্ৰন্থাগাৰ বিজ্ঞান
- ਲਾਇਬ੍ਰੇਰੀ ਵਿਗਿਆਨ
- நூலகவியல்
- ಗ್ರಂಥಾಲಯ ವಿಜ್ಞಾನ
- ഗ്രന്ഥാലയ വിവര ശാസ്ത്രം
- පුස්තකාලවේදය
- บรรณารักษศาสตร์และสารนิเทศศาสตร์
- ບັນນາລັກສາດແລະສາລະນິເທດສາດ
- የመጻሕፍት ቤትና የመረጃ ጥናት
- បណ្ណារក្សសាស្ត្រនិងវិទ្យាសាស្រ្តព័ត៌មាន
- 図書館学
- 图书馆信息学
- 图书馆学
- 圖書館信息學
- 圖書館學
- 도서관학
has_id_wikidata: Q199655
practiced_by:
- '[[_Standards/WikiData/WD~librarian,182436|WD~librarian,182436]]'
- '[[_Standards/WikiData/WD~cataloger,33109310|WD~cataloger,33109310]]'
- '[[_Standards/WikiData/WD~library_scientist,55389138|WD~library_scientist,55389138]]'
- '[[_Standards/WikiData/WD~metadata_librarian,80490291|WD~metadata_librarian,80490291]]'
instance_of:
- '[[_Standards/WikiData/WD~job_activity,192581|WD~job_activity,192581]]'
- '[[_Standards/WikiData/WD~field_of_work,627436|WD~field_of_work,627436]]'
- '[[_Standards/WikiData/WD~field_of_study,1047113|WD~field_of_study,1047113]]'
- '[[_Standards/WikiData/WD~academic_major,4671286|WD~academic_major,4671286]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829|WD~academic_discipline,11862829]]'
described_by_source:
- '[[../../../../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._3,124737616|WD~Armenian_Soviet_Encyclopedia,_vol._3,124737616]]'
has_part_s_:
- '[[_Standards/WikiData/WD~library_reference_service,630961|WD~library_reference_service,630961]]'
- '[[_Standards/WikiData/WD~cataloging,3535655|WD~cataloging,3535655]]'
- '[[_Standards/WikiData/WD~library_collection_development,3567214|WD~library_collection_development,3567214]]'
- '[[_Standards/WikiData/WD~library_and_archives_activities,29586013|WD~library_and_archives_activities,29586013]]'
- '[[_Standards/WikiData/WD~library_service,59309634|WD~library_service,59309634]]'
- '[[_Standards/WikiData/WD~document,49848|WD~document,49848]]'
part_of:
- '[[_Standards/WikiData/WD~librarianship,856625|WD~librarianship,856625]]'
- '[[_Standards/WikiData/WD~library_and_information_science,13420675|WD~library_and_information_science,13420675]]'
- '[[_Standards/WikiData/WD~information_science,16387|WD~information_science,16387]]'
is_the_study_of: '[[_Standards/WikiData/WD~knowledge_organization,1929761|WD~knowledge_organization,1929761]]'
different_from:
- '[[_Standards/WikiData/WD~library_and_information_science,13420675|WD~library_and_information_science,13420675]]'
- '[[_Standards/WikiData/WD~library_and_archives_activities,29586013|WD~library_and_archives_activities,29586013]]'
- '[[_Standards/WikiData/WD~librarianship,99231505|WD~librarianship,99231505]]'
said_to_be_the_same_as:
- '[[_Standards/WikiData/WD~library_and_archives_activities,29586013|WD~library_and_archives_activities,29586013]]'
- '[[_Standards/WikiData/WD~librarianship,99231505|WD~librarianship,99231505]]'
subclass_of: '[[_Standards/WikiData/WD~information_science,16387|WD~information_science,16387]]'
Colon_Classification: 2
Universal_Decimal_Classification: 02
U_S_National_Archives_Identifier: 10636496
Dewey_Decimal_Classification: 020
Commons_category: Library and information science
image: http://commons.wikimedia.org/wiki/Special:FilePath/Ancientlibraryalex.jpg
Regensburg_Classification: AN 65800
Wolfram_Language_entity_code: Entity["Concept", "LibraryScience::c7x2k"]
MeSH_tree_code: L01.583
dv_has_:
  name_:
    af: biblioteekkunde
    am: የመጻሕፍት ቤትና የመረጃ ጥናት
    ar: علم المكتبات
    as: গ্ৰন্থাগাৰ বিজ্ঞান
    ast: biblioteconomia
    az: kitabxanaşünaslıq
    be: бібліятэказнаўства
    be_tarask: бібліятэчная справа
    bn: গ্রন্থাগার বিজ্ঞান
    br: Levraouegouriezh
    bs: bibliotekarstvo
    ca: biblioteconomia
    ckb: زانستی کتێبخانە
    cs: knihovní věda
    cy: Llyfrgellyddiaeth
    da: biblioteksvidenskab
    de: Bibliothekswissenschaft
    el: βιβλιοθηκονομία
    en: library science
    en_ca: library science
    en_gb: library science
    eo: bibliotekscienco
    es: biblioteconomía y documentación
    et: raamatukoguteadus
    eu: Bibliotekonomia
    fa: علم کتابداری
    fi: kirjastotiede
    fr: science des bibliothèques
    ga: eolaíocht leabharlainne
    gl: bibliotecoloxía
    gsw: Bibliothekswüsseschaft
    gv: Oaylleeaght lioarlann
    he: מידענות
    hi: पुस्तकालय विज्ञान
    hr: knjižničarstvo
    hu: könyvtártudomány
    hy: գրադարանագիտություն
    id: ilmu perpustakaan
    io: Libraro ed informo cienco
    is: Bókasafns- og upplýsingafræði
    it: biblioteconomia
    ja: 図書館学
    jv: Ngèlmu kapustakan
    km: បណ្ណារក្សសាស្ត្រនិងវិទ្យាសាស្រ្តព័ត៌មាន
    kn: ಗ್ರಂಥಾಲಯ ವಿಜ್ಞಾನ
    ko: 도서관학
    lb: Bibliothéikswëssenschaft
    lo: ບັນນາລັກສາດແລະສາລະນິເທດສາດ
    lt: bibliotekininkystė
    lv: bibliotēkzinātne
    mk: библиотекарство
    ml: ഗ്രന്ഥാലയ വിവര ശാസ്ത്രം
    mr: ग्रंथालयशास्त्र
    ms: sains perpustakaan
    ms_arab: ساءينس ڤرڤوستاکاءن
    nb: bibliotek- og informasjonsvitenskap
    ne: पुस्तकालय विज्ञान
    nl: bibliotheekwetenschap
    nn: bibliotek- og informasjonsvitskap
    oc: Sciéncias de l'informacion e de las bibliotècas
    pa: ਲਾਇਬ੍ਰੇਰੀ ਵਿਗਿਆਨ
    pl: bibliotekoznawstwo
    ps: د کتابتون معلوماتو ساینس
    pt: biblioteconomia
    pt_br: biblioteconomia
    ro: biblioteconomie
    ru: библиотековедение
    sco: librar science
    se: girjerádjodieđa
    sh: Bibliotekarstvo
    si: පුස්තකාලවේදය
    sk: knihovníctvo
    sl: bibliotekarstvo in informacijska znanost
    smn: kirjerájutieđâ
    sms: ǩeʹrjjpõrtt-tiođ
    sq: shkencë e bibliotekës
    sr: библиотекарство
    sr_ec: библиотекарство
    sr_el: bibliotekarstvo
    sv: biblioteks- och informationsvetenskap
    ta: நூலகவியல்
    tg: илми китобдонӣ
    th: บรรณารักษศาสตร์และสารนิเทศศาสตร์
    tl: Agham pang-aklatan at pang-impormasyon
    tly: kitobxonəšynosəti
    tr: kütüphane bilimi
    tt: китапханәчелек
    uk: бібліотекознавство
    ur: کتب خانہ سائنس
    uz: Kutubxonashunoslik
    vec: biblioteconomia
    vi: khoa học thư viện
    vro: Raamadukogondus
    war: Syensya hin sulod-barasahan
    wuu: 图书馆学
    yue: 圖書館學
    zh: 圖書館學
    zh_cn: 图书馆学
    zh_hans: 图书馆学
    zh_hant: 圖書館學
    zh_hk: 圖書館信息學
    zh_sg: 图书馆信息学
    zh_tw: 圖書館學
---

# [[Library_Science]] 

#is_/same_as :: [[_Standards/WikiData/WD~Library_science,199655|WD~Library_science,199655]] 

## #has_/text_of_/abstract 

> Library and information science (LIS) are two academic disciplines that study all aspects of 
> the creation, organization, management, communication, and use of recorded information. 
> 
> It underlies a variety of professional activities such as 
> - information management, 
> - librarianship, and 
> - archiving and records management, 
> - educating professionals for work in those areas, and 
> - carrying out research to improve practice.
>
> Library science and information science are two original disciplines; 
> however, they are within the same field of study. 
> 
> Library science is a subfield of applied information science. 
> Due to the strong connection, sometimes the two terms are used synonymously.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Library%20and%20information%20science) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Agent/Community/Organization/Business/Local_Business/Library/Library_Science|Library_Science]] 

### #is_/same_as :: [[/_public/Society/Agent/Community/Organization/Business/Local_Business/Library/Library_Science.public|Library_Science.public]] 

### #is_/same_as :: [[/_internal/Society/Agent/Community/Organization/Business/Local_Business/Library/Library_Science.internal|Library_Science.internal]] 

### #is_/same_as :: [[/_protect/Society/Agent/Community/Organization/Business/Local_Business/Library/Library_Science.protect|Library_Science.protect]] 

### #is_/same_as :: [[/_private/Society/Agent/Community/Organization/Business/Local_Business/Library/Library_Science.private|Library_Science.private]] 

### #is_/same_as :: [[/_personal/Society/Agent/Community/Organization/Business/Local_Business/Library/Library_Science.personal|Library_Science.personal]] 

### #is_/same_as :: [[/_secret/Society/Agent/Community/Organization/Business/Local_Business/Library/Library_Science.secret|Library_Science.secret]] 

