---
aliases:
- biblioteca escolar
- biblioteca scolastica
- Biblioteka e shkollës
- biblioteka szkolna
- bibliothèque scolaire
- Eskolako liburutegi
- iskolai könyvtár
- kooliraamatukogu
- koulukirjasto
- lerneja biblioteko
- llyfrgell ysgol
- Maktab kutubxonasi
- māktāp kitaphanāse
- okul kütüphanesi
- Perpustakaan Sekolah
- school library
- schoolbibliotheek
- Schulbibliothek
- skolas bibliotēka
- skolbibliotek
- Skolebibliotek
- skulebibliotek
- skuvlagirjerádju
- skuvlegærjagåetie
- skåvllåbibliotehkka
- skåvvlågirrjevuarrká
- Skólabókasafn
- Umtapolwazi wesikole
- Óbá akwukwo úlò akwukwo
- školní knihovna
- Školska biblioteka
- školska knjižnica
- škooulǩeʹrjjpõrtt
- škovlâkirjerááju
- šolska knjžnica
- Σχολική βιβλιοθήκη
- шкільна бібліотека
- школска библиотека
- школьная бібліятэка
- школьная библиотека
- Դպրոցական գրադարան
- ספריית בית ספר
- مكتبة مدرسية
- کتابخانه آموزشگاهی
- ਸਕੂਲ ਲਾਇਬ੍ਰੇਰੀ
- பள்ளி நூலகம்
- ห้องสมุดโรงเรียน
- 学校図書館
- 学校图书馆
- 學校圖書館
- 학교 도서관
has_id_wikidata: Q1076099
instance_of: '[[_Standards/WikiData/WD~type_of_library,25397890]]'
contains: '[[_Standards/WikiData/WD~school_library_book,92259219]]'
part_of: '[[_Standards/WikiData/WD~school,3914]]'
subclass_of: '[[_Standards/WikiData/WD~library,7075]]'
Colon_Classification: 231
image: http://commons.wikimedia.org/wiki/Special:FilePath/Boxwood%20PS%20Library%205.jpg
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q1321%20%28spa%29-Centenoyespelta-biblioteca%20escolar.wav
Provenio_UUID: 4377f790-1b9d-42b0-9fd6-394a9908d479
Commons_category: School libraries
dv_has_:
  name_:
    ar: مكتبة مدرسية
    be_tarask: школьная бібліятэка
    bs: Školska biblioteka
    ca: biblioteca escolar
    cs: školní knihovna
    cy: llyfrgell ysgol
    da: Skolebibliotek
    de: Schulbibliothek
    el: Σχολική βιβλιοθήκη
    en: school library
    eo: lerneja biblioteko
    es: biblioteca escolar
    et: kooliraamatukogu
    eu: Eskolako liburutegi
    fa: کتابخانه آموزشگاهی
    fi: koulukirjasto
    fr: bibliothèque scolaire
    he: ספריית בית ספר
    hr: školska knjižnica
    hu: iskolai könyvtár
    hy: Դպրոցական գրադարան
    id: Perpustakaan Sekolah
    ig: Óbá akwukwo úlò akwukwo
    is: Skólabókasafn
    it: biblioteca scolastica
    ja: 学校図書館
    ko: 학교 도서관
    lv: skolas bibliotēka
    ms: Perpustakaan sekolah
    nb: skolebibliotek
    nl: schoolbibliotheek
    nn: skulebibliotek
    pa: ਸਕੂਲ ਲਾਇਬ੍ਰੇਰੀ
    pl: biblioteka szkolna
    pt: biblioteca escolar
    ru: школьная библиотека
    se: skuvlagirjerádju
    sje: skåvvlågirrjevuarrká
    sl: šolska knjžnica
    sma: skuvlegærjagåetie
    smj: skåvllåbibliotehkka
    smn: škovlâkirjerááju
    sms: škooulǩeʹrjjpõrtt
    sq: Biblioteka e shkollës
    sr: школска библиотека
    sr_ec: школска библиотека
    sty: māktāp kitaphanāse
    sv: skolbibliotek
    ta: பள்ளி நூலகம்
    th: ห้องสมุดโรงเรียน
    tr: okul kütüphanesi
    uk: шкільна бібліотека
    uz: Maktab kutubxonasi
    yue: 學校圖書館
    zh: 學校圖書館
    zh_hans: 学校图书馆
    zh_hant: 學校圖書館
    zh_hk: 學校圖書館
    zu: Umtapolwazi wesikole
---

# [[School_Library]] 

#is_/same_as :: [[../../../../../../../WikiData/WD~School_library,1076099|WD~School_library,1076099]]  

## #has_/text_of_/abstract 

> A school library (or a school media center) is a library within a school 
> where students, and sometimes their parents and staff have access 
> to borrow a variety of resources, often literary or digital. 
> 
> The goal of a school library or media center is 
> to ensure that all members of the school community have equitable access 
> "to books and reading, to information, and to information technology". 
> 
> A school library or media center "uses all types of media . . . 
> is automated, and utilizes the Internet [as well as books] for information gathering." 
>
> School libraries are distinct from public libraries 
> because they serve as "learner-oriented laboratories 
> which support, extend, and individualize the school's curriculum... 
> A school library serves as the center and coordinating agency for all material used in the school."
>
> [Wikipedia](https://en.wikipedia.org/wiki/School%20library) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Agent/Community/Organization/Business/Local_Business/Library/School_Library|School_Library]] 

### #is_/same_as :: [[/_public/Society/Agent/Community/Organization/Business/Local_Business/Library/School_Library.public|School_Library.public]] 

### #is_/same_as :: [[/_internal/Society/Agent/Community/Organization/Business/Local_Business/Library/School_Library.internal|School_Library.internal]] 

### #is_/same_as :: [[/_protect/Society/Agent/Community/Organization/Business/Local_Business/Library/School_Library.protect|School_Library.protect]] 

### #is_/same_as :: [[/_private/Society/Agent/Community/Organization/Business/Local_Business/Library/School_Library.private|School_Library.private]] 

### #is_/same_as :: [[/_personal/Society/Agent/Community/Organization/Business/Local_Business/Library/School_Library.personal|School_Library.personal]] 

### #is_/same_as :: [[/_secret/Society/Agent/Community/Organization/Business/Local_Business/Library/School_Library.secret|School_Library.secret]] 

