---
aliases:
- Alan Parsons Project
- Alan_Parsons_Project
- The Alan Parsons Project
- Алън Парсънс Проджект
- آلن پارسونز پراجکت
- الن پارسونز پراجکت
- مشروع آلان بارسونز (فريق روك)
- தி ஆலன் பார்சன்சு புரோஜெக்டு
- アラン・パーソンズ・プロジェクト
- 亚伦·派森計畫
- 亚伦·派森计划
- 亞倫派森實驗樂團
- 앨런 파슨스 프로젝트
has_id_wikidata: Q533478
dv_has_:
  name_:
    ar: مشروع آلان بارسونز (فريق روك)
    ast: The Alan Parsons Project
    az: The Alan Parsons Project
    azb: الن پارسونز پراجکت
    bg: Алън Парсънс Проджект
    ca: The Alan Parsons Project
    cs: The Alan Parsons Project
    da: The Alan Parsons Project
    de: The Alan Parsons Project
    en: The Alan Parsons Project
    es: The Alan Parsons Project
    fa: آلن پارسونز پراجکت
    fi: The Alan Parsons Project
    fr: The Alan Parsons Project
    ga: The Alan Parsons Project
    gl: The Alan Parsons Project
    he: The Alan Parsons Project
    hu: The Alan Parsons Project
    hy: The Alan Parsons Project
    id: The Alan Parsons Project
    it: The Alan Parsons Project
    ja: アラン・パーソンズ・プロジェクト
    ka: The Alan Parsons Project
    ko: 앨런 파슨스 프로젝트
    la: The Alan Parsons Project
    mul: The Alan Parsons Project
    nb: The Alan Parsons Project
    nds: The Alan Parsons Project
    nl: The Alan Parsons Project
    nn: The Alan Parsons Project
    pl: The Alan Parsons Project
    pt: The Alan Parsons Project
    ro: The Alan Parsons Project
    ru: The Alan Parsons Project
    sv: The Alan Parsons Project
    ta: தி ஆலன் பார்சன்சு புரோஜெக்டு
    tr: The Alan Parsons Project
    uk: The Alan Parsons Project
    vi: The Alan Parsons Project
    zh: 亞倫派森實驗樂團
    zh_hans: 亚伦·派森计划
    zh_hant: 亚伦·派森計畫
---
# [[Alan_Parsons_Project]] 

## #has_/text_of_/abstract  

> The **Alan Parsons Project** were a British rock band active between 1975 and 1990, 
> whose core membership consisted of producer, audio engineer, musician, and composer [[Parsons,Alan]] 
> and singer, songwriter, and pianist [[Eric Woolfson]]. 
> They shared writing credits on almost all of their songs, with Parsons producing or co-producing all of the recordings, 
> while being  accompanied by various session musicians, some relatively consistent.
>
> The Alan Parsons Project released eleven studio albums over a 15-year career, 
> the most successful ones being I Robot (1977), The Turn of a Friendly Card (1980) and Eye in the Sky (1982). 
> Many of their albums are conceptual in nature and focus on science fiction, supernatural, literary and sociological themes. 
> Among the group's most popular songs are "I Wouldn't Want to Be Like You", 
> "Games People Play", "Time", "Sirius"/"Eye in the Sky", and "Don't Answer Me".
>
> [Wikipedia](https://en.wikipedia.org/wiki/The%20Alan%20Parsons%20Project) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Communication/Media/Music/Music_Genre/Progressive_Rock/Alan_Parsons_Project|Alan_Parsons_Project]] 

### #is_/same_as :: [[/_public/Society/Communication/Media/Music/Music_Genre/Progressive_Rock/Alan_Parsons_Project.public|Alan_Parsons_Project.public]] 

### #is_/same_as :: [[/_internal/Society/Communication/Media/Music/Music_Genre/Progressive_Rock/Alan_Parsons_Project.internal|Alan_Parsons_Project.internal]] 

### #is_/same_as :: [[/_protect/Society/Communication/Media/Music/Music_Genre/Progressive_Rock/Alan_Parsons_Project.protect|Alan_Parsons_Project.protect]] 

### #is_/same_as :: [[/_private/Society/Communication/Media/Music/Music_Genre/Progressive_Rock/Alan_Parsons_Project.private|Alan_Parsons_Project.private]] 

### #is_/same_as :: [[/_personal/Society/Communication/Media/Music/Music_Genre/Progressive_Rock/Alan_Parsons_Project.personal|Alan_Parsons_Project.personal]] 

### #is_/same_as :: [[/_secret/Society/Communication/Media/Music/Music_Genre/Progressive_Rock/Alan_Parsons_Project.secret|Alan_Parsons_Project.secret]] 

