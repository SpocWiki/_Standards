---
aliases:
- beleşmäleq
- Chilwero cha kubekerapo
- erreferentzia-lan
- gwaith cyfeiriol
- hakuteos
- Izibhalo zamaphatho
- iṣẹ́ ìtọ́kasí
- karya referensi
- karya rujukan
- kaynak kitap
- lucrare de referință
- Manazarta
- Nachschlagewerk
- naslaanwerk
- naslagwerk
- Nokieksel
- oberenn dave
- obra de referencia
- obra de referència
- obra de referência
- ohcangirji
- ooccâmǩeʹrjj
- opera di consultazione
- oppslagsverk
- opslagsværk
- opus referentiale
- ouvrage de référence
- praca referencyjna
- Príručková publikácia
- reference wirk
- reference work
- Reference_Work
- referenciamunka
- referencverko
- referenslitteratur
- referentni rad
- referentno djelo
- Referenzwerk
- referenčno delo
- referenční dílo
- sangguniang gawa
- saothar tagartha
- teatmeteos
- tài liệu tham khảo
- Uuccâmkirje
- uzziņu līdzeklis
- vepër referimi
- òbra de referencia
- òpara de refarensa
- žinynas
- έργο αναφοράς
- Анықтамалық басылым
- белешмә
- белешмәлек
- даведнік
- довідник
- Лавлагааны бүтээл
- маълумотнома
- прирачно дело
- референтни рад
- референтно дело
- справочник
- տեղեկատու
- ספר יעץ
- حوالہ جاتی مواد
- عمل مرجعى
- عمل مرجعي
- کاری سەرچاوە
- کتاب مرجع
- আকর গ্রন্থ
- প্ৰসংগ পুথি
- ഗ്രന്ഥം
- งานอ้างอิง
- ཡོ་བྱད་དཔེ་ཆ།
- 参考図書
- 工具书
- 工具書
- 참고서
has_id_wikidata: Q13136
subclass_of:
- '[[_Standards/WikiData/WD~project,170584]]'
- '[[_Standards/WikiData/WD~publication,732577]]'
- '[[_Standards/WikiData/WD~tertiary_source,1063801]]'
- '[[_Standards/WikiData/WD~instructional_materials,1348645]]'
- '[[_Standards/WikiData/WD~written_work,47461344]]'
- '[[_Standards/WikiData/WD~document,49848]]'
instance_of: '[[_Standards/WikiData/WD~literary_genre,223393]]'
on_focus_list_of_Wikimedia_project: '[[_Standards/WikiData/WD~Open_Culture_GLAM_Glossary,115123216]]'
properties_for_this_type: has edition or translation
U_S_National_Archives_Identifier:
- 10644302
- 10634148
ACM_Classification_Code_2012_: 10002946
Universal_Decimal_Classification: 030
UNSPSC_Code: 55101524
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q150%20%28fra%29-Visiteur%20Journ%C3%A9e%202%20%28Madehub%29-ouvrage%20de%20r%C3%A9f%C3%A9rence.wav
UMLS_CUI: C0034922
MeSH_tree_code: L01.462.500.682.192.836
Commons_category: Reference works
dv_has_:
  name_:
    af: naslaanwerk
    ar: عمل مرجعي
    arz: عمل مرجعى
    as: প্ৰসংগ পুথি
    ast: obra de referencia
    ba: белешмә
    be: даведнік
    be_tarask: даведнік
    bg: справочник
    bn: আকর গ্রন্থ
    bo: ཡོ་བྱད་དཔེ་ཆ།
    br: oberenn dave
    bs: referentni rad
    ca: obra de referència
    ckb: کاری سەرچاوە
    cs: referenční dílo
    cv: Справочник
    cy: gwaith cyfeiriol
    da: opslagsværk
    de: Nachschlagewerk
    el: έργο αναφοράς
    en: reference work
    en_ca: Reference work
    en_gb: reference work
    eo: referencverko
    es: obra de referencia
    et: teatmeteos
    eu: erreferentzia-lan
    fa: کتاب مرجع
    fi: hakuteos
    fr: ouvrage de référence
    ga: saothar tagartha
    gl: obra de referencia
    ha: Manazarta
    he: ספר יעץ
    hr: referentno djelo
    hu: referenciamunka
    hy: տեղեկատու
    id: karya referensi
    it: opera di consultazione
    ja: 参考図書
    kk: Анықтамалық басылым
    ko: 참고서
    la: opus referentiale
    lt: žinynas
    lv: uzziņu līdzeklis
    mk: прирачно дело
    ml: ഗ്രന്ഥം
    mn: Лавлагааны бүтээл
    ms: karya rujukan
    nb: oppslagsverk
    nds: Nokieksel
    nl: naslagwerk
    nn: oppslagsverk
    oc: òbra de referencia
    pl: praca referencyjna
    pt: obra de referência
    pt_br: obra de referência
    ro: lucrare de referință
    ru: справочник
    sco: reference wirk
    se: ohcangirji
    sk: Príručková publikácia
    sl: referenčno delo
    smn: Uuccâmkirje
    sms: ooccâmǩeʹrjj
    sq: vepër referimi
    sr: референтно дело
    sr_ec: референтни рад
    sv: referenslitteratur
    tg: маълумотнома
    th: งานอ้างอิง
    tl: sangguniang gawa
    tr: kaynak kitap
    tt: белешмәлек
    tt_cyrl: белешмәлек
    tt_latn: beleşmäleq
    tum: Chilwero cha kubekerapo
    uk: довідник
    ur: حوالہ جاتی مواد
    vec: òpara de refarensa
    vi: tài liệu tham khảo
    yo: iṣẹ́ ìtọ́kasí
    zh: 工具书
    zh_cn: 工具书
    zh_hans: 工具书
    zh_hant: 工具書
    zh_hk: 工具書
    zh_tw: 工具書
    zu: Izibhalo zamaphatho
---
# [[Reference_Work]] 

#is_/same_as :: [[../../../../../WikiData/WD~reference_work,13136|WD~reference_work,13136]] 

## #has_/text_of_/abstract 

> A reference work is a document, such as a paper, book or periodical 
> (or their electronic equivalents), to which one can refer for information. 
> 
> The information is intended to be found quickly when needed. 
> Such works are usually referred to for particular pieces of information, 
> rather than read beginning to end. 
> 
> The writing style used in these works is informative; 
> the authors avoid opinions and the use of the first person, and emphasize facts.
>
> Indices are a common navigation feature in many types of reference works. 
> Many reference works are put together by a team of contributors 
> whose work is coordinated by one or more editors, rather than by an individual author. 
> 
> Updated editions are usually published as needed, 
> in some cases annually, such as Whitaker's Almanack, and Who's Who.
>
> Reference works include textbooks, almanacs, atlases, bibliographies, 
> biographical sources, catalogs such as library catalogs and art catalogs, 
> concordances, dictionaries, directories such as business directories and telephone directories, discographies, encyclopedias, filmographies, gazetteers, glossaries, handbooks, indices such as bibliographic indices and citation indices, manuals, research guides, thesauruses, and yearbooks. 
> 
> Reference works, while traditionally printed, are often available in electronic form 
> and can be obtained as reference software, CD-ROMs, DVDs, or online through the Internet. 
> 
> Wikipedia, an online encyclopedia, is both the largest and the most-read reference work in history.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Reference%20work) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Communication/Media/Creative_Work/Digital_Document/Reference_Work|Reference_Work]] 

### #is_/same_as :: [[/_public/Society/Communication/Media/Creative_Work/Digital_Document/Reference_Work.public|Reference_Work.public]] 

### #is_/same_as :: [[/_internal/Society/Communication/Media/Creative_Work/Digital_Document/Reference_Work.internal|Reference_Work.internal]] 

### #is_/same_as :: [[/_protect/Society/Communication/Media/Creative_Work/Digital_Document/Reference_Work.protect|Reference_Work.protect]] 

### #is_/same_as :: [[/_private/Society/Communication/Media/Creative_Work/Digital_Document/Reference_Work.private|Reference_Work.private]] 

### #is_/same_as :: [[/_personal/Society/Communication/Media/Creative_Work/Digital_Document/Reference_Work.personal|Reference_Work.personal]] 

### #is_/same_as :: [[/_secret/Society/Communication/Media/Creative_Work/Digital_Document/Reference_Work.secret|Reference_Work.secret]] 

