---
aliases:
- musi Sutoku
- Sudoko
- Sudoku
- suduku
- Swdocw
- szúdoku
- Sò͘-to̍k
- Sūdoku
- Σουντόκου
- Судоку
- Սուդոկու
- סודאקו
- סודוקו
- سودوكو
- سودوکو
- سوودۆوکوو
- सु डोकु
- सुडोकू
- ਸੁਡੋਕੂ
- સુડોકુ
- சுடோக்கு
- సుడోకు
- ಸುಡೋಕು
- സുഡോക്കു
- සූදකු
- ซูโดะกุ
- ဆူဒိုကု
- 数独
- 數獨
- 스도쿠
has_id_wikidata: Q44914
based_on: '[[_Standards/WikiData/WD~Latin_square,679367]]'
subclass_of:
- '[[_Standards/WikiData/WD~mind_sport,1188693]]'
- '[[_Standards/WikiData/WD~puzzle,13698]]'
instance_of:
- '[[_Standards/WikiData/WD~mind_sport,1188693]]'
- '[[_Standards/WikiData/WD~logic_puzzle,1774662]]'
- '[[_Standards/WikiData/WD~riddle,47054]]'
- '[[_Standards/WikiData/WD~hobby,47728]]'
discoverer_or_inventor: '[[_Standards/WikiData/WD~Howard_Garns,1631817]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137]]'
different_from: '[[_Standards/WikiData/WD~Kaodoku,47004782]]'
genre: '[[_Standards/WikiData/WD~puzzle,13698]]'
social_media_followers: 16615
OmegaWiki_Defined_Meaning: 715964
Commons_category: Sudoku
subreddit: sudoku
publication_date: 1979-01-01T00:00:00Z
image:
- http://commons.wikimedia.org/wiki/Special:FilePath/Samurai-sudoku.png
- http://commons.wikimedia.org/wiki/Special:FilePath/Sudoku%20Puzzle%20by%20L2G-20050714%20standardized%20layout.svg
Stack_Exchange_tag: https://stackoverflow.com/tags/sudoku
Baidu_Tieba_name: 数独
dv_has_:
  name_:
    af: Sudoku
    an: Sudoku
    ar: سودوكو
    ast: Sudoku
    az: Sudoku
    ba: Судоку
    be-tarask: судоку
    bg: Судоку
    br: Sudoku
    bs: Sudoku
    ca: sudoku
    ckb: سوودۆوکوو
    cs: sudoku
    cy: Swdocw
    da: sudoku
    de: Sudoku
    el: Σουντόκου
    en: sudoku
    eo: Sudoko
    es: sudoku
    et: Sudoku
    eu: Sudoku
    fa: سودوکو
    fi: sudoku
    fr: sudoku
    fy: Sudoku
    ga: Sūdoku
    gl: Sudoku
    gu: સુડોકુ
    he: סודוקו
    hi: सु डोकु
    hr: Sudoku
    hu: szúdoku
    hy: Սուդոկու
    ia: sudoku
    id: Sudoku
    io: Sudoku
    is: Sudoku
    it: sudoku
    ja: 数独
    jv: Sudoku
    kab: suduku
    ki: Sudoku
    kk: Судоку
    kn: ಸುಡೋಕು
    ko: 스도쿠
    ku: Sudoku
    la: Sudoku
    lb: Sudoku
    lt: Sudoku
    lv: Sudoku
    lzh: 數獨
    mk: Судоку
    ml: സുഡോക്കു
    mr: सुडोकू
    ms: Sudoku
    my: ဆူဒိုကု
    nan: Sò͘-to̍k
    nb: sudoku
    nds: Sudoku
    nl: sudoku
    nn: sudoku
    oc: Sudoku
    pa: ਸੁਡੋਕੂ
    pl: Sudoku
    pt: sudoku
    pt-br: Sudoku
    ro: sudoku
    ru: Судоку
    sco: Sudoku
    sh: Sudoku
    si: සූදකු
    sk: sudoku
    sl: sudoku
    sq: Sudoku
    sr: Судоку
    su: Sudoku
    sv: sudoku
    ta: சுடோக்கு
    te: సుడోకు
    th: ซูโดะกุ
    tl: Sudoku
    tok: musi Sutoku
    tr: Sudoku
    uk: Судоку
    uz: Sudoku
    vi: Sudoku
    wa: Sudoku
    war: Sudoku
    wuu: 数独
    yi: סודאקו
    yue: 數獨
    zh: 數獨
    zh-hans: 数独
    zh-hant: 數獨
    zh-hk: 數獨
---
# [[Sudoku]] 

#is_/same_as :: [[../../../../../../WikiData/WD~Sudoku,44914|WD~Sudoku,44914]]  

## #has_/text_of_/abstract 

> **Sudoku** (Japanese: 数独, romanized: sūdoku, lit. 'digit-single'; originally: Number Place) 
> is a logic-based, combinatorial number-placement puzzle. 
> 
> In classic Sudoku, the objective is to fill a 9 × 9 grid with digits 
> so that each column, each row, and each of the nine 3 × 3 subgrids that compose the grid 
> (also called "boxes", "blocks", or "regions") contains all of the digits from 1 to 9. 
> 
> The puzzle setter provides a partially completed grid, which, 
> for a well-posed puzzle, has a single solution.
>
> French newspapers featured similar puzzles in the 19th century, 
> and the modern form of the puzzle first appeared in 1979 puzzle books by Dell Magazines 
> under the name Number Place. 
> 
> However, the puzzle type only began to gain widespread popularity in 1986 
> when it was published by the Japanese puzzle company Nikoli under the name Sudoku, 
> meaning "single number". 
> 
> In newspapers outside of Japan, 
> it first appeared in The Conway Daily Sun (New Hampshire) in September 2004, 
> and then The Times (London) in November 2004, 
> both of which were thanks to the efforts of the Hong Kong judge Wayne Gould, 
> who devised a computer program to rapidly produce unique puzzles.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Sudoku) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Communication/Media/Creative_Work/Game/Puzzle/Sudoku|Sudoku]] 

### #is_/same_as :: [[/_public/Society/Communication/Media/Creative_Work/Game/Puzzle/Sudoku.public|Sudoku.public]] 

### #is_/same_as :: [[/_internal/Society/Communication/Media/Creative_Work/Game/Puzzle/Sudoku.internal|Sudoku.internal]] 

### #is_/same_as :: [[/_protect/Society/Communication/Media/Creative_Work/Game/Puzzle/Sudoku.protect|Sudoku.protect]] 

### #is_/same_as :: [[/_private/Society/Communication/Media/Creative_Work/Game/Puzzle/Sudoku.private|Sudoku.private]] 

### #is_/same_as :: [[/_personal/Society/Communication/Media/Creative_Work/Game/Puzzle/Sudoku.personal|Sudoku.personal]] 

### #is_/same_as :: [[/_secret/Society/Communication/Media/Creative_Work/Game/Puzzle/Sudoku.secret|Sudoku.secret]] 

