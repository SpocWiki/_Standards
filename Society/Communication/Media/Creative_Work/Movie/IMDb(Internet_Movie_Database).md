---
has_id_wikidata: Q37312
dv_has_:
  name_:
    acm: معرف IMDb
    af: Internet-rolprentdatabasis
    an: Internet Movie Database
    ar: قاعدة بيانات الأفلام على الإنترنت
    arz: انترنت موڤى داتابيز
    as: ইণ্টাৰনেট মুভি ডাটাবেছ
    ast: Internet Movie Database
    az: IMDb
    azb: اینترنتده فیلملرین بیلگی قایناغی
    ba: Internet Movie Database
    ban: IMDb
    bar: Internet Movie Database
    bcl: IMDb
    be: Internet Movie Database
    be_tarask: IMDb
    bg: Internet Movie Database
    bho: आइएमडीबी
    bn: আইএমডিবি
    br: IMDb
    bs: Internet Movie Database
    ca: Internet Movie Database
    ce: Internet Movie Database
    ceb: Internet Movie Database
    ckb: بنکەدراوەی ئینتەرنێتیی فیلمەکان
    cs: Internet Movie Database
    cv: Internet Movie Database
    cy: Internet Movie Database
    da: Internet Movie Database
    de: IMDb
    de_ch: Internet Movie Database
    diq: IMDb
    dty: इन्टरनेट चलचित्र डेटाबेस
    el: Internet Movie Database
    en: IMDb
    en_ca: Internet Movie Database
    en_gb: Internet Movie Database
    eo: Interreta filma datumbazo
    es: Internet Movie Database
    et: Internet Movie Database
    eu: Internet Movie Database
    fa: دادگان اینترنتی فیلمها
    fi: Internet Movie Database
    fo: Internet Movie Database
    fr: Internet Movie Database
    frr: IMDb
    fy: Internet Movie Database
    ga: IMDb
    gd: IMDb
    gl: Internet Movie Database
    gsw: Internet Movie Database
    gu: ઇન્ટરનેટ મૂવી ડેટાબેઝ
    gv: Internet Movie Database
    ha: IMDb
    he: IMDb
    hi: इंटरनेट मूवी डेटाबेस
    hif: IMDb
    hr: Internet Movie Database
    hu: Internet Movie Database
    hy: Internet Movie Database
    id: Internet Movie Database
    ig: IMDb
    ilo: Internet Movie Database
    is: Internet Movie Database
    it: Internet Movie Database
    ja: IMDb
    ka: კინოფილმების ინტერნეტ-მონაცემთა ბაზა
    kk: Internet Movie Database
    kl: Internet Movie Database
    kn: ಐ ಎಮ್ ಡಿ ಬಿ
    ko: 인터넷 영화 데이터베이스
    ku: IMDb
    ky: Internet Movie Database
    la: Internet Movie Database
    lb: Internet Movie Database
    lo: ອິນເຕີເນັດ ມູວີເດຕາເບສ
    lt: The Internet Movie Database
    lv: Internet Movie Database
    mai: इन्टरनेट चलचित्र भण्डारण
    mk: Семрежна филмска база на податоци
    ml: ഇന്റർനെറ്റ് മൂവി ഡാറ്റാബേസ്
    mr: इंटरनेट मुव्ही डेटाबेस
    ms: Internet Movie Database
    mul: Internet Movie Database
    my: Internet Movie Database
    nan: Bāng-lō͘ Tiān-iáⁿ Chu-liāu-khò͘
    nan-hani: 網路電影資料庫
    nan-hant: 網路電影資料庫
    nan-latn-pehoeji: Bāng-lō͘ Tiān-iáⁿ Chu-liāu-khò͘
    nan-latn-tailo: Bāng-lōo Tiān-iánn Tsu-liāu-khòo
    nb: The Internet Movie Database
    nds: Internet Movie Database
    ne: इन्टरनेट चलचित्र डेटाबेस
    nl: Internet Movie Database
    nn: The Internet Movie Database
    nod: ᩍᨶᨴᩮᩋᩁ᩺ᨶᩮᩢᨲᨾᩪᩅᩦᨯᩮᨲᩣᨷᩮᩈ
    oc: Internet Movie Database
    or: ଇଣ୍ଟରନେଟ ମୁଭି ଡାଟାବେସ
    os: Internet Movie Database
    pa: ਇੰਟਰਨੈੱਟ ਮੂਵੀ ਡੈਟਾਬੇਸ
    pam: Internet Movie Database
    pl: IMDb
    pnb: انٹرنیٹ مووی ڈیٹابیس
    ps: انټرنیټ فلم ډیټابیس
    pt: Internet Movie Database
    pt_br: Internet Movie Database
    ro: Internet Movie Database
    ru: Internet Movie Database
    sa: अन्तरजालचलचित्रसञ्चिका
    sah: Internet Movie Database
    scn: Internet Movie Database
    sco: Internet Movie Database
    sd: انٽرنيٽ مووي ڊيٽابيس
    sh: Internet Movie Database
    si: ඉන්ටර්නෙට් මූවී ඩේටාබේස්
    sk: Internet Movie Database
    sl: Internet Movie Database
    so: Internet Movie Database
    sq: Internet Movie Database
    sr: IMDb
    sr_ec: Internet Movie Database
    sv: Internet Movie Database
    sw: Internet Movie Database
    ta: ஐ.எம்.டி.பி இணையத்தளம்
    te: ఇంటర్నెట్ మూవీ డేటాబేస్
    tg: Бонки Иттилоотии Интернетии Филмҳо
    tg_cyrl: Internet Movie Database
    th: อินเทอร์เน็ตมูวีเดตาเบส
    tl: Internet Movie Database
    tr: Internet Movie Database
    tt: Internet Movie Database
    tt_cyrl: Internet Movie Database
    uk: Internet Movie Database
    ur: انٹرنیٹ مووی ڈیٹابیس
    uz: Internet Movie Database
    vi: Internet Movie Database
    wuu: 互聯網電影數據庫
    xmf: კინოფილმეფიშ ინტერნეტ-მუნაჩემეფიშ ბაზა
    yi: IMDb
    yo: Internet Movie Database
    yue: 互聯網電影資料庫
    zgh: ⴰⵢ ⵉⵎ ⴷⵉ ⴱⵉ
    zh: 互联网电影数据库
    zh_cn: 互联网电影数据库
    zh_hans: 互联网电影数据库
    zh_hant: 網路電影資料庫
    zh_tw: 網路電影資料庫
    zu: Internet Movie Database
aliases:
- Bāng-lōo Tiān-iánn Tsu-liāu-khòo
- Bāng-lō͘ Tiān-iáⁿ Chu-liāu-khò͘
- IMDb
- Internet Movie Database
- Internet-rolprentdatabasis
- Interreta filma datumbazo
- The Internet Movie Database
- Бонки Иттилоотии Интернетии Филмҳо
- Семрежна филмска база на податоци
- انترنت موڤى داتابيز
- انٹرنیٹ مووی ڈیٹابیس
- انټرنیټ فلم ډیټابیس
- انٽرنيٽ مووي ڊيٽابيس
- اینترنتده فیلملرین بیلگی قایناغی
- بنکەدراوەی ئینتەرنێتیی فیلمەکان
- دادگان اینترنتی فیلمها
- قاعدة بيانات الأفلام على الإنترنت
- معرف IMDb
- अन्तरजालचलचित्रसञ्चिका
- आइएमडीबी
- इंटरनेट मुव्ही डेटाबेस
- इंटरनेट मूवी डेटाबेस
- इन्टरनेट चलचित्र डेटाबेस
- इन्टरनेट चलचित्र भण्डारण
- আইএমডিবি
- ইণ্টাৰনেট মুভি ডাটাবেছ
- ਇੰਟਰਨੈੱਟ ਮੂਵੀ ਡੈਟਾਬੇਸ
- ઇન્ટરનેટ મૂવી ડેટાબેઝ
- ଇଣ୍ଟରନେଟ ମୁଭି ଡାଟାବେସ
- ஐ.எம்.டி.பி இணையத்தளம்
- ఇంటర్నెట్ మూవీ డేటాబేస్
- ಐ ಎಮ್ ಡಿ ಬಿ
- ഇന്റർനെറ്റ് മൂവി ഡാറ്റാബേസ്
- ඉන්ටර්නෙට් මූවී ඩේටාබේස්
- อินเทอร์เน็ตมูวีเดตาเบส
- ອິນເຕີເນັດ ມູວີເດຕາເບສ
- კინოფილმების ინტერნეტ-მონაცემთა ბაზა
- კინოფილმეფიშ ინტერნეტ-მუნაჩემეფიშ ბაზა
- ᩍᨶᨴᩮᩋᩁ᩺ᨶᩮᩢᨲᨾᩪᩅᩦᨯᩮᨲᩣᨷᩮᩈ
- ⴰⵢ ⵉⵎ ⴷⵉ ⴱⵉ
- 互联网电影数据库
- 互聯網電影數據庫
- 互聯網電影資料庫
- 網路電影資料庫
- 인터넷 영화 데이터베이스
---

# [[IMDb(Internet_Movie_Database)]] 

#is_/same_as :: [[_Standards/WikiData/WD~Internet_Movie_Database,37312|WD~Internet_Movie_Database,37312]]  
#is_/similar_to :: [[OMDB(Open_Media_Database)]]  

## #has_/text_of_/abstract 

> IMDb, historically known as the Internet Movie Database, 
> is an online database of information related to films, television series, podcasts, home videos, video games, and streaming content online – 
> including cast, production crew and biographies, plot summaries, trivia, ratings, 
> and fan and critical reviews. 
> 
> IMDb began as a fan-operated movie database on the Usenet group "rec.arts.movies" 
> in 1990, and moved to the Web in 1993. 
> Since 1998, it has been owned and operated by IMDb.com, Inc., a subsidiary of Amazon.
>
> The site's message boards were disabled in February 2017. 
> As of 2024, IMDb was the 51st most visited website on the Internet, as ranked by Semrush. 
> As of March 2022, the database contained some 10.1 million titles 
> (including television episodes), 11.5 million person records, 
> and 83 million registered users.
>
> [Wikipedia](https://en.wikipedia.org/wiki/IMDb) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[IMDb(Internet_Movie_Database)|IMDb(Internet_Movie_Database)]] 

### #is_/same_as :: [[/_public/Society/Communication/Media/Movie/IMDb(Internet_Movie_Database).public|IMDb(Internet_Movie_Database).public]] 

### #is_/same_as :: [[/_internal/Society/Communication/Media/Movie/IMDb(Internet_Movie_Database).internal|IMDb(Internet_Movie_Database).internal]] 

### #is_/same_as :: [[/_protect/Society/Communication/Media/Movie/IMDb(Internet_Movie_Database).protect|IMDb(Internet_Movie_Database).protect]] 

### #is_/same_as :: [[/_private/Society/Communication/Media/Movie/IMDb(Internet_Movie_Database).private|IMDb(Internet_Movie_Database).private]] 

### #is_/same_as :: [[/_personal/Society/Communication/Media/Movie/IMDb(Internet_Movie_Database).personal|IMDb(Internet_Movie_Database).personal]] 

### #is_/same_as :: [[/_secret/Society/Communication/Media/Movie/IMDb(Internet_Movie_Database).secret|IMDb(Internet_Movie_Database).secret]] 

