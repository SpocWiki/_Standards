---
has_time_started: 1895-01-01
aliases:
- diffusion radiophonique
- emissió de ràdio
- hòng-sàng
- Hörfunk
- irrati
- kasafidie
- komunikacioni medijum
- kringkasting
- oddajanje radija
- pag-broadcast ha radyo
- penyiaran tetuang udara
- phát thanh
- radialismo
- radiering
- radio
- radio broadcasting
- radio stanica
- Radio-brodkasto
- radio-uitsending
- radiodiffusione
- radiodifusión
- radiodifuziune
- radiodyfuzja
- radioeshittirish
- radiointi
- radiokomunikado
- radiosakari
- radiouitzending
- radyo yayıncılığı
- radyodifizyon
- raidió
- ringhääling
- rozhlas
- ràdio
- rádio
- rádióadás
- siaran
- transmetimi radiofonik
- ūtwyrp
- ραδιοφωνία
- радіомовлення
- радио
- радиовещание
- радиоемитување
- радиоразпръскване
- радиоуктуруу
- радиофидиуæн
- радыёвяшчанне
- радыёвяшчаньне
- суханпарокании родиюӣ
- ռադիոհեռարձակում
- רדיו
- إذاعة
- البث التلفزي
- ریڈیو اسٹیشن
- سخنپراکنی رادیویی
- نشرکاری
- وەشاندنی ڕادیۆیی
- प्रसारण
- ஒலிபரப்பு
- റേഡിയോ
- 無線放送
- 电台广播
- 電台廣播
- 방송
has_id_wikidata: Q273623
uses:
- '[[_Standards/WikiData/WD~radio_receiver,159391]]'
- '[[_Standards/WikiData/WD~radio_program,1555508]]'
- '[[_Standards/WikiData/WD~Q1644403,1644403]]'
- '[[_Standards/WikiData/WD~radio,872]]'
history_of_topic:
- '[[_Standards/WikiData/WD~history_of_radio,1517374]]'
- '[[_Standards/WikiData/WD~timeline_of_radio,4501817]]'
studied_by: '[[_Standards/WikiData/WD~broadcast_engineering,2176252]]'
different_from:
- '[[_Standards/WikiData/WD~radio_reception,3537732]]'
- '[[_Standards/WikiData/WD~radio_,12830760]]'
- '[[_Standards/WikiData/WD~radio,872]]'
has_use: '[[_Standards/WikiData/WD~information_exchange,6031064]]'
Wikimedia_outline: '[[_Standards/WikiData/WD~outline_of_radio,7112714]]'
subclass_of: '[[_Standards/WikiData/WD~broadcasting,15078788]]'
inception: 1895-01-01T00:00:00Z
DPLA_subject_term: Radio broadcasting
Commons_category: Radio broadcasting
Dewey_Decimal_Classification:
- 384.5
- 302.2344
- 384.54
- 791.44
IPTC_NewsCode: mediatopic/20000050
dv_has_:
  name_:
    af: radio-uitsending
    ak: kasafidie
    ang: ūtwyrp
    ar: إذاعة
    arq: البث التلفزي
    ast: radiodifusión
    be: радыёвяшчанне
    be-tarask: радыёвяшчаньне
    bg: радиоразпръскване
    ca: emissió de ràdio
    ckb: وەشاندنی ڕادیۆیی
    cs: rozhlas
    da: radio
    de: Hörfunk
    el: ραδιοφωνία
    en: radio broadcasting
    eo: radiokomunikado
    es: radiodifusión
    et: ringhääling
    eu: irrati
    fa: سخنپراکنی رادیویی
    fi: radiointi
    fr: diffusion radiophonique
    ga: raidió
    gl: radiodifusión
    he: רדיו
    hi: प्रसारण
    hif: radio
    hr: radio stanica
    ht: radyodifizyon
    hu: rádióadás
    hy: ռադիոհեռարձակում
    id: siaran
    io: Radio-brodkasto
    it: radiodiffusione
    ja: 無線放送
    ko: 방송
    ky: радиоуктуруу
    lv: radiosakari
    mk: радиоемитување
    ml: റേഡിയോ
    mn: радио
    ms: penyiaran tetuang udara
    nan: hòng-sàng
    nb: kringkasting
    nl: radiouitzending
    nn: kringkasting
    oc: ràdio
    os: радиофидиуæн
    pl: radiodyfuzja
    pnb: ریڈیو اسٹیشن
    pt-br: radialismo
    ro: radiodifuziune
    ru: радиовещание
    se: rádio
    sl: oddajanje radija
    smn: radio
    sms: radio
    sq: transmetimi radiofonik
    sr: komunikacioni medijum
    su: siaran
    sv: radiering
    ta: ஒலிபரப்பு
    tg: суханпарокании родиюӣ
    tr: radyo yayıncılığı
    uk: радіомовлення
    ur: نشرکاری
    uz: radioeshittirish
    vec: ràdio
    vi: phát thanh
    war: pag-broadcast ha radyo
    wuu: 电台广播
    yue: 電台廣播
    zh: 電台廣播
    zh-cn: 电台广播
    zh-hans: 电台广播
    zh-hant: 電台廣播
    zh-hk: 電台廣播
    zh-mo: 電台廣播
    zh-my: 电台广播
    zh-sg: 电台广播
    zh-tw: 電台廣播
---
# [[Radio_Broadcasting]] 

#is_/same_as :: [[WD~Radio_broadcasting,273623]] 

## #has_/text_of_/abstract 

> Radio broadcasting is the transmission of electromagnetic radiation (radio waves) to receivers over a wide territory. 
> Most broadcasts are audio (sound), sometimes with embedded metadata. 
> Listeners need a broadcast radio receiver to pick up these signals. 
> 
> "Terrestrial" broadcasts, including AM, FM and DAB stations, originate signals from a land-based transmitter, 
> while "satellite radio" signals originate from a satellite in Earth orbit.
>
> Individual own programming, or are affiliated with a radio network that provides content, either in broadcast syndication or by simulcasting, or both. 
> 
> The most common transmission technologies are analog and digital signals.  
> 
> Analog radio uses one of two modulation methods: 
> - amplitude modulation, used by AM radio, or 
> - frequency modulation, for FM radio.  
> 
> A newer technique, digital radio stations, transmit using one of several different digital audio standards, such as 
> - DAB (Digital Audio Broadcasting), 
> - HD radio, or 
> - DRM (Digital Radio Mondiale).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Radio%20broadcasting) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Society/Communication/Communication_Dimension/Space-Communication/Broadcast_Channel/Radio_Channel/Radio_Broadcasting|Radio_Broadcasting]] 

### #is_/same_as :: [[/_public/Society/Communication/Communication_Dimension/Space-Communication/Broadcast_Channel/Radio_Channel/Radio_Broadcasting.public|Radio_Broadcasting.public]] 

### #is_/same_as :: [[/_internal/Society/Communication/Communication_Dimension/Space-Communication/Broadcast_Channel/Radio_Channel/Radio_Broadcasting.internal|Radio_Broadcasting.internal]] 

### #is_/same_as :: [[/_protect/Society/Communication/Communication_Dimension/Space-Communication/Broadcast_Channel/Radio_Channel/Radio_Broadcasting.protect|Radio_Broadcasting.protect]] 

### #is_/same_as :: [[/_private/Society/Communication/Communication_Dimension/Space-Communication/Broadcast_Channel/Radio_Channel/Radio_Broadcasting.private|Radio_Broadcasting.private]] 

### #is_/same_as :: [[/_personal/Society/Communication/Communication_Dimension/Space-Communication/Broadcast_Channel/Radio_Channel/Radio_Broadcasting.personal|Radio_Broadcasting.personal]] 

### #is_/same_as :: [[/_secret/Society/Communication/Communication_Dimension/Space-Communication/Broadcast_Channel/Radio_Channel/Radio_Broadcasting.secret|Radio_Broadcasting.secret]] 

