---
aliases:
  - Gas-fired power plant
  - gas-fired power station
  - natural gas power plant
  - methane gas power plant
  - Gaskraftwerk
has_id_wikidata: Q2944640
subclass_of: "[[_Standards/WikiData/WD~thermal_power_station,200297]]"
on_focus_list_of_Wikimedia_project: "[[_Standards/WikiData/WD~WikiProject_Climate_change,15305047]]"
topic_s_main_category: "[[_Standards/WikiData/WD~Category_Gas_fired_power_stations,54356426]]"
carbon_footprint: 490
image:
  - http://commons.wikimedia.org/wiki/Special:FilePath/Deponiegas-Kraftwerk%20Beselich%201990.jpg
  - http://commons.wikimedia.org/wiki/Special:FilePath/Gaskraftwerk.PNG
Commons_category: Gas-fired power plants
OpenStreetMap_tag:
  - generator:source=gas
  - plant:source=gas
---

# [[Gas-fired_Power_Plant]] 

#is_/same_as :: [[WD~Gas-fired_power_plant,2944640]] 

## #has_/text_of_/abstract 

> A **Gas-fired power plant**, sometimes referred to as gas-fired power station, 
> natural gas power plant, or methane gas power plant, 
> is a thermal power station that burns natural gas to generate electricity. 
> 
> Gas-fired power plants generate almost a quarter of world electricity 
> and are significant sources of greenhouse gas emissions. 
> 
> Most energy planning relies on gas fired plants to provide 
> dispatchable energy generation to **compensate for variable renewable energy deficits**, 
> where hydropower or interconnectors are not available. 
> In the early 2020s batteries became competitive with gas peaker plants. 
> 
> Some natural gas plants are dual fire with other kinds of fuels, such as oil, coal or hydrogen. 
> 
> Most analysts don't think natural gas turbines will be successfully 
> converted to hydrogen as part of the energy transition, becoming a stranded asset.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Gas-fired%20power%20plant)  

