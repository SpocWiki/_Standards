---
aliases:
- antañiqiq ch'ipachina
- argraffydd
- Drucker
- drukarka
- ebimiselo
- enprimri
- estampadoira
- ilo li lon e sitelen tan ilo
- imprentadora
- impresora
- impressora
- impressorium
- Imprimante
- imprimantă
- inprimagailu
- kichapishi
- kinoke masinahikepitcikan
- lia̍t-ìn-ki
- mesin apdrek
- moullerez
- máy in
- neşırxaney
- nyomtató
- pencetak
- pencetak komputer
- pisač
- počítačová tiskárna
- prentari
- prenter
- printer
- Printer(Computing)
- printeri
- printeris
- printilo
- printéir
- printеr
- priʹnttjem-mašina
- rekenaardrukker
- sicrirece
- skrivar
- skrivare
- skriver
- spausdintuvas
- stampanta
- stampante
- stampanti
- stanpante
- tiskalnik
- tjaelije
- tjálan
- tlačiareň
- tulostin
- yazıcı
- çaper
- čálán
- čááláán
- štampač
- εκτυπωτής
- басып шығарғыш
- печатач
- принтер
- прынтар
- прынтэр
- хэвлэгч
- штампач
- տպիչ
- דרוקער
- מדפסת
- خەتباسقۇ (پىرىنتىر)
- طابعة
- طابعه
- پرنٹر
- پرینتر
- چاپکەر
- چاپگر
- प्रिंटर
- प्रिण्टर
- प्रिन्टर
- প্রিন্টার
- প্ৰিন্টাৰ
- ਪ੍ਰਿੰਟਰ
- કમ્પ્યુટર પ્રિન્ટર
- கணினி அச்சுப்பொறி
- కంప్యూటర్ ప్రింటర్
- ಪ್ರಿಂಟರ್
- പ്രിന്റർ
- เครื่องพิมพ์
- ຈັກພິມ
- ပရင်တာ
- ၸၢၵ်ႈၾိမ်းလိၵ်ႈ
- პრინტერი
- プリンター
- 列印機
- 印表機
- 打印机
- 打印機
- 프린터
has_id_wikidata: Q82
has_use: '[[_Standards/WikiData/WD~printing_press,144334|WD~printing_press,144334]]'
subclass_of:
- '[[_Standards/WikiData/WD~peripheral,178648|WD~peripheral,178648]]'
- '[[_Standards/WikiData/WD~electronic_device,2858615|WD~electronic_device,2858615]]'
- '[[_Standards/WikiData/WD~printing_machine,12820205|WD~printing_machine,12820205]]'
different_from:
- '[[_Standards/WikiData/WD~Drucker,362123|WD~Drucker,362123]]'
- '[[_Standards/WikiData/WD~Yazıcı,2599843|WD~Yazıcı,2599843]]'
- '[[_Standards/WikiData/WD~printing_company,6500733|WD~printing_company,6500733]]'
- '[[_Standards/WikiData/WD~Printer,10953985|WD~Printer,10953985]]'
has_characteristic: '[[_Standards/WikiData/WD~printer_resolution,54315216|WD~printer_resolution,54315216]]'
has_part_s_: '[[_Standards/WikiData/WD~printer_carriage,97584727|WD~printer_carriage,97584727]]'
facet_of: '[[_Standards/WikiData/WD~office_technology,106707749|WD~office_technology,106707749]]'
Nomenclature_for_Museum_Cataloging: 10521
Amazon_com_browse_node: 172635
Commons_gallery: Printer
OmegaWiki_Defined_Meaning: 960286
Dewey_Decimal_Classification:
- 4.77
- 681.62
Commons_category: Printers
image: http://commons.wikimedia.org/wiki/Special:FilePath/HP%20Deskjet%20All%20in%20One%20Printer.jpg
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q150%20%28fra%29-Fabricio%20Cardenas%20%28Culex%29-imprimante.wav
IEV_number: 171-04-46
MCN_code: 8443.16.00
subreddit: printers
Baidu_Tieba_name: 打印机
Unicode_character:
- "\U0001F5A8"
- "\U0001F5B6"
vocalized_name: מַדְפֶּסֶת
OpenStreetMap_tag: amenity=printer
dv_has_:
  name_:
    af: rekenaardrukker
    an: imprentadora
    ar: طابعة
    arz: طابعه
    as: প্ৰিন্টাৰ
    ast: imprentadora
    atj: kinoke masinahikepitcikan
    az: printer
    ba: принтер
    be: прынтар
    be_tarask: прынтэр
    bew: mesin apdrek
    bg: принтер
    bho: प्रिंटर
    bn: প্রিন্টার
    br: moullerez
    bs: pisač
    ca: impressora
    ckb: چاپکەر
    cs: počítačová tiskárna
    cy: argraffydd
    da: printer
    de: Drucker
    de_ch: Drucker
    diq: neşırxaney
    el: εκτυπωτής
    en: printer
    en_ca: printer
    en_gb: printer
    eo: printilo
    es: impresora
    et: printer
    eu: inprimagailu
    fa: چاپگر
    fi: tulostin
    fr: imprimante
    fy: printer
    ga: printéir
    gcr: enprimri
    gl: impresora
    gsw: Drucker
    gu: કમ્પ્યુટર પ્રિન્ટર
    he: מדפסת
    hi: प्रिण्टर
    hr: pisač
    hu: nyomtató
    hy: տպիչ
    id: pencetak
    is: prentari
    it: stampante
    ja: プリンター
    jv: pencetak
    ka: პრინტერი
    kaa: printеr
    kk: басып шығарғыш
    kn: ಪ್ರಿಂಟರ್
    ko: 프린터
    krc: принтер
    ku: çaper
    kv: принтер
    ky: принтер
    la: impressorium
    lb: Printer
    lmo: stampanta
    ln: ebimiselo
    lo: ຈັກພິມ
    lt: spausdintuvas
    lv: printeris
    mhr: принтер
    mk: печатач
    ml: പ്രിന്റർ
    mn: хэвлэгч
    mr: प्रिंटर
    ms: pencetak komputer
    my: ပရင်တာ
    mzn: پرینتر
    nan: lia̍t-ìn-ki
    nb: skriver
    ne: प्रिन्टर
    nl: printer
    nn: skrivar
    oc: estampadoira
    pa: ਪ੍ਰਿੰਟਰ
    pl: drukarka
    pnb: پرنٹر
    pt: impressora
    pt_br: impressora
    qu: antañiqiq ch'ipachina
    ro: imprimantă
    ru: принтер
    scn: stampanti
    sco: prenter
    se: čálán
    sg: Imprimante
    sh: printer
    shn: ၸၢၵ်ႈၾိမ်းလိၵ်ႈ
    sk: tlačiareň
    sl: tiskalnik
    sma: tjaelije
    smj: tjálan
    smn: čááláán
    sms: priʹnttjem-mašina
    sq: printeri
    sr: штампач
    sr_ec: штампач
    sr_el: štampač
    sv: skrivare
    sw: kichapishi
    ta: கணினி அச்சுப்பொறி
    te: కంప్యూటర్ ప్రింటర్
    tg: принтер
    th: เครื่องพิมพ์
    tok: ilo li lon e sitelen tan ilo
    tr: yazıcı
    ug: خەتباسقۇ (پىرىنتىر)
    uk: принтер
    ur: پرنٹر
    uz: printer
    vec: stanpante
    vi: máy in
    wa: sicrirece
    wuu: 打印机
    xmf: პრინტერი
    yi: דרוקער
    yue: 列印機
    zh: 打印機
    zh_cn: 打印机
    zh_hans: 打印机
    zh_hant: 打印機
    zh_hk: 列印機
    zh_mo: 列印機
    zh-my: 打印机
    zh_sg: 打印机
    zh_tw: 印表機
---

# [[Printer(Computing)]] 🖨 



#is_/same_as :: [[_Standards/WikiData/WD~Printer,82|WD~Printer,82]] 

## #has_/text_of_/abstract 

> A printer is a peripheral machine which makes a durable representation of graphics or text, 
> usually on paper. 
> 
> While most output is human-readable, bar 
> code printers are an example of an expanded use for printers. 
> 
> Different types of printers include 3D printers, inkjet printers, laser printers, and thermal printers.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Printer%20(computing)) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/IT/Computer_Hardware/Peripheral/Printer(Computing)|Printer(Computing)]] 

### #is_/same_as :: [[/_public/Technology/IT/Computer_Hardware/Peripheral/Printer(Computing).public|Printer(Computing).public]] 

### #is_/same_as :: [[/_internal/Technology/IT/Computer_Hardware/Peripheral/Printer(Computing).internal|Printer(Computing).internal]] 

### #is_/same_as :: [[/_protect/Technology/IT/Computer_Hardware/Peripheral/Printer(Computing).protect|Printer(Computing).protect]] 

### #is_/same_as :: [[/_private/Technology/IT/Computer_Hardware/Peripheral/Printer(Computing).private|Printer(Computing).private]] 

### #is_/same_as :: [[/_personal/Technology/IT/Computer_Hardware/Peripheral/Printer(Computing).personal|Printer(Computing).personal]] 

### #is_/same_as :: [[/_secret/Technology/IT/Computer_Hardware/Peripheral/Printer(Computing).secret|Printer(Computing).secret]] 

