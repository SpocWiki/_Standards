---
aliases:
- ahli sains komputer
- amontyoriether
- bilgisayar bilimci
- cientista da computação
- cientista de computação
- computer scientist
- Computer_Scientist
- datalog
- datavetare
- datorzinātnieks
- enhormáticu teóricu
- eolaí ríomhaireachta
- gwyddonydd cyfrifiadurol
- ilmuwan komputer
- Imformatiker
- informaatik
- informatician
- informaticien ou informaticienne
- informatico
- informaticus
- informatik
- informatikar
- informatikari
- informatikaŕ
- Informatiker
- informatikus
- informatičar
- informatyk
- informàtego
- informàtic
- informàtico
- informático teórico
- informáticu teóricu
- infôrmatikî
- kompiuterių mokslininkas
- komputero-ciencisto
- komputosciencisto
- kompyuter alimi
- Kompyuter olimi
- kompyuter qániygesi
- kompûternas
- mpahay kajimirindra
- neach-saidheans coimpiutaireachd
- nhà khoa học máy tính
- nünömavan
- počítačový vědec
- računalni znanstvenik
- računalnikar
- računarski naučnik
- rekenaar wetenskaplike
- sheanseyr co-earrooder
- shkencëtar i kompjuterit
- stlennegour
- tietojenkäsittelytieteilijä
- tölvunarfræðingur
- xjenzat informatiku
- Zanayoğê komputeri
- επιστήμονας υπολογιστών
- інформатик
- Информатик
- информатикæйы специалист
- информатика өлкәсендә белгеч
- информатичар
- муҳандиси роёна
- специалист в области информатики
- спецыяліст у галіне інфарматыкі
- спэцыяліст у галіне інфарматыкі
- համակարգչային գիտնական
- אינפארמאטיקער
- מדען מחשב
- أنفورماتيسيان
- اهلي ساءينس کومڤوتر
- زانای کۆمپیوتەر
- عالم حاسوب
- مهندس رایانه
- کمپیوٹر سائنس دان
- کَمپیوٗٹَر ساینَس دان
- कंप्यूटर वैज्ञानिक
- संगणक-विज्ञानी
- কম্পিউটার বিজ্ঞানী
- କମ୍ପ୍ୟୁଟର ବୈଜ୍ଞାନିକ
- கணினி விஞ்ஞானி
- కంప్యూటర్ శాస్త్రవేత్త
- ಗಣಕ ವಿಜ್ಞಾನಿ
- കമ്പ്യൂട്ടര് ഗവേഷകന്
- ინფორმატიკოსი
- ኮምፒተር ሳይንቲስት
- ọkà mmụta sayensị kọmputa
- 电脑科学家
- 計算機科学者
- 計算機科學家
- 计算机科学家
- 電腦科學家
- 컴퓨터 과학자
has_id_wikidata: Q82594
patron_saint: '[[_Standards/WikiData/WD~Isidore_of_Seville,166876]]'
subclass_of:
- '[[_Standards/WikiData/WD~systems_analyst,942569]]'
- '[[_Standards/WikiData/WD~technology_specialist,5157565]]'
- '[[_Standards/WikiData/WD~scientist,901]]'
said_to_be_the_same_as:
- '[[_Standards/WikiData/WD~computer_science_expert,1391300]]'
- '[[_Standards/WikiData/WD~IT_professional,56604309]]'
industry:
- '[[_Standards/WikiData/WD~service_sector,1959446]]'
- '[[_Standards/WikiData/WD~industrial_sector,8148]]'
has_list: '[[_Standards/WikiData/WD~list_of_computer_scientists,2476620]]'
uses:
- '[[_Standards/WikiData/WD~computing_terminology,3457057]]'
- '[[_Standards/WikiData/WD~computer_science_journal,127867413]]'
different_from: '[[_Standards/WikiData/WD~computational_scientist,5157338]]'
category_for_eponymous_categories: '[[_Standards/WikiData/WD~Q59241091,59241091]]'
field_of_this_occupation: '[[_Standards/WikiData/WD~computer_science,21198]]'
instance_of: '[[_Standards/WikiData/WD~profession,28640]]'
KldB_2010_occupation_code: 43124
ISCO_08_occupation_class: 2511
OmegaWiki_Defined_Meaning: 1516968
male_form_of_label:
- informatyk
- informatico
- informaticien
- عالم حاسوب
- informatik
- cientista da computação
- информатичар
- enhormáticu teóricu
- Informatiker
- kompiuterių mokslininkas
- дослідник у галузі інформатики
- hinünömavan
female_form_of_label:
- informatica
- Informatikerin
- informaticienne
- informatica
- informàtica
- cientista da computação
- informatyczka
- מדענית מחשב
- informaticiană
- عَالِمَة حاسوب
- informática teórica
- informatička
- komputikistino
- informática teórica
- enhormática teórica
- informática teórica
- komputatoro-ciencistino
- Informatikerin
- kompiuterių mokslininkė
- datorzinātniece
- računalničarka
- shkencëtare e kompjuterit
- информатичарка
- дослідниця у галузі інформатики
- jinünömavan
Commons_category: Computer scientists
image: http://commons.wikimedia.org/wiki/Special:FilePath/Ingenieur%20reseaux.jpg
Dewey_Decimal_Classification: 004.092
Wolfram_Language_entity_code: Entity["Concept", "ComputerScientist::n3qj3"]
dv_has_:
  name_:
    af: rekenaar wetenskaplike
    am: ኮምፒተር ሳይንቲስት
    ar: عالم حاسوب
    ary: أنفورماتيسيان
    ast: informáticu teóricu
    az: informatik
    be: спецыяліст у галіне інфарматыкі
    be-tarask: спэцыяліст у галіне інфарматыкі
    bg: Информатик
    bn: কম্পিউটার বিজ্ঞানী
    br: stlennegour
    bs: računarski naučnik
    ca: informàtic
    ckb: زانای کۆمپیوتەر
    crh: kompyuter alimi
    crh-latn: kompyuter alimi
    cs: počítačový vědec
    cy: gwyddonydd cyfrifiadurol
    da: datalog
    de: Informatiker
    diq: Zanayoğê komputeri
    dsb: informatikaŕ
    el: επιστήμονας υπολογιστών
    en: computer scientist
    en-ca: computer scientist
    en-gb: computer scientist
    eo: komputosciencisto
    es: informático teórico
    et: informaatik
    eu: informatikari
    ext: enhormáticu teóricu
    fa: مهندس رایانه
    fi: tietojenkäsittelytieteilijä
    fr: informaticien ou informaticienne
    ga: eolaí ríomhaireachta
    gd: neach-saidheans coimpiutaireachd
    gl: informático teórico
    gsw: Imformatiker
    gv: sheanseyr co-earrooder
    he: מדען מחשב
    hi: कंप्यूटर वैज्ञानिक
    hr: računalni znanstvenik
    hsb: informatikar
    hu: informatikus
    hy: համակարգչային գիտնական
    id: ilmuwan komputer
    ig: ọkà mmụta sayensị kọmputa
    io: komputero-ciencisto
    is: tölvunarfræðingur
    it: informatico
    ja: 計算機科学者
    ka: ინფორმატიკოსი
    kaa: kompyuter qániygesi
    kn: ಗಣಕ ವಿಜ್ಞಾನಿ
    ko: 컴퓨터 과학자
    ks: کَمپیوٗٹَر ساینَس دان
    ku: kompûternas
    ku-latn: kompûternas
    kw: amontyoriether
    lb: Informatiker
    lij: informàtico
    lt: kompiuterių mokslininkas
    lv: datorzinātnieks
    mg: mpahay kajimirindra
    mk: информатичар
    ml: കമ്പ്യൂട്ടര് ഗവേഷകന്
    mr: संगणक-विज्ञानी
    ms: ahli sains komputer
    ms-arab: اهلي ساءينس کومڤوتر
    mt: xjenzat informatiku
    nb: informatiker
    nl: informaticus
    nn: informatikar
    oc: informatician
    or: କମ୍ପ୍ୟୁଟର ବୈଜ୍ଞାନିକ
    os: информатикæйы специалист
    pl: informatyk
    pt: cientista de computação
    pt-br: cientista da computação
    ro: informatician
    ru: специалист в области информатики
    sco: computer scientist
    sk: informatik
    sl: računalnikar
    sq: shkencëtar i kompjuterit
    sr: информатичар
    sr-ec: информатичар
    sr-el: informatičar
    sv: datavetare
    ta: கணினி விஞ்ஞானி
    te: కంప్యూటర్ శాస్త్రవేత్త
    tg: муҳандиси роёна
    tl: computer scientist
    tr: bilgisayar bilimci
    tt: информатика өлкәсендә белгеч
    tt-cyrl: информатика өлкәсендә белгеч
    uk: інформатик
    ur: کمپیوٹر سائنس دان
    uz: Kompyuter olimi
    vec: informàtego
    vi: nhà khoa học máy tính
    vo: nünömavan
    wa: infôrmatikî
    yi: אינפארמאטיקער
    yue: 電腦科學家
    zh: 電腦科學家
    zh-cn: 计算机科学家
    zh-hans: 计算机科学家
    zh-hant: 電腦科學家
    zh-hk: 計算機科學家
    zh-mo: 電腦科學家
    zh-my: 电脑科学家
    zh-sg: 电脑科学家
    zh-tw: 電腦科學家
---
# [[Computer_Scientist]] 

#is_/same_as :: [[_Standards/WikiData/WD~computer_scientist,82594]] 

## #has_/text_of_/abstract 

> A **computer scientist** is a scientist who specializes in the academic study of computer science. 
>
> Computer scientists typically work on the theoretical side of computation. 
> Although computer scientists can also focus their work and research on specific areas 
> (such as algorithm and data structure development and design, software engineering, 
> information theory, database theory, theoretical computer science, numerical analysis, 
> programming language theory, compiler, computer graphics, computer vision, robotics, 
> computer architecture, operating system), 
> their foundation is the theoretical study of computing from which these other fields derive.
>
> A primary goal of computer scientists is to develop or validate models, often mathematical, 
> to describe the properties of computational systems (processors, programs, 
> computers interacting with people, computers interacting with other computers, etc.) 
> with an overall objective of discovering designs that yield useful benefits 
> (faster, smaller, cheaper, more precise, etc.).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Computer%20scientist) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/IT/Computer_Science/Computer_Scientist|Computer_Scientist]] 

### #is_/same_as :: [[/_public/Technology/IT/Computer_Science/Computer_Scientist.public|Computer_Scientist.public]] 

### #is_/same_as :: [[/_internal/Technology/IT/Computer_Science/Computer_Scientist.internal|Computer_Scientist.internal]] 

### #is_/same_as :: [[/_protect/Technology/IT/Computer_Science/Computer_Scientist.protect|Computer_Scientist.protect]] 

### #is_/same_as :: [[/_private/Technology/IT/Computer_Science/Computer_Scientist.private|Computer_Scientist.private]] 

### #is_/same_as :: [[/_personal/Technology/IT/Computer_Science/Computer_Scientist.personal|Computer_Scientist.personal]] 

### #is_/same_as :: [[/_secret/Technology/IT/Computer_Science/Computer_Scientist.secret|Computer_Scientist.secret]] 

