---
aliases:
- Ciencia computacional teórica
- ciência da computação teórica
- ilmu komputer teoretis
- Informatega teorica
- informatica teorica
- informatică teoretică
- informatika teoriko
- Informatiko teoriala
- informatique théorique
- Informatyka (teoretyczna)
- Informàtica teòrica
- Khoa học máy tính lý thuyết
- kuramsal bilgisayar bilimi
- Nəzəri informatika
- sains komputer teori
- Teoreetiline informaatika
- teoretická informatika
- Teoretikal na agham pangkompyuter
- teoretisk datalogi
- teoretisk datavetenskap
- teoretična računalniška znanost
- Teoretsko računarstvo
- teoria komputiko
- teorijska informatika
- teorinė informatika
- Theoreetesch Informatik
- theoretical computer science
- Theoretical_Computer_Science
- theoretische informatica
- Theoretische Informatik
- Tietojenkäsittelyteoria
- Θεωρητική Πληροφορική
- теоретическая информатика
- теоретична інформатика
- Теоретична информатика
- теоретска информатика
- теоријска информатика
- тэарэтычная інфарматыка
- տեսական համակարգչային գիտություն
- תאוריה של מדעי המחשב
- علوم نظری رایانه
- معلوماتية نظرية
- তাত্ত্বিক কম্পিউটার বিজ্ঞান
- સૈદ્ધાન્તિક કમ્પ્યુટર વિજ્ઞાન
- သဘောတရားဆိုင်ရာ ကွန်ပျူတာသိပ္ပံ
- 理論計算機科学
- 理論計算機科學
- 理論電腦科學
- 理论计算机科学
- 이론 전산학
has_id_wikidata: Q2878974
has_part_s_:
- '[[_Standards/WikiData/WD~computational_complexity_theory,205084|WD~computational_complexity_theory,205084]]'
- '[[_Standards/WikiData/WD~automata_theory,214526|WD~automata_theory,214526]]'
- '[[_Standards/WikiData/WD~database_theory,4809258|WD~database_theory,4809258]]'
- '[[_Standards/WikiData/WD~algorithmics,13636890|WD~algorithmics,13636890]]'
subclass_of:
- '[[_Standards/WikiData/WD~fundamental_science,580234|WD~fundamental_science,580234]]'
- '[[_Standards/WikiData/WD~mathematics,395|WD~mathematics,395]]'
- '[[_Standards/WikiData/WD~computer_science,21198|WD~computer_science,21198]]'
instance_of:
- '[[_Standards/WikiData/WD~area_of_mathematics,1936384|WD~area_of_mathematics,1936384]]'
- '[[_Standards/WikiData/WD~academic_major,4671286|WD~academic_major,4671286]]'
- '[[_Standards/WikiData/WD~branch_of_computer_science,123370638|WD~branch_of_computer_science,123370638]]'
topic_s_main_Wikimedia_portal: '[[_Standards/WikiData/WD~Portal_Theoretical_computer_science,112676076|WD~Portal_Theoretical_computer_science,112676076]]'
part_of: '[[_Standards/WikiData/WD~computer_science,21198|WD~computer_science,21198]]'
All_Science_Journal_Classification_Codes: 2614
Basisklassifikation: 54.10
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q150%20%28fra%29-0x010C-informatique%20th%C3%A9orique.wav
Commons_category: Theoretical computer science
Stack_Exchange_site_URL: https://cstheory.stackexchange.com
image: http://commons.wikimedia.org/wiki/Special:FilePath/DFAexample.svg
Dewey_Decimal_Classification: 004.0151
dv_has_:
  name_:
    ar: معلوماتية نظرية
    ast: Ciencia computacional teórica
    az: Nəzəri informatika
    be: тэарэтычная інфарматыка
    be_tarask: тэарэтычная інфарматыка
    bg: Теоретична информатика
    bn: তাত্ত্বিক কম্পিউটার বিজ্ঞান
    ca: Informàtica teòrica
    cs: teoretická informatika
    da: teoretisk datalogi
    de: theoretische Informatik
    de_ch: Theoretische Informatik
    el: Θεωρητική Πληροφορική
    en: theoretical computer science
    en_ca: Theoretical computer science
    en_gb: theoretical computer science
    eo: teoria komputiko
    es: ciencia computacional teórica
    et: Teoreetiline informaatika
    eu: informatika teoriko
    fa: علوم نظری رایانه
    fi: Tietojenkäsittelyteoria
    fr: informatique théorique
    gl: Ciencia computacional teórica
    gu: સૈદ્ધાન્તિક કમ્પ્યુટર વિજ્ઞાન
    he: תאוריה של מדעי המחשב
    hr: Teoretsko računarstvo
    hy: տեսական համակարգչային գիտություն
    id: ilmu komputer teoretis
    io: Informatiko teoriala
    it: informatica teorica
    ja: 理論計算機科学
    ko: 이론 전산학
    lb: Theoreetesch Informatik
    lmo: Informatega teorica
    lt: teorinė informatika
    mk: теоретска информатика
    ms: sains komputer teori
    my: သဘောတရားဆိုင်ရာ ကွန်ပျူတာသိပ္ပံ
    nl: theoretische informatica
    pl: Informatyka (teoretyczna)
    pt: ciência da computação teórica
    pt_br: ciência da computação teórica
    ro: informatică teoretică
    ru: теоретическая информатика
    sco: theoretical computer science
    sk: Teoretická informatika
    sl: teoretična računalniška znanost
    sr: теоријска информатика
    sr_ec: теоријска информатика
    sr_el: teorijska informatika
    sv: teoretisk datavetenskap
    tl: Teoretikal na agham pangkompyuter
    tr: kuramsal bilgisayar bilimi
    uk: теоретична інформатика
    vi: Khoa học máy tính lý thuyết
    wuu: 理论计算机科学
    yue: 理論電腦科學
    zh: 理論計算機科學
    zh_cn: 理论计算机科学
---

# [[Theoretical_Computer_Science]] 

#is_/same_as :: [[../../../WikiData/WD~Theoretical_computer_science,2878974|WD~Theoretical_computer_science,2878974]]  

## #has_/text_of_/abstract 

> Theoretical computer science is a subfield of computer science and mathematics 
> that focuses on the abstract and mathematical foundations of computation.
>
> It is difficult to circumscribe the theoretical areas precisely. 
> The ACM's Special Interest Group on Algorithms and Computation Theory (SIGACT) 
> provides the following description:
>
> TCS covers a wide variety of topics including algorithms, data structures, computational complexity, parallel and distributed computation, probabilistic computation, quantum computation, automata theory, information theory, cryptography, program semantics and verification, algorithmic game theory, machine learning, computational biology, computational economics, computational geometry, and computational number theory and algebra. 
> 
> Work in this field is often distinguished by its emphasis on mathematical technique and rigor.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Theoretical%20computer%20science) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/IT/Computer_Science/Theoretical_Computer_Science|Theoretical_Computer_Science]] 

### #is_/same_as :: [[/_public/Technology/IT/Computer_Science/Theoretical_Computer_Science.public|Theoretical_Computer_Science.public]] 

### #is_/same_as :: [[/_internal/Technology/IT/Computer_Science/Theoretical_Computer_Science.internal|Theoretical_Computer_Science.internal]] 

### #is_/same_as :: [[/_protect/Technology/IT/Computer_Science/Theoretical_Computer_Science.protect|Theoretical_Computer_Science.protect]] 

### #is_/same_as :: [[/_private/Technology/IT/Computer_Science/Theoretical_Computer_Science.private|Theoretical_Computer_Science.private]] 

### #is_/same_as :: [[/_personal/Technology/IT/Computer_Science/Theoretical_Computer_Science.personal|Theoretical_Computer_Science.personal]] 

### #is_/same_as :: [[/_secret/Technology/IT/Computer_Science/Theoretical_Computer_Science.secret|Theoretical_Computer_Science.secret]] 

