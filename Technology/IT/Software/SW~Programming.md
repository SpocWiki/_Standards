---
aliases:
- bernamekirin
- bilgisayar programlama
- coding
- Computer programming
- Computer_Programming
- dasturlash
- Komputerala programifado
- kooʹdjummuš
- Kɔmputa so dwumadi ahorow
- lập trình máy tính
- ohjelmistem
- ohjelmointi
- pemrograman
- pengaturcaraan komputer
- programacion d'ordenadors
- programació d'ordinadors
- programación
- programado
- programare
- programasion
- Programassion informàtica
- programavimas
- programazio
- programação de computadores
- programimi kompjuterik
- programiranje
- Programmalastırıw
- programmation
- programmatura computatralis
- programmazion
- programmazione
- programmeerimine
- programmeren
- programmering
- Programmierung
- programmirleme
- Programméierung
- programmēšana
- programnayış
- programovanie
- programování
- programowanie komputerów
- proqramlaşdırma
- računalniško programiranje
- računalno programiranje
- rekenaarprogrammering
- ríomhchlárú
- sampla
- számítógép-programozás
- Ukuhlelelisa isiCikizi
- προγραμματισμός
- Барномасозӣ
- бағдарламалау
- кампутарнае праграмаваньне
- косонрадъяны
- праграмаванне
- програмирање
- програмиране
- програмлау
- программæтæ аразын
- Программăлав
- программалау
- программалоо
- программирование
- программлымаш
- програмування
- програмчлал
- проґрамованя
- ծրագրավորում
- קאמפיוטער פראגראמירן
- תכנות מחשבים
- برمجة
- برمجه
- برنامهنویسی
- بەرنامەسازی
- ڤڠاتورچاراءن کومڤوتر
- کمپيوټري برېښليک
- کمپیوٹر پروگرامنگ
- ߢߍߥߟߊߦߊߟߌ
- क्रमानुदेशन
- संगणक प्रोग्रामिंग
- सङ्गणककार्यविधिः
- কম্পিউটার প্রোগ্রামিং
- কম্পিউটাৰ প্ৰগ্ৰেমিং
- கணினி நிரலாக்கம்
- ಕಂಪ್ಯೂಟರ್ ಪ್ರೋಗ್ರಾಮಿಂಗ್
- കമ്പ്യൂട്ടർ പ്രോഗ്രാമിങ്ങ്
- පරිගණක ක්රමලේඛකරණය
- การเขียนโปรแกรมคอมพิวเตอร์
- កុំព្យូទ័រផ្រូក្រាមមីង (Computer programming)
- ᨄᨆᨚᨁᨛᨑᨆ ᨀᨚᨇᨘᨈᨛᨑᨙ
- コンピュータプログラミング
- 电脑程式设计
- 计算机程序设计
- 電腦程序設計
- 電腦程式設計
- 컴퓨터 프로그래밍
has_id_wikidata: Q80006
has_characteristic:
- '[[/_Standards/WikiData/WD~programming_paradigm,188267|WD~programming_paradigm,188267]]'
- '[[/_Standards/WikiData/WD~programming_language,9143|WD~programming_language,9143]]'
follows: '[[/_Standards/WikiData/WD~pseudocode,189224|WD~pseudocode,189224]]'
subclass_of:
- '[[/_Standards/WikiData/WD~skill,205961|WD~skill,205961]]'
- '[[/_Standards/WikiData/WD~activity,1914636|WD~activity,1914636]]'
partially_coincident_with: '[[/_Standards/WikiData/WD~software_development,638608|WD~software_development,638608]]'
part_of:
- '[[/_Standards/WikiData/WD~software_development,638608|WD~software_development,638608]]'
- '[[/_Standards/WikiData/WD~computer_science,21198|WD~computer_science,21198]]'
practiced_by:
- '[[/_Standards/WikiData/WD~software_engineer,1709010|WD~software_engineer,1709010]]'
- '[[/_Standards/WikiData/WD~programmer,5482740|WD~programmer,5482740]]'
instance_of:
- '[[/_Standards/WikiData/WD~academic_major,4671286|WD~academic_major,4671286]]'
- '[[/_Standards/WikiData/WD~academic_discipline,11862829|WD~academic_discipline,11862829]]'
- '[[/_Standards/WikiData/WD~computer_science_term,66747126|WD~computer_science_term,66747126]]'
Wikimedia_outline: '[[/_Standards/WikiData/WD~outline_of_computer_programming,7112591|WD~outline_of_computer_programming,7112591]]'
topic_s_main_Wikimedia_portal: '[[/_Standards/WikiData/WD~Portal_Computer_programming,10729459|WD~Portal_Computer_programming,10729459]]'
product_or_material_produced: '[[/_Standards/WikiData/WD~computer_program,40056|WD~computer_program,40056]]'
Merchant_Category_Code: 7372
GitHub_topic: programming
subreddit:
- programming
- coding
OmegaWiki_Defined_Meaning: 879583
Classification_of_Instructional_Programs_code: 11.02
social_media_followers:
- 515599
- 5427662
Dewey_Decimal_Classification: 005
Commons_category: Computer programming
image: http://commons.wikimedia.org/wiki/Special:FilePath/Programmer%20writing%20code%20with%20Unit%20Tests.jpg
Universal_Decimal_Classification: 004.42
video: http://commons.wikimedia.org/wiki/Special:FilePath/Live%20Coding%20of%20a%20Simple%20Amazon%20Echo%20%28Alexa%29%20Node.js%20Skill.webm
WordLift_URL: http://data.thenextweb.com/tnw/entity/SW~Programming
dv_has_:
  name_:
    af: rekenaarprogrammering
    an: Programación
    ar: برمجة
    arz: برمجه
    as: কম্পিউটাৰ প্ৰগ্ৰেমিং
    ast: programación
    az: proqramlaşdırma
    ba: программалау
    be: праграмаванне
    be_tarask: кампутарнае праграмаваньне
    bg: програмиране
    bn: কম্পিউটার প্রোগ্রামিং
    bs: programiranje
    bug: ᨄᨆᨚᨁᨛᨑᨆ ᨀᨚᨇᨘᨈᨛᨑᨙ
    ca: programació d'ordinadors
    ckb: بەرنامەسازی
    cs: programování
    cv: Программăлав
    da: programmering
    de: Programmierung
    diq: programnayış
    el: προγραμματισμός
    en: computer programming
    en_gb: computer programming
    en-us: computer programming
    eo: programado
    es: programación
    et: programmeerimine
    eu: programazio
    fa: برنامهنویسی
    fi: ohjelmointi
    fr: programmation
    ga: ríomhchlárú
    gl: programación
    he: תכנות מחשבים
    hi: क्रमानुदेशन
    hr: računalno programiranje
    hu: számítógép-programozás
    hy: ծրագրավորում
    id: pemrograman
    io: Komputerala programifado
    it: programmazione
    ja: コンピュータプログラミング
    jbo: sampla
    kaa: Programmalastırıw
    kk: бағдарламалау
    km: កុំព្យូទ័រផ្រូក្រាមមីង (Computer programming)
    kn: ಕಂಪ್ಯೂಟರ್ ಪ್ರೋಗ್ರಾಮಿಂಗ್
    ko: 컴퓨터 프로그래밍
    ku_latn: bernamekirin
    ky: программалоо
    la: programmatura computatralis
    lb: Programméierung
    lmo: programmazion
    lt: programavimas
    lv: programmēšana
    mhr: программлымаш
    mk: програмирање
    ml: കമ്പ്യൂട്ടർ പ്രോഗ്രാമിങ്ങ്
    mn: програмчлал
    mr: संगणक प्रोग्रामिंग
    ms: pengaturcaraan komputer
    ms_arab: ڤڠاتورچاراءن کومڤوتر
    nb: programmering
    nl: programmeren
    nn: programmering
    nqo: ߢߍߥߟߊߦߊߟߌ
    oc: programacion d'ordenadors
    os: программæтæ аразын
    pl: programowanie komputerów
    pms: Programassion informàtica
    pnb: کمپیوٹر پروگرامنگ
    ps: کمپيوټري برېښليک
    pt: programação de computadores
    pt_br: programação de computadores
    ro: programare
    ru: программирование
    rue: проґрамованя
    sa: सङ्गणककार्यविधिः
    se: programmeren
    sh: programiranje
    si: පරිගණක ක්රමලේඛකරණය
    sk: programovanie
    sl: računalniško programiranje
    smn: ohjelmistem
    sms: kooʹdjummuš
    sq: programimi kompjuterik
    sr: програмирање
    sr_ec: програмирање
    sv: programmering
    ta: கணினி நிரலாக்கம்
    tg: Барномасозӣ
    th: การเขียนโปรแกรมคอมพิวเตอร์
    tk: programmirleme
    tl: computer programming
    tr: bilgisayar programlama
    tt: программалау
    tt_cyrl: програмлау
    tw: Kɔmputa so dwumadi ahorow
    udm: косонрадъяны
    uk: програмування
    ur: کمپیوٹر پروگرامنگ
    uz: dasturlash
    vec: programasion
    vi: lập trình máy tính
    wuu: 计算机程序设计
    yi: קאמפיוטער פראגראמירן
    yue: 電腦程式設計
    zh: 计算机程序设计
    zh_cn: 计算机程序设计
    zh_hans: 计算机程序设计
    zh_hant: 電腦程序設計
    zh_hk: 電腦程式設計
    zh-my: 电脑程式设计
    zh_tw: 電腦程式設計
    zu: Ukuhlelelisa isiCikizi
---

# [[SW~Programming]] 

#is_/same_as :: [[../../../WikiData/WD~Computer_programming,80006|WD~Computer_programming,80006]]

## #has_/text_of_/abstract 

> Computer programming or coding is the composition of sequences of instructions, 
> called programs, that computers can follow to perform tasks. 
> 
> It involves designing and implementing algorithms, step-by-step specifications of procedures, 
> by writing code in one or more programming languages. 
> 
> Programmers typically use high-level programming languages 
> that are more easily intelligible to humans than machine code, 
> which is directly executed by the central processing unit. 
> 
> Proficient programming usually requires expertise in several different subjects, including 
> - knowledge of the application domain, 
> - details of programming languages and generic code libraries, 
> - specialized algorithms, and 
> - formal logic.
>
> Auxiliary tasks accompanying and related to programming include analyzing requirements, testing, debugging (investigating and fixing problems), implementation of build systems, and management of derived artifacts, such as programs' machine code. While these are sometimes considered programming, often the term software development is used for this larger overall process – with the terms programming, implementation, and coding reserved for the writing and editing of code per se. Sometimes software development is known as software engineering, especially when it employs formal methods or follows an engineering design process.
>	
> [Wikipedia](https://en.wikipedia.org/wiki/Computer%20programming) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[SW~Programming|SW~Programming]] 

### #is_/same_as :: [[/_public/Technology/IT/Software/SW~Programming.public|Computer_Programming.public]] 

### #is_/same_as :: [[/_internal/Technology/IT/Software/SW~Programming.internal|Computer_Programming.internal]] 

### #is_/same_as :: [[/_protect/Technology/IT/Software/SW~Programming.protect|Computer_Programming.protect]] 

### #is_/same_as :: [[/_private/Technology/IT/Software/SW~Programming.private|Computer_Programming.private]] 

### #is_/same_as :: [[/_personal/Technology/IT/Software/SW~Programming.personal|Computer_Programming.personal]] 

### #is_/same_as :: [[/_secret/Technology/IT/Software/SW~Programming.secret|Computer_Programming.secret]] 

