---
aliases:
- Brodostrojarstvo
- Deniz teknolojisi mühendisliği
- engenharia naval
- enginyeria naval
- Enxeñaría Naval e Oceánica
- génie maritime
- ingegneria navale
- ingeniería naval
- Inhinyeriyang pangkaragatan
- Kejuruteraan marin
- ladijsko strojništvo
- Marine Engineering
- maritieme werkbouwkunde
- Mechanik wachtowy
- meritekniikka
- Naval architecture
- Naval engineering
- Naval_Engineering
- naŭtika inĝenierio
- pomorsko inženjerstvo
- Schiffsingenieurwesen
- Secção de máquinas
- sjöingenjörsvetenskap
- Teknik kelautan
- марская інжынэрыя
- Морская инженерия
- Морська справа
- поморско инжењерство
- הנדסה ימית
- بحری انجینئرنگ
- سمندري انجینري
- مهندسی دریا
- هندسة بحرية
- नौइंजीनियरी
- วิศวกรรมสมุทรศาสตร์
- 海洋工学
- 航海工程
- 轮机工程
- 해양공학
has_id_wikidata: Q118291
subclass_of:
- '[[_Standards/WikiData/WD~spatial_planning,149013]]'
- '[[_Standards/WikiData/WD~engineering,11023]]'
practiced_by: '[[_Standards/WikiData/WD~marine_engineer,1644847]]'
part_of:
- '[[_Standards/WikiData/WD~marine_technology,6764311]]'
- '[[_Standards/WikiData/WD~naval_architecture_and_marine_engineering,101910631]]'
instance_of: '[[_Standards/WikiData/WD~branch_of_engineering,12015335]]'
Dewey_Decimal_Classification: 623
OmegaWiki_Defined_Meaning: 2260
image: http://commons.wikimedia.org/wiki/Special:FilePath/Marine-engineering.jpg
Commons_category: Marine engineering
dv_has_:
  name_:
    ar: هندسة بحرية
    be-tarask: марская інжынэрыя
    ca: enginyeria naval
    de: Schiffsingenieurwesen
    en: marine engineering
    en-ca: Marine engineering
    en-gb: marine engineering
    eo: naŭtika inĝenierio
    es: ingeniería naval
    fa: مهندسی دریا
    fi: meritekniikka
    fr: génie maritime
    gl: Enxeñaría Naval e Oceánica
    he: הנדסה ימית
    hi: नौइंजीनियरी
    hr: Brodostrojarstvo
    id: Teknik kelautan
    it: ingegneria navale
    ja: 海洋工学
    ko: 해양공학
    ms: Kejuruteraan marin
    nl: maritieme werkbouwkunde
    pl: Mechanik wachtowy
    ps: سمندري انجینري
    pt: engenharia naval
    pt-br: Secção de máquinas
    ru: Морская инженерия
    sl: ladijsko strojništvo
    sr: поморско инжењерство
    sr-ec: поморско инжењерство
    sr-el: pomorsko inženjerstvo
    sv: sjöingenjörsvetenskap
    th: วิศวกรรมสมุทรศาสตร์
    tl: Inhinyeriyang pangkaragatan
    tr: Deniz teknolojisi mühendisliği
    uk: Морська справа
    ur: بحری انجینئرنگ
    vi: marine engineering
    yue: 航海工程
    zh: 轮机工程
    zh-cn: 轮机工程
---
# [[Naval_Engineering]] 

#is_/same_as :: [[../WikiData/WD~Naval_architecture,1136352|WD~Naval_architecture,1136352]] 

## #has_/text_of_/abstract 

> Naval architecture, or **Naval engineering**, is an engineering discipline incorporating elements of 
> mechanical, electrical, electronic, software and safety engineering 
> as applied to the engineering design process, shipbuilding, maintenance, 
> and operation of marine vessels and structures. 
> 
> Naval architecture involves basic and applied research, design, development, design evaluation (classification) and calculations during all stages of the life of a marine vehicle. Preliminary design of the vessel, its detailed design, construction, trials, operation and maintenance, launching and dry-docking are the main activities involved. Ship design calculations are also required for ships being modified (by means of conversion, rebuilding, modernization, or repair). Naval architecture also involves formulation of safety regulations and damage-control rules and the approval and certification of ship designs to meet statutory and non-statutory requirements.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Naval%20architecture) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Naval_Engineering|Naval_Engineering]] 

### #is_/same_as :: [[/_public/Technology/Naval_Engineering.public|Naval_Engineering.public]] 

### #is_/same_as :: [[/_internal/Technology/Naval_Engineering.internal|Naval_Engineering.internal]] 

### #is_/same_as :: [[/_protect/Technology/Naval_Engineering.protect|Naval_Engineering.protect]] 

### #is_/same_as :: [[/_private/Technology/Naval_Engineering.private|Naval_Engineering.private]] 

### #is_/same_as :: [[/_personal/Technology/Naval_Engineering.personal|Naval_Engineering.personal]] 

### #is_/same_as :: [[/_secret/Technology/Naval_Engineering.secret|Naval_Engineering.secret]] 

