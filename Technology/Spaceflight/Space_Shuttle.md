---
aliases:
- avaruussukkula
- Baibén spacial
- Bolak-balik angkasa lepas
- Daugkartinis erdvėlaivis
- Egorvulzun
- Espazio-anezka
- Fazoviy moki
- Geimskutla
- Gwennol Ofod
- kosmopramo
- Kosmosesüstik
- Lasador na pangkalawan
- navette spatiale américaine
- Navicula spatialis
- Navèt spasyal amérikenn
- Panambangan Antariksa
- Pesawat ulang-alik
- Rodorsceaþel
- Romferge
- romferje
- Ruimtependeltuig
- Rumfærge
- Rúmdarferja
- Spaal Spoar
- Space Shuttle
- Spaco-paromo
- Spejs šatl
- Speys Şattl
- Spies Shokl
- Thài-khong-so
- Tointeálaí Spáis
- transbordador espacial
- Tresbordador STS
- Tàu con thoi
- Uzay mekiği
- vaivém
- wahadłowiec kosmiczny
- Yágháhoníkáángóó ałánátʼahí
- ônibus espacial
- Διαστημικό Λεωφορείο
- Касьмічны чаўнок
- космічний човник
- Космическа совалка
- Спејс шатл
- Спейс шатл
- спейс-шаттл
- Սփեյս Շաթլ
- מעבורת חלל
- ספעיס שאטעל
- خلائي شٽل
- خلائی شٹل
- شاتل فضایی
- فضا شاتئلی
- فضائی مکوک
- فضايي شټل
- مكوك فضاء
- مەکۆکی ئاسمانی
- अंतराळ शटल
- अंतरिक्ष शटल
- स्पेस शटल्स
- স্পেস শাটল
- ਪੁਲਾੜ ਵਾਹਨ
- விண்ணோடம்
- స్పేస్ షటిల్
- സ്പേസ് ഷട്ടിൽ
- අභ්යවකාශ ෂටල
- กระสวยอวกาศ
- ກະສວຍອາວະກາດ
- အာကာသလွန်းပျံ
- ᎦᎸᎶᎯ ᏪᏙᎯ
- Ọkọ̀-àlọbọ̀ Òfurufú
- スペースシャトル
- 天梭
- 太空梭
- 太空穿梭機
- 太空船
- 穿梭機
- 航天飛機
- 航天飞机
- 우주왕복선
has_id_wikidata: Q48806
has_use: '[[_Standards/WikiData/WD~orbital_spaceflight,218455]]'
manufacturer:
- '[[_Standards/WikiData/WD~Alliant_Techsystems,238765]]'
- '[[_Standards/WikiData/WD~Martin_Marietta,626800]]'
- '[[_Standards/WikiData/WD~United_Space_Alliance,1541150]]'
- '[[_Standards/WikiData/WD~Thiokol,2357348]]'
- '[[_Standards/WikiData/WD~Lockheed_Martin,7240]]'
- '[[_Standards/WikiData/WD~Boeing,66]]'
start_point: '[[_Standards/WikiData/WD~Kennedy_Space_Center_Launch_Complex_39,845774]]'
powered_by:
- '[[_Standards/WikiData/WD~RS_25,1030042]]'
- '[[_Standards/WikiData/WD~Space_Shuttle_Orbital_Maneuvering_System,1263822]]'
- '[[_Standards/WikiData/WD~Space_Shuttle_Solid_Rocket_Booster,1422862]]'
has_part_s_:
- '[[_Standards/WikiData/WD~Space_Shuttle_external_tank,1061044]]'
- '[[_Standards/WikiData/WD~Space_Shuttle_orbiter,1064394]]'
- '[[_Standards/WikiData/WD~Space_Shuttle_Solid_Rocket_Booster,1422862]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Infobox_space_shuttle,6266629]]'
instance_of:
- '[[_Standards/WikiData/WD~rocket_model,110055303]]'
- '[[_Standards/WikiData/WD~spacecraft_model,117384800]]'
different_from: '[[_Standards/WikiData/WD~space_shuttle,125217474]]'
subclass_of: '[[_Standards/WikiData/WD~reusable_launch_vehicle,30128]]'
country_of_origin: '[[_Standards/WikiData/WD~United_States,30]]'
diameter: 343
height: 2209
Nomenclature_for_Museum_Cataloging: 11734
image: http://commons.wikimedia.org/wiki/Special:FilePath/STS120LaunchHiRes-edit1.jpg
first_flight: 1981-04-12T00:00:00Z
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/Pl%C3%A1nik%20orbitera%202.JPG
equivalent_class: http://dbpedia.org/ontology/SpaceShuttle
exact_match: http://wordnet-rdf.princeton.edu/wn30/04266014-n
U_S_National_Archives_Identifier: 10644333
RIA_Novosti_reference: 50932260
Krugosvet_article_archived_: aviaciya-i-kosmonavtika/kosmicheskiy-korabl-shattl
DPLA_subject_term: Space shuttle
Commons_gallery: Space Shuttle
Commons_category: Space Shuttles
dv_has_:
  name_:
    af: Ruimtependeltuig
    ang: Rodorsceaþel
    ar: مكوك فضاء
    ast: Tresbordador STS
    az: Speys Şattl
    azb: فضا شاتئلی
    be: Спейс шатл
    be-tarask: Касьмічны чаўнок
    bg: Космическа совалка
    bn: স্পেস শাটল
    br: Egorvulzun
    bs: Space Shuttle
    ca: transbordador espacial
    chr: ᎦᎸᎶᎯ ᏪᏙᎯ
    ckb: مەکۆکی ئاسمانی
    cs: Space Shuttle
    cy: Gwennol Ofod
    da: Rumfærge
    de: Space Shuttle
    el: Διαστημικό Λεωφορείο
    en: Space Shuttle
    en-ca: Space Shuttle
    en-gb: Space Shuttle
    eo: kosmopramo
    es: transbordador espacial
    et: Kosmosesüstik
    eu: Espazio-anezka
    fa: شاتل فضایی
    fi: avaruussukkula
    fo: Rúmdarferja
    fr: navette spatiale américaine
    frr: Space Shuttle
    ga: Tointeálaí Spáis
    gan: 太空船
    gcr: Navèt spasyal amérikenn
    gl: transbordador espacial
    gv: Spaal Spoar
    ha: Space Shuttle
    he: מעבורת חלל
    hi: अंतरिक्ष शटल
    hr: Space Shuttle
    hu: Space Shuttle
    hy: Սփեյս Շաթլ
    id: Pesawat ulang-alik
    io: Spaco-paromo
    is: Geimskutla
    it: Space Shuttle
    ja: スペースシャトル
    jam: Spies Shokl
    jv: Panambangan Antariksa
    ko: 우주왕복선
    ky: Спейс Шатл
    la: Navicula spatialis
    lb: Space Shuttle
    li: Space Shuttle
    lo: ກະສວຍອາວະກາດ
    lt: Daugkartinis erdvėlaivis
    lv: Space Shuttle
    lzh: 天梭
    mk: Спејс шатл
    ml: സ്പേസ് ഷട്ടിൽ
    mr: अंतराळ शटल
    ms: Bolak-balik angkasa lepas
    mwl: Baibén spacial
    my: အာကာသလွန်းပျံ
    nan: Thài-khong-so
    nb: Romferge
    nds: Space Shuttle
    ne: स्पेस शटल्स
    nl: Space Shuttle
    nn: romferje
    nv: Yágháhoníkáángóó ałánátʼahí
    pa: ਪੁਲਾੜ ਵਾਹਨ
    pl: wahadłowiec kosmiczny
    pnb: خلائی شٹل
    ps: فضايي شټل
    pt: vaivém
    pt-br: ônibus espacial
    ro: Space Shuttle
    ru: спейс-шаттл
    scn: Space Shuttle
    sd: خلائي شٽل
    sh: Space Shuttle
    si: අභ්යවකාශ ෂටල
    sk: Space Shuttle
    sl: Space Shuttle
    sr: Spejs šatl
    stq: Space Shuttle
    sv: Space Shuttle
    ta: விண்ணோடம்
    te: స్పేస్ షటిల్
    th: กระสวยอวกาศ
    tl: Lasador na pangkalawan
    tr: Uzay mekiği
    uk: космічний човник
    ur: فضائی مکوک
    uz: Fazoviy moki
    vec: Space Shuttle
    vi: Tàu con thoi
    wuu: 航天飞机
    yi: ספעיס שאטעל
    yo: Ọkọ̀-àlọbọ̀ Òfurufú
    yue: 穿梭機
    zh: 航天飞机
    zh-cn: 航天飞机
    zh-hans: 航天飞机
    zh-hant: 航天飛機
    zh-hk: 太空穿梭機
    zh-tw: 太空梭
---
# [[Space_Shuttle]] 

#is_/same_as :: [[../../WikiData/WD~Space_Shuttle,48806|WD~Space_Shuttle,48806]]  

## #has_/text_of_/abstract 

> The Space Shuttle is a retired, partially reusable low Earth orbital spacecraft system 
> operated from 1981 to 2011 by the U.S. National Aeronautics and Space Administration (NASA) 
> as part of the Space Shuttle program. 
> 
> Its official program name was the Space Transportation System (STS), 
> taken from the 1969 plan led by U.S. vice president Spiro Agnew 
> for a system of reusable spacecraft where it was the only item funded for development.
>
> The first (STS-1) of four orbital test flights occurred in 1981, 
> leading to operational flights (STS-5) beginning in 1982. 
> 
> Five complete Space Shuttle orbiter vehicles were built 
> and flown on a total of 135 missions from 1981 to 2011. 
> 
> They launched from the Kennedy Space Center (KSC) in Florida. 
> Operational missions launched numerous satellites, interplanetary probes, 
> and the Hubble Space Telescope (HST), conducted science experiments in orbit, 
> participated in the Shuttle-Mir program with Russia, 
> and participated in the construction and servicing of the International Space Station (ISS). 
> 
> The Space Shuttle fleet's total mission time was 1,323 days.
>
> Space Shuttle components include the Orbiter Vehicle (OV) with three clustered Rocketdyne RS-25 main engines, a pair of recoverable solid rocket boosters (SRBs), and the expendable external tank (ET) containing liquid hydrogen and liquid oxygen. The Space Shuttle was launched vertically, like a conventional rocket, with the two SRBs operating in parallel with the orbiter's three main engines, which were fueled from the ET. The SRBs were jettisoned before the vehicle reached orbit, while the main engines continued to operate, and the ET was jettisoned after main engine cutoff and just before orbit insertion, which used the orbiter's two Orbital Maneuvering System (OMS) engines. At the conclusion of the mission, the orbiter fired its OMS to deorbit and reenter the atmosphere. The orbiter was protected during reentry by its thermal protection system tiles, and it glided as a spaceplane to a runway landing, usually to the Shuttle Landing Facility at KSC, Florida, or to Rogers Dry Lake in Edwards Air Force Base, California. If the landing occurred at Edwards, the orbiter was flown back to the KSC atop the Shuttle Carrier Aircraft (SCA), a specially modified Boeing 747 designed to carry the shuttle above it.
>
> The first orbiter, Enterprise, was built in 1976 and used in Approach and Landing Tests (ALT), but had no orbital capability. Four fully operational orbiters were initially built: Columbia, Challenger, Discovery, and Atlantis. Of these, two were lost in mission accidents: Challenger in 1986 and Columbia in 2003, with a total of 14 astronauts killed. A fifth operational (and sixth in total) orbiter, Endeavour, was built in 1991 to replace Challenger. The three surviving operational vehicles were retired from service following Atlantis's final flight on July 21, 2011. The U.S. relied on the Russian Soyuz spacecraft to transport astronauts to the ISS from the last Shuttle flight until the launch of the Crew Dragon Demo-2 mission in May 2020.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Space%20Shuttle) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Spaceflight/Space_Shuttle|Space_Shuttle]] 

### #is_/same_as :: [[/_public/Technology/Spaceflight/Space_Shuttle.public|Space_Shuttle.public]] 

### #is_/same_as :: [[/_internal/Technology/Spaceflight/Space_Shuttle.internal|Space_Shuttle.internal]] 

### #is_/same_as :: [[/_protect/Technology/Spaceflight/Space_Shuttle.protect|Space_Shuttle.protect]] 

### #is_/same_as :: [[/_private/Technology/Spaceflight/Space_Shuttle.private|Space_Shuttle.private]] 

### #is_/same_as :: [[/_personal/Technology/Spaceflight/Space_Shuttle.personal|Space_Shuttle.personal]] 

### #is_/same_as :: [[/_secret/Technology/Spaceflight/Space_Shuttle.secret|Space_Shuttle.secret]] 

