---
aliases:
- aegruum
- aika-avaruus
- continuum spatio-temporale
- czasoprzestrzeń
- efanvos-termyn
- erdvėlaikis
- espace-temps
- espacio-tiempo
- Espacitemps
- espaciu-tiempu
- espaitemps
- espasyo-panahon
- espazio-denbora
- espazo-tempo
- espaço-tempo
- fazo-vaqt
- fəza-zaman
- gofod-amser
- hapësirë-kohë
- kalarakan-oras
- không-thời gian
- laiktelpa
- Nafasiwakati
- pacha
- priestoročas
- Prostor-vreme
- prostor-čas
- prostorvrijeme
- Raumzeit
- Raumzäit
- ruang waktu
- ruang-masa
- ruimtetijd
- ruimtetyd
- rumtid
- rúmtíð
- rümtidj
- Sam-yeroo
- spacetime
- Spacotempo
- spactempo
- spatio-tempore
- spaziotempo
- spazziutempu
- spațiu-timp
- spàsio-tenpo
- spás-am
- tidrom
- téridő
- tímarúm
- uzayzaman
- àyè ní àkókò
- časoprostor
- χωροχρόνος
- време-простор
- инсьӧрдыр
- кеңістік пен уақыт
- прастора-час
- простір-час
- простор-време
- пространство-време
- пространство-время
- уçлăх-вăхăт
- фазо-замон
- фәза-вакыт
- տարածաժամանակ
- מרחב-זמן
- تشيال وخت
- خلا-وقت تسلسل
- رواڠ-ماس
- زمان و مکاں
- زمكان
- سبيس-تايم
- سپیس ٹائیم
- فضازامان
- فضازمان
- کاتجێ
- काल-अवकाश
- दिक्-काल
- স্থান-কাল
- ਸਪੇਸਟਾਈਮ
- வெளிநேரம்
- സ്ഥലകാലം
- අවකාශ-කාල
- กาล-อวกาศ
- ເວລາອາວະກາດ
- အာကာသအချိန်
- ოფირჩა-ბორჯი
- სივრცე-დრო
- 时空
- 時空
- 시공간
has_id_wikidata: Q133327
instance_of:
- '[[_Standards/WikiData/WD~physical_phenomenon,1293220]]'
- '[[_Standards/WikiData/WD~continuum,1704572]]'
different_from:
- '[[_Standards/WikiData/WD~Raumzeit,2133417]]'
- '[[_Standards/WikiData/WD~Spacetime,7572733]]'
subclass_of: '[[_Standards/WikiData/WD~spacetime_curvature,4240689]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Spacetime,8084437]]'
union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486]]'
contains: '[[_Standards/WikiData/WD~spacetime_region,23956024]]'
manifestation_of: '[[_Standards/WikiData/WD~spacetime_model,130249235]]'
has_part_s_:
- '[[_Standards/WikiData/WD~time,11471]]'
- '[[_Standards/WikiData/WD~space,107]]'
Commons_category: Spacetime
image: http://commons.wikimedia.org/wiki/Special:FilePath/Spacetime%20curvature2.png
Stack_Exchange_tag: https://physics.stackexchange.com/tags/spacetime
IEV_number: 113-01-01
dv_has_:
  name_:
    af: ruimtetyd
    ar: زمكان
    arz: سبيس-تايم
    as: স্থান-কাল
    ast: espaciu-tiempu
    az: fəza-zaman
    azb: فضازامان
    be: прастора-час
    be-tarask: прастора-час
    bg: пространство-време
    bho: काल-अवकाश
    bn: স্থান-কাল
    bs: prostorvrijeme
    ca: espaitemps
    ckb: کاتجێ
    cs: časoprostor
    cv: уçлăх-вăхăт
    cy: gofod-amser
    da: rumtid
    de: Raumzeit
    el: χωροχρόνος
    en: spacetime
    eo: spactempo
    es: espacio-tiempo
    et: aegruum
    eu: espazio-denbora
    fa: فضازمان
    fi: aika-avaruus
    fo: rúmtíð
    fr: espace-temps
    frr: rümtidj
    ga: spás-am
    gl: espazo-tempo
    gsw: Raumzeit
    he: מרחב-זמן
    hi: दिक्-काल
    hr: prostorvrijeme
    hu: téridő
    hy: տարածաժամանակ
    ia: spatio-tempore
    id: ruang waktu
    io: Spacotempo
    is: tímarúm
    it: spaziotempo
    ja: 時空
    ka: სივრცე-დრო
    kk: кеңістік пен уақыт
    ko: 시공간
    kw: efanvos-termyn
    la: continuum spatio-temporale
    lb: Raumzäit
    lo: ເວລາອາວະກາດ
    lt: erdvėlaikis
    lv: laiktelpa
    mk: време-простор
    ml: സ്ഥലകാലം
    mr: काल-अवकाश
    ms: ruang-masa
    ms-arab: رواڠ-ماس
    my: အာကာသအချိန်
    nb: tidrom
    nl: ruimtetijd
    nn: tidrom
    oc: Espacitemps
    om: Sam-yeroo
    pa: ਸਪੇਸਟਾਈਮ
    pl: czasoprzestrzeń
    pnb: سپیس ٹائیم
    ps: تشيال وخت
    pt: espaço-tempo
    pt-br: espaço-tempo
    qu: pacha
    rki: အာကာသအချိန်
    ro: spațiu-timp
    ru: пространство-время
    scn: spazziutempu
    sco: spacetime
    sd: خلا-وقت تسلسل
    sh: Prostor-vreme
    si: අවකාශ-කාල
    sk: priestoročas
    sl: prostor-čas
    sq: hapësirë-kohë
    sr: простор-време
    sr-ec: простор-време
    sr-el: prostor-vreme
    sv: rumtid
    sw: Nafasiwakati
    ta: வெளிநேரம்
    tg: фазо-замон
    th: กาล-อวกาศ
    tl: espasyo-panahon
    tr: uzayzaman
    tt: фәза-вакыт
    tt-cyrl: фәза-вакыт
    udm: инсьӧрдыр
    uk: простір-час
    ur: زمان و مکاں
    uz: fazo-vaqt
    vec: spàsio-tenpo
    vi: không-thời gian
    war: kalarakan-oras
    wuu: 时空
    xmf: ოფირჩა-ბორჯი
    yo: àyè ní àkókò
    yue: 時空
    zh: 时空
    zh-cn: 时空
    zh-hans: 时空
    zh-hant: 時空
    zh-hk: 時空
    zh-sg: 时空
    zh-tw: 時空
---

# [[Spacetime]] 

#is_/same_as :: [[../../WikiData/WD~Spacetime,133327|WD~Spacetime,133327]]  

## #has_/text_of_/abstract 

> In physics, **Spacetime**, also called the space-time continuum, 
> is a mathematical model that fuses the three dimensions of space 
> and the one dimension of time into a single four-dimensional continuum. 
> 
> Spacetime diagrams are useful in visualizing and understanding relativistic effects, 
> such as how different observers perceive where and when events occur.
>
> Until the turn of the 20th century, the assumption had been 
> that the three-dimensional geometry of the universe 
> (its description in terms of locations, shapes, distances, and directions) 
> was distinct from time (the measurement of when events occur within the universe). 
> 
> However, space and time took on new meanings 
> with the Lorentz transformation and special theory of relativity.
>
> In 1908, Hermann Minkowski presented a geometric interpretation of special relativity 
> that fused time and the three spatial dimensions into a single four-dimensional continuum 
> now known as Minkowski space. 
> 
> This interpretation proved vital to the general theory of relativity, 
> wherein spacetime is curved by mass and energy.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Spacetime) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Spaceflight/Spacetime|Spacetime]] 

### #is_/same_as :: [[/_public/Technology/Spaceflight/Spacetime.public|Spacetime.public]] 

### #is_/same_as :: [[/_internal/Technology/Spaceflight/Spacetime.internal|Spacetime.internal]] 

### #is_/same_as :: [[/_protect/Technology/Spaceflight/Spacetime.protect|Spacetime.protect]] 

### #is_/same_as :: [[/_private/Technology/Spaceflight/Spacetime.private|Spacetime.private]] 

### #is_/same_as :: [[/_personal/Technology/Spaceflight/Spacetime.personal|Spacetime.personal]] 

### #is_/same_as :: [[/_secret/Technology/Spaceflight/Spacetime.secret|Spacetime.secret]] 

