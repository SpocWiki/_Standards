---
aliases:
- almmiseasan
- avaruusluotain
- chwiliedydd gofod
- espazio zunda
- Flaybay
- Kipimaanga
- komovuotâluođân
- kosmická sonda
- Kosmoprobilo
- kosmosesond
- kosmosondilo
- kozmická sonda
- Kuar angkasa lepas
- Könnunarfar
- onbemande ruimtetuig
- prob antariksa
- Raumsond
- Raumsonde
- romsonde
- ruimtesonde
- rumsonde
- Ruumsond
- Ruumsonde
- rymdsond
- Rümsonde
- sonda espacial
- sonda espaciala
- Sonda hapësinore
- sonda kosmiczna
- sonda spasiałe
- sonda spaziale
- sonde spatiale
- sondă spațială
- sonn espasyal
- space probe
- Speculatrum spatiale
- spástaiscéalaí
- starpplanētu zonde
- Svemirska sonda
- thăm dò không gian
- uzay sondası
- vesoljska sonda
- űrszonda
- Ρομποτικό διαστημόπλοιο
- аўтаматычная міжпланетная станцыя
- аўтаматычная міжплянэтная станцыя
- автоматична міжпланетна станція
- Автоматична междупланетна станция
- вселенска сонда
- космический зонд
- свемирска сонда
- ավտոմատ միջմոլորակային կայան
- גשושית
- مسبار فضائي
- پشکنەری ئاسمانی
- کاوشگر فضایی
- ߝߏ߬ߟߏ߲߬ߝߊ߬ߟߊ߲߬ߕߍ ߘߐ߬ߛߏ߰ߙߋ߬ߟߊ߲
- अंतरिक्ष शोध यान
- ਪੁਲਾੜ ਟਟੋਲ
- விண்ணுளவி
- స్పేస్ ప్రోబ్
- ยานสำรวจอวกาศ
- ავტომატური საპლანეტათშორისო სადგური
- 太空探測器
- 宇宙探査機
- 空间探测器
- 우주 탐사선
has_id_wikidata: Q26529
has_use:
- '[[_Standards/WikiData/WD~space_exploration,180046]]'
- '[[_Standards/WikiData/WD~science_project,1298668]]'
subclass_of: '[[_Standards/WikiData/WD~robotic_spacecraft,1378139]]'
topic_has_template:
- '[[_Standards/WikiData/WD~Template_Solar_System_probes,17487508]]'
- '[[_Standards/WikiData/WD~Template_Infobox_space_probe,25819628]]'
has_part_s_: '[[_Standards/WikiData/WD~space_instrument,100349043]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817]]'
Nomenclature_for_Museum_Cataloging: 11742
image: http://commons.wikimedia.org/wiki/Special:FilePath/Cassini%20Saturn%20Orbit%20Insertion.jpg
Krugosvet_article_archived_: aviaciya-i-kosmonavtika/kosmicheskiy-zond
Wolfram_Language_entity_type: DeepSpaceProbe
Commons_category: Space probes
dv_has_:
  name_:
    af: onbemande ruimtetuig
    an: Sonda espacial
    ar: مسبار فضائي
    ast: sonda espacial
    az: Flaybay
    be: аўтаматычная міжпланетная станцыя
    be-tarask: аўтаматычная міжплянэтная станцыя
    bg: Автоматична междупланетна станция
    bs: Svemirska sonda
    ca: sonda espacial
    ckb: پشکنەری ئاسمانی
    cs: kosmická sonda
    cy: chwiliedydd gofod
    da: rumsonde
    de: Raumsonde
    el: Ρομποτικό διαστημόπλοιο
    en: space probe
    eo: kosmosondilo
    es: sonda espacial
    et: kosmosesond
    eu: espazio zunda
    fa: کاوشگر فضایی
    fi: avaruusluotain
    fr: sonde spatiale
    frr: Rümsonde
    ga: spástaiscéalaí
    gl: sonda espacial
    gsw: Ruumsonde
    he: גשושית
    hi: अंतरिक्ष शोध यान
    ht: sonn espasyal
    hu: űrszonda
    hy: ավտոմատ միջմոլորակային կայան
    id: prob antariksa
    io: Kosmoprobilo
    is: Könnunarfar
    it: sonda spaziale
    ja: 宇宙探査機
    ka: ავტომატური საპლანეტათშორისო სადგური
    ko: 우주 탐사선
    la: Speculatrum spatiale
    lb: Raumsond
    lt: starpplanētu zonde
    lv: starpplanētu zonde
    mk: вселенска сонда
    ms: Kuar angkasa lepas
    nb: romsonde
    nds: Ruumsond
    nl: ruimtesonde
    nn: romsonde
    nqo: ߝߏ߬ߟߏ߲߬ߝߊ߬ߟߊ߲߬ߕߍ ߘߐ߬ߛߏ߰ߙߋ߬ߟߊ߲
    oc: sonda espaciala
    pa: ਪੁਲਾੜ ਟਟੋਲ
    pl: sonda kosmiczna
    pt: sonda espacial
    ro: sondă spațială
    ru: космический зонд
    sco: space probe
    se: almmiseasan
    sh: Svemirska sonda
    sk: kozmická sonda
    sl: vesoljska sonda
    smn: komovuotâluođân
    sq: Sonda hapësinore
    sr: свемирска сонда
    sr-ec: свемирска сонда
    sr-el: svemirska sonda
    sv: rymdsond
    sw: Kipimaanga
    ta: விண்ணுளவி
    te: స్పేస్ ప్రోబ్
    th: ยานสำรวจอวกาศ
    tl: Space probe
    tr: uzay sondası
    uk: автоматична міжпланетна станція
    vec: sonda spasiałe
    vi: thăm dò không gian
    wuu: 空间探测器
    yue: 太空探測器
    zh: 空间探测器
---

# [[Space_Probe]] 

#is_/same_as :: [[../../WikiData/WD~Space_probe,26529|WD~Space_probe,26529]]  

## #has_/text_of_/abstract 

> Uncrewed spacecraft or robotic spacecraft are spacecraft without people on board. 
> 
> Uncrewed spacecraft may have varying levels of autonomy from human input, 
> such as remote control, or remote guidance. 
> 
> They may also be autonomous, in which they have a pre-programmed list of operations 
> that will be executed unless otherwise instructed. 
> A robotic spacecraft for scientific measurements is often called a space probe or space observatory.
>
> Many space missions are more suited to telerobotic rather than crewed operation, 
> due to lower cost and risk factors. 
> 
> In addition, some planetary destinations such as Venus or the vicinity of Jupiter 
> are too hostile for human survival, given current technology. 
> 
> Outer planets such as Saturn, Uranus, and Neptune are too distant to reach with current crewed spaceflight technology, so telerobotic probes are the only way to explore them. Telerobotics also allows exploration of regions that are vulnerable to contamination by Earth micro-organisms since spacecraft can be sterilized. Humans can not be sterilized in the same way as a spaceship, as they coexist with numerous micro-organisms, and these micro-organisms are also hard to contain within a spaceship or spacesuit.
>
> The first uncrewed space mission was Sputnik, launched October 4, 1957 to orbit the Earth. Nearly all satellites, landers and rovers are robotic spacecraft. Not every uncrewed spacecraft is a robotic spacecraft; for example, a reflector ball is a non-robotic uncrewed spacecraft. Space missions where other animals but no humans are on-board are called uncrewed missions.
>
> Many habitable spacecraft also have varying levels of robotic features. For example, the space stations Salyut 7 and Mir, and the International Space Station module Zarya, were capable of remote guided station-keeping and docking maneuvers with both resupply craft and new modules. Uncrewed resupply spacecraft are increasingly used for crewed space stations.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Uncrewed%20spacecraft) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Spaceflight/Space_Probe|Space_Probe]] 

### #is_/same_as :: [[/_public/Technology/Spaceflight/Space_Probe.public|Space_Probe.public]] 

### #is_/same_as :: [[/_internal/Technology/Spaceflight/Space_Probe.internal|Space_Probe.internal]] 

### #is_/same_as :: [[/_protect/Technology/Spaceflight/Space_Probe.protect|Space_Probe.protect]] 

### #is_/same_as :: [[/_private/Technology/Spaceflight/Space_Probe.private|Space_Probe.private]] 

### #is_/same_as :: [[/_personal/Technology/Spaceflight/Space_Probe.personal|Space_Probe.personal]] 

### #is_/same_as :: [[/_secret/Technology/Spaceflight/Space_Probe.secret|Space_Probe.secret]] 

