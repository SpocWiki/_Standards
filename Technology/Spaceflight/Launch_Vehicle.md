---
aliases:
- bererakett
- bærerakett
- cerbyd lansio
- daşıyıcı raket
- draagraket
- Drorakéit
- eltuvchi raketa
- Espazio-anezka
- feithicil lainseála
- foguete espacial
- fırlatma aracı
- hordozórakéta
- Kandaraan paluncua roket
- kanderakett
- kantoraketti
- kendaraan peluncur luar angkasa
- kosminė raketa
- lanceur
- lançador
- launch vehicle
- llanzadera espacial
- løfteraket
- nereuzebla lanĉo-sistemo
- nesējraķete
- noseća raketa
- nosilna raketa
- nosná raketa
- raketa-nosač
- rakieta nośna
- rymdraket
- Trägerrakete
- tên lửa đẩy
- vehicle de llançament
- vehicul de lansare
- vehículo de lanzamiento
- vettore
- veículo de lançamento
- Αναλώσιμο σύστημα εκτόξευσης
- Зымыран-тасығыш
- носечка ракета
- ракета носител
- ракета-носій
- ракета-носач
- ракета-носитель
- ракета-носьбіт
- Хөөргөх төхөөрөмж
- կրող հրթիռ
- משגר לוויינים וחלליות
- صاروخ حامل
- مووشەک ھەڵگر
- وسیله پرتاب
- উৎক্ষেপক যান
- செலுத்து வாகனம்
- ప్రయోగ వాహనం
- ಉಡ್ಡಯನ ವಾಹನಗಳು
- വിക്ഷേപണ വാഹനം
- რაკეტა-მატარებელი
- ローンチ・ヴィークル
- 运载火箭
- 運載火箭
- 발사 차량
has_id_wikidata: Q697175
part_of: '[[_Standards/WikiData/WD~space_mission,2133344]]'
different_from: '[[_Standards/WikiData/WD~Q19645718,19645718]]'
Wikidata_property: space launch vehicle
has_use: '[[_Standards/WikiData/WD~spaceflight,5916]]'
subclass_of: '[[_Standards/WikiData/WD~rocket,41291]]'
Commons_category: Space launch vehicles
image: http://commons.wikimedia.org/wiki/Special:FilePath/Apollo%2015%20launch.jpg
dv_has_:
  name_:
    ar: صاروخ حامل
    ast: llanzadera espacial
    az: daşıyıcı raket
    be: ракета-носьбіт
    be-tarask: ракета-носьбіт
    bg: ракета носител
    bn: উৎক্ষেপক যান
    bs: noseća raketa
    ca: vehicle de llançament
    ckb: مووشەک ھەڵگر
    cs: nosná raketa
    cy: cerbyd lansio
    da: løfteraket
    de: Trägerrakete
    el: Αναλώσιμο σύστημα εκτόξευσης
    en: launch vehicle
    en-ca: launch vehicle
    en-gb: launch vehicle
    en-us: launch vehicle
    eo: nereuzebla lanĉo-sistemo
    es: vehículo de lanzamiento
    et: kanderakett
    eu: Espazio-anezka
    fa: وسیله پرتاب
    fi: kantoraketti
    fr: lanceur
    ga: feithicil lainseála
    gl: foguete espacial
    he: משגר לוויינים וחלליות
    hr: raketa-nosač
    hu: hordozórakéta
    hy: կրող հրթիռ
    id: kendaraan peluncur luar angkasa
    it: vettore
    ja: ローンチ・ヴィークル
    ka: რაკეტა-მატარებელი
    kk: Зымыран-тасығыш
    kn: ಉಡ್ಡಯನ ವಾಹನಗಳು
    ko: 발사 차량
    lb: Drorakéit
    lt: kosminė raketa
    lv: nesējraķete
    min: Kandaraan paluncua roket
    mk: носечка ракета
    ml: വിക്ഷേപണ വാഹനം
    mn: Хөөргөх төхөөрөмж
    nb: bærerakett
    nl: draagraket
    nn: bererakett
    oc: lançador
    pl: rakieta nośna
    pt: veículo de lançamento
    pt-br: veículo de lançamento
    ro: vehicul de lansare
    ru: ракета-носитель
    sco: launch vehicle
    sh: raketa-nosač
    sk: nosná raketa
    sl: nosilna raketa
    sr: ракета-носач
    sr-ec: ракета-носач
    sr-el: raketa-nosač
    sv: rymdraket
    ta: செலுத்து வாகனம்
    te: ప్రయోగ వాహనం
    tr: fırlatma aracı
    uk: ракета-носій
    uz: eltuvchi raketa
    vi: tên lửa đẩy
    yue: 運載火箭
    zh: 运载火箭
---
# [[Launch_Vehicle]] 

#is_/same_as :: [[WD~Launch_vehicle,697175]] 

## #has_/text_of_/abstract 

> A launch vehicle is typically a rocket-powered vehicle 
> designed to carry a payload (a crewed spacecraft or satellites) from Earth's surface 
> or lower atmosphere to outer space. 
> 
> The most common form is the ballistic missile-shaped multistage rocket, 
> but the term is more general and also encompasses vehicles like the Space Shuttle. 
> 
> Most launch vehicles operate from a launch pad, supported by a launch control center 
> and systems such as vehicle assembly and fueling. 
> Launch vehicles are engineered with advanced aerodynamics and technologies, 
> which contribute to high operating costs.
>
> An orbital launch vehicle must lift its payload at least to the boundary of space, 
> approximately 150 km (93 mi) and accelerate it to a horizontal velocity of at least 7,814 m/s (17,480 mph). Suborbital vehicles launch their payloads to lower velocity or are launched at elevation angles greater than horizontal.
>
> Practical orbital launch vehicles use chemical propellants such as solid fuel, liquid hydrogen, kerosene, liquid oxygen, or hypergolic propellants.
>
> Launch vehicles are classified by their orbital payload capacity, ranging from small-, medium-, heavy- to super-heavy lift.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Launch%20vehicle) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Spaceflight/Launch_Vehicle|Launch_Vehicle]] 

### #is_/same_as :: [[/_public/Technology/Spaceflight/Launch_Vehicle.public|Launch_Vehicle.public]] 

### #is_/same_as :: [[/_internal/Technology/Spaceflight/Launch_Vehicle.internal|Launch_Vehicle.internal]] 

### #is_/same_as :: [[/_protect/Technology/Spaceflight/Launch_Vehicle.protect|Launch_Vehicle.protect]] 

### #is_/same_as :: [[/_private/Technology/Spaceflight/Launch_Vehicle.private|Launch_Vehicle.private]] 

### #is_/same_as :: [[/_personal/Technology/Spaceflight/Launch_Vehicle.personal|Launch_Vehicle.personal]] 

### #is_/same_as :: [[/_secret/Technology/Spaceflight/Launch_Vehicle.secret|Launch_Vehicle.secret]] 

