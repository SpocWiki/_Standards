---
aliases:
- alaway
- antakuru
- a̱taintuut
- chitima
- Chrien
- comboio
- cuch
- Cug
- engbunduka
- fasuna
- ferata
- fiarandalamby
- Fó-chhâ
- gadi
- hamaxostichus
- hojceh
- huōi-chiă
- jarnbanetog
- jernbanetog
- jonuz
- juna
- juná
- járnbrautarlest
- kareta api
- karita api
- karéta api
- kereta api
- keteke
- kǫʼ naʼałbąąsii diné bee nágéhé
- lia̍t-chhia
- lukalu
- ma'aataemeo'o
- mba'yjua
- pociąg
- poojâs
- poyezd
- pínpán
- qatar
- rong
- rulu tqrngul
- rélùwéè
- sepur
- silamalay
- suluku kpɛyaɣ
- tamacint
- tareen
- tepozcōātl
- teren
- tog
- toga
- tok
- tomo tawa linja
- traain
- traein
- traen
- Train
- traino
- trajno
- traukinys
- traukėnīs
- trein
- trem
- tren
- trene
- treni
- treno
- trenu
- trien
- trieno
- Tring
- trèana
- trén
- tréno
- trên
- trǣg
- tàu hỏa
- tåg
- tågå
- tåhka
- tååge
- uloliwe
- vilciens
- vlak
- vonat
- voz
- ziliji
- Zuch
- Zug
- ćah
- τρένο
- адәыҕба
- влак
- воз
- галт тэрэг
- кшниулав
- по̄яс
- поїзд
- поез
- поезд
- пойыз
- пуез
- пуйăс
- суксурҕа
- Улав
- цягнік
- цӏерпошт
- գնացք
- շոգեկառք
- באן
- רכבת
- اطر
- اورګاډی
- تران
- ترينو
- ريل گاڏي
- ریہل
- شەمەندەفەر
- قاطار (نقلیه)
- قطار
- ٹرین
- रेल
- रेलगाड़ी
- रेल्वे
- রেলগাড়ি
- ৰেলগাড়ী
- ਰੇਲ
- தொடர்வண்டி
- రైలు
- ರೈಲು
- തീവണ്ടി
- දුම්රිය
- รถไฟ
- ລົດໄຟ
- མེ་འཁོར།
- မီးရထား
- လူတ်ႉၾႆး
- მატარებელი
- ባቡር
- ᱨᱮᱞ ᱜᱟᱹᱰᱤ
- ụgbọ okporo igwe
- ⴰⵍⴰⵡⴰⵢ
- 列車
- 火車
- 火车
- 鐵路列車
- 铁路列车
- 열차
has_id_wikidata: Q870
Commons_category: Trains
described_by_source:
- '[[/_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[/_Standards/WikiData/WD~Metropolitan_Museum_of_Art_Tagging_Vocabulary,106727050|WD~Metropolitan_Museum_of_Art_Tagging_Vocabulary,106727050]]'
different_from:
- '[[/_Standards/WikiData/WD~train_service,15141321|WD~train_service,15141321]]'
- '[[/_Standards/WikiData/WD~pod,112893788|WD~pod,112893788]]'
equivalent_class: http://dbpedia.org/ontology/Train
has_part_s_:
- '[[/_Standards/WikiData/WD~locomotive,93301|WD~locomotive,93301]]'
- '[[/_Standards/WikiData/WD~railroad_car,208502|WD~railroad_car,208502]]'
has_use:
- '[[/_Standards/WikiData/WD~rail_and_rope_guided_transport,350783|WD~rail_and_rope_guided_transport,350783]]'
- '[[/_Standards/WikiData/WD~rail_transport,3565868|WD~rail_transport,3565868]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/UBTZ%202TE116UM-022%20Cagaan%20Had%20-%20Sumangijn%20Zoo.jpg
instance_of: '[[/_Standards/WikiData/WD~mode_of_transport,334166|WD~mode_of_transport,334166]]'
MIL_STD_2525_Symbol_Identification_Code: 10011500001500000000
Nomenclature_for_Museum_Cataloging: 12378
OmegaWiki_Defined_Meaning: 3834
OpenStreetMap_tag_or_key: Key:train
page_banner: http://commons.wikimedia.org/wiki/Special:FilePath/FS%20E%20444%20084%20Cervo%20Andora%20banner.jpg
small_logo_or_icon: http://commons.wikimedia.org/wiki/Special:FilePath/Ic%20directions%20train%2048px.svg
subclass_of:
- '[[/_Standards/WikiData/WD~rolling_stock,1414135|WD~rolling_stock,1414135]]'
- '[[/_Standards/WikiData/WD~wheeled_vehicle,15618781|WD~wheeled_vehicle,15618781]]'
topic_s_main_template:
- '[[/_Standards/WikiData/WD~Template_Infobox_train,5902995|WD~Template_Infobox_train,5902995]]'
- '[[/_Standards/WikiData/WD~Q106811496,106811496|WD~Q106811496,106811496]]'
U_S_National_Archives_Identifier: 10675596
video: http://commons.wikimedia.org/wiki/Special:FilePath/Aerial%20drone%20chase%20of%20a%20train.webm
dv_has_:
  name_:
    ab: адәыҕба
    aeb-arab: ترينو
    af: trein
    am: ባቡር
    an: tren
    ang: trǣg
    ann: teren
    anp: रेलगाड़ी
    ar: قطار
    ary: تران
    arz: اطر
    as: ৰেলগাড়ী
    ast: tren
    az: qatar
    azb: قاطار (نقلیه)
    ba: поезд
    bbc: kareta api
    be: цягнік
    be-tarask: цягнік
    bew: sepur
    bg: влак
    bho: रेलगाड़ी
    bjn: karita api
    bn: রেলগাড়ি
    bo: མེ་འཁོར།
    br: tren
    bs: voz
    bxr: поезд
    ca: tren
    cdo: huōi-chiă
    ce: цӏерпошт
    chy: ma'aataemeo'o
    ckb: شەمەندەفەر
    crh: tren
    cs: vlak
    csb: cuch
    cv: пуйăс
    cy: trên
    da: tog
    dag: ziliji
    de: Zug
    de-ch: Zug
    diq: trene
    ee: keteke
    el: τρένο
    eml: trein
    en: train
    en-ca: train
    en-gb: train
    eo: trajno
    es: tren
    et: rong
    eu: tren
    fa: قطار
    fi: juna
    fo: tok
    fon: pínpán
    fr: train
    fur: Tren
    fy: Trein
    ga: traein
    gan: 火車
    gd: trèana
    gl: tren
    gn: mba'yjua
    gsw: Zug
    gv: traen
    hak: Fó-chhâ
    he: רכבת
    hi: रेलगाड़ी
    hif: train
    hr: vlak
    hsb: ćah
    ht: tren
    hu: vonat
    hy: գնացք
    hyw: շոգեկառք
    ia: traino
    id: kereta api
    ig: ụgbọ okporo igwe
    io: treno
    is: járnbrautarlest
    it: treno
    ja: 列車
    jam: Chrien
    jv: sepur
    ka: მატარებელი
    kaa: poyezd
    kab: tamacint
    kbp: suluku kpɛyaɣ
    kcg: a̱taintuut
    kg: lukalu
    kk: пойыз
    kn: ರೈಲು
    ko: 열차
    ks: ریہل
    ku: trên
    kw: tren
    ky: поезд
    la: hamaxostichus
    lad: tréno
    lb: Zuch
    li: Trein
    lij: treno
    lld: ferata
    lmo: tren
    ln: engbunduka
    lo: ລົດໄຟ
    lt: traukinys
    lv: vilciens
    lzh: 火車
    mag: रेलगाड़ी
    map-bms: Sepur
    mdf: Улав
    mg: fiarandalamby
    mhr: поезд
    mk: воз
    ml: തീവണ്ടി
    mn: галт тэрэг
    mr: रेल्वे
    ms: kereta api
    my: မီးရထား
    myv: кшниулав
    nah: tepozcōātl
    nan: lia̍t-chhia
    nap: trieno
    nb: jernbanetog
    nds: Tog
    nds-nl: traain
    ne: रेल
    new: रेल
    nl: trein
    nn: jarnbanetog
    nrm: train
    nv: kǫʼ naʼałbąąsii diné bee nágéhé
    oc: tren
    olo: juna
    pa: ਰੇਲ
    pdc: Train
    pih: trien
    pl: pociąg
    pms: treno
    pnb: ٹرین
    ps: اورګاډی
    pt: comboio
    pt-br: trem
    qu: antakuru
    rm: tren
    rmf: fasuna
    rmy: gadi
    ro: tren
    ru: поезд
    sah: суксурҕа
    sat: ᱨᱮᱞ ᱜᱟᱹᱰᱤ
    sc: trenu
    scn: trenu
    sco: train
    sd: ريل گاڏي
    se: toga
    sgs: traukėnīs
    sh: vlak
    shi: alaway
    shn: လူတ်ႉၾႆး
    si: දුම්රිය
    sjd: по̄яс
    sje: tågå
    sk: vlak
    sl: vlak
    sma: tååge
    smj: tåhka
    smn: juná
    sms: poojâs
    sn: chitima
    so: tareen
    sq: treni
    sr: воз
    sr-ec: воз
    su: karéta api
    sv: tåg
    sw: treni
    szl: Cug
    szy: silamalay
    ta: தொடர்வண்டி
    te: రైలు
    th: รถไฟ
    tig: ባቡር
    tl: tren
    tok: tomo tawa linja
    tr: tren
    trv: rulu tqrngul
    tt: пуез
    tt-cyrl: поез
    tw: keteke
    udm: поезд
    uk: поїзд
    ur: قطار
    uz: poyezd
    vec: tren
    vep: jonuz
    vi: tàu hỏa
    vls: Tring
    vro: rong
    wa: trén
    war: tren
    wuu: 火车
    xh: uloliwe
    yi: באן
    yo: rélùwéè
    yue: 火車
    za: hojceh
    zgh: ⴰⵍⴰⵡⴰⵢ
    zh: 铁路列车
    zh-hans: 铁路列车
    zh-hant: 鐵路列車
    zh-tw: 鐵路列車
Unicode_character: 🚆
title: 🚆 Train
---

# [[Train]] 🚆 

#is_/same_as :: [[../../../WikiData/WD~Train,870|WD~Train,870]] 

## #has_/text_of_/abstract 

> A **Train** (from Old French trahiner, from Latin trahere, "to pull, to draw") 
> is a series of connected vehicles that run along a railway track and transport people or freight. 
> 
> Trains are typically pulled or pushed by locomotives (often known simply as "engines"), 
> though some are self-propelled, such as multiple units or railcars. 
> 
> Passengers and cargo are carried in railroad cars, also known as wagons or carriages. 
> Trains are designed to a certain gauge, or distance between rails. 
> Most trains operate on steel tracks with steel wheels, 
> the low friction of which makes them more efficient than other forms of transport. 
> Many countries use rail transport.
>
> Trains have their roots in wagonways, which used railway tracks 
> and were powered by horses or pulled by cables. 
> 
> Following the invention of the steam locomotive in the United Kingdom in 1802, 
> trains rapidly spread around the world, allowing freight and passengers to 
> move over land faster and cheaper than ever possible before. 
> 
> Rapid transit and trams were first built in the late 1800s to transport large numbers of people in and around cities. Beginning in the 1920s, and accelerating following World War II, diesel and electric locomotives replaced steam as the means of motive power. Following the development of cars, trucks, and extensive networks of highways which offered greater mobility, as well as faster airplanes, trains declined in importance and market share, and many rail lines were abandoned. The spread of buses led to the closure of many rapid transit and tram systems during this time as well.
>
> Since the 1970s, governments, environmentalists, and train advocates have promoted increased use of trains due to their greater fuel efficiency and lower greenhouse gas emissions compared to other modes of land transport. High-speed rail, first built in the 1960s, has proven competitive with cars and planes over short to medium distances. Commuter rail has grown in importance since the 1970s as an alternative to congested highways and a means to promote development, as has light rail in the 21st century. Freight trains remain important for the transport of bulk commodities such as coal and grain, as well as being a means of reducing road traffic congestion by freight trucks.
>
> While conventional trains operate on relatively flat tracks with two rails, a number of specialized trains exist which are significantly different in their mode of operation. Monorails operate on a single rail, while funiculars and rack railways are uniquely designed to traverse steep slopes. Experimental trains such as high speed maglevs, which use magnetic levitation to float above a guideway, are under development in the 2020s and offer higher speeds than even the fastest conventional trains. Trains which use alternative fuels such as natural gas and hydrogen are another 21st-century development.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Train) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Transport/Rail_Transport/Train|Train]] 

### #is_/same_as :: [[/_public/Technology/Transport/Rail_Transport/Train.public|Train.public]] 

### #is_/same_as :: [[/_internal/Technology/Transport/Rail_Transport/Train.internal|Train.internal]] 

### #is_/same_as :: [[/_protect/Technology/Transport/Rail_Transport/Train.protect|Train.protect]] 

### #is_/same_as :: [[/_private/Technology/Transport/Rail_Transport/Train.private|Train.private]] 

### #is_/same_as :: [[/_personal/Technology/Transport/Rail_Transport/Train.personal|Train.personal]] 

### #is_/same_as :: [[/_secret/Technology/Transport/Rail_Transport/Train.secret|Train.secret]] 

