---
aliases:
- Kereta api Maglev
- Kereta maglev
- kolej magnetyczna
- lebegő mágnesvasút
- Maglev
- Maglev Train
- MAGLEV voz
- Maglevtåg
- Maglev_Train
- Magneetsweeftrein
- magneetzweeftrein
- Magnethõljukrong
- magnetic levitation
- magnetleva trajno
- Magnetno levitacijski vlak
- Magnetno levitacijski voz
- Magnetschwebebahn
- Magnetsvævebane
- magnētiskās levitācijas vilciens
- Manyetik raylı tren
- Mæȝuppā
- traein ardaithe go maighnéadach
- train à sustentation magnétique
- Transrapid
- Tren de levitación magnética
- Tren Maglev
- Trenat me e energji magnetike
- Trenioù a levitadur magnetek
- Treno a levitazion magnetica
- treno a levitazione magnetica
- Treno Maglev
- Tàu đệm từ
- Μαγνητικό τρένο
- маглеў
- Маглев
- МАГЛЕВ воз
- магнетнолебдечки воз
- מאגלעוו באן
- רכבת ריחוף מגנטי
- علناطیس
- قطار مغناطيسي معلق
- قطار مگلو
- ماگلەۆ
- مقناطیسی ریل
- ميگليو
- मैग्लेव ट्रेन
- চুম্বক-উত্তোলিত রেল
- ਮੈਗਲੇਵ
- மிதக்கும் தொடர்வண்டி
- ಮ್ಯಾಗ್ಲೆವ್ ಟ್ರೈನ್
- മാഗ് ലെവ് ട്രെയിൻ
- รถไฟพลังแม่เหล็ก
- သံလိုက်ရထား
- მაგლევი
- 磁悬浮列车
- 磁懸浮列車
- 磁気浮上式鉄道
- 磁浮列車
- 자기부상열차
has_id_wikidata: Q160047
powered_by:
- '[[_Standards/WikiData/WD~jet_force,748254]]'
- '[[_Standards/WikiData/WD~linear_motor,1164566]]'
uses: '[[_Standards/WikiData/WD~magnetic_levitation,1061251]]'
instance_of: '[[_Standards/WikiData/WD~train_and_rail_category,16858238]]'
subclass_of: '[[_Standards/WikiData/WD~train,870]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Shanghai%20maglev%20train.jpg
Commons_gallery: Maglev
Commons_category: Magnetic levitation trains
dv_has_:
  name_:
    af: Magneetsweeftrein
    ang: Mæȝuppā
    ar: قطار مغناطيسي معلق
    ast: Tren de levitación magnética
    be: маглеў
    be-tarask: Маглеў
    bg: Маглев
    bn: চুম্বক-উত্তোলিত রেল
    br: Trenioù a levitadur magnetek
    ca: Tren Maglev
    cs: Maglev
    da: Magnetsvævebane
    de: Magnetschwebebahn
    el: Μαγνητικό τρένο
    en: maglev
    eo: magnetleva trajno
    es: tren de levitación magnética
    et: Magnethõljukrong
    eu: Maglev
    fa: قطار مگلو
    fi: Maglev
    fr: train à sustentation magnétique
    ga: traein ardaithe go maighnéadach
    gl: Tren de levitación magnética
    he: רכבת ריחוף מגנטי
    hi: मैग्लेव ट्रेन
    hr: Magnetno levitacijski vlak
    hu: lebegő mágnesvasút
    id: Kereta maglev
    it: treno a levitazione magnetica
    ja: 磁気浮上式鉄道
    jv: Maglev
    ka: მაგლევი
    kk: Маглев
    kk-arab: ماگلەۆ
    kk-cn: ماگلەۆ
    kk-cyrl: Маглев
    kk-kz: Маглев
    kk-latn: Maglev
    kk-tr: Maglev
    kn: ಮ್ಯಾಗ್ಲೆವ್ ಟ್ರೈನ್
    ko: 자기부상열차
    lmo: Treno a levitazion magnetica
    lt: Maglev
    lv: magnētiskās levitācijas vilciens
    mk: магнетнолебдечки воз
    ml: മാഗ് ലെവ് ട്രെയിൻ
    mn: Маглев
    ms: Kereta api Maglev
    my: သံလိုက်ရထား
    nb: maglev
    nl: magneetzweeftrein
    pa: ਮੈਗਲੇਵ
    pl: kolej magnetyczna
    pms: Treno Maglev
    pnb: مقناطیسی ریل
    pt: maglev
    pt-br: maglev
    ro: maglev
    ru: Маглев
    sah: Маглев
    sco: Maglev
    sd: ميگليو
    sh: Magnetno levitacijski voz
    sk: Maglev
    sl: maglev
    sq: Trenat me e energji magnetike
    sr: МАГЛЕВ воз
    sr-ec: МАГЛЕВ воз
    sr-el: MAGLEV voz
    sv: Maglevtåg
    ta: மிதக்கும் தொடர்வண்டி
    th: รถไฟพลังแม่เหล็ก
    tr: Manyetik raylı tren
    uk: маглев
    ur: علناطیس
    vi: Tàu đệm từ
    wuu: 磁悬浮列车
    yi: מאגלעוו באן
    yue: 磁浮列車
    zh: 磁懸浮列車
    zh-cn: 磁悬浮列车
    zh-hans: 磁悬浮列车
    zh-hant: 磁浮列車
    zh-hk: 磁懸浮列車
    zh-sg: 磁悬浮列车
    zh-tw: 磁浮列車
---
# [[Maglev_Train]] 

#is_/same_as :: [[WD~Maglev_train,160047]] 

## #has_/text_of_/abstract 

> Maglev (derived from magnetic levitation) is a system of rail transport 
> whose rolling stock is levitated by electromagnets rather than rolled on wheels, 
> eliminating rolling resistance.
>
> Compared to conventional railways, maglev trains have higher top speeds, 
> superior acceleration and deceleration, lower maintenance costs, 
> improved gradient handling, and lower noise. 
> However, they are more expensive to build, cannot use existing infrastructure, 
> and use more energy at high speeds.
>
> Maglev trains have set several speed records. The train speed record of 603 km/h (375 mph) 
> was set by the experimental Japanese L0 Series maglev in 2015. 
> From 2002 until 2021, the record for the highest operational speed of a passenger train 
> of 431 kilometres per hour (268 mph) was held by the Shanghai maglev train, 
> which uses German Transrapid technology. 
> 
> The service connects Shanghai Pudong International Airport 
> and the outskirts of central Pudong, Shanghai. 
> At its historical top speed, it covered the distance of 30.5 kilometres (19 mi) 
> in just over 8 minutes (average speed: 228.75 km/h).
>
> Different maglev systems achieve levitation in different ways, 
> which broadly fall into two categories: 
> electromagnetic suspension (EMS) and electrodynamic suspension (EDS). 
> 
> Propulsion is typically provided by a linear motor. 
> The power needed for levitation is typically 
> **not a large percentage of the overall energy** consumption of a high-speed maglev system. 
> Instead, overcoming drag takes the most energy. 
> Vactrain technology has been proposed as a means to overcome this limitation.
>
> Despite over a century of research and development, 
> there are only seven operational maglev trains today — 
> four in China, two in South Korea, and one in Japan.
>
> Two inter-city maglev lines are currently under construction, 
> the Chūō Shinkansen connecting Tokyo and Nagoya, 
> and a line between Changsha and Liuyang in Hunan Province, China.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Maglev) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Transport/Rail_Transport/Maglev_Train|Maglev_Train]] 

### #is_/same_as :: [[/_public/Technology/Transport/Rail_Transport/Maglev_Train.public|Maglev_Train.public]] 

### #is_/same_as :: [[/_internal/Technology/Transport/Rail_Transport/Maglev_Train.internal|Maglev_Train.internal]] 

### #is_/same_as :: [[/_protect/Technology/Transport/Rail_Transport/Maglev_Train.protect|Maglev_Train.protect]] 

### #is_/same_as :: [[/_private/Technology/Transport/Rail_Transport/Maglev_Train.private|Maglev_Train.private]] 

### #is_/same_as :: [[/_personal/Technology/Transport/Rail_Transport/Maglev_Train.personal|Maglev_Train.personal]] 

### #is_/same_as :: [[/_secret/Technology/Transport/Rail_Transport/Maglev_Train.secret|Maglev_Train.secret]] 

