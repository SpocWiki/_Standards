---
aliases:
- alama ya barabarani
- arwydd traffig
- biển báo giao thông
- Ceļa zīme
- comhartha tráchta
- dopravná značka
- dopravní značka
- Ferkearsboerd
- Ferðsluskelti
- Ilhanan sa dalan
- indicator rutier
- jelzőtábla
- johtolatmearka
- Jol belgileri
- kelio ženklas
- kelė znuoks
- liikennemerkki
- liiklusmärk
- Palatandaang pantrapiko
- panneau routier
- panèu de sinhalisacion rotiera
- papan tanda lalu lintas
- prometni znak
- prometni znakovi
- rahtegalbba
- rambu lalu lintas
- rambu rambu lalu lintas
- road sign
- saobraćajni znak
- segnal stradal
- segnale stradale
- senyal de trànsit
- señal de tráfico
- señales de tráficu
- Shenja e trafikut
- Signal stradal
- Signum viale
- sinais de tráfico
- sinalização rodoviária
- traffec sign
- Traffic Sign
- Traffic_Sign
- trafik işaretleri
- trafikkskilt
- trafiko seinale
- trafiksigno
- umferðarmerki
- vejtavle
- verkeersbord
- Verkehrsschild
- Verkehrszeichen
- Verkéiersschëld
- viegasjyllta
- vägmärke
- wobchadna značka
- yol nişanları
- yoʻl belgilari
- znak drogowy
- çул палли
- κυκλοφοριακή πινακίδα
- аломатҳои роҳ
- вожне
- дарожны знак
- дорожній знак
- дорожный знак
- жол белгілері
- замын хөдөлгөөний тэмдэг
- пътен знак
- саобраћајни знак
- сообраќаен знак
- фæндаджы нысан
- Ճանապարհային նշան
- תמרור
- اچ وڃ جي نشاني
- جول بەلگىلەرى
- لافتة مرورية
- نشان راہ
- نشانهای راهنمایی و رانندگی
- يول بەلگىسى
- ٹریفک سائین
- यातायात संकेत
- வழிகாட்டி அடையாளம்
- මාර්ග සංඥා
- เครื่องหมายจราจร
- ပၢႆႉတၢင်းလူင်
- လမ်းညွှန်ဆိုင်းဘုတ်
- საგზაო ნიშნები
- 交通標誌
- 路牌
- 道路交通标志
- 道路交通標誌
- 道路標識
- 교통표지
has_id_wikidata: Q170285
instance_of: '[[_Standards/WikiData/WD~notation,2001982]]'
different_from:
- '[[_Standards/WikiData/WD~emergency_sign,2146470]]'
- '[[_Standards/WikiData/WD~road_traffic_control_device,7339568]]'
subclass_of:
- '[[_Standards/WikiData/WD~road_equipment,2354362]]'
- '[[_Standards/WikiData/WD~road_traffic_control_device,7339568]]'
- '[[_Standards/WikiData/WD~physical_sign,105449313]]'
part_of:
- '[[_Standards/WikiData/WD~traffic_sign_theft,3178948]]'
- '[[_Standards/WikiData/WD~traffic_signing,22812380]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Traffic_signs,13361292]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._7,123625363]]'
Nomenclature_for_Museum_Cataloging: 11442
Iconclass_notation: 46C1151
Commons_category: Road signs
image: http://commons.wikimedia.org/wiki/Special:FilePath/Vyhrazen%C3%BD%20j%C3%ADzdn%C3%AD%20pruh%20pro%20cyklisty.jpg
UNSPSC_Code: 55121710
IEV_number: 845-31-061
Commons_gallery: Traffic sign
Dewey_Decimal_Classification:
- 625.794
- 346.045
- 343.0946
- 343.0982
- 388.413122
- 388.3122
described_at_URL:
- https://treaties.un.org/doc/Publication/UNTS/Volume%201091/volume-1091-I-16743-English.pdf
- https://assets.publishing.service.gov.uk/media/560aa95b40f0b6036a00002a/the-highway-code-traffic-signs.pdf
- https://www.europcar.com.ph/uploads/1175/Road_Traffic_Signs_v4.pdf
- https://imagenes.racc.es/pub/ficheros/adjuntos/adjuntos_roadsignsleafletonlineversion2008_jzq_d4a673fb.pdf
- https://mutcd.fhwa.dot.gov/services/publications/fhwaop02084/us_road_symbol_signs.pdf
- https://assets.publishing.service.gov.uk/media/656ef4271104cf0013fa74ef/know-your-traffic-signs-dft.pdf
OpenStreetMap_key: traffic_sign
dv_has_:
  name_:
    ar: لافتة مرورية
    ast: señales de tráficu
    az: yol nişanları
    be: дарожны знак
    be-tarask: дарожны знак
    bg: пътен знак
    ca: senyal de trànsit
    ceb: Ilhanan sa dalan
    cs: dopravní značka
    cv: çул палли
    cy: arwydd traffig
    da: vejtavle
    de: Verkehrsschild
    el: κυκλοφοριακή πινακίδα
    en: traffic sign
    en-gb: road sign
    eo: trafiksigno
    es: señal de tráfico
    et: liiklusmärk
    eu: trafiko seinale
    fa: نشانهای راهنمایی و رانندگی
    fi: liikennemerkki
    fo: Ferðsluskelti
    fr: panneau routier
    fy: Ferkearsboerd
    ga: comhartha tráchta
    gl: sinais de tráfico
    he: תמרור
    hi: यातायात संकेत
    hr: prometni znakovi
    hsb: wobchadna značka
    hu: jelzőtábla
    hy: Ճանապարհային նշան
    id: rambu lalu lintas
    is: umferðarmerki
    it: segnale stradale
    ja: 道路標識
    jv: rambu rambu lalu lintas
    ka: საგზაო ნიშნები
    kaa: Jol belgileri
    kk: жол белгілері
    kk-arab: جول بەلگىلەرى
    kk-cn: جول بەلگىلەرى
    kk-cyrl: жол белгілері
    kk-kz: жол белгілері
    kk-latn: jol belgileri
    kk-tr: jol belgileri
    ko: 교통표지
    la: Signum viale
    lb: Verkéiersschëld
    lmo: segnal stradal
    lt: kelio ženklas
    lv: Ceļa zīme
    mk: сообраќаен знак
    mn: замын хөдөлгөөний тэмдэг
    ms: papan tanda lalu lintas
    my: လမ်းညွှန်ဆိုင်းဘုတ်
    myv: вожне
    nb: trafikkskilt
    nl: verkeersbord
    nn: trafikkskilt
    oc: panèu de sinhalisacion rotiera
    os: фæндаджы нысан
    pl: znak drogowy
    pms: Signal stradal
    pnb: ٹریفک سائین
    pt: sinalização rodoviária
    ro: indicator rutier
    ru: дорожный знак
    sco: traffec sign
    sd: اچ وڃ جي نشاني
    se: johtolatmearka
    sgs: kelė znuoks
    sh: saobraćajni znak
    shn: ပၢႆႉတၢင်းလူင်
    si: මාර්ග සංඥා
    sje: viegasjyllta
    sk: dopravná značka
    sl: prometni znak
    smj: rahtegalbba
    sq: Shenja e trafikut
    sr: саобраћајни знак
    sr-ec: саобраћајни знак
    sr-el: saobraćajni znak
    sv: vägmärke
    sw: alama ya barabarani
    ta: வழிகாட்டி அடையாளம்
    tg: аломатҳои роҳ
    th: เครื่องหมายจราจร
    tl: Palatandaang pantrapiko
    tr: trafik işaretleri
    ug: يول بەلگىسى
    uk: дорожній знак
    ur: نشان راہ
    uz: yoʻl belgilari
    vi: biển báo giao thông
    wuu: 道路交通标志
    yue: 路牌
    zh: 道路交通標誌
    zh-cn: 道路交通标志
    zh-hans: 道路交通标志
    zh-hant: 道路交通標誌
    zh-hk: 交通標誌
    zh-sg: 道路交通标志
    zh-tw: 道路交通標誌
---

# [[Traffic_Sign]] 

#is_/same_as :: [[../../../../WikiData/WD~Traffic_sign,170285|WD~Traffic_sign,170285]] 

## #has_/text_of_/abstract 

> Traffic signs or road signs are signs erected at the side of or above roads 
> to give instructions or provide information to road users. 
> 
> The earliest signs were simple wooden or stone milestones. 
> Later, signs with directional arms were introduced, for example 
> the fingerposts in the United Kingdom 
> and their wooden counterparts in Saxony.
>
> With traffic volumes increasing since the 1930s, 
> many countries have adopted pictorial signs or 
> otherwise simplified and standardized their signs to overcome language barriers, 
> and enhance traffic safety. 
> 
> Such pictorial signs use symbols (often silhouettes) in place of words 
> and are usually based on international protocols. 
> 
> Such signs were first developed in Europe, 
> and have been adopted by most countries to varying degrees.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Traffic%20sign) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Transport/Road_Transport/Traffic/Traffic_Sign|Traffic_Sign]] 

### #is_/same_as :: [[/_public/Technology/Transport/Road_Transport/Traffic/Traffic_Sign.public|Traffic_Sign.public]] 

### #is_/same_as :: [[/_internal/Technology/Transport/Road_Transport/Traffic/Traffic_Sign.internal|Traffic_Sign.internal]] 

### #is_/same_as :: [[/_protect/Technology/Transport/Road_Transport/Traffic/Traffic_Sign.protect|Traffic_Sign.protect]] 

### #is_/same_as :: [[/_private/Technology/Transport/Road_Transport/Traffic/Traffic_Sign.private|Traffic_Sign.private]] 

### #is_/same_as :: [[/_personal/Technology/Transport/Road_Transport/Traffic/Traffic_Sign.personal|Traffic_Sign.personal]] 

### #is_/same_as :: [[/_secret/Technology/Transport/Road_Transport/Traffic/Traffic_Sign.secret|Traffic_Sign.secret]] 

