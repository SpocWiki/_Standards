---
aliases:
- Abrid
- baalka
- bidezidor
- gångstig
- pad
- pado
- path
- patika
- Pazea
- percurso pedestre
- pešpot
- Pfad
- polku
- puć
- Rada
- Semita
- senda
- sendeiro
- sender
- Senter
- sentier
- sentiero
- Senté
- sienda
- staza
- stezka
- sti
- Taks
- track
- Trail
- Trail (course à piés)
- Trampelpfad
- Treko
- trilha
- Troi
- trywydd
- Wee
- Weg
- Wäg
- ösvény
- ścieżka
- Πεζοπορικά μονοπάτια
- Орох
- патека
- стаза
- стежка
- Сукмак
- сцежка
- сьцежка
- Тача (некъ)
- тропа
- שביל
- تریل
- جادہ
- مسلك
- ڈنڈی
- ڕێچکە
- पगडंडी
- ਟ੍ਰੇਲ
- トレイル
- 小径
- 小徑
- 小路
- 트레일
has_id_wikidata: Q628179
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Arkeologisen_kulttuuriperinnön_opas,65109596]]'
subclass_of: '[[_Standards/WikiData/WD~path,5004679]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~outdoor_route,11899420]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Infobox_trail,13362630]]'
maintenance_method: '[[_Standards/WikiData/WD~trail_maintenance,66314461]]'
properties_for_this_type:
- terminus
- length
- degree of difficulty
- cumulative elevation gain
- located in the administrative territorial entity
Commons_gallery: Trail
U_S_National_Archives_Identifier: 10642332
image: http://commons.wikimedia.org/wiki/Special:FilePath/Burg%20Epprechtstein.jpg
GeoNames_feature_code: R.TRL
Commons_category: Trails
OpenStreetMap_tag:
- highway=path
- highway=track
dv_has_:
  name_:
    an: Sender
    ar: مسلك
    ast: sienda
    be: сцежка
    be-tarask: сьцежка
    ca: sender
    ce: Тача (некъ)
    ckb: ڕێچکە
    cs: stezka
    cv: Сукмак
    cy: trywydd
    da: sti
    de: Trampelpfad
    el: Πεζοπορικά μονοπάτια
    en: trail
    eo: pado
    es: senda
    et: Rada
    eu: bidezidor
    fa: تریل
    fi: polku
    fr: sentier
    gl: sendeiro
    gsw: Wäg
    he: שביל
    hi: पगडंडी
    hsb: puć
    hu: ösvény
    io: Treko
    it: sentiero
    ja: トレイル
    ko: 트레일
    la: Semita
    lb: Wee
    lld: Troi
    lmo: Senter
    mk: патека
    nb: sti
    nds: Weg
    nl: pad
    nn: sti
    pa: ਟ੍ਰੇਲ
    pcd: Trail (course à piés)
    pl: ścieżka
    pms: Senté
    pnb: ڈنڈی
    pt: percurso pedestre
    pt-br: trilha
    ru: тропа
    sah: Орох
    sco: trail
    sgs: Taks
    shi: Abrid
    sl: pešpot
    sma: baalka
    sr: стаза
    sr-ec: стаза
    sr-el: staza
    sv: gångstig
    tr: patika
    tt: сукмак
    uk: стежка
    ur: جادہ
    vec: Troi
    wa: Pazea
    yue: 小路
    zh: 小径
    zh-hans: 小径
    zh-hant: 小徑
---
# [[Trail]] 

#is_/same_as :: [[../../../WikiData/WD~Trail,628179|WD~Trail,628179]]  

## #has_/text_of_/abstract 

> A **Trail**, also known as a path or track, is an unpaved lane or a small paved road 
> (though it can also be a route along a navigable waterways) generally 
> not intended for usage by motorized vehicles, usually passing through a natural area. 
> However, it is sometimes applied to highways in North America. 
>
> In the United Kingdom and Ireland, a path or footpath 
> is the preferred term for a pedestrian or hiking trail. 
> 
> In the US, the term was historically used for a route into or through wild territory 
> used by explorers and migrants (e.g. the Oregon Trail).  
> In the United States, "trace" is a synonym for trail, as in Natchez Trace.
>
> Some trails are restricted to use by only walkers, or cyclists, or equestrians, 
> or for snowshoeing, or cross-country skiing, others, for example 
> bridleways in the UK, are shared, and can be used by walkers, cyclists and equestrians. 
> 
> Although most ban motorized use, there are unpaved trails used by dirt bikes, 
> quad bikes and other off-road vehicles, usually for extreme sports and rally races. 
> 
> In some places, like the Alps, trails are used by alpine agrarian communities 
> for moving cattle and other livestock.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Trail) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Transport/Road_Transport/Trail|Trail]] 

### #is_/same_as :: [[/_public/Technology/Transport/Road_Transport/Trail.public|Trail.public]] 

### #is_/same_as :: [[/_internal/Technology/Transport/Road_Transport/Trail.internal|Trail.internal]] 

### #is_/same_as :: [[/_protect/Technology/Transport/Road_Transport/Trail.protect|Trail.protect]] 

### #is_/same_as :: [[/_private/Technology/Transport/Road_Transport/Trail.private|Trail.private]] 

### #is_/same_as :: [[/_personal/Technology/Transport/Road_Transport/Trail.personal|Trail.personal]] 

### #is_/same_as :: [[/_secret/Technology/Transport/Road_Transport/Trail.secret|Trail.secret]] 

