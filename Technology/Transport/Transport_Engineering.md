---
aliases:
- dopravní inženýrství
- engenharia de transportes
- enginyeria de transports
- Garraioaren ingeniaritza
- ingegneria dei trasporti
- Ingegnerìa dij traspòrt
- Ingeniaria transportationis
- ingeniería del transporte
- ingineria transporturilor
- ingénierie des systèmes de transport
- Inhinyeriyang pangtransportasyon
- innealtóireachta iompair
- Inxeniería del tresporte
- Inxhinieria e transportit
- inzegnaria dei trasporti
- Inżynieria transportu
- közlekedésmérnöki tudomány
- liikennetekniikka
- peirianneg cludiant
- prometno inženirstvo
- trafikkteknikk
- transport engineering
- Transport inženerligi
- Transportation engineering
- Transportation_Engineering
- transportingeniørvidenskab
- Transportinĝenieriko
- Transportno inženjerstvo
- Transport_Engineering
- Uhandisi wa usafiri
- Verkehrsingenieurwesen
- vervoerskunde
- транспартная інжынерыя
- Транспартная інжынэрыя
- транспортна інженерія
- транспортная инженерия
- Տրանսպորտային ճարտարագիտություն
- הנדסת תחבורה
- ئەندازیاریی گواستنەوە
- مهندسی ترافیک (حملونقل)
- هندسة النقل
- वाहतूक अभियांत्रिकी
- পরিবহন প্রকৌশল
- போக்குவரத்துப் பொறியியல்
- วิศวกรรมขนส่ง
- ትራንስፖርት ምህንድስና
- 交通工学
- 交通工程
- 교통공학
has_id_wikidata: Q775325
part_of:
- '[[_Standards/WikiData/WD~civil_engineering,77590|WD~civil_engineering,77590]]'
- '[[_Standards/WikiData/WD~transport_sciences,2516517|WD~transport_sciences,2516517]]'
subclass_of:
- '[[_Standards/WikiData/WD~civil_engineering,77590|WD~civil_engineering,77590]]'
- '[[_Standards/WikiData/WD~transport_sciences,2516517|WD~transport_sciences,2516517]]'
instance_of:
- '[[_Standards/WikiData/WD~field_of_work,627436|WD~field_of_work,627436]]'
- '[[_Standards/WikiData/WD~academic_major,4671286|WD~academic_major,4671286]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829|WD~academic_discipline,11862829]]'
- '[[_Standards/WikiData/WD~branch_of_engineering,12015335|WD~branch_of_engineering,12015335]]'
practiced_by: '[[_Standards/WikiData/WD~transportation_engineer,11875786|WD~transportation_engineer,11875786]]'
Colon_Classification: D5
Commons_category: Transport engineering
Iconclass_notation: 47B212
dv_has_:
  name_:
    am: ትራንስፖርት ምህንድስና
    ar: هندسة النقل
    ast: Inxeniería del tresporte
    be: транспартная інжынерыя
    be-tarask: Транспартная інжынэрыя
    bn: পরিবহন প্রকৌশল
    ca: enginyeria de transports
    ckb: ئەندازیاریی گواستنەوە
    cs: dopravní inženýrství
    cy: peirianneg cludiant
    da: transportingeniørvidenskab
    de: Verkehrsingenieurwesen
    en: transportation engineering
    eo: Transportinĝenieriko
    es: ingeniería del transporte
    eu: Garraioaren ingeniaritza
    fa: مهندسی ترافیک (حملونقل)
    fi: liikennetekniikka
    fr: ingénierie des systèmes de transport
    ga: innealtóireachta iompair
    he: הנדסת תחבורה
    hu: közlekedésmérnöki tudomány
    hy: Տրանսպորտային ճարտարագիտություն
    it: ingegneria dei trasporti
    ja: 交通工学
    ko: 교통공학
    la: Ingeniaria transportationis
    mr: वाहतूक अभियांत्रिकी
    nb: trafikkteknikk
    nl: vervoerskunde
    nn: trafikkteknikk
    pl: Inżynieria transportu
    pms: Ingegnerìa dij traspòrt
    pt: engenharia de transportes
    ro: ingineria transporturilor
    ru: транспортная инженерия
    sl: prometno inženirstvo
    sq: Inxhinieria e transportit
    sr: Transportno inženjerstvo
    sw: Uhandisi wa usafiri
    ta: போக்குவரத்துப் பொறியியல்
    th: วิศวกรรมขนส่ง
    tk: Transport inženerligi
    tl: Inhinyeriyang pangtransportasyon
    uk: транспортна інженерія
    vec: inzegnaria dei trasporti
    wuu: 交通工程
    yue: 交通工程
    zh: 交通工程
    zh-hant: 交通工程
    zh-tw: 交通工程
---

# [[Transport_Engineering]] 

#is_/same_as :: [[../../WikiData/WD~Transportation_engineering,775325|WD~Transportation_engineering,775325]]  

## #has_/text_of_/abstract 

> Transportation engineering or transport engineering 
> is the application of technology and scientific principles 
> to the planning, functional design, operation and management of facilities 
> for any mode of transportation to provide for the 
> safe, efficient, rapid, comfortable, convenient, economical, and environmentally compatible 
> movement of people and goods transport.
> 
> Transportation engineering is considered a sub discipline of civil engineering, 
> focusing on infrastructure systems that enable mobility of people and goods.
> 
> It addresses both passenger and freight systems, 
> balancing safety, cost effectiveness, and environmental impact.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Transportation%20engineering) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Transport/Transport_Engineering|Transport_Engineering]] 

### #is_/same_as :: [[/_public/Technology/Transport/Transport_Engineering.public|Transport_Engineering.public]] 

### #is_/same_as :: [[/_internal/Technology/Transport/Transport_Engineering.internal|Transport_Engineering.internal]] 

### #is_/same_as :: [[/_protect/Technology/Transport/Transport_Engineering.protect|Transport_Engineering.protect]] 

### #is_/same_as :: [[/_private/Technology/Transport/Transport_Engineering.private|Transport_Engineering.private]] 

### #is_/same_as :: [[/_personal/Technology/Transport/Transport_Engineering.personal|Transport_Engineering.personal]] 

### #is_/same_as :: [[/_secret/Technology/Transport/Transport_Engineering.secret|Transport_Engineering.secret]] 

