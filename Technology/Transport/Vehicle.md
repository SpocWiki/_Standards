---
aliases:
- antapaykachaq
- automjet
- behikulo
- carbyd
- cerbyd
- ekúmbi
- Faartjüchen
- Fahrzeug
- farartæki
- farkost
- feithicil
- fievru
- fiävru
- Fohrtüüg
- fordon
- gaadiid
- gandaran
- gari
- Gefier
- gău-tŭng gĕ̤ng-gê̤ṳ
- ibilgailu
- Imoto/Inqwelo amfutha
- Isithuthi
- is_a_vehicle
- jármű
- jååʹttemneävv
- jězdźidło
- kandaraan
- karbed
- kau-thong kang-khū
- kendaraan
- kenderaan
- kulkuneuvo
- kumbi
- kyangcung
- kâu-thûng kûng-khí
- køretøj/fartøj/fly
- lunadan
- mexo de trasporto
- muchovha
- ollepaskro
- otoo
- padsakayan
- pojazd
- sarakyan
- sasakyan
- Shararrisaa
- sõidoriist
- sõiduk
- taşıt
- tcheriant indjén
- tetunggangan
- transportlīdzeklis
- transporto priemonė
- transportyor
- vakamotoka
- Veculo
- vehicle
- vehicul
- vehiculo
- vehiculum
- vehículo
- vehículu
- veicolo
- veturilo
- Veìcoj
- veìcol
- veìculu
- veículo
- veïcul
- viikl
- voertuig
- vozidlo
- vozilo
- véhicule
- xe
- όχημα
- возило
- воситаҳои нақлиёт
- көлө
- превозно средство
- транспартны сродак
- транспорт чарасы
- транспортний засіб
- транспортное средство
- унаа каражаты
- шупшыктымаш йӧн
- փոխադրամիջոց
- כלי תחבורה
- פארמיטל
- سواري
- سواری
- مركبة
- مركوب
- نقلیه وسیلهسی
- وسیله نقلیه
- پێڕاگوێز
- کندراءن
- گاڑی
- गसा
- यान
- वाहन
- वाहनम्
- सवारी साधन
- যানবাহন
- ਸਵਾਰੀ
- வண்டி
- వాహనము
- ವಾಹನ
- വാഹനം
- වාහන
- ยานพาหนะ
- ຍານພາຫະນະ
- ယာဉ်
- ọkọ̀
- 乗り物
- 交通工具
- 車
- 載具
- 载具
- 탈것
has_id_wikidata: Q42889
confidential: public
cssclasses:
- Type
draft: false
expiryDate: ''
isDeleted: false
isReadOnly: false
keywords:
- vehicle
layout: ''
license: CC BY-SA 4.0
linkTitle: is_a_vehicle Class
publish: true
publishDate: ''
tags:
- class/Class
- schema-org/Class
title: is_a_vehicle Class
type: Type
dv_has_:
  name_:
    af: voertuig
    ar: مركبة
    ary: مركوب
    ast: vehículu
    azb: نقلیه وسیلهسی
    bcl: lunadan
    be: транспартны сродак
    be-tarask: транспартны сродак
    bew: gandaran
    bg: превозно средство
    bho: यान
    bn: যানবাহন
    br: karbed
    bs: vozilo
    ca: vehicle
    cdo: gău-tŭng gĕ̤ng-gê̤ṳ
    ckb: پێڕاگوێز
    cs: vozidlo
    cy: cerbyd
    da: køretøj/fartøj/fly
    de: Fahrzeug
    dtp: padsakayan
    el: όχημα
    eml: veìcol
    en: vehicle
    en-ca: vehicle
    en-gb: vehicle
    eo: veturilo
    es: vehículo
    et: sõiduk
    eu: ibilgailu
    fa: وسیله نقلیه
    fi: kulkuneuvo
    fj: vakamotoka
    fr: véhicule
    frr: Faartjüchen
    ga: feithicil
    gan: 車
    gl: vehículo
    gsw: Fahrzeug
    gv: carbyd
    ha: vehicle
    hak: kâu-thûng kûng-khí
    he: כלי תחבורה
    hi: वाहन
    hr: vozilo
    hsb: jězdźidło
    hu: jármű
    hy: փոխադրամիջոց
    ia: vehiculo
    id: kendaraan
    is: farartæki
    it: veicolo
    ja: 乗り物
    jam: viikl
    jv: tetunggangan
    kcg: kyangcung
    kg: kumbi
    kn: ವಾಹನ
    ko: 탈것
    krj: sarakyan
    ky: унаа каражаты
    la: vehiculum
    lb: Gefier
    lfn: Veculo
    ln: ekúmbi
    lo: ຍານພາຫະນະ
    lt: transporto priemonė
    lv: transportlīdzeklis
    mhr: шупшыктымаш йӧн
    min: kandaraan
    mk: возило
    ml: വാഹനം
    mr: वाहन
    ms: kenderaan
    ms-arab: کندراءن
    my: ယာဉ်
    nan: kau-thong kang-khū
    nb: farkost
    nds: Fohrtüüg
    ne: सवारी साधन
    new: गसा
    nl: voertuig
    nn: farkost
    oc: veïcul
    om: Shararrisaa
    pa: ਸਵਾਰੀ
    pl: pojazd
    pms: Veìcoj
    pnb: سواری
    pt: veículo
    pt-br: veículo
    qu: antapaykachaq
    rmy: ollepaskro
    ro: vehicul
    ru: транспортное средство
    sa: वाहनम्
    sah: көлө
    scn: veìculu
    sd: سواري
    se: fievru
    sg: otoo
    sh: vozilo
    si: වාහන
    sk: vozidlo
    sl: vozilo
    smn: fiävru
    sms: jååʹttemneävv
    sn: muchovha
    so: gaadiid
    sq: automjet
    sr: возило
    sr-ec: возило
    sr-el: vozilo
    sv: fordon
    sw: gari
    ta: வண்டி
    te: వాహనము
    tg: воситаҳои нақлиёт
    th: ยานพาหนะ
    tl: sasakyan
    tr: taşıt
    tt: транспорт чарасы
    tt-cyrl: транспорт чарасы
    uk: транспортний засіб
    ur: گاڑی
    uz: transportyor
    vec: mexo de trasporto
    vi: xe
    vro: sõidoriist
    wa: tcheriant indjén
    war: behikulo
    wuu: 载具
    xh: Imoto/Inqwelo amfutha
    yi: פארמיטל
    yo: ọkọ̀
    yue: 交通工具
    zh: 載具
    zh-hans: 载具
    zh-hant: 載具
    zh-hk: 載具
    zh-tw: 載具
    zu: Isithuthi
---
# [[Vehicle]] 

Class of all vehicles.

Tag Instances like this: 
#is_a_/vehicle

A vehicle is a device that is designed or used to transport people or cargo over land, water, air, or through space.

#has_/parent_class :: [[../../Society/Agent/Community/Organization/Business/Product|Product]]  

#has_/child_/class :: [ BusOrCoach, Car, Motorcycle, MotorizedBicycle ]  

#has_/properties :: [ accelerationTime, additionalProperty, additionalType, aggregateRating, alternateName, asin, audience, award, awards, bodyType, brand, callSign, cargoVolume, category, color, countryOfAssembly, countryOfLastProcessing, countryOfOrigin, dateVehicleFirstRegistered, depth, description, disambiguatingDescription, driveWheelConfiguration, emissionsCO2, fuelCapacity, fuelConsumption, fuelEfficiency, fuelType, funding, gtin, gtin12, gtin13, gtin14, gtin8, hasAdultConsideration, hasEnergyConsumptionDetails, hasMeasurement, hasMerchantReturnPolicy, hasProductReturnPolicy, height, identifier, image, inProductGroupWithID, isAccessoryOrSparePartFor, isConsumableFor, isFamilyFriendly, isRelatedTo, isSimilarTo, isVariantOf, itemCondition, keywords, knownVehicleDamages, logo, mainEntityOfPage, manufacturer, material, meetsEmissionStandard, mileageFromOdometer, mobileUrl, model, modelDate, mpn, name, negativeNotes, nsn, numberOfAirbags, numberOfAxles, numberOfDoors, numberOfForwardGears, numberOfPreviousOwners, offers, pattern, payload, positiveNotes, potentialAction, productID, productionDate, purchaseDate, releaseDate, review, reviews, sameAs, seatingCapacity, size, sku, slogan, speed, steeringPosition, stupidProperty, subjectOf, tongueWeight, trailerWeight, url, vehicleConfiguration, vehicleEngine, vehicleIdentificationNumber, vehicleInteriorColor, vehicleInteriorType, vehicleModelDate, vehicleSeatingCapacity, vehicleSpecialUsage, vehicleTransmission, weight, weightTotal, wheelbase, width ]  

#is_/same_as :: [[../../WikiData/WD~Vehicle,42889|WD~Vehicle,42889]] 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Transport/Vehicle|Vehicle]] 

### #is_/same_as :: [[/_public/Technology/Transport/Vehicle.public|Vehicle.public]] 

### #is_/same_as :: [[/_internal/Technology/Transport/Vehicle.internal|Vehicle.internal]] 

### #is_/same_as :: [[/_protect/Technology/Transport/Vehicle.protect|Vehicle.protect]] 

### #is_/same_as :: [[/_private/Technology/Transport/Vehicle.private|Vehicle.private]] 

### #is_/same_as :: [[/_personal/Technology/Transport/Vehicle.personal|Vehicle.personal]] 

### #is_/same_as :: [[/_secret/Technology/Transport/Vehicle.secret|Vehicle.secret]] 

