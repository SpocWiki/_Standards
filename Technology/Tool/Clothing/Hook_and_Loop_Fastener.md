---
aliases:
- Akroch-bendo
- belkro
- borrelås
- Caewr bachyn-a-dolen
- cierre de gancho y bucle
- Cırtcırt
- fermeture autoagrippante
- Franskur rennilás
- Hook and Loop Fastener
- Hook-and-loop fastener
- Hook-and-loop fasteners
- hook-and-pile fasteners
- Hook_and_Loop_Fastener
- ježek
- kardborrband
- Khóa Velcro
- Klettverschluss
- klittenband
- lapfermilo
- Lipukai
- līpslēdzējs
- Perepet
- suchý zip
- Takjapael
- Tarranauha
- touch fasteners
- tépőzár
- Velcro
- veta adherent
- Zarru de gabitu y bucle
- Застібка-липучка
- зашпілька-ліпучка
- Текстилна закопчалка
- Текстильная застёжка
- Чичак трака
- Տեքստիլ ճարմանդ
- צמדן
- لاصق فيلكرو
- ولکرو
- வெல்க்ரோ
- ಕೊಕ್ಕೆ ಮತ್ತು ಕುಣಿಕೆ ಜೋಡಣೆ
- ᏗᎦᏯᎸᎩ
- 尼龙搭扣
- 搭扣
- 面ファスナー
- 魔術貼
- 魔鬼氈
- 찍찍이
has_id_wikidata: Q15411420
discoverer_or_inventor: '[[_Standards/WikiData/WD~George_de_Mestral,123525]]'
different_from:
- '[[_Standards/WikiData/WD~Velcro,207649]]'
- '[[_Standards/WikiData/WD~Velcro,7918852]]'
subclass_of: '[[_Standards/WikiData/WD~fastener,2002016]]'
inspired_by: '[[_Standards/WikiData/WD~Arctium_lappa,26932]]'
Nomenclature_for_Museum_Cataloging: 6839
montage_image: http://commons.wikimedia.org/wiki/Special:FilePath/Hook%20and%20loop%20fastener%20collage.png
image: http://commons.wikimedia.org/wiki/Special:FilePath/Velcro6.jpg
Commons_category: Hook-and-loop fasteners
time_of_discovery_or_invention: 1941-01-01T00:00:00Z
dv_has_:
  name_:
    ar: لاصق فيلكرو
    ast: Zarru de gabitu y bucle
    be-tarask: зашпілька-ліпучка
    bg: Текстилна закопчалка
    ca: veta adherent
    chr: ᏗᎦᏯᎸᎩ
    cs: suchý zip
    cy: Caewr bachyn-a-dolen
    da: Velcro
    de: Klettverschluss
    en: hook and loop fastener
    eo: lapfermilo
    es: cierre de gancho y bucle
    et: Takjapael
    eu: belkro
    fa: ولکرو
    fi: Tarranauha
    fr: fermeture autoagrippante
    gl: Velcro
    he: צמדן
    hu: tépőzár
    hy: Տեքստիլ ճարմանդ
    id: Perepet
    io: Akroch-bendo
    is: Franskur rennilás
    it: velcro
    ja: 面ファスナー
    kn: ಕೊಕ್ಕೆ ಮತ್ತು ಕುಣಿಕೆ ಜೋಡಣೆ
    ko: 찍찍이
    lt: Lipukai
    lv: līpslēdzējs
    nb: borrelås
    nl: klittenband
    nn: borrelås
    pl: Velcro
    ru: Текстильная застёжка
    sl: ježek
    sr: Чичак трака
    sv: kardborrband
    ta: வெல்க்ரோ
    tr: Cırtcırt
    uk: Застібка-липучка
    vi: Khóa Velcro
    wuu: 搭扣
    yue: 魔術貼
    zh: 尼龙搭扣
    zh-hant: 魔鬼氈
    zh-hk: 魔術貼
---
# [[Hook_and_Loop_Fastener]] 

#is_/same_as :: [[WD~Hook_and_loop_fastener,15411420]] 

## #has_/text_of_/abstract 

> Hook-and-loop fasteners, commonly known as Velcro (a genericized trademark), 
> hook-and-pile fasteners or touch fasteners are versatile fastening devices 
> that allow two surfaces to be repeatedly attached and detached with ease. 
> 
> Invented in the mid-20th century, they are widely used in 
> clothing, accessories, and various industrial and consumer applications. 
> 
> The fastener consists of two complementary components: 
> one with tiny hooks and the other with soft loops. 
> 
> When pressed together, the hooks catch the loops, creating a secure but temporary bond. 
> The fasteners can be separated by peeling or pulling the surfaces apart, 
> often producing a distinctive ripping sound.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Hook-and-loop%20fastener) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Tool/Clothing/Hook_and_Loop_Fastener|Hook_and_Loop_Fastener]] 

### #is_/same_as :: [[/_public/Technology/Tool/Clothing/Hook_and_Loop_Fastener.public|Hook_and_Loop_Fastener.public]] 

### #is_/same_as :: [[/_internal/Technology/Tool/Clothing/Hook_and_Loop_Fastener.internal|Hook_and_Loop_Fastener.internal]] 

### #is_/same_as :: [[/_protect/Technology/Tool/Clothing/Hook_and_Loop_Fastener.protect|Hook_and_Loop_Fastener.protect]] 

### #is_/same_as :: [[/_private/Technology/Tool/Clothing/Hook_and_Loop_Fastener.private|Hook_and_Loop_Fastener.private]] 

### #is_/same_as :: [[/_personal/Technology/Tool/Clothing/Hook_and_Loop_Fastener.personal|Hook_and_Loop_Fastener.personal]] 

### #is_/same_as :: [[/_secret/Technology/Tool/Clothing/Hook_and_Loop_Fastener.secret|Hook_and_Loop_Fastener.secret]] 

