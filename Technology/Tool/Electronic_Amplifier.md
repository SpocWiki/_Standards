---
aliases:
- aimplitheoir leictreonach
- amp
- Amplifaya
- Amplificador
- amplificador electrònic
- amplificador electrónico
- amplificador electrónicu
- amplificateur électronique
- Amplificator electronic
- amplificator electronicus
- Amplificator eletrònich
- amplificatore elettronico
- amplifier
- Amplifikatör
- anplifikadore elektroniko
- electronic amplifier
- Electronic_Amplifier
- Elektr signallarni kuchaytirish
- elektra amplifilo
- elektroniese versterker
- elektronische versterker
- Elektronisk forsterkar
- elektronisk forsterker
- elektronisk forstærker
- Elektroniskais pastiprinātājs
- elektronski ojačevalnik
- elektrooniline võimendi
- erősítő áramkör
- förstärkare
- Gücləndirici
- Khuếch đại điện tử
- mwyhadur electronig
- penguat elektronik
- Pojačavač snage
- Pojačivač
- Përforcuesi elektronik
- Verstärker
- wzmacniacz sygnałów elektrycznych
- zesilovač
- Zosilňovač
- ενισχυτής
- електронний підсилювач
- електронски засилувач
- Лампалуу күчөткүч
- узмацняльнік
- Усилвател
- электронны узмацняльнік
- электронный усилитель
- מגבר חשמלי
- تقویتکننده الکترونیکی
- مضخم إلكتروني
- इलेक्ट्रोनिक एम्प्लिफायर
- प्रवर्धक
- বিবর্ধক
- ਐਂਪਲੀਫਾਇਰ
- மின்பெருக்கி
- ตัวขยายสัญญาณ
- 増幅回路
- 放大器
- 放大器电路
- 放大器電路
- 증폭 회로
has_id_wikidata: Q211554
different_from:
- '[[_Standards/WikiData/WD~operational_amplifier,178947]]'
- '[[_Standards/WikiData/WD~amplifier,347110]]'
- '[[_Standards/WikiData/WD~amplifier,115281453]]'
- '[[_Standards/WikiData/WD~amplifier,27094]]'
instance_of: '[[_Standards/WikiData/WD~electronic_circuit,1815901]]'
subclass_of:
- '[[_Standards/WikiData/WD~electronic_device,2858615]]'
- '[[_Standards/WikiData/WD~amplifier,115281453]]'
described_by_source: '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/One%20T%20Amp.PNG
UMLS_CUI: C0002684
MeSH_tree_code: E07.305.061
Commons_category: Electronic amplifiers
dv_has_:
  name_:
    af: elektroniese versterker
    ar: مضخم إلكتروني
    ast: amplificador electrónicu
    az: Gücləndirici
    be: узмацняльнік
    be-tarask: электронны узмацняльнік
    bg: Усилвател
    bn: বিবর্ধক
    ca: amplificador electrònic
    cs: zesilovač
    cy: mwyhadur electronig
    da: elektronisk forstærker
    de: Verstärker
    el: ενισχυτής
    en: electronic amplifier
    eo: elektra amplifilo
    es: amplificador electrónico
    et: elektrooniline võimendi
    eu: anplifikadore elektroniko
    fa: تقویتکننده الکترونیکی
    fr: amplificateur électronique
    ga: aimplitheoir leictreonach
    gl: amplificador electrónico
    gsw: Verstärker
    he: מגבר חשמלי
    hi: प्रवर्धक
    hu: erősítő áramkör
    id: penguat elektronik
    it: amplificatore elettronico
    ja: 増幅回路
    ko: 증폭 회로
    ky: Лампалуу күчөткүч
    la: amplificator electronicus
    lv: Elektroniskais pastiprinātājs
    mk: електронски засилувач
    nb: elektronisk forsterker
    ne: इलेक्ट्रोनिक एम्प्लिफायर
    nl: elektronische versterker
    nn: Elektronisk forsterkar
    pa: ਐਂਪਲੀਫਾਇਰ
    pl: wzmacniacz sygnałów elektrycznych
    pms: Amplificator eletrònich
    pt: Amplificador
    ro: Amplificator electronic
    ru: электронный усилитель
    sh: Pojačivač
    sk: Zosilňovač
    sl: elektronski ojačevalnik
    sq: Përforcuesi elektronik
    sr: Pojačavač snage
    sv: förstärkare
    sw: Amplifaya
    ta: மின்பெருக்கி
    th: ตัวขยายสัญญาณ
    tr: Amplifikatör
    uk: електронний підсилювач
    uz: Elektr signallarni kuchaytirish
    vi: Khuếch đại điện tử
    yue: 放大器
    zh: 放大器
    zh-cn: 放大器
    zh-hans: 放大器电路
    zh-hant: 放大器電路
    zh-hk: 放大器電路
    zh-sg: 放大器电路
    zh-tw: 放大器
---
# [[Electronic_Amplifier]] 

#is_/same_as :: [[../../WikiData/WD~Electronic_amplifier,211554|WD~Electronic_amplifier,211554]] 

## #has_/text_of_/abstract 

> An amplifier, electronic amplifier or (informally) amp 
> is an electronic device that can increase the magnitude of a signal (a time-varying voltage or current).  
> It is a two-port electronic circuit that uses electric power from a power supply 
> to increase the amplitude (magnitude of the voltage or current) of a signal applied to its input terminals, 
> producing a proportionally greater amplitude signal at its output. 
> 
> The amount of amplification provided by an amplifier is measured by its gain: 
> the ratio of output voltage, current, or power to input. 
> An amplifier is defined as a circuit that has a power gain greater than one.
>
> An amplifier can be either a separate piece of equipment or an electrical circuit contained within another device. Amplification is fundamental to modern electronics, and amplifiers are widely used in almost all electronic equipment. Amplifiers can be categorized in different ways. One is by the frequency of the electronic signal being amplified. For example, audio amplifiers amplify signals of less than 20 kHz, radio frequency (RF) amplifiers amplify frequencies in the range between 20 kHz and 300 GHz, and servo amplifiers and instrumentation amplifiers may work with very low frequencies down to direct current. Amplifiers can also be categorized by their physical placement in the signal chain; a preamplifier may precede other signal processing stages, for example, while a power amplifier is usually used after other amplifier stages to provide enough output power for the final use of the signal. The first practical electrical device which could amplify was the triode vacuum tube, invented in 1906 by Lee De Forest, which led to the first amplifiers around 1912. Today most amplifiers use transistors.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Amplifier) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Tool/Electronic_Amplifier|Electronic_Amplifier]] 

### #is_/same_as :: [[/_public/Technology/Tool/Electronic_Amplifier.public|Electronic_Amplifier.public]] 

### #is_/same_as :: [[/_internal/Technology/Tool/Electronic_Amplifier.internal|Electronic_Amplifier.internal]] 

### #is_/same_as :: [[/_protect/Technology/Tool/Electronic_Amplifier.protect|Electronic_Amplifier.protect]] 

### #is_/same_as :: [[/_private/Technology/Tool/Electronic_Amplifier.private|Electronic_Amplifier.private]] 

### #is_/same_as :: [[/_personal/Technology/Tool/Electronic_Amplifier.personal|Electronic_Amplifier.personal]] 

### #is_/same_as :: [[/_secret/Technology/Tool/Electronic_Amplifier.secret|Electronic_Amplifier.secret]] 

