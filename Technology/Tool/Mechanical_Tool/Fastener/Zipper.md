---
aliases:
- Alat pengancing
- Aysay wichq'ana
- blixtlås
- cerniera lampo
- cipzár
- Clusura tractilis
- Cremalleira
- cremallera
- fecho-éclair
- fermeture éclair
- Fermoar
- fermuar
- giessemlássa
- glidelås
- Jiâk-kùh
- kremailera
- kråankelaesie
- luottolohkka
- lynlås
- Là-lièn
- Patentni zatvarač
- Phéc-mơ-tuya
- Qıfıllıca
- Rajsferšlus
- Reissverschluss
- Reißverschluss
- Rennilás
- Resileti
- Resleting
- Rietsluter
- ritsleting
- ritssluiter
- ritssluiting
- ruttimsalvân
- rāvējslēdzējs
- Serr-prim
- Sip
- Sipaq
- Siper
- Slèteng
- Sıldırma ilgek
- Tirett
- tõmblukk
- užtrauktukas
- vetoketju
- zadrga
- zamek błyskawiczny
- Zanjir tugma
- zdrhovadlo
- zip fastener
- Zipe
- Zipilo
- Zipo
- Zipper
- zips
- Zipu
- zíper
- Çиçĕм (çаклаткăч)
- ǩieʹssempäʹdd
- φερμουάρ
- замок-маланка
- застёжка-молния
- застібка-блискавка
- зашпілька-маланка
- Патент
- Рајсфершлус
- Сылдырма ілгек
- Цахилгаан
- Цип
- Կայծակաճարմանդ
- רוכסן
- السلسلة
- زمام منزلق
- زپ
- زیپ
- سسته
- سىلدىرما ىلگەك
- فەرمووار
- ज़िपर
- জিপার
- பல்லிணைவுப் பட்டிகை
- ซิป
- ဇစ်
- ႁုတ်ႉ
- ዚፕ
- ᏗᎧᏁᏍᏙᏗ
- 拉鏈
- 拉链
- 線ファスナー
- 지퍼
has_id_wikidata: Q101761
subclass_of:
- '[[_Standards/WikiData/WD~joining_technology,266328]]'
- '[[_Standards/WikiData/WD~mercery,350039]]'
- '[[_Standards/WikiData/WD~fastener,2002016]]'
discoverer_or_inventor: '[[_Standards/WikiData/WD~Whitcomb_L._Judson,731876]]'
instance_of:
- '[[_Standards/WikiData/WD~Q12106034,12106034]]'
- '[[_Standards/WikiData/WD~Q107555073,107555073]]'
time_of_discovery_or_invention: 1893-01-01T00:00:00Z
Nomenclature_for_Museum_Cataloging: 6847
Commons_gallery: Zipper
Commons_category: Zippers
image: http://commons.wikimedia.org/wiki/Special:FilePath/Coil%20plastic%20and%20metal%20zippers.jpg
video: http://commons.wikimedia.org/wiki/Special:FilePath/Zipper%20animated.gif
OmegaWiki_Defined_Meaning: 1638101
Common_Procurement_Vocabulary_code: 18453000
Iconclass_notation: 41D26441
Dewey_Decimal_Classification: 687.8
dv_has_:
  name_:
    aeb: السلسلة
    af: ritssluiter
    am: ዚፕ
    ar: زمام منزلق
    ary: السلسلة
    arz: سسته
    az: Qıfıllıca
    be: замок-маланка
    be-tarask: зашпілька-маланка
    bew: Slèteng
    bg: Цип
    bn: জিপার
    br: Serr-prim
    ca: cremallera
    chr: ᏗᎧᏁᏍᏙᏗ
    cs: zdrhovadlo
    cv: Çиçĕм (çаклаткăч)
    cy: Sip
    da: lynlås
    de: Reißverschluss
    de-ch: Reissverschluss
    el: φερμουάρ
    en: zipper
    eo: Zipo
    es: cremallera
    et: tõmblukk
    eu: kremailera
    fa: زیپ
    fi: vetoketju
    fr: fermeture éclair
    gl: Cremalleira
    hak: Là-lièn
    he: רוכסן
    hi: ज़िपर
    hr: Patentni zatvarač
    hu: cipzár
    hy: Կայծակաճարմանդ
    id: ritsleting
    ik: Sipaq
    io: Zipilo
    is: Rennilás
    it: cerniera lampo
    ja: 線ファスナー
    kk: Сылдырма ілгек
    kk-arab: سىلدىرما ىلگەك
    kk-cn: سىلدىرما ىلگەك
    kk-cyrl: Сылдырма ілгек
    kk-kz: Сылдырма ілгек
    kk-latn: Sıldırma ilgek
    kk-tr: Sıldırma ilgek
    ko: 지퍼
    ku: fermuar
    ku-arab: فەرمووار
    ku-latn: fermuar
    la: Clusura tractilis
    lb: Tirett
    lfn: Zipe
    lt: užtrauktukas
    lv: rāvējslēdzējs
    min: Resleting
    mk: Патент
    mn: Цахилгаан
    ms: Alat pengancing
    my: ဇစ်
    nan: Jiâk-kùh
    nb: glidelås
    nds: Rietsluter
    nia: Resileti
    nl: ritssluiting
    nn: Glidelås
    pl: zamek błyskawiczny
    pt: fecho-éclair
    pt-br: zíper
    qu: Aysay wichq'ana
    ro: Fermoar
    ru: застёжка-молния
    sd: زپ
    se: luottolohkka
    sh: Patentni zatvarač
    shn: ႁုတ်ႉ
    sk: zips
    sl: zadrga
    sma: kråankelaesie
    smj: giessemlássa
    smn: ruttimsalvân
    sms: ǩieʹssempäʹdd
    sr: Рајсфершлус
    sr-ec: Рајсфершлус
    sr-el: Rajsferšlus
    sv: blixtlås
    sw: Zipu
    ta: பல்லிணைவுப் பட்டிகை
    th: ซิป
    tl: Siper
    tr: Fermuar
    uk: застібка-блискавка
    uz: Zanjir tugma
    vi: Phéc-mơ-tuya
    war: Siper
    wuu: 拉链
    yue: 拉鏈
    zh: 拉鏈
    zh-cn: 拉链
    zh-hans: 拉链
    zh-hant: 拉鏈
    zh-hk: 拉鏈
    zh-sg: 拉链
    zh-tw: 拉鏈
---
# [[Zipper]] 

#is_/same_as :: [[../../../../WikiData/WD~Zipper,101761|WD~Zipper,101761]] 

## #has_/text_of_/abstract 

> A **Zipper** (N. America), zip, zip fastener (UK), formerly known as a clasp locker, 
> is a commonly used device for binding together two edges of fabric or other flexible material. 
> 
> Used in clothing (e.g. jackets and jeans), luggage and other bags, camping gear 
> (e.g. tents and sleeping bags), and many other items, 
> zippers come in a wide range of sizes, shapes, and colors. 
> 
> In 1892, Whitcomb L. Judson, an American inventor from Chicago, 
> patented the original design from which the modern device evolved.
>
> The zipper gets its name from a brand of rubber boots (or galoshes) it was used on in 1923. 
> The galoshes could be fastened with a single zip of the hand, 
> and soon the hookless fasteners came to be called "Zippers".
>
> [Wikipedia](https://en.wikipedia.org/wiki/Zipper) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Tool/Mechanical_Tool/Fastener/Zipper|Zipper]] 

### #is_/same_as :: [[/_public/Technology/Tool/Mechanical_Tool/Fastener/Zipper.public|Zipper.public]] 

### #is_/same_as :: [[/_internal/Technology/Tool/Mechanical_Tool/Fastener/Zipper.internal|Zipper.internal]] 

### #is_/same_as :: [[/_protect/Technology/Tool/Mechanical_Tool/Fastener/Zipper.protect|Zipper.protect]] 

### #is_/same_as :: [[/_private/Technology/Tool/Mechanical_Tool/Fastener/Zipper.private|Zipper.private]] 

### #is_/same_as :: [[/_personal/Technology/Tool/Mechanical_Tool/Fastener/Zipper.personal|Zipper.personal]] 

### #is_/same_as :: [[/_secret/Technology/Tool/Mechanical_Tool/Fastener/Zipper.secret|Zipper.secret]] 

