---
aliases:
- cetvel
- chhak
- chhioh
- chizg'ich
- erregela graduatu
- jidar
- joonlaud
- lenjir
- Lienholt
- lineal
- linealo
- lineāls
- liniaal
- liniilo
- linijka
- liniuotė
- linjal
- liòng-chióh
- pembaris
- penggaris
- Penyuran
- pravítko
- pren mesur
- ravnalo
- ravnilo
- regla
- regla graduada
- regle
- regra
- regula
- rialóir
- riga
- riglă
- rula
- ruler
- règ
- règle
- régua
- rùilear
- thước
- tipahikanatikw
- tutuku
- viivain
- vizorja
- vonalzó
- xətkeş
- χάρακας
- лінійка
- лінарка
- лінейка
- лењир
- линейка
- линијар
- линийка
- масштабо линейка
- сызғыш
- тӳрĕшке
- քանոն
- סרגל
- خطکش
- مسطرة
- مسطره (اداه)
- ڕاستە
- کرښی
- அளவுகோல்
- రూలర్ (స్కేల్)
- റൂളർ
- ไม้บรรทัด
- ပေတံ
- မျဉ်းတံ
- မႆႉထတ်း
- სახაზავი
- 定規
- 尺
- 尺子
- 間尺
- 자
has_id_wikidata: Q104555
partially_coincident_with: '[[_Standards/WikiData/WD~straightedge,354733]]'
subclass_of:
- '[[_Standards/WikiData/WD~straightedge,354733]]'
- '[[_Standards/WikiData/WD~stationery,875696]]'
- '[[_Standards/WikiData/WD~measuring_instrument,2041172]]'
- '[[_Standards/WikiData/WD~metrological_mechanism,4198689]]'
- '[[_Standards/WikiData/WD~measuring_tools_&_sensor,116971226]]'
different_from:
- '[[_Standards/WikiData/WD~ruler,1097498]]'
- '[[_Standards/WikiData/WD~remmalag,11757562]]'
has_part_s_of_the_class: '[[_Standards/WikiData/WD~scale_marking,2632744]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137]]'
described_by_source: '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
instance_of: '[[_Standards/WikiData/WD~product_category,63981612]]'
measures: '[[_Standards/WikiData/WD~length,36253]]'
Nomenclature_for_Museum_Cataloging: 10351
Commons_gallery: Ruler
image: http://commons.wikimedia.org/wiki/Special:FilePath/Measurement%20unit.jpg
exact_match:
- http://wordnet-rdf.princeton.edu/wn30/04118776-n
- http://wordnet-rdf.princeton.edu/wn31/104062517-n
Iconclass_notation: 49D511(RULER)
EAGLE_id: decor/lod/265
Wolfram_Language_entity_code: Entity["MeasurementDevice", "Ruler"]
Commons_category: Rulers
dv_has_:
  name_:
    ar: مسطرة
    arz: مسطره (اداه)
    ast: regla graduada
    atj: tipahikanatikw
    az: xətkeş
    be: лінейка
    be-tarask: лінарка
    bg: линийка
    ca: regle
    cdo: liòng-chióh
    ckb: ڕاستە
    cs: pravítko
    cv: тӳрĕшке
    cy: pren mesur
    da: lineal
    de: Lineal
    dtp: tutuku
    el: χάρακας
    eml: riga
    en: ruler
    en-ca: Ruler
    en-gb: Ruler
    eo: liniilo
    es: regla
    et: joonlaud
    eu: erregela graduatu
    fa: خطکش
    fi: viivain
    fr: règle
    ga: rialóir
    gd: rùilear
    gl: regra
    hak: chhak
    he: סרגל
    hr: ravnalo
    ht: règ
    hu: vonalzó
    hy: քանոն
    ia: regula
    iba: Penyuran
    id: penggaris
    io: linealo
    it: riga
    ja: 定規
    jv: penggaris
    ka: სახაზავი
    kk: сызғыш
    ko: 자
    la: regula
    lt: liniuotė
    lv: lineāls
    lzh: 尺
    mk: линијар
    ml: റൂളർ
    ms: pembaris
    my: ပေတံ
    nan: chhioh
    nb: linjal
    nds: Lienholt
    nl: liniaal
    nn: linjal
    pl: linijka
    ps: کرښی
    pt: régua
    pt-br: régua
    rki: မျဉ်းတံ
    ro: riglă
    ru: линейка
    sh: ravnalo
    shn: မႆႉထတ်း
    sk: pravítko
    sl: ravnilo
    sq: vizorja
    sr: лењир
    sr-ec: лењир
    sr-el: lenjir
    su: jidar
    sv: linjal
    sw: rula
    ta: அளவுகோல்
    te: రూలర్ (స్కేల్)
    th: ไม้บรรทัด
    tr: cetvel
    udm: масштабо линейка
    uk: лінійка
    uz: chizg'ich
    vi: thước
    wuu: 尺子
    yue: 間尺
    zh: 尺子
    zh-cn: 尺子
    zh-hans: 尺子
    zh-hant: 尺子
    zh-hk: 尺子
    zh-sg: 尺子
    zh-tw: 尺子
Unicode_character: 📏
---

# [[Ruler]] 📏 



#is_/same_as :: [[WD~Ruler,104555]] 

## #has_/text_of_/abstract 

> A **Ruler**, sometimes called a rule, scale, line gauge, or metre/meter stick, 
> is an instrument used to make length measurements, 
> whereby a length is read from a series of markings called "rules" along an edge of the device. 
> Usually, the instrument is rigid and the edge itself is a straightedge ("ruled straightedge"), 
> which additionally allows one to draw straighter lines. 
> 
> Rulers are an important tool  in geometry, geography and mathematics. 
> They have been used since at least 2650 BC.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Ruler) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Tool/Instrumentation/Measuring_Instrument/Ruler|Ruler]] 

### #is_/same_as :: [[/_public/Technology/Tool/Instrumentation/Measuring_Instrument/Ruler.public|Ruler.public]] 

### #is_/same_as :: [[/_internal/Technology/Tool/Instrumentation/Measuring_Instrument/Ruler.internal|Ruler.internal]] 

### #is_/same_as :: [[/_protect/Technology/Tool/Instrumentation/Measuring_Instrument/Ruler.protect|Ruler.protect]] 

### #is_/same_as :: [[/_private/Technology/Tool/Instrumentation/Measuring_Instrument/Ruler.private|Ruler.private]] 

### #is_/same_as :: [[/_personal/Technology/Tool/Instrumentation/Measuring_Instrument/Ruler.personal|Ruler.personal]] 

### #is_/same_as :: [[/_secret/Technology/Tool/Instrumentation/Measuring_Instrument/Ruler.secret|Ruler.secret]] 

