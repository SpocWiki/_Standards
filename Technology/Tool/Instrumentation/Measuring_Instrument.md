---
aliases:
- alat pengukur
- alat pengukuran
- alat za mjerenje
- aparell de mesura
- gléas tomhais
- instrument de mesure
- instrument de măsură
- instrumento de medición
- instrumento de medida
- instrumento de medição
- instrumento de mezura
- instrumentu de midida
- Measuring Instrument
- Measuring_Instrument
- meetinstrument
- merací prístroj
- merilni instrument
- Messgerät
- Messinstrument
- mezurilo
- mittalaite
- mjerni instrument
- mjet matës
- mjitynstrumint
- mätinstrument
- måleinstrument
- mérőeszköz
- mõõteriist
- mērinstruments
- měrjenski nastroj
- měřidlo
- neurgailu
- offeryn mesur
- oʻlchov asboblari
- panukat
- przyrząd pomiarowy
- strumento di misura
- thiết bị đo
- ölçü aracı
- ölçü cihazı
- όργανο μέτρησης
- алат за мјерење
- афзори назоратию андозкунанда
- вимірювальний прилад
- вымяральны прыбор
- измерителен уред
- измерительный прибор
- мерен уред
- мертан лэсьтэт
- үлчәү коралы
- өлчөгүч курал
- өлшем құралдарының
- չափիչ սարք
- מכשיר מדידה
- ابزار اندازهگیری
- مقياس
- मापन उपकरण
- ਮਾਪਣ ਵਾਲੇ ਉਪਕਰਨ
- அளவிடும் கருவிகள்
- కొలత పరికరం
- ಅಳೆಯುವ ಸಾಧನ
- അളവുപകരണം
- මිනුම් උපකරණ
- เครื่องมือวัด
- မီတာ
- မီႇတႃႇ
- 测量仪器
- 測量儀器
- 計測機器
- 量度工具
- 계측기
has_id_wikidata: Q2041172
has_list: '[[_Standards/WikiData/WD~list_of_measuring_devices,158656|WD~list_of_measuring_devices,158656]]'
has_part_s_:
- '[[_Standards/WikiData/WD~sensor,167676|WD~sensor,167676]]'
- '[[_Standards/WikiData/WD~display_device,327065|WD~display_device,327065]]'
- '[[_Standards/WikiData/WD~measurement_scale,1370617|WD~measurement_scale,1370617]]'
- '[[_Standards/WikiData/WD~metrological_mechanism,4198689|WD~metrological_mechanism,4198689]]'
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~International_Vocabulary_of_Metrology_(3rd_edition,_2012),70257574|WD~International_Vocabulary_of_Metrology_(3rd_edition,_2012),70257574]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._8,124737635|WD~Armenian_Soviet_Encyclopedia,_vol._8,124737635]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~measurement_tool,1047213|WD~measurement_tool,1047213]]'
subclass_of: '[[_Standards/WikiData/WD~measurement_tool,1047213|WD~measurement_tool,1047213]]'
has_characteristic:
- '[[_Standards/WikiData/WD~sensitivity,1295020|WD~sensitivity,1295020]]'
- '[[_Standards/WikiData/WD~accuracy,1298969|WD~accuracy,1298969]]'
- '[[_Standards/WikiData/WD~resolution,3937033|WD~resolution,3937033]]'
- '[[_Standards/WikiData/WD~accuracy,54988221|WD~accuracy,54988221]]'
part_of:
- '[[_Standards/WikiData/WD~measuring_equipment,1584378|WD~measuring_equipment,1584378]]'
- '[[_Standards/WikiData/WD~measurement_technique,1924397|WD~measurement_technique,1924397]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Measuring_instruments,26004201|WD~Template_Measuring_instruments,26004201]]'
has_use: '[[_Standards/WikiData/WD~measurement,12453|WD~measurement,12453]]'
facet_of: '[[_Standards/WikiData/WD~measurement,12453|WD~measurement,12453]]'
Product_and_Service_Code: 52
OmegaWiki_Defined_Meaning: 4233
Dewey_Decimal_Classification: 681
U_S_National_Archives_Identifier: 10639532
Commons_category: Measuring instruments
image: http://commons.wikimedia.org/wiki/Special:FilePath/Digital%20Multimeter%20Aka.jpg
IEV_number: 311-03-01
Iconclass_notation: 49D521
Wolfram_Language_entity_type: MeasurementDevice
Commons_gallery: Measuring instrument
Krugosvet_article_archived_: nauka_i_tehnika/tehnologiya_i_promyshlennost/IZMERITELNIE_INSTRUMENTI.html
dv_has_:
  name_:
    ar: مقياس
    ast: instrumentu de midida
    az: ölçü cihazı
    be: вымяральны прыбор
    be-tarask: вымяральны прыбор
    bg: измерителен уред
    ca: aparell de mesura
    cs: měřidlo
    cy: offeryn mesur
    da: måleinstrument
    de: Messgerät
    de-ch: Messgerät
    el: όργανο μέτρησης
    en: measuring instrument
    en-ca: measuring instrument
    en-gb: measuring instrument
    eo: mezurilo
    es: instrumento de medición
    et: mõõteriist
    eu: neurgailu
    fa: ابزار اندازهگیری
    fi: mittalaite
    fr: instrument de mesure
    fy: mjitynstrumint
    ga: gléas tomhais
    gl: instrumento de medida
    he: מכשיר מדידה
    hi: मापन उपकरण
    hr: mjerni instrument
    hsb: měrjenski nastroj
    hu: mérőeszköz
    hy: չափիչ սարք
    id: alat pengukur
    it: strumento di misura
    ja: 計測機器
    kk: өлшем құралдарының
    kn: ಅಳೆಯುವ ಸಾಧನ
    ko: 계측기
    ky: өлчөгүч курал
    lv: mērinstruments
    mk: мерен уред
    ml: അളവുപകരണം
    ms: alat pengukuran
    my: မီတာ
    nb: måleinstrument
    nl: meetinstrument
    nn: måleinstrument
    pa: ਮਾਪਣ ਵਾਲੇ ਉਪਕਰਨ
    pl: przyrząd pomiarowy
    pt: instrumento de medição
    pt-br: instrumento de medida
    ro: instrument de măsură
    ru: измерительный прибор
    sh: mjerni instrument
    shn: မီႇတႃႇ
    si: මිනුම් උපකරණ
    sk: merací prístroj
    sl: merilni instrument
    sq: mjet matës
    sr: алат за мјерење
    sr-ec: алат за мјерење
    sr-el: alat za mjerenje
    sv: mätinstrument
    ta: அளவிடும் கருவிகள்
    te: కొలత పరికరం
    tg: афзори назоратию андозкунанда
    th: เครื่องมือวัด
    tl: panukat
    tr: ölçü aracı
    tt: үлчәү коралы
    tt-cyrl: үлчәү коралы
    udm: мертан лэсьтэт
    uk: вимірювальний прилад
    uz: oʻlchov asboblari
    vec: instrumento de mezura
    vi: thiết bị đo
    yue: 量度工具
    zh: 测量仪器
    zh-hant: 測量儀器
    zh-tw: 測量儀器
---

# [[Measuring_Instrument]] 

#is_/same_as :: [[../../../WikiData/WD~Measuring_instrument,2041172|WD~Measuring_instrument,2041172]] 

## #has_/text_of_/abstract 

> Instrumentation is a collective term for measuring instruments, 
> used for indicating, measuring, and recording physical quantities. 
> 
> It is also a field of study about the art and science about making measurement instruments, 
> involving the related areas of metrology, automation, and control theory. 
> The term has its origins in the art and science of scientific instrument-making.
>
> Instrumentation can refer to devices as simple as direct-reading thermometers, 
> or as complex as multi-sensor components of industrial control systems. 
> 
> Instruments can be found in laboratories, refineries, factories and vehicles, 
> as well as in everyday household use (e.g., smoke detectors and thermostats).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Instrumentation) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Tool/Instrumentation/Measuring_Instrument|Measuring_Instrument]] 

### #is_/same_as :: [[/_public/Technology/Tool/Instrumentation/Measuring_Instrument.public|Measuring_Instrument.public]] 

### #is_/same_as :: [[/_internal/Technology/Tool/Instrumentation/Measuring_Instrument.internal|Measuring_Instrument.internal]] 

### #is_/same_as :: [[/_protect/Technology/Tool/Instrumentation/Measuring_Instrument.protect|Measuring_Instrument.protect]] 

### #is_/same_as :: [[/_private/Technology/Tool/Instrumentation/Measuring_Instrument.private|Measuring_Instrument.private]] 

### #is_/same_as :: [[/_personal/Technology/Tool/Instrumentation/Measuring_Instrument.personal|Measuring_Instrument.personal]] 

### #is_/same_as :: [[/_secret/Technology/Tool/Instrumentation/Measuring_Instrument.secret|Measuring_Instrument.secret]] 

