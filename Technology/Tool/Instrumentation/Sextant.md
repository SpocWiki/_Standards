---
has_time_started: 1757-01-01
aliases:
- Kvadrant
- Kính lục phân
- lio̍k-hun-gî
- Liòng-tiĕng-chióh
- secstant
- seiseamhán
- Sekstan
- Sekstant
- sekstantas
- sekstanto
- sekstants
- sekstantti
- sestante
- sextant
- Sextante
- Sextanto
- Sudusi
- szextáns
- Εξάντας
- Квадрант
- секстант
- Секстант (инструмент)
- сэкстант
- Սեքստանտ
- סקסטנט
- آلۂ مسدس
- سدس
- سکستانت
- سێکستانت
- كۆادرانت
- सैक्स्टैंट
- সেক্সট্যান্ট
- அறுபாகைமானி
- ദൂരകോണമാപിനി
- เครื่องวัดแดด
- ဆက်စတန့်အင်္သာတိုင်း
- სექსტანტი
- 六分仪
- 六分儀
- 육분의
has_id_wikidata: Q179430
replaces: '[[_Standards/WikiData/WD~astrolabe,164992]]'
used_by: '[[_Standards/WikiData/WD~navigator,254651]]'
discoverer_or_inventor:
- '[[_Standards/WikiData/WD~Abu_Mahmud_Khojandi,335123]]'
- '[[_Standards/WikiData/WD~John_Bird,375165]]'
different_from:
- '[[_Standards/WikiData/WD~Sextant,409461]]'
- '[[_Standards/WikiData/WD~Sekstant,12151731]]'
- '[[_Standards/WikiData/WD~Sextante,12400435]]'
- '[[_Standards/WikiData/WD~Sestante,121335963]]'
subclass_of:
- '[[_Standards/WikiData/WD~astronomical_instrument,751997]]'
- '[[_Standards/WikiData/WD~navigational_instrument,1780422]]'
- '[[_Standards/WikiData/WD~reflecting_instrument,7307234]]'
described_by_source:
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~The_Nuttall_Encyclopædia,3181656]]'
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138]]'
- "[[_Standards/WikiData/WD~The_New_Student's_Reference_Work,16082057]]"
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._10,124737637]]'
start_time: 1757-01-01T00:00:00Z
measures: '[[_Standards/WikiData/WD~angle,11352]]'
Nomenclature_for_Museum_Cataloging: 10160
Dewey_Decimal_Classification: 522.4
U_S_National_Archives_Identifier: 10675931
Commons_gallery: Sextant
Iconclass_notation: 46C2822
image: http://commons.wikimedia.org/wiki/Special:FilePath/Sextant%20von%20Alexander%20von%20Humboldt.jpg
user_manual_URL: https://en.wikipedia.org/wiki/File:Using_sextant_swing.gif
Wolfram_Language_entity_code: Entity["MeasurementDevice", "Sextant"]
Krugosvet_article_archived_: nauka_i_tehnika/tehnologiya_i_promyshlennost/SEKSTANT.html
Commons_category: Sextants
dv_has_:
  name_:
    af: Sekstant
    ar: سدس
    ast: Sextante
    az: Sekstant
    be: секстант
    be-tarask: сэкстант
    bg: Секстант
    bn: সেক্সট্যান্ট
    br: Sekstant
    ca: Sextant
    cdo: Liòng-tiĕng-chióh
    ckb: سێکستانت
    cs: sextant
    cy: secstant
    da: Sekstant
    de: Sextant
    el: Εξάντας
    en: sextant
    eo: sekstanto
    es: sextante
    eu: Sestante
    fa: سکستانت
    fi: sekstantti
    fr: sextant
    ga: seiseamhán
    gl: Sextante
    he: סקסטנט
    hi: सैक्स्टैंट
    hr: Sekstant
    hu: szextáns
    hy: Սեքստանտ
    id: Sekstan
    ig: Sextant
    io: Sextanto
    is: Sextant
    it: sestante
    ja: 六分儀
    ka: სექსტანტი
    kk: Квадрант
    kk-arab: كۆادرانت
    kk-cn: كۆادرانت
    kk-cyrl: Квадрант
    kk-kz: Квадрант
    kk-latn: Kvadrant
    kk-tr: Kvadrant
    ko: 육분의
    lt: sekstantas
    lv: sekstants
    mk: Секстант (инструмент)
    ml: ദൂരകോണമാപിനി
    ms: Sekstan
    my: ဆက်စတန့်အင်္သာတိုင်း
    nan: lio̍k-hun-gî
    nb: sekstant
    nl: sextant
    nn: Sekstant
    oc: Sextant
    pl: Sekstant
    pt: sextante
    pt-br: sextante
    ro: Sextant
    ru: секстант
    sh: Sekstant
    sk: sextant
    sl: sekstant
    sr: секстант
    sr-ec: секстант
    sr-el: Sekstant
    sv: sextant
    sw: Sudusi
    ta: அறுபாகைமானி
    th: เครื่องวัดแดด
    tr: Sekstant
    uk: секстант
    ur: آلۂ مسدس
    uz: Sekstant
    vi: Kính lục phân
    wuu: 六分仪
    yue: 六分儀
    zh: 六分仪
    zh-cn: 六分仪
    zh-hans: 六分仪
    zh-hant: 六分儀
    zh-hk: 六分儀
    zh-sg: 六分仪
    zh-tw: 六分儀
---
# [[Sextant]] 

#is_/same_as :: [[../../../WikiData/WD~Sextant,179430|WD~Sextant,179430]]  

## #has_/text_of_/abstract 

> A **Sextant** is a doubly reflecting navigation instrument 
> that measures the angular distance between two visible objects. 
> 
> The primary use of a sextant is to measure the angle 
> between an astronomical object and the horizon for the purposes of celestial navigation.
>
> The estimation of this angle, the altitude, 
> is known as sighting or shooting the object, or taking a sight. 
> 
> The angle, and the time when it was measured, 
> can be used to calculate a position line on a nautical or aeronautical chart—
> for example, sighting the Sun at noon or Polaris at night (in the Northern Hemisphere) 
> to estimate latitude (with sight reduction). 
> 
> Sighting the height of a landmark can give a measure of distance off and, 
> held horizontally, a sextant can measure angles between objects for a position on a chart. 
> 
> A sextant can also be used to measure the lunar distance between 
> the moon and another celestial object (such as a star or planet) 
> in order to determine Greenwich Mean Time and hence longitude.
>
> The principle of the instrument was first implemented  around 1731 
> by John Hadley (1682–1744) and Thomas Godfrey (1704–1749), 
> but it was also found later in the unpublished writings of Isaac Newton (1643–1727).
>
> In 1922, it was modified for aeronautical navigation 
> by Portuguese navigator and naval officer Gago Coutinho.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Sextant) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Tool/Instrumentation/Sextant|Sextant]] 

### #is_/same_as :: [[/_public/Technology/Tool/Instrumentation/Sextant.public|Sextant.public]] 

### #is_/same_as :: [[/_internal/Technology/Tool/Instrumentation/Sextant.internal|Sextant.internal]] 

### #is_/same_as :: [[/_protect/Technology/Tool/Instrumentation/Sextant.protect|Sextant.protect]] 

### #is_/same_as :: [[/_private/Technology/Tool/Instrumentation/Sextant.private|Sextant.private]] 

### #is_/same_as :: [[/_personal/Technology/Tool/Instrumentation/Sextant.personal|Sextant.personal]] 

### #is_/same_as :: [[/_secret/Technology/Tool/Instrumentation/Sextant.secret|Sextant.secret]] 

