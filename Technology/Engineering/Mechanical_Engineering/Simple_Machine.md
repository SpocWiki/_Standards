---
aliases:
- basit makine
- eenvoudige machine
- egyszerű gép
- Einfache Maschine
- enkel maskin
- enkel maskine
- Grunnvél
- INguxa elula
- jednoduchý stroj
- Jednostavne mašine
- jednostavni stroj
- jynn sempel
- Maashinoota sasalphoo
- macchina semplice
- Machin konpoze
- Machin senp
- Machina simplex
- machine simple
- Makina sinple
- Makinang payak
- Maquina simpla
- maszyna prosta
- mașini simple
- meaisín simplí
- Mesin ringkas
- Màchina sempia
- màquina simple
- Máquina cenciella
- máquina simple
- máquina simples
- máy đơn giản
- pesawat sederhana
- preprosti stroj
- prosta mašina
- Sikllalla kuyuna
- Simpla mashino
- simpla maŝino
- simple machine
- Simple_Machine
- teclyn neu beiriant syml
- vienkāršs mehānisms
- yksinkertainen kone
- Ансат механизм
- Жай механизмдер
- Прост механизъм
- прості механізми
- проста машина
- простейший механизм
- просты мэханізм
- Судургу механизм
- Պարզ մեխանիզմ
- מכונה פשוטה
- آلة بسيطة
- ماشینهای ساده
- مەکینەی ساکار
- सरल यंत्र
- सरल यन्त्र
- সরল যন্ত্র
- সৰল যন্ত্ৰ
- எளிய இயந்திரம்
- సరళ యంత్రం
- ലഘുയന്ത്രം
- เครื่องกลอย่างง่าย
- 単純機械
- 简单机械
- 簡單機械
- 단순 기계
has_id_wikidata: Q186713
subclass_of:
- '[[_Standards/WikiData/WD~mechanism,517596|WD~mechanism,517596]]'
- '[[_Standards/WikiData/WD~appliance,1183543|WD~appliance,1183543]]'
described_by_source:
- '[[_Standards/WikiData/WD~The_Nuttall_Encyclopædia,3181656|WD~The_Nuttall_Encyclopædia,3181656]]'
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138|WD~Granat_Encyclopedic_Dictionary,4532138]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
uses: '[[_Standards/WikiData/WD~mechanical_advantage,5378501|WD~mechanical_advantage,5378501]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Simple_machines,13422858|WD~Template_Simple_machines,13422858]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q9610%20%28ben%29-Tahmid-%E0%A6%B8%E0%A6%B0%E0%A6%B2%20%E0%A6%AF%E0%A6%A8%E0%A7%8D%E0%A6%A4%E0%A7%8D%E0%A6%B0.wav
image: http://commons.wikimedia.org/wiki/Special:FilePath/Table%20of%20Mechanicks%2C%20Cyclopaedia%2C%20Volume%202.png
Commons_category: Simple machines
dv_has_:
  name_:
    an: Maquina simpla
    ar: آلة بسيطة
    as: সৰল যন্ত্ৰ
    ast: Máquina cenciella
    be_tarask: просты мэханізм
    bg: Прост механизъм
    bn: সরল যন্ত্র
    bs: Jednostavne mašine
    ca: màquina simple
    ckb: مەکینەی ساکار
    cs: jednoduchý stroj
    cv: Ансат механизм
    cy: teclyn neu beiriant syml
    da: enkel maskine
    de: Einfache Maschine
    en: simple machine
    eo: simpla maŝino
    es: máquina simple
    eu: Makina sinple
    fa: ماشینهای ساده
    fi: yksinkertainen kone
    fr: machine simple
    ga: meaisín simplí
    gcr: Machin senp
    gl: máquina simple
    gsw: Einfache Maschine
    he: מכונה פשוטה
    hi: सरल यंत्र
    hr: jednostavni stroj
    ht: Machin konpoze
    hu: egyszerű gép
    hy: Պարզ մեխանիզմ
    id: pesawat sederhana
    io: Simpla mashino
    is: Grunnvél
    it: macchina semplice
    ja: 単純機械
    kk: Жай механизмдер
    ko: 단순 기계
    kw: jynn sempel
    la: Machina simplex
    lv: vienkāršs mehānisms
    mk: проста машина
    ml: ലഘുയന്ത്രം
    ms: Mesin ringkas
    nb: enkel maskin
    ne: सरल यन्त्र
    new: सरल यन्त्र
    nl: eenvoudige machine
    nn: enkel maskin
    oc: Maquina simpla
    om: Maashinoota sasalphoo
    pl: maszyna prosta
    pms: Màchina sempia
    pt: máquina simples
    qu: Sikllalla kuyuna
    ro: mașini simple
    ru: простейший механизм
    sah: Судургу механизм
    sh: Jednostavni stroj
    sk: Jednoduchý stroj
    sl: preprosti stroj
    sr: проста машина
    sr_ec: проста машина
    sr_el: prosta mašina
    sv: enkel maskin
    ta: எளிய இயந்திரம்
    te: సరళ యంత్రం
    th: เครื่องกลอย่างง่าย
    tl: Makinang payak
    tr: basit makine
    uk: прості механізми
    vi: máy đơn giản
    wuu: 简单机械
    yue: 簡單機械
    zh: 简单机械
    zh_cn: 简单机械
    zh_hans: 简单机械
    zh_hant: 簡單機械
    zh_hk: 簡單機械
    zh_sg: 简单机械
    zh_tw: 簡單機械
    zu: INguxa elula
---

# [[Simple_Machine]] 

#is_/same_as :: [[../../../WikiData/WD~Simple_machine,186713|WD~Simple_machine,186713]] 

## #has_/text_of_/abstract 

> A **simple machine** is a mechanical device that changes the direction or magnitude of a force. 
> In general, they can be defined as the simplest mechanisms that use mechanical advantage 
> (also called leverage) to multiply force. 
> Usually the term refers to the six classical simple machines that were defined by Renaissance scientists:
> - Lever 
> - Wheel and axle 
> - Pulley 
> - Inclined plane
> - Wedge
> - Screw
>
> A simple machine uses a single applied force to do work against a single load force. 
> Ignoring friction losses, the work done on the load is equal to the work done by the applied force. 
> The machine can increase the amount of the output force, 
> at the cost of a proportional decrease in the distance moved by the load. 
> The ratio of the output to the applied force is called the mechanical advantage.
>
> Simple machines can be regarded as the elementary "building blocks" 
> of which all more complicated machines (sometimes called "compound machines") are composed. 
> For example, wheels, levers, and pulleys are all used in the mechanism of a bicycle. 
> The mechanical advantage of a compound machine is just 
> the product of the mechanical advantages of the simple machines of which it is composed.
>
> Although they continue to be of great importance in mechanics and applied science, 
> modern mechanics has moved beyond the view of the simple machines as the ultimate building blocks 
> of which all machines are composed, which arose in the Renaissance 
> as a neoclassical amplification of ancient Greek texts. 
> 
> The great variety and sophistication of modern machine linkages, 
> which arose during the Industrial Revolution, is inadequately described by these six simple categories. 
> Various post-Renaissance authors have compiled expanded lists of "simple machines", 
> often using terms like basic machines, compound machines, or machine elements 
> to distinguish them from the classical simple machines above. 
> 
> By the late 1800s, Franz Reuleaux had identified hundreds of machine elements, 
> calling them simple machines. 
> 
> Modern machine theory analyzes machines as kinematic chains 
> composed of elementary linkages called kinematic pairs.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Simple%20machine) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Mechanical_Engineering/Simple_Machine|Simple_Machine]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Mechanical_Engineering/Simple_Machine.public|Simple_Machine.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Mechanical_Engineering/Simple_Machine.internal|Simple_Machine.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Mechanical_Engineering/Simple_Machine.protect|Simple_Machine.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Mechanical_Engineering/Simple_Machine.private|Simple_Machine.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Mechanical_Engineering/Simple_Machine.personal|Simple_Machine.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Mechanical_Engineering/Simple_Machine.secret|Simple_Machine.secret]] 

