---
aliases:
- asse della ruota
- Gololleefi ulee
- hengerkerék
- hjul och axel
- Kaliding ngan ehe
- kolo in os
- Kolo na hřídeli
- Kołowrót
- pyörä ja akseli
- Pöör
- Rad mit Welle
- rado-akso
- Roa e assal
- roată și ax
- Roda dan gandar
- roda e eixe
- ros hag aghel
- roue et moyeu
- Rueda y eixe
- Tekerlek ve dingil
- torn de corró
- torno
- tornu (fisika)
- Točak i osovina
- Wellrad
- Wheel and Axle
- Wheel_and_Axle
- wiel en as
- Winde
- Windlass
- Wou nan aks
- віхар
- ворот
- Колело и ос
- коловорот
- Төгүр уонна сис
- גלגל עם ציר
- دولاب وجزع
- چرخ و محور
- ڕەوڕەوە و تەوەرە
- पाङ्ग्रा र बिँड
- சில்லும் அச்சாணியும்
- చక్రం మరియు ఇరుసు
- ሽክርክርና ዘንግ
- 輪軸
- 축바퀴
has_id_wikidata: Q592289
subclass_of:
- '[[_Standards/WikiData/WD~lifting_equipment,173878|WD~lifting_equipment,173878]]'
- '[[_Standards/WikiData/WD~simple_machine,186713|WD~simple_machine,186713]]'
has_part_s_:
- '[[_Standards/WikiData/WD~axle,188209|WD~axle,188209]]'
- '[[_Standards/WikiData/WD~wheel,446|WD~wheel,446]]'
different_from:
- '[[_Standards/WikiData/WD~windlass,2373406|WD~windlass,2373406]]'
- '[[_Standards/WikiData/WD~Q16291272,16291272|WD~Q16291272,16291272]]'
described_by_source: '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
part_of: '[[_Standards/WikiData/WD~water_well,43483|WD~water_well,43483]]'
Commons_category: Windlass
image: http://commons.wikimedia.org/wiki/Special:FilePath/Wheelaxle%20quackenbos.gif
dv_has_:
  name_:
    am: ሽክርክርና ዘንግ
    an: Rueda y eixe
    ar: دولاب وجزع
    be_tarask: віхар
    bg: Колело и ос
    bs: Točak i osovina
    ca: torn de corró
    ckb: ڕەوڕەوە و تەوەرە
    cs: Kolo na hřídeli
    de: Wellrad
    en: wheel and axle
    eo: rado-akso
    es: torno
    et: Pöör
    eu: tornu (fisika)
    fa: چرخ و محور
    fi: pyörä ja akseli
    fr: roue et moyeu
    gl: roda e eixe
    he: גלגל עם ציר
    ht: Wou nan aks
    hu: hengerkerék
    it: asse della ruota
    ja: 輪軸
    ko: 축바퀴
    kw: ros hag aghel
    ms: Roda dan gandar
    ne: पाङ्ग्रा र बिँड
    nl: wiel en as
    om: Gololleefi ulee
    pl: Kołowrót
    pms: Roa e assal
    ro: roată și ax
    ru: ворот
    sah: Төгүр уонна сис
    sl: kolo in os
    sr: Točak i osovina
    sv: hjul och axel
    ta: சில்லும் அச்சாணியும்
    te: చక్రం మరియు ఇరుసు
    tr: Tekerlek ve dingil
    uk: коловорот
    war: Kaliding ngan ehe
    yue: 輪軸
    zh: 輪軸
---

# [[Wheel_and_Axle]] 

#is_/same_as :: [[../../../../../WikiData/WD~Wheel_and_axle,592289|WD~Wheel_and_axle,592289]]  

## #has_/text_of_/abstract 

> The wheel and axle is a simple machine, 
> consisting of a wheel attached to a smaller axle so that these two parts rotate together, 
> in which a force is transferred from one to the other. 
> 
> The wheel and axle can be viewed as a version of the lever, 
> with a drive force applied tangentially to the perimeter of the wheel, 
> and a load force applied to the axle supported in a bearing, which serves as a fulcrum.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Wheel%20and%20axle) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Mechanical_Engineering/Machine/Classical_simple_Machine/Wheel_and_Axle|Wheel_and_Axle]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Mechanical_Engineering/Machine/Classical_simple_Machine/Wheel_and_Axle.public|Wheel_and_Axle.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Mechanical_Engineering/Machine/Classical_simple_Machine/Wheel_and_Axle.internal|Wheel_and_Axle.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Mechanical_Engineering/Machine/Classical_simple_Machine/Wheel_and_Axle.protect|Wheel_and_Axle.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Mechanical_Engineering/Machine/Classical_simple_Machine/Wheel_and_Axle.private|Wheel_and_Axle.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Mechanical_Engineering/Machine/Classical_simple_Machine/Wheel_and_Axle.personal|Wheel_and_Axle.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Mechanical_Engineering/Machine/Classical_simple_Machine/Wheel_and_Axle.secret|Wheel_and_Axle.secret]] 

