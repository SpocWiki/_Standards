---
aliases:
- angenharie de software
- công nghệ phần mềm
- dasturiy taʼminot muhandisligi
- endezyariya nivîsbariyê
- engenharia de software
- engignerìa dël software
- enginyeria de programari
- enxeñaría de software
- génie logiciel
- hugbúnaðarverkfræði
- ingegneria del software
- ingeniaria corporum programmatum
- ingeniería de software
- inginerie software
- inhenyeriyang pang-software
- inhenyerya hin software
- injeniorarto di programaro
- injiniyan software
- innealtóireacht bogearraí
- Inxeniería del software
- inxhinieria softuerike
- inzegnaria del software
- inżynieria oprogramowania
- jeshaghteyrys cooid vog
- kejuruteraan perisian
- llamp'ukaq k'illikachay
- ohjelmistotuotanto
- peirianneg meddalwedd
- programara inĝenierarto
- programmalıq injiniring
- programminženierija
- programsko inženirstvo
- programsko inženjerstvo
- programvareutvikling
- programvaruutveckling
- programų inžinerija
- proqram təminatı mühəndisliyi
- rekayasa perangkat lunak
- rékayasa software
- saftware ingineerin
- sagteware-ingenieurswese
- softverski inženjering
- softvérové inžinierstvo
- software engineering
- software-ingeniaritza
- Softwaretechnik
- softwarové inženýrství
- szoftvertechnológia
- tiätturohjelmpyevtittem
- Uundaji bidhaa pepe za tarakilishi
- yazılım mühendisliği
- μηχανική λογισμικού
- інжынерыя праграмнага забеспячэння
- бағдарламалық жасақтама инжинирингі
- инжењеринг софтвера
- инженерия программного обеспечения
- праграмны інжынірынг
- програм хангамжийн инженерчлэл
- програмна інженерія
- програмско инженерство
- софтуерно инженерство
- ծրագրային ապահովման ճարտարագիտություն
- הנדסת תוכנה
- ווייכווארג אינזשעניריע
- ئەندازیاریی نەرمامێر
- د سافټوير انجنیري
- سافٹ ویئر انجینئرنگ
- سافٽ ويئر انجنيئرنگ
- مهندسی نرمافزار
- نرمأفزار ٚ مۊهندسي
- هندسة البرمجيات
- کجوروتراءن ڤرايسين
- सॉफ्टवेयर इंजीनियरिंग
- सॉफ्टवेयर इंजीनियरी
- সফটওয়্যার প্রকৌশল
- ସଫ୍ଟୱେର ଇଞ୍ଜିନିୟରିଂ
- மென்பொருட் பொறியியல்
- സോഫ്റ്റ്വെയർ എഞ്ചിനീയറിങ്
- මෘදුකාංග ඉංජිනේරු ශිල්පය
- วิศวกรรมซอฟต์แวร์
- ሶፍትዌር ምህንድስና
- የሶፍትዌር አሠራር
- វិស្វកម្មផ្នែកទន់
- ソフトウェア工学
- 軟件工程
- 軟體工程
- 软件工程
- 소프트웨어 공학
- 쏘프트웨어공학
has_id_wikidata: Q80993
is_the_study_of: '[[_Standards/WikiData/WD~software_development,638608]]'
part_of:
- '[[_Standards/WikiData/WD~systems_engineering,682496]]'
- '[[_Standards/WikiData/WD~information_technology,11661]]'
subclass_of:
- '[[_Standards/WikiData/WD~systems_engineering,682496]]'
- '[[_Standards/WikiData/WD~computer_science,21198]]'
instance_of:
- '[[_Standards/WikiData/WD~field_of_study,1047113]]'
- '[[_Standards/WikiData/WD~field_of_study,2267705]]'
- '[[_Standards/WikiData/WD~economic_sector,3958441]]'
- '[[_Standards/WikiData/WD~academic_major,4671286]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829]]'
- '[[_Standards/WikiData/WD~branch_of_engineering,12015335]]'
used_by:
- '[[_Standards/WikiData/WD~information_engineering,1254373]]'
- '[[_Standards/WikiData/WD~organizational_engineering,7102019]]'
practiced_by:
- '[[_Standards/WikiData/WD~software_engineer,1709010]]'
- '[[_Standards/WikiData/WD~research_software_engineer,106268090]]'
history_of_topic: '[[_Standards/WikiData/WD~history_of_software_engineering,3348143]]'
has_list: '[[_Standards/WikiData/WD~Outline_of_software_engineering,7112734]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Software_engineering,10963663]]'
topic_s_main_Wikimedia_portal: '[[_Standards/WikiData/WD~Portal_Software_engineering,63138188]]'
OmegaWiki_Defined_Meaning: 662384
OKRB_011_2022_code: 6-05-0612-01
Stack_Exchange_site_URL:
- https://softwareengineering.stackexchange.com/
- https://stackoverflow.com/
image: http://commons.wikimedia.org/wiki/Special:FilePath/Iterative%20development%20model.svg
Universal_Decimal_Classification: 004.56
Commons_category: Software engineering
GitHub_topic: software-engineering
subreddit: SoftwareEngineering
dv_has_:
  name_:
    af: sagteware-ingenieurswese
    am: የሶፍትዌር አሠራር
    ar: هندسة البرمجيات
    ast: Inxeniería del software
    az: proqram təminatı mühəndisliyi
    be: інжынерыя праграмнага забеспячэння
    be_tarask: праграмны інжынірынг
    bg: софтуерно инженерство
    bho: सॉफ्टवेयर इंजीनियरिंग
    bn: সফটওয়্যার প্রকৌশল
    bs: softverski inženjering
    ca: enginyeria de programari
    ckb: ئەندازیاریی نەرمامێر
    cs: softwarové inženýrství
    cy: peirianneg meddalwedd
    da: software engineering
    de: Softwaretechnik
    de_ch: Softwaretechnik
    el: μηχανική λογισμικού
    en: software engineering
    en_ca: software engineering
    en_gb: software engineering
    eo: programara inĝenierarto
    es: ingeniería de software
    eu: software-ingeniaritza
    fa: مهندسی نرمافزار
    fi: ohjelmistotuotanto
    fr: génie logiciel
    ga: innealtóireacht bogearraí
    gl: enxeñaría de software
    glk: نرمأفزار ٚ مۊهندسي
    gv: jeshaghteyrys cooid vog
    ha: injiniyan software
    he: הנדסת תוכנה
    hi: सॉफ्टवेयर इंजीनियरी
    hr: programsko inženjerstvo
    hu: szoftvertechnológia
    hy: ծրագրային ապահովման ճարտարագիտություն
    id: rekayasa perangkat lunak
    io: injeniorarto di programaro
    is: hugbúnaðarverkfræði
    it: ingegneria del software
    ja: ソフトウェア工学
    kaa: programmalıq injiniring
    kk: бағдарламалық жасақтама инжинирингі
    km: វិស្វកម្មផ្នែកទន់
    ko: 소프트웨어 공학
    ko-kp: 쏘프트웨어공학
    ku: endezyariya nivîsbariyê
    la: ingeniaria corporum programmatum
    lo: software engineering
    lt: programų inžinerija
    lv: programminženierija
    mk: програмско инженерство
    ml: സോഫ്റ്റ്വെയർ എഞ്ചിനീയറിങ്
    mn: програм хангамжийн инженерчлэл
    ms: kejuruteraan perisian
    ms_arab: کجوروتراءن ڤرايسين
    mwl: angenharie de software
    nb: programvareutvikling
    nl: software engineering
    nn: programvareutvikling
    or: ସଫ୍ଟୱେର ଇଞ୍ଜିନିୟରିଂ
    pl: inżynieria oprogramowania
    pms: engignerìa dël software
    ps: د سافټوير انجنیري
    pt: engenharia de software
    pt_br: engenharia de software
    qu: llamp'ukaq k'illikachay
    ro: inginerie software
    ru: инженерия программного обеспечения
    sco: saftware ingineerin
    sd: سافٽ ويئر انجنيئرنگ
    sh: programsko inženjerstvo
    si: මෘදුකාංග ඉංජිනේරු ශිල්පය
    sk: softvérové inžinierstvo
    sl: programsko inženirstvo
    smn: tiätturohjelmpyevtittem
    sq: inxhinieria softuerike
    sr: инжењеринг софтвера
    sr_ec: инжењеринг софтвера
    su: rékayasa software
    sv: programvaruutveckling
    sw: Uundaji bidhaa pepe za tarakilishi
    ta: மென்பொருட் பொறியியல்
    th: วิศวกรรมซอฟต์แวร์
    ti: ሶፍትዌር ምህንድስና
    tl: inhenyeriyang pang-software
    tr: yazılım mühendisliği
    uk: програмна інженерія
    ur: سافٹ ویئر انجینئرنگ
    uz: dasturiy taʼminot muhandisligi
    vec: inzegnaria del software
    vi: công nghệ phần mềm
    war: inhenyerya hin software
    wuu: 软件工程
    yi: ווייכווארג אינזשעניריע
    yue: 軟件工程
    zh: 软件工程
    zh_hans: 软件工程
    zh_hant: 軟體工程
---
# [[Software_Engineering]] 

#is_/same_as :: [[../../WikiData/WD~Software_engineering,80993|WD~Software_engineering,80993]]  

## #has_/text_of_/abstract 

> Software engineering is a branch of both computer science and engineering 
> focused on designing, developing, testing, and maintaining software applications. 
> 
> It involves applying engineering principles and computer programming expertise 
> to develop software systems that meet user needs.
>
> The terms programmer and coder overlap software engineer, 
> but they imply only the construction aspect of a typical software engineer workload.
>
> A software engineer applies a software development process, which involves 
> defining, implementing, testing, managing, and maintaining software systems, 
> as well as developing the software development process itself.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Software%20engineering) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Software_Engineering|Software_Engineering]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Software_Engineering.public|Software_Engineering.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Software_Engineering.internal|Software_Engineering.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Software_Engineering.protect|Software_Engineering.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Software_Engineering.private|Software_Engineering.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Software_Engineering.personal|Software_Engineering.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Software_Engineering.secret|Software_Engineering.secret]] 

