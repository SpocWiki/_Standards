---
aliases:
- angenharie macánica
- ars mechanica
- engenharia mecânica
- enginyeria mecànica
- engèni mecanic
- enxeñaría mecánica
- fasahar injiniya ta inji
- génie mécanique
- gépészmérnöki tudomány
- Ingegneri mecaneghi
- ingegneria meccanica
- Ingegnerìa mecànica
- ingeniaritza mekaniko
- ingeniería mecánica
- inghenyeriya mekanikanhën
- inginerie mecanică
- inhenyerya nga mekanikal
- inhinyeriyang mekanikal
- Injinir mekanikal
- innealtóireacht mheicniúil
- inxeniería mecánica
- inxhinieria mekanike
- inġinerija mekkanika
- inżynieria mechaniczna
- jéni mékanik
- kejuruteraan mekanik
- konetekniikka
- kî-hài kûng-chhàng
- kỹ thuật cơ khí
- mach'a purichiq
- makine mühendisliği
- Maschinenbau
- Maschinenbu
- Maschinnebau
- mashinasozlik sanoati
- masinaehitus
- maskinteknik
- maskinteknikk
- maşyn inženerligi
- maşınqayırma
- mašinstvo
- mašīnbūves teorija
- Mechanical engineering
- Mechanical_Engineering
- mechanikos inžinerija
- meganiese ingenieurswese
- meĥanika inĝenierarto
- mikianikal injinierin
- ncignirìa miccànica
- pūhanga pūrere
- strojarstvo
- strojništvo
- strojárstvo
- strojírenství
- teknik mesin
- uhandisi mitambo
- werktuigbouwkunde
- werktuigkundige
- μηχανολογία
- массыыналааһын
- машин үйлдвэрлэл
- Машина жасау
- машина төзү сәнәгате
- машиналар эшләү
- машинобудування
- машиностроене
- машиностроение
- машинство
- машынабудаванне
- машынабудаваньне
- мошинсозӣ
- մեքենաշինություն
- הנדסת מכונות
- آلاتی ہندسیات
- ئەندازیاریی میکانیکی
- ماشىنىىسازلىق ئىنژىنىرلىقى
- مهندسی مکانیک
- ميڪينيڪل انجنيئرنگ
- هندسة ميكانيكية
- هندسه ميكانيكيه
- मेकानिकल इन्जिनियरिङ
- यांत्रिक अभियांत्रिकी
- यांत्रिक इंजीनियरी
- যন্ত্র প্রকৌশল
- যান্ত্ৰিক অভিযান্ত্ৰিকী
- મિકેનિકલ એન્જિનિયરિંગ
- இயந்திரவியல் பொறியியல்
- మెకానికల్ ఇంజనీరింగ్
- ಯಾ೦ತ್ರಿಕ ಎ೦ಜಿನಿಯರಿ೦ಗ್
- മെക്കാനിക്കൽ എൻജിനീയറിങ്ങ്
- යාන්ත්රික ඉංජිනේරු විද්යාව
- วิศวกรรมเครื่องกล
- စက်မှုအင်ဂျင်နီယာ ဘာသာရပ်
- მანქანათმშენებლობა
- ⵜⴰⵏⵣⴳⴳⵉⵜ ⵜⴰⵎⵉⴽⴰⵏⵉⴽⵜ
- 工程學
- 机械工程
- 機械工学
- 機械工程
- 기계공학
has_id_wikidata: Q101333
different_from:
- '[[_Standards/WikiData/WD~civil_engineering,77590]]'
- '[[_Standards/WikiData/WD~structural_engineering,633538]]'
- '[[_Standards/WikiData/WD~manufacture_of_machinery_and_equipment,1957908]]'
- '[[_Standards/WikiData/WD~mechanical_engineering_studies,2358766]]'
- '[[_Standards/WikiData/WD~design_engineering,5638464]]'
instance_of:
- '[[_Standards/WikiData/WD~industry,268592]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829]]'
- '[[_Standards/WikiData/WD~branch_of_engineering,12015335]]'
practiced_by: '[[_Standards/WikiData/WD~mechanical_engineer,1906857]]'
described_by_source:
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
said_to_be_the_same_as:
- '[[_Standards/WikiData/WD~Q12056592,12056592]]'
- '[[_Standards/WikiData/WD~Q12335235,12335235]]'
topic_s_main_Wikimedia_portal: '[[_Standards/WikiData/WD~Portal_Mechanics,18820035]]'
has_list: '[[_Standards/WikiData/WD~Glossary_of_mechanical_engineering,24899018]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mechanics,52677583]]'
subclass_of: '[[_Standards/WikiData/WD~engineering,11023]]'
IAB_code: 105
Basisklassifikation: 52
OmegaWiki_Defined_Meaning: 2301
All_Science_Journal_Classification_Codes: 2210
U_S_National_Archives_Identifier: 10639809
Colon_Classification: D6
image: http://commons.wikimedia.org/wiki/Special:FilePath/1993%20bmw%20325is%20engine.jpg
Stack_Exchange_tag: https://engineering.stackexchange.com/tags/mechanical-engineering
Dewey_Decimal_Classification: 621.8
Wolfram_Language_entity_code: Entity["Concept", "MechanicalEngineering::z5q5c"]
Commons_category: Mechanical engineering
IPTC_NewsCode: mediatopic/20000716
dv_has_:
  name_:
    af: meganiese ingenieurswese
    ar: هندسة ميكانيكية
    arz: هندسه ميكانيكيه
    as: যান্ত্ৰিক অভিযান্ত্ৰিকী
    ast: inxeniería mecánica
    az: maşınqayırma
    ba: машиналар эшләү
    bcl: inhinyeriyang mekanikal
    be: машынабудаванне
    be_tarask: машынабудаваньне
    bg: машиностроене
    bn: যন্ত্র প্রকৌশল
    bs: mašinstvo
    ca: enginyeria mecànica
    ckb: ئەندازیاریی میکانیکی
    cs: strojírenství
    da: maskinteknik
    de: Maschinenbau
    el: μηχανολογία
    en: mechanical engineering
    eo: meĥanika inĝenierarto
    es: ingeniería mecánica
    et: masinaehitus
    eu: ingeniaritza mekaniko
    fa: مهندسی مکانیک
    fi: konetekniikka
    fr: génie mécanique
    ga: innealtóireacht mheicniúil
    gan: 工程學
    gcr: jéni mékanik
    gl: enxeñaría mecánica
    gu: મિકેનિકલ એન્જિનિયરિંગ
    ha: fasahar injiniya ta inji
    hak: kî-hài kûng-chhàng
    he: הנדסת מכונות
    hi: यांत्रिक इंजीनियरी
    hr: strojarstvo
    hu: gépészmérnöki tudomány
    hy: մեքենաշինություն
    iba: Injinir mekanikal
    id: teknik mesin
    it: ingegneria meccanica
    ja: 機械工学
    jam: mikianikal injinierin
    ka: მანქანათმშენებლობა
    kk: Машина жасау
    kn: ಯಾ೦ತ್ರಿಕ ಎ೦ಜಿನಿಯರಿ೦ಗ್
    ko: 기계공학
    krj: inghenyeriya mekanikanhën
    la: ars mechanica
    lb: Maschinnebau
    li: werktuigkundige
    lt: mechanikos inžinerija
    lv: mašīnbūves teorija
    mi: pūhanga pūrere
    mk: машинство
    ml: മെക്കാനിക്കൽ എൻജിനീയറിങ്ങ്
    mn: машин үйлдвэрлэл
    mr: यांत्रिक अभियांत्रिकी
    ms: kejuruteraan mekanik
    mt: inġinerija mekkanika
    mwl: angenharie macánica
    my: စက်မှုအင်ဂျင်နီယာ ဘာသာရပ်
    nb: maskinteknikk
    nds: Maschinenbu
    ne: मेकानिकल इन्जिनियरिङ
    nl: werktuigbouwkunde
    nn: maskinteknikk
    oc: engèni mecanic
    pl: inżynieria mechaniczna
    pms: Ingegnerìa mecànica
    pt: engenharia mecânica
    qu: mach'a purichiq
    ro: inginerie mecanică
    ru: машиностроение
    sah: массыыналааһын
    scn: ncignirìa miccànica
    sd: ميڪينيڪل انجنيئرنگ
    sh: mašinstvo
    si: යාන්ත්රික ඉංජිනේරු විද්යාව
    sk: strojárstvo
    sl: strojništvo
    sq: inxhinieria mekanike
    sr: машинство
    sr_ec: машинство
    sr_el: mašinstvo
    sv: maskinteknik
    sw: uhandisi mitambo
    ta: இயந்திரவியல் பொறியியல்
    te: మెకానికల్ ఇంజనీరింగ్
    tg: мошинсозӣ
    th: วิศวกรรมเครื่องกล
    tk: maşyn inženerligi
    tl: inhinyeriyang mekanikal
    tr: makine mühendisliği
    tt: машина төзү сәнәгате
    ug: ماشىنىىسازلىق ئىنژىنىرلىقى
    uk: машинобудування
    ur: آلاتی ہندسیات
    uz: mashinasozlik sanoati
    vec: Ingegneri mecaneghi
    vi: kỹ thuật cơ khí
    war: inhenyerya nga mekanikal
    wuu: 机械工程
    yue: 機械工程
    zgh: ⵜⴰⵏⵣⴳⴳⵉⵜ ⵜⴰⵎⵉⴽⴰⵏⵉⴽⵜ
    zh: 机械工程
    zh_cn: 机械工程
    zh_hans: 机械工程
    zh_hant: 機械工程
    zh_hk: 機械工程
    zh_sg: 机械工程
    zh_tw: 機械工程
---
# [[Mechanical_Engineering]] 

#is_/same_as :: [[../../WikiData/WD~Mechanical_engineering,101333|WD~Mechanical_engineering,101333]]  

## #has_/text_of_/abstract 

> **Mechanical engineering** is the study of physical machines that may involve force and movement. 
> It is an engineering branch that combines engineering physics and mathematics principles 
> with materials science, to design, analyze, manufacture, and maintain mechanical systems. 
> It is one of the oldest and broadest of the engineering branches.
>
> Mechanical engineering requires an understanding of core areas including mechanics, dynamics, thermodynamics, materials science, design, structural analysis, and electricity. In addition to these core principles, mechanical engineers use tools such as computer-aided design (CAD), computer-aided manufacturing (CAM), computer-aided engineering (CAE), and product lifecycle management to design and analyze manufacturing plants, industrial equipment and machinery, heating and cooling systems, transport systems, motor vehicles, aircraft, watercraft, robotics, medical devices, weapons, and others.
>
> Mechanical engineering emerged as a field during the Industrial Revolution in Europe in the 18th century; however, its development can be traced back several thousand years around the world. In the 19th century, developments in physics led to the development of mechanical engineering science. The field has continually evolved to incorporate advancements; today mechanical engineers are pursuing developments in such areas as composites, mechatronics, and nanotechnology. It also overlaps with aerospace engineering, metallurgical engineering, civil engineering, structural engineering, electrical engineering, manufacturing engineering, chemical engineering, industrial engineering, and other engineering disciplines to varying amounts. Mechanical engineers may also work in the field of biomedical engineering, specifically with biomechanics, transport phenomena, biomechatronics, bionanotechnology, and modelling of biological systems.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Mechanical%20engineering) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Mechanical_Engineering|Mechanical_Engineering]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Mechanical_Engineering.public|Mechanical_Engineering.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Mechanical_Engineering.internal|Mechanical_Engineering.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Mechanical_Engineering.protect|Mechanical_Engineering.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Mechanical_Engineering.private|Mechanical_Engineering.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Mechanical_Engineering.personal|Mechanical_Engineering.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Mechanical_Engineering.secret|Mechanical_Engineering.secret]] 

