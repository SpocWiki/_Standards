---
aliases:
- aménagement urbain
- aredo urban
- arredo urbano
- dearadh uirbeach
- desenho urbano
- deseño urbano
- design urban
- Designatio urbana
- diseño urbano
- Diseñu urbanu
- disinn urban
- disseny urbà
- Hiri diseinu
- kaupunkisuunnittelu
- Kentsel tasarım
- mała architektura
- navrhování měst
- Perancangan perkotaan
- stadsplanering
- stedenbouwkunde
- Städtebau
- Thiết kế đô thị
- urban design
- Urbani dizajn
- urbanistično projektiranje
- urbant design
- urboplanado
- горадабудаўнічае праектаванне
- горадабудаўнічае праектаваньне
- градостроительное проектирование
- містобудівне проєктування
- Урбан дизајн
- Урбани дизајн
- шәһәр төзелешен проектлаштыру
- քաղաքային դիզայն
- עיצוב עירוני
- تصميم عمراني
- طراحی شهری
- நகர்ப்புற வடிவமைப்பு
- การออกแบบชุมชนเมือง
- ურბანული დიზაინი
- アーバンデザイン
- 城市设计
- 都市設計
- 도시설계
has_id_wikidata: Q63100
partially_coincident_with:
- '[[_Standards/WikiData/WD~urban_planning,69883]]'
- '[[_Standards/WikiData/WD~architecture,12271]]'
subclass_of: '[[_Standards/WikiData/WD~design,82604]]'
instance_of:
- '[[_Standards/WikiData/WD~industry,268592]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829]]'
described_by_source: '[[_Standards/WikiData/WD~Dresdner_Hefte,14916674]]'
studied_by: '[[_Standards/WikiData/WD~urban_studies,23578769]]'
practiced_by: '[[_Standards/WikiData/WD~urban_designer,84308440]]'
uses: '[[_Standards/WikiData/WD~urbanism,59950]]'
Commons_category: Urban design
spoken_text_audio: http://commons.wikimedia.org/wiki/Special:FilePath/Es-Dise%C3%B1o%20urbano-article.ogg
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q150%20%28fra%29-Visiteuse%20JEP%20%28Madehub%29-am%C3%A9nagement%20urbain.wav
image: http://commons.wikimedia.org/wiki/Special:FilePath/PM%20122173%20B%20Oudenaarde.jpg
dv_has_:
  name_:
    ar: تصميم عمراني
    ast: Diseñu urbanu
    be: горадабудаўнічае праектаванне
    be-tarask: горадабудаўнічае праектаваньне
    bs: Urbani dizajn
    ca: disseny urbà
    cs: navrhování měst
    da: urban design
    de: Städtebau
    en: urban design
    en-gb: urban design
    eo: urboplanado
    es: diseño urbano
    eu: Hiri diseinu
    fa: طراحی شهری
    fi: kaupunkisuunnittelu
    fr: aménagement urbain
    ga: dearadh uirbeach
    gl: deseño urbano
    he: עיצוב עירוני
    hy: քաղաքային դիզայն
    id: Perancangan perkotaan
    it: arredo urbano
    ja: アーバンデザイン
    ka: ურბანული დიზაინი
    ko: 도시설계
    la: Designatio urbana
    mk: Урбан дизајн
    mt: disinn urban
    nb: urbant design
    nl: stedenbouwkunde
    pl: mała architektura
    pt: desenho urbano
    ro: design urban
    ru: градостроительное проектирование
    sl: urbanistično projektiranje
    sr: Урбани дизајн
    sv: stadsplanering
    ta: நகர்ப்புற வடிவமைப்பு
    th: การออกแบบชุมชนเมือง
    tr: Kentsel tasarım
    tt: шәһәр төзелешен проектлаштыру
    uk: містобудівне проєктування
    vec: aredo urban
    vi: Thiết kế đô thị
    yue: 都市設計
    zh: 城市设计
---
# [[Urban_Design]] 

#is_/same_as :: [[../../../../WikiData/WD~Urban_design,63100|WD~Urban_design,63100]]

process of designing and shaping cities, towns and villages (often from scratch)
for specific outcomes

## #has_/text_of_/abstract 

> Urban design is an approach to the design of buildings and the spaces between them 
> that focuses on specific design processes and outcomes based on geographical location. 
> 
> In addition to designing and shaping the physical features of 
> towns, cities, and regional spaces, 
> urban design considers 'bigger picture' issues of 
> economic, social and environmental value and social design. 
> 
> The scope of a project can range from a local street or public space 
> to an entire city and surrounding areas. 
> 
> Urban designers connect the fields of architecture, landscape architecture 
> and urban planning to better organize local and community environments' 
> dependent upon geographical location.
>
> Some important focuses of urban design include its historical impact, 
> paradigm shifts, its interdisciplinary nature, and issues related to urban design.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Urban%20design) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Civil_Engineering/Urban_Planning/Urban_Design|Urban_Design]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Civil_Engineering/Urban_Planning/Urban_Design.public|Urban_Design.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Civil_Engineering/Urban_Planning/Urban_Design.internal|Urban_Design.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Civil_Engineering/Urban_Planning/Urban_Design.protect|Urban_Design.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Civil_Engineering/Urban_Planning/Urban_Design.private|Urban_Design.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Civil_Engineering/Urban_Planning/Urban_Design.personal|Urban_Design.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Civil_Engineering/Urban_Planning/Urban_Design.secret|Urban_Design.secret]] 

