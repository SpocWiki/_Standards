---
aliases:
- anyagszilárdság
- beriktik
- fasthetslære
- fastleikslære
- Festigkeitslehre
- forca e materialeve
- hållfasthetslära
- Kekuatan bahan
- lujuusoppi
- materialen erresistentzia
- materiālu pretestība
- mechanics of materials
- meĥaniko de materialoj
- mukavemet
- möhkəmlik
- nauka o čvrstoći
- pevnosť
- pružnost a pevnost
- resistencia de materiales
- resistenza meccanica
- resistència de materials
- resistência mecânica
- rezistența materialelor
- righneas
- résistance des matériaux
- sterkteleer
- sức bền vật liệu
- trdnost
- tugevusõpetus
- wytrzymałość materiałów
- Αντοχή των υλικών
- бат бөхийн онол
- беріктік
- материалаша духьале яр
- материалдар ҡаршылығы
- материалсен чăтăмлăхĕ
- материалъёслэн пумитъяськонлыксы
- опір матеріалів
- сопротивление материалов
- супраціўленне матэрыялаў
- супраціўленьне матэрыялаў
- цврстина на материјалите
- якост
- Նյութերի դիմադրություն
- חוזק חומרים
- بەرىكتىك
- مقاومة المواد
- مکانیک مواد
- पदार्थ प्रबलता
- পদার্থের শক্তি
- పదార్థాల బలం
- പ്രബലത
- მასალათა გამძლეობა
- 材料力学
- 材料力學
- 재료역학
has_id_wikidata: Q240553
described_by_source:
- '[[../../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- "[[_Standards/WikiData/WD~Otto's_encyclopedia,2041543]]"
subclass_of:
- '[[_Standards/WikiData/WD~solid_mechanics,1080293]]'
- '[[_Standards/WikiData/WD~materials_engineering,1710723]]'
OmegaWiki_Defined_Meaning: 3643
U_S_National_Archives_Identifier: 10643394
Commons_category: Strength of materials
image: http://commons.wikimedia.org/wiki/Special:FilePath/Discorsi%20e%20dimostrazioni%20matematiche%20intorno%20%C3%A0%20due%20nuoue%20scienze%20di%20Galileo%20Galilei%20%28Leida%2C%201638%29.tif
Krugosvet_article_archived_: nauka_i_tehnika/tehnologiya_i_promyshlennost/SOPROTIVLENIE_MATERIALOV.html
Dewey_Decimal_Classification: 620.105
dv_has_:
  name_:
    ar: مقاومة المواد
    ast: resistencia de materiales
    az: möhkəmlik
    ba: материалдар ҡаршылығы
    be: супраціўленне матэрыялаў
    be-tarask: супраціўленьне матэрыялаў
    bg: якост
    bn: পদার্থের শক্তি
    ca: resistència de materials
    cs: pružnost a pevnost
    cv: материалсен чăтăмлăхĕ
    de: Festigkeitslehre
    el: Αντοχή των υλικών
    en: mechanics of materials
    eo: meĥaniko de materialoj
    es: resistencia de materiales
    et: tugevusõpetus
    eu: materialen erresistentzia
    fa: مکانیک مواد
    fi: lujuusoppi
    fr: résistance des matériaux
    ga: righneas
    he: חוזק חומרים
    hi: पदार्थ प्रबलता
    hr: nauka o čvrstoći
    hu: anyagszilárdság
    hy: Նյութերի դիմադրություն
    id: Kekuatan bahan
    inh: материалаша духьале яр
    it: resistenza meccanica
    ja: 材料力学
    ka: მასალათა გამძლეობა
    kk: беріктік
    kk-arab: بەرىكتىك
    kk-cn: بەرىكتىك
    kk-cyrl: беріктік
    kk-kz: беріктік
    kk-latn: beriktik
    kk-tr: beriktik
    ko: 재료역학
    lv: materiālu pretestība
    mk: цврстина на материјалите
    ml: പ്രബലത
    mn: бат бөхийн онол
    ms: kekuatan bahan
    nb: fasthetslære
    nl: sterkteleer
    nn: fastleikslære
    pl: wytrzymałość materiałów
    pt: resistência mecânica
    ro: rezistența materialelor
    ru: сопротивление материалов
    sh: nauka o čvrstoći
    sk: pevnosť
    sl: trdnost
    sq: forca e materialeve
    sr: nauka o čvrstoći
    sv: hållfasthetslära
    te: పదార్థాల బలం
    tr: mukavemet
    udm: материалъёслэн пумитъяськонлыксы
    uk: опір матеріалів
    vi: sức bền vật liệu
    wuu: 材料力学
    yue: 材料力學
    zh: 材料力学
    zh-cn: 材料力学
    zh-hans: 材料力学
    zh-hant: 材料力學
    zh-hk: 材料力學
    zh-sg: 材料力学
    zh-tw: 材料力學
---
# [[Strength_of_Material]] 

#is_/same_as :: [[../../../../../WikiData/WD~Strength_of_materials,240553|WD~Strength_of_materials,240553]]  

## #has_/text_of_/abstract 

> The strength of materials is determined using 
> various methods of calculating the stresses and strains in structural members, 
> such as beams, columns, and shafts. 
> 
> The methods employed to predict the response of a structure under loading 
> and its susceptibility to various failure modes 
> takes into account the properties of the materials such as its yield strength, 
> ultimate strength, Young's modulus, and Poisson's ratio. 
> 
> In addition, the mechanical element's macroscopic properties 
> (geometric properties) such as its length, width, thickness, boundary constraints 
> and abrupt changes in geometry such as holes are considered.
>
> The theory began with the consideration of 
> the behavior of 1D and 2D members of structures, 
> whose states of stress can be approximated as two dimensional, 
> and was then generalized to three dimensions 
> to develop a more complete theory of the elastic and plastic behavior of materials. 
> 
> An important founding pioneer in mechanics of materials was Stephen Timoshenko.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Strength%20of%20materials) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Civil_Engineering/Construction/Building_Material/Strength_of_Material|Strength_of_Material]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Civil_Engineering/Construction/Building_Material/Strength_of_Material.public|Strength_of_Material.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Civil_Engineering/Construction/Building_Material/Strength_of_Material.internal|Strength_of_Material.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Civil_Engineering/Construction/Building_Material/Strength_of_Material.protect|Strength_of_Material.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Civil_Engineering/Construction/Building_Material/Strength_of_Material.private|Strength_of_Material.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Civil_Engineering/Construction/Building_Material/Strength_of_Material.personal|Strength_of_Material.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Civil_Engineering/Construction/Building_Material/Strength_of_Material.secret|Strength_of_Material.secret]] 

