---
aliases:
- bene
- Berǧ (tasegda)
- birc
- Bokštas
- Buokšts
- doardna
- dorre
- Inqaba
- kowela
- Kula
- kule
- kullë
- Kulukulu kuduyuu
- Lantawan
- manara
- menara
- Minaro
- Minora
- Mnara
- Pagungan
- Qulle
- Qüllə (memarlıq)
- stolp
- Thah
- tháp
- Tilikambo
- toarnna
- toer
- tomo sewi linja
- Toorn
- Tor
- toranj
- tore
- toren
- toring
- torma
- torn
- torni
- tornis
- tornâ
- torony
- Torr
- Torre
- torri
- Tou
- tour
- Tour (édifis)
- Towa
- Tower
- Tuam
- Tuerm
- Turm
- Turmo
- turn
- turo
- Turre
- Turri
- turris
- tårdn
- tårn
- tórre
- túr
- tŵr
- veža
- věž
- wieża
- wěža
- Čuhunduz
- πύργος
- башне
- башня
- БӀов
- вежа
- кабаз
- кевмар
- Кула
- мæсыг
- манара
- Мунара
- Мұнара
- Самхаг
- Тала
- торањ
- трѣмъ
- цамхаг
- աշտարակ
- Աշտարակներ
- טורעם
- מגדל
- برج
- برج (عمارت)
- بورج
- بورج (تیکینتی)
- مۇنار
- ܡܓܕܠܐ
- बुर्ज
- मनोरा
- টাৱাৰ
- সুউচ্চ স্থাপনা
- ਟਾਵਰ
- கோபுரம்
- ഗോപുരം
- කුළුණු
- หอคอย
- မျှော်စင်
- კოშკი
- ᎢᎾᎢᎦᏘ ᎠᏁᏍᎨᏓ
- 塔
- 塔式建筑
- 塔式建築
- 西洋塔
- 탑
has_id_wikidata: Q12518
Commons_category: Towers
Commons_gallery: Tower
described_by_source:
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Gujin_Tushu_Jicheng,1768721|WD~Gujin_Tushu_Jicheng,1768721]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~Metropolitan_Museum_of_Art_Tagging_Vocabulary,106727050|WD~Metropolitan_Museum_of_Art_Tagging_Vocabulary,106727050]]'
equivalent_class: http://dbpedia.org/ontology/Tower
GeoNames_feature_code: S.TOWR
icon: http://commons.wikimedia.org/wiki/Special:FilePath/Noun%20232996%20cc%20Castle.svg
image: http://commons.wikimedia.org/wiki/Special:FilePath/Tour%20Eiffel%20Wikimedia%20Commons%20%28cropped%29.jpg
instance_of: '[[_Standards/WikiData/WD~type_of_structure,111972893|WD~type_of_structure,111972893]]'
model_item: '[[_Standards/WikiData/WD~Eiffel_Tower,243|WD~Eiffel_Tower,243]]'
Nomenclature_for_Museum_Cataloging: 958
OmegaWiki_Defined_Meaning: 6890
OpenStreetMap_tag_or_key:
- Tag:building=tower
- Tag:man_made=tower
properties_for_this_type:
- located in time zone
- inception
- official website
- heritage designation
- width
- height
- diameter
- architect
- located in the administrative territorial entity
- architectural style
subclass_of:
- '[[_Standards/WikiData/WD~architectural_structure,811979|WD~architectural_structure,811979]]'
- '[[_Standards/WikiData/WD~building,41176|WD~building,41176]]'
topic_s_main_template: '[[_Standards/WikiData/WD~Q22845830,22845830|WD~Q22845830,22845830]]'
dv_has_:
  name_:
    af: toring
    an: Torre
    ar: برج
    arc: ܡܓܕܠܐ
    arz: برج
    as: টাৱাৰ
    ast: torre
    az: Qüllə (memarlıq)
    azb: بورج (تیکینتی)
    ba: Башня
    bar: Tuam
    be: вежа
    be_tarask: вежа
    bg: Кула
    bjn: Pagungan
    bn: সুউচ্চ স্থাপনা
    br: tour
    bs: toranj
    bxr: Самхаг
    ca: torre
    ce: БӀов
    chr: ᎢᎾᎢᎦᏘ ᎠᏁᏍᎨᏓ
    ckb: بورج
    crh: Qulle
    cs: věž
    csb: torma
    cu: трѣмъ
    cy: tŵr
    da: tårn
    de: Turm
    el: πύργος
    en: tower
    en_ca: tower
    en_gb: tower
    eo: turo
    es: torre
    et: torn
    eu: dorre
    fa: برج
    fi: torni
    fr: tour
    fy: toer
    ga: túr
    gcr: Tour (édifis)
    gl: torre
    gn: tórre
    gsw: Turm
    ha: bene
    haw: kowela
    he: מגדל
    hi: बुर्ज
    hif: Tower
    hr: toranj
    hsb: wěža
    hu: torony
    hy: աշտարակ
    hyw: Աշտարակներ
    ia: Turre
    iba: Menara
    id: menara
    ilo: torre
    io: Turmo
    is: turn
    it: torre
    ja: 塔
    jam: Towa
    jv: Menara
    ka: კოშკი
    kab: Berǧ (tasegda)
    kbp: Kulukulu kuduyuu
    kk: Мұнара
    ko: 탑
    ku: birc
    ku_latn: birc
    kw: Tour
    ky: Мунара
    la: turris
    lad: Kula
    lb: Tuerm
    lmo: Torr
    lt: Bokštas
    lv: tornis
    lzh: 塔
    mg: Tilikambo
    mhr: башне
    min: Minaro
    mk: кула
    ml: ഗോപുരം
    mn: Цамхаг
    mr: मनोरा
    ms: menara
    mt: torri
    my: မျှော်စင်
    myv: кевмар
    nan: Thah
    nb: tårn
    nds: Toorn
    nl: toren
    nn: tårn
    nrm: Tou
    oc: Torre
    os: мæсыг
    pa: ਟਾਵਰ
    pl: wieża
    pms: Tor
    pnb: برج
    pt: torre
    pt_br: torre
    qu: Turri
    ro: turn
    ru: башня
    sah: Тала
    scn: turri
    sd: برج (عمارت)
    se: doardna
    sgs: Buokšts
    sh: toranj
    si: කුළුණු
    sje: tårdn
    sk: veža
    sl: stolp
    smj: toarnna
    smn: tornâ
    sq: kullë
    sr: торањ
    sv: torn
    sw: Mnara
    szl: wieża
    ta: கோபுரம்
    th: หอคอย
    tl: Tore
    tok: tomo sewi linja
    tr: kule
    tt: манара
    tt_cyrl: манара
    tt_latn: manara
    udm: кабаз
    ug: مۇنار
    uk: вежа
    ur: برج
    uz: Minora
    vec: tore
    vep: Čuhunduz
    vi: tháp
    vls: Torre
    war: Lantawan
    wuu: 塔式建筑
    xal: цамхаг
    xh: Inqaba
    yi: טורעם
    yue: 西洋塔
    zh: 塔式建築
    zh_cn: 塔式建筑
    zh_hans: 塔式建筑
    zh_hant: 塔式建築
    zh_hk: 塔式建築
    zh_mo: 塔式建築
    zh-my: 塔式建筑
    zh_sg: 塔式建筑
    zh_tw: 塔式建築
---

# [[Tower]] 

#is_/same_as :: [[../../../../../../WikiData/WD~Tower,12518|WD~Tower,12518]] 

## #has_/text_of_/abstract 

> A **Tower** is a tall structure, taller than it is wide, often by a significant factor. 
> Towers are distinguished from masts by their lack of guy-wires and are therefore, 
> along with tall buildings, self-supporting structures.
>
> Towers are specifically distinguished from buildings in that they are 
> built not to be habitable but to serve other functions using the height of the tower. 
> 
> For example, the height of a clock tower improves the visibility of the clock, 
> and the height of a tower in a fortified building such as a castle 
> increases the visibility of the surroundings for defensive purposes. 
> 
> Towers may also be built for observation, leisure, or telecommunication purposes. 
> A tower can stand alone or be supported by adjacent buildings, 
> or it may be a feature on top of a larger structure or building.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Tower) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Civil_Engineering/Construction/Architecture/High-rise_Building/Tower|Tower]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Civil_Engineering/Construction/Architecture/High-rise_Building/Tower.public|Tower.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Civil_Engineering/Construction/Architecture/High-rise_Building/Tower.internal|Tower.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Civil_Engineering/Construction/Architecture/High-rise_Building/Tower.protect|Tower.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Civil_Engineering/Construction/Architecture/High-rise_Building/Tower.private|Tower.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Civil_Engineering/Construction/Architecture/High-rise_Building/Tower.personal|Tower.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Civil_Engineering/Construction/Architecture/High-rise_Building/Tower.secret|Tower.secret]] 

