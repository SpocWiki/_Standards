---
aliases:
- Architectura sacra
- architecture religieuse
- architektura sakralna
- architettura religiosa
- Arkitektura relihiyosa
- Arkitektura sakratu
- arquitectura religiosa
- Arquitectura relixiosa
- arquitectura sacra
- arquitetura religiosa
- Arquiteutura relixosa
- Arsitektur sakral
- dini mimari
- Gine-gine masu tsarki
- müqəddəs memarlıq
- pensaernïaeth crefyddol
- pyhä arkkitehtuuri
- religia arkitekturo
- religieuze architectuur
- Religinė architektūra
- religious architecture
- religiøs arkitektur
- Sacral architecture
- Sacral_Architecture
- sacred architecture
- Sakraalarhitektuur
- sakral arkitektur
- Sakralarchitektur
- Sakralbau
- sakralna arhitektura
- Sakrálna architektura
- sakrální architektura
- Sios
- szakrális építészet
- vjerska arhitektura
- религиозная архитектура
- сакрална зграда
- сакральна архітектура
- сакральная архітэктура
- свештена архитектура
- Храмова архитектура
- Ғибәҙәтхана архитектураһы
- Տաճարային ճարտարապետություն
- אדריכלות דתית
- عمارة دينية
- معماری مذهبی
- نژیاروانیی دینی
- उपासनास्थल
- புனிதக் கட்டிடக்கலை
- 宗教建筑
- 宗教建築
has_id_wikidata: Q47848
subclass_of:
- '[[_Standards/WikiData/WD~religious_art,2864737]]'
- '[[_Standards/WikiData/WD~architecture,12271]]'
facet_of: '[[_Standards/WikiData/WD~religion,9174]]'
instance_of: '[[_Standards/WikiData/WD~architectural_style,32880]]'
Dewey_Decimal_Classification: 726
Basisklassifikation:
- 21.70
- 56.88
image: http://commons.wikimedia.org/wiki/Special:FilePath/Sint-Martinuskerk%20Moorsel%20-%20392145%20-%20onroerenderfgoed.jpg
Provenio_UUID: 707648c8-a554-448c-b0ae-9bcf0fd19835
Commons_category: Religious architecture
dv_has_:
  name_:
    ar: عمارة دينية
    ast: Arquiteutura relixosa
    az: müqəddəs memarlıq
    ba: Ғибәҙәтхана архитектураһы
    be: сакральная архітэктура
    be_tarask: сакральная архітэктура
    bg: Храмова архитектура
    ca: arquitectura religiosa
    ckb: نژیاروانیی دینی
    cs: sakrální architektura
    cy: pensaernïaeth crefyddol
    de: Sakralarchitektur
    en: sacred architecture
    eo: religia arkitekturo
    es: arquitectura sacra
    et: Sakraalarhitektuur
    eu: Arkitektura sakratu
    fa: معماری مذهبی
    fi: pyhä arkkitehtuuri
    fr: architecture religieuse
    gl: Arquitectura relixiosa
    gsw: Sakralbau
    ha: Gine-gine masu tsarki
    he: אדריכלות דתית
    hi: उपासनास्थल
    hr: vjerska arhitektura
    hu: szakrális építészet
    hy: Տաճարային ճարտարապետություն
    id: Arsitektur sakral
    it: architettura religiosa
    ja: 宗教建築
    la: Architectura sacra
    lt: Religinė architektūra
    mk: свештена архитектура
    nb: religiøs arkitektur
    nl: religieuze architectuur
    nn: religiøs arkitektur
    oc: arquitectura religiosa
    pl: architektura sakralna
    pt: arquitetura religiosa
    ru: религиозная архитектура
    sh: sakralna arhitektura
    sk: Sakrálna architektura
    sl: sakralna arhitektura
    sr: сакрална зграда
    sv: sakral arkitektur
    ta: புனிதக் கட்டிடக்கலை
    tpi: Sios
    tr: dini mimari
    uk: сакральна архітектура
    war: Arkitektura relihiyosa
    zh: 宗教建築
    zh_hans: 宗教建筑
---
# [[Sacral_Architecture]] 

#is_/same_as :: [[WD~Sacred_architecture,47848]] 

## #has_/text_of_/abstract 

> Sacral architecture (also known as sacred architecture or religious architecture) 
> is a religious architectural practice 
> concerned with the design and construction of places of worship or sacred 
> or intentional space, such as churches, mosques, stupas, synagogues, and temples. 
> 
> Many cultures devoted considerable resources 
> to their sacred architecture and places of worship. 
> 
> Religious and sacred spaces are amongst the most impressive 
> and permanent monolithic buildings created by humanity. 
> 
> Conversely, sacred architecture as a locale for meta-intimacy 
> may also be non-monolithic, ephemeral and intensely private, personal and non-public.
>
> Sacred, religious and holy structures often evolved over centuries 
> and were the largest buildings in the world, prior to the modern skyscraper. 
> 
> While the various styles employed in sacred architecture 
> sometimes reflected trends in other structures, these styles also remained 
> unique from the contemporary architecture used in other structures. 
> 
> With the rise of Christianity and Islam, 
> religious buildings increasingly became centres of worship, prayer and meditation.
>
> The Western scholarly discipline of the history of architecture itself 
> closely follows the history of religious architecture from ancient times 
> until the Baroque period, at least. 
> 
> Sacred geometry, iconography, and the use of sophisticated semiotics 
> such as signs, symbols and religious motifs are endemic to sacred architecture.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Sacral%20architecture) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Civil_Engineering/Construction/Architecture/Architectural_Style/Sacral_Architecture|Sacral_Architecture]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Civil_Engineering/Construction/Architecture/Architectural_Style/Sacral_Architecture.public|Sacral_Architecture.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Civil_Engineering/Construction/Architecture/Architectural_Style/Sacral_Architecture.internal|Sacral_Architecture.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Civil_Engineering/Construction/Architecture/Architectural_Style/Sacral_Architecture.protect|Sacral_Architecture.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Civil_Engineering/Construction/Architecture/Architectural_Style/Sacral_Architecture.private|Sacral_Architecture.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Civil_Engineering/Construction/Architecture/Architectural_Style/Sacral_Architecture.personal|Sacral_Architecture.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Civil_Engineering/Construction/Architecture/Architectural_Style/Sacral_Architecture.secret|Sacral_Architecture.secret]] 

