---
aliases:
- Automaatjuhtimine
- automatika
- automatinio valdymo teorija
- automation engineering
- automatique
- automatyka
- Control engineering
- control systems engineering
- Controllo automatico
- Control_Engineering
- engenharia de controle e automação
- Engignerìa dël control
- Enginyeria de control
- Ingeniaria moderationis
- ingeniería de control
- ingineria sistemelor de control
- Inhenyeriya hin kontrol
- innealtóireacht rialúcháin
- Inxeniería de control
- Inxhinieria e kontrollit
- irányítástechnika
- kejuruteraan kawalan
- Khòng-chè kang-têng
- Kontrol ingeniaritza
- Kontrol injeniorarto
- Kontrol mühendisliği
- kontrolno inženirstvo
- regeltechniek
- Regelungstechnik
- reginĝenierio
- reglerteknik
- reguleringsteknikk
- säätötekniikka
- Teknik kendali
- Điều khiển tự động
- аўтаматыка
- Автоматичен контрол
- аутоматика
- теорія автоматичного керування
- теория автоматического управления
- тэорыя аўтаматычнага кіравання
- Хяналт тохируулгын инженер
- הנדסת מכשור ובקרה
- تضبیطی ہندسیات
- مهندسی کنترل
- هندسة التحكم
- کجوروتراءن کاولن
- नियंत्रण प्रौद्योगिकी
- கட்டுப்பாட்டுப் பொறியியல்
- วิศวกรรมระบบอุปกรณ์และการควบคุม
- 制御工学
- 控制工程
- 제어공학
has_id_wikidata: Q4917288
subclass_of:
- '[[_Standards/WikiData/WD~systems_engineering,682496|WD~systems_engineering,682496]]'
- '[[_Standards/WikiData/WD~engineering,11023|WD~engineering,11023]]'
part_of:
- '[[_Standards/WikiData/WD~automation_technology,787422|WD~automation_technology,787422]]'
- '[[_Standards/WikiData/WD~engineering_cybernetics,1777655|WD~engineering_cybernetics,1777655]]'
- '[[_Standards/WikiData/WD~control_engineering,_mechatronics_and_robotics,113104535|WD~control_engineering,_mechatronics_and_robotics,113104535]]'
instance_of:
- '[[_Standards/WikiData/WD~academic_major,4671286|WD~academic_major,4671286]]'
- '[[_Standards/WikiData/WD~branch_of_engineering,12015335|WD~branch_of_engineering,12015335]]'
Wikimedia_outline: '[[_Standards/WikiData/WD~outline_of_control_engineering,7112595|WD~outline_of_control_engineering,7112595]]'
icon: http://commons.wikimedia.org/wiki/Special:FilePath/Control%20Engineering%20Icon.svg
image: http://commons.wikimedia.org/wiki/Special:FilePath/Loaded%20centrifugal%20governor%20%28New%20Catechism%20of%20the%20Steam%20Engine%2C%201904%29.jpg
Stack_Exchange_tag: https://engineering.stackexchange.com/tags/control-engineering
Commons_category: Control engineering
Krugosvet_article: nauka_i_tehnika/tehnologiya_i_promyshlennost/AVTOMATICHESKOE_UPRAVLENIE_I_REGULIROVANIE.html
dv_has_:
  name_:
    ar: هندسة التحكم
    ast: Inxeniería de control
    be: тэорыя аўтаматычнага кіравання
    be_tarask: аўтаматыка
    bg: Автоматичен контрол
    bs: automatika
    ca: Enginyeria de control
    de: Regelungstechnik
    en: control engineering
    eo: reginĝenierio
    es: ingeniería de control
    et: Automaatjuhtimine
    eu: Kontrol ingeniaritza
    fa: مهندسی کنترل
    fi: säätötekniikka
    fr: automatique
    ga: innealtóireacht rialúcháin
    he: הנדסת מכשור ובקרה
    hi: नियंत्रण प्रौद्योगिकी
    hu: irányítástechnika
    id: Teknik kendali
    io: Kontrol injeniorarto
    it: Controllo automatico
    ja: 制御工学
    ko: 제어공학
    la: Ingeniaria moderationis
    lt: automatinio valdymo teorija
    mn: Хяналт тохируулгын инженер
    ms: kejuruteraan kawalan
    ms_arab: کجوروتراءن کاولن
    nan: Khòng-chè kang-têng
    nb: reguleringsteknikk
    nl: regeltechniek
    nn: reguleringsteknikk
    pl: automatyka
    pms: Engignerìa dël control
    pt: engenharia de controle e automação
    ro: ingineria sistemelor de control
    ru: теория автоматического управления
    sh: Automatika
    sl: kontrolno inženirstvo
    sq: Inxhinieria e kontrollit
    sr: аутоматика
    sv: reglerteknik
    ta: கட்டுப்பாட்டுப் பொறியியல்
    th: วิศวกรรมระบบอุปกรณ์และการควบคุม
    tr: Kontrol mühendisliği
    uk: теорія автоматичного керування
    ur: تضبیطی ہندسیات
    vi: Điều khiển tự động
    war: Inhenyeriya hin kontrol
    yue: 控制工程
    zh: 控制工程
    zh_hans: 控制工程
---

# [[Control_Engineering]] 

#is_/same_as :: [[../../WikiData/WD~Control_engineering,4917288|WD~Control_engineering,4917288]] 

## #has_/text_of_/abstract 

> Control engineering, also known as control systems engineering 
> and, in some European countries, automation engineering, 
> is an engineering discipline that deals with control systems, 
> applying control theory to design equipment and systems 
> with desired behaviors in control environments
> 
> . The discipline of controls overlaps and is usually taught along with electrical engineering, 
> chemical engineering and mechanical engineering at many institutions around the world.
>
> The practice uses sensors and detectors to measure the output performance of the process being controlled; these measurements are used to provide corrective feedback helping to achieve the desired performance. Systems designed to perform without requiring human input are called automatic control systems (such as cruise control for regulating the speed of a car). Multi-disciplinary in nature, control systems engineering activities focus on implementation of control systems mainly derived by mathematical modeling of a diverse range of systems.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Control%20engineering) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Technology/Engineering/Control_Engineering|Control_Engineering]] 

### #is_/same_as :: [[/_public/Technology/Engineering/Control_Engineering.public|Control_Engineering.public]] 

### #is_/same_as :: [[/_internal/Technology/Engineering/Control_Engineering.internal|Control_Engineering.internal]] 

### #is_/same_as :: [[/_protect/Technology/Engineering/Control_Engineering.protect|Control_Engineering.protect]] 

### #is_/same_as :: [[/_private/Technology/Engineering/Control_Engineering.private|Control_Engineering.private]] 

### #is_/same_as :: [[/_personal/Technology/Engineering/Control_Engineering.personal|Control_Engineering.personal]] 

### #is_/same_as :: [[/_secret/Technology/Engineering/Control_Engineering.secret|Control_Engineering.secret]] 

