---
aliases:
- Cidu Dan’it
- Dewleta uniter
- dewleta yekpare
- Eenheetsstaat
- eenheidsstaat
- egységes állam
- Eiheitsstaat
- Einheitsstaat
- Einingarríki
- einskapsstat
- enhedsstat
- enhetsstat
- enotna država
- Estado unitario
- Estado unitário
- estat unitari
- Estatu unitario
- estáu unitariu
- gwladwriaeth unedol
- Inusang estado
- jednotny stat
- Nagara kahijian
- negara kesatuan
- nhà nước đơn nguyên
- non foederalis
- państwo unitarne
- Shteti unitar
- Si̱tet muna̱pyia̱
- Stadi unitar
- stat unitar
- Stat uniter
- stato unitari
- Stato unitario
- stato unitàrio
- Steat unnaneagh
- stát aonadach
- unitaarriik
- Unitar davlat
- Unitar dövlət
- Unitar döwlet
- Unitar mámleket
- unitarinė valstybė
- unitario nga estado
- unitarna država
- unitary state
- Unitaryong estado
- unitárny štát
- unitární stát
- unitāra valsts
- unueca ŝtato
- Wnïtarlı memleket
- yhtenäisvaltio
- État unitaire
- üniter devlet
- ενιαίο κράτος
- давлати унитари
- Давлати унитарӣ
- иаку аҳәынҭқарра
- Нэгдмэл улс
- унітарна держава
- унітарная дзяржава
- унитар дәүләт
- унитар кун
- Унитардык мамлекет
- Унитарлы мемлекет
- унитарна держава
- унитарна држава
- Унитарна държава
- унитарное государство
- унитарон паддзахад
- ունիտար պետություն
- מדינת שלטון מואחד
- اونیتار دؤولت
- حکومت متمرکز
- دولة مركزية
- دەوڵەتی یەکگرتوو
- مرکزي حکومت
- نݢارا کساتوان
- وحداني رياست
- وحدانی ریاست
- ۋنىيتارلى مەملەكەت
- ߞߊ߬ߝߏ߫ ߞߎ߲߬ߞߋߟߋ߲
- एकात्मक राज्य
- এককেন্দ্রিক রাষ্ট্র
- ਏਕਾਤਮਕ ਦੇਸ਼
- ஒருமுக அரசு
- ఐక్యప్రభుత్వం
- ಏಕೀಕೃತ ರಾಜ್ಯ
- ඒකීය රාජ්ය
- รัฐเดี่ยว
- ລັດເອກະພາບ
- တစ်ပြည်ထောင်စနစ်
- უნიტარული სახელმწიფო
- უნიტარული სახენწჷფო
- ዘይምርዕዉ ኩነታት
- 单一制
- 単一国家
- 單一制
- 단일 국가
has_id_wikidata: Q179164
instance_of: '[[_Standards/WikiData/WD~form_of_government,1307214|WD~form_of_government,1307214]]'
opposite_of:
- '[[_Standards/WikiData/WD~federal_system,22676603|WD~federal_system,22676603]]'
- '[[_Standards/WikiData/WD~federation,43702|WD~federation,43702]]'
has_part_s_: '[[_Standards/WikiData/WD~unitary_parliamentary_republic,22909549|WD~unitary_parliamentary_republic,22909549]]'
different_from: '[[_Standards/WikiData/WD~centralized_government,25585330|WD~centralized_government,25585330]]'
subclass_of: '[[_Standards/WikiData/WD~state,7275|WD~state,7275]]'
vocalized_name: מְדִינַת שִׁלְטוֹן מָאֳחָד
dv_is_:
  opposite_of: '[[Federation]]'
  same_as:
  - '[[../../../../../WikiData/WD~Unitary_state,179164|WD~Unitary_state,179164]]'
  - '[[/_Standards/Earth/Geography/Place/Administrative_Area/State/Unitary_State|Unitary_State]]'
  - '[[/_public/Earth/Geography/Place/Administrative_Area/State/Unitary_State.public|Unitary_State.public]]'
  - '[[/_internal/Earth/Geography/Place/Administrative_Area/State/Unitary_State.internal|Unitary_State.internal]]'
  - '[[/_protect/Earth/Geography/Place/Administrative_Area/State/Unitary_State.protect|Unitary_State.protect]]'
  - '[[/_private/Earth/Geography/Place/Administrative_Area/State/Unitary_State.private|Unitary_State.private]]'
  - '[[/_personal/Earth/Geography/Place/Administrative_Area/State/Unitary_State.personal|Unitary_State.personal]]'
  - '[[/_secret/Earth/Geography/Place/Administrative_Area/State/Unitary_State.secret|Unitary_State.secret]]'
dv_is_opposite_of: '[[Federation]]'
dv_is_same_as:
- '[[../../../../../WikiData/WD~Unitary_state,179164|WD~Unitary_state,179164]]'
- '[[/_Standards/Earth/Geography/Place/Administrative_Area/State/Unitary_State|Unitary_State]]'
- '[[/_public/Earth/Geography/Place/Administrative_Area/State/Unitary_State.public|Unitary_State.public]]'
- '[[/_internal/Earth/Geography/Place/Administrative_Area/State/Unitary_State.internal|Unitary_State.internal]]'
- '[[/_protect/Earth/Geography/Place/Administrative_Area/State/Unitary_State.protect|Unitary_State.protect]]'
- '[[/_private/Earth/Geography/Place/Administrative_Area/State/Unitary_State.private|Unitary_State.private]]'
- '[[/_personal/Earth/Geography/Place/Administrative_Area/State/Unitary_State.personal|Unitary_State.personal]]'
- '[[/_secret/Earth/Geography/Place/Administrative_Area/State/Unitary_State.secret|Unitary_State.secret]]'
dv_has_:
  name_:
    ab: иаку аҳәынҭқарра
    ar: دولة مركزية
    ast: estáu unitariu
    az: Unitar dövlət
    azb: اونیتار دؤولت
    ba: унитар дәүләт
    be: унітарная дзяржава
    be_tarask: унітарная дзяржава
    bg: Унитарна държава
    bn: এককেন্দ্রিক রাষ্ট্র
    bs: unitarna država
    ca: estat unitari
    ceb: Inusang estado
    ckb: دەوڵەتی یەکگرتوو
    cs: unitární stát
    cy: gwladwriaeth unedol
    da: enhedsstat
    de: Einheitsstaat
    diq: Dewleta uniter
    el: ενιαίο κράτος
    en: unitary state
    en_gb: unitary state
    eo: unueca ŝtato
    es: Estado unitario
    et: unitaarriik
    eu: Estatu unitario
    fa: حکومت متمرکز
    fi: yhtenäisvaltio
    fr: État unitaire
    ga: stát aonadach
    gl: Estado unitario
    gsw: Eiheitsstaat
    gv: Steat unnaneagh
    he: מדינת שלטון מואחד
    hi: एकात्मक राज्य
    hr: unitarna država
    hsb: jednotny stat
    hu: egységes állam
    hy: ունիտար պետություն
    ia: stato unitari
    id: negara kesatuan
    ilo: unitario nga estado
    is: Einingarríki
    it: Stato unitario
    ja: 単一国家
    jv: Negara kesatuan
    ka: უნიტარული სახელმწიფო
    kaa: Unitar mámleket
    kcg: Si̱tet muna̱pyia̱
    kk: Унитарлы мемлекет
    kk_arab: ۋنىيتارلى مەملەكەت
    kk-cn: ۋنىيتارلى مەملەكەت
    kk_cyrl: Унитарлы мемлекет
    kk-kz: Унитарлы мемлекет
    kk_latn: Wnïtarlı memleket
    kk-tr: Wnïtarlı memleket
    kn: ಏಕೀಕೃತ ರಾಜ್ಯ
    ko: 단일 국가
    ku: dewleta yekpare
    ku_latn: dewleta yekpare
    ky: Унитардык мамлекет
    la: non foederalis
    lb: Eenheetsstaat
    lld: Stat uniter
    lo: ລັດເອກະພາບ
    lt: unitarinė valstybė
    lv: unitāra valsts
    mk: унитарна држава
    mn: Нэгдмэл улс
    ms: negara kesatuan
    ms_arab: نݢارا کساتوان
    my: တစ်ပြည်ထောင်စနစ်
    nb: enhetsstat
    ne: एकात्मक राज्य
    nl: eenheidsstaat
    nn: einskapsstat
    nqo: ߞߊ߬ߝߏ߫ ߞߎ߲߬ߞߋߟߋ߲
    oc: Estat unitari
    os: унитарон паддзахад
    pa: ਏਕਾਤਮਕ ਦੇਸ਼
    pl: państwo unitarne
    pnb: وحدانی ریاست
    ps: مرکزي حکومت
    pt: Estado unitário
    pt_br: Estado unitário
    rm: Stadi unitar
    ro: stat unitar
    ru: унитарное государство
    rue: унитарна держава
    sd: وحداني رياست
    sh: Unitarna država
    si: ඒකීය රාජ්ය
    sk: unitárny štát
    sl: enotna država
    sq: Shteti unitar
    sr: унитарна држава
    sr_ec: унитарна држава
    sr_el: unitarna država
    su: Nagara kahijian
    sv: enhetsstat
    ta: ஒருமுக அரசு
    te: ఐక్యప్రభుత్వం
    tg: Давлати унитарӣ
    tg_cyrl: давлати унитари
    th: รัฐเดี่ยว
    ti: ዘይምርዕዉ ኩነታት
    tk: Unitar döwlet
    tl: Unitaryong estado
    tr: üniter devlet
    tt: унитар дәүләт
    tt_cyrl: унитар дәүләт
    udm: унитар кун
    uk: унітарна держава
    ur: وحدانی ریاست
    uz: Unitar davlat
    vec: stato unitàrio
    vi: nhà nước đơn nguyên
    wuu: 单一制
    xmf: უნიტარული სახენწჷფო
    yue: 單一制
    za: Cidu Dan’it
    zh: 单一制
    zh_cn: 单一制
    zh_hans: 单一制
    zh_hant: 單一制
    zh_hk: 單一制
    zh_sg: 单一制
    zh_tw: 單一制
---

# [[Unitary_State]] 

is_opposite_of = `=this.dv_is_opposite_of`
#is_/same_as :: [[_Standards/WikiData/WD~Unitary_state,179164|WD~Unitary_state,179164]] 

## #has_/text_of_/abstract 

> A unitary state is a sovereign state governed as a single entity 
> in which the central government is the supreme authority. 
> 
> The central government may create or abolish administrative divisions 
> (sub-national or sub-state units). 
> 
> Such units exercise only the powers that the central government chooses to delegate. 
> Although political power may be delegated 
> through devolution to regional or local governments by statute, 
> the central government may alter the statute, 
> to override the decisions of devolved governments or expand their powers.
>
> The modern unitary state concept originated in France; in the aftermath of the Hundred Years' War, national feelings that emerged from the war unified France. The war accelerated the process of transforming France from a feudal monarchy to a unitary state. The French then later spread unitary states by conquests, throughout Europe during and after the Napoleonic Wars, and to the world through the vast French colonial empire. Presently, prefects remain an illustration of the French unitary state system, as the representatives of the State in each department, tasked with upholding central government policies.
>
> Unitary states stand in contrast to federations, also known as federal states. A large majority of the UN member countries, 166 out of 193, have a unitary system of government, while significant population and land mass is under some kind of federation.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Unitary%20state) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Earth/Geography/Place/Administrative_Area/State/Unitary_State|Unitary_State]] 

### #is_/same_as :: [[/_public/Earth/Geography/Place/Administrative_Area/State/Unitary_State.public|Unitary_State.public]] 

### #is_/same_as :: [[/_internal/Earth/Geography/Place/Administrative_Area/State/Unitary_State.internal|Unitary_State.internal]] 

### #is_/same_as :: [[/_protect/Earth/Geography/Place/Administrative_Area/State/Unitary_State.protect|Unitary_State.protect]] 

### #is_/same_as :: [[/_private/Earth/Geography/Place/Administrative_Area/State/Unitary_State.private|Unitary_State.private]] 

### #is_/same_as :: [[/_personal/Earth/Geography/Place/Administrative_Area/State/Unitary_State.personal|Unitary_State.personal]] 

### #is_/same_as :: [[/_secret/Earth/Geography/Place/Administrative_Area/State/Unitary_State.secret|Unitary_State.secret]] 

