---
aliases:
- Bábkový štát
- bábállam
- chính phủ bù nhìn
- Civitas neurospastica
- dummy government
- estado monicreque
- Estado-fantoche
- estat titella
- Estatu txotxongilo
- Fazalsteat
- gobierno títere
- Gobiernu títere
- gouvernement fantoche
- Governamento marionette
- Govèrn fantòche
- Gwladwriaeth byped
- Istadu buatone
- Ka-lé chèng-khoân
- kukla devlet
- kukla dövlət
- Leppstjórn
- loutkový stát
- lydrike
- marioneta ŝtato
- Marionetinė valstybė
- marionetna država
- marionetska država
- Marionettenregierung
- marionettstat
- marionešu valsts
- negara boneka
- nukkehallitus
- nukuriik
- papet na estado
- papet nga estado
- papët nga estado
- puppet government
- puppet régime
- puppet state
- Puppet_State
- pápit a estádu
- rząd marionetkowy
- Shteti kukull
- Stad margodenn
- Stat fantocc
- stat marionetă
- Stato fantoccio
- stato piàvoło
- stát ar adhastar
- vasallstat
- Vasalstaat
- vazalstaat
- Vazalstaot
- κράτος-μαριονέτα
- къаранчха кърал
- маріонеткова держава
- Марионетна държава
- марионеточное государство
- марионетска држава
- марыянеткавая дзяржава
- марыянэткавая дзяржава
- Хүүхэлдэй улс
- մարիոնետային պետություն
- ממשלת בובות
- حكومة عميلة
- دولة دمية
- دولت دستنشانده
- دەوڵەتی بووکەڵە
- کٹھ پتلی ریاست
- তাঁবেদার রাষ্ট্র
- ਚਮਚਾਰਾਜ
- රූකඩ රාජ්ය
- รัฐหุ่นเชิด
- ລັດຫຸ່ນເຊີດ
- მარიონეტული სახელმწიფო
- მარიონეტული სახენწჷფო
- 傀儡政府
- 傀儡政权
- 傀儡政権
- 傀儡政權
- 괴뢰정권
has_id_wikidata: Q208164
opposite_of: '[[_Standards/WikiData/WD~great_power,185145|WD~great_power,185145]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~satellite_state,325261|WD~satellite_state,325261]]'
different_from:
- '[[_Standards/WikiData/WD~client_state,1151405|WD~client_state,1151405]]'
- '[[_Standards/WikiData/WD~vassal_state,1371288|WD~vassal_state,1371288]]'
subclass_of: '[[_Standards/WikiData/WD~client_state,1151405|WD~client_state,1151405]]'
facet_of:
- '[[_Standards/WikiData/WD~international_law,4394526|WD~international_law,4394526]]'
- '[[_Standards/WikiData/WD~political_science,36442|WD~political_science,36442]]'
position_held_by_head_of_the_organization: '[[_Standards/WikiData/WD~Puppet_ruler,7260719|WD~Puppet_ruler,7260719]]'
Commons_category: Puppet states
dv_is_:
  same_as:
  - '[[../../../../../WikiData/WD~Puppet_state,208164|WD~Puppet_state,208164]]'
  - '[[/_Standards/Earth/Geography/Place/Administrative_Area/State/Puppet_State|Puppet_State]]'
  - '[[/_public/Earth/Geography/Place/Administrative_Area/State/Puppet_State.public|Puppet_State.public]]'
  - '[[/_internal/Earth/Geography/Place/Administrative_Area/State/Puppet_State.internal|Puppet_State.internal]]'
  - '[[/_protect/Earth/Geography/Place/Administrative_Area/State/Puppet_State.protect|Puppet_State.protect]]'
  - '[[/_private/Earth/Geography/Place/Administrative_Area/State/Puppet_State.private|Puppet_State.private]]'
  - '[[/_personal/Earth/Geography/Place/Administrative_Area/State/Puppet_State.personal|Puppet_State.personal]]'
  - '[[/_secret/Earth/Geography/Place/Administrative_Area/State/Puppet_State.secret|Puppet_State.secret]]'
dv_is_same_as:
- '[[../../../../../WikiData/WD~Puppet_state,208164|WD~Puppet_state,208164]]'
- '[[/_Standards/Earth/Geography/Place/Administrative_Area/State/Puppet_State|Puppet_State]]'
- '[[/_public/Earth/Geography/Place/Administrative_Area/State/Puppet_State.public|Puppet_State.public]]'
- '[[/_internal/Earth/Geography/Place/Administrative_Area/State/Puppet_State.internal|Puppet_State.internal]]'
- '[[/_protect/Earth/Geography/Place/Administrative_Area/State/Puppet_State.protect|Puppet_State.protect]]'
- '[[/_private/Earth/Geography/Place/Administrative_Area/State/Puppet_State.private|Puppet_State.private]]'
- '[[/_personal/Earth/Geography/Place/Administrative_Area/State/Puppet_State.personal|Puppet_State.personal]]'
- '[[/_secret/Earth/Geography/Place/Administrative_Area/State/Puppet_State.secret|Puppet_State.secret]]'
dv_has_:
  name_:
    af: Vasalstaat
    ar: حكومة عميلة
    arz: دولة دمية
    ast: Gobiernu títere
    az: kukla dövlət
    be: марыянеткавая дзяржава
    be_tarask: марыянэткавая дзяржава
    bg: Марионетна държава
    bn: তাঁবেদার রাষ্ট্র
    br: Stad margodenn
    bs: marionetska država
    ca: estat titella
    ceb: papet nga estado
    ckb: دەوڵەتی بووکەڵە
    cs: loutkový stát
    cy: Gwladwriaeth byped
    de: Marionettenregierung
    el: κράτος-μαριονέτα
    en: puppet state
    en-us: puppet state
    eo: marioneta ŝtato
    es: gobierno títere
    et: nukuriik
    eu: Estatu txotxongilo
    fa: دولت دستنشانده
    fi: nukkehallitus
    fr: gouvernement fantoche
    fy: Fazalsteat
    ga: stát ar adhastar
    gl: estado monicreque
    he: ממשלת בובות
    hr: Marionetska država
    hu: bábállam
    hy: մարիոնետային պետություն
    ia: Governamento marionette
    id: negara boneka
    is: Leppstjórn
    it: Stato fantoccio
    ja: 傀儡政権
    ka: მარიონეტული სახელმწიფო
    ko: 괴뢰정권
    krc: къаранчха кърал
    krj: papët nga estado
    la: Civitas neurospastica
    li: Vazalstaot
    lmo: Stat fantocc
    lo: ລັດຫຸ່ນເຊີດ
    lt: Marionetinė valstybė
    lv: marionešu valsts
    mk: марионетска држава
    mn: Хүүхэлдэй улс
    ms: Negara boneka
    nan: Ka-lé chèng-khoân
    nb: lydrike
    nl: vazalstaat
    nn: marionettstat
    oc: Govèrn fantòche
    pa: ਚਮਚਾਰਾਜ
    pam: pápit a estádu
    pl: rząd marionetkowy
    pnb: کٹھ پتلی ریاست
    pt: Estado-fantoche
    pt_br: Estado-fantoche
    ro: stat marionetă
    ru: марионеточное государство
    sc: Istadu buatone
    sh: marionetska država
    si: රූකඩ රාජ්ය
    sk: Bábkový štát
    sl: marionetna država
    sq: Shteti kukull
    sr: марионетска држава
    sr_ec: марионетска држава
    sr_el: marionetska država
    sv: vasallstat
    th: รัฐหุ่นเชิด
    tl: papet na estado
    tr: kukla devlet
    uk: маріонеткова держава
    ur: کٹھ پتلی ریاست
    vec: stato piàvoło
    vi: chính phủ bù nhìn
    wuu: 傀儡政权
    xmf: მარიონეტული სახენწჷფო
    yue: 傀儡政府
    zh: 傀儡政權
    zh_cn: 傀儡政权
    zh_hans: 傀儡政权
    zh_hant: 傀儡政權
    zh_hk: 傀儡政權
    zh_sg: 傀儡政权
    zh_tw: 傀儡政權
---

# [[Puppet_State]] 

#is_/same_as :: [[../../../../../WikiData/WD~Puppet_state,208164|WD~Puppet_state,208164]]  

## #has_/text_of_/abstract 

> A puppet state, puppet régime, puppet government or dummy government 
> is a state that is de jure independent but de facto 
> completely dependent upon an outside power and subject to its orders. 
> 
> Puppet states have nominal sovereignty, 
> except that a foreign power effectively exercises control 
> through economic or military support. 
> 
> By leaving a local government in existence 
> the outside power evades all responsibility, 
> while at the same time successfully paralysing the local government they tolerate.
>
> Puppet states differ from allies, who choose their actions of their own initiative 
> or in accordance with treaties they have voluntarily entered. 
> 
> Puppet states are forced into legally endorsing actions already taken by a foreign power.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Puppet%20state) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Earth/Geography/Place/Administrative_Area/State/Puppet_State|Puppet_State]] 

### #is_/same_as :: [[/_public/Earth/Geography/Place/Administrative_Area/State/Puppet_State.public|Puppet_State.public]] 

### #is_/same_as :: [[/_internal/Earth/Geography/Place/Administrative_Area/State/Puppet_State.internal|Puppet_State.internal]] 

### #is_/same_as :: [[/_protect/Earth/Geography/Place/Administrative_Area/State/Puppet_State.protect|Puppet_State.protect]] 

### #is_/same_as :: [[/_private/Earth/Geography/Place/Administrative_Area/State/Puppet_State.private|Puppet_State.private]] 

### #is_/same_as :: [[/_personal/Earth/Geography/Place/Administrative_Area/State/Puppet_State.personal|Puppet_State.personal]] 

### #is_/same_as :: [[/_secret/Earth/Geography/Place/Administrative_Area/State/Puppet_State.secret|Puppet_State.secret]] 

