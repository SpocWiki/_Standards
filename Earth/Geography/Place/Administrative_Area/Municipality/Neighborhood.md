---
aliases:
- Alebùghu aŋ ukɔ̄n
- apkaime
- asum
- auzo
- bairro
- barri
- barrio
- barriu
- bisindario
- Buorren
- buurt of wijk
- bykvarter
- cartier
- comharsanacht
- cuartiero
- cymdogaeth
- część miejscowości
- cámko
- dah gbilli
- dzǎl
- eelue
- gaje na dotò
- hverfi
- ipáá
- kaarrubaan
- Kaaʼtiə́
- Kalitié
- karter
- kartiye
- kartié
- Kasilinganan
- kejiranan
- khu phố
- kvartalo
- kwartier
- kwata
- kɔɔ́m ɔlɔpɔnɛ́ɛ
- lagje
- lepfó
- lepfú áláʼ
- li mo dù sho e mvəm la' kii wò
- li'shu
- libok
- lingkungan
- láʼ
- Lîn-lí
- mahalle
- mikrorajonas
- mkpe njúù
- Mtaa
- měšćanska štwórć
- məhəllə
- naapurusto
- Naverschop
- neighborhood
- neighbourhood
- neighbourhuid
- ngɔ́dɔ́
- ntèʼ
- nu-gara
- Nēahġebūrhōd
- nɛkɛ́tɛ
- område
- quarter
- quarteri
- quartier
- quartiere
- quartièr
- Quarté
- ritɔp íní
- saàŋ
- semt
- soseska
- Stadtviertel
- Stodviadl
- strøk
- stääddial
- susedstvo
- susjedstvo
- tax
- tǎŋla'
- Veedel
- vicinitate
- Vico
- vicus urbanus
- Vwazen
- városnegyed
- woonbuurt
- ákələ ndɔw
- časť obce
- čtvrť
- ɛfágáfágá
- συνοικία
- квартал
- мәхәллә
- ноҳияи шаҳр
- отдельная часть города
- район міста
- соседство
- суседство
- сых
- частка горада
- քաղաքի շրջան
- געגנט
- שכונה
- حاره
- حي
- محله
- محلہ
- محلہٕ
- پاڙو
- ګاونډيتوب
- گوانڈھ
- گەڕەک
- ߞߌ߲߬ߘߊ
- मुहल्ला
- मोहल्ला
- वस्तीचा प्रदेश
- পাড়া
- ನೆರೆಹೊರೆ
- ย่าน
- უბანი
- ᱴᱚᱞᱟ
- ”ripwik"
- 分區
- 近邻社区
- 近鄰社區
- 邻舍
- 都市の地区
- 鄰舍
- 동네
has_id_wikidata: Q123705
said_to_be_the_same_as:
- '[[_Standards/WikiData/WD~microdistrict,604435]]'
- '[[_Standards/WikiData/WD~bairro,16837104]]'
different_from:
- '[[_Standards/WikiData/WD~city_block,1348006]]'
- '[[_Standards/WikiData/WD~quarter,2983893]]'
- '[[_Standards/WikiData/WD~city_district,4286337]]'
- '[[_Standards/WikiData/WD~místní_část,18114025]]'
- '[[_Standards/WikiData/WD~municipal_part_of_the_Czech_Republic,61089180]]'
- '[[_Standards/WikiData/WD~part_of_municipality_in_Czechoslovakia,108000498]]'
described_by_source:
- "[[_Standards/WikiData/WD~Otto's_encyclopedia,2041543]]"
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Lean_Logic,101314624]]'
- '[[_Standards/WikiData/WD~Green_Map_Icons,103892210]]'
subclass_of:
- '[[_Standards/WikiData/WD~human_geographic_territorial_entity,15642541]]'
- '[[_Standards/WikiData/WD~section_of_populated_place,48907157]]'
part_of:
- '[[_Standards/WikiData/WD~municipio,115740682]]'
- '[[_Standards/WikiData/WD~city,515]]'
- '[[_Standards/WikiData/WD~municipality,15284]]'
properties_for_this_type:
- location
- postal code
- Commons category
- coordinate location
- Walk Score ID
- country
- image
- located in the administrative territorial entity
facet_of: '[[_Standards/WikiData/WD~administrative_territorial_entity,56061]]'
OmegaWiki_Defined_Meaning: 479536
DPLA_subject_term: Neighborhoods
Commons_category: Neighborhoods
U_S_National_Archives_Identifier: 10642227
Wolfram_Language_entity_type: Neighborhood
spoken_text_audio: http://commons.wikimedia.org/wiki/Special:FilePath/Nl-Buurt-article.ogg
image: http://commons.wikimedia.org/wiki/Special:FilePath/Shimokitazawa%20Station%20one%20week%20before%20track%20relocation%202013-03-16%20R0012271%20%288573277185%29.jpg
described_at_URL: https://www.whosonfirst.org/docs/placetypes/#neighbourhood
OpenStreetMap_tag: place=neighbourhood
dv_is_:
  same_as:
  - '[[../../../../../WikiData/WD~Neighborhood,123705]]'
  - '[[/_Standards/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood|Neighborhood]]'
  - '[[/_public/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.public|Neighborhood.public]]'
  - '[[/_internal/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.internal|Neighborhood.internal]]'
  - '[[/_protect/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.protect|Neighborhood.protect]]'
  - '[[/_private/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.private|Neighborhood.private]]'
  - '[[/_personal/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.personal|Neighborhood.personal]]'
  - '[[/_secret/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.secret|Neighborhood.secret]]'
dv_is_same_as:
- '[[../../../../../WikiData/WD~Neighborhood,123705]]'
- '[[/_Standards/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood|Neighborhood]]'
- '[[/_public/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.public|Neighborhood.public]]'
- '[[/_internal/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.internal|Neighborhood.internal]]'
- '[[/_protect/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.protect|Neighborhood.protect]]'
- '[[/_private/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.private|Neighborhood.private]]'
- '[[/_personal/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.personal|Neighborhood.personal]]'
- '[[/_secret/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.secret|Neighborhood.secret]]'
dv_has_:
  name_:
    af: woonbuurt
    agq: Alebùghu aŋ ukɔ̄n
    an: Vico
    ang: Nēahġebūrhōd
    ar: حي
    arz: حاره
    ast: barriu
    az: məhəllə
    azb: محله
    ba: мәхәллә
    bag: ipáá
    bar: Stodviadl
    bas: libok
    bax: li'shu
    bbj: láʼ
    be: частка горада
    be-tarask: квартал
    bg: квартал
    bho: मोहल्ला
    bkc: gaje na dotò
    bkh: ritɔp íní
    bkm: ntèʼ
    bn: পাড়া
    br: karter
    bs: susjedstvo
    byv: tǎŋla'
    ca: barri
    ceb: Kasilinganan
    ckb: گەڕەک
    cs: čtvrť
    cy: cymdogaeth
    da: bykvarter
    de: Stadtviertel
    diq: semt
    dua: kwata
    el: συνοικία
    en: neighborhood
    en-ca: neighbourhood
    en-gb: neighbourhood
    en-us: neighborhood
    eo: kvartalo
    es: barrio
    et: asum
    eto: ngɔ́dɔ́
    etu: mkpe njúù
    eu: auzo
    ewo: dzǎl
    ext: barriu
    fa: محله
    fi: naapurusto
    fmp: cámko
    fr: quartier
    frr: stääddial
    fy: Buorren
    ga: comharsanacht
    gan: 鄰舍
    gl: barrio
    gsw: Stadtviertel
    gya: nu-gara
    he: שכונה
    hi: मुहल्ला
    hr: susjedstvo
    hsb: měšćanska štwórć
    ht: Vwazen
    hu: városnegyed
    hy: քաղաքի շրջան
    ia: vicinitate
    id: lingkungan
    ilo: kaarrubaan
    is: hverfi
    isu: ákələ ndɔw
    it: quartiere
    ja: 都市の地区
    ka: უბანი
    ker: kartiye
    kn: ನೆರೆಹೊರೆ
    ko: 동네
    ks: محلہٕ
    ksh: Veedel
    ku: tax
    ku-latn: tax
    la: vicus urbanus
    lb: Quartier
    lem: eelue
    lmo: quarter
    ln: Kalitié
    lns: li mo dù sho e mvəm la' kii wò
    lt: mikrorajonas
    lv: apkaime
    mcp: ”ripwik"
    mk: соседство
    mr: वस्तीचा प्रदेश
    ms: kejiranan
    mt: kwartier
    mua: dah gbilli
    nan: Lîn-lí
    nb: strøk
    nds: Naverschop
    nl: buurt of wijk
    nmg: kartié
    nn: strøk
    nnh: lepfó
    nqo: ߞߌ߲߬ߘߊ
    oc: quartièr
    os: сых
    pap: bisindario
    pl: część miejscowości
    pms: Quarté
    pnb: گوانڈھ
    ps: ګاونډيتوب
    pt: bairro
    pt-br: bairro
    ro: cartier
    ru: отдельная часть города
    sat: ᱴᱚᱞᱟ
    scn: quarteri
    sco: neighbourhuid
    sd: پاڙو
    sh: susjedstvo
    sk: časť obce
    sl: soseska
    sq: lagje
    sr: суседство
    sr-ec: суседство
    sr-el: susedstvo
    sv: område
    sw: Mtaa
    tg: ноҳияи шаҳр
    th: ย่าน
    tr: mahalle
    tt: мәхәллә
    tvu: nɛkɛ́tɛ
    uk: район міста
    ur: محلہ
    vec: cuartiero
    vi: khu phố
    vut: saàŋ
    wes: quarter
    wuu: 邻舍
    yas: ɛfágáfágá
    yat: Kaaʼtiə́
    yav: kɔɔ́m ɔlɔpɔnɛ́ɛ
    ybb: lepfú áláʼ
    yi: געגנט
    yue: 分區
    zh: 近鄰社區
    zh-cn: 近邻社区
    zh-hans: 近邻社区
    zh-hant: 近鄰社區
    zh-hk: 近鄰社區
    zh-sg: 近邻社区
    zh-tw: 近鄰社區
---
# [[Neighborhood]] 

#is_/same_as :: [[WD~Neighborhood,123705]] 

## #has_/text_of_/abstract 

> A neighbourhood (Commonwealth English) or **Neighborhood** (American English) 
> is a geographically localized community within a larger town, city, suburb or rural area, 
> sometimes consisting of a single street and the buildings lining it. 
> 
> Neighbourhoods are often social communities 
> with considerable face-to-face interaction among members. 
> 
> Researchers have not agreed on an exact definition, 
> but the following may serve as a starting point: 
> "Neighbourhood is generally defined spatially as a specific geographic area 
> and functionally as a set of social networks. 
> Neighbourhoods, then, are the spatial units in which face-to-face social interactions occur—the personal settings and situations where residents seek to realise common values, socialise youth, and maintain effective social control."
>
> [Wikipedia](https://en.wikipedia.org/wiki/Neighbourhood) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood|Neighborhood]] 

### #is_/same_as :: [[/_public/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.public|Neighborhood.public]] 

### #is_/same_as :: [[/_internal/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.internal|Neighborhood.internal]] 

### #is_/same_as :: [[/_protect/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.protect|Neighborhood.protect]] 

### #is_/same_as :: [[/_private/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.private|Neighborhood.private]] 

### #is_/same_as :: [[/_personal/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.personal|Neighborhood.personal]] 

### #is_/same_as :: [[/_secret/Earth/Geography/Place/Administrative_Area/Municipality/Neighborhood.secret|Neighborhood.secret]] 

