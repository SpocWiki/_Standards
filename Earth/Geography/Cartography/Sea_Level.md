---
aliases:
- Aras laut
- asta deryayê
- Dengiz sathi
- deniz seviyesi
- Dəniz səviyyəsi
- Haavon'ny ranomasina
- havnivå
- havoverflade
- havsnivå
- Hladina mora
- hladina moře
- Hái-pêng-bīn
- itsas maila
- Jooga bada
- jūras līmenis
- jūros lygis
- Katupngan ha dagat
- leibhéal na farraige
- liveddu dû mari
- livel d'a mar
- Livel dël mar
- livello del mare
- livello do mâ
- Livellu de su mare
- liveło del mar
- Marala nivelo
- Maris aequor
- marnivelo
- Meeresspiegel
- merenpinnan taso
- Meretase
- morska gladina
- mực nước biển
- Nang dhuwur laut
- niveau de la mer
- Nivel dal mâr
- nivel de la mar
- nivel del mar
- Nivel medio do mar
- Niveli i detit
- Nivell del mar
- nivell mitjà del mar
- Nivello del mar
- nivelul mării
- nivo mora
- Nivèl de la mar
- Ntshī amaane
- nível médio do mar
- Oke oke osimiri
- pantar ti baybay
- permukaan laut
- poziom morza
- Razina mora
- sea level
- Seenivo
- Seevlak
- Siapeegel
- Sjávarmál
- tengerszint
- Usawa bahari wastani
- zeeniveau
- Zeêniveau
- Zieëniveau
- Στάθμη θαλάσσης
- Абсоліоттік биіктік
- Далайн түвшин
- денджызы æмвæз
- Деңиз деңгээли
- диңгез дәрәҗәсе
- Диңгеҙ кимәле
- морско ниво
- Морско равнище
- ниво мора
- рівень моря
- Сатҳи баҳр
- Тинӗс шайӗ
- узровень мора
- уровень моря
- ХӀордан локхалла
- բարձրություն ծովի մակարդակից
- גובה פני הים
- ئاستا دەریایێ
- ئاستی دەریا
- دنیز سویهسی
- سطح آبهای آزاد
- سطح سمندر
- سمندری پدھر
- مستوى سطح البحر
- समुद्र तल
- समुद्रसपाटी
- সমুদ্র সমতল
- கடல் மட்டம்
- సముద్రమట్టం
- സമുദ്രനിരപ്പ്
- ระดับน้ำทะเล
- ပင်လယ်ရေမျက်နှာပြင်
- ზუღაშ დონე
- ზღვის დონე
- 海平面
- 海面
- 해수면
has_id_wikidata: Q125465
country: '[[_Standards/WikiData/WD~internationality,1072012]]'
subclass_of:
- '[[_Standards/WikiData/WD~water_level,1929501]]'
- '[[_Standards/WikiData/WD~vertical_datum,9364781]]'
instance_of: '[[_Standards/WikiData/WD~vertical_datum,9364781]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/De-Meeresspiegel.ogg
image: http://commons.wikimedia.org/wiki/Special:FilePath/Israel%20Sea%20Level%20BW%201.JPG
Commons_category: Sea level
dv_is_:
  same_as:
  - '[[../../../WikiData/WD~Sea_level,125465]]'
  - '[[/_Standards/Earth/Geography/Cartography/Sea_Level|Sea_Level]]'
  - '[[/_public/Earth/Geography/Cartography/Sea_Level.public|Sea_Level.public]]'
  - '[[/_internal/Earth/Geography/Cartography/Sea_Level.internal|Sea_Level.internal]]'
  - '[[/_protect/Earth/Geography/Cartography/Sea_Level.protect|Sea_Level.protect]]'
  - '[[/_private/Earth/Geography/Cartography/Sea_Level.private|Sea_Level.private]]'
  - '[[/_personal/Earth/Geography/Cartography/Sea_Level.personal|Sea_Level.personal]]'
  - '[[/_secret/Earth/Geography/Cartography/Sea_Level.secret|Sea_Level.secret]]'
dv_is_same_as:
- '[[../../../WikiData/WD~Sea_level,125465]]'
- '[[/_Standards/Earth/Geography/Cartography/Sea_Level|Sea_Level]]'
- '[[/_public/Earth/Geography/Cartography/Sea_Level.public|Sea_Level.public]]'
- '[[/_internal/Earth/Geography/Cartography/Sea_Level.internal|Sea_Level.internal]]'
- '[[/_protect/Earth/Geography/Cartography/Sea_Level.protect|Sea_Level.protect]]'
- '[[/_private/Earth/Geography/Cartography/Sea_Level.private|Sea_Level.private]]'
- '[[/_personal/Earth/Geography/Cartography/Sea_Level.personal|Sea_Level.personal]]'
- '[[/_secret/Earth/Geography/Cartography/Sea_Level.secret|Sea_Level.secret]]'
dv_has_:
  name_:
    af: Seevlak
    an: livel d'a mar
    ar: مستوى سطح البحر
    ast: nivel de la mar
    az: Dəniz səviyyəsi
    azb: دنیز سویهسی
    ba: Диңгеҙ кимәле
    be: узровень мора
    be_tarask: узровень мора
    bg: Морско равнище
    bho: समुद्र तल
    bn: সমুদ্র সমতল
    bs: nivo mora
    ca: nivell mitjà del mar
    ce: ХӀордан локхалла
    ckb: ئاستی دەریا
    cs: hladina moře
    cv: Тинӗс шайӗ
    da: havoverflade
    de: Meeresspiegel
    el: Στάθμη θαλάσσης
    en: sea level
    eo: marnivelo
    es: nivel del mar
    et: Meretase
    eu: itsas maila
    fa: سطح آبهای آزاد
    fi: merenpinnan taso
    fr: niveau de la mer
    frr: Siapeegel
    fur: Nivel dal mâr
    fy: Seenivo
    ga: leibhéal na farraige
    gl: Nivel medio do mar
    ha: Sea level
    he: גובה פני הים
    hi: समुद्र तल
    hr: Razina mora
    hu: tengerszint
    hy: բարձրություն ծովի մակարդակից
    ia: Nivello del mar
    id: permukaan laut
    ig: Oke oke osimiri
    ilo: pantar ti baybay
    io: Marala nivelo
    is: Sjávarmál
    it: livello del mare
    ja: 海面
    ka: ზღვის დონე
    kk: Абсоліоттік биіктік
    ko: 해수면
    ku: asta deryayê
    ku_arab: ئاستا دەریایێ
    ku_latn: asta deryayê
    ky: Деңиз деңгээли
    la: Maris aequor
    li: Zieëniveau
    lij: livello do mâ
    lmo: Nivell del mar
    lt: jūros lygis
    lv: jūras līmenis
    lzh: 海平面
    map_bms: Nang dhuwur laut
    mg: Haavon'ny ranomasina
    mk: морско ниво
    ml: സമുദ്രനിരപ്പ്
    mn: Далайн түвшин
    mr: समुद्रसपाटी
    ms: Aras laut
    my: ပင်လယ်ရေမျက်နှာပြင်
    nan: Hái-pêng-bīn
    nb: havnivå
    nl: zeeniveau
    nn: havnivå
    oc: Nivèl de la mar
    os: денджызы æмвæз
    pl: poziom morza
    pms: Livel dël mar
    pnb: سمندری پدھر
    pt: nível médio do mar
    ro: nivelul mării
    ru: уровень моря
    rue: Уровень моря
    sc: Livellu de su mare
    scn: liveddu dû mari
    sco: sea level
    sh: Razina mora
    sk: Hladina mora
    sl: morska gladina
    so: Jooga bada
    sq: Niveli i detit
    sr: ниво мора
    sv: havsnivå
    sw: Usawa bahari wastani
    ta: கடல் மட்டம்
    te: సముద్రమట్టం
    tg: Сатҳи баҳр
    th: ระดับน้ำทะเล
    tr: deniz seviyesi
    tt: диңгез дәрәҗәсе
    tt_cyrl: диңгез дәрәҗәсе
    uk: рівень моря
    ur: سطح سمندر
    uz: Dengiz sathi
    vec: liveło del mar
    vi: mực nước biển
    war: Katupngan ha dagat
    wuu: 海平面
    xmf: ზუღაშ დონე
    ybb: Ntshī amaane
    yue: 海平面
    zea: Zeêniveau
    zh: 海平面
    zh_cn: 海平面
    zh_hans: 海平面
    zh_hant: 海平面
    zh_hk: 海平面
    zh_sg: 海平面
    zh_tw: 海平面
---

# [[Sea_Level]] 

#is_/same_as :: [[WD~Sea_level,125465]] 

## #has_/text_of_/abstract 

> Mean sea level (MSL, often shortened to sea level) is an average surface level 
> of one or more among Earth's coastal bodies of water 
> from which heights such as elevation may be measured. 
> 
> The global MSL is a type of vertical datum – a standardised geodetic datum – 
> that is used, for example, as a chart datum in cartography and marine navigation, 
> or, in aviation, as the standard sea level at which atmospheric pressure is measured 
> to calibrate altitude and, consequently, aircraft flight levels. 
> 
> A common and relatively straightforward mean sea-level standard is instead 
> a long-term average of tide gauge readings at a particular reference location.
>
> The term above sea level generally refers to the height above mean sea level (AMSL). 
> The term APSL means above present sea level, 
> comparing sea levels in the past with the level today.
>
> Earth's radius at sea level is 6,378.137 km (3,963.191 mi) at the equator. 
> It is 6,356.752 km (3,949.903 mi) at the poles and 6,371.001 km (3,958.756 mi) on average. 
> 
> This flattened spheroid, combined with local gravity anomalies, defines the geoid of the Earth, 
> which approximates the local mean sea level for locations in the open ocean. 
> 
> The geoid includes a significant depression in the Indian Ocean, 
> whose surface dips as much as 106 m (348 ft) below the global mean sea level 
> (excluding minor effects such as tides and currents).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Sea%20level) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Earth/Geography/Cartography/Sea_Level|Sea_Level]] 

### #is_/same_as :: [[/_public/Earth/Geography/Cartography/Sea_Level.public|Sea_Level.public]] 

### #is_/same_as :: [[/_internal/Earth/Geography/Cartography/Sea_Level.internal|Sea_Level.internal]] 

### #is_/same_as :: [[/_protect/Earth/Geography/Cartography/Sea_Level.protect|Sea_Level.protect]] 

### #is_/same_as :: [[/_private/Earth/Geography/Cartography/Sea_Level.private|Sea_Level.private]] 

### #is_/same_as :: [[/_personal/Earth/Geography/Cartography/Sea_Level.personal|Sea_Level.personal]] 

### #is_/same_as :: [[/_secret/Earth/Geography/Cartography/Sea_Level.secret|Sea_Level.secret]] 

