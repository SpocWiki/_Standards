---
aliases:
- a Föld szerkezete
- budowa Ziemi
- Cấu trúc Trái Đất
- Eerdopbo
- Estructura de la Tèrra
- Estructura interna de la Terra
- Estructura interna de la Tierra
- Estrutura interna da Terra
- innerer Aufbau der Erde
- internal structure of Earth
- Intiiria chokcha a di hOert
- jordens struktur
- Lurraren egitura
- Maan sisäinen rakenne
- Muundo wa dunia
- Ndërtimi i brendshëm i Tokës
- Opbau vun der Äerd
- opbouw van de Aarde
- Stavba Zeme
- stratifikace Země
- straturile Pământului
- Striktir entern di Latè
- struchtúr an Domhain
- Structura Telluris
- structure interne de la Terre
- struktur Bumi
- Struktura Zemlje
- strukturo de Tero
- struktuur van die Aarde
- struttura interna della Terra
- strwythur y Ddaear
- Yerin quruluşu
- Yerküre
- Zemes uzbūve
- Zəmini soxtemon
- Žemės sandara
- Будова Землі
- будова Зямлі
- Вътрешен строеж на Земята
- Грађа Земље
- Дэлхийн цөм
- Ер төҙөлөшө
- Жер құрылымы
- Состав на Земјата
- структура Земли
- Երկրաբանական կտրվածք
- מבנה השכבות של כדור הארץ
- بنية الأرض
- د ځمکې جوړښت
- زمین درست بین
- ساختار زمین
- ڌرتيءَ جو اندريون حصو
- یئر چکیردگی
- पृथ्वी की आतंरिक संरचना
- पृथ्वी के अंदरूनी बनावट
- पृथ्वीच्या अंतरंगाची रचना
- পৃথিবীর গঠন
- পৃথিৱীৰ আভ্যন্তৰীণ গঠন
- ਧਰਤੀ ਦਾ ਢਾਂਚਾ
- புவியின் கட்டமைப்பு
- భూమి నిర్మాణం
- അന്തർഭൗമഘടന
- පෘථිවියේ අභ්යන්තර ව්යුහය
- โครงสร้างของโลก
- დედამიწის სტრუქტურა
- 地球の構造
- 地球嘅結構
- 地球构造
- 地球構造
- 지구의 구조
has_id_wikidata: Q1664027
studied_by:
- '[[_Standards/WikiData/WD~seismology,83371]]'
- '[[_Standards/WikiData/WD~geophysics,46255]]'
different_from: '[[_Standards/WikiData/WD~earth_structure,5327174]]'
instance_of: '[[_Standards/WikiData/WD~structure,6671777]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Earthsinterior,11207190]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._3,124737616]]'
facet_of: '[[_Standards/WikiData/WD~Earth,2]]'
Commons_category: Structure of the Earth
Krugosvet_article_archived_: Earth_sciences/geologiya/ZEMLI_STROENIE.html
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Structure_of_Earth,1664027|WD~Structure_of_Earth,1664027]]'
  - '[[/_Standards/Earth/Climate/Structure_of_Earth|Structure_of_Earth]]'
  - '[[/_public/Earth/Climate/Structure_of_Earth.public|Structure_of_Earth.public]]'
  - '[[/_internal/Earth/Climate/Structure_of_Earth.internal|Structure_of_Earth.internal]]'
  - '[[/_protect/Earth/Climate/Structure_of_Earth.protect|Structure_of_Earth.protect]]'
  - '[[/_private/Earth/Climate/Structure_of_Earth.private|Structure_of_Earth.private]]'
  - '[[/_personal/Earth/Climate/Structure_of_Earth.personal|Structure_of_Earth.personal]]'
  - '[[/_secret/Earth/Climate/Structure_of_Earth.secret|Structure_of_Earth.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Structure_of_Earth,1664027|WD~Structure_of_Earth,1664027]]'
- '[[/_Standards/Earth/Climate/Structure_of_Earth|Structure_of_Earth]]'
- '[[/_public/Earth/Climate/Structure_of_Earth.public|Structure_of_Earth.public]]'
- '[[/_internal/Earth/Climate/Structure_of_Earth.internal|Structure_of_Earth.internal]]'
- '[[/_protect/Earth/Climate/Structure_of_Earth.protect|Structure_of_Earth.protect]]'
- '[[/_private/Earth/Climate/Structure_of_Earth.private|Structure_of_Earth.private]]'
- '[[/_personal/Earth/Climate/Structure_of_Earth.personal|Structure_of_Earth.personal]]'
- '[[/_secret/Earth/Climate/Structure_of_Earth.secret|Structure_of_Earth.secret]]'
dv_has_:
  name_:
    af: struktuur van die Aarde
    ar: بنية الأرض
    as: পৃথিৱীৰ আভ্যন্তৰীণ গঠন
    ast: Estructura interna de la Tierra
    az: Yerin quruluşu
    azb: یئر چکیردگی
    ba: Ер төҙөлөшө
    be: будова Зямлі
    be_tarask: будова Зямлі
    bg: Вътрешен строеж на Земята
    bho: पृथ्वी के अंदरूनी बनावट
    bn: পৃথিবীর গঠন
    ca: Estructura interna de la Terra
    cs: stratifikace Země
    cy: strwythur y Ddaear
    de: innerer Aufbau der Erde
    en: internal structure of Earth
    eo: strukturo de Tero
    es: estructura interna de la Tierra
    eu: Lurraren egitura
    fa: ساختار زمین
    fi: Maan sisäinen rakenne
    fr: structure interne de la Terre
    ga: struchtúr an Domhain
    gcr: Striktir entern di Latè
    gl: Estrutura interna da Terra
    he: מבנה השכבות של כדור הארץ
    hi: पृथ्वी की आतंरिक संरचना
    hr: Struktura Zemlje
    hu: a Föld szerkezete
    hy: Երկրաբանական կտրվածք
    id: struktur Bumi
    it: struttura interna della Terra
    ja: 地球の構造
    jam: Intiiria chokcha a di hOert
    ka: დედამიწის სტრუქტურა
    kk: Жер құрылымы
    ko: 지구의 구조
    la: Structura Telluris
    lb: Opbau vun der Äerd
    lt: Žemės sandara
    lv: Zemes uzbūve
    mk: Состав на Земјата
    ml: അന്തർഭൗമഘടന
    mn: Дэлхийн цөм
    mr: पृथ्वीच्या अंतरंगाची रचना
    ms: Struktur Bumi
    mzn: زمین درست بین
    nds: Eerdopbo
    nl: opbouw van de Aarde
    oc: Estructura de la Tèrra
    pa: ਧਰਤੀ ਦਾ ਢਾਂਚਾ
    pl: budowa Ziemi
    ps: د ځمکې جوړښت
    pt: Estrutura interna da Terra
    ro: straturile Pământului
    ru: структура Земли
    sd: ڌرتيءَ جو اندريون حصو
    sh: Struktura Zemlje
    si: පෘථිවියේ අභ්යන්තර ව්යුහය
    sk: Stavba Zeme
    sl: struktura Zemlje
    sq: Ndërtimi i brendshëm i Tokës
    sr: Грађа Земље
    sv: jordens struktur
    sw: Muundo wa dunia
    ta: புவியின் கட்டமைப்பு
    te: భూమి నిర్మాణం
    th: โครงสร้างของโลก
    tly: Zəmini soxtemon
    tr: Yerküre
    uk: Будова Землі
    vi: Cấu trúc Trái Đất
    wuu: 地球构造
    yue: 地球嘅結構
    zh: 地球構造
    zh_cn: 地球构造
---

# [[Structure_of_Earth]] 

#is_/same_as :: [[../../WikiData/WD~Structure_of_Earth,1664027|WD~Structure_of_Earth,1664027]]  

## #has_/text_of_/abstract 

> The internal structure of Earth is the layers of the planet Earth, 
> excluding its atmosphere and hydrosphere. 
> 
> The structure consists of an outer silicate solid crust, 
> a highly viscous asthenosphere, and solid mantle, 
> a liquid outer core whose flow generates the Earth's magnetic field, 
> and a solid inner core.
>
> Scientific understanding of the internal structure of Earth 
> is based on observations of topography and bathymetry, 
> observations of rock in outcrop, 
> samples brought to the surface from greater depths 
> by volcanoes or volcanic activity, 
> 
> analysis of the seismic waves that pass through Earth, 
> measurements of the gravitational and magnetic fields of Earth, 
> and experiments with crystalline solids at pressures and temperatures characteristic of Earth's deep interior.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Internal%20structure%20of%20Earth) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Earth/Climate/Structure_of_Earth|Structure_of_Earth]] 

### #is_/same_as :: [[/_public/Earth/Climate/Structure_of_Earth.public|Structure_of_Earth.public]] 

### #is_/same_as :: [[/_internal/Earth/Climate/Structure_of_Earth.internal|Structure_of_Earth.internal]] 

### #is_/same_as :: [[/_protect/Earth/Climate/Structure_of_Earth.protect|Structure_of_Earth.protect]] 

### #is_/same_as :: [[/_private/Earth/Climate/Structure_of_Earth.private|Structure_of_Earth.private]] 

### #is_/same_as :: [[/_personal/Earth/Climate/Structure_of_Earth.personal|Structure_of_Earth.personal]] 

### #is_/same_as :: [[/_secret/Earth/Climate/Structure_of_Earth.secret|Structure_of_Earth.secret]] 

