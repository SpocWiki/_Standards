---
aliases:
- abuurrraa ji’oogiraafawaa
- archwilio daearyddol
- deranderî
- ekspedisiya
- ekspeditsioon
- eksploracja
- eksplorimi
- Entdeckungsreise
- ergerzhadenn
- espedizio
- esploración
- esplorado
- esplorasion
- esplorazione
- exploracion
- exploració
- exploración
- exploración geográfica
- explorare
- exploratio
- exploration
- exploração
- felfedezés
- geografiniai tyrinėjimai
- Istraživanja
- istraživanje
- kâşif
- objaviteľská cesta
- objevitelská cesta
- Ontdekkingsreise
- ontdekkingsreizen
- opdagelsesrejse
- oppdagelsesreise
- oppdagingsreise
- penjelajahan
- raziskovanje
- sploration
- taiscéaladh
- teritoriāla izpēte
- Thàm-hiám
- thám hiểm
- tutkimusmatka
- upelelezi
- upptäcktsresa
- Ûntdekkingsreizen
- εξερεύνηση
- изследование
- истраживање
- истражување
- котыр ласянь географи эскеронъёс
- наукова експедиція
- сири арыйыы
- территориаль тикшеренеү
- территориальное исследование
- тэрытарыяльнае даследаванне
- тэрытарыяльнае дасьледаваньне
- Աշխարհագրական հետազոտություն
- אויספארשונג
- מגלה ארצות
- استكشاف
- جهانگردی
- کھوج
- अन्वेषक
- अन्वेषण
- खोजी यात्रा
- অভিযান
- ಅನ್ವೇಷಣೆ
- പര്യവേക്ഷണം
- การสำรวจ
- ტერიტორიული რკუალა
- 探検
- 探险
- 探險
- 탐험
has_id_wikidata: Q6502154
said_to_be_the_same_as: '[[_Standards/WikiData/WD~expedition,2401485]]'
practiced_by:
- '[[_Standards/WikiData/WD~scientific_explorer,10498147]]'
- '[[_Standards/WikiData/WD~explorer,11900058]]'
- '[[_Standards/WikiData/WD~expedition_leader,116485046]]'
topic_s_main_Wikimedia_portal: '[[_Standards/WikiData/WD~Portal_Exploration,18857150]]'
subclass_of:
- '[[_Standards/WikiData/WD~exploration,116967296]]'
- '[[_Standards/WikiData/WD~travel,61509]]'
Commons_category: Exploration
page_banner: http://commons.wikimedia.org/wiki/Special:FilePath/Footprints%20Banner.jpg
image: http://commons.wikimedia.org/wiki/Special:FilePath/Kazimierz%20Nowak%20in%20jungle%202.jpg
dv_is_:
  same_as:
  - '[[../../WikiData/WD~Exploration,6502154|WD~Exploration,6502154]]'
  - '[[/_Standards/Earth/Explorer/Exploration|Exploration]]'
  - '[[/_public/Earth/Explorer/Exploration.public|Exploration.public]]'
  - '[[/_internal/Earth/Explorer/Exploration.internal|Exploration.internal]]'
  - '[[/_protect/Earth/Explorer/Exploration.protect|Exploration.protect]]'
  - '[[/_private/Earth/Explorer/Exploration.private|Exploration.private]]'
  - '[[/_personal/Earth/Explorer/Exploration.personal|Exploration.personal]]'
  - '[[/_secret/Earth/Explorer/Exploration.secret|Exploration.secret]]'
dv_is_same_as:
- '[[../../WikiData/WD~Exploration,6502154|WD~Exploration,6502154]]'
- '[[/_Standards/Earth/Explorer/Exploration|Exploration]]'
- '[[/_public/Earth/Explorer/Exploration.public|Exploration.public]]'
- '[[/_internal/Earth/Explorer/Exploration.internal|Exploration.internal]]'
- '[[/_protect/Earth/Explorer/Exploration.protect|Exploration.protect]]'
- '[[/_private/Earth/Explorer/Exploration.private|Exploration.private]]'
- '[[/_personal/Earth/Explorer/Exploration.personal|Exploration.personal]]'
- '[[/_secret/Earth/Explorer/Exploration.secret|Exploration.secret]]'
dv_has_:
  name_:
    af: Ontdekkingsreise
    anp: अन्वेषण
    ar: استكشاف
    ast: esploración
    awa: अन्वेषक
    az: ekspedisiya
    ba: территориаль тикшеренеү
    be: тэрытарыяльнае даследаванне
    be_tarask: тэрытарыяльнае дасьледаваньне
    bg: изследование
    bn: অভিযান
    br: ergerzhadenn
    bs: istraživanje
    ca: exploració
    cs: objevitelská cesta
    cy: archwilio daearyddol
    da: opdagelsesrejse
    de: Entdeckungsreise
    el: εξερεύνηση
    en: exploration
    eo: esplorado
    es: exploración geográfica
    et: ekspeditsioon
    eu: espedizio
    fa: جهانگردی
    fi: tutkimusmatka
    fr: exploration
    fy: Ûntdekkingsreizen
    ga: taiscéaladh
    gl: exploración
    he: מגלה ארצות
    hi: खोजी यात्रा
    hr: Istraživanja
    hu: felfedezés
    hy: Աշխարհագրական հետազոտություն
    ia: exploration
    id: penjelajahan
    it: esplorazione
    ja: 探検
    kn: ಅನ್ವೇಷಣೆ
    ko: 탐험
    ku: deranderî
    la: exploratio
    lt: geografiniai tyrinėjimai
    lv: teritoriāla izpēte
    mk: истражување
    ml: പര്യവേക്ഷണം
    ms: penjelajahan
    nan: Thàm-hiám
    nb: oppdagelsesreise
    nl: ontdekkingsreizen
    nn: oppdagingsreise
    oc: exploracion
    om: abuurrraa ji’oogiraafawaa
    pl: eksploracja
    pt: exploração
    ro: explorare
    ru: территориальное исследование
    sah: сири арыйыы
    sco: sploration
    sh: Istraživanja
    sk: objaviteľská cesta
    sl: raziskovanje
    sq: eksplorimi
    sr: истраживање
    sr_ec: истраживање
    sr_el: istraživanje
    sv: upptäcktsresa
    sw: upelelezi
    th: การสำรวจ
    tr: kâşif
    udm: котыр ласянь географи эскеронъёс
    uk: наукова експедиція
    ur: کھوج
    vec: esplorasion
    vi: thám hiểm
    wuu: 探险
    xmf: ტერიტორიული რკუალა
    yi: אויספארשונג
    yue: 探險
    zh: 探险
    zh_cn: 探险
    zh_hans: 探险
    zh_hant: 探險
    zh_hk: 探險
    zh_tw: 探險
---

# [[Exploration]] 

#is_/same_as :: [[../../WikiData/WD~Exploration,6502154|WD~Exploration,6502154]] 

## #has_/text_of_/abstract 

> **Exploration** is the process of exploring, an activity which has some expectation of discovery. 
> Organised exploration is largely a human activity, 
> but exploratory activity is common to most organisms capable of directed locomotion 
> and the ability to learn, and has been described in, amongst others, social insects foraging behaviour, where feedback from returning individuals affects the activity of other members of the group.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Exploration) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Earth/Explorer/Exploration|Exploration]] 

### #is_/same_as :: [[/_public/Earth/Explorer/Exploration.public|Exploration.public]] 

### #is_/same_as :: [[/_internal/Earth/Explorer/Exploration.internal|Exploration.internal]] 

### #is_/same_as :: [[/_protect/Earth/Explorer/Exploration.protect|Exploration.protect]] 

### #is_/same_as :: [[/_private/Earth/Explorer/Exploration.private|Exploration.private]] 

### #is_/same_as :: [[/_personal/Earth/Explorer/Exploration.personal|Exploration.personal]] 

### #is_/same_as :: [[/_secret/Earth/Explorer/Exploration.secret|Exploration.secret]] 

