---
has_id_wikidata: Q1059178
dv_has_:
  name_:
    ar: استخبارات المصادر المفتوحة
    az: Açıq mənbə kəşfiyyatı
    be: разведка на аснове адкрытых крыніц
    be_tarask: выведка з адкрытых крыніцаў
    bg: OSINT
    ca: intel·ligència de codi obert
    cs: zpravodajství z otevřených zdrojů
    da: open-source efterretning og undersøgelse
    de: open source intelligence
    de_ch: Open Source Intelligence
    en: open-source intelligence
    en_ca: open-source intelligence
    en_gb: open-source intelligence
    eo: spionado per malfermita fonto
    es: Inteligencia de fuentes abiertas
    et: avalikel allikatel põhinev luure
    eu: Open-source Intelligence
    fa: اطلاعات منبعباز
    fi: avointen lähteiden tiedustelu
    fr: renseignement d'origine sources ouvertes
    he: מודיעין ממקורות גלויים
    hi: मुक्तस्रोत आसूचना
    hu: OSINT
    id: Kecerdasan sumber terbuka
    io: Informo di Publika Fonto
    it: Open-source Intelligence
    ja: オープン＝ソース・インテリジェンス
    ko: 공개출처정보
    lmo: Open Source Intelligence
    lv: izlūkošana ar atklāti pieejamu informācijas avotu palīdzību
    nb: frikildeetterretning
    nl: open source intelligence
    nn: frikjeldeetterretning
    pa: ਓਪਨ ਸੋਰਸ ਇੰਟੈਲੀਜੈਂਸ
    pl: biały wywiad
    ps: د استخباراتو پرانيستى سرچينه
    pt: OSINT
    pt_br: OSINT
    ru: OSINT
    sl: obveščevalna dejavnost javnih virov
    sv: open source intelligence
    syl: ꠈꠥꠟꠣꠝꠦꠟꠣ ꠉꠥꠄꠘ꠆ꠖꠣ ꠔꠁꠔ꠆ꠔ
    th: ข่าวกรองจากแหล่งข่าวเปิด
    tr: Açık kaynak istihbaratı
    uk: OSINT
    uz: Ochiq manbalarga koʻra qidiruv
    zh: 公开来源情报
    zh_cn: 开放来源情报
    zh_hant: 開放來源情報
    zh_tw: 開放來源情報
aliases:
- avalikel allikatel põhinev luure
- avointen lähteiden tiedustelu
- Açık kaynak istihbaratı
- Açıq mənbə kəşfiyyatı
- biały wywiad
- frikildeetterretning
- frikjeldeetterretning
- Informo di Publika Fonto
- Inteligencia de fuentes abiertas
- intel·ligència de codi obert
- izlūkošana ar atklāti pieejamu informācijas avotu palīdzību
- Kecerdasan sumber terbuka
- obveščevalna dejavnost javnih virov
- Ochiq manbalarga koʻra qidiruv
- open source intelligence
- open-source efterretning og undersøgelse
- open-source intelligence
- OSINT
- renseignement d'origine sources ouvertes
- spionado per malfermita fonto
- zpravodajství z otevřených zdrojů
- выведка з адкрытых крыніцаў
- разведка на аснове адкрытых крыніц
- מודיעין ממקורות גלויים
- استخبارات المصادر المفتوحة
- اطلاعات منبعباز
- د استخباراتو پرانيستى سرچينه
- मुक्तस्रोत आसूचना
- ਓਪਨ ਸੋਰਸ ਇੰਟੈਲੀਜੈਂਸ
- ข่าวกรองจากแหล่งข่าวเปิด
- オープン＝ソース・インテリジェンス
- 公开来源情报
- 开放来源情报
- 開放來源情報
- ꠈꠥꠟꠣꠝꠦꠟꠣ ꠉꠥꠄꠘ꠆ꠖꠣ ꠔꠁꠔ꠆ꠔ
- 공개출처정보
---
## #has_/text_of_/abstract 

> Open-source intelligence (**OSINT**) 
> is the collection and analysis of data gathered from open sources 
> (overt sources and publicly available information) 
> to produce actionable intelligence. 
> 
> OSINT is primarily used in national security, law enforcement, 
> and business intelligence functions and is of value to 
> analysts who use non-sensitive intelligence in answering 
> classified, unclassified, or proprietary intelligence requirements 
> across the previous intelligence disciplines.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Open-source%20intelligence)  

## #has_/example 

[[2014-Shooting_of_Flight_MH17]] by [[Bellingcat]] 

