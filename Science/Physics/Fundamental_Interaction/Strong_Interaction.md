---
aliases:
- bashkëveprimi i fortë
- Chrang intarakshan
- Daya nukleus kuat
- elkarreragin nuklear indartsu
- Entèraksyon fò
- Entéraksyon fò
- erős kölcsönhatás
- fargekraft
- fort-interago
- forta nuklea forto
- força forte
- força nuclear forta
- fòrça nucleara fòrta
- Gaya nuklir kuat
- Güclü qarşılıqlı təsir
- güçlü etkileşim
- idirghníomhú láidir
- interacción nuclear forte
- Interacción nuclear fuerte
- Interacción nucleyar fuerte
- Interaction fort
- interaction forte
- interacțiunea tare
- interaksiyong malakas
- Interassion fòrta
- interazione forte
- intirazzioni forti
- Iñuku huk'i sinchi micha
- jaka interakcija
- jaka sila
- Kiông-la̍t
- Kuchli o'zaro ta'sir
- küçlü qarşılıqlı tesir
- Makusog nga pahilabotay
- močna jedrska sila
- Mwingiliano madhubuti
- oddziaływanie silne
- rhyngweithiad cryf
- silná interakce
- silná interakcia hmotných objektov
- Starchi Wächselwürkig
- stark växelverkan
- Starke Kraft
- starke Wechselwirkung
- sterk wisselwerking
- Sterke kearnkrêft
- sterke kernkracht
- Sterki kjarnakraftur
- stiprioji sąveika
- stiprā mijiedarbība
- strang interaction
- strong force
- Strong interaction
- strong nuclear force
- Strong_Interaction
- stærk kernekraft
- tugev vastastikmõju
- tương tác mạnh
- vahva vuorovaikutus
- vis fortis
- Vägev vastamižpaineg
- ισχυρή αλληλεπίδραση
- јака интеракција
- Бот-боттарынче күштүг кылдыныг
- Вăйлă хире-хирĕç вăйăм
- Күшті өзара әсерлесу
- көслө үҙ-ара тәьҫир итешеү
- Көчле тәэсир итешү
- моцнае ўзаемадзеянне
- моцнае ўзаемадзеяньне
- силно заемодејство
- Силно ядрено взаимодействие
- сильна взаємодія
- сильное взаимодействие
- Хүчтэй харилцан үйлчлэл
- юн вачекусыпъяськон
- Ուժեղ փոխազդեցություն
- הכוח החזק
- שטארקע קראפט
- تآثر قوي
- تکڑا میل جول
- قوي تعامل
- قوی تفاعل
- نیروی هستهای قوی
- کارلێکی بەھێز
- گوجلو چکیردک ائتگیلیشیم
- प्रबल अन्योन्य क्रिया
- প্ৰবল আন্তঃক্ৰিয়া
- সবল নিউক্লিয় বল
- ਤਕੜਾ ਮੇਲ-ਜੋਲ
- வலிய இடைவினை
- బలమైన పరస్పర చర్య
- ശക്ത ന്യൂക്ലിയാർ പ്രവർത്തനം
- อันตรกิริยาอย่างเข้ม
- အားကောင်းစွာ အချင်းချင်းသက်ရောက်မှု
- ძლიერი ურთიერთქმედება
- 強い相互作用
- 強相互作用
- 强相互作用
- 강한 상호작용
- 강한호상작용
has_id_wikidata: Q11415
Commons_category: Strong nuclear force
Dewey_Decimal_Classification: 539.7548
Stack_Exchange_tag: https://physics.stackexchange.com/tags/strong-force
described_by_source:
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Nuclear%20Force%20anim%20smaller.gif
instance_of:
- '[[_Standards/WikiData/WD~fundamental_interaction,104934]]'
subclass_of:
- '[[_Standards/WikiData/WD~elementary_particle_interaction,71130616]]'
dv_has_:
  name_:
    af: sterk wisselwerking
    an: Interacción nucleyar fuerte
    ar: تآثر قوي
    as: প্ৰবল আন্তঃক্ৰিয়া
    ast: Interacción nuclear fuerte
    az: Güclü qarşılıqlı təsir
    azb: گوجلو چکیردک ائتگیلیشیم
    ba: көслө үҙ-ара тәьҫир итешеү
    be: моцнае ўзаемадзеянне
    be_tarask: моцнае ўзаемадзеяньне
    bg: Силно ядрено взаимодействие
    bn: সবল নিউক্লিয় বল
    bs: jaka interakcija
    ca: força nuclear forta
    ckb: کارلێکی بەھێز
    crh: küçlü qarşılıqlı tesir
    cs: silná interakce
    cv: Вăйлă хире-хирĕç вăйăм
    cy: rhyngweithiad cryf
    da: stærk kernekraft
    de: starke Wechselwirkung
    el: ισχυρή αλληλεπίδραση
    en: strong interaction
    eo: forta nuklea forto
    es: interacción nuclear fuerte
    et: tugev vastastikmõju
    eu: elkarreragin nuklear indartsu
    fa: نیروی هستهای قوی
    fi: vahva vuorovaikutus
    fr: interaction forte
    fy: Sterke kearnkrêft
    ga: idirghníomhú láidir
    gcr: Entéraksyon fò
    gl: interacción nuclear forte
    gsw: Starchi Wächselwürkig
    he: הכוח החזק
    hi: प्रबल अन्योन्य क्रिया
    hif: Strong interaction
    hr: jaka sila
    ht: Entèraksyon fò
    hu: erős kölcsönhatás
    hy: Ուժեղ փոխազդեցություն
    ia: interaction forte
    id: Gaya nuklir kuat
    ie: Interaction fort
    io: fort-interago
    is: Sterki kjarnakraftur
    it: interazione forte
    ja: 強い相互作用
    jam: Chrang intarakshan
    ka: ძლიერი ურთიერთქმედება
    kk: Күшті өзара әсерлесу
    ko: 강한 상호작용
    ko-kp: 강한호상작용
    la: vis fortis
    lt: stiprioji sąveika
    lv: stiprā mijiedarbība
    mk: силно заемодејство
    ml: ശക്ത ന്യൂക്ലിയാർ പ്രവർത്തനം
    mn: Хүчтэй харилцан үйлчлэл
    ms: Daya nukleus kuat
    my: အားကောင်းစွာ အချင်းချင်းသက်ရောက်မှု
    nan: Kiông-la̍t
    nb: fargekraft
    nds: Starke Kraft
    nl: sterke kernkracht
    nn: fargekraft
    oc: fòrça nucleara fòrta
    pa: ਤਕੜਾ ਮੇਲ-ਜੋਲ
    pl: oddziaływanie silne
    pms: Interassion fòrta
    pnb: تکڑا میل جول
    ps: قوي تعامل
    pt: força forte
    qu: Iñuku huk'i sinchi micha
    ro: interacțiunea tare
    ru: сильное взаимодействие
    scn: intirazzioni forti
    sco: strang interaction
    sh: Jaka interakcija
    sk: silná interakcia hmotných objektov
    sl: močna jedrska sila
    sq: bashkëveprimi i fortë
    sr: јака интеракција
    su: Gaya nuklir kuat
    sv: stark växelverkan
    sw: Mwingiliano madhubuti
    ta: வலிய இடைவினை
    te: బలమైన పరస్పర చర్య
    th: อันตรกิริยาอย่างเข้ม
    tl: interaksiyong malakas
    tr: güçlü etkileşim
    tt: Көчле тәэсир итешү
    tyv: Бот-боттарынче күштүг кылдыныг
    udm: юн вачекусыпъяськон
    uk: сильна взаємодія
    ur: قوی تفاعل
    uz: Kuchli o'zaro ta'sir
    vep: Vägev vastamižpaineg
    vi: tương tác mạnh
    war: Makusog nga pahilabotay
    wuu: 强相互作用
    yi: שטארקע קראפט
    yue: 強相互作用
    zh: 强相互作用
---
# [[Strong_Interaction]] 

#is_/same_as :: [[../../../WikiData/WD~Strong_interaction,11415|WD~Strong_interaction,11415]] 

## #has_/text_of_/abstract 

> In nuclear physics and particle physics, the **Strong interaction**, 
> also called the strong force or strong nuclear force, is a fundamental interaction 
> that confines quarks into protons, neutrons, and other hadron particles. 
> The strong interaction also binds neutrons and protons to create atomic nuclei, 
> where it is called the nuclear force.
>
> Most of the mass of a proton or neutron is the result of the strong interaction energy; 
> the individual quarks provide only about 1% of the mass of a proton. 
> At the range of 10−15 m (1 femtometer, slightly more than the radius of a nucleon), 
> the strong force is approximately 100 times as strong as electromagnetism, 
> 106 times as strong as the weak interaction, and 1038 times as strong as gravitation.
>
> In the context of atomic nuclei, the force binds protons and neutrons together 
> to form a nucleus and is called the nuclear force (or residual strong force). 
> 
> Because the force is mediated by massive, short lived mesons on this scale, 
> the residual strong interaction obeys a distance-dependent behavior between nucleons 
> that is quite different from when it is acting to bind quarks within hadrons. 
> 
> There are also differences in the binding energies of the nuclear force 
> with regard to nuclear fusion versus nuclear fission. 
> 
> Nuclear fusion accounts for most energy production in the Sun and other stars. 
> Nuclear fission allows for decay of radioactive elements and isotopes, 
> although it is often mediated by the weak interaction. 
> 
> Artificially, the energy associated with the nuclear force 
> is partially released in nuclear power and nuclear weapons, 
> both in uranium or plutonium-based fission weapons and in fusion weapons like the hydrogen bomb.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Strong%20interaction) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Fundamental_Interaction/Strong_Interaction|Strong_Interaction]] 

### #is_/same_as :: [[/_public/Science/Physics/Fundamental_Interaction/Strong_Interaction.public|Strong_Interaction.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Fundamental_Interaction/Strong_Interaction.internal|Strong_Interaction.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Fundamental_Interaction/Strong_Interaction.protect|Strong_Interaction.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Fundamental_Interaction/Strong_Interaction.private|Strong_Interaction.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Fundamental_Interaction/Strong_Interaction.personal|Strong_Interaction.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Fundamental_Interaction/Strong_Interaction.secret|Strong_Interaction.secret]] 

