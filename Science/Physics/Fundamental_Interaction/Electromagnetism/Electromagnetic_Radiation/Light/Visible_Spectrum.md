---
aliases:
- argi ikusgai
- barevné spektrum
- Det optiske spektret
- espectre visible
- espectro visible
- espectro visível
- espectru visible
- gong'
- Görünür ışık
- haske
- Khó-kiàn kng-sòaⁿ
- Ko‘rinadigan spektr
- Lichtspektrum
- Liichtspektrum
- Litróf
- lumière visible
- látható spektrum
- Niisum
- nähtav spekter
- näkyvän valon spektri
- Optické spektrum
- phổ nhìn thấy được
- redzamā gaisma
- regimasis spektras
- sbectrwm gweladwy
- Sigbare spektrum
- Sinar tampak
- Speactram an t-Solais
- Spectru vizibil
- Spectrum visuale
- speictream infheicthe
- Spektar
- Spektr gwelus
- spektrum kasat mata
- Spektrum yang boleh dilihat
- Spetro eletromagnetal
- Spetro vixibiłe
- spettro visibile
- Spéktrum cahaya katémbong
- suno pi ken lukin
- synliga spektrumet
- synligt spektrum
- videbla spektro
- Vidljiva svjetlost
- vidni spekter
- visible spectrum
- Visual spectrum
- visuelt spektrum
- Vizebl spekchrom
- zichtbaar spectrum
- światło widzialne
- şebanga ronahiya bînraw
- şebenga ronahiya bînraw
- ορατό φάσμα
- бачнае выпраменьванне
- бачнае выпраменьваньне
- видљиви спектар
- Видим спектър
- видиме світло
- видимое излучение
- видлив спектар
- Курăнакан пайăркалав
- күренмә нурланыш
- Үзэгдэх спектр
- Տեսանելի ճառագայթում
- הספקטרום הנראה
- روشني جا رنگ
- شەبەنگی بینراو
- طيف مرئي
- طیف مرئی
- مرئی طیف
- प्रत्यक्ष वर्णक्रम
- भिजिबल स्पेक्ट्रम
- দৃশ্যমান বর্ণালী
- ਪ੍ਰਤੱਖ ਪ੍ਰਕਾਸ਼
- கட்புலனாகும் நிறமாலை
- ದೃಗ್ಗೋಚರ ರೋಹಿತ
- දෘශ්ය වර්ණාවලිය
- สเปกตรัมมองเห็นได้
- ხილული გამოსხივება
- ᎬᎪᏩᏛᏗ ᏗᎧᏃᏗ
- 可見光
- 可視光線
- 可见光
- 가시광선
has_id_wikidata: Q76299
part_of: '[[_Standards/WikiData/WD~electromagnetic_spectrum,133139]]'
described_by_source:
- '[[_Standards/WikiData/WD~The_Nuttall_Encyclopædia,3181656]]'
- "[[_Standards/WikiData/WD~The_New_Student's_Reference_Work,16082057]]"
subclass_of: '[[_Standards/WikiData/WD~light,9128]]'
follows: '[[_Standards/WikiData/WD~ultraviolet_radiation,11391]]'
followed_by: '[[_Standards/WikiData/WD~infrared_radiation,11388]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Light%20Amplification%20by%20Stimulated%20Emission%20of%20Radiation.jpg
video: http://commons.wikimedia.org/wiki/Special:FilePath/Light%20Amplification%20by%20Stimulated%20Emission%20of%20Radiation.ogv
Commons_category: Visible spectrum
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q1321%20%28spa%29-Txtdgtl-espectro%20visible.wav
Stack_Exchange_tag: https://physics.stackexchange.com/tags/visible-light
dv_has_:
  name_:
    af: Sigbare spektrum
    ar: طيف مرئي
    ast: espectru visible
    be: бачнае выпраменьванне
    be-tarask: бачнае выпраменьваньне
    bg: Видим спектър
    bn: দৃশ্যমান বর্ণালী
    br: Spektr gwelus
    bs: Vidljiva svjetlost
    ca: espectre visible
    chr: ᎬᎪᏩᏛᏗ ᏗᎧᏃᏗ
    ckb: شەبەنگی بینراو
    cs: barevné spektrum
    cv: Курăнакан пайăркалав
    cy: sbectrwm gweladwy
    da: synligt spektrum
    de: Lichtspektrum
    el: ορατό φάσμα
    en: visible spectrum
    en-ca: visible spectrum
    en-gb: visible spectrum
    eo: videbla spektro
    es: espectro visible
    et: nähtav spekter
    eu: argi ikusgai
    fa: طیف مرئی
    fi: näkyvän valon spektri
    fr: lumière visible
    ga: speictream infheicthe
    gd: Speactram an t-Solais
    gl: Espectro visible
    gur: Niisum
    ha: haske
    hak: gong'
    he: הספקטרום הנראה
    hi: प्रत्यक्ष वर्णक्रम
    hif: Visual spectrum
    hr: Spektar
    hu: látható spektrum
    hy: Տեսանելի ճառագայթում
    id: spektrum kasat mata
    io: Videbla spektro
    is: Litróf
    it: spettro visibile
    ja: 可視光線
    jam: Vizebl spekchrom
    ka: ხილული გამოსხივება
    kn: ದೃಗ್ಗೋಚರ ರೋಹಿತ
    ko: 가시광선
    ku: şebanga ronahiya bînraw
    ku-latn: şebenga ronahiya bînraw
    la: Spectrum visuale
    lb: Liichtspektrum
    lfn: Spetro eletromagnetal
    lt: regimasis spektras
    lv: redzamā gaisma
    min: Sinar tampak
    mk: видлив спектар
    mn: Үзэгдэх спектр
    ms: Spektrum yang boleh dilihat
    nan: Khó-kiàn kng-sòaⁿ
    nb: visuelt spektrum
    new: भिजिबल स्पेक्ट्रम
    nl: zichtbaar spectrum
    nn: Det optiske spektret
    pa: ਪ੍ਰਤੱਖ ਪ੍ਰਕਾਸ਼
    pl: światło widzialne
    pt: espectro visível
    ro: Spectru vizibil
    ru: видимое излучение
    sd: روشني جا رنگ
    sh: Spektar
    si: දෘශ්ය වර්ණාවලිය
    sk: Optické spektrum
    sl: vidni spekter
    sr: видљиви спектар
    su: Spéktrum cahaya katémbong
    sv: synliga spektrumet
    ta: கட்புலனாகும் நிறமாலை
    th: สเปกตรัมมองเห็นได้
    tok: suno pi ken lukin
    tr: Görünür ışık
    tt: күренмә нурланыш
    tt-cyrl: күренмә нурланыш
    uk: видиме світло
    ur: مرئی طیف
    uz: Ko‘rinadigan spektr
    vec: Spetro vixibiłe
    vi: phổ nhìn thấy được
    wuu: 可见光
    yue: 可見光
    zh: 可见光
    zh-hant: 可見光
    zh-hk: 可見光
---
# [[Visible_Spectrum]] 

#is_/same_as :: [[../../../../../../WikiData/WD~Visible_spectrum,76299|WD~Visible_spectrum,76299]]  

## #has_/text_of_/abstract 

> The visible spectrum is the band of the electromagnetic spectrum 
> that is visible to the human eye. 
> Electromagnetic radiation in this range of wavelengths is called visible light (or simply light).
>
> The optical spectrum is sometimes considered to be the same as the visible spectrum, but some authors define the term more broadly, to include the ultraviolet and infrared parts of the electromagnetic spectrum as well, known collectively as optical radiation.
>
> A typical human eye will respond to wavelengths from about 380 to about 750 nanometers. In terms of frequency, this corresponds to a band in the vicinity of 400–790 terahertz. These boundaries are not sharply defined and may vary per individual. Under optimal conditions, these limits of human perception can extend to 310 nm (ultraviolet) and 1100 nm (near infrared).
>
> The spectrum does not contain all the colors that the human visual system can distinguish. Unsaturated colors such as pink, or purple variations like magenta, for example, are absent because they can only be made from a mix of multiple wavelengths. Colors containing only one wavelength are also called pure colors or spectral colors.
>
> Visible wavelengths pass largely unattenuated through the Earth's atmosphere via the "optical window" region of the electromagnetic spectrum. An example of this phenomenon is when clean air scatters blue light more than red light, and so the midday sky appears blue (apart from the area around the Sun which appears white because the light is not scattered as much). The optical window is also referred to as the "visible window" because it overlaps the human visible response spectrum. The near infrared (NIR) window lies just out of the human vision, as well as the medium wavelength infrared (MWIR) window, and the long-wavelength or far-infrared (LWIR or FIR) window, although other animals may perceive them.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Visible%20spectrum) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Fundamental_Interaction/Electromagnetism/Electromagnetic_Radiation/Light/Visible_Spectrum|Visible_Spectrum]] 

### #is_/same_as :: [[/_public/Science/Physics/Fundamental_Interaction/Electromagnetism/Electromagnetic_Radiation/Light/Visible_Spectrum.public|Visible_Spectrum.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Fundamental_Interaction/Electromagnetism/Electromagnetic_Radiation/Light/Visible_Spectrum.internal|Visible_Spectrum.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Fundamental_Interaction/Electromagnetism/Electromagnetic_Radiation/Light/Visible_Spectrum.protect|Visible_Spectrum.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Fundamental_Interaction/Electromagnetism/Electromagnetic_Radiation/Light/Visible_Spectrum.private|Visible_Spectrum.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Fundamental_Interaction/Electromagnetism/Electromagnetic_Radiation/Light/Visible_Spectrum.personal|Visible_Spectrum.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Fundamental_Interaction/Electromagnetism/Electromagnetic_Radiation/Light/Visible_Spectrum.secret|Visible_Spectrum.secret]] 

