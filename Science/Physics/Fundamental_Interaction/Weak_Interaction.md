---
aliases:
- bashkëveprimi i dobët
- Daya nukleus lemah
- Desfort-interago
- elkarrekintza nuklear ahul
- Entéraksyon fèb
- força fraca
- força nuclear feble
- fòrça nucleara febla
- Gaya nuklir lemah
- gyenge kölcsönhatás
- heikko vuorovaikutus
- idirghníomhú lag
- Interacción débil
- interacción nuclear débil
- Interacción nucleyar feble
- Interaction debil
- interaction debile
- interaction faible
- interacțiunea slabă
- interaksiyong mahina
- Interassion debla
- interazione debole
- intirazzioni dèbbuli
- Jio̍k-la̍t
- Kuchsiz o'zaro ta'sir
- Liaunasis sāveikavėms
- malforta nuklea forto
- Maluya nga pahilabotay
- Mwingiliano dhaifu
- nõrk vastastikmõju
- oddziaływanie słabe
- rhyngweithiad gwan
- schwache Wechselwirkung
- Schwachi Wächselwürkig
- silpnoji sąveika
- Slaba interakcija
- Slaba nuklearna sila
- slabá interakce
- slabá interakcia hmotných objektov
- svag kernekraft
- svag växelverkan
- svak kjernekraft
- Swacke Kraft
- swak wisselwerking
- Swakke kearnkrêft
- tương tác yếu
- Veiki kjarnakraftur
- Ven vastamižpaineg
- vis debilis
- vājā mijiedarbība
- waik interaction
- weak force
- weak interaction
- weak nuclear force
- Weak_Interaction
- Wiik intarakshan
- zayıf nükleer kuvvet
- zayıf qarşılıqlı tesir
- zwakke kernkracht
- šibka jedrska sila
- ασθενής αλληλεπίδραση
- Бот-боттарынче кошкак кылдыныг
- Вăйсăр хире-хирĕç вăйăм
- Зәгыйфь тәэсир итешү
- көсһөҙ үҙ-ара тәьҫир итешеү
- ляб вачекусыпъяськон
- слаба интеракција
- слабка взаємодія
- слабо заемодејство
- Слабо ядрено взаимодействие
- слабое ўзаемадзеянне
- слабое ўзаемадзеяньне
- слабое взаимодействие
- Сул Харилцан Үйлчлэл
- Әлсіз өзара әсерлесу
- Өз ара начар аракеттешүү
- Թույլ փոխազդեցություն
- הכוח החלש
- שוואכע קראפט
- قوة نووية ضعيفة
- ماڑا میل جول
- نحیف تفاعل
- نیروی هستهای ضعیف
- کارلێکی لاواز
- दुर्बल अन्योन्य क्रिया
- দুর্বল নিউক্লিয় বল
- দুৰ্বল আন্তঃক্ৰিয়া
- ਮਾੜਾ ਮੇਲ-ਜੋਲ
- வலிகுறை இடைவினை
- ദുർബല അണുകേന്ദ്രബലം
- อันตรกิริยาอย่างอ่อน
- အားနည်းစွာ အချင်းချင်းသက်ရောက်မှု
- სუსტი ურთიერთქმედება
- 弱い相互作用
- 弱相互作用
- 약한 상호작용
has_id_wikidata: Q11418
Commons_category: Weak nuclear force
Dewey_Decimal_Classification: 539.7544
Stack_Exchange_tag: https://physics.stackexchange.com/tags/weak-interaction
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._4,124737630|WD~Armenian_Soviet_Encyclopedia,_vol._4,124737630]]'
instance_of: '[[_Standards/WikiData/WD~fundamental_interaction,104934|WD~fundamental_interaction,104934]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/PiPlus%20muon%20decay.svg
subclass_of: '[[_Standards/WikiData/WD~electroweak_interaction,466416|WD~electroweak_interaction,466416]]'
schematic:
- http://commons.wikimedia.org/wiki/Special:FilePath/%D9%85%D8%AE%D8%B7%D8%B7%20%D8%AA%D9%81%D9%83%D9%83%20%D8%A7%D9%84%D9%83%D9%88%D8%A7%D8%B1%D9%83%20%D8%A8%D8%A5%D8%B4%D8%B9%D8%A7%D8%B9%20%D8%A8%D9%8A%D8%AA%D8%A7.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Diagrama%20desintegraci%C3%B3%20feble.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Diagramme%20de%20la%20d%C3%A9sint%C3%A9gration%20des%20quarks%20par%20radioactivit%C3%A9%20beta.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Quark%20weak%20interactions-tr.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Quark%20weak%20interactions-vi.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Weak%20Decay%20%28flipped%29.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Weak%20Decay%20%28flipped%29%20mk.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Weak%20decay%20diagram%20zh-hans.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Weak%20decay%20diagram%20zh-hant.svg
dv_has_:
  name_:
    af: swak wisselwerking
    an: Interacción nucleyar feble
    ar: قوة نووية ضعيفة
    as: দুৰ্বল আন্তঃক্ৰিয়া
    ast: Interacción débil
    ba: көсһөҙ үҙ-ара тәьҫир итешеү
    be: слабое ўзаемадзеянне
    be_tarask: слабое ўзаемадзеяньне
    bg: Слабо ядрено взаимодействие
    bn: দুর্বল নিউক্লিয় বল
    bs: Slaba nuklearna sila
    ca: força nuclear feble
    ckb: کارلێکی لاواز
    crh: zayıf qarşılıqlı tesir
    cs: slabá interakce
    cv: Вăйсăр хире-хирĕç вăйăм
    cy: rhyngweithiad gwan
    da: svag kernekraft
    de: schwache Wechselwirkung
    el: ασθενής αλληλεπίδραση
    en: weak interaction
    eo: malforta nuklea forto
    es: interacción débil
    et: nõrk vastastikmõju
    eu: elkarrekintza nuklear ahul
    fa: نیروی هستهای ضعیف
    fi: heikko vuorovaikutus
    fr: interaction faible
    fy: Swakke kearnkrêft
    ga: idirghníomhú lag
    gcr: Entéraksyon fèb
    gl: interacción nuclear débil
    gsw: Schwachi Wächselwürkig
    he: הכוח החלש
    hi: दुर्बल अन्योन्य क्रिया
    hif: Weak interaction
    hr: slaba nuklearna sila
    hu: gyenge kölcsönhatás
    hy: Թույլ փոխազդեցություն
    ia: interaction debile
    id: Gaya nuklir lemah
    ie: Interaction debil
    io: Desfort-interago
    is: Veiki kjarnakraftur
    it: interazione debole
    ja: 弱い相互作用
    jam: Wiik intarakshan
    ka: სუსტი ურთიერთქმედება
    kk: Әлсіз өзара әсерлесу
    ko: 약한 상호작용
    ky: Өз ара начар аракеттешүү
    la: vis debilis
    lt: silpnoji sąveika
    lv: vājā mijiedarbība
    mk: слабо заемодејство
    ml: ദുർബല അണുകേന്ദ്രബലം
    mn: Сул Харилцан Үйлчлэл
    ms: Daya nukleus lemah
    my: အားနည်းစွာ အချင်းချင်းသက်ရောက်မှု
    nan: Jio̍k-la̍t
    nb: svak kjernekraft
    nds: Swacke Kraft
    nl: zwakke kernkracht
    nn: svak kjernekraft
    oc: fòrça nucleara febla
    pa: ਮਾੜਾ ਮੇਲ-ਜੋਲ
    pl: oddziaływanie słabe
    pms: Interassion debla
    pnb: ماڑا میل جول
    pt: força fraca
    ro: interacțiunea slabă
    ru: слабое взаимодействие
    scn: intirazzioni dèbbuli
    sco: waik interaction
    sgs: Liaunasis sāveikavėms
    sh: Slaba interakcija
    sk: slabá interakcia hmotných objektov
    sl: šibka jedrska sila
    sq: bashkëveprimi i dobët
    sr: слаба интеракција
    sr_ec: слаба интеракција
    sr_el: slaba interakcija
    su: Gaya nuklir lemah
    sv: svag växelverkan
    sw: Mwingiliano dhaifu
    ta: வலிகுறை இடைவினை
    th: อันตรกิริยาอย่างอ่อน
    tl: interaksiyong mahina
    tr: zayıf nükleer kuvvet
    tt: Зәгыйфь тәэсир итешү
    tyv: Бот-боттарынче кошкак кылдыныг
    udm: ляб вачекусыпъяськон
    uk: слабка взаємодія
    ur: نحیف تفاعل
    uz: Kuchsiz o'zaro ta'sir
    vep: Ven vastamižpaineg
    vi: tương tác yếu
    war: Maluya nga pahilabotay
    wuu: 弱相互作用
    yi: שוואכע קראפט
    yue: 弱相互作用
    zh: 弱相互作用
---

# [[Weak_Interaction]] 

#is_/same_as :: [[../../../WikiData/WD~Weak_interaction,11418|WD~Weak_interaction,11418]] 

## #has_/text_of_/abstract 

> In nuclear physics and particle physics, the **weak interaction**, weak force or the weak nuclear force, 
> is one of the four known [[../Fundamental_Interaction|Fundamental_Interaction]]s . 
> 
> It is the mechanism of interaction between subatomic particles 
> that is responsible for the radioactive decay of atoms: 
> 
> The weak interaction participates in nuclear fission and nuclear fusion. 
> The theory describing its behaviour and effects 
> is sometimes called quantum flavordynamics (QFD); however, the term QFD is rarely used, 
> because the weak force is better understood by electroweak theory (EWT).
>
> The effective range of the weak force is limited to subatomic distances 
> and is less than the diameter of a proton.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Weak%20interaction) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Fundamental_Interaction/Weak_Interaction|Weak_Interaction]] 

### #is_/same_as :: [[/_public/Science/Physics/Fundamental_Interaction/Weak_Interaction.public|Weak_Interaction.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Fundamental_Interaction/Weak_Interaction.internal|Weak_Interaction.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Fundamental_Interaction/Weak_Interaction.protect|Weak_Interaction.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Fundamental_Interaction/Weak_Interaction.private|Weak_Interaction.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Fundamental_Interaction/Weak_Interaction.personal|Weak_Interaction.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Fundamental_Interaction/Weak_Interaction.secret|Weak_Interaction.secret]] 

