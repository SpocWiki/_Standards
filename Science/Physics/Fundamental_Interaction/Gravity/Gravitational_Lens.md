---
aliases:
- Anya ndọda
- Ferenn c'hravitadurel
- grabitazio-lente
- gravita lenso
- gravitaatiolinssi
- gravitacijska leča
- Gravitacijske leće
- Gravitacinis lęšis
- Gravitaciona sočiva
- Gravitasielens
- gravitasjonslinse
- gravitational lens
- Gravitational_Lens
- gravitationslins
- Gravitationslinseeffekt
- Gravitationslinseneffekt
- Gravitatsion linzalar
- Gravitatsioonilääts
- Gravitačná šošovka
- gravitační čočka
- gravitációs lencse
- Gravitācijas lēca
- kütleçekimsel merceklenme
- Lensa gravitasi
- lent gravitatòria
- Lente gravitacional
- lente gravitazionale
- Lentilha gravitacionala
- lentille gravitationnelle
- lentilă gravitațională
- Lenzi ya uvutano
- lionsa imtharraingteach
- soczewkowanie grawitacyjne
- Thấu kính hấp dẫn
- zwaartekrachtlens
- Βαρυτικός φακός
- гравітаційна лінза
- Гравітацыйная лінза
- Гравитацилле линза
- Гравитацион линза
- Гравитационна леща
- гравитационная линза
- гравитациска леќа
- Գրավիտացիոն ոսպնյակ
- עידוש כבידתי
- ثقلی عدسہ
- عدسة الجاذبية
- همگرایی گرانشی
- गुरुत्वाकर्षक लेंस
- মহাকর্ষীয় পরকলা
- ਗਰੈਵੀਟੇਸ਼ਨਲ ਲੈੱਨਜ਼ਿੰਗ
- ஈர்ப்பு வில்லை
- ഗ്രാവിറ്റേഷനൽ ലെൻസ്
- เลนส์ความโน้มถ่วง
- გრავიტაციული ლინზირება
- 引力透鏡
- 引力透镜
- 引力透镜效应
- 重力レンズ
- 重力透鏡效應
- 중력렌즈
has_id_wikidata: Q185243
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Gravitational_lensing,15793169]]'
instance_of: '[[_Standards/WikiData/WD~astronomical_object_type,17444909]]'
subclass_of: '[[_Standards/WikiData/WD~astronomical_object,6999]]'
OmegaWiki_Defined_Meaning: 1246240
image: http://commons.wikimedia.org/wiki/Special:FilePath/A%20Horseshoe%20Einstein%20Ring%20from%20Hubble.JPG
video: http://commons.wikimedia.org/wiki/Special:FilePath/BBH%20gravitational%20lensing%20of%20gw150914.webm
Commons_category: Gravitational lensing
Krugosvet_article: nauka_i_tehnika/fizika/GRAVITATSIONNAYA_LINZA.html
defining_formula: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>&#x03B8;</mi><mo stretchy="false">=</mo><mrow data-mjx-texclass="ORD"><mfrac><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mn>4</mn><mi>G</mi><mi>M</mi></mrow></mrow><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mi>r</mi><msup><mi>c</mi><mrow data-mjx-texclass="ORD"><mn>2</mn></mrow></msup></mrow></mrow></mfrac></mrow></mstyle></mrow></math>
dv_has_:
  name_:
    af: Gravitasielens
    ar: عدسة الجاذبية
    ast: Lente gravitacional
    be: Гравітацыйная лінза
    be_tarask: гравітацыйная лінза
    bg: Гравитационна леща
    bn: মহাকর্ষীয় পরকলা
    br: Ferenn c'hravitadurel
    ca: lent gravitatòria
    cs: gravitační čočka
    cv: Гравитацилле линза
    da: Gravitationslinseeffekt
    de: Gravitationslinseneffekt
    el: Βαρυτικός φακός
    en: gravitational lens
    eo: gravita lenso
    es: lente gravitacional
    et: Gravitatsioonilääts
    eu: grabitazio-lente
    fa: همگرایی گرانشی
    fi: gravitaatiolinssi
    fr: lentille gravitationnelle
    ga: lionsa imtharraingteach
    gl: lente gravitacional
    he: עידוש כבידתי
    hi: गुरुत्वाकर्षक लेंस
    hr: Gravitacijske leće
    hu: gravitációs lencse
    hy: Գրավիտացիոն ոսպնյակ
    id: Lensa gravitasi
    ig: Anya ndọda
    it: lente gravitazionale
    ja: 重力レンズ
    ka: გრავიტაციული ლინზირება
    ko: 중력렌즈
    lt: Gravitacinis lęšis
    lv: Gravitācijas lēca
    mk: гравитациска леќа
    ml: ഗ്രാവിറ്റേഷനൽ ലെൻസ്
    nb: gravitasjonslinse
    nl: zwaartekrachtlens
    nn: Gravitasjonslinse
    oc: Lentilha gravitacionala
    pa: ਗਰੈਵੀਟੇਸ਼ਨਲ ਲੈੱਨਜ਼ਿੰਗ
    pl: soczewkowanie grawitacyjne
    pt: lente gravitacional
    ro: lentilă gravitațională
    ru: гравитационная линза
    sh: Gravitacijske leće
    sk: Gravitačná šošovka
    sl: gravitacijska leča
    sr: Gravitaciona sočiva
    sv: gravitationslins
    sw: Lenzi ya uvutano
    ta: ஈர்ப்பு வில்லை
    th: เลนส์ความโน้มถ่วง
    tr: kütleçekimsel merceklenme
    tt: Гравитацион линза
    uk: гравітаційна лінза
    ur: ثقلی عدسہ
    uz: Gravitatsion linzalar
    vi: Thấu kính hấp dẫn
    wuu: 引力透镜效应
    yue: 引力透鏡
    zh: 引力透镜
    zh_cn: 引力透镜效应
    zh_hans: 引力透镜效应
    zh_hant: 重力透鏡效應
    zh_hk: 重力透鏡效應
    zh_sg: 引力透镜效应
    zh_tw: 重力透鏡效應
---
# [[Gravitational_Lens]] 

#is_/same_as :: [[../../../../WikiData/WD~Gravitational_lens,185243|WD~Gravitational_lens,185243]] 
## #has_/text_of_/abstract 

> A gravitational lens is matter, such as a cluster of galaxies or a point particle, 
> that bends light from a distant source as it travels toward an observer. 
> 
> The amount of gravitational lensing is described by Albert Einstein's general theory of relativity. 
> If light is treated as corpuscles travelling at the speed of light, 
> Newtonian physics also predicts the bending of light, 
> **but only half of that predicted by general relativity**.
>
> Orest Khvolson (1924) and Frantisek Link (1936) are generally 
> credited with being the first to discuss the effect in print, 
> but it is more commonly associated with Einstein, 
> who made unpublished calculations on it in 1912 
> and published an article on the subject in 1936.
>
> In 1937, Fritz Zwicky posited that galaxy clusters could act as gravitational lenses, 
> a claim confirmed in 1979 by observation of the Twin QSO SBS 0957+561.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Gravitational%20lens) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Lens|Gravitational_Lens]] 

### #is_/same_as :: [[/_public/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Lens.public|Gravitational_Lens.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Lens.internal|Gravitational_Lens.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Lens.protect|Gravitational_Lens.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Lens.private|Gravitational_Lens.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Lens.personal|Gravitational_Lens.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Lens.secret|Gravitational_Lens.secret]] 

