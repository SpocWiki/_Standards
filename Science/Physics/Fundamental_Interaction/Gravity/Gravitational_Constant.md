---
aliases:
- Bān-iú-ín-le̍k siông-sò͘
- Cavendish gravitational constant
- Constans gravitatis
- constant de la gravitació
- Constanta gravitacionala
- constanta gravitațională
- Constante da gravitación universal
- Constante de gravitación universal
- constante gravitacional universal
- constante gravitationnelle
- costante di gravitazione universale
- Custanti di gravitazzioni universali
- cysonyn disgyrchol
- Den universelle gravitationskonstant
- G
- Grabitasyonal na konstante
- grabitazio unibertsalaren konstantea
- gravita konstanto
- gravitaatiovakio
- gravitacijos konstanta
- Gravitacijska konstanta
- gravitasjonskonstanten
- gravitatieconstante
- gravitational constant
- Gravitational_Constant
- Gravitationskonstante
- gravitationskonstanten
- Gravitatsion daimi
- Gravitatsion doimiy
- gravitatsioonikonstant
- Gravitačná konštanta
- gravitační konstanta
- gravitációs állandó
- Gravitācijas konstante
- Grawitatjuunstaal
- hằng số hấp dẫn
- Newtonian constant of gravitation
- Newtonian gravitational constant
- pemalar graviti
- stała grawitacji
- tairiseach imtharraingthe
- Tetapan gravitasi
- universal gravitational constant
- yerçekimi sabiti
- Σταθερά της βαρύτητας
- гравітаційна стала
- гравітацыйная канстанта
- гравітацыйная пастаянная
- гравитаци константи
- гравитациона константа
- Гравитационна константа
- гравитационная постоянная
- Гравитациска константа
- Гравитациялық тұрақты
- Գրավիտացիոն հաստատուն
- קבוע הכבידה
- ثابت الجاذبية
- ثابت گرانش
- نەگۆڕی ڕاکێشان
- ڤمالر ݢراۏيتي
- کوتله چکیمی ثابیتی
- गुरुत्व स्थिरांक
- गुरुत्वाकर्षक स्थिरांक
- गुरुत्वाकर्षण स्थिर अङ्क
- गुरुत्वीय स्थिराङ्क
- মহাকর্ষ ধ্রুবক
- মহাকৰ্ষণিক ধ্ৰুৱক
- ஈர்ப்பியல் மாறிலி
- గురుత్వస్థిరాంకము
- ഗുരുത്വാകർഷണസ്ഥിരാങ്കം
- ค่าคงตัวความโน้มถ่วง
- ကမ္ဘာမြေဆွဲအားကိန်းသေ
- გრავიტაციული კონსტანტა
- გრავიტაციული მუდმივა
- 万有引力定数
- 万有引力常数
- 萬有引力常數
- 중력 상수
has_id_wikidata: Q18373
instance_of:
- '[[_Standards/WikiData/WD~physical_constant,173227]]'
- '[[_Standards/WikiData/WD~UCUM_constant,83155724]]'
different_from:
- '[[_Standards/WikiData/WD~standard_acceleration_of_free_fall,13400897]]'
- '[[_Standards/WikiData/WD~gravitational_constant,78337869]]'
- '[[_Standards/WikiData/WD~gravitational_coupling_constant,32021]]'
recommended_unit_of_measurement: '[[_Standards/WikiData/WD~cubic_metre_per_kilogram_square_second,78336909]]'
notation: '[[_Standards/WikiData/WD~G,9739]]'
numeric_value: 6.6743e-11
IEV_number: 113-03-17
quantity_symbol_LaTeX_: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>G</mi></mstyle></mrow></math>
image: http://commons.wikimedia.org/wiki/Special:FilePath/Gravitational%20constant.jpg
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q9610%20%28ben%29-Tahmid-%E0%A6%AE%E0%A6%B9%E0%A6%BE%E0%A6%95%E0%A6%B0%E0%A7%8D%E0%A6%B7%20%E0%A6%A7%E0%A7%8D%E0%A6%B0%E0%A7%81%E0%A6%AC%E0%A6%95.wav
Wolfram_Language_unit_code: '"GravitationalConstant"'
UCUM_code: '[G]'
Wolfram_Language_entity_code: Entity["PhysicalConstant", "GravitationalConstant"]
Commons_category: Gravitational constant
dv_has_:
  name_:
    ar: ثابت الجاذبية
    as: মহাকৰ্ষণিক ধ্ৰুৱক
    ast: Constante de gravitación universal
    azb: کوتله چکیمی ثابیتی
    be: гравітацыйная пастаянная
    be_tarask: гравітацыйная канстанта
    bg: Гравитационна константа
    bn: মহাকর্ষ ধ্রুবক
    ca: constant de la gravitació
    ckb: نەگۆڕی ڕاکێشان
    cs: gravitační konstanta
    cv: гравитаци константи
    cy: cysonyn disgyrchol
    da: Den universelle gravitationskonstant
    de: Gravitationskonstante
    el: Σταθερά της βαρύτητας
    en: Newtonian gravitational constant
    eo: gravita konstanto
    es: constante de gravitación universal
    et: gravitatsioonikonstant
    eu: grabitazio unibertsalaren konstantea
    fa: ثابت گرانش
    fi: gravitaatiovakio
    fr: constante gravitationnelle
    frr: Grawitatjuunstaal
    ga: tairiseach imtharraingthe
    gl: Constante da gravitación universal
    he: קבוע הכבידה
    hi: गुरुत्वाकर्षक स्थिरांक
    hr: Gravitacijska konstanta
    hu: gravitációs állandó
    hy: Գրավիտացիոն հաստատուն
    id: Tetapan gravitasi
    it: costante di gravitazione universale
    ja: 万有引力定数
    ka: გრავიტაციული მუდმივა
    kk: Гравитациялық тұрақты
    ko: 중력 상수
    la: Constans gravitatis
    lt: gravitacijos konstanta
    lv: Gravitācijas konstante
    mag: गुरुत्वीय स्थिराङ्क
    mk: Гравитациска константа
    ml: ഗുരുത്വാകർഷണസ്ഥിരാങ്കം
    mr: गुरुत्व स्थिरांक
    ms: pemalar graviti
    ms_arab: ڤمالر ݢراۏيتي
    mul: G
    my: ကမ္ဘာမြေဆွဲအားကိန်းသေ
    nan: Bān-iú-ín-le̍k siông-sò͘
    nb: gravitasjonskonstanten
    ne: गुरुत्वाकर्षण स्थिर अङ्क
    nl: gravitatieconstante
    nn: Gravitasjonskonstanten
    oc: Constanta gravitacionala
    pl: stała grawitacji
    pt: constante gravitacional universal
    ro: constanta gravitațională
    ru: гравитационная постоянная
    scn: Custanti di gravitazzioni universali
    sh: Gravitacijska konstanta
    sk: Gravitačná konštanta
    sl: gravitacijska konstanta
    sr: гравитациона константа
    sr_ec: гравитациона константа
    sv: gravitationskonstanten
    ta: ஈர்ப்பியல் மாறிலி
    te: గురుత్వస్థిరాంకము
    th: ค่าคงตัวความโน้มถ่วง
    tl: Grabitasyonal na konstante
    tr: yerçekimi sabiti
    tt: Gravitatsion daimi
    uk: гравітаційна стала
    uz: Gravitatsion doimiy
    vi: hằng số hấp dẫn
    wuu: 万有引力常数
    xmf: გრავიტაციული კონსტანტა
    yue: 萬有引力常數
    zh: 万有引力常数
    zh_hant: 萬有引力常數
    zh_tw: 萬有引力常數
---

# [[Gravitational_Constant]] 

#is_/same_as :: [[WD~Gravitational_constant,18373]] 

## #has_/text_of_/abstract 

> The gravitational constant is an empirical physical constant 
> that gives the strength of the gravitational field induced by a mass. 
> It is involved in the calculation of gravitational effects 
> in Sir Isaac Newton's law of universal gravitation 
> and in Albert Einstein's theory of general relativity. 
> 
> It is also known as the universal gravitational constant, the Newtonian constant of gravitation, or the Cavendish gravitational constant, denoted by the capital letter G.
>
> In Newton's law, it is the proportionality constant connecting the gravitational force between two bodies with the product of their masses and the inverse square of their distance. In the Einstein field equations, it quantifies the relation between the geometry of spacetime and the stress–energy tensor.
>
> The measured value of the constant is known with some certainty to four significant digits. In SI units, its value is approximately 6.6743×10-11 m3⋅kg-1⋅s-2.
>
> The modern notation of Newton's law involving G was introduced in the 1890s by C. V. Boys. The first implicit measurement with an accuracy within about 1% is attributed to Henry Cavendish in a 1798 experiment.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Gravitational%20constant) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Constant|Gravitational_Constant]] 

### #is_/same_as :: [[/_public/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Constant.public|Gravitational_Constant.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Constant.internal|Gravitational_Constant.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Constant.protect|Gravitational_Constant.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Constant.private|Gravitational_Constant.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Constant.personal|Gravitational_Constant.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Fundamental_Interaction/Gravity/Gravitational_Constant.secret|Gravitational_Constant.secret]] 

