---
aliases:
- an fhisic theoiriciúil
- elméleti fizika
- ffiseg ddamcaniaethol
- fisica teorica
- Fisika teoretikal
- Fisika teoretis
- fisika teorikoa
- Fisika tioritis
- fizică teoretică
- fizik teori
- Fizik teorikel
- fizika teorike
- Fiziko teorik
- fizyka teoretyczna
- Fizîka teorîk
- física teòrica
- Física teórica
- kuramsal fizik
- nəzəri fizika
- Physica theoretica
- Physica theoric
- physique théorique
- teoreetiline füüsika
- teoreetlâš fyysiik
- teoreettinen fysiikka
- teoreettlaž fysikk
- teorehtalaš fysihkka
- teoretická fyzika
- Teoretiese fisika
- teoretisk fysik
- teoretisk fysikk
- teoretična fizika
- Teoretyske natuerkunde
- Teori-teori fisika
- teoria fiziko
- Teoriala fiziko
- Teorijska fizika
- teorinė fizika
- teorētiskā fizika
- Theoreetesch Physik
- theoretical pheesics
- Theoretical physics
- Theoretical_Physics
- theoretische natuurkunde
- Theoretische Physik
- vật lý lý thuyết
- θεωρητική φυσική
- Онолын физик
- теоретик физика
- теоретикон физикæ
- теоретическая физика
- теоретична фізика
- Теоретична физика
- теоријска физика
- Теорилле физика
- теориска физика
- тэарэтычная фізіка
- тэарэтычная фізыка
- Физикалық теория
- физики назарӣ
- տեսական ֆիզիկա
- טעארעטישע פיזיק
- פיזיקה תאורטית
- تھیورٹیکل فزکس
- فيزياء نظرية
- فيزيک تيوري
- فیزیک نظری
- فیزیکی تیۆریک
- نظري فزیک
- نظری طبیعیات
- सैद्धांतिक भौतिकी
- सैद्धान्तिक भौतिकी
- তাত্ত্বিক পদার্থবিদ্যা
- ਸਿਧਾਂਤਕ ਭੌਤਿਕ ਵਿਗਿਆਨ
- கோட்பாட்டுவாத இயற்பியல்
- ฟิสิกส์ทฤษฎี
- თეორეტიკული ფიზიკა
- თეორიული ფიზიკა
- 理論物理学
- 理論物理學
- 理论物理学
- 리론물리학
- 이론물리학
has_id_wikidata: Q18362
said_to_be_the_same_as: '[[_Standards/WikiData/WD~mathematical_physics,156495|WD~mathematical_physics,156495]]'
opposite_of:
- '[[_Standards/WikiData/WD~applied_physics,373065|WD~applied_physics,373065]]'
- '[[_Standards/WikiData/WD~experimental_physics,18366|WD~experimental_physics,18366]]'
instance_of: '[[_Standards/WikiData/WD~branch_of_physics,4162444|WD~branch_of_physics,4162444]]'
practiced_by: '[[_Standards/WikiData/WD~theoretical_physicist,19350898|WD~theoretical_physicist,19350898]]'
part_of: '[[_Standards/WikiData/WD~physics,413|WD~physics,413]]'
subclass_of: '[[_Standards/WikiData/WD~physics,413|WD~physics,413]]'
Dewey_Decimal_Classification: 530.1
OmegaWiki_Defined_Meaning: 795609
image: http://commons.wikimedia.org/wiki/Special:FilePath/%C3%89cole%20de%20Physique%20des%20Houches%20%28Les%20Houches%20Physics%20School%29%20main%20lecture%20hall%201972.jpg
Stack_Exchange_tag: https://hsm.stackexchange.com/tags/theoretical-physics
Commons_category: Theoretical physics
dv_has_:
  name_:
    af: Teoretiese fisika
    ar: فيزياء نظرية
    ast: Física teórica
    az: nəzəri fizika
    be: тэарэтычная фізіка
    be_tarask: тэарэтычная фізыка
    bg: Теоретична физика
    bho: सैद्धांतिक भौतिकी
    bn: তাত্ত্বিক পদার্থবিদ্যা
    br: Fizik teorikel
    bs: Teorijska fizika
    ca: física teòrica
    ckb: فیزیکی تیۆریک
    cs: teoretická fyzika
    cv: Теорилле физика
    cy: ffiseg ddamcaniaethol
    da: teoretisk fysik
    de: theoretische Physik
    diq: Fiziko teorik
    el: θεωρητική φυσική
    en: theoretical physics
    eo: teoria fiziko
    es: física teórica
    et: teoreetiline füüsika
    eu: fisika teorikoa
    fa: فیزیک نظری
    fi: teoreettinen fysiikka
    fr: physique théorique
    fy: Teoretyske natuerkunde
    ga: an fhisic theoiriciúil
    gl: Física teórica
    he: פיזיקה תאורטית
    hi: सैद्धान्तिक भौतिकी
    hr: teorijska fizika
    hu: elméleti fizika
    hy: տեսական ֆիզիկա
    ia: Physica theoric
    id: Fisika teoretis
    io: Teoriala fiziko
    it: fisica teorica
    ja: 理論物理学
    ka: თეორეტიკული ფიზიკა
    kk: Физикалық теория
    ko: 이론물리학
    ko-kp: 리론물리학
    ku: Fizîka teorîk
    la: Physica theoretica
    lb: Theoreetesch Physik
    lez: теоретик физика
    lt: teorinė fizika
    lv: teorētiskā fizika
    map_bms: Teori-teori fisika
    mk: теориска физика
    mn: Онолын физик
    ms: fizik teori
    ms_arab: فيزيک تيوري
    nb: teoretisk fysikk
    nl: theoretische natuurkunde
    nn: teoretisk fysikk
    oc: fisica teorica
    os: теоретикон физикæ
    pa: ਸਿਧਾਂਤਕ ਭੌਤਿਕ ਵਿਗਿਆਨ
    pl: fizyka teoretyczna
    pnb: تھیورٹیکل فزکس
    ps: نظري فزیک
    pt: física teórica
    pt_br: física teórica
    ro: fizică teoretică
    ru: теоретическая физика
    sco: theoretical pheesics
    se: teorehtalaš fysihkka
    sh: Teorijska fizika
    sk: teoretická fyzika
    sl: teoretična fizika
    smn: teoreetlâš fyysiik
    sms: teoreettlaž fysikk
    sq: fizika teorike
    sr: теоријска физика
    su: Fisika tioritis
    sv: teoretisk fysik
    ta: கோட்பாட்டுவாத இயற்பியல்
    tg: физики назарӣ
    th: ฟิสิกส์ทฤษฎี
    tl: theoretical physics
    tr: kuramsal fizik
    tt: теоретик физика
    uk: теоретична фізика
    ur: نظری طبیعیات
    vi: vật lý lý thuyết
    war: Fisika teoretikal
    wuu: 理论物理学
    xmf: თეორიული ფიზიკა
    yi: טעארעטישע פיזיק
    yue: 理論物理學
    zh: 理论物理学
    zh_hans: 理论物理学
    zh_hant: 理論物理學
---

# [[Theoretical_Physics]] 

#is_/same_as :: [[../../WikiData/WD~Theoretical_physics,18362|WD~Theoretical_physics,18362]]  

## #has_/text_of_/abstract 

> Theoretical physics is a branch of physics that employs mathematical models 
> and abstractions of physical objects and systems 
> to rationalize, explain, and predict natural phenomena. 
> 
> This is in contrast to experimental physics, 
> which uses experimental tools to probe these phenomena.
>
> The advancement of science generally 
> depends on the interplay between experimental studies and theory. 
> 
> In some cases, theoretical physics adheres to standards of mathematical rigour 
> while giving little weight to experiments and observations. 
> 
> For example, while developing special relativity, 
> Albert Einstein was concerned with the Lorentz transformation 
> which left Maxwell's equations invariant, 
> but was apparently uninterested in the Michelson–Morley experiment 
> on Earth's drift through a luminiferous aether. 
> 
> Conversely, Einstein was awarded the Nobel Prize for explaining the photoelectric effect, 
> previously an experimental result lacking a theoretical formulation.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Theoretical%20physics) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Theoretical_Physics|Theoretical_Physics]] 

### #is_/same_as :: [[/_public/Science/Physics/Theoretical_Physics.public|Theoretical_Physics.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Theoretical_Physics.internal|Theoretical_Physics.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Theoretical_Physics.protect|Theoretical_Physics.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Theoretical_Physics.private|Theoretical_Physics.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Theoretical_Physics.personal|Theoretical_Physics.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Theoretical_Physics.secret|Theoretical_Physics.secret]] 

