---
aliases:
- athari ya Doppler
- Dopler effekti
- Doplera efekts
- Doplerio efektas
- Doplerov efekat
- Doppler efektu
- Doppler effect
- Doppler Effekt
- Doppler etkisi
- Doppler shift
- Doppler-efekte
- Doppler-effek
- Doppler-Effekt
- Doppler-effektus
- Doppler-ilmiö
- dopplereffect
- dopplereffekt
- dopplerhrif
- Doppleri efekt
- Dopplerjev pojav
- Dopplerov efekt
- Dopplerov jav
- Doppler_Effect
- Dopplerův jev
- Efecte Doppler
- efecto Doppler
- efectul Doppler
- Efed Doppler
- efeito Doppler
- efek Doppler
- efekt Dopplera
- efekti i Dopplerit
- efet Doppler
- efeutu Doppler
- effaith Doppler
- effectus Doppler
- effet Doppler
- effetto Doppler
- efiko de Doppler
- efè Doppler
- efècte Doppler
- epekto Doppler
- epektong Doppler
- hiệu ứng Doppler
- Iarmhairt Doppler
- kesan Doppler
- φαινόμενο Ντόπλερ
- Доплер эффекчĕ
- Доплер құбылысы
- Доплеров ефекат
- Доплеров ефект
- Допплер эффекты
- ефект Доплера
- эфект Доплера
- эффект Доплера
- эфэкт Доплера
- Դոպլերի էֆեկտ
- אפקט דופלר
- اثر دوپلر
- تأثير دوبلر
- ڈوپلر ایفکٹ
- کاریگەریی دۆپلەر
- डप्लर इफेक्ट
- डॉप्लर प्रभाव
- ড'পলাৰ প্ৰভাৱ
- ডপলার ক্রিয়া
- ਡਾਪਲਰ ਪ੍ਰਭਾਵ
- ଡୋପ୍ଲର୍ ପ୍ରଭାବ
- டாப்ளர் விளைவு
- డాప్లర్ ప్రభావం
- ಡಾಪ್ಲರ್ ಪರಿಣಾಮ
- ഡോപ്ലർ പ്രഭാവം
- ඩොප්ලර් ආචරණය
- ปรากฏการณ์ด็อพเพลอร์
- ဒေါ့ပလာ အကျိုးသက်ရောက်မှု
- დოპლერის ეფექტი
- ドップラー効果
- 多普勒效应
- 杜卜勒效應
- 都卜勒效應
- 도플러 효과
has_id_wikidata: Q76436
named_after: '[[_Standards/WikiData/WD~Christian_Doppler,84284|WD~Christian_Doppler,84284]]'
discoverer_or_inventor: '[[_Standards/WikiData/WD~Christian_Doppler,84284|WD~Christian_Doppler,84284]]'
instance_of: '[[_Standards/WikiData/WD~physical_phenomenon,1293220|WD~physical_phenomenon,1293220]]'
described_by_source:
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138|WD~Granat_Encyclopedic_Dictionary,4532138]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._3,124737616|WD~Armenian_Soviet_Encyclopedia,_vol._3,124737616]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
defining_formula: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>f</mi><mo stretchy="false">=</mo><mrow data-mjx-texclass="INNER"><mo data-mjx-texclass="OPEN">(</mo><mrow data-mjx-texclass="ORD"><mfrac><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mi>c</mi><mo stretchy="false">+</mo><msub><mi>v</mi><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mtext>r</mtext></mrow></mrow></msub></mrow></mrow><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mi>c</mi><mo stretchy="false">+</mo><msub><mi>v</mi><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mtext>s</mtext></mrow></mrow></msub></mrow></mrow></mfrac></mrow><mo data-mjx-texclass="CLOSE">)</mo></mrow><msub><mi>f</mi><mrow data-mjx-texclass="ORD"><mn>0</mn></mrow></msub></mstyle></mrow></math>
time_of_discovery_or_invention: 1842-01-01T00:00:00Z
OmegaWiki_Defined_Meaning: 427379
Commons_gallery: Doppler effect
Commons_category: Doppler effect
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/Doppler%20effect%20diagrammatic.png
video:
- http://commons.wikimedia.org/wiki/Special:FilePath/Dopplereffectsourcemovingrightatmach0.7.gif
- http://commons.wikimedia.org/wiki/Special:FilePath/Dopplerfrequenz.gif
- http://commons.wikimedia.org/wiki/Special:FilePath/Juli%202016%20-%20Spoedtransport%2C%20Huisarts%2C%20Brandweer%2C%20Politie%20en%20Ambulances%20met%20spoed%20in%20Rotterdam%20-451.webm
image: http://commons.wikimedia.org/wiki/Special:FilePath/Picture%20of%20the%20first%20%27wall%20formula%27%20in%20the%20city%20of%20Utrecht%2001.jpg
audio: http://commons.wikimedia.org/wiki/Special:FilePath/Speeding-car-horn%20doppler%20effect%20sample.ogg
Stack_Exchange_tag: https://physics.stackexchange.com/tags/doppler-effect
UMLS_CUI: C0013041
Wolfram_Language_entity_code: Entity["PhysicalEffect", "DopplerEffect"]
MeSH_tree_code: G01.226
Krugosvet_article: nauka_i_tehnika/fizika/DOPLERA_EFFEKT.html
dv_has_:
  name_:
    af: Doppler-effek
    ar: تأثير دوبلر
    as: ড'পলাৰ প্ৰভাৱ
    ast: efeutu Doppler
    az: Dopler effekti
    be: эфект Доплера
    be_tarask: эфэкт Доплера
    bg: Доплеров ефект
    bn: ডপলার ক্রিয়া
    br: Efed Doppler
    bs: Dopplerov efekt
    ca: Efecte Doppler
    ckb: کاریگەریی دۆپلەر
    cs: Dopplerův jev
    cv: Доплер эффекчĕ
    cy: effaith Doppler
    da: dopplereffekt
    de: Dopplereffekt
    el: φαινόμενο Ντόπλερ
    en: Doppler effect
    eo: efiko de Doppler
    es: efecto Doppler
    et: Doppleri efekt
    eu: Doppler efektu
    fa: اثر دوپلر
    fi: Doppler-ilmiö
    fr: effet Doppler
    ga: Iarmhairt Doppler
    gl: efecto Doppler
    gsw: Doppler-Effekt
    he: אפקט דופלר
    hi: डॉप्लर प्रभाव
    hr: Dopplerov efekt
    ht: efè Doppler
    hu: Doppler-effektus
    hy: Դոպլերի էֆեկտ
    id: efek Doppler
    is: dopplerhrif
    it: effetto Doppler
    ja: ドップラー効果
    ka: დოპლერის ეფექტი
    kk: Доплер құбылысы
    kn: ಡಾಪ್ಲರ್ ಪರಿಣಾಮ
    ko: 도플러 효과
    la: effectus Doppler
    lt: Doplerio efektas
    lv: Doplera efekts
    mk: Доплеров ефект
    ml: ഡോപ്ലർ പ്രഭാവം
    ms: kesan Doppler
    my: ဒေါ့ပလာ အကျိုးသက်ရောက်မှု
    nb: dopplereffekt
    ne: डप्लर इफेक्ट
    nl: dopplereffect
    nn: dopplereffekt
    nov: Doppler-efekte
    oc: efècte Doppler
    or: ଡୋପ୍ଲର୍ ପ୍ରଭାବ
    pa: ਡਾਪਲਰ ਪ੍ਰਭਾਵ
    pl: efekt Dopplera
    pms: efet Doppler
    pnb: ڈوپلر ایفکٹ
    pt: efeito Doppler
    ro: efectul Doppler
    ru: эффект Доплера
    sco: Doppler effect
    sh: Doplerov efekat
    si: ඩොප්ලර් ආචරණය
    sk: Dopplerov jav
    sl: Dopplerjev pojav
    sq: efekti i Dopplerit
    sr: Доплеров ефекат
    sv: dopplereffekt
    sw: athari ya Doppler
    ta: டாப்ளர் விளைவு
    te: డాప్లర్ ప్రభావం
    th: ปรากฏการณ์ด็อพเพลอร์
    tl: epektong Doppler
    tr: Doppler etkisi
    tt: Допплер эффекты
    uk: ефект Доплера
    ur: ڈوپلر ایفکٹ
    uz: Dopler effekti
    vi: hiệu ứng Doppler
    war: epekto Doppler
    wuu: 多普勒效应
    yue: 杜卜勒效應
    zh: 多普勒效应
    zh_hans: 多普勒效应
    zh_tw: 都卜勒效應
---

# [[Doppler_Effect]] 

#is_/same_as :: [[../../../WikiData/WD~Doppler_effect,76436|WD~Doppler_effect,76436]] 

## #has_/text_of_/abstract 

> The Doppler effect (also Doppler shift) is the change in the frequency of a wave 
> in relation to an observer who is moving relative to the source of the wave. 
> 
> The Doppler effect is named after the physicist Christian Doppler, who described the phenomenon in 1842. 
> A common example of Doppler shift is the change of pitch heard 
> when a vehicle sounding a horn approaches and recedes from an observer. 
> Compared to the emitted frequency, the received frequency is higher during the approach, 
> identical at the instant of passing by, and lower during the recession.
>
> When the source of the sound wave is moving towards the observer, 
> each successive cycle of the wave is emitted from a position closer to the observer than the previous cycle. 
> Hence, from the observer's perspective, the time between cycles is reduced, 
> meaning the frequency is increased. 
> 
> Conversely, if the source of the sound wave is moving away from the observer, 
> each cycle of the wave is emitted from a position farther from the observer than the previous cycle, 
> so the arrival time between successive cycles is increased, thus reducing the frequency.
>
> For waves that propagate in a medium, such as sound waves, 
> the velocity of the observer and of the source are relative to the medium in which the waves are transmitted. 
> The total Doppler effect in such cases may therefore result from motion of the source, 
> motion of the observer, motion of the medium, or any combination thereof. 
> 
> For waves propagating in vacuum, as is possible for electromagnetic waves or gravitational waves, 
> only the difference in velocity between the observer and the source needs to be considered.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Doppler%20effect) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Wave/Doppler_Effect|Doppler_Effect]] 

### #is_/same_as :: [[/_public/Science/Physics/Wave/Doppler_Effect.public|Doppler_Effect.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Wave/Doppler_Effect.internal|Doppler_Effect.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Wave/Doppler_Effect.protect|Doppler_Effect.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Wave/Doppler_Effect.private|Doppler_Effect.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Wave/Doppler_Effect.personal|Doppler_Effect.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Wave/Doppler_Effect.secret|Doppler_Effect.secret]] 

