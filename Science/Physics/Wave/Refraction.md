---
aliases:
- athraonadh
- bangos lūžis
- Brechung
- Chiat-siā
- Dabee darbuu
- Errefrakzioa
- fénytörés
- Gaismas laušana
- khúc xạ
- Kırılım
- lichtbreking
- Ligbreking
- Liichtrefraktioun
- Ljósbrot
- lom svetlobe
- lysbrytning
- pembiasan
- Pemlèngsongan
- plygiant
- Refraccion
- refracció
- refracción
- Refractio
- refraction
- refracție
- Refraición
- refrakce
- Refrakcija
- refrakcja
- Refrakshion
- Refraksi
- Refraksiya
- refraksjon
- Refraksyon
- refraktion
- refrakto
- refraktsioon
- refração
- Rifrassion
- Rifrazion
- rifrazione
- Rifrazzioni
- Rutsaudzo
- réfraction
- taittuminen
- toijum
- İşığın sınması
- διάθλαση
- заломлення
- Лом світла
- пераламленьне
- праламленне
- прекршување
- преламање светлости
- преломление
- Пречупване
- Рефракци
- Рефракция
- Ռեֆրակցիա
- ברעכונג
- שבירה
- انعطاف
- انكسار
- شکانەوەی ڕووناکی
- شکست نور
- अपवर्तन
- अपवर्तनम्
- আলোর প্রতিসরণ
- প্ৰতিসৰণ
- ਅਪਵਰਤਨ
- ஒளி முறிவு
- వక్రీభవనం
- ವಕ್ರೀಭವನ
- അപവർത്തനം
- වර්තනය
- การหักเห
- ການຫັກລົບ
- የብርሃን ስብረት
- 屈折
- 折射
- 굴절
has_id_wikidata: Q72277
has_effect: '[[_Standards/WikiData/WD~reflection,165939]]'
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~The_Nuttall_Encyclopædia,3181656]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
different_from: '[[_Standards/WikiData/WD~refractory_period,1736732]]'
subclass_of: '[[_Standards/WikiData/WD~surface_phenomenon,4366377]]'
Commons_gallery: Refraction
Commons_category: Refraction
OmegaWiki_Defined_Meaning: 666035
image: http://commons.wikimedia.org/wiki/Special:FilePath/Refraction%20animation.gif
Iconclass_notation: 49E252
video: http://commons.wikimedia.org/wiki/Special:FilePath/Refraction%20of%20a%20quantum%20particle.webm
Wolfram_Language_entity_code: Entity["PhysicalEffect", "Refraction"]
dv_has_:
  name_:
    af: Ligbreking
    am: የብርሃን ስብረት
    ar: انكسار
    as: প্ৰতিসৰণ
    ast: Refraición
    az: İşığın sınması
    be: праламленне
    be_tarask: пераламленьне
    bew: Pemlèngsongan
    bg: Пречупване
    bn: আলোর প্রতিসরণ
    ca: refracció
    ckb: شکانەوەی ڕووناکی
    cs: refrakce
    cv: Рефракци
    cy: plygiant
    da: refraktion
    de: Brechung
    el: διάθλαση
    en: refraction
    eo: refrakto
    es: refracción
    et: refraktsioon
    eu: Errefrakzioa
    fa: شکست نور
    fi: taittuminen
    fr: réfraction
    ga: athraonadh
    gl: Refracción
    gsw: Brechung
    he: שבירה
    hi: अपवर्तन
    hr: Refrakcija
    ht: Refraksyon
    hu: fénytörés
    hy: Ռեֆրակցիա
    id: Refraksi
    is: Ljósbrot
    it: rifrazione
    ja: 屈折
    jv: Refraksi
    kk: Рефракция
    kn: ವಕ್ರೀಭವನ
    ko: 굴절
    ky: Рефракция
    la: Refractio
    lb: Liichtrefraktioun
    lmo: Rifrazion
    lo: ການຫັກລົບ
    lt: bangos lūžis
    lv: Gaismas laušana
    lzh: 折射
    mk: прекршување
    ml: അപവർത്തനം
    mr: अपवर्तन
    ms: pembiasan
    nan: Chiat-siā
    nb: lysbrytning
    nl: lichtbreking
    nn: refraksjon
    oc: Refraccion
    om: Dabee darbuu
    pa: ਅਪਵਰਤਨ
    pcm: Refrakshion
    pl: refrakcja
    pms: Rifrassion
    pt: refração
    ro: refracție
    ru: преломление
    rue: Лом світла
    sa: अपवर्तनम्
    scn: Rifrazzioni
    sco: refraction
    sd: انعطاف
    sh: Refrakcija
    si: වර්තනය
    sl: lom svetlobe
    smn: toijum
    sn: Rutsaudzo
    sr: преламање светлости
    sv: refraktion
    ta: ஒளி முறிவு
    te: వక్రీభవనం
    th: การหักเห
    tl: refraction
    tr: Kırılım
    uk: заломлення
    ur: انعطاف
    uz: Refraksiya
    vi: khúc xạ
    wuu: 折射
    yi: ברעכונג
    yue: 折射
    zh: 折射
    zh_cn: 折射
    zh_hans: 折射
    zh_tw: 折射
---

# [[Refraction]] 

#is_/same_as :: [[WD~Refraction,72277]] 

## #has_/text_of_/abstract 

> In physics, **Refraction** is the redirection of a wave 
> as it passes from one medium to another. 
> 
> The redirection can be caused by the wave's change in speed 
> or by a change in the medium. 
> 
> Refraction of light is the most commonly observed phenomenon, 
> but other waves such as sound waves and water waves also experience refraction. 
> 
> How much a wave is refracted is determined by the change in wave speed 
> and the initial direction of wave propagation relative to the direction of change in speed.
>
> Optical prisms and lenses use refraction to redirect light, as does the human eye. 
> The refractive index of materials varies with the wavelength of light, 
> and thus the angle of the refraction also varies correspondingly. 
> 
> This is called dispersion and allows prisms  and raindrops in rainbows 
> to divide white light into its constituent spectral colors.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Refraction) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Wave/Refraction|Refraction]] 

### #is_/same_as :: [[/_public/Science/Physics/Wave/Refraction.public|Refraction.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Wave/Refraction.internal|Refraction.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Wave/Refraction.protect|Refraction.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Wave/Refraction.private|Refraction.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Wave/Refraction.personal|Refraction.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Wave/Refraction.secret|Refraction.secret]] 

