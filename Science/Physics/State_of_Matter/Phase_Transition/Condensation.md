---
aliases:
- Aneznez
- comhdhlúthú
- Condensa
- Condensacion
- condensació
- condensación
- Condensare
- condensatie
- Condensation
- condensazione
- condensação
- cseppfolyósodás
- cyddwyso
- Gluthyans
- kapalnění
- Kondansasyon
- Kondensacija
- Kondensacïya
- kondensado
- Kondensasi
- Kondensasie
- Kondensasiya
- kondensasjon
- Kondensasyon
- Kondensation
- kondensatsioon
- Kondensatsiya
- Kondensaсiya
- Kondensering
- Kondensācija
- Kondentsazio
- Kondenzacija
- Kondenzimi
- Kubundira
- Kuondensacėjė
- lavvâm
- Mmiri na-ekpo ọkụ
- ngưng tụ
- pemeluwapan
- Sandaro
- skraplanie
- Skvapalňovanie
- tiivistyminen
- Yoğuşma
- υγροποίηση
- вуӟектон
- кандэнсацыя
- Кондензација
- Кондензация
- конденсація
- Конденсаци
- конденсация
- сыекчага әйләнү
- Կոնդենսացում
- התעבות
- קאנדענסירונג
- تكثيف
- عمل تکثیف
- كوندەنساتسىييا
- میعان
- چڕبوونەوە
- संघनन
- ঘনীভবন
- குளிர்வித்தல்
- സാന്ദ്രീകരണം
- การควบแน่น
- 凝結
- 凝縮
- 凝结
- 응축
has_id_wikidata: Q166583
instance_of: '[[_Standards/WikiData/WD~phase_change,185357|WD~phase_change,185357]]'
opposite_of: '[[_Standards/WikiData/WD~vaporization,6452502|WD~vaporization,6452502]]'
has_effect: '[[_Standards/WikiData/WD~liquid_state_of_matter,6761456|WD~liquid_state_of_matter,6761456]]'
described_by_source:
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632|WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632]]'
uses: '[[_Standards/WikiData/WD~gaseous_state_of_matter,29147941|WD~gaseous_state_of_matter,29147941]]'
studied_by: '[[_Standards/WikiData/WD~thermodynamics,11473|WD~thermodynamics,11473]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Phase%20change%20-%20en.svg
Digital_Atlas_of_Idaho_URL: https://digitalatlas.cose.isu.edu/clima/imaging/conden.htm
Iconclass_notation: 22A3121
Commons_category: Condensation
dv_has_:
  name_:
    af: Kondensasie
    ar: تكثيف
    ast: Condensación
    az: Kondensasiya
    bcl: Kondensasyon
    be: кандэнсацыя
    be_tarask: кандэнсацыя
    bg: Кондензация
    bn: ঘনীভবন
    bs: Kondenzacija
    ca: condensació
    ckb: چڕبوونەوە
    cs: kapalnění
    cy: cyddwyso
    da: Kondensering
    de: Kondensation
    el: υγροποίηση
    en: condensation
    eo: kondensado
    es: condensación
    et: kondensatsioon
    eu: Kondentsazio
    fa: میعان
    fi: tiivistyminen
    fr: condensation
    ga: comhdhlúthú
    gl: condensación
    ha: Sandaro
    he: התעבות
    hi: संघनन
    hr: kondenzacija
    ht: Kondansasyon
    hu: cseppfolyósodás
    hy: Կոնդենսացում
    id: Kondensasi
    ig: Mmiri na-ekpo ọkụ
    it: condensazione
    ja: 凝縮
    kaa: Kondensaсiya
    kab: Aneznez
    kk: Конденсация
    kk_arab: كوندەنساتسىييا
    kk-cn: كوندەنساتسىييا
    kk_cyrl: Конденсация
    kk-kz: Конденсация
    kk_latn: Kondensacïya
    kk-tr: Kondensacïya
    ko: 응축
    kw: Gluthyans
    ky: Конденсация
    lmo: Condensa
    lt: Kondensacija
    lv: Kondensācija
    mk: Кондензација
    ml: സാന്ദ്രീകരണം
    mn: Конденсаци
    ms: pemeluwapan
    nb: kondensasjon
    nl: condensatie
    nn: kondensasjon
    oc: Condensacion
    pl: skraplanie
    pt: condensação
    ro: Condensare
    ru: конденсация
    sgs: Kuondensacėjė
    sh: Kondenzacija
    sk: Skvapalňovanie
    sl: kondenzacija
    smn: lavvâm
    sn: Kubundira
    sq: Kondenzimi
    sr: кондензација
    sr_ec: кондензација
    sr_el: kondenzacija
    sv: kondensering
    sw: condensation
    ta: குளிர்வித்தல்
    th: การควบแน่น
    tl: condensation
    tr: Yoğuşma
    tt: сыекчага әйләнү
    tt_cyrl: сыекчага әйләнү
    udm: вуӟектон
    uk: конденсація
    ur: عمل تکثیف
    uz: Kondensatsiya
    vi: ngưng tụ
    war: Kondensasyon
    wuu: 凝结
    yi: קאנדענסירונג
    yue: 凝結
    zh: 凝結
    zh_cn: 凝结
    zh_hans: 凝结
    zh_hant: 凝結
    zh_hk: 凝結
    zh_sg: 凝结
    zh_tw: 凝結
---

# [[Condensation]] 

#has_/complement :: [[Vaporization]]  
#is_/same_as :: [[../../../../WikiData/WD~Condensation,166583|WD~Condensation,166583]] 

## #has_/text_of_/abstract 

> **Condensation** is the change of the [[../../State_of_Matter|state of matter]] from the [[gas]] phase into the [[liquid]] phase, 
> and is the reverse of [[Vaporization]]. 
> 
> The word most often refers to the water cycle. 
> It can also be defined as the change in the state of water vapor to liquid water 
> when in contact with a liquid or solid surface or cloud condensation nuclei within the atmosphere. 
> 
> When the transition happens from the gaseous phase into the solid phase directly, 
> the change is called deposition. 
> 
> Condensation is usually associated with water.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Condensation) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/State_of_Matter/Phase_Transition/Condensation|Condensation]] 

### #is_/same_as :: [[/_public/Science/Physics/State_of_Matter/Phase_Transition/Condensation.public|Condensation.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/State_of_Matter/Phase_Transition/Condensation.internal|Condensation.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/State_of_Matter/Phase_Transition/Condensation.protect|Condensation.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/State_of_Matter/Phase_Transition/Condensation.private|Condensation.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/State_of_Matter/Phase_Transition/Condensation.personal|Condensation.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/State_of_Matter/Phase_Transition/Condensation.secret|Condensation.secret]] 

