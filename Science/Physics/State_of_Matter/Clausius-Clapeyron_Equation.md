---
has_id_wikidata: Q745416
dv_has_:
  name_:
    ar: معادلة كلاوزيوس-كلابيرون
    be: Ураўненне Клапейрона—Клаўзіуса
    bn: ক্লাউজিউস-ক্লাপেরোঁ সমীকরণ
    ca: Equació de Clausius-Clapeyron
    cs: Clausiova-Clapeyronova rovnice
    de: Clausius-Clapeyron-Gleichung
    en: Clausius–Clapeyron relation
    eo: Ekvacio de Clausius-Clapeyron
    es: Relación de Clausius-Clapeyron
    et: Clapeyroni-Clausiuse võrrand
    fa: معادله کلازیوس-کلاپیرون
    fi: Clausiuksen–Clapeyronin yhtälö
    fr: formule de Clausius-Clapeyron
    he: משוואת קלאוזיוס-קלפרון
    hu: Clausius–Clapeyron-egyenlet
    hy: Կլապեյրոն-Կլաուզիուսի հավասարում
    it: equazione di Clapeyron
    ja: クラウジウス-クラペイロンの式
    kk: Клапейрон - Клаузиус теңдеуі
    kk_arab: كلاپەيرون - كلاۋزىيۋس تەڭدەۋى
    kk_cyrl: Клапейрон - Клаузиус теңдеуі
    kk_latn: Klapeýron - Klawzïws teñdewi
    ko: 클라우지우스-클라페이롱 방정식
    ky: Клапейрон—Клаузиус теңдемеси
    nb: Clausius-Clapeyron-ligningen
    nl: clausius-clapeyronvergelijking
    oc: Emile Clapeyron
    pl: równanie Clausiusa-Clapeyrona
    ro: relația Clausius–Clapeyron
    ru: Уравнение Клапейрона — Клаузиуса
    sl: Clausius-Clapeyronova enačba
    sr: Клаузијус-Клаперон однос
    sv: Clausius–Clapeyron-ekvationen
    ta: கிளாசியசு-கிளாப்பிரான் சமன்பாடு
    uk: Співвідношення Клаузіуса — Клапейрона
    uz: Klapeyron-klauzius tenglamasi
    vi: Phương trình Clausius-Clapeyron
    zh: 克劳修斯－克拉佩龙方程
    zh_hans: 克劳修斯－克拉佩龙方程
    zh_hant: 克勞修斯－克拉佩龍方程
    zh_hk: 克勞修斯－克拉佩龍方程
aliases:
- Clapeyroni-Clausiuse võrrand
- Clausiova-Clapeyronova rovnice
- Clausiuksen–Clapeyronin yhtälö
- Clausius-Clapeyron-Gleichung
- Clausius-Clapeyron-ligningen
- Clausius-Clapeyronova enačba
- clausius-clapeyronvergelijking
- Clausius–Clapeyron relation
- Clausius–Clapeyron-egyenlet
- Clausius–Clapeyron-ekvationen
- Ekvacio de Clausius-Clapeyron
- Emile Clapeyron
- Equació de Clausius-Clapeyron
- equazione di Clapeyron
- formule de Clausius-Clapeyron
- Klapeyron-klauzius tenglamasi
- Klapeýron - Klawzïws teñdewi
- Phương trình Clausius-Clapeyron
- Relación de Clausius-Clapeyron
- relația Clausius–Clapeyron
- równanie Clausiusa-Clapeyrona
- Клапейрон - Клаузиус теңдеуі
- Клапейрон—Клаузиус теңдемеси
- Клаузијус-Клаперон однос
- Співвідношення Клаузіуса — Клапейрона
- Ураўненне Клапейрона—Клаўзіуса
- Уравнение Клапейрона — Клаузиуса
- Կլապեյրոն-Կլաուզիուսի հավասարում
- משוואת קלאוזיוס-קלפרון
- كلاپەيرون - كلاۋزىيۋس تەڭدەۋى
- معادلة كلاوزيوس-كلابيرون
- معادله کلازیوس-کلاپیرون
- ক্লাউজিউস-ক্লাপেরোঁ সমীকরণ
- கிளாசியசு-கிளாப்பிரான் சமன்பாடு
- クラウジウス-クラペイロンの式
- 克劳修斯－克拉佩龙方程
- 克勞修斯－克拉佩龍方程
- 클라우지우스-클라페이롱 방정식
---

# [[Clausius-Clapeyron_Equation]] 

#has_/formula_ :: ln⁡(P2/P1) = -ΔHvap/R(1/T2-1/T1)
#has_/formula_/latex :: $\ln\left(\frac{P_2}{P_1}\right) = -\frac{\Delta H_{vap}}{R} \left(\frac{1}{T_2} - \frac{1}{T_1}\right)$ 

## #has_/text_of_/abstract 

> The Clausius–Clapeyron relation, in chemical thermodynamics, 
> specifies the temperature dependence of pressure, most importantly vapor pressure, 
> at a discontinuous phase transition between two phases of matter of a single constituent. 
> 
> It is named after Rudolf Clausius and Benoît Paul Émile Clapeyron. 
> However, this relation was in fact originally derived by Sadi Carnot 
> in his Reflections on the Motive Power of Fire, which was published in 1824 
> but largely ignored until it was rediscovered by Clausius, Clapeyron, and Lord Kelvin decades later.  
> 
> Kelvin said of Carnot's argument that 
> "nothing in the whole range of Natural Philosophy is more remarkable 
> than the establishment of general laws by such a process of reasoning."
>
> Kelvin and his brother James Thomson confirmed the relation experimentally in 1849–50, 
> and it was historically important 
> as a very early successful application of theoretical thermodynamics.  
> Its relevance to meteorology and climatology is 
> the increase of the water-holding capacity of the atmosphere 
> by about 7% for every 1 °C (1.8 °F) rise in temperature.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Clausius%E2%80%93Clapeyron%20relation) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/State_of_Matter/Clausius-Clapeyron_Equation|Clausius-Clapeyron_Equation]] 

### #is_/same_as :: [[/_public/Science/Physics/State_of_Matter/Clausius-Clapeyron_Equation.public|Clausius-Clapeyron_Equation.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/State_of_Matter/Clausius-Clapeyron_Equation.internal|Clausius-Clapeyron_Equation.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/State_of_Matter/Clausius-Clapeyron_Equation.protect|Clausius-Clapeyron_Equation.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/State_of_Matter/Clausius-Clapeyron_Equation.private|Clausius-Clapeyron_Equation.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/State_of_Matter/Clausius-Clapeyron_Equation.personal|Clausius-Clapeyron_Equation.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/State_of_Matter/Clausius-Clapeyron_Equation.secret|Clausius-Clapeyron_Equation.secret]] 

