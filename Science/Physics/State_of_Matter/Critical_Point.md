---
aliases:
- creetical pynt
- critical point
- critical state
- Critical_Point
- fazalıq ötw temperatwrası
- kriitiline punkt
- kriittinen piste
- kriittisâš čuogâstâh
- kritický bod
- kritieke punt
- kritik nokta
- kritik nuqta
- kritikus pont
- kritisch Punkt
- kritisch punt
- kritischer Punkt
- kritisk punkt
- kritična tačka
- kritična temperatura
- kritična točka
- lîm-kài-tiám
- point critique
- ponch critic
- ponto crítico
- punct critic
- puncto critic
- punctum criticum
- punkt krytyczny
- punt crític
- punto critico
- punto crítico
- puntong kritikal
- puntu crìticu
- puntu críticu
- puntu kritiko
- sojla punkto
- titik genting
- titik kritis
- điểm tới hạn
- κρίσιμο σημείο
- критическая точка
- критична тачка
- критична точка
- крытычны пункт
- нуқтаи буҳронӣ
- синкерлĕ пăнчă
- фазалық өту температурасы
- Կրիտիկական կետ
- נקודה קריטית
- فازالىق ٴوتۋ تەمپەراتۋراسى
- فاصل نقطہ
- نقطة حرجة
- نقطه بحرانی
- कृटिकल फुति
- क्रांतिक बिन्दु
- ক্রান্তিবিন্দু
- ಅಧಿವಾಹಕತೆ
- อุณหภูมิวิกฤต
- 临界点
- 臨界点
- 臨界點
- 림계점
- 임계점
has_id_wikidata: Q111059
subclass_of: '[[_Standards/WikiData/WD~thermodynamic_state,230875|WD~thermodynamic_state,230875]]'
different_from: '[[_Standards/WikiData/WD~critical_point,577705|WD~critical_point,577705]]'
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138|WD~Granat_Encyclopedic_Dictionary,4532138]]'
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632|WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632]]'
has_part_s_:
- '[[_Standards/WikiData/WD~critical_pressure,3402613|WD~critical_pressure,3402613]]'
- '[[_Standards/WikiData/WD~critical_temperature,109330092|WD~critical_temperature,109330092]]'
studied_by: '[[_Standards/WikiData/WD~thermodynamics,11473|WD~thermodynamics,11473]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Punkt%20krytyczny%20220px.png
Commons_category: Critical point (thermodynamics)
dv_has_:
  name_:
    af: kritieke punt
    an: punto critico
    ar: نقطة حرجة
    ast: puntu críticu
    be: крытычны пункт
    be_tarask: крытычны пункт
    bg: критична точка
    bn: ক্রান্তিবিন্দু
    bs: kritična temperatura
    ca: punt crític
    cs: kritický bod
    cv: синкерлĕ пăнчă
    da: kritisk punkt
    de: kritischer Punkt
    el: κρίσιμο σημείο
    en: critical point
    eo: sojla punkto
    es: punto crítico
    et: kriitiline punkt
    eu: puntu kritiko
    fa: نقطه بحرانی
    fi: kriittinen piste
    fr: point critique
    gl: punto crítico
    he: נקודה קריטית
    hi: क्रांतिक बिन्दु
    hu: kritikus pont
    hy: Կրիտիկական կետ
    ia: puncto critic
    id: titik kritis
    it: punto critico
    ja: 臨界点
    kk: фазалық өту температурасы
    kk_arab: فازالىق ٴوتۋ تەمپەراتۋراسى
    kk-cn: فازالىق ٴوتۋ تەمپەراتۋراسى
    kk_cyrl: фазалық өту температурасы
    kk-kz: фазалық өту температурасы
    kk_latn: fazalıq ötw temperatwrası
    kk-tr: fazalıq ötw temperatwrası
    kn: ಅಧಿವಾಹಕತೆ
    ko: 임계점
    ko-kp: 림계점
    la: punctum criticum
    mk: критична точка
    ms: titik genting
    nan: lîm-kài-tiám
    nb: kritisk punkt
    nds: kritisch Punkt
    new: कृटिकल फुति
    nl: kritisch punt
    nn: kritisk punkt
    oc: ponch critic
    pl: punkt krytyczny
    pnb: فاصل نقطہ
    pt: ponto crítico
    ro: punct critic
    ru: критическая точка
    scn: puntu crìticu
    sco: creetical pynt
    sk: kritický bod
    sl: kritična točka
    smn: kriittisâš čuogâstâh
    sr: критична тачка
    sr_ec: критична тачка
    sr_el: kritična tačka
    sv: kritisk punkt
    tg: нуқтаи буҳронӣ
    th: อุณหภูมิวิกฤต
    tl: puntong kritikal
    tr: kritik nokta
    uk: критична точка
    ur: فاصل نقطہ
    uz: kritik nuqta
    vi: điểm tới hạn
    wuu: 临界点
    yue: 臨界點
    zh: 临界点
    zh_cn: 临界点
    zh_hans: 临界点
    zh_hant: 臨界點
    zh_hk: 臨界點
    zh_mo: 臨界點
    zh-my: 临界点
    zh_sg: 临界点
    zh_tw: 臨界點
---

# [[Critical_Point]] 

#is_/same_as :: [[../../../WikiData/WD~Critical_point,111059|WD~Critical_point,111059]] 

## #has_/text_of_/abstract 

> In thermodynamics, a critical point (or critical state) is the end point of a phase equilibrium curve. 
> One example is the liquid–vapor critical point, the end point of the pressure–temperature curve 
> that designates conditions under which a liquid and its vapor can coexist. 
> 
> At higher temperatures, the gas comes into a supercritical phase, 
> and so cannot be liquefied by pressure alone. 
> 
> At the critical point, defined by a critical temperature Tc and a critical pressure pc, 
> phase boundaries vanish. 
> 
> Other examples include the liquid–liquid critical points in mixtures, 
> and the ferromagnet–paramagnet transition (Curie temperature) 
> in the absence of an external magnetic field.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Critical%20point%20(thermodynamics)) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/State_of_Matter/Critical_Point|Critical_Point]] 

### #is_/same_as :: [[/_public/Science/Physics/State_of_Matter/Critical_Point.public|Critical_Point.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/State_of_Matter/Critical_Point.internal|Critical_Point.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/State_of_Matter/Critical_Point.protect|Critical_Point.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/State_of_Matter/Critical_Point.private|Critical_Point.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/State_of_Matter/Critical_Point.personal|Critical_Point.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/State_of_Matter/Critical_Point.secret|Critical_Point.secret]] 

