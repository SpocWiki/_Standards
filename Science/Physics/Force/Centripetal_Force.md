---
aliases:
- centripeta forto
- Centripetal Force
- centripetalkraft
- Centripetalna sila
- Centripetal_Force
- centripetális erő
- centrtieces spēks
- daya memusat
- dostredivá sila
- dostředivá síla
- Forca qëndërsynuese
- force centripète
- forza centripeta
- forza centrípeta
- força centrípeta
- forță centripetă
- fosi yerutondapakati
- fuercia centrípeta
- fuerza centrípeta
- fòrça centripèta
- fòs santripèd
- fórsa láraimsitheach
- gaya sentripetal
- gaya séntripétal
- grym mewngyrchol
- hiòng-sim-le̍k
- indar zentripetu
- keskihakuvoima
- lực hướng tâm
- Markazga intilma kuch
- merkezcil kuvvet
- middelpuntsoekende krag
- middelpuntzoekende kracht
- miðsóknarafl
- sentripetalkraft
- siła dośrodkowa
- tsentripetaaljõud
- Zentripetalkraft
- įcentrinė jėga
- κεντρομόλος δύναμη
- варринелли вăй
- доцентрова сила
- нармальнае паскарэнне
- нармальнае паскарэньне
- центрипетална сила
- центростремителна сила
- центростремительная сила
- Կենտրոնաձիգ ուժ
- כוח צנטריפטלי
- قوة جذب مركزي
- مرکز مائل قوت
- نیروی مرکزگرا
- چەقە هێز
- अपकेंद्र बल
- अभिकेन्द्रीय बल
- কেন্দ্রমুখী বল
- மையநோக்கு விசை
- అభికేంద్ర బలం
- ಕೇ೦ದ್ರಾಭಿಮುಖ ಬಲ
- അഭികേന്ദ്രബലം
- แรงสู่ศูนย์กลาง
- ແຮງສູນກາງ
- ცენტრისკენული ძალა
- አዙሪት ጉልበት
- 向心力
- 구심력
has_id_wikidata: Q172881
has_part_s_:
- '[[_Standards/WikiData/WD~radius,173817]]'
- '[[_Standards/WikiData/WD~force,11402]]'
- '[[_Standards/WikiData/WD~mass,11423]]'
- '[[_Standards/WikiData/WD~velocity,11465]]'
different_from: '[[_Standards/WikiData/WD~centrifugal_force,178733]]'
has_effect:
- '[[_Standards/WikiData/WD~centripetal_acceleration,2248131]]'
- '[[_Standards/WikiData/WD~curvilinear_motion,6025295]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137]]'
opposite_of: '[[_Standards/WikiData/WD~reactive_centrifugal_force,15087352]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632]]'
ISQ_dimension: "<math class=\"mwe-math-element mwe-math-element-inline\" xmlns=\"http://www.w3.org/1998/Math/MathML\"><mrow data-mjx-texclass=\"ORD\"><mstyle displaystyle=\"true\" scriptlevel=\"0\"><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5AB</mi></mrow><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5AC</mi></mrow><msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5B3</mi></mrow><mrow data-mjx-texclass=\"ORD\"><mrow data-mjx-texclass=\"ORD\"><mo stretchy=\"false\">&#x2212;</mo><mn>2</mn></mrow></mrow></msup></mstyle></mrow></math>"
defining_formula: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mover><mi>F</mi><mo>→</mo></mover></mrow></mrow><mo stretchy="false">=</mo><mo stretchy="false">&#x2212;</mo><mrow data-mjx-texclass="ORD"><mfrac><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mi>m</mi><msup><mi>v</mi><mrow data-mjx-texclass="ORD"><mn>2</mn></mrow></msup><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mover><mi>r</mi><mo>^</mo></mover></mrow></mrow></mrow></mrow><mrow data-mjx-texclass="ORD"><mi>r</mi></mrow></mfrac></mrow></mstyle></mrow></math>
subclass_of: '[[_Standards/WikiData/WD~force,11402]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Rollercoaster%20Tornado%20Avonturenpark%20Hellendoorn%20Netherlands.jpg
schematic:
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal-force-fi.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal%20force%20diagram.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal%20force%20diagram%20bn.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal%20force%20diagram%20mk.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal%20force%20diagram%20no.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal%20force%20diagram%20sr.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal%20force%20diagram%20ta.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Centripetal%20force%20el.png
- http://commons.wikimedia.org/wiki/Special:FilePath/Force%20centripete.svg
Stack_Exchange_tag: https://physics.stackexchange.com/tags/centripetal-force
Commons_category: Centripetal force
dv_has_:
  name_:
    af: middelpuntsoekende krag
    am: አዙሪት ጉልበት
    ar: قوة جذب مركزي
    ast: fuercia centrípeta
    be: нармальнае паскарэнне
    be_tarask: нармальнае паскарэньне
    bg: центростремителна сила
    bn: কেন্দ্রমুখী বল
    ca: força centrípeta
    ckb: چەقە هێز
    cs: dostředivá síla
    cv: варринелли вăй
    cy: grym mewngyrchol
    da: centripetalkraft
    de: Zentripetalkraft
    el: κεντρομόλος δύναμη
    en: centripetal force
    eo: centripeta forto
    es: fuerza centrípeta
    et: tsentripetaaljõud
    eu: indar zentripetu
    fa: نیروی مرکزگرا
    fi: keskihakuvoima
    fr: force centripète
    ga: fórsa láraimsitheach
    gl: forza centrípeta
    he: כוח צנטריפטלי
    hi: अभिकेन्द्रीय बल
    hr: Centripetalna sila
    ht: fòs santripèd
    hu: centripetális erő
    hy: Կենտրոնաձիգ ուժ
    id: gaya sentripetal
    is: miðsóknarafl
    it: forza centripeta
    ja: 向心力
    ka: ცენტრისკენული ძალა
    kn: ಕೇ೦ದ್ರಾಭಿಮುಖ ಬಲ
    ko: 구심력
    lo: ແຮງສູນກາງ
    lt: įcentrinė jėga
    lv: centrtieces spēks
    lzh: 向心力
    mag: अभिकेन्द्रीय बल
    mk: центрипетална сила
    ml: അഭികേന്ദ്രബലം
    mr: अपकेंद्र बल
    ms: daya memusat
    nan: hiòng-sim-le̍k
    nb: sentripetalkraft
    nl: middelpuntzoekende kracht
    nn: sentripetalkraft
    oc: fòrça centripèta
    pl: siła dośrodkowa
    pt: força centrípeta
    ro: forță centripetă
    ru: центростремительная сила
    sk: dostredivá sila
    sl: centripetalna sila
    sn: fosi yerutondapakati
    sq: Forca qëndërsynuese
    sr: центрипетална сила
    su: gaya séntripétal
    sv: centripetalkraft
    ta: மையநோக்கு விசை
    te: అభికేంద్ర బలం
    th: แรงสู่ศูนย์กลาง
    tr: merkezcil kuvvet
    uk: доцентрова сила
    ur: مرکز مائل قوت
    uz: Markazga intilma kuch
    vi: lực hướng tâm
    wuu: 向心力
    yue: 向心力
    zh: 向心力
    zh_cn: 向心力
    zh_hans: 向心力
    zh_hant: 向心力
    zh_hk: 向心力
    zh_sg: 向心力
    zh_tw: 向心力
---
# [[Centripetal_Force]] 

#is_/same_as :: [[../../../WikiData/WD~Centripetal_force,172881|WD~Centripetal_force,172881]] 


## #has_/text_of_/abstract 

> Centripetal force (from Latin centrum, "center" and petere, "to seek") 
> is the force that makes a body follow a curved path. 
> 
> The direction of the centripetal force is always orthogonal to the motion of the body 
> and towards the fixed point of the instantaneous center of curvature of the path. 
> 
> Isaac Newton coined the term, describing it as "a force by which bodies are drawn or impelled, 
> or in any way tend, towards a point as to a centre". 
> In Newtonian mechanics, gravity provides the centripetal force causing astronomical orbits.
>
> One common example involving centripetal force 
> is the case in which a body moves with uniform speed along a circular path. 
> The centripetal force is directed at right angles to the motion 
> and also along the radius towards the centre of the circular path. 
> The mathematical description was derived in 1659 by the Dutch physicist Christiaan Huygens.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Centripetal%20force) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Force/Centripetal_Force|Centripetal_Force]] 

### #is_/same_as :: [[/_public/Science/Physics/Force/Centripetal_Force.public|Centripetal_Force.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Force/Centripetal_Force.internal|Centripetal_Force.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Force/Centripetal_Force.protect|Centripetal_Force.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Force/Centripetal_Force.private|Centripetal_Force.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Force/Centripetal_Force.personal|Centripetal_Force.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Force/Centripetal_Force.secret|Centripetal_Force.secret]] 

