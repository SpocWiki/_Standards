---
aliases:
- addaden n tenga
- aggregaasjetastân
- Aggregaattostand
- aggregatietoestand
- aggregatietoestandj
- aggregationstillstånd
- aggregattilstand
- Aggregatzoustand
- Aggregatzuestand
- Aggregatzustand
- Aggregoatietoestand
- agregaatolek
- agregat alı
- Agregatine olend
- agregatinė būsena
- agregatna stanja
- agregatno stanje
- agregātstāvoklis
- bu̍t-chit chōng-thài
- cyflwr mater
- efnishamur
- estado de agregación da materia
- estado de la materia
- estado di agregashon
- estado han materya
- estados físicos da matéria
- estat de la matèria
- estato d'a materia
- estaus fásicus de matéria
- estáu d'agregación de la materia
- eta matyè
- Falkaalee watootaa
- Fase (materia)
- gjendja agregate
- hali maada
- halmazállapot
- Izinhlobo Ze Matter
- keadaan jirim
- léta di matyèr
- maddenin hâlleri
- maddənin aqreqat halları
- materiaren egoera
- Medeguos fazė
- misandu yeSebhe
- moddaning agregat holatlari
- olomuoto
- orroomhäämi
- skupenstvo
- skupenství
- staideanna an damhna
- stan skupienia materii
- stare de agregare
- stat de la materia
- state o matter
- State of Matter
- States of matter
- State_of_Matter
- stato de materia
- stato de materio
- stato della materia
- Statu de materie
- statu dâ materia
- status materiae
- t'inki kachkay
- toestand van materie
- trạng thái vật chất
- tustant
- vu̍t-chṳt chhong-thai
- wujud zat
- ávdnasa orrunhápmi
- état de la matière
- Ŭk-cék câung-tái
- κατάσταση της ύλης
- агрегат хәл
- агрегатла тăрăм
- агрегатний стан
- агрегатно стање
- агрегатно състояние
- агрегатное состояние вещества
- агрегатон уавæр
- агрегаттык абал
- агрегаттық күй
- агрэгатны стан
- бодисын төлөв
- бодосой түлэб
- состојба на материјата
- тырметъёслэн суро-пожо огдырлыкъёссы
- эмен улмылык
- ագրեգատային վիճակ
- מצב צבירה
- צושטאנד פון מאטעריע
- حالة المادة
- حالتهای ماده
- دۆخی ماددە
- ماده حالتلری
- مادے دی ونڈ
- مادے کی حالت
- کأداءن جيريم
- द्रव्याच्या अवस्था
- पदार्थ की अवस्थाएँ
- पदार्थ को अवस्था
- पदार्थके अवस्था
- পদার্থের অবস্থা
- পদাৰ্থৰ অৱস্থা
- ਪਦਾਰਥ ਦੀਆਂ ਅਵਸਥਾਵਾਂ
- ପଦାର୍ଥର ଅବସ୍ଥା
- பொருட்களின் நிலை
- పదార్థం స్థితి
- ದ್ರವ್ಯ ಹಂತಗಳು
- ദ്രവ്യത്തിന്റെ അവസ്ഥകൾ
- අවධිය
- สถานะ
- ဒြပ်တို့၏ အခြေအနေ
- აგრეგატული მდგომარეობა
- ኵነት ነገር
- 物態
- 物質の状態
- 物质状态
- 相態
- 물질의 상태
has_id_wikidata: Q11430
Commons_category: States of aggregation
described_by_source:
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Technical_Encyclopedia,_1st_edition,20078551|WD~Technical_Encyclopedia,_1st_edition,20078551]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817|WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817]]'
Dewey_Decimal_Classification: 530.4
different_from:
- '[[_Standards/WikiData/WD~phase,104837|WD~phase,104837]]'
- '[[_Standards/WikiData/WD~physical_quantity,107715|WD~physical_quantity,107715]]'
disjoint_union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486|WD~list_of_values_as_qualifiers,23766486]]'
has_list: '[[_Standards/WikiData/WD~list_of_states_of_matter,3271631|WD~list_of_states_of_matter,3271631]]'
Iconclass_notation: 22A31
image:
- http://commons.wikimedia.org/wiki/Special:FilePath/Physics%20matter%20state%20transition%201%20en.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Phase%20change%20-%20en.svg
instance_of:
- '[[_Standards/WikiData/WD~physical_property,4373292|WD~physical_property,4373292]]'
- '[[_Standards/WikiData/WD~metaclass,19478619|WD~metaclass,19478619]]'
is_metaclass_for: '[[_Standards/WikiData/WD~phase,104837|WD~phase,104837]]'
OmegaWiki_Defined_Meaning: 3614
part_of: '[[_Standards/WikiData/WD~phase,104837|WD~phase,104837]]'
subclass_of:
- '[[_Standards/WikiData/WD~type,21146257|WD~type,21146257]]'
topic_s_main_template: '[[_Standards/WikiData/WD~Template_States_of_matter,6147219|WD~Template_States_of_matter,6147219]]'
Wikidata_property: phase of matter
Wolfram_Language_entity_type: MatterPhase
dv_has_:
  name_:
    af: toestand van materie
    an: estato d'a materia
    ar: حالة المادة
    as: পদাৰ্থৰ অৱস্থা
    ast: estáu d'agregación de la materia
    az: maddənin aqreqat halları
    azb: ماده حالتلری
    ba: агрегат хәл
    be: агрэгатны стан
    be_tarask: агрэгатны стан
    bg: агрегатно състояние
    bn: পদার্থের অবস্থা
    bs: agregatno stanje
    bxr: бодосой түлэб
    ca: estat de la matèria
    cdo: Ŭk-cék câung-tái
    ckb: دۆخی ماددە
    crh: agregat alı
    cs: skupenství
    cv: агрегатла тăрăм
    cy: cyflwr mater
    da: aggregattilstand
    de: Aggregatzustand
    el: κατάσταση της ύλης
    en: state of matter
    en_ca: state of matter
    en_gb: state of matter
    eo: stato de materio
    es: estado de la materia
    et: agregaatolek
    eu: materiaren egoera
    ext: estaus fásicus de matéria
    fa: حالتهای ماده
    fi: olomuoto
    fr: état de la matière
    frr: tustant
    fy: aggregaasjetastân
    ga: staideanna an damhna
    gcr: léta di matyèr
    gl: estado de agregación da materia
    gsw: Aggregatzuestand
    hak: vu̍t-chṳt chhong-thai
    he: מצב צבירה
    hi: पदार्थ की अवस्थाएँ
    hif: States of matter
    hr: agregatna stanja
    ht: eta matyè
    hu: halmazállapot
    hy: ագրեգատային վիճակ
    ia: stato de materia
    id: wujud zat
    ie: Statu de materie
    is: efnishamur
    it: stato della materia
    ja: 物質の状態
    jv: wujud zat
    ka: აგრეგატული მდგომარეობა
    kab: addaden n tenga
    kk: агрегаттық күй
    kn: ದ್ರವ್ಯ ಹಂತಗಳು
    ko: 물질의 상태
    ky: агрегаттык абал
    la: status materiae
    lb: Aggregatzoustand
    li: aggregatietoestandj
    lmo: stat de la materia
    lt: agregatinė būsena
    lv: agregātstāvoklis
    lzh: 物態
    mag: पदार्थके अवस्था
    mhr: эмен улмылык
    mk: состојба на материјата
    ml: ദ്രവ്യത്തിന്റെ അവസ്ഥകൾ
    mn: бодисын төлөв
    mr: द्रव्याच्या अवस्था
    ms: keadaan jirim
    ms_arab: کأداءن جيريم
    my: ဒြပ်တို့၏ အခြေအနေ
    nan: bu̍t-chit chōng-thài
    nb: aggregattilstand
    nds: Aggregaattostand
    ne: पदार्थ को अवस्था
    nl: aggregatietoestand
    nn: aggregattilstand
    oc: estat de la matèria
    om: Falkaalee watootaa
    or: ପଦାର୍ଥର ଅବସ୍ଥା
    os: агрегатон уавæр
    pa: ਪਦਾਰਥ ਦੀਆਂ ਅਵਸਥਾਵਾਂ
    pap: estado di agregashon
    pl: stan skupienia materii
    pms: Fase (materia)
    pnb: مادے دی ونڈ
    pt: estados físicos da matéria
    pt_br: estados físicos da matéria
    qu: t'inki kachkay
    ro: stare de agregare
    ru: агрегатное состояние вещества
    scn: statu dâ materia
    sco: state o matter
    se: ávdnasa orrunhápmi
    sgs: Medeguos fazė
    sh: agregatna stanja
    si: අවධිය
    sk: skupenstvo
    sl: agregatno stanje
    smn: orroomhäämi
    sn: misandu yeSebhe
    sq: gjendja agregate
    sr: агрегатно стање
    sv: aggregationstillstånd
    sw: hali maada
    ta: பொருட்களின் நிலை
    te: పదార్థం స్థితి
    th: สถานะ
    ti: ኵነት ነገር
    tr: maddenin hâlleri
    udm: тырметъёслэн суро-пожо огдырлыкъёссы
    uk: агрегатний стан
    ur: مادے کی حالت
    uz: moddaning agregat holatlari
    vep: Agregatine olend
    vi: trạng thái vật chất
    vls: Aggregoatietoestand
    war: estado han materya
    wuu: 物质状态
    yi: צושטאנד פון מאטעריע
    yue: 相態
    zh: 物质状态
    zh_cn: 物质状态
    zh_hans: 物质状态
    zh_hant: 物质状态
    zu: Izinhlobo Ze Matter
---

# [[State_of_Matter]] 

#is_/same_as :: [[../../WikiData/WD~State_of_matter,11430|WD~State_of_matter,11430]] 
#is_/similar_to :: [[State_of_Matter/Phase_Transition/Phase(Matter)|Phase(Matter)]] 

|          | Press | Shape | Volume | Isolation | Magnetic |
| -------- | :---: | :---: | :----: | :-------: | :------: |
| Solid    |   T   |   Y   |   Y    |    Y/N    |    N     |
| Liquid   |   T   |   N   |   Y    |    Y/N    |    N     |
| Gas      |   T   |   N   |   N    |     N     |    N     |
| Plasma   |   T   |   N   |   N    |     Y     |    Y     |
| QGP      |   T   |   N   |   N    |     Y     |    Y     |
| Electron |   Q   |   N   |   Y    |     N     |    Y     |
| Neutron  |   Q   |   N   |   Y    |     Y     |    Y     |

- The [[Quantum_Mechanics/Standard_Model/Hadron/Large_Hadron_Collider/Quark–Gluon_Plasma|Quark–Gluon_Plasma]] is a new State where Gluons and Quarks are unbound within their Plasma. 
- electron-degenerate Matter is present in [[../../Astronomy/Star/Star~Class/White_Dwarf|White_Dwarf]]s 
- Bose–Einstein Condensate 
- Fermionic Condensate 
- Quantum mechanical pressure, rather than Thermal pressure, counteracts gravitational collapse 
	- [[../../Astronomy/Star/Star~Class/White_Dwarf|White_Dwarf]]: 
	- [[../../Astronomy/Star/Star~Class/Neutron_Star|Neutron_Star]]: 


## #has_/text_of_/abstract 

> In physics, a **State of matter** is one of the distinct forms in which matter can exist. 
> Four states of matter are observable in everyday life: 
> - [[State_of_Matter/Solid|Solid]], 
> - [[State_of_Matter/Liquid|Liquid]], 
> - [[State_of_Matter/Gas|Gas]], and 
> - [[State_of_Matter/Plasma|Plasma]]. 
> 
> Many **intermediate states** are known to exist, such as liquid crystal, 
> and some states only exist under extreme conditions, such as 
> - Bose–Einstein condensates like Super-Fluids and 
> - Fermionic condensates (in extreme cold), 
> [[Quantum_Mechanics/Standard_Model/Fermion/Majorana-Fermion|Majorana-Fermion]]s 
> - neutron-degenerate matter (in extreme density), and 
> - quark–gluon plasma (at extremely high energy).
>
> Historically, the distinction is based on qualitative differences in properties. 
> 
> Matter in the solid state maintains a fixed volume (assuming no change in temperature or air pressure) 
> and shape, with component particles (atoms, molecules or ions) close together and fixed into place. 
> 
> Matter in the liquid state maintains a fixed volume (assuming no change in temperature or air pressure), but has a variable shape that adapts to fit its container. Its particles are still close together but move freely. 
> 
> Matter in the gaseous state has both variable volume and shape, adapting both to fit its container. Its particles are neither close together nor fixed in place. 
> 
> Matter in the plasma state has variable volume and shape, and contains neutral atoms as well as a significant number of ions and electrons, both of which can move around freely.
>
> The term phase is sometimes used as a synonym for state of matter, 
> but it is possible for a single compound to form different phases that are in the same state of matter. 
> For example, ice is the solid state of water, but there are multiple phases of ice with different crystal structures, 
> which are formed at different pressures and temperatures.
>
> [Wikipedia](https://en.wikipedia.org/wiki/State%20of%20matter) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/State_of_Matter|State_of_Matter]] 

### #is_/same_as :: [[/_public/Science/Physics/State_of_Matter.public|State_of_Matter.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/State_of_Matter.internal|State_of_Matter.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/State_of_Matter.protect|State_of_Matter.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/State_of_Matter.private|State_of_Matter.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/State_of_Matter.personal|State_of_Matter.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/State_of_Matter.secret|State_of_Matter.secret]] 

