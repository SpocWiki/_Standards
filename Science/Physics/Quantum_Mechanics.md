---
aliases:
- cơ học lượng tử
- Fuzalmechanik
- Kimiyyar kwantom
- kuantom mikianix
- kuantum mekaniği
- kvant mehanikası
- kvant mexanika
- kvant mexanikası
- kvantamekanihkka
- kvantemekanik
- kvantemekanikk
- kvantinė mechanika
- kvantmehaanika
- Kvantmehanik
- kvantmekaaniga
- kvantmekanik
- kvantna mehanika
- kvantová mechanika
- kvanttimekaniikka
- kvanttmekanikk
- kvanttâmekaniik
- kvantu mehānika
- kvantuma meĥaniko
- kvantummechanika
- kvantėnė mekanėka
- kwantenmechaanik
- kwantowŏ mechanika
- kwantummechanica
- kwantummeganika
- liông-cṳ̄ lĭk-hŏk
- liōng-chú le̍k-ha̍k
- Meacanaigs quantumach
- mecaneg cwantwm
- mecanica quantica
- mecanică cuantică
- meccanica quantistica
- mechanica quantic
- mechanica quantica
- mechanika kwantowa
- mecànega cuantìstega
- mecànica quàntica
- mecàniga di quanta
- mecánica cuántica
- mecânica quântica
- meicnic chandamach
- mekanik kantik
- mekanik kuantum
- mekanika kuantike
- mekanika kuantiko
- mekanika kuantum
- mekanika kwantum
- mekanikang kwantum
- mekanikerezh kwantek
- mekaniks na kwantum
- mekanîka quantum
- mekkanika kwantistika
- Mekánika kuántika
- miccànica quantìstica
- mécanique quantique
- mékanik kantik
- mékanika kuantum
- Quantemechanik
- Quantenmechanik
- Quantum mechanics
- quantumo mekaniko
- Quantum_Mechanics
- skammtafræði
- sona pi tawa pi wan pi lili ale (sona lili)
- Tamikanikt tasmktant
- Ukuguxazela kohoyana
- umakanika kwanta
- ñʋŋ hoɖe
- κβαντική μηχανική
- квант механик
- квант механикасы
- квант механикаһы
- квантăллă механика
- квантавая механіка
- квантавая мэханіка
- квантна механика
- квантова механіка
- квантова механика
- Квантовай физика
- квантовая механика
- квантовой механика
- квантон механикæ
- кванттык механика
- квантын механика
- механикаи квантӣ
- толқындық механика
- քուանտային մեգանիկ
- քվանտային մեխանիկա
- מכניקת הקוונטים
- קוואנטן-מעכאניק
- الميكانيكا الكمية
- مقداریہ آلاتیات
- ميكانيكا الكم
- ميکانيک کوانتوم
- مکانیک کوانتوم
- میکانیکی کوانتۆم
- کوانتوم مکانیکی
- کوانتومی فیزیک
- کوانٹم مکینکس
- کوانټوم میخانیک
- ڪوانٽم مڪينڪس
- क्वांटम मैकेनिक्स
- क्वांटम सिद्धांत
- क्वान्टम मेकानिक्स
- क्वान्टम मेकानिक्स्
- पुंज यामिकी
- प्रमात्रा यान्त्रिकी
- सूक्ष्मभौतिकशास्त्रम्
- কোয়ান্টাম বলবিজ্ঞান
- কোৱান্টাম বলবিজ্ঞান
- ਮਿਕਦਾਰ ਮਕੈਨਕੀ
- குவாண்டம் விசையியல்
- క్వాంటం యాంత్రిక శాస్త్రం
- ಕ್ವಾಂಟಮ್ ಭೌತಶಾಸ್ತ್ರ
- ക്വാണ്ടം ബലതന്ത്രം
- ක්වොන්ටම් යාන්ත් ර විද් යා ව
- กลศาสตร์ควอนตัม
- ကွမ်တမ်မက္ကင်းနစ်
- კვანტური მექანიკა
- ⵜⴰⵙⵏⴷⴷⵓⵜ ⵜⴰⵙⵎⴽⵜⴰⵢⵜ
- 量子力学
- 量子力學
- 량자력학
- 양자역학
has_id_wikidata: Q944
Baidu_Tieba_name: 量子力学
Commons_category: Quantum mechanics
creator:
- '[[_Standards/WikiData/WD~Wolfgang_Pauli,65989|WD~Wolfgang_Pauli,65989]]'
- '[[_Standards/WikiData/WD~Louis_de_Broglie,83331|WD~Louis_de_Broglie,83331]]'
- '[[_Standards/WikiData/WD~John_Stewart_Bell,370077|WD~John_Stewart_Bell,370077]]'
- '[[_Standards/WikiData/WD~Albert_Einstein,937|WD~Albert_Einstein,937]]'
- '[[_Standards/WikiData/WD~Max_Planck,9021|WD~Max_Planck,9021]]'
- '[[_Standards/WikiData/WD~Erwin_Schrödinger,9130|WD~Erwin_Schrödinger,9130]]'
- '[[_Standards/WikiData/WD~John_von_Neumann,17455|WD~John_von_Neumann,17455]]'
- '[[_Standards/WikiData/WD~Werner_Heisenberg,40904|WD~Werner_Heisenberg,40904]]'
- '[[_Standards/WikiData/WD~Paul_Dirac,47480|WD~Paul_Dirac,47480]]'
- '[[_Standards/WikiData/WD~Max_Born,58978|WD~Max_Born,58978]]'
- '[[_Standards/WikiData/WD~Pascual_Jordano,61761|WD~Pascual_Jordano,61761]]'
described_by_source: '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1926_1947),20078554|WD~Great_Soviet_Encyclopedia_(1926_1947),20078554]]'
Dewey_Decimal_Classification: 530.12
GitHub_topic: quantum-mechanics
history_of_topic: '[[_Standards/WikiData/WD~history_of_quantum_mechanics,2358974|WD~history_of_quantum_mechanics,2358974]]'
Iconclass_notation: 4.9e+213
image: http://commons.wikimedia.org/wiki/Special:FilePath/Ondaparticula.JPG
instance_of:
- '[[_Standards/WikiData/WD~branch_of_physics,4162444|WD~branch_of_physics,4162444]]'
- '[[_Standards/WikiData/WD~physical_theory,9357058|WD~physical_theory,9357058]]'
Krugosvet_article: fizika/kvantovaya-mehanika
MeSH_tree_code: H01.671.579.800.750
OmegaWiki_Defined_Meaning: 796351
part_of:
- '[[_Standards/WikiData/WD~modern_physics,658544|WD~modern_physics,658544]]'
- '[[_Standards/WikiData/WD~quantum_physics,1144457|WD~quantum_physics,1144457]]'
PhilPapers_topic: quantum-mechanics
practiced_by: '[[_Standards/WikiData/WD~quantum_theoretician,114964328|WD~quantum_theoretician,114964328]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q9610%20%28ben%29-Tahmid-%E0%A6%95%E0%A7%8B%E0%A6%AF%E0%A6%BC%E0%A6%BE%E0%A6%A8%E0%A7%8D%E0%A6%9F%E0%A6%BE%E0%A6%AE%20%E0%A6%AC%E0%A6%B2%E0%A6%AC%E0%A6%BF%E0%A6%9C%E0%A7%8D%E0%A6%9E%E0%A6%BE%E0%A6%A8.wav
publication_date: 1900
short_name:
- MQ
- MQ
- КМ
- QM
- MC
- CHLT
Stack_Exchange_tag: https://physics.stackexchange.com/tags/quantum-mechanics
subclass_of: '[[_Standards/WikiData/WD~mechanics,41217|WD~mechanics,41217]]'
topic_s_main_template:
- '[[_Standards/WikiData/WD~Template_Quantum_mechanics,6534867|WD~Template_Quantum_mechanics,6534867]]'
- '[[_Standards/WikiData/WD~Template_Quantum_mechanics_topics,19794166|WD~Template_Quantum_mechanics_topics,19794166]]'
UMLS_CUI: C0034385
dv_has_:
  name_:
    af: kwantummeganika
    an: mecanica quantica
    anp: क्वांटम सिद्धांत
    ar: ميكانيكا الكم
    arz: الميكانيكا الكمية
    as: কোৱান্টাম বলবিজ্ঞান
    ast: mecánica cuántica
    az: kvant mexanikası
    azb: کوانتوم مکانیکی
    ba: квант механикаһы
    ban: Mékanika kuantum
    bar: Fuzalmechanik
    bcl: mekanikang kwantum
    be: квантавая механіка
    be_tarask: квантавая мэханіка
    bg: квантова механика
    bho: क्वांटम मैकेनिक्स
    bn: কোয়ান্টাম বলবিজ্ঞান
    br: mekanikerezh kwantek
    bs: kvantna mehanika
    bxr: квантын механика
    ca: mecànica quàntica
    cdo: liông-cṳ̄ lĭk-hŏk
    ckb: میکانیکی کوانتۆم
    crh: kvant mehanikası
    cs: kvantová mechanika
    cv: квантăллă механика
    cy: mecaneg cwantwm
    da: kvantemekanik
    de: Quantenmechanik
    de_ch: Quantenmechanik
    el: κβαντική μηχανική
    en: quantum mechanics
    en_ca: quantum mechanics
    en_gb: quantum mechanics
    eo: kvantuma meĥaniko
    es: mecánica cuántica
    et: kvantmehaanika
    eu: mekanika kuantiko
    ext: mecánica cuántica
    fa: مکانیک کوانتوم
    fi: kvanttimekaniikka
    fr: mécanique quantique
    frr: kwantenmechaanik
    ga: meicnic chandamach
    gcr: mékanik kantik
    gd: Meacanaigs quantumach
    gl: mecánica cuántica
    gn: Mekánika kuántika
    gsw: Quantemechanik
    ha: Kimiyyar kwantom
    he: מכניקת הקוונטים
    hi: प्रमात्रा यान्त्रिकी
    hif: quantum mechanics
    hr: kvantna mehanika
    ht: mekanik kantik
    hu: kvantummechanika
    hy: քվանտային մեխանիկա
    hyw: քուանտային մեգանիկ
    ia: mechanica quantic
    id: mekanika kuantum
    ig: Quantum mechanics
    io: quantumo mekaniko
    is: skammtafræði
    it: meccanica quantistica
    ja: 量子力学
    jam: kuantom mikianix
    ka: კვანტური მექანიკა
    kaa: Kvant mexanika
    kbp: ñʋŋ hoɖe
    kk: толқындық механика
    kn: ಕ್ವಾಂಟಮ್ ಭೌತಶಾಸ್ತ್ರ
    ko: 양자역학
    ko-kp: 량자력학
    ku: mekanîka quantum
    ky: кванттык механика
    la: mechanica quantica
    lb: Quantemechanik
    li: kwantummechanica
    lmo: mecàniga di quanta
    lt: kvantinė mechanika
    lv: kvantu mehānika
    lzh: 量子力學
    mk: квантна механика
    ml: ക്വാണ്ടം ബലതന്ത്രം
    mn: квант механик
    mr: पुंज यामिकी
    ms: mekanik kuantum
    ms_arab: ميکانيک کوانتوم
    mt: mekkanika kwantistika
    my: ကွမ်တမ်မက္ကင်းနစ်
    mzn: کوانتومی فیزیک
    nan: liōng-chú le̍k-ha̍k
    nb: kvantemekanikk
    nds: Quantenmechanik
    ne: क्वान्टम मेकानिक्स
    new: क्वान्टम मेकानिक्स्
    nl: kwantummechanica
    nn: kvantemekanikk
    oc: mecanica quantica
    os: квантон механикæ
    pa: ਮਿਕਦਾਰ ਮਕੈਨਕੀ
    pl: mechanika kwantowa
    pms: mecànica quàntica
    pnb: کوانٹم مکینکس
    ps: کوانټوم میخانیک
    pt: mecânica quântica
    pt_br: mecânica quântica
    rki: ကွမ်တမ်မက္ကင်းနစ်
    ro: mecanică cuantică
    ru: квантовая механика
    rue: квантова механіка
    sa: सूक्ष्मभौतिकशास्त्रम्
    sah: Квантовай физика
    scn: miccànica quantìstica
    sco: quantum mechanics
    sd: ڪوانٽم مڪينڪس
    se: kvantamekanihkka
    sgs: kvantėnė mekanėka
    sh: kvantna mehanika
    shi: Tamikanikt tasmktant
    si: ක්වොන්ටම් යාන්ත් ර විද් යා ව
    sk: kvantová mechanika
    sl: kvantna mehanika
    smn: kvanttâmekaniik
    sms: kvanttmekanikk
    sq: mekanika kuantike
    sr: квантна механика
    sr_ec: квантна механика
    su: mékanika kuantum
    sv: kvantmekanik
    sw: umakanika kwanta
    szl: kwantowŏ mechanika
    ta: குவாண்டம் விசையியல்
    te: క్వాంటం యాంత్రిక శాస్త్రం
    tg: механикаи квантӣ
    th: กลศาสตร์ควอนตัม
    tl: mekaniks na kwantum
    tok: sona pi tawa pi wan pi lili ale (sona lili)
    tr: kuantum mekaniği
    tt: квант механикасы
    udm: квантовой механика
    uk: квантова механіка
    ur: مقداریہ آلاتیات
    uz: kvant mexanika
    vec: mecànega cuantìstega
    vep: Kvantmehanik
    vi: cơ học lượng tử
    vro: kvantmekaaniga
    war: mekanika kwantum
    wuu: 量子力学
    yi: קוואנטן-מעכאניק
    yue: 量子力學
    zgh: ⵜⴰⵙⵏⴷⴷⵓⵜ ⵜⴰⵙⵎⴽⵜⴰⵢⵜ
    zh: 量子力学
    zh_hans: 量子力学
    zu: Ukuguxazela kohoyana
---

# [[Quantum_Mechanics]] 

#is_/same_as :: [[../../WikiData/WD~Quantum_mechanics,944|WD~Quantum_mechanics,944]] 

## #has_/text_of_/abstract 

> **Quantum mechanics** is a fundamental theory 
> that describes the behavior of nature at and below the scale of atoms. 
> It is the foundation of all quantum physics, which includes quantum chemistry, 
> quantum field theory, quantum technology, and quantum information science.
>
> Quantum mechanics can describe many systems that classical physics cannot. 
> Classical physics can describe many aspects of nature at an ordinary (macroscopic and (optical) microscopic) scale, 
> but is not sufficient for describing them at very small submicroscopic (atomic and subatomic) scales. 
> 
> Most theories in classical physics can be derived from quantum mechanics 
> as an approximation, valid at large (macroscopic/microscopic) scale.
>
> Quantum systems have bound states that are quantized to discrete values of 
> energy, momentum, angular momentum, and other quantities, 
> in contrast to classical systems where these quantities can be measured continuously. 
> 
> Measurements of quantum systems show characteristics of both particles and waves 
> (wave–particle duality), and there are limits to how accurately the value of a physical quantity can be predicted prior to its measurement, given a complete set of initial conditions (the uncertainty principle).
>
> Quantum mechanics arose gradually from theories to explain observations that could not be reconciled with classical physics, such as Max Planck's solution in 1900 to the black-body radiation problem, and the correspondence between energy and frequency in Albert Einstein's 1905 paper, which explained the photoelectric effect. These early attempts to understand microscopic phenomena, now known as the "old quantum theory", led to the full development of quantum mechanics in the mid-1920s by Niels Bohr, Erwin Schrödinger, Werner Heisenberg, Max Born, Paul Dirac and others. The modern theory is formulated in various specially developed mathematical formalisms. In one of them, a mathematical entity called the wave function provides information, in the form of probability amplitudes, about what measurements of a particle's energy, momentum, and other physical properties may yield.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Quantum%20mechanics) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Quantum_Mechanics|Quantum_Mechanics]] 

### #is_/same_as :: [[/_public/Science/Physics/Quantum_Mechanics.public|Quantum_Mechanics.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Quantum_Mechanics.internal|Quantum_Mechanics.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Quantum_Mechanics.protect|Quantum_Mechanics.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Quantum_Mechanics.private|Quantum_Mechanics.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Quantum_Mechanics.personal|Quantum_Mechanics.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Quantum_Mechanics.secret|Quantum_Mechanics.secret]] 

