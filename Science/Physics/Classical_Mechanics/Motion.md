---
aliases:
- bevegelse
- bevægelse
- Beweegung
- beweging
- Bewegung
- chuyển động
- gerak
- gibanje
- gluaisne
- gwayans
- hareket
- higidura
- Hreyfing
- hərəkət
- johtu
- Kretanje
- Kuyuy
- liike
- liikumine
- lëvizje
- mechanický pohyb
- mechaninis judėjimas
- Mehaniki hereket
- mehāniskā kustība
- mexanik harakat
- mișcare (fizică)
- Mosyon
- Motion
- moto
- Motsi
- motu
- Motus
- mouvement
- Mouvman (mékanik)
- movado
- movement
- movemento
- moviment
- movimento
- Movimentu
- movimiento
- movimientu
- movo
- mozgás
- mudiant
- Muoshan
- Mòto (fisica)
- Paghiro
- pergerakan
- pohyb
- ruch
- rörelse
- rørsle
- sochii
- tawa
- Tín-tāng
- Umdiki
- ìsún
- Ông-dông (Ŭk-lī-hŏk)
- κίνηση
- визылан
- движење
- движение
- кретање
- механічны рух
- Механикăлла куçăм
- Механикалық қозғалыс
- мэханічны рух
- рух
- Хөдөлгөөн
- Ҳаракат
- մեխանիկական շարժում
- Շարժում (մեքենագիտութիւն)
- באוועגונג
- תנועה
- جووڵە
- حاراكة (فيزيك)
- حركة
- حرکت
- حرڪت (فزڪس)
- حَرکَتھ (موشَن)
- ڤرݢرقن
- गति
- गती
- चाल
- গতি
- চলন
- ਗਤੀ
- இயக்கம்
- చలనం
- ಚಲನೆ
- ചലനം
- การเคลื่อนที่
- ရွေ့လျားခြင်း
- მექანიკური გადაადგილება
- ᐊᐅᓚᔪᖅ
- ចលនានៅក្នុងរូបវិទ្យា
- 运动
- 運動
- 郁
- 운동
has_id_wikidata: Q79782
has_part_s_:
- '[[_Standards/WikiData/WD~rotation,107617|WD~rotation,107617]]'
- '[[_Standards/WikiData/WD~linear_motion,2035846|WD~linear_motion,2035846]]'
has_characteristic:
- '[[_Standards/WikiData/WD~trajectory,193139|WD~trajectory,193139]]'
- '[[_Standards/WikiData/WD~acceleration,11376|WD~acceleration,11376]]'
- '[[_Standards/WikiData/WD~velocity,11465|WD~velocity,11465]]'
subclass_of:
- '[[_Standards/WikiData/WD~change,1150070|WD~change,1150070]]'
- '[[_Standards/WikiData/WD~physical_process,17320256|WD~physical_process,17320256]]'
manifestation_of: '[[_Standards/WikiData/WD~geometric_motion,4300759|WD~geometric_motion,4300759]]'
topic_has_template: '[[_Standards/WikiData/WD~Q19860238,19860238|WD~Q19860238,19860238]]'
criterion_used: '[[_Standards/WikiData/WD~relative_location,23008351|WD~relative_location,23008351]]'
studied_by: '[[_Standards/WikiData/WD~mechanics,41217|WD~mechanics,41217]]'
Dewey_Decimal_Classification: 152
Iconclass_notation: 51L1
Commons_category: Kinematics
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/Circlestrafing%20animation.gif
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/Q79782-ar.ogg
image: http://commons.wikimedia.org/wiki/Special:FilePath/Simple%20harmonic%20oscillator.gif
UMLS_CUI: C0026597
MeSH_tree_code: G01.482
dv_has_:
  name_:
    af: beweging
    ar: حركة
    ary: حاراكة (فيزيك)
    as: চলন
    ast: movimientu
    az: hərəkət
    azb: حرکت
    bcl: Paghiro
    be: механічны рух
    be_tarask: мэханічны рух
    bg: движение
    bho: गति
    bn: গতি
    bs: kretanje
    ca: moviment
    cdo: Ông-dông (Ŭk-lī-hŏk)
    ckb: جووڵە
    cs: mechanický pohyb
    cv: Механикăлла куçăм
    cy: mudiant
    da: bevægelse
    de: Bewegung
    el: κίνηση
    en: motion
    en_ca: Motion
    en_gb: motion
    eo: movado
    es: movimiento
    et: liikumine
    eu: higidura
    ext: Movimientu
    fa: حرکت
    fi: liike
    fr: mouvement
    ga: gluaisne
    gcr: Mouvman (mékanik)
    gl: movemento
    gsw: Bewegung
    ha: Motsi
    he: תנועה
    hi: गति
    hr: gibanje
    hu: mozgás
    hy: մեխանիկական շարժում
    hyw: Շարժում (մեքենագիտութիւն)
    id: gerak
    io: movo
    is: Hreyfing
    it: moto
    iu: ᐊᐅᓚᔪᖅ
    ja: 運動
    jam: Muoshan
    ka: მექანიკური გადაადგილება
    kk: Механикалық қозғалыс
    km: ចលនានៅក្នុងរូបវិទ្យា
    kn: ಚಲನೆ
    ko: 운동
    ks: حَرکَتھ (موشَن)
    kw: gwayans
    la: Motus
    lb: Beweegung
    lmo: Mòto (fisica)
    lt: mechaninis judėjimas
    lv: mehāniskā kustība
    mag: गति
    mk: движење
    ml: ചലനം
    mn: Хөдөлгөөн
    mr: गती
    ms: pergerakan
    ms_arab: ڤرݢرقن
    my: ရွေ့လျားခြင်း
    nan: Tín-tāng
    nb: bevegelse
    ne: चाल
    nl: beweging
    nn: rørsle
    oc: movement
    om: sochii
    pa: ਗਤੀ
    pl: ruch
    ps: حرکت
    pt: movimento
    pt_br: movimento
    qu: Kuyuy
    rki: ရွေ့လျားခြင်း
    ro: mișcare (fizică)
    ru: движение
    sc: Movimentu
    scn: motu
    sco: motion
    sd: حرڪت (فزڪس)
    se: johtu
    sh: Kretanje
    sk: pohyb
    sl: gibanje
    sq: lëvizje
    sr: кретање
    sr_ec: кретање
    sr_el: kretanje
    su: Gerak
    sv: rörelse
    ta: இயக்கம்
    tcy: ಚಲನೆ
    te: చలనం
    tg: Ҳаракат
    th: การเคลื่อนที่
    tk: Mehaniki hereket
    tl: Mosyon
    tok: tawa
    tr: hareket
    udm: визылан
    uk: рух
    ur: حرکت
    uz: mexanik harakat
    vec: movimento
    vi: chuyển động
    wuu: 运动
    yi: באוועגונג
    yo: ìsún
    yue: 郁
    zh: 運動
    zh_hant: 運動
    zu: Umdiki
---

# [[Motion]] 

#is_/same_as :: [[_Standards/WikiData/WD~Motion,79782|WD~Motion,79782]] 

## #has_/text_of_/abstract 

> In physics, **Motion** is when an object changes its position with respect to a reference point in a given time. Motion is mathematically described in terms of displacement, distance, velocity, acceleration, speed, and frame of reference to an observer, measuring the change in position of the body relative to that frame with a change in time. The branch of physics describing the motion of objects without reference to their cause is called kinematics, while the branch studying forces and their effect on motion is called dynamics.
>
> If an object is not in motion relative to a given frame of reference, it is said to be at rest, motionless, immobile, stationary, or to have a constant or time-invariant position with reference to its surroundings. Modern physics holds that, as there is no absolute frame of reference, Isaac Newton's concept of absolute motion cannot be determined. Everything in the universe can be considered to be in motion.
>
> Motion applies to various physical systems: objects, bodies, matter particles, matter fields, radiation, radiation fields, radiation particles, curvature, and space-time. One can also speak of the motion of images, shapes, and boundaries. In general, the term motion signifies a continuous change in the position or configuration of a physical system in space. For example, one can talk about the motion of a wave or the motion of a quantum particle, where the configuration consists of the probabilities of the wave or particle occupying specific positions.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Motion) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Classical_Mechanics/Motion|Motion]] 

### #is_/same_as :: [[/_public/Science/Physics/Classical_Mechanics/Motion.public|Motion.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Classical_Mechanics/Motion.internal|Motion.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Classical_Mechanics/Motion.protect|Motion.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Classical_Mechanics/Motion.private|Motion.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Classical_Mechanics/Motion.personal|Motion.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Classical_Mechanics/Motion.secret|Motion.secret]] 

