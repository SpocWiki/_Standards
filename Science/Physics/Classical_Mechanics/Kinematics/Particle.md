---
aliases:
- Aitzal
- corpuscle
- cząstka
- cáithnín
- daļiņa
- deeltje
- delec
- gba̱ram
- grimca
- gronyn
- heneg
- hiukkanen
- hạt (vật lý)
- ijo lili
- kabanzu
- osake
- pairticle
- partezeła
- particedda
- particella
- Particle
- particula
- particule
- particulă
- partihkkal
- Partikel
- partikkel
- partiklo
- partikula
- partikulo
- Partisela
- partiċella
- partícula
- parçacık
- putin
- részecske
- suudoo
- Teilchen
- zarah
- zarra
- zərrəcik
- ögn
- častica
- čestica
- částice
- σωματίδιο
- зарра
- кисәкчек
- пĕрчĕ
- частинка
- частица
- частиця
- часціца
- часьцінка
- честица
- честичка
- մասնիկ
- חלקיק
- تەنۆکە
- جسيم
- ذره
- كسيد
- कण
- ਕਣ
- துகள்
- ಕಣ
- ອະນຸພາກ
- အမှုန်
- 粒子
- 립자
- 입자
has_id_wikidata: Q1621273
subclass_of: '[[_Standards/WikiData/WD~physical_object,223557|WD~physical_object,223557]]'
studied_by: '[[_Standards/WikiData/WD~point_mechanics,3332138|WD~point_mechanics,3332138]]'
described_by_source: '[[_Standards/WikiData/WD~National_Encyclopedia_of_Uzbekistan,5217764|WD~National_Encyclopedia_of_Uzbekistan,5217764]]'
topic_has_template:
- '[[_Standards/WikiData/WD~Template_Infobox_particle,6484397|WD~Template_Infobox_particle,6484397]]'
- '[[_Standards/WikiData/WD~Template_Particles,6768470|WD~Template_Particles,6768470]]'
disjoint_union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486|WD~list_of_values_as_qualifiers,23766486]]'
properties_for_this_type: interaction
OmegaWiki_Defined_Meaning: 6926
image:
- http://commons.wikimedia.org/wiki/Special:FilePath/GMAW.welding.af.ncs.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/ColloidalStability.png
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q188%20%28deu%29-Michael%20Schoenitzer%20%28MichaelSchoenitzer%29-Teilchen.wav
IEV_number:
- 113-05-01
- 881-02-02
Stack_Exchange_tag: https://astronomy.stackexchange.com/tags/particles
dv_has_:
  name_:
    af: Partikel
    anp: कण
    ar: جسيم
    ary: كسيد
    az: zərrəcik
    bar: Aitzal
    be: часціца
    be_tarask: часьцінка
    bg: частица
    bs: čestica
    ca: partícula
    ckb: تەنۆکە
    cs: částice
    cv: пĕрчĕ
    cy: gronyn
    da: partikel
    de: Teilchen
    el: σωματίδιο
    en: particle
    en_ca: particle
    en_gb: particle
    eo: partiklo
    es: partícula
    et: osake
    eu: partikula
    fa: ذره
    fi: hiukkanen
    fr: particule
    ga: cáithnín
    gl: partícula
    he: חלקיק
    hi: कण
    hu: részecske
    hy: մասնիկ
    id: partikel
    io: partikulo
    is: ögn
    it: particella
    ja: 粒子
    jv: partikel
    kcg: gba̱ram
    kn: ಕಣ
    ko: 입자
    ko-kp: 립자
    la: particula
    lmo: Partisela
    lo: ອະນຸພາກ
    lv: daļiņa
    mag: कण
    mk: честичка
    ms: zarah
    ms_arab: ذره
    mt: partiċella
    my: အမှုန်
    nb: partikkel
    nl: deeltje
    nn: partikkel
    om: suudoo
    pa: ਕਣ
    pl: cząstka
    pt: partícula
    pt_br: partícula
    ro: particulă
    ru: частица
    rue: частиця
    scn: particedda
    sco: pairticle
    se: partihkkal
    sh: čestica
    sk: častica
    sl: delec
    sms: partikkel
    sn: kabanzu
    sq: grimca
    sr: честица
    sr_ec: честица
    sr_el: čestica
    sv: partikel
    ta: துகள்
    tg: зарра
    tl: partikula
    tok: ijo lili
    tr: parçacık
    tt: кисәкчек
    uk: частинка
    uz: zarra
    vec: partezeła
    vep: heneg
    vi: hạt (vật lý)
    vro: putin
    yue: 粒子
    zh: 粒子
    zh_cn: 粒子
    zh_hans: 粒子
    zh_hant: 粒子
    zh_hk: 粒子
    zh_mo: 粒子
    zh-my: 粒子
    zh_sg: 粒子
    zh_tw: 粒子
---

# [[Particle]] 

#is_/same_as :: [[_Standards/WikiData/WD~Particle,1621273|WD~Particle,1621273]] 

## #has_/text_of_/abstract 

> In the physical sciences, a **Particle** (or corpuscle in older texts) is a small localized object 
> which can be described by several physical or chemical properties, such as volume, density, or mass. 
> 
> They vary greatly in size or quantity, from subatomic particles like the electron, 
> to microscopic particles like atoms and molecules, 
> to macroscopic particles like powders and other granular materials. 
> 
> Particles can also be used to create scientific models of even larger objects 
> depending on their density, such as humans moving in a crowd or celestial bodies in motion.
>
> The term particle is rather general in meaning, and is refined as needed by various scientific fields. 
> 
> Anything that is composed of particles may be referred to as being particulate. 
> However, the noun particulate is most frequently used 
> to refer to pollutants in the Earth's atmosphere, which are a suspension of unconnected particles, 
> rather than a connected particle aggregation.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Particle) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Classical_Mechanics/Kinematics/Particle|Particle]] 

### #is_/same_as :: [[/_public/Science/Physics/Classical_Mechanics/Kinematics/Particle.public|Particle.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Classical_Mechanics/Kinematics/Particle.internal|Particle.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Classical_Mechanics/Kinematics/Particle.protect|Particle.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Classical_Mechanics/Kinematics/Particle.private|Particle.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Classical_Mechanics/Kinematics/Particle.personal|Particle.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Classical_Mechanics/Kinematics/Particle.secret|Particle.secret]] 

