---
aliases:
- amateeka ga Newton ag'okuva
- balaod nga mosyon ha Newton
- các định luật về chuyển động của Newton
- deddfau mudiant Newton
- dlíthe na gluaiseachta
- hukum gerak Newton
- hukum-hukum gerakan Newton
- Imithetho yomdiki kaNewton
- islgan n umussu n Newton
- laj dël moviment ëd Newton
- lalwa di mouvman di Newton
- leges de Newton
- leges motus Newtoni
- Legi di Newton
- legile lui Newton
- leis de Newton
- leyes de Newton
- leĝoj de Newton pri movado
- liggi di Newton
- ligjet e Njutonit
- lleis de Newton
- lois du mouvement de Newton
- Lwa mouvman Newton
- Lögmál Newtons
- mawatho matano ma Newton
- mekaniikan peruslait
- mga batas ng mosyon ni Newton
- mitemo yeruhambo yaNewton
- Newton ciiduu paɣtʋ ndɩ ndɩ
- Newton se bewegingswette
- Newton törvényei
- Newton zien wetten
- Newton's laws o motion
- Newton's laws of motion
- Newton'ın hareket yasaları
- Newtonen legeak
- Newtoni seadused
- Newtoni säädüseq
- Newtonove pohybové zákony
- Newtonovi zakoni
- Newtonovi zakoni gibanja
- Newtonovi zakoni kretanja
- Newtonovy pohybové zákony
- Newtons bevegelseslover
- Newtons love
- Newtons rörelselagar
- Newtons rørslelover
- Newtonsche Axiome
- Newtonsche Gesetze
- Ngiù-dóng ông-dông-hŏk dêng-lŭk
- Niutono dėsniai
- Nyuton qanunları
- Nyutonning mexanika qonunlari
- Nyuutan laa a muoshan
- principi della dinamica
- wetten fan Newton
- wetten van Newton
- wètte van Newton
- zasady dynamiki Newtona
- àwọn òfin ìsún Newton
- Ņūtona likumi
- Νόμοι κίνησης του Νεύτωνα
- Њутнови закони
- Закони на Нютон
- закони Ньютона
- Законы Ньютана
- законы Ньютона
- Ньютон законнары
- Ньютон саккунĕсем
- Ньютон сокуоннара
- Ньютонның үшінші заңы
- Ньютоновы законы рушаня
- Ньютоной хуулинууд
- Ньютоны хуулиуд
- Նիւթոնի շարժման օրէնքներ
- Նյուտոնի օրենքներ
- חוקי התנועה של ניוטון
- د خوځښت په اړه د نیوټن قوانین
- قوانين نيوتن للحركة
- قوانین حرکت نیوتن
- نيوٽن جي حرڪت جا قانون
- نیوتون یاسالاری
- نیوٹن دا پہلا قنون
- نیوٹن دے چلن دے قنون
- نیوٹن کے قوانین حرکت
- یاساکانی جووڵەی نیوتن
- न्युटनक गति नियम
- न्युटनको गति नियम
- न्युटनया गतिया नियमत
- न्यूटन के गति के नियम
- न्यूटन के गति नियम
- न्यूटनके गति नियम
- न्यूटनचे गतीचे नियम
- নিউটনের গতিসূত্রসমূহ
- নিউটনৰ গতিৰ সূত্ৰ
- ਨਿਊਟਨ ਦੇ ਗਤੀ ਦੇ ਨਿਯਮ
- ગતિના નિયમો
- நியூட்டனின் இயக்க விதிகள்
- న్యూటన్ సూత్రాలు
- ನ್ಯೂಟನ್ನ ಚಲನೆಯ ನಿಯಮಗಳು
- ന്യൂട്ടന്റെ ചലനനിയമങ്ങൾ
- චලිතය පිළිබඳ නිව්ටන් නියම
- กฎการเคลื่อนที่ของนิวตัน
- နယူတန်ဧ ရွေ့လျားမှုဆိုင်ရာနိယာမ
- နယူတန်၏ ရွေ့လျားမှုဆိုင်ရာ နိယာမများ
- ნიუტონის კანონები
- ច្បាប់ចលនារបស់ញូតុន
- ニュートンの運動3法則
- 牛頓定律
- 牛頓運動定律
- 牛顿运动定律
- 뉴턴 운동 법칙
has_id_wikidata: Q38433
has_part_s_of_the_class: '[[_Standards/WikiData/WD~physical_law,214070|WD~physical_law,214070]]'
instance_of:
- '[[_Standards/WikiData/WD~scientific_law,408891|WD~scientific_law,408891]]'
- '[[_Standards/WikiData/WD~triad,29430681|WD~triad,29430681]]'
has_part_s_:
- "[[_Standards/WikiData/WD~Newton's_second_law_of_motion_for_constant_mass,2397319|WD~Newton's_second_law_of_motion_for_constant_mass,2397319]]"
- "[[_Standards/WikiData/WD~Newton's_third_law_of_motion,3235565|WD~Newton's_third_law_of_motion,3235565]]"
- "[[_Standards/WikiData/WD~Newton's_first_law,3235671|WD~Newton's_first_law,3235671]]"
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718|WD~Armenian_Soviet_Encyclopedia,2657718]]'
union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486|WD~list_of_values_as_qualifiers,23766486]]'
named_after: '[[_Standards/WikiData/WD~Isaac_Newton,935|WD~Isaac_Newton,935]]'
discoverer_or_inventor: '[[_Standards/WikiData/WD~Isaac_Newton,935|WD~Isaac_Newton,935]]'
studied_by: '[[_Standards/WikiData/WD~classical_mechanics,11397|WD~classical_mechanics,11397]]'
copyright_status: '[[_Standards/WikiData/WD~public_domain,19652|WD~public_domain,19652]]'
place_of_publication: '[[_Standards/WikiData/WD~London,84|WD~London,84]]'
publication_date: 1687-07-05T00:00:00Z
video:
- http://commons.wikimedia.org/wiki/Special:FilePath/Secondlaw.ogv
- http://commons.wikimedia.org/wiki/Special:FilePath/First%20law.ogv
- http://commons.wikimedia.org/wiki/Special:FilePath/Thirdlaw.ogv
image: http://commons.wikimedia.org/wiki/Special:FilePath/Newtons%20laws%20in%20latin.jpg
Commons_category: Newton's laws of motion
dv_has_:
  name_:
    af: Newton se bewegingswette
    ar: قوانين نيوتن للحركة
    as: নিউটনৰ গতিৰ সূত্ৰ
    ast: lleis de Newton
    az: Nyuton qanunları
    azb: نیوتون یاسالاری
    ban: Hukum gerak Newton
    bar: Newtonsche Axiome
    be: Законы Ньютана
    be_tarask: Законы Ньютана
    bg: Закони на Нютон
    bho: न्यूटन के गति के नियम
    bn: নিউটনের গতিসূত্রসমূহ
    bs: Newtonovi zakoni kretanja
    bxr: Ньютоной хуулинууд
    ca: lleis de Newton
    cdo: Ngiù-dóng ông-dông-hŏk dêng-lŭk
    ckb: یاساکانی جووڵەی نیوتن
    cs: Newtonovy pohybové zákony
    cv: Ньютон саккунĕсем
    cy: deddfau mudiant Newton
    da: Newtons love
    de: Newtonsche Gesetze
    el: Νόμοι κίνησης του Νεύτωνα
    en: Newton's laws of motion
    eo: leĝoj de Newton pri movado
    es: leyes de Newton
    et: Newtoni seadused
    eu: Newtonen legeak
    fa: قوانین حرکت نیوتن
    fi: mekaniikan peruslait
    fr: lois du mouvement de Newton
    fy: wetten fan Newton
    ga: dlíthe na gluaiseachta
    gcr: lalwa di mouvman di Newton
    gl: leis de Newton
    gsw: Newtonsche Gesetze
    gu: ગતિના નિયમો
    he: חוקי התנועה של ניוטון
    hi: न्यूटन के गति नियम
    hr: Newtonovi zakoni gibanja
    ht: Lwa mouvman Newton
    hu: Newton törvényei
    hy: Նյուտոնի օրենքներ
    hyw: Նիւթոնի շարժման օրէնքներ
    ia: leges de Newton
    id: hukum gerak Newton
    io: Legi di Newton
    is: Lögmál Newtons
    it: principi della dinamica
    ja: ニュートンの運動3法則
    jam: Nyuutan laa a muoshan
    ka: ნიუტონის კანონები
    kbp: Newton ciiduu paɣtʋ ndɩ ndɩ
    ki: mawatho matano ma Newton
    kk: Ньютонның үшінші заңы
    km: ច្បាប់ចលនារបស់ញូតុន
    kn: ನ್ಯೂಟನ್ನ ಚಲನೆಯ ನಿಯಮಗಳು
    ko: 뉴턴 운동 법칙
    la: leges motus Newtoni
    lg: amateeka ga Newton ag'okuva
    li: wètte van Newton
    lt: Niutono dėsniai
    lv: Ņūtona likumi
    lzh: 牛頓定律
    mag: न्यूटनके गति नियम
    mai: न्युटनक गति नियम
    mk: Њутнови закони
    ml: ന്യൂട്ടന്റെ ചലനനിയമങ്ങൾ
    mn: Ньютоны хуулиуд
    mr: न्यूटनचे गतीचे नियम
    ms: hukum-hukum gerakan Newton
    my: နယူတန်၏ ရွေ့လျားမှုဆိုင်ရာ နိယာမများ
    nb: Newtons bevegelseslover
    nds_nl: Newton zien wetten
    ne: न्युटनको गति नियम
    new: न्युटनया गतिया नियमत
    nl: wetten van Newton
    nn: Newtons rørslelover
    oc: leis de Newton
    pa: ਨਿਊਟਨ ਦੇ ਗਤੀ ਦੇ ਨਿਯਮ
    pl: zasady dynamiki Newtona
    pms: laj dël moviment ëd Newton
    pnb: نیوٹن دے چلن دے قنون
    ps: د خوځښت په اړه د نیوټن قوانین
    pt: leis de Newton
    rki: နယူတန်ဧ ရွေ့လျားမှုဆိုင်ရာနိယာမ
    ro: legile lui Newton
    ru: законы Ньютона
    rue: Ньютоновы законы рушаня
    sah: Ньютон сокуоннара
    scn: liggi di Newton
    sco: Newton's laws o motion
    sd: نيوٽن جي حرڪت جا قانون
    sh: Newtonovi zakoni
    shi: islgan n umussu n Newton
    si: චලිතය පිළිබඳ නිව්ටන් නියම
    sk: Newtonove pohybové zákony
    skr: نیوٹن دا پہلا قنون
    sl: Newtonovi zakoni gibanja
    sn: mitemo yeruhambo yaNewton
    sq: ligjet e Njutonit
    sr: Њутнови закони
    sr_ec: Њутнови закони
    su: hukum gerak Newton
    sv: Newtons rörelselagar
    ta: நியூட்டனின் இயக்க விதிகள்
    te: న్యూటన్ సూత్రాలు
    th: กฎการเคลื่อนที่ของนิวตัน
    tl: mga batas ng mosyon ni Newton
    tr: Newton'ın hareket yasaları
    tt: Ньютон законнары
    uk: закони Ньютона
    ur: نیوٹن کے قوانین حرکت
    uz: Nyutonning mexanika qonunlari
    vi: các định luật về chuyển động của Newton
    vro: Newtoni säädüseq
    war: balaod nga mosyon ha Newton
    wuu: 牛顿运动定律
    yo: àwọn òfin ìsún Newton
    yue: 牛頓運動定律
    zh: 牛顿运动定律
    zh_cn: 牛顿运动定律
    zh_hans: 牛顿运动定律
    zh_hant: 牛頓運動定律
    zu: Imithetho yomdiki kaNewton
---

# [[Newton's_Laws_of_Motion]] 

#is_/same_as :: [[_Standards/WikiData/WD~Newton's_laws_of_motion,38433|WD~Newton's_laws_of_motion,38433]] 

## #has_/text_of_/abstract 

> Newton's laws of motion are three physical laws 
> that describe the relationship between the motion of an object and the forces acting on it. 
> 
> These laws, which provide the basis for Newtonian mechanics, 
> can be paraphrased as follows:
> - A body remains at rest, or in motion at a constant speed in a straight line, 
>   unless it is acted upon by a force.
> - At any instant of time, the net force on a body 
> - is equal to the body's acceleration multiplied by its mass or, 
>   equivalently, the rate at which the body's momentum is changing with time.
> - If two bodies exert forces on each other, 
>   these forces have the same magnitude but opposite directions.
>
> The three laws of motion were first stated by Isaac Newton 
> in his Philosophiæ Naturalis Principia Mathematica 
> (Mathematical Principles of Natural Philosophy), originally published in 1687. 
> 
> Newton used them to investigate 
> and explain the motion of many physical objects and systems. 
> 
> In the time since Newton, new insights, especially around the concept of energy, 
> built the field of classical mechanics on his foundations. 
> 
> Limitations to Newton's laws have also been discovered; 
> new theories are necessary when objects 
> - move at very high speeds (special relativity), 
> - are very massive (general relativity), 
> - or are very small (quantum mechanics).
>
> [Wikipedia](https://en.wikipedia.org/wiki/Newton's%20laws%20of%20motion) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Classical_Mechanics/Newton's_Laws_of_Motion|Newton's_Laws_of_Motion]] 

### #is_/same_as :: [[/_public/Science/Physics/Classical_Mechanics/Newton's_Laws_of_Motion.public|Newton's_Laws_of_Motion.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Classical_Mechanics/Newton's_Laws_of_Motion.internal|Newton's_Laws_of_Motion.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Classical_Mechanics/Newton's_Laws_of_Motion.protect|Newton's_Laws_of_Motion.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Classical_Mechanics/Newton's_Laws_of_Motion.private|Newton's_Laws_of_Motion.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Classical_Mechanics/Newton's_Laws_of_Motion.personal|Newton's_Laws_of_Motion.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Classical_Mechanics/Newton's_Laws_of_Motion.secret|Newton's_Laws_of_Motion.secret]] 

