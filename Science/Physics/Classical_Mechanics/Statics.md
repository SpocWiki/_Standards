---
aliases:
- Estatika
- estàtica
- estática
- kaji daya mantap
- mécanique statique
- sona pi ijo awen
- Staatika
- statica
- Statics
- Statică
- Statiikka
- Statik
- Statika
- statikk
- statiko
- Statyka
- Statîk
- Statïka
- Stàtica
- Stöðufræði
- Tĩnh học
- στατική
- статика
- Статыка
- Ստատիկա
- סטטיקה
- ئهستاتیکا
- استاتیک
- ایستاتیک
- ستاتىيكا
- سكونيات
- سکونیات
- स्थैतिकी
- স্থিতিবিদ্যা
- நிலையியல்
- സ്റ്റാറ്റിൿസ്
- ස්ථිතිකය
- สถิตยศาสตร์
- სტატიკა
- ស្តាទិច
- 静力学
- 靜力學
- 정역학
has_id_wikidata: Q169019
opposite_of: '[[_Standards/WikiData/WD~dynamics,128030|WD~dynamics,128030]]'
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718|WD~Armenian_Soviet_Encyclopedia,2657718]]'
- '[[_Standards/WikiData/WD~Yuzhakov_Big_Encyclopedia,4091878|WD~Yuzhakov_Big_Encyclopedia,4091878]]'
- "[[_Standards/WikiData/WD~The_New_Student's_Reference_Work,16082057|WD~The_New_Student's_Reference_Work,16082057]]"
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
instance_of: '[[_Standards/WikiData/WD~branch_of_physics,4162444|WD~branch_of_physics,4162444]]'
maintained_by_WikiProject:
- '[[_Standards/WikiData/WD~WikiProject_Physics,8487193|WD~WikiProject_Physics,8487193]]'
- '[[_Standards/WikiData/WD~WikiProject_Civil_engineering,21829766|WD~WikiProject_Civil_engineering,21829766]]'
subclass_of: '[[_Standards/WikiData/WD~mechanics,41217|WD~mechanics,41217]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Beam%20in%20static%20equilibrium.svg
Colon_Classification: B71:2
Krugosvet_article: nauka_i_tehnika/fizika/STATIKA.html
Commons_category: Statics
described_at_URL:
- https://engineeringstatics.org/Chapter_01.html
- https://physics.info/statics/
dv_has_:
  name_:
    ar: سكونيات
    ast: Estática
    az: Statika
    azb: ایستاتیک
    bar: Statik
    be: Статыка
    be_tarask: статыка
    bg: Статика
    bn: স্থিতিবিদ্যা
    bs: Statika
    ca: estàtica
    ckb: ئهستاتیکا
    cs: statika
    cv: Статика
    da: Statik
    de: Statik
    de_ch: Statik
    el: στατική
    en: statics
    en_ca: Statics
    en_gb: statics
    eo: statiko
    es: estática
    et: Staatika
    eu: Estatika
    fa: استاتیک
    fi: Statiikka
    fr: mécanique statique
    gl: estática
    gsw: Statik
    he: סטטיקה
    hi: स्थैतिकी
    hr: Statika
    ht: Statik
    hu: statika
    hy: Ստատիկա
    id: Statika
    io: Statiko
    is: Stöðufræði
    it: statica
    ja: 静力学
    ka: სტატიკა
    kk: Статика
    kk_arab: ستاتىيكا
    kk-cn: ستاتىيكا
    kk_cyrl: Статика
    kk-kz: Статика
    kk_latn: Statïka
    kk-tr: Statïka
    km: ស្តាទិច
    ko: 정역학
    ku: Statîk
    ku_latn: Statîk
    ky: статика
    lt: Statika
    lv: statika
    mk: статика
    ml: സ്റ്റാറ്റിൿസ്
    ms: kaji daya mantap
    nb: statikk
    nl: statica
    nn: statikk
    pl: Statyka
    pms: Stàtica
    pt: estática
    pt_br: Estática
    ro: Statică
    ru: статика
    sh: Statika
    si: ස්ථිතිකය
    sk: Statika
    sl: statika
    sq: Statika
    sr: Статика
    sr_ec: Статика
    sr_el: Statika
    sv: Statik
    ta: நிலையியல்
    th: สถิตยศาสตร์
    tok: sona pi ijo awen
    tr: Statik
    uk: статика
    ur: سکونیات
    vi: Tĩnh học
    wuu: 静力学
    yue: 靜力學
    zh: 静力学
    zh_cn: 静力学
    zh_hans: 静力学
    zh_hant: 靜力學
    zh_hk: 靜力學
    zh_sg: 静力学
    zh_tw: 靜力學
---

# [[Statics]] 

#has_/complement :: [[Classical_Mechanics/Dynamics|Dynamics]] 
#is_/same_as :: [[../../../WikiData/WD~Statics,169019|WD~Statics,169019]] 

## #has_/text_of_/abstract 

> **Statics** is the branch of classical mechanics 
> that is concerned with the analysis of force and torque acting on a physical system 
> that does not experience an acceleration, but rather is in equilibrium with its environment. 
>
> If F  is the total of the forces acting on the system, 
> m  is the mass of the system and 
> a  is the acceleration of the system, Newton's second law states that F = m a
>  (the bold font indicates a vector quantity, i.e. one with both magnitude and direction). 
>  
>  If a = 0, then F = 0. 
>  
>  As for a system in static equilibrium, the acceleration equals zero, 
>  the system is either at rest, or its center of mass moves at constant velocity. 
>
> The application of the assumption of zero acceleration to the summation of moments acting on the system 
> leads to M = I α = 0, where 
> M is the summation of all moments acting on the system, 
> I  is the moment of inertia of the mass and 
> α  is the angular acceleration of the system. 
>
> Together, the equations 
>  (the 'first condition for equilibrium') and 
>  (the 'second condition for equilibrium') can be used to solve for unknown quantities acting on the system.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Statics) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Classical_Mechanics/Statics|Statics]] 

### #is_/same_as :: [[/_public/Science/Physics/Classical_Mechanics/Statics.public|Statics.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Classical_Mechanics/Statics.internal|Statics.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Classical_Mechanics/Statics.protect|Statics.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Classical_Mechanics/Statics.private|Statics.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Classical_Mechanics/Statics.personal|Statics.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Classical_Mechanics/Statics.secret|Statics.secret]] 

