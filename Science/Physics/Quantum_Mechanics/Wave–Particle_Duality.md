---
aliases:
- Aaltohiukkasdualismi
- Bangos ir dalelės dualumas
- Bølge–partikkel-dualiteten
- Dalga parçacık ikiliği
- delčnovalovna dualnost
- Dualidad ng alon-partikulo
- dualidad onda corpúsculo
- Dualidade onda-corpúsculo
- Dualidá onda corpúsculu
- dualismo onda-particella
- dualismul corpuscul-undă
- dualita částice a vlnění
- Dualitas gelombang-partikel
- Dualitat ona-partícula
- dualiteit van golven en deeltjes
- dualité onde-corpuscule
- Dualizam
- dualizm korpuskularno-falowy
- déacht toinne is cáithnín
- hullám-részecske kettősség
- Kembaran zarah-gelombang
- Korpuskulaar-laineline dualism
- Korpuskulyar dalğa dualizmi
- Lưỡng tính sóng-hạt
- onda-partikla dualeco
- Partikel-bølge dualitet
- Uhin-partikula dualtasuna
- Våg-partikeldualitet
- wave–particle duality
- Welle-Teilchen-Dualismus
- časticovo-vlnový dualizmus
- κυματοσωματιδιακός δυϊσμός
- браново-честична двојност
- Долгион ба биетийн давхарлал
- дуаліта частицї і вовнїня
- карпускулярна-хвалевы дуалізм
- Корпускула-хум дуализмĕ
- корпускуляр-дулкынча дуализм
- корпускулярно-волновой дуализм
- корпускулярно-вълнов дуализъм
- корпускулярно-хвильовий дуалізм
- принцип дуалности талас-честица
- Մասնիկ-ալիքային երկվություն
- דואליות גל-חלקיק
- ازدواجية موجة جسيم
- دوگانگی موج و ذره
- موج-ٹوٹا دوہراپن
- तरंग-कण द्विरूप
- তরঙ্গ-কণা দ্বৈততা
- ਤਰੰਗ–ਕਣ ਦਵੈਤ
- ତରଙ୍ଗ-କଣିକା ଦ୍ଵୈତବାଦ
- அலை துகள் இருமை
- තරංග–අංශු ද්විත්වය
- ทวิภาคของคลื่น–อนุภาค
- 波粒二象性
- 粒子と波動の二重性
- 파동-입자 이중성
- 파동―립자 2중성
has_id_wikidata: Q193068
instance_of:
- '[[_Standards/WikiData/WD~physical_law,214070]]'
- '[[_Standards/WikiData/WD~quantum_effect,1872885]]'
studied_by: '[[_Standards/WikiData/WD~quantum_mechanics,944]]'
manifestation_of: '[[_Standards/WikiData/WD~uncertainty_principle,44746]]'
video:
- http://commons.wikimedia.org/wiki/Special:FilePath/Dualit%C3%A9%20onde-corpuscule.ogv
- http://commons.wikimedia.org/wiki/Special:FilePath/Wave-particle%20duality.gif
- http://commons.wikimedia.org/wiki/Special:FilePath/Wave-particle%20duality.ogv
- http://commons.wikimedia.org/wiki/Special:FilePath/Wavelet.gif
image: http://commons.wikimedia.org/wiki/Special:FilePath/Wave%20packet.svg
Stack_Exchange_tag: https://physics.stackexchange.com/tags/wave-particle-duality
Commons_gallery: Wave-particle duality
ComputerLanguage_com_definition: wave-particle+duality
Commons_category: Wave–particle duality
dv_has_:
  name_:
    ar: ازدواجية موجة جسيم
    ast: Dualidá onda corpúsculu
    az: Korpuskulyar dalğa dualizmi
    be: карпускулярна-хвалевы дуалізм
    be-tarask: карпускулярна-хвалевы дуалізм
    bg: корпускулярно-вълнов дуализъм
    bn: তরঙ্গ-কণা দ্বৈততা
    ca: Dualitat ona-partícula
    cs: dualita částice a vlnění
    cv: Корпускула-хум дуализмĕ
    da: Partikel-bølge dualitet
    de: Welle-Teilchen-Dualismus
    el: κυματοσωματιδιακός δυϊσμός
    en: wave–particle duality
    eo: onda-partikla dualeco
    es: dualidad onda corpúsculo
    et: Korpuskulaar-laineline dualism
    eu: Uhin-partikula dualtasuna
    fa: دوگانگی موج و ذره
    fi: Aaltohiukkasdualismi
    fr: dualité onde-corpuscule
    ga: déacht toinne is cáithnín
    gl: Dualidade onda-corpúsculo
    gsw: Welle-Teilchen-Dualismus
    he: דואליות גל-חלקיק
    hi: तरंग-कण द्विरूप
    hr: Dualizam
    hu: hullám-részecske kettősség
    hy: Մասնիկ-ալիքային երկվություն
    id: Dualitas gelombang-partikel
    it: dualismo onda-particella
    ja: 粒子と波動の二重性
    ko: 파동-입자 이중성
    ko-kp: 파동―립자 2중성
    lt: Bangos ir dalelės dualumas
    mk: браново-честична двојност
    mn: Долгион ба биетийн давхарлал
    ms: Kembaran zarah-gelombang
    nb: Bølge–partikkel-dualiteten
    nl: dualiteit van golven en deeltjes
    or: ତରଙ୍ଗ-କଣିକା ଦ୍ଵୈତବାଦ
    pa: ਤਰੰਗ–ਕਣ ਦਵੈਤ
    pl: dualizm korpuskularno-falowy
    pnb: موج-ٹوٹا دوہراپن
    pt: dualidade onda-corpúsculo
    ro: dualismul corpuscul-undă
    ru: корпускулярно-волновой дуализм
    rue: дуаліта частицї і вовнїня
    sh: Dualizam
    si: තරංග–අංශු ද්විත්වය
    sk: časticovo-vlnový dualizmus
    sl: delčnovalovna dualnost
    sr: принцип дуалности талас-честица
    sv: Våg-partikeldualitet
    ta: அலை துகள் இருமை
    th: ทวิภาคของคลื่น–อนุภาค
    tl: Dualidad ng alon-partikulo
    tr: Dalga parçacık ikiliği
    tt: корпускуляр-дулкынча дуализм
    uk: корпускулярно-хвильовий дуалізм
    vi: Lưỡng tính sóng-hạt
    wuu: 波粒二象性
    yue: 波粒二象性
    zh: 波粒二象性
    zh-cn: 波粒二象性
    zh-hans: 波粒二象性
    zh-hant: 波粒二象性
    zh-hk: 波粒二象性
    zh-sg: 波粒二象性
    zh-tw: 波粒二象性
---
# [[Wave–Particle_Duality]] 

#is_/same_as :: [[../../../WikiData/WD~Wave–particle_duality,193068|WD~Wave–particle_duality,193068]] 

## #has_/text_of_/abstract 

> **Wave–particle duality** is the concept in quantum mechanics 
> that fundamental entities of the universe, like photons and electrons, 
> exhibit particle or wave properties according to the experimental circumstances. 
> 
> It expresses the inability of the classical concepts such as particle or wave 
> to fully describe the behavior of quantum objects. 
> 
> During the 19th and early 20th centuries, light was found to behave as a wave, 
> then later was discovered to have a particle-like behavior, 
> whereas electrons behaved like particles in early experiments, 
> then later were discovered to have wave-like behavior. 
> 
> The concept of duality arose to name these seeming contradictions.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Wave%E2%80%93particle%20duality) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Quantum_Mechanics/Wave–Particle_Duality|Wave–Particle_Duality]] 

### #is_/same_as :: [[/_public/Science/Physics/Quantum_Mechanics/Wave–Particle_Duality.public|Wave–Particle_Duality.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Quantum_Mechanics/Wave–Particle_Duality.internal|Wave–Particle_Duality.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Quantum_Mechanics/Wave–Particle_Duality.protect|Wave–Particle_Duality.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Quantum_Mechanics/Wave–Particle_Duality.private|Wave–Particle_Duality.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Quantum_Mechanics/Wave–Particle_Duality.personal|Wave–Particle_Duality.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Quantum_Mechanics/Wave–Particle_Duality.secret|Wave–Particle_Duality.secret]] 

