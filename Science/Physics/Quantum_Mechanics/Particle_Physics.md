---
aliases:
- Dalelių fizika
- deeltjesfysica
- Deeltjiefisika
- elementārdaļiņu fizika
- Ffiseg gronynnau
- fisic cháithníní
- Fisica dei particulas
- fisica delle particelle
- fisika partikel
- Fizica particulelor elementare
- Fizik di sé partikil
- Fizik patikil yo
- fizik zarah
- Fizika elementarnih čestica
- fizika osnovnih delcev
- Fiziko di partikuli
- fizyka cząstek elementarnych
- fyzika častíc
- fyzika částic
- Fìzega de łe partezełe
- física de partículas
- Física de partícules
- Fîzîka keriyan
- hissəciklər fizikası
- hiukkasfysiikka
- I-Particle physics
- osakestefüüsika
- Paatikl fizix
- pairticle pheesics
- particle physics
- partihkalfysihkka
- partikelfysik
- partikkelfysikk
- partikla fiziko
- Partikulen fisika
- parçacık fiziği
- Physica de particulas
- physica particularum elementariarum
- physique des particules
- Pisika han partikulo
- Pisikang pampartikulo
- Pudinidõ füüsiga
- részecskefizika
- Teilchenphysik
- vật lý hạt
- Öreindafræði
- čalmaašfyysiik
- Ŋñɩtɩɩɩ pe tɛ tomnaɣ kpɛlɛkʋʋ
- σωματιδιακή φυσική
- тӱҥлык пырче физике
- фізіка элементарных часціц
- фізика елементарних частинок
- фізыка элемэнтарных часьцінак
- физика елементарних честица
- Физика на елементарните частици
- физика на елементарните честички
- физика элементарных частиц
- физики заррот
- элементар өлөшсәләр физикаһы
- Элементарлă пĕрчĕсен физики
- элементарон хайыгты физикæ
- տարրական մասնիկների ֆիզիկա
- פיזיקת חלקיקים
- د ذراتو فزیک
- ذراتی طبیعیات
- فيزياء الجسيمات
- فيزيک ذره
- فیزیک ذرات
- कण भौतिकशास्त्र
- कण भौतिकी
- मूलकण भौतिकशास्त्र
- কণা পদার্থবিজ্ঞান
- কণা পদাৰ্থবিজ্ঞান
- ਕਣ ਭੌਤਿਕ ਵਿਗਿਆਨ
- କଣିକା ପଦାର୍ଥ ବିଜ୍ଞାନ
- துகள் இயற்பியல்
- కణ భౌతికశాస్త్రం
- ಕಣ ಭೌತಶಾಸ್ತ್ರ
- കണികാഭൗതികം
- අංශු භෞතික විද්යාව
- ฟิสิกส์พลังงานสูง
- အမှုန်ရူပဗေဒ
- ელემენტარული ნაწილაკების ფიზიკა
- 粒子物理学
- 粒子物理學
- 素粒子物理学
- 립자물리학
- 입자물리학
has_id_wikidata: Q18334
is_the_study_of: '[[_Standards/WikiData/WD~subatomic_particle,177013]]'
partially_coincident_with: '[[_Standards/WikiData/WD~high_energy_physics,350265]]'
part_of:
- '[[_Standards/WikiData/WD~modern_physics,658544]]'
- '[[_Standards/WikiData/WD~physics,413]]'
subclass_of: '[[_Standards/WikiData/WD~modern_physics,658544]]'
instance_of: '[[_Standards/WikiData/WD~branch_of_physics,4162444]]'
practiced_by: '[[_Standards/WikiData/WD~particle_physicist,63017728]]'
OmegaWiki_Defined_Meaning: 8229
image: http://commons.wikimedia.org/wiki/Special:FilePath/Institute%20of%20nuclear%20and%20particle%20physics%2000048%20REP%20SDR-65%20%280666%29.jpg
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q9610%20%28ben%29-Tahmid-%E0%A6%95%E0%A6%A3%E0%A6%BE%20%E0%A6%AA%E0%A6%A6%E0%A6%BE%E0%A6%B0%E0%A7%8D%E0%A6%A5%E0%A6%AC%E0%A6%BF%E0%A6%9C%E0%A7%8D%E0%A6%9E%E0%A6%BE%E0%A6%A8.wav
Stack_Exchange_tag: https://physics.stackexchange.com/tags/particle-physics
Dewey_Decimal_Classification: 539.72
IPTC_NewsCode: mediatopic/20000734
Commons_gallery: Particle physics
Commons_category: Particle physics
GitHub_topic: particle-physics
dv_has_:
  name_:
    af: Deeltjiefisika
    ar: فيزياء الجسيمات
    as: কণা পদাৰ্থবিজ্ঞান
    ast: Física de partícules
    az: hissəciklər fizikası
    ba: элементар өлөшсәләр физикаһы
    be: фізіка элементарных часціц
    be_tarask: фізыка элемэнтарных часьцінак
    bg: Физика на елементарните частици
    bho: कण भौतिकी
    bn: কণা পদার্থবিজ্ঞান
    bs: Fizika elementarnih čestica
    ca: física de partícules
    cs: fyzika částic
    cv: Элементарлă пĕрчĕсен физики
    cy: Ffiseg gronynnau
    da: partikelfysik
    de: Teilchenphysik
    el: σωματιδιακή φυσική
    en: particle physics
    en_ca: Particle physics
    en_gb: particle physics
    eo: partikla fiziko
    es: física de partículas
    et: osakestefüüsika
    eu: Partikulen fisika
    fa: فیزیک ذرات
    fi: hiukkasfysiikka
    fr: physique des particules
    ga: fisic cháithníní
    gcr: Fizik di sé partikil
    gl: física de partículas
    gsw: Teilchenphysik
    he: פיזיקת חלקיקים
    hi: कण भौतिकी
    hr: fizika elementarnih čestica
    ht: Fizik patikil yo
    hu: részecskefizika
    hy: տարրական մասնիկների ֆիզիկա
    ia: Physica de particulas
    id: fisika partikel
    io: Fiziko di partikuli
    is: Öreindafræði
    it: fisica delle particelle
    ja: 素粒子物理学
    jam: Paatikl fizix
    ka: ელემენტარული ნაწილაკების ფიზიკა
    kbp: Ŋñɩtɩɩɩ pe tɛ tomnaɣ kpɛlɛkʋʋ
    kn: ಕಣ ಭೌತಶಾಸ್ತ್ರ
    ko: 입자물리학
    ko-kp: 립자물리학
    ku: Fîzîka keriyan
    la: physica particularum elementariarum
    lt: Dalelių fizika
    lv: elementārdaļiņu fizika
    mag: कण भौतिकशास्त्र
    mhr: тӱҥлык пырче физике
    mk: физика на елементарните честички
    ml: കണികാഭൗതികം
    mr: मूलकण भौतिकशास्त्र
    ms: fizik zarah
    ms_arab: فيزيک ذره
    my: အမှုန်ရူပဗေဒ
    nb: partikkelfysikk
    nl: deeltjesfysica
    nn: partikkelfysikk
    oc: Fisica dei particulas
    or: କଣିକା ପଦାର୍ଥ ବିଜ୍ଞାନ
    os: элементарон хайыгты физикæ
    pa: ਕਣ ਭੌਤਿਕ ਵਿਗਿਆਨ
    pl: fizyka cząstek elementarnych
    ps: د ذراتو فزیک
    pt: física de partículas
    pt_br: física de partículas
    rki: အမှုန်ရူပဗေဒ
    ro: Fizica particulelor elementare
    ru: физика элементарных частиц
    sco: pairticle pheesics
    se: partihkalfysihkka
    sh: Fizika elementarnih čestica
    si: අංශු භෞතික විද්යාව
    sk: fyzika častíc
    sl: fizika osnovnih delcev
    smn: čalmaašfyysiik
    sms: partikkelfysikk
    sr: физика елементарних честица
    su: Fisika partikel
    sv: partikelfysik
    ta: துகள் இயற்பியல்
    te: కణ భౌతికశాస్త్రం
    tg: физики заррот
    th: ฟิสิกส์พลังงานสูง
    tl: Pisikang pampartikulo
    tr: parçacık fiziği
    uk: фізика елементарних частинок
    ur: ذراتی طبیعیات
    vec: Fìzega de łe partezełe
    vi: vật lý hạt
    vro: Pudinidõ füüsiga
    war: Pisika han partikulo
    wuu: 粒子物理学
    xh: I-Particle physics
    yue: 粒子物理學
    zh: 粒子物理學
    zh_hans: 粒子物理学
    zh_hant: 粒子物理學
---

# [[Particle_Physics]] 

#is_/same_as :: [[WD~Particle_physics,18334]] 

## #has_/text_of_/abstract 

> Particle physics or high-energy physics is the study of fundamental particles and forces 
> that constitute matter and radiation. 
> 
> The field also studies combinations of elementary particles up to the scale of protons and neutrons, 
> while the study of combinations of protons and neutrons is called nuclear physics.
>
> The fundamental particles in the universe are classified in the Standard Model as 
> - fermions (matter particles) and 
> - bosons (force-carrying particles). 
> 
> There are three generations of fermions, although ordinary matter is made only from the first fermion generation. The first generation consists of up and down quarks which form protons and neutrons, and electrons and electron neutrinos. The three fundamental interactions known to be mediated by bosons are electromagnetism, the weak interaction, and the strong interaction.
>
> Quarks form hadrons, but cannot exist on their own. Hadrons that contain an odd number of quarks are called baryons and those that contain an even number are called mesons. Two baryons, the proton and the neutron, make up most of the mass of ordinary matter. Mesons are unstable and the longest-lived last for only a few hundredths of a microsecond. They occur after collisions between particles made of quarks, such as fast-moving protons and neutrons in cosmic rays. Mesons are also produced in cyclotrons or other particle accelerators.
>
> Particles have corresponding antiparticles with the same mass but with opposite electric charges. For example, the antiparticle of the electron is the positron. The electron has a negative electric charge, the positron has a positive charge. These antiparticles can theoretically form a corresponding form of matter called antimatter. Some particles, such as the photon, are their own antiparticle.
>
> These elementary particles are excitations of the quantum fields that also govern their interactions. The dominant theory explaining these fundamental particles and fields, along with their dynamics, is called the Standard Model. The reconciliation of gravity to the current particle physics theory is not solved; many theories have addressed this problem, such as loop quantum gravity, string theory and supersymmetry theory.
>
> Experimental particle physics is the study of these particles in radioactive processes and in particle accelerators such as the Large Hadron Collider. Theoretical particle physics is the study of these particles in the context of cosmology and quantum theory. The two are closely interrelated: the Higgs boson was postulated theoretically before being confirmed by experiments.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Particle%20physics) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Quantum_Mechanics/Particle_Physics|Particle_Physics]] 

### #is_/same_as :: [[/_public/Science/Physics/Quantum_Mechanics/Particle_Physics.public|Particle_Physics.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Quantum_Mechanics/Particle_Physics.internal|Particle_Physics.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Quantum_Mechanics/Particle_Physics.protect|Particle_Physics.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Quantum_Mechanics/Particle_Physics.private|Particle_Physics.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Quantum_Mechanics/Particle_Physics.personal|Particle_Physics.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Quantum_Mechanics/Particle_Physics.secret|Particle_Physics.secret]] 

