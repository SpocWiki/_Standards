---
aliases:
- Asas larangan Pauli
- Exkluzo-principo di Pauli
- Kanuni ya Pauli
- nguyên lý loại trừ Pauli
- Parimi i përjashtimit i Paulit
- Pauli bók-siŏng-ṳ̀ng nguòng-lī
- Pauli bô-chò-hoé goân-lí
- Pauli dışlama ilkesi
- Pauli Exclusion Principle
- 'Pauli exclusion principle (German: Pauli-Ausschlussprinzip'
- Pauli istisno prinsipi
- Pauli princips
- Pauli printsiip
- Pauli-elv
- Pauli-Prinzip
- Pauliho vylučovací princip
- Pauliho vylučovací princíp
- Paulijev princip
- Paulijev princip isključenja
- Paulijevo izključitveno načelo
- Paulin kieltosääntö
- Paulio draudimo principas
- Pauliren bazterketa printzipioa
- Paulis eksklusjonsprinsipp
- Paulis udelukkelsesprincip
- Paulis uteslutningsprincip
- Pauli_Exclusion_Principle
- principe d'exclusion de Pauli
- Principi d'exclusion de Pauli
- Principi d'exclusió de Pauli
- Principio de exclusión de Pauli
- principio di esclusione di Pauli
- Principiu d'esclusión de Pauli
- Principiul de excluziune
- Principium exclusionis Pauli
- principo de ekskludo
- Princípio de exclusão de Pauli
- Prinsip pengecualian Pauli
- Prinsipi d'esclusion ëd Pauli
- Prionsabal eisiaimh Pauli
- reguła Pauliego
- Uitsluitingsbeginsel van Pauli
- Uitsluitingsprincipe van Pauli
- Απαγορευτική αρχή του Πάουλι
- Паули мәсләге
- Паули принцип
- Паули хасах зарчим
- Паулијев принцип
- Паулиево начело
- Паулилэн инъетэз
- Принцип виключення Паулі
- Принцип на Паули
- Принцип Паули
- Прынцып Паўлі
- Պաուլիի սկզբունք
- עקרון האיסור של פאולי
- اصل طرد پائولی
- مبدأ استبعاد باولي
- پالی اصول
- पाउली अपवर्जन नियम
- পাউলির অপবর্জন নীতি
- পাউলিৰ নিষেধ নীতি
- પાઉલીનો અપવર્જનનો નિયમ
- பவுலி தவிர்ப்புத் தத்துவம்
- അപവർജന നിയമം
- หลักการกีดกันของเพาลี
- პაულის პრინციპი
- パウリの排他原理
- 包立不相容原理
- 泡利不相容原理
- 泡利弗相容原理
- 鮑利不相容原理
- 파울리 배타 원리
has_id_wikidata: Q131594
named_after: '[[_Standards/WikiData/WD~Wolfgang_Pauli,65989|WD~Wolfgang_Pauli,65989]]'
discoverer_or_inventor: '[[_Standards/WikiData/WD~Wolfgang_Pauli,65989|WD~Wolfgang_Pauli,65989]]'
subclass_of: '[[_Standards/WikiData/WD~principle_of_identity,4378893|WD~principle_of_identity,4378893]]'
instance_of: '[[_Standards/WikiData/WD~education_theory,5341232|WD~education_theory,5341232]]'
facet_of: '[[_Standards/WikiData/WD~quantum_mechanics,944|WD~quantum_mechanics,944]]'
Wolfram_Language_entity_code: Entity["PhysicalEffect", "PauliExclusionPrinciple"]
dv_has_:
  name_:
    af: Uitsluitingsbeginsel van Pauli
    ar: مبدأ استبعاد باولي
    as: পাউলিৰ নিষেধ নীতি
    ast: Principiu d'esclusión de Pauli
    be: Прынцып Паўлі
    be_tarask: прынцып Паўлі
    bg: Принцип на Паули
    bn: পাউলির অপবর্জন নীতি
    bs: Paulijev princip isključenja
    ca: Principi d'exclusió de Pauli
    cdo: Pauli bók-siŏng-ṳ̀ng nguòng-lī
    cs: Pauliho vylučovací princip
    da: Paulis udelukkelsesprincip
    de: Pauli-Prinzip
    el: Απαγορευτική αρχή του Πάουλι
    en: Pauli exclusion principle
    eo: principo de ekskludo
    es: Principio de exclusión de Pauli
    et: Pauli printsiip
    eu: Pauliren bazterketa printzipioa
    fa: اصل طرد پائولی
    fi: Paulin kieltosääntö
    fr: principe d'exclusion de Pauli
    ga: Prionsabal eisiaimh Pauli
    gl: Principio de exclusión de Pauli
    gu: પાઉલીનો અપવર્જનનો નિયમ
    he: עקרון האיסור של פאולי
    hi: पाउली अपवर्जन नियम
    hr: Paulijev princip isključenja
    hu: Pauli-elv
    hy: Պաուլիի սկզբունք
    id: Asas larangan Pauli
    io: Exkluzo-principo di Pauli
    it: principio di esclusione di Pauli
    ja: パウリの排他原理
    ka: პაულის პრინციპი
    kk: Паули принцип
    ko: 파울리 배타 원리
    la: Principium exclusionis Pauli
    lt: Paulio draudimo principas
    lv: Pauli princips
    lzh: 泡利不相容原理
    mk: Паулиево начело
    ml: അപവർജന നിയമം
    mn: Паули хасах зарчим
    ms: Prinsip pengecualian Pauli
    nan: Pauli bô-chò-hoé goân-lí
    nb: Paulis eksklusjonsprinsipp
    nl: Uitsluitingsprincipe van Pauli
    nn: Paulis eksklusjonsprinsipp
    oc: Principi d'exclusion de Pauli
    pl: reguła Pauliego
    pms: Prinsipi d'esclusion ëd Pauli
    pnb: پالی اصول
    pt: Princípio de exclusão de Pauli
    ro: Principiul de excluziune
    ru: Принцип Паули
    sco: Pauli exclusion principle
    sh: Paulijev princip isključenja
    sk: Pauliho vylučovací princíp
    sl: Paulijevo izključitveno načelo
    sq: Parimi i përjashtimit i Paulit
    sr: Паулијев принцип
    sr_ec: Паулијев принцип
    sr_el: Paulijev princip
    sv: Paulis uteslutningsprincip
    sw: Kanuni ya Pauli
    ta: பவுலி தவிர்ப்புத் தத்துவம்
    th: หลักการกีดกันของเพาลี
    tr: Pauli dışlama ilkesi
    tt: Паули мәсләге
    udm: Паулилэн инъетэз
    uk: Принцип виключення Паулі
    uz: Pauli istisno prinsipi
    vi: nguyên lý loại trừ Pauli
    wuu: 泡利弗相容原理
    yue: 鮑利不相容原理
    zh: 泡利不相容原理
    zh_cn: 泡利不相容原理
    zh_hans: 泡利不相容原理
    zh_hant: 包立不相容原理
    zh_hk: 包立不相容原理
    zh_sg: 泡利不相容原理
    zh_tw: 包立不相容原理
---

# [[Pauli_Exclusion_Principle]] 

#is_/same_as :: [[_Standards/WikiData/WD~Pauli_exclusion_principle,131594|WD~Pauli_exclusion_principle,131594]] 

## #has_/text_of_/abstract 

> In quantum mechanics, the Pauli exclusion principle (German: Pauli-Ausschlussprinzip) 
> states that two or more identical particles with half-integer spins (i.e. fermions) 
> cannot simultaneously occupy the same quantum state within a system. 
> 
> This principle was formulated by Austrian physicist Wolfgang Pauli 
> in 1925 for electrons, 
> and later extended to all fermions with his spin–statistics theorem of 1940.
>
> In the case of electrons in atoms, the exclusion principle can be stated as follows: 
> in a poly-electron atom it is impossible for any two electrons 
> to have the same two values of all four of their quantum numbers, which are: 
> - n, the principal quantum number; 
> - ℓ, the azimuthal quantum number; 
> - mℓ, the magnetic quantum number; and 
> - ms, the spin quantum number. 
> For example, if two electrons reside in the same orbital, 
> then their values of n, ℓ, and mℓ are equal. 
> In that case, the two values of ms (spin) pair must be different. 
> 
> Since the only two possible values for the spin projection ms are +1/2 and -1/2, 
> it follows that one electron must have ms = +1/2 and one ms = -1/2.
>
> Particles with an integer spin (bosons) are not subject to the Pauli exclusion principle. 
> Any number of identical bosons can occupy the same quantum state, 
> such as photons produced by a laser, or atoms found in a Bose–Einstein condensate.
>
> A rigorous statement which justifies the exclusion principle is: 
> under the exchange of two identical particles, 
> the total (many-particle) wave function is **antisymmetric for fermions** 
> and **symmetric for bosons**. 
> 
> This means that if the space and spin coordinates of two identical particles 
> are interchanged, then the total wave function changes sign for fermions, 
> but does not change sign for bosons. 
> 
> So, if hypothetically two fermions were in the same state—
> for example, in the same atom in the same orbital with the same spin—
> then interchanging them would change nothing and the total wave function would be unchanged. 
> 
> However, the only way a total wave function can both change sign 
> (which is required for fermions), and also remain unchanged, 
> is that such a function must be zero everywhere, which means such a state cannot exist. 
> 
> This reasoning does not apply to bosons because the sign does not change.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Pauli%20exclusion%20principle) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Quantum_Mechanics/Standard_Model/Fermion/Pauli_Exclusion_Principle|Pauli_Exclusion_Principle]] 

### #is_/same_as :: [[/_public/Science/Physics/Quantum_Mechanics/Standard_Model/Fermion/Pauli_Exclusion_Principle.public|Pauli_Exclusion_Principle.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Quantum_Mechanics/Standard_Model/Fermion/Pauli_Exclusion_Principle.internal|Pauli_Exclusion_Principle.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Quantum_Mechanics/Standard_Model/Fermion/Pauli_Exclusion_Principle.protect|Pauli_Exclusion_Principle.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Quantum_Mechanics/Standard_Model/Fermion/Pauli_Exclusion_Principle.private|Pauli_Exclusion_Principle.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Quantum_Mechanics/Standard_Model/Fermion/Pauli_Exclusion_Principle.personal|Pauli_Exclusion_Principle.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Quantum_Mechanics/Standard_Model/Fermion/Pauli_Exclusion_Principle.secret|Pauli_Exclusion_Principle.secret]] 

