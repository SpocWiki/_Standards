---
aliases:
- alkeishiukkanen
- cząstka elementarna
- cáithníní bunúsacha
- elementa partiklo
- Elementaardeelken
- elementaarosakesed
- elementair deeltje
- elementair deilkes
- elementar hissəcik
- elementar zarracha
- Elementaraitzal
- Elementardeelchen
- Elementardeelken
- elementarioji dalelė
- elementarna čestica
- Elementarpaat
- elementarpartikel
- Elementarteilchen
- elementary particle
- elementarya nga partikulo
- Elementary_Particle
- elementárna častica
- elementární částice
- elementærpartikkel
- elementêre deeltjie
- elementārdaļiņa
- elemi részecske
- fundamental particle
- funtsezko partikula
- grimca elementare
- hạt sơ cấp
- INhlayiyana eyisiseko
- keriyê hîmanî
- ki-pún lia̍p-chú
- osnovni delec
- partezela elementar
- particedda elimintari
- particella elementare
- particla elemental
- particula elementara
- particula elementaris
- particule élémentaire
- particulă elementară
- partikel dasar
- partikel éleménter
- partissela elementar
- partícula eilementar
- partícula elemental
- partícula elementar
- partíkula elementár
- primara partikulo
- rannig elfennel
- temel parçacık
- tiksi k'atacha
- vuođđopartihkkal
- vuâđđpartikkel
- zarah asas
- öreind
- στοιχειώδες σωματίδιο
- елементарна частица
- елементарна честица
- елементарна честичка
- Зарраҳои бунёдӣ
- никĕсле пĕрчĕ
- физикийн дакъалг
- фундаментальна частинка
- фундаментальная частица
- фундаментальная часціца
- эгэл бөөм
- элементар кисәкчек
- элемэнтарная часьцінка
- қарапайым бөлшектер
- տարրական մասնիկներ
- חלקיק יסודי
- بنوردیلە
- بنیادی ذرهلر
- بنیادی ذرہ
- بنیادی ٹوٹا
- جسيم أولي
- ذرات بنیادی
- فرتيتة أولية
- मूलभूत कण
- मौलिक कन
- মৌলিক কণা
- ਮੁੱਢਲਾ ਕਣ
- அடிப்படைத் துகள்
- ప్రాథమిక కణం
- ಮೂಲಕಣಗಳು
- അടിസ്ഥാനകണം
- อนุภาคมูลฐาน
- ခြေခံအမှုန်များ
- ელემენტარული ნაწილაკი
- 元粒
- 基本粒子
- 素粒子
- 기본 입자
has_id_wikidata: Q43116
opposite_of: '[[_Standards/WikiData/WD~composite_particle,3366856|WD~composite_particle,3366856]]'
part_of: '[[_Standards/WikiData/WD~composite_particle,3366856|WD~composite_particle,3366856]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~fundamental_particle,11426419|WD~fundamental_particle,11426419]]'
instance_of: '[[_Standards/WikiData/WD~type_of_quantum_particle,22675015|WD~type_of_quantum_particle,22675015]]'
disjoint_union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486|WD~list_of_values_as_qualifiers,23766486]]'
different_from:
- '[[_Standards/WikiData/WD~quantum_particle,28693603|WD~quantum_particle,28693603]]'
- '[[_Standards/WikiData/WD~indivisible_particle,28846113|WD~indivisible_particle,28846113]]'
subclass_of:
- '[[_Standards/WikiData/WD~indivisible_particle,28846113|WD~indivisible_particle,28846113]]'
- '[[_Standards/WikiData/WD~elementary_object,63243922|WD~elementary_object,63243922]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._11,124355862|WD~Armenian_Soviet_Encyclopedia,_vol._11,124355862]]'
interaction: '[[_Standards/WikiData/WD~gravity,11412|WD~gravity,11412]]'
antiparticle: '[[_Standards/WikiData/WD~elementary_particle,43116|WD~elementary_particle,43116]]'
OmegaWiki_Defined_Meaning: 8222
Dewey_Decimal_Classification: 539.721
image: http://commons.wikimedia.org/wiki/Special:FilePath/Standard%20Model%20of%20Elementary%20Particles.svg
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q188%20%28deu%29-Michael%20Schoenitzer%20%28MichaelSchoenitzer%29-Elementarteilchen.wav
IEV_number:
- 113-05-02
- 881-02-45
Commons_category: Elementary particles
MeSH_tree_code: G01.249
dv_has_:
  name_:
    af: elementêre deeltjie
    an: particla elemental
    ar: جسيم أولي
    ary: فرتيتة أولية
    ast: partícula elemental
    az: elementar hissəcik
    azb: بنیادی ذرهلر
    bar: Elementaraitzal
    be: фундаментальная часціца
    be_tarask: элемэнтарная часьцінка
    bg: елементарна частица
    bho: मौलिक कन
    bn: মৌলিক কণা
    br: rannig elfennel
    bs: elementarna čestica
    ca: partícula elemental
    ce: физикийн дакъалг
    ckb: بنوردیلە
    cs: elementární částice
    cv: никĕсле пĕрчĕ
    da: elementarpartikel
    de: Elementarteilchen
    el: στοιχειώδες σωματίδιο
    en: elementary particle
    en_ca: elementary particle
    eo: elementa partiklo
    es: partícula elemental
    et: elementaarosakesed
    eu: funtsezko partikula
    fa: ذرات بنیادی
    fi: alkeishiukkanen
    fr: particule élémentaire
    frr: Elementaardeelken
    ga: cáithníní bunúsacha
    gl: partícula elemental
    gsw: Elementarteilchen
    he: חלקיק יסודי
    hi: मूलभूत कण
    hr: elementarna čestica
    hu: elemi részecske
    hy: տարրական մասնիկներ
    id: partikel dasar
    io: primara partikulo
    is: öreind
    it: particella elementare
    ja: 素粒子
    ka: ელემენტარული ნაწილაკი
    kk: қарапайым бөлшектер
    kn: ಮೂಲಕಣಗಳು
    ko: 기본 입자
    ku: keriyê hîmanî
    la: particula elementaris
    lb: Elementardeelchen
    li: elementair deilkes
    lmo: partezela elementar
    lt: elementarioji dalelė
    lv: elementārdaļiņa
    lzh: 元粒
    mag: मूलभूत कण
    mk: елементарна честичка
    ml: അടിസ്ഥാനകണം
    mn: эгэл бөөм
    mr: मूलभूत कण
    ms: zarah asas
    mwl: partícula eilementar
    my: ခြေခံအမှုန်များ
    nan: ki-pún lia̍p-chú
    nb: elementærpartikkel
    nds: Elementardeelken
    ne: मूलभूत कण
    nl: elementair deeltje
    nn: elementærpartikkel
    oc: particula elementara
    pa: ਮੁੱਢਲਾ ਕਣ
    pl: cząstka elementarna
    pms: partissela elementar
    pnb: بنیادی ٹوٹا
    pt: partícula elementar
    qu: tiksi k'atacha
    ro: particulă elementară
    ru: фундаментальная частица
    scn: particedda elimintari
    se: vuođđopartihkkal
    sh: elementarna čestica
    sk: elementárna častica
    sl: osnovni delec
    sms: vuâđđpartikkel
    sq: grimca elementare
    sr: елементарна честица
    sr_ec: елементарна честица
    sr_el: elementarna čestica
    stq: Elementarpaat
    su: partikel éleménter
    sv: elementarpartikel
    ta: அடிப்படைத் துகள்
    te: ప్రాథమిక కణం
    tet: partíkula elementár
    tg: Зарраҳои бунёдӣ
    th: อนุภาคมูลฐาน
    tr: temel parçacık
    tt: элементар кисәкчек
    uk: фундаментальна частинка
    ur: بنیادی ذرہ
    uz: elementar zarracha
    vi: hạt sơ cấp
    war: elementarya nga partikulo
    wuu: 基本粒子
    yue: 基本粒子
    zh: 基本粒子
    zu: INhlayiyana eyisiseko
---

# [[Elementary_Particle]] 

#is_/same_as :: [[../../../../WikiData/WD~Elementary_particle,43116|WD~Elementary_particle,43116]] 

## #has_/text_of_/abstract 

> In particle physics, an elementary particle or fundamental particle 
> is a  subatomic particle that is not composed of other particles. 
> 
> The [[../Standard_Model|Standard Model]] presently recognizes seventeen distinct particles—twelve fermions and five bosons. 
> As a consequence of flavor and color combinations and antimatter, 
> the fermions and bosons are known to have 48 and 13 variations, respectively. 
> Among the 61 elementary particles embraced by the Standard Model number: electrons and other leptons, quarks, and the fundamental bosons. Subatomic particles such as protons or neutrons, which contain two or more elementary particles, are known as composite particles.
>
> Ordinary matter is composed of atoms, themselves once thought to be indivisible elementary particles. 
> The name atom comes from the Ancient Greek word ἄτομος (atomos) which means indivisible or uncuttable. 
> Despite the theories about atoms that had existed for thousands of years, the factual existence of atoms remained controversial until 1905. In that year, Albert Einstein published his paper on Brownian motion, putting to rest theories that had regarded molecules as mathematical illusions. Einstein subsequently identified matter as ultimately composed of various concentrations of energy.
>
> Subatomic constituents of the atom were first identified toward the end of the 19th century, beginning with the electron, followed by the proton in 1919, the photon in the 1920s, and the neutron in 1932. By that time, the advent of quantum mechanics had radically altered the definition of a "particle" by putting forward an understanding in which they carried out a simultaneous existence as matter waves.
>
> Many theoretical elaborations upon, and beyond, the Standard Model have been made since its codification in the 1970s. These include notions of supersymmetry, which double the number of elementary particles by hypothesizing that each known particle associates with a "shadow" partner far more massive. However, like an additional elementary boson mediating gravitation, such superpartners remain undiscovered as of 2025.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Elementary%20particle) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Quantum_Mechanics/Standard_Model/Elementary_Particle|Elementary_Particle]] 

### #is_/same_as :: [[/_public/Science/Physics/Quantum_Mechanics/Standard_Model/Elementary_Particle.public|Elementary_Particle.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Quantum_Mechanics/Standard_Model/Elementary_Particle.internal|Elementary_Particle.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Quantum_Mechanics/Standard_Model/Elementary_Particle.protect|Elementary_Particle.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Quantum_Mechanics/Standard_Model/Elementary_Particle.private|Elementary_Particle.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Quantum_Mechanics/Standard_Model/Elementary_Particle.personal|Elementary_Particle.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Quantum_Mechanics/Standard_Model/Elementary_Particle.secret|Elementary_Particle.secret]] 

