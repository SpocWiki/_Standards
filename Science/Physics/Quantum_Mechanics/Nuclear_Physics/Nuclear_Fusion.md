---
aliases:
- azday aɣisan
- Babalina niwukilaara
- Branduolių sąlaja
- comhleá núicléach
- Fizyon
- fusi nuklir
- Fusio nuclearis
- Fusio nuklear
- fusion
- Fusion nuclear
- Fusion nucleara
- fusion nucléaire
- fusione nucleare
- fusioni nucliari
- fusió nuclear
- fusión nuclear
- Fusión nucleyar
- Fusyon nuclear
- fusão nuclear
- fuusio
- fuusioreaktio
- Fuzija
- fuziune nucleară
- Fuzyono nukleer
- Füzyon
- Hu̍t-chú iông-ha̍p
- jaderná fúze
- Jadrová syntéza
- jedrsko zlivanje
- Jehe'a mbytégua
- Kearnfúzje
- kernefusion
- kernfusie
- Kernfusion
- Kjarnasamruni
- kjernefusjon
- kjernefysisk fusjon
- kodolsintēze
- Käädenfusion
- Lakuran nuklear
- magfúzió
- Myeyungano wa kinyuklia
- nuclear fusion
- Nuclear_Fusion
- nuklea fuzio
- nuklearna fuzija
- Phản ứng tổng hợp hạt nhân
- reakcja termojądrowa
- Runyenguko rweShomwe
- Termonüvə reaksiyası
- Termoyadroviy reaksiya
- tuumaühinemine
- Ukuxubana kwenuzi
- Ymasiad niwclear
- Πυρηνική σύντηξη
- јадрено соединување
- нуклеарна фузија
- Термо нуклонсар реакци
- Термотөш реакциясе
- термоядерная реакция
- термоядрен синтез
- Термоядролық реакциялар
- Тэрмаядзерная рэакцыя
- Цөмийн нэгдэл
- Ядерний синтез
- ջերմամիջուկային ռեակցիա
- היתוך גרעיני
- اندماج نووي
- ایٹمی فیوژن
- فوزیون
- ناوکەیەکگرتن
- نديماج نووي
- نویاتی ائتلاف
- هستوي ترکیب
- همجوشی هستهای
- नाभिकीय संलयन
- কেন্দ্রীণ সংযোজন
- ਨਿਊਕਲੀ ਮੇਲ
- அணுக்கரு இணைவு
- కేంద్రక సంలీనం
- ಬೈಜಿಕ ಸಂಲಯನ
- അണുസംയോജനം
- න් යෂ්ටික විලයනය
- นิวเคลียร์ฟิวชั่น
- နျူကလီးယား ပေါင်းစည်းမှု
- နျူးကလီးယား ခြွဉ်းဗူႏဝင်ꩻ
- თერმობირთვული რეაქცია
- 原子核融合
- 核聚变
- 核融合
- 핵융합
has_id_wikidata: Q13082
studied_by: '[[_Standards/WikiData/WD~nuclear_physics,81197|WD~nuclear_physics,81197]]'
different_from:
- '[[_Standards/WikiData/WD~nuclear_meltdown,105807|WD~nuclear_meltdown,105807]]'
- '[[_Standards/WikiData/WD~melting,106080|WD~melting,106080]]'
- '[[_Standards/WikiData/WD~fusion,398377|WD~fusion,398377]]'
- '[[_Standards/WikiData/WD~karyogamy,1454719|WD~karyogamy,1454719]]'
subclass_of:
- '[[_Standards/WikiData/WD~nuclear_reaction,238323|WD~nuclear_reaction,238323]]'
- '[[_Standards/WikiData/WD~connection,12819913|WD~connection,12819913]]'
has_use: '[[_Standards/WikiData/WD~fusion_power,641442|WD~fusion_power,641442]]'
instance_of: '[[_Standards/WikiData/WD~concept_in_physics,33104303|WD~concept_in_physics,33104303]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._9,124737636|WD~Armenian_Soviet_Encyclopedia,_vol._9,124737636]]'
OmegaWiki_Defined_Meaning: 2575
image: http://commons.wikimedia.org/wiki/Special:FilePath/Nuclear%20fusion.gif
U_S_National_Archives_Identifier: 10641972
Basisklassifikation: 33.81
IEV_number: 881-03-55
UMLS_CUI: C0028577
MeSH_tree_code:
- G01.536.652
- N06.230.132.580.520
Krugosvet_article: nauka_i_tehnika/tehnologiya_i_promyshlennost/YADERNI_SINTEZ.html
Commons_gallery: Nuclear fusion
Commons_category: Nuclear fusion
Library_of_Congress_Classification: QC790.95-QC791.8
dv_has_:
  name_:
    af: kernfusie
    an: Fusión nucleyar
    ar: اندماج نووي
    ary: نديماج نووي
    ast: Fusión nuclear
    az: Termonüvə reaksiyası
    azb: فوزیون
    be: Тэрмаядзерная рэакцыя
    be_tarask: тэрмаядзерная рэакцыя
    bg: термоядрен синтез
    blk: နျူးကလီးယား ခြွဉ်းဗူႏဝင်ꩻ
    bn: কেন্দ্রীণ সংযোজন
    bs: nuklearna fuzija
    ca: fusió nuclear
    ckb: ناوکەیەکگرتن
    cs: jaderná fúze
    cv: Термо нуклонсар реакци
    cy: Ymasiad niwclear
    da: kernefusion
    de: Kernfusion
    diq: Fuzyono nukleer
    el: Πυρηνική σύντηξη
    en: nuclear fusion
    eo: nuklea fuzio
    es: fusión nuclear
    et: tuumaühinemine
    eu: Fusio nuklear
    fa: همجوشی هستهای
    fi: fuusioreaktio
    fr: fusion nucléaire
    fy: Kearnfúzje
    ga: comhleá núicléach
    gl: fusión nuclear
    gn: Jehe'a mbytégua
    he: היתוך גרעיני
    hi: नाभिकीय संलयन
    hr: Nuklearna fuzija
    ht: Fizyon
    hu: magfúzió
    hy: ջերմամիջուկային ռեակցիա
    id: fusi nuklir
    is: Kjarnasamruni
    it: fusione nucleare
    ja: 原子核融合
    jv: fusi nuklir
    ka: თერმობირთვული რეაქცია
    kab: azday aɣisan
    kk: Термоядролық реакциялар
    kn: ಬೈಜಿಕ ಸಂಲಯನ
    ko: 핵융합
    la: Fusio nuclearis
    li: Kernfusie
    lt: Branduolių sąlaja
    lv: kodolsintēze
    lzh: 核融合
    mk: јадрено соединување
    ml: അണുസംയോജനം
    mn: Цөмийн нэгдэл
    ms: Lakuran nuklear
    my: နျူကလီးယား ပေါင်းစည်းမှု
    nan: Hu̍t-chú iông-ha̍p
    nb: kjernefysisk fusjon
    ne: नाभिकीय संलयन
    nl: kernfusie
    nn: kjernefusjon
    oc: Fusion nucleara
    om: Babalina niwukilaara
    pa: ਨਿਊਕਲੀ ਮੇਲ
    pl: reakcja termojądrowa
    pms: Fusion nuclear
    pnb: ایٹمی فیوژن
    ps: هستوي ترکیب
    pt: fusão nuclear
    ro: fuziune nucleară
    ru: термоядерная реакция
    scn: fusioni nucliari
    sco: nuclear fusion
    sh: Fuzija
    si: න් යෂ්ටික විලයනය
    sk: Jadrová syntéza
    sl: jedrsko zlivanje
    smj: fuusio
    smn: fuusio
    sms: fuusio
    sn: Runyenguko rweShomwe
    sr: нуклеарна фузија
    stq: Käädenfusion
    su: Fusi nuklir
    sv: fusion
    sw: Myeyungano wa kinyuklia
    ta: அணுக்கரு இணைவு
    te: కేంద్రక సంలీనం
    th: นิวเคลียร์ฟิวชั่น
    tr: Füzyon
    tt: Термотөш реакциясе
    uk: Ядерний синтез
    ur: نویاتی ائتلاف
    uz: Termoyadroviy reaksiya
    vi: Phản ứng tổng hợp hạt nhân
    war: Fusyon nuclear
    wuu: 核聚变
    yue: 原子核融合
    zh: 核聚变
    zh_hans: 核聚变
    zh_tw: 核融合
    zu: Ukuxubana kwenuzi
---

# [[Nuclear_Fusion]] 

#is_/same_as :: [[../../../../WikiData/WD~Nuclear_fusion,13082|WD~Nuclear_fusion,13082]] 

## #has_/text_of_/abstract 

> **nuclear fusion** is a reaction in which two or more atomic nuclei combine to form a larger nucleus. 
> The difference in mass between the reactants and products is manifested 
> as either the release or absorption of energy. 
> 
> This difference in mass arises as a result of the difference in nuclear binding energy 
> between the atomic nuclei before and after the fusion reaction. 
> Nuclear fusion is the process that powers all active stars, via many reaction pathways.
>
> Fusion processes require an extremely large triple product of temperature, density, and confinement time. These conditions occur only in stellar cores, advanced nuclear weapons, and are approached in fusion power experiments.
>
> A nuclear fusion process that produces atomic nuclei lighter than nickel-62 is generally exothermic, due to the positive gradient of the nuclear binding energy curve. The most fusible nuclei are among the lightest, especially deuterium, tritium, and helium-3. The opposite process, nuclear fission, is most energetic for very heavy nuclei, especially the actinides.
>
> Applications of fusion include fusion power, thermonuclear weapons, boosted fission weapons, neutron sources, and superheavy element production.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Nuclear%20fusion) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Quantum_Mechanics/Nuclear_Physics/Nuclear_Fusion|Nuclear_Fusion]] 

### #is_/same_as :: [[/_public/Science/Physics/Quantum_Mechanics/Nuclear_Physics/Nuclear_Fusion.public|Nuclear_Fusion.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Quantum_Mechanics/Nuclear_Physics/Nuclear_Fusion.internal|Nuclear_Fusion.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Quantum_Mechanics/Nuclear_Physics/Nuclear_Fusion.protect|Nuclear_Fusion.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Quantum_Mechanics/Nuclear_Physics/Nuclear_Fusion.private|Nuclear_Fusion.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Quantum_Mechanics/Nuclear_Physics/Nuclear_Fusion.personal|Nuclear_Fusion.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Quantum_Mechanics/Nuclear_Physics/Nuclear_Fusion.secret|Nuclear_Fusion.secret]] 

