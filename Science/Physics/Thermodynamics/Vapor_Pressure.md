---
aliases:
- brú gaile
- Buhar basıncı
- ciśnienie pary nasyconej
- dampdruk
- Dampfdruck
- damptrykk
- Gufuþrýstingur
- gőznyomás
- höyrynpaine
- küllastunud auru rõhk
- lievlâteedâ
- lurrun-presio
- Napon pare
- parni tlak
- Penetan uwab
- presion a vapor
- presiune de vapori
- presión de vapor
- Pressio vaporis
- pression de vapeur
- Pression de vapor satur
- pressione di vapore
- pressió de vapor
- pressão de vapor
- Presyon vapè
- Pritisak pare
- Tekanan uap
- Tekanan wap
- Tensioni di vapuri
- tlak vodní páry
- vapor pressure
- vaporpremo
- vapour pressure
- áp suất hơi
- ångtryck
- æфсæст тæф
- Τάση ατμών
- Напон паре
- насичена пара
- Насычаная пара
- Насыщенный пар
- парен притисок
- парно налягане
- Тăраннă пăс
- тырмыт ву бусо омыр
- Уурын даралт
- Հագեցած գոլորշի
- לחץ אדים
- بخاری دباؤ
- ضغط البخار
- فشار بخار
- भाप दाब
- वाष्प दाब
- বাষ্প চাপ
- ஆவியமுக்கம்
- బాష్ప పీడనం
- ความดันไอ
- 蒸气压
- 蒸気圧
- 蒸氣壓
- 증기압
has_id_wikidata: Q2661322
subclass_of:
- '[[_Standards/WikiData/WD~chemical_quantity,71550700]]'
- '[[_Standards/WikiData/WD~pressure,39552]]'
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633]]'
quantity_symbol_LaTeX_: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><msub><mi>p</mi><mrow data-mjx-texclass="ORD"><mrow data-mjx-texclass="ORD"><mi mathvariant="normal">v</mi></mrow></mrow></msub></mstyle></mrow></math>
ISQ_dimension: "<math class=\"mwe-math-element mwe-math-element-inline\" xmlns=\"http://www.w3.org/1998/Math/MathML\"><mrow data-mjx-texclass=\"ORD\"><mstyle displaystyle=\"true\" scriptlevel=\"0\"><msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5AB</mi></mrow><mrow data-mjx-texclass=\"ORD\"><mrow data-mjx-texclass=\"ORD\"><mo stretchy=\"false\">&#x2212;</mo><mn>1</mn></mrow></mrow></msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5AC</mi></mrow><msup><mrow data-mjx-texclass=\"ORD\"><mi>\U0001D5B3</mi></mrow><mrow data-mjx-texclass=\"ORD\"><mrow data-mjx-texclass=\"ORD\"><mo stretchy=\"false\">&#x2212;</mo><mn>2</mn></mrow></mrow></msup></mstyle></mrow></math>"
main_Wikidata_property: vapor pressure
studied_by: '[[_Standards/WikiData/WD~thermodynamics,11473]]'
video: http://commons.wikimedia.org/wiki/Special:FilePath/02.%D0%9F%D0%B8%D1%88%D1%82%D0%BE%D0%BB%20%D0%B5%D0%BF%D1%80%D1%83%D0%B2%D0%B5%D1%82%D0%B0%20%D1%81%D0%BE%20%D0%B0%D0%BB%D0%BA%D0%BE%D1%85%D0%BE%D0%BB.ogv
image: http://commons.wikimedia.org/wiki/Special:FilePath/Vapor%20pressure.svg
UMLS_CUI: C0563290
MeSH_tree_code: G01.374.715.951
dv_has_:
  name_:
    af: Dampdruk
    an: Presión de vapor
    ar: ضغط البخار
    ast: Presión de vapor
    be: Насычаная пара
    be-tarask: насычаная пара
    bg: парно налягане
    bho: भाप दाब
    bn: বাষ্প চাপ
    bs: Pritisak pare
    ca: pressió de vapor
    cs: tlak vodní páry
    cv: Тăраннă пăс
    de: Dampfdruck
    el: Τάση ατμών
    en: vapor pressure
    en-gb: vapour pressure
    eo: vaporpremo
    es: presión de vapor
    et: küllastunud auru rõhk
    eu: lurrun-presio
    fa: فشار بخار
    fi: höyrynpaine
    fr: pression de vapeur
    ga: brú gaile
    gl: presión de vapor
    he: לחץ אדים
    hi: वाष्प दाब
    ht: Presyon vapè
    hu: gőznyomás
    hy: Հագեցած գոլորշի
    id: Tekanan uap
    is: Gufuþrýstingur
    it: pressione di vapore
    ja: 蒸気圧
    jv: Penetan uwab
    ko: 증기압
    la: Pressio vaporis
    lmo: Pression de vapor satur
    mk: парен притисок
    mn: Уурын даралт
    ms: Tekanan wap
    nb: damptrykk
    nl: dampdruk
    nn: damptrykk
    os: æфсæст тæф
    pl: ciśnienie pary nasyconej
    pt: pressão de vapor
    ro: presiune de vapori
    ru: Насыщенный пар
    scn: Tensioni di vapuri
    sh: Napon pare
    sl: parni tlak
    smn: lievlâteedâ
    sr: Напон паре
    sr-ec: Напон паре
    sv: ångtryck
    ta: ஆவியமுக்கம்
    te: బాష్ప పీడనం
    th: ความดันไอ
    tr: Buhar basıncı
    udm: тырмыт ву бусо омыр
    uk: насичена пара
    ur: بخاری دباؤ
    vec: presion a vapor
    vi: áp suất hơi
    wuu: 蒸气压
    yue: 蒸氣壓
    zh: 蒸氣壓
---
# [[Vapor_Pressure]] 

#is_/same_as :: [[../../../WikiData/WD~Vapor_pressure,2661322|WD~Vapor_pressure,2661322]]  

## #has_/text_of_/abstract 

> Vapor pressure or equilibrium vapor pressure is the pressure exerted by a vapor in thermodynamic equilibrium 
> with its condensed phases (solid or liquid) at a given temperature in a closed system. 
> 
> The equilibrium vapor pressure is an indication of a liquid's thermodynamic tendency to evaporate. 
> It relates to the balance of particles escaping from the liquid (or solid) 
> in equilibrium with those in a coexisting vapor phase. 
> 
> A substance with a high vapor pressure at normal temperatures is often referred to as volatile. 
> The pressure exhibited by vapor present above a liquid surface is known as vapor pressure. 
> As the temperature of a liquid increases, the attractive interactions between liquid molecules become less significant in comparison to the entropy of those molecules in the gas phase, increasing the vapor pressure. Thus, liquids with strong intermolecular interactions are likely to have smaller vapor pressures, with the reverse true for weaker interactions.
>
> The vapor pressure of any substance increases non-linearly with temperature, often described by the Clausius–Clapeyron relation. The atmospheric pressure boiling point of a liquid (also known as the normal boiling point) is the temperature at which the vapor pressure equals the ambient atmospheric pressure. With any incremental increase in that temperature, the vapor pressure becomes sufficient to overcome atmospheric pressure and cause the liquid to form vapor bubbles. Bubble formation in greater depths of liquid requires a slightly higher temperature due to the higher fluid pressure, due to hydrostatic pressure of the fluid mass above. More important at shallow depths is the higher temperature required to start bubble formation. The surface tension of the bubble wall leads to an overpressure in the very small initial bubbles.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Vapor%20pressure) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Thermodynamics/Vapor_Pressure|Vapor_Pressure]] 

### #is_/same_as :: [[/_public/Science/Physics/Thermodynamics/Vapor_Pressure.public|Vapor_Pressure.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Thermodynamics/Vapor_Pressure.internal|Vapor_Pressure.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Thermodynamics/Vapor_Pressure.protect|Vapor_Pressure.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Thermodynamics/Vapor_Pressure.private|Vapor_Pressure.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Thermodynamics/Vapor_Pressure.personal|Vapor_Pressure.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Thermodynamics/Vapor_Pressure.secret|Vapor_Pressure.secret]] 

