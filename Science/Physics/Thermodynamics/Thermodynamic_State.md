---
aliases:
- Egoera termodinamiko
- estado termodinámico
- estado termodinâmico
- estat termodinàmic
- Estáu termodinámicu
- Jia̍t-le̍k-ha̍k chōng-thài
- keadaan termodinamik
- stan termodynamiczny
- stare termodinamică
- stato termodinamico
- Stav systému
- Termodinamiese toestand
- termodinamik durum
- termodinamika stato
- termodinamikai állapot
- Termodinamičko stanje
- termodinamsko stanje
- Termodynaaminen tila
- termodynamický stav
- termodynamisk tilstand
- Termodynamiskt tillstånd
- thermodynamic state
- thermodynamischer Zustand
- trạng thái nhiệt động
- état thermodynamique
- стан термодинамічної системи
- термодинамик халәт
- термодинамическое состояние
- термодинамичка состојба
- тэрмадынамічны стан
- מצב תרמודינמי
- حالة ثرموديناميكية
- حالت ترمودینامیکی
- کأداءن ترموديناميک
- ऊष्मागतिक अवस्था
- 热力学状态
- 熱力学状態
- 熱力學狀態
- 열역학적 상태
has_id_wikidata: Q230875
subclass_of: '[[_Standards/WikiData/WD~physical_state,230855]]'
has_part_s_: '[[_Standards/WikiData/WD~state_function,230937]]'
related_property: phase of matter
studied_by: '[[_Standards/WikiData/WD~thermodynamics,11473]]'
has_characteristic: '[[_Standards/WikiData/WD~thermodynamic_equilibrium,51363]]'
dv_has_:
  name_:
    af: Termodinamiese toestand
    ar: حالة ثرموديناميكية
    ast: Estáu termodinámicu
    be_tarask: тэрмадынамічны стан
    ca: estat termodinàmic
    cs: Stav systému
    de: thermodynamischer Zustand
    en: thermodynamic state
    eo: termodinamika stato
    es: estado termodinámico
    eu: Egoera termodinamiko
    fa: حالت ترمودینامیکی
    fi: Termodynaaminen tila
    fr: état thermodynamique
    gl: estado termodinámico
    he: מצב תרמודינמי
    hi: ऊष्मागतिक अवस्था
    hu: termodinamikai állapot
    it: stato termodinamico
    ja: 熱力学状態
    ko: 열역학적 상태
    mag: ऊष्मागतिक अवस्था
    mk: термодинамичка состојба
    ms: keadaan termodinamik
    ms_arab: کأداءن ترموديناميک
    nan: Jia̍t-le̍k-ha̍k chōng-thài
    nb: termodynamisk tilstand
    nn: termodynamisk tilstand
    pl: stan termodynamiczny
    pt: estado termodinâmico
    ro: stare termodinamică
    ru: термодинамическое состояние
    sh: Termodinamičko stanje
    sk: termodynamický stav
    sl: termodinamsko stanje
    sr: Termodinamičko stanje
    sv: Termodynamiskt tillstånd
    tr: termodinamik durum
    tt: термодинамик халәт
    uk: стан термодинамічної системи
    vi: trạng thái nhiệt động
    yue: 熱力學狀態
    zh: 熱力學狀態
    zh_cn: 热力学状态
    zh_hans: 热力学状态
    zh_hant: 熱力學狀態
    zh_hk: 熱力學狀態
    zh_mo: 熱力學狀態
    zh-my: 热力学状态
    zh_sg: 热力学状态
    zh_tw: 熱力學狀態
---
# [[Thermodynamic_State]] 

#is_/same_as :: [[../../../WikiData/WD~Thermodynamic_state,230875|WD~Thermodynamic_state,230875]]  

## #has_/text_of_/abstract 

> In thermodynamics, a thermodynamic state of a system is its condition at a specific time; 
> that is, fully identified by values of a suitable set of parameters known as state variables, 
> state parameters or thermodynamic variables. 
> 
> Once such a set of values of thermodynamic variables has been specified for a system, 
> the values of all thermodynamic properties of the system are uniquely determined. 
> 
> Usually, by default, a thermodynamic state is taken to be one of thermodynamic equilibrium. 
> This means that the state is not merely the condition of the system at a specific time, 
> but that the condition is the same, unchanging, over an indefinitely long duration of time.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Thermodynamic%20state) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Thermodynamics/Thermodynamic_State|Thermodynamic_State]] 

### #is_/same_as :: [[/_public/Science/Physics/Thermodynamics/Thermodynamic_State.public|Thermodynamic_State.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Thermodynamics/Thermodynamic_State.internal|Thermodynamic_State.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Thermodynamics/Thermodynamic_State.protect|Thermodynamic_State.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Thermodynamics/Thermodynamic_State.private|Thermodynamic_State.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Thermodynamics/Thermodynamic_State.personal|Thermodynamic_State.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Thermodynamics/Thermodynamic_State.secret|Thermodynamic_State.secret]] 

