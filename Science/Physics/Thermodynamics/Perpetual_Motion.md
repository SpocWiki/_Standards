---
aliases:
- Abadiy dvigatel
- amžinasis variklis
- betiereko mugimendu
- Daimi mühərrik
- devridaim
- eterna movo
- evig rörelse
- Ewigdurende beweging
- gerak abadi
- Ikiliikkuja
- Moto contínuo
- moto perpetuo
- mouvement perpétuel
- Movemento perpetuo
- moviment perpetu
- móvil perpetuo
- Móvil perpetuu
- mōtus perpetuus
- Mūžīgais dzinējs
- Perpetual motion
- Perpetual_Motion
- Perpetuum mobile
- stalno gibanje
- Walang hanggang mosyon
- örökmozgó
- Động cơ vĩnh cửu
- Αεικίνητο
- вічний двигун
- Вечен двигател
- вечны рухавік
- вечный двигатель
- Муҳаррики абадӣ
- Мәңгілік қозғалтқыш
- Перпетуум мобиле
- Түбөлүк кыймылдаткыч
- чик дугдылытэк
- Հավերժական շարժիչ
- תנועה נצחית
- تلپاتې يا پرله پسې حرکت
- حركة أبدية
- حرکت دائمی
- دائمی حرکت
- निरन्तर गति
- இடையறா இயக்கம்
- ಸಾರ್ವಕಾಲಿಕ ಚಲನಾ ಯಂತ್ರ
- നിലയ്ക്കാത്ത ചലനങ്ങൾ
- เครื่องจักรนิรันดร์
- მუდმივი ძრავა
- 永久機関
- 永动机
- 永動機
- 영구 기관
has_id_wikidata: Q12623
subclass_of:
- '[[_Standards/WikiData/WD~motion,79782|WD~motion,79782]]'
- '[[_Standards/WikiData/WD~hypothetical_technology,17028492|WD~hypothetical_technology,17028492]]'
different_from:
- '[[_Standards/WikiData/WD~Perpetuum_Mobile,512946|WD~Perpetuum_Mobile,512946]]'
- '[[_Standards/WikiData/WD~Perpetuum_Mobile,110905247|WD~Perpetuum_Mobile,110905247]]'
- '[[_Standards/WikiData/WD~perpetual_motion_machine,113129622|WD~perpetual_motion_machine,113129622]]'
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- "[[_Standards/WikiData/WD~Otto's_encyclopedia,2041543|WD~Otto's_encyclopedia,2041543]]"
- '[[_Standards/WikiData/WD~Granat_Encyclopedic_Dictionary,4532138|WD~Granat_Encyclopedic_Dictionary,4532138]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633|WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633]]'
video: http://commons.wikimedia.org/wiki/Special:FilePath/%D0%92%D0%B5%D1%87%D0%BD%D1%8B%D0%B9%20%D0%B4%D0%B2%D0%B8%D0%B3%D0%B0%D1%82%D0%B5%D0%BB%D1%8C.webm
image: http://commons.wikimedia.org/wiki/Special:FilePath/Deutsches%20Museum%206.jpg
dv_has_:
  name_:
    af: Ewigdurende beweging
    ar: حركة أبدية
    ast: Móvil perpetuu
    az: Daimi mühərrik
    be_tarask: вечны рухавік
    bg: Вечен двигател
    ca: moviment perpetu
    cv: Перпетуум мобиле
    de: Perpetuum mobile
    el: Αεικίνητο
    en: perpetual motion
    eo: eterna movo
    es: móvil perpetuo
    eu: betiereko mugimendu
    fa: حرکت دائمی
    fi: Ikiliikkuja
    fr: mouvement perpétuel
    gl: Movemento perpetuo
    he: תנועה נצחית
    hi: निरन्तर गति
    hu: örökmozgó
    hy: Հավերժական շարժիչ
    id: gerak abadi
    it: moto perpetuo
    ja: 永久機関
    ka: მუდმივი ძრავა
    kk: Мәңгілік қозғалтқыш
    kn: ಸಾರ್ವಕಾಲಿಕ ಚಲನಾ ಯಂತ್ರ
    ko: 영구 기관
    ky: Түбөлүк кыймылдаткыч
    la: mōtus perpetuus
    lt: amžinasis variklis
    lv: Mūžīgais dzinējs
    lzh: 永動機
    ml: നിലയ്ക്കാത്ത ചലനങ്ങൾ
    nb: evig rörelse
    nl: Perpetuum mobile
    pnb: دائمی حرکت
    ps: تلپاتې يا پرله پسې حرکت
    pt: Moto contínuo
    ru: вечный двигатель
    sl: stalno gibanje
    sr: Перпетуум мобиле
    sv: evig rörelse
    ta: இடையறா இயக்கம்
    tg: Муҳаррики абадӣ
    th: เครื่องจักรนิรันดร์
    tl: Walang hanggang mosyon
    tr: devridaim
    udm: чик дугдылытэк
    uk: вічний двигун
    ur: دائمی حرکت
    uz: Abadiy dvigatel
    vi: Động cơ vĩnh cửu
    wuu: 永动机
    yue: 永動機
    zh: 永动机
---

# [[Perpetual_Motion]] 

#is_/same_as :: [[_Standards/WikiData/WD~Perpetual_motion,12623|WD~Perpetual_motion,12623]] 

## #has_/text_of_/abstract 

> Perpetual motion is the motion of bodies that continues forever in an unperturbed system. 
> A perpetual motion machine is a hypothetical machine 
> that can do work indefinitely without an external energy source. 
> 
> This kind of machine is impossible, 
> since its existence would violate the first and/or second laws of thermodynamics. 
> These laws of thermodynamics apply regardless of the size of the system. 
> 
> Thus, machines that extract energy from finite sources cannot operate indefinitely 
> because they are driven by the energy stored in the source, which will eventually be exhausted. 
> 
> A common example is devices powered by ocean currents, 
> whose energy is ultimately derived from the Sun, which itself will eventually burn out.
>
> In 2016, new states of matter, time crystals, were discovered in which, on a microscopic scale, 
> the component atoms are in continual repetitive motion, 
> thus satisfying the literal definition of "perpetual motion". 
> 
> However, these do not constitute perpetual motion machines in the traditional sense, 
> or violate thermodynamic laws, because they are in their quantum ground state, 
> so no energy can be extracted from them; they exhibit motion without energy.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Perpetual%20motion) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Thermodynamics/Perpetual_Motion|Perpetual_Motion]] 

### #is_/same_as :: [[/_public/Science/Physics/Thermodynamics/Perpetual_Motion.public|Perpetual_Motion.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Thermodynamics/Perpetual_Motion.internal|Perpetual_Motion.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Thermodynamics/Perpetual_Motion.protect|Perpetual_Motion.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Thermodynamics/Perpetual_Motion.private|Perpetual_Motion.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Thermodynamics/Perpetual_Motion.personal|Perpetual_Motion.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Thermodynamics/Perpetual_Motion.secret|Perpetual_Motion.secret]] 

