---
aliases:
- algemene gaswet
- Batas ng ideyal na gas
- dlí an gháis idéalaigh
- egyesített gáztörvény
- ekvacio de ideala gaso
- Equazion de stat di gas perfètt
- equazione di stato dei gas perfetti
- Gas idealen legeak
- Hukum gas unggul
- ideaalikaasulaki
- Ideaalse gaasi olekuvõrrand
- Ideal Gas Law
- ideal gas law, also called the general gas equation
- Ideal gaz holati tenglamasi
- Ideala gaslagen
- Ideale gaswet
- Idealgasligning
- Idealgassloven
- idealiųjų dujų būsenos lygtis
- Ideal_Gas_Law
- Ideālas gāzes likums
- Jednadžba stanja idealnog plina
- Jednačina stanja idealnog gasa
- Laj general dij gas
- Legea gazelor ideale
- Lei dos gases ideais
- ley de los gases ideales
- Ligji i gazit ideal
- Llei de los gases ideales
- llei dels gasos ideals
- loi des gaz parfaits
- Lí-sióng khì-thé chōng-thài hong-thêng-sek
- Persamaan gas ideal
- Phương trình trạng thái khí lý tưởng
- Równanie Clapeyrona
- splošna plinska enačba
- Stavová rovnica ideálneho plynu
- stavová rovnice ideálního plynu
- thermische Zustandsgleichung idealer Gase
- Zakon idealnog plina
- İdeal gaz yasası
- İdeal qazın hal tənliyi
- Καταστατική εξίσωση των αερίων
- Једначина стања идеалног гаса
- Гей-Люссак мыйзамдары
- Идеаллă газăн тăрăм танлăхĕ
- идеальной газлэн инэт уравнениез
- Клапејрон-Менделеева равенка
- Клапейрон теңдеуі
- рівняння стану ідеального газу
- раўнаньне стану ідэальнага газу
- ураўненне стану ідэальнага газу
- Уравнение на Клапейрон – Менделеев
- Уравнение состояния идеального газа
- Կլապեյրոնի հավասարում
- משוואת הגז האידאלי
- قانون الغازات المثالية
- قانون گاز آرمانی
- یاسای گازی نمونەیی
- आदर्श गैस समीकरण
- आदर्श वायू समीकरण
- আদর্শ গ্যাস সূত্র
- আদৰ্শ গেছ সূত্ৰ
- கருத்தியல் வளிம விதி
- ആദർശ വാതക നിയമം
- සර්වත්ර වායු නියමය
- กฎของแก๊สอุดมคติ
- 理想气体状态方程
- 理想気体の状態方程式
- 理想氣體狀態方程
- 理想氣體狀態方程式
- 이상기체 법칙
has_id_wikidata: Q191785
studied_by:
- '[[_Standards/WikiData/WD~statistical_mechanics,188715|WD~statistical_mechanics,188715]]'
- '[[_Standards/WikiData/WD~thermodynamics,11473|WD~thermodynamics,11473]]'
instance_of:
- '[[_Standards/WikiData/WD~equation_of_state,214967|WD~equation_of_state,214967]]'
- '[[_Standards/WikiData/WD~gas_law,379231|WD~gas_law,379231]]'
named_after:
- '[[_Standards/WikiData/WD~Benoît_Paul_Émile_Clapeyron,313551|WD~Benoît_Paul_Émile_Clapeyron,313551]]'
- '[[_Standards/WikiData/WD~Dmitri_Mendeleev,9106|WD~Dmitri_Mendeleev,9106]]'
maintained_by_WikiProject: '[[_Standards/WikiData/WD~WikiProject_Mathematics,8487137|WD~WikiProject_Mathematics,8487137]]'
described_by_source:
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632|WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632]]'
in_defining_formula:
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>p</mi></mstyle></mrow></math>
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>T</mi></mstyle></mrow></math>
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>n</mi></mstyle></mrow></math>
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>R</mi></mstyle></mrow></math>
- <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>V</mi></mstyle></mrow></math>
Commons_category: Ideal gas law
defining_formula: <math class="mwe-math-element mwe-math-element-inline" xmlns="http://www.w3.org/1998/Math/MathML"><mrow data-mjx-texclass="ORD"><mstyle displaystyle="true" scriptlevel="0"><mi>p</mi><mi>V</mi><mo stretchy="false">=</mo><mi>n</mi><mi>R</mi><mi>T</mi></mstyle></mrow></math>
dv_has_:
  name_:
    af: Ideale gaswet
    ar: قانون الغازات المثالية
    as: আদৰ্শ গেছ সূত্ৰ
    ast: Llei de los gases ideales
    az: İdeal qazın hal tənliyi
    be: ураўненне стану ідэальнага газу
    be_tarask: раўнаньне стану ідэальнага газу
    bg: Уравнение на Клапейрон – Менделеев
    bn: আদর্শ গ্যাস সূত্র
    bs: Zakon idealnog plina
    ca: llei dels gasos ideals
    ckb: یاسای گازی نمونەیی
    cs: stavová rovnice ideálního plynu
    cv: Идеаллă газăн тăрăм танлăхĕ
    da: Idealgasligning
    de: thermische Zustandsgleichung idealer Gase
    el: Καταστατική εξίσωση των αερίων
    en: ideal gas law
    eo: ekvacio de ideala gaso
    es: ley de los gases ideales
    et: Ideaalse gaasi olekuvõrrand
    eu: Gas idealen legeak
    fa: قانون گاز آرمانی
    fi: ideaalikaasulaki
    fr: loi des gaz parfaits
    ga: dlí an gháis idéalaigh
    gl: Lei dos gases ideais
    he: משוואת הגז האידאלי
    hi: आदर्श गैस समीकरण
    hr: Jednadžba stanja idealnog plina
    hu: egyesített gáztörvény
    hy: Կլապեյրոնի հավասարում
    id: Persamaan gas ideal
    it: equazione di stato dei gas perfetti
    ja: 理想気体の状態方程式
    kk: Клапейрон теңдеуі
    ko: 이상기체 법칙
    ky: Гей-Люссак мыйзамдары
    lmo: Equazion de stat di gas perfètt
    lt: idealiųjų dujų būsenos lygtis
    lv: Ideālas gāzes likums
    mk: Клапејрон-Менделеева равенка
    ml: ആദർശ വാതക നിയമം
    mr: आदर्श वायू समीकरण
    ms: Hukum gas unggul
    nan: Lí-sióng khì-thé chōng-thài hong-thêng-sek
    nb: Idealgassloven
    nl: algemene gaswet
    pl: Równanie Clapeyrona
    pms: Laj general dij gas
    pt: lei dos gases ideais
    ro: Legea gazelor ideale
    ru: Уравнение состояния идеального газа
    sco: ideal gas law
    sh: Jednačina stanja idealnog gasa
    si: සර්වත්ර වායු නියමය
    sk: Stavová rovnica ideálneho plynu
    sl: splošna plinska enačba
    sq: Ligji i gazit ideal
    sr: Једначина стања идеалног гаса
    sr_ec: Једначина стања идеалног гаса
    sr_el: Jednačina stanja idealnog gasa
    sv: Ideala gaslagen
    ta: கருத்தியல் வளிம விதி
    th: กฎของแก๊สอุดมคติ
    tl: Batas ng ideyal na gas
    tr: İdeal gaz yasası
    udm: идеальной газлэн инэт уравнениез
    uk: рівняння стану ідеального газу
    uz: Ideal gaz holati tenglamasi
    vi: Phương trình trạng thái khí lý tưởng
    wuu: 理想气体状态方程
    yue: 理想氣體狀態方程式
    zh: 理想气体状态方程
    zh_cn: 理想气体状态方程
    zh_hans: 理想气体状态方程
    zh_hant: 理想氣體狀態方程
    zh_hk: 理想氣體狀態方程式
    zh_sg: 理想气体状态方程
    zh_tw: 理想氣體狀態方程式
---

# [[Ideal_Gas_Law]] 

#is_/same_as :: [[_Standards/WikiData/WD~Ideal_gas_law,191785|WD~Ideal_gas_law,191785]] 

## #has_/text_of_/abstract 

> The ideal gas law, also called the general gas equation, 
> is the equation of state of a hypothetical ideal gas. 
> 
> It is a good approximation of the behavior of many gases under many conditions, 
> although it has several limitations with high Densities(Pressures) or low Temperatures or Volumes. 
> 
> It was first stated by Benoît Paul Émile Clapeyron in 1834 as a combination of the empirical 
> - Boyle's law, Charles's law, Avogadro's law, and Gay-Lussac's law. 
> 
> The ideal gas law is often written in an empirical form: pV=nRT with
> - p the pressure, 
> - V volume 
> - T temperature 
> - n amount (i.e. number) of substance n; and
> - R the ideal gas constant.
>
> It can also be derived from the microscopic kinetic theory, 
> as was achieved (independently) by August Krönig in 1856 and Rudolf Clausius in 1857.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Ideal%20gas%20law) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Thermodynamics/Ideal_Gas/Ideal_Gas_Law|Ideal_Gas_Law]] 

### #is_/same_as :: [[/_public/Science/Physics/Thermodynamics/Ideal_Gas/Ideal_Gas_Law.public|Ideal_Gas_Law.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Thermodynamics/Ideal_Gas/Ideal_Gas_Law.internal|Ideal_Gas_Law.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Thermodynamics/Ideal_Gas/Ideal_Gas_Law.protect|Ideal_Gas_Law.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Thermodynamics/Ideal_Gas/Ideal_Gas_Law.private|Ideal_Gas_Law.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Thermodynamics/Ideal_Gas/Ideal_Gas_Law.personal|Ideal_Gas_Law.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Thermodynamics/Ideal_Gas/Ideal_Gas_Law.secret|Ideal_Gas_Law.secret]] 

