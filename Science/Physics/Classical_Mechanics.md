---
aliases:
- Classical mechanics
- Classical_Mechanics
- cơ học cổ điển
- klaasslaž mekanikk
- klasická mechanika
- klasik mekanik
- klasika mekaniko
- klasika meĥaniko
- klasikal mikianix
- klasikinė mechanika
- klasikong mekaniks
- klasiskā mehānika
- klasična mehanika
- klassieke mechanica
- klassieke meganika
- klassihkalaš mekanihkka
- klassik mehanika
- klassik mexanika
- klassikaline mehaanika
- klassiklâš mekaniik
- Klassine mehanik
- klassinen mekaniikka
- Klassische Mechanik
- klassisk mekanik
- klassisk mekanikk
- klasszikus mechanika
- kó͘-tián le̍k-ha̍k
- mecaneg glasurol
- mecanega classega
- mecanica clasica
- mecanica classic
- mecanica classica
- mecanică clasică
- meccanica classica
- mechanica classic
- mechanica classica
- mechanika klasyczna
- mecànica clàssica
- mecánica clásica
- mecânica clássica
- meicnic Newton
- mekanik klasik
- mekanika klasik
- mekanika klasika
- mekanika klasike
- mekanika klasiko
- mekkanika klassika
- mécanique classique
- mékanik newtonyenn
- sígild aflfræði
- umakanika kawaida
- κλασική μηχανική
- класічна механіка
- класічная механіка
- класическа механика
- класична механіка
- класична механика
- классик механика
- классикăлла механика
- классикалық механика
- классикан механика
- Классиктиг механика
- классическай механика
- классическая механика
- клясычная мэханіка
- механикаи классикӣ
- сонгодог механик
- Һунгадаг механика
- Դասական մեխանիկա
- מכניקה קלאסית
- קלאסישע מעכאניק
- الميكانيكا الكلاسيكيه
- روائتی آلاتیات
- ميكانيكا تقليدية
- مکانیک کلاسیک
- میکانیکی کلاسیک
- کلاسیک مکانیک
- کلاسیکل مکینکس
- ڪلاسيڪل ميڪانيات
- अभिजात यामिकी
- क्लासिकल मैकेनिक्स
- चिरसम्मत बलविज्ञान
- चिरसम्मत यांत्रिकी
- शास्त्रीय यान्त्रिकी
- চিরায়ত বলবিজ্ঞান
- ধ্ৰুপদী বলবিজ্ঞান
- ਟਕਸਾਲੀ ਮਕੈਨਕੀ
- மரபார்ந்த விசையியல்
- సాంప్రదాయ యాంత్రికశాస్త్రం
- ಗತಿವಿಜ್ಞಾನ
- ഉദാത്തബലതന്ത്രം
- සම්භාව් යය යාන්ත් ර විද් යාව
- กลศาสตร์ดั้งเดิม
- ဂန္ထဝင် မက္ကင်းနစ်
- კლასიკური მექანიკა
- 古典力学
- 經典力學
- 经典力学
- 고전력학
- 고전역학
has_id_wikidata: Q11397
Commons_category: Classical mechanics
different_from:
- '[[_Standards/WikiData/WD~analytical_mechanics,833065|WD~analytical_mechanics,833065]]'
history_of_topic:
- '[[_Standards/WikiData/WD~timeline_of_classical_mechanics,2967073|WD~timeline_of_classical_mechanics,2967073]]'
- '[[_Standards/WikiData/WD~history_of_classical_mechanics,5867520|WD~history_of_classical_mechanics,5867520]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Newtons%20cradle%20animation%20book.gif
named_after: '[[_Standards/WikiData/WD~Isaac_Newton,935|WD~Isaac_Newton,935]]'
PhilPapers_topic: classical-mechanics
Stack_Exchange_tag:
- https://physics.stackexchange.com/tags/classical-mechanics
- https://mathoverflow.net/tags/classical-mechanics
- https://math.stackexchange.com/tags/classical-mechanics
subclass_of:
- '[[_Standards/WikiData/WD~mechanics,41217|WD~mechanics,41217]]'
topic_s_main_template:
- '[[_Standards/WikiData/WD~Template_Classical_mechanics,6234453|WD~Template_Classical_mechanics,6234453]]'
dv_has_:
  name_:
    af: klassieke meganika
    an: mecanica clasica
    ar: ميكانيكا تقليدية
    arz: الميكانيكا الكلاسيكيه
    as: ধ্ৰুপদী বলবিজ্ঞান
    ast: mecánica clásica
    az: klassik mexanika
    azb: کلاسیک مکانیک
    ba: классик механика
    be: класічная механіка
    be_tarask: клясычная мэханіка
    bg: класическа механика
    bho: क्लासिकल मैकेनिक्स
    bn: চিরায়ত বলবিজ্ঞান
    bs: klasična mehanika
    bxr: Һунгадаг механика
    ca: mecànica clàssica
    ce: классикан механика
    ckb: میکانیکی کلاسیک
    crh: klassik mehanika
    cs: klasická mechanika
    cv: классикăлла механика
    cy: mecaneg glasurol
    da: klassisk mekanik
    de: klassische Mechanik
    el: κλασική μηχανική
    en: classical mechanics
    eo: klasika meĥaniko
    es: mecánica clásica
    et: klassikaline mehaanika
    eu: mekanika klasiko
    fa: مکانیک کلاسیک
    fi: klassinen mekaniikka
    fr: mécanique classique
    ga: meicnic Newton
    gcr: mékanik newtonyenn
    gl: mecánica clásica
    gsw: klassische Mechanik
    he: מכניקה קלאסית
    hi: चिरसम्मत यांत्रिकी
    hif: classical mechanics
    hr: klasična mehanika
    hu: klasszikus mechanika
    hy: Դասական մեխանիկա
    ia: mechanica classic
    id: mekanika klasik
    ie: mecanica classic
    io: klasika mekaniko
    is: sígild aflfræði
    it: meccanica classica
    ja: 古典力学
    jam: klasikal mikianix
    ka: კლასიკური მექანიკა
    kaa: klassik mexanika
    kk: классикалық механика
    kn: ಗತಿವಿಜ್ಞಾನ
    ko: 고전역학
    ko-kp: 고전력학
    la: mechanica classica
    lfn: mecanica clasica
    lmo: mecanega classega
    lt: klasikinė mechanika
    lv: klasiskā mehānika
    lzh: 經典力學
    mag: चिरसम्मत बलविज्ञान
    map_bms: mekanika klasik
    mk: класична механика
    ml: ഉദാത്തബലതന്ത്രം
    mn: сонгодог механик
    mr: अभिजात यामिकी
    ms: mekanik klasik
    mt: mekkanika klassika
    my: ဂန္ထဝင် မက္ကင်းနစ်
    nan: kó͘-tián le̍k-ha̍k
    nb: klassisk mekanikk
    nds: Klassische Mechanik
    new: शास्त्रीय यान्त्रिकी
    nl: klassieke mechanica
    nn: klassisk mekanikk
    oc: mecanica classica
    pa: ਟਕਸਾਲੀ ਮਕੈਨਕੀ
    pl: mechanika klasyczna
    pms: Mecànica clàssica
    pnb: کلاسیکل مکینکس
    pt: mecânica clássica
    pt_br: mecânica clássica
    ro: mecanică clasică
    ru: классическая механика
    rue: класічна механіка
    sah: классическай механика
    sco: classical mechanics
    sd: ڪلاسيڪل ميڪانيات
    se: klassihkalaš mekanihkka
    sh: klasična mehanika
    si: සම්භාව් යය යාන්ත් ර විද් යාව
    sk: klasická mechanika
    sl: klasična mehanika
    smn: klassiklâš mekaniik
    sms: klaasslaž mekanikk
    sq: mekanika klasike
    sr: класична механика
    sv: klassisk mekanik
    sw: umakanika kawaida
    ta: மரபார்ந்த விசையியல்
    te: సాంప్రదాయ యాంత్రికశాస్త్రం
    tg: механикаи классикӣ
    th: กลศาสตร์ดั้งเดิม
    tl: klasikong mekaniks
    tr: klasik mekanik
    tt: классик механика
    tyv: Классиктиг механика
    uk: класична механіка
    ur: روائتی آلاتیات
    uz: klassik mexanika
    vep: Klassine mehanik
    vi: cơ học cổ điển
    war: mekanika klasika
    wuu: 经典力学
    xmf: კლასიკური მექანიკა
    yi: קלאסישע מעכאניק
    yue: 經典力學
    zh: 经典力学
    zh_hans: 经典力学
    zh_hant: 經典力學
---

# [[Classical_Mechanics]] 

#is_/same_as :: [[../../WikiData/WD~Classical_mechanics,11397|WD~Classical_mechanics,11397]] 
#is_/same_as :: [[../../WikiData/WD~Mechanics,41217|WD~Mechanics,41217]] 

## #has_/text_of_/abstract 

> **Classical mechanics** is a physical theory describing the motion of objects such as projectiles, parts of machinery, spacecraft, planets, stars, and galaxies. The development of classical mechanics involved substantial change in the methods and philosophy of physics. The qualifier classical distinguishes this type of mechanics from physics developed after the revolutions in physics of the early 20th century, all of which revealed limitations in classical mechanics.
>
> The earliest formulation of classical mechanics is often referred to as Newtonian mechanics. It consists of the physical concepts based on the 17th century foundational works of Sir Isaac Newton, and the mathematical methods invented by Newton, Gottfried Wilhelm Leibniz, Leonhard Euler and others to describe the motion of bodies under the influence of forces. Later, methods based on energy were developed by Euler, Joseph-Louis Lagrange, William Rowan Hamilton and others, leading to the development of analytical mechanics (which includes Lagrangian mechanics and Hamiltonian mechanics). These advances, made predominantly in the 18th and 19th centuries, extended beyond earlier works; they are, with some modification, used in all areas of modern physics.
>
> If the present state of an object that obeys the laws of classical mechanics is known, it is possible to determine how it will move in the future, and how it has moved in the past. Chaos theory shows that the long term predictions of classical mechanics are not reliable. Classical mechanics provides accurate results when studying objects that are not extremely massive and have speeds not approaching the speed of light. With objects  about the size of an atom's diameter, it becomes necessary to use quantum mechanics. To describe velocities approaching the speed of light, special relativity is needed. In cases where objects become extremely massive, general relativity becomes applicable. Some modern sources include relativistic mechanics in classical physics, as representing the field in its most developed and accurate form.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Classical%20mechanics) 


## #has_/text_of_/abstract 

> **Mechanics** (from Ancient Greek  μηχανική (mēkhanikḗ) 'of machines') is the area of physics 
> concerned with the relationships between force, matter, and motion among physical objects. 
> Forces applied to objects may result in displacements, 
> which are changes of an object's position relative to its environment.
>
> Theoretical expositions of this branch of physics has its origins in Ancient Greece, for instance, 
> in the writings of Aristotle and Archimedes (see History of classical mechanics and Timeline of classical mechanics). 
> 
> During the early modern period, scientists such as Galileo Galilei, Johannes Kepler, 
> Christiaan Huygens, and Isaac Newton laid the foundation for what is now known as classical mechanics.
>
> In the 20th century the concepts of classical mechanics were challenged by new discoveries, 
> leading to fundamentally new approaches including relativistic mechanics and quantum mechanics.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Mechanics) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Classical_Mechanics|Classical_Mechanics]] 

### #is_/same_as :: [[/_public/Science/Physics/Classical_Mechanics.public|Classical_Mechanics.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Classical_Mechanics.internal|Classical_Mechanics.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Classical_Mechanics.protect|Classical_Mechanics.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Classical_Mechanics.private|Classical_Mechanics.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Classical_Mechanics.personal|Classical_Mechanics.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Classical_Mechanics.secret|Classical_Mechanics.secret]] 

