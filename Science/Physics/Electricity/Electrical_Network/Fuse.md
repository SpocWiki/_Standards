---
aliases:
- baujyenjswh
- bezpiecznik elektryczny
- cầu chì
- elektrická pojistka
- Elektrisk sikring
- Električni osigurač
- fandogardilo
- Fius elektrik
- Fizik
- fiús
- fuse
- Fusibel
- fusibile
- fusible
- fusível
- Fyuzi
- Hiù-juh
- olvadóbiztosító
- Osigurač
- Poistka
- Saqtandırğış
- saugiklis
- Sekring
- Sicherung
- siekrik
- sigorta
- Siguranță fuzibilă
- sihkkarasti
- sikring
- sudetteijee
- sulake
- sulavkaitse
- säkring
- Sékring
- varovalka
- zekering
- Ηλεκτρική ασφάλεια
- Електрически предпазител
- електрични осигурач
- забясьпечнік
- запобіжник
- Осигурувач
- Сақтандырғыш
- электрический предохранитель
- Электро сыхчă
- электрычны засцерагальнік
- էլեկտրական ապահովիչ
- מבטח
- ساقتاندىرعىش
- فاصمة منصهرة
- فیثا غورث
- فیوز
- फ्युज
- फ्यूज
- ਫਿਊਜ਼
- உருகுக்கம்பி
- ಎಲೆಕ್ಟ್ರಿಕ್ ಫ್ಯೂಸ್
- വൈദ്യുതഫ്യൂസ്
- පිලිසිය (විදිලි)
- ฟิวส์
- 保险丝
- 保險絲
- 菲士
- 電力ヒューズ
- 퓨즈
has_id_wikidata: Q182221
instance_of:
- '[[_Standards/WikiData/WD~electrical_element,210729]]'
- '[[_Standards/WikiData/WD~passive_electronic_component,9252564]]'
different_from:
- '[[_Standards/WikiData/WD~circuit_breaker,211058]]'
- '[[_Standards/WikiData/WD~Fusible,235778]]'
- '[[_Standards/WikiData/WD~fuse,664571]]'
opposite_of: '[[_Standards/WikiData/WD~Antifuse,365870]]'
described_by_source:
- '[[../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._1,123560817]]'
has_characteristic:
- '[[_Standards/WikiData/WD~breaking_capacity,779913]]'
- '[[_Standards/WikiData/WD~rated_voltage,816369]]'
subclass_of:
- '[[_Standards/WikiData/WD~sacrificial_part,7397371]]'
- '[[_Standards/WikiData/WD~power_resistor,64706764]]'
said_to_be_the_same_as:
- '[[_Standards/WikiData/WD~fuse_element,12375422]]'
- '[[_Standards/WikiData/WD~Q16137204,16137204]]'
has_use: '[[_Standards/WikiData/WD~overcurrent_protection,15856134]]'
OmegaWiki_Defined_Meaning: 1589486
image:
- http://commons.wikimedia.org/wiki/Special:FilePath/Smeltveiligheid.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Electrical%20Fuse%20%28aka%29.jpg
- http://commons.wikimedia.org/wiki/Special:FilePath/Hochspannungssicherung.jpg
Iconclass_notation: 41B115
UNSPSC_Code: 39121623
MCN_code: 8536.10.00
Commons_category: Electrical fuses
dv_has_:
  name_:
    ar: فاصمة منصهرة
    ast: Fusible
    be: электрычны засцерагальнік
    be_tarask: забясьпечнік
    bg: Електрически предпазител
    bs: Električni osigurač
    ca: fusible
    cs: elektrická pojistka
    cv: Электро сыхчă
    da: Elektrisk sikring
    de: Sicherung
    el: Ηλεκτρική ασφάλεια
    en: fuse
    eo: fandogardilo
    es: fusible
    et: sulavkaitse
    eu: fusible
    fa: فیوز
    fi: sulake
    fr: fusible
    ga: fiús
    gl: Fusible
    he: מבטח
    hi: फ्यूज
    hr: Osigurač
    ht: Fizik
    hu: olvadóbiztosító
    hy: էլեկտրական ապահովիչ
    id: Sekring
    it: fusibile
    ja: 電力ヒューズ
    jv: Sékring
    kk: Сақтандырғыш
    kk_arab: ساقتاندىرعىش
    kk-cn: ساقتاندىرعىش
    kk_cyrl: Сақтандырғыш
    kk-kz: Сақтандырғыш
    kk_latn: Saqtandırğış
    kk-tr: Saqtandırğış
    kn: ಎಲೆಕ್ಟ್ರಿಕ್ ಫ್ಯೂಸ್
    ko: 퓨즈
    lmo: Fusibel
    lt: saugiklis
    mk: Осигурувач
    ml: വൈദ്യുതഫ്യൂസ്
    ms: Fius elektrik
    nan: Hiù-juh
    nb: sikring
    ne: फ्युज
    nl: zekering
    pa: ਫਿਊਜ਼
    pl: bezpiecznik elektryczny
    pt: fusível
    ro: Siguranță fuzibilă
    ru: электрический предохранитель
    sco: fuse
    se: sihkkarasti
    sh: Osigurač
    si: පිලිසිය (විදිලි)
    sk: Poistka
    sl: varovalka
    smj: siekrik
    smn: sudetteijee
    sr: електрични осигурач
    sr_ec: Електрични осигурач
    sr_el: Električni osigurač
    sv: säkring
    sw: Fyuzi
    ta: உருகுக்கம்பி
    th: ฟิวส์
    tr: sigorta
    uk: запобіжник
    ur: فیثا غورث
    vi: cầu chì
    wuu: 保险丝
    yue: 菲士
    za: baujyenjswh
    zh: 保險絲
    zh_cn: 保险丝
    zh_hans: 保险丝
    zh_hant: 保險絲
    zh_hk: 保險絲
    zh_sg: 保险丝
    zh_tw: 保險絲
---
# [[Fuse]] 

#is_/same_as :: [[../../../../WikiData/WD~Fuse,182221|WD~Fuse,182221]] 

## #has_/text_of_/abstract 

> In electronics and electrical engineering, a **Fuse** is an electrical safety device 
> that operates to provide overcurrent protection of an electrical circuit. 
> 
> Its essential component is a metal wire or strip that melts when too much current flows through it, 
> thereby stopping or interrupting the current. 
> 
> It is a sacrificial device; once a fuse has operated, it is an open circuit, 
> and must be replaced or rewired, depending on its type.
>
> Fuses have been used as essential safety devices from the early days of electrical engineering. Today there are thousands of different fuse designs which have specific current and voltage ratings, breaking capacity, and response times, depending on the application. The time and current operating characteristics of fuses are chosen to provide adequate protection without needless interruption. Wiring regulations usually define a maximum fuse current rating for particular circuits. A fuse can be used to mitigate short circuits, overloading, mismatched loads, or device failure. When a damaged live wire makes contact with a metal case that is connected to ground, a short circuit will form and the fuse will melt.
>
> A fuse is an automatic means of removing power from a faulty system, often abbreviated to ADS (automatic disconnection of supply). Circuit breakers have replaced fuses in many contexts, but have significantly different characteristics, and fuses are still used when space, resiliency or cost are significant factors.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Fuse%20(electrical)) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Electricity/Electrical_Network/Fuse|Fuse]] 

### #is_/same_as :: [[/_public/Science/Physics/Electricity/Electrical_Network/Fuse.public|Fuse.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Electricity/Electrical_Network/Fuse.internal|Fuse.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Electricity/Electrical_Network/Fuse.protect|Fuse.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Electricity/Electrical_Network/Fuse.private|Fuse.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Electricity/Electrical_Network/Fuse.personal|Fuse.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Electricity/Electrical_Network/Fuse.secret|Fuse.secret]] 

