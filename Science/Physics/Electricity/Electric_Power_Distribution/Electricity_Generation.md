---
aliases:
- cynhyrchu trydan
- electricity generation
- elektriciteitsopwekking
- elektrienergia tootmine
- elektrik istehsalı
- elektrik üretme
- Elektrizitatea sortzea
- Elektrizitätsgewinnung
- elproduktion
- faamoeproduksjovne
- fábmobuvtadibme
- generació d'electricitat
- generación de energía eléctrica
- Generarea electricității
- genero de elektro
- geração de eletricidade
- giniúint leictreachais
- Hoat-tiān
- kraftproduksjon
- kragopwekking
- liâdggpuuʹtʼtõs
- Pembangkitan listrik
- penjanaan elektrik
- prodhimi i elektricitetit
- production d'électricité
- Produjhaedje di corant
- produkcja energii elektrycznej
- Produssion d'energìa elétrica
- produzione di energia elettrica
- proizvodnja elektrike
- Proizvodnja električne energije
- sähköntuotanto
- sản xuất điện năng
- villamosenergia-termelés
- výroba elektřiny
- xeneración d'enerxía llétrico
- xeración de electricidade
- zenerasion de ełetrisità
- ġenerazzjoni tal-elettriku
- šleađgabuvttadeapmi
- šleđgâpyevtittem
- генеруванння електроенергії
- генэрацыя электраэнэргіі
- производња електричне енергије
- Производство на електроенергия
- производство электроэнергии
- Цахилгаан эрчим хүчний үйлдвэрлэл
- էլեկտրաէներգիայի արտադրություն
- ייצור חשמל
- بجلی تیاری
- بجلی کی پیداوار
- توليد الكهرباء
- تولید انرژی الکتریکی
- विद्युत उत्पादन
- वीजनिर्मिती
- বিদ্যুৎ শক্তি উৎপাদন
- ਬਿਜਲਈ ਨਿਰਮਾਣ
- ବିଦ୍ୟୁତ ଉତ୍ପାଦନ
- மின் உற்பத்தி
- విద్యుత్తు ఉత్పత్తి
- വൈദ്യുതോൽപ്പാദനം
- විදුලිය ජනනය කිරිමේ ක්රම
- လျှပ်စစ်ဓာတ်အား ထုတ်လုပ်ခြင်း
- 发电
- 発電
- 發電
- 발전
has_id_wikidata: Q383973
instance_of:
- '[[_Standards/WikiData/WD~quality_process,3406372]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829]]'
subclass_of:
- '[[_Standards/WikiData/WD~energy_production,3406659]]'
- '[[_Standards/WikiData/WD~process,10843872]]'
- '[[_Standards/WikiData/WD~electrical_engineering,43035]]'
on_focus_list_of_Wikimedia_project: '[[_Standards/WikiData/WD~WikiProject_Climate_change,15305047]]'
Wikidata_property: annual energy output
OpenStreetMap_tag_or_key: Key:power
image: http://commons.wikimedia.org/wiki/Special:FilePath/Mohave%20Generating%20Station%201.jpg
IEV_number: 601-01-06
Commons_category: Electric power generation
dv_has_:
  name_:
    af: kragopwekking
    ar: توليد الكهرباء
    ast: xeneración d'enerxía llétrico
    az: elektrik istehsalı
    be_tarask: генэрацыя электраэнэргіі
    bg: Производство на електроенергия
    bn: বিদ্যুৎ শক্তি উৎপাদন
    ca: generació d'electricitat
    cs: výroba elektřiny
    cy: cynhyrchu trydan
    da: Elproduktion
    de: Elektrizitätsgewinnung
    en: electricity generation
    eo: genero de elektro
    es: generación de energía eléctrica
    et: elektrienergia tootmine
    eu: Elektrizitatea sortzea
    fa: تولید انرژی الکتریکی
    fi: sähköntuotanto
    fr: production d'électricité
    ga: giniúint leictreachais
    gl: xeración de electricidade
    he: ייצור חשמל
    hi: विद्युत उत्पादन
    hr: Proizvodnja električne energije
    hu: villamosenergia-termelés
    hy: էլեկտրաէներգիայի արտադրություն
    id: Pembangkitan listrik
    it: produzione di energia elettrica
    ja: 発電
    ko: 발전
    ml: വൈദ്യുതോൽപ്പാദനം
    mn: Цахилгаан эрчим хүчний үйлдвэрлэл
    mr: वीजनिर्मिती
    ms: penjanaan elektrik
    mt: ġenerazzjoni tal-elettriku
    my: လျှပ်စစ်ဓာတ်အား ထုတ်လုပ်ခြင်း
    nan: Hoat-tiān
    nb: kraftproduksjon
    nl: elektriciteitsopwekking
    or: ବିଦ୍ୟୁତ ଉତ୍ପାଦନ
    pa: ਬਿਜਲਈ ਨਿਰਮਾਣ
    pl: produkcja energii elektrycznej
    pms: Produssion d'energìa elétrica
    pnb: بجلی تیاری
    pt: geração de eletricidade
    pt_br: geração de eletricidade
    ro: Generarea electricității
    ru: производство электроэнергии
    se: šleađgabuvttadeapmi
    sh: Proizvodnja električne energije
    si: විදුලිය ජනනය කිරිමේ ක්රම
    sl: proizvodnja elektrike
    sma: faamoeproduksjovne
    smj: fábmobuvtadibme
    smn: šleđgâpyevtittem
    sms: liâdggpuuʹtʼtõs
    sq: prodhimi i elektricitetit
    sr: производња електричне енергије
    sv: elproduktion
    ta: மின் உற்பத்தி
    te: విద్యుత్తు ఉత్పత్తి
    tr: elektrik üretme
    uk: генеруванння електроенергії
    ur: بجلی کی پیداوار
    vec: zenerasion de ełetrisità
    vi: sản xuất điện năng
    wa: Produjhaedje di corant
    wuu: 发电
    yue: 發電
    zh: 發電
    zh_cn: 发电
    zh_hans: 发电
    zh_hant: 發電
    zh_hk: 發電
    zh_sg: 发电
    zh_tw: 發電
---
# [[Electricity_Generation]] 

#is_/same_as :: [[../../../../WikiData/WD~Electricity_generation,383973|WD~Electricity_generation,383973]] 

## #has_/text_of_/abstract 

> Electricity generation is the process of generating electric power from sources of primary energy. For utilities in the electric power industry, it is the stage prior to its delivery (transmission, distribution, etc.) to end users or its storage, using for example, the pumped-storage method.
>
> Consumable electricity is not freely available in nature, so it must be "produced", transforming other forms of energy to electricity. Production is carried out in power stations, also called "power plants". Electricity is most often generated at a power plant by electromechanical generators, primarily driven by heat engines fueled by combustion or nuclear fission, but also by other means such as the kinetic energy of flowing water and wind. Other energy sources include solar photovoltaics and geothermal power. There are exotic and speculative methods to recover energy, such as proposed fusion reactor designs which aim to directly extract energy from intense magnetic fields generated by fast-moving charged particles generated by the fusion reaction (see magnetohydrodynamics). 
>
> Phasing out coal-fired power stations and eventually gas-fired power stations, or, if practical, capturing their greenhouse gas emissions, is an important part of the energy transformation required to limit climate change. Vastly more solar power and wind power is forecast to be required, with electricity demand increasing strongly with further electrification of transport, homes and industry. However, in 2023, it was reported that the global electricity supply was approaching peak CO2 emissions thanks to the growth of solar and wind power.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Electricity%20generation) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Electricity/Electric_Power_Distribution/Electricity_Generation|Electricity_Generation]] 

### #is_/same_as :: [[/_public/Science/Physics/Electricity/Electric_Power_Distribution/Electricity_Generation.public|Electricity_Generation.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Electricity/Electric_Power_Distribution/Electricity_Generation.internal|Electricity_Generation.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Electricity/Electric_Power_Distribution/Electricity_Generation.protect|Electricity_Generation.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Electricity/Electric_Power_Distribution/Electricity_Generation.private|Electricity_Generation.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Electricity/Electric_Power_Distribution/Electricity_Generation.personal|Electricity_Generation.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Electricity/Electric_Power_Distribution/Electricity_Generation.secret|Electricity_Generation.secret]] 

