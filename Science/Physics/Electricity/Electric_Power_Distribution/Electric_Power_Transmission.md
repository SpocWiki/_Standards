---
aliases:
- Electric power transmission
- Electric_Power_Transmission
- elektr tokini uzatish
- elektriciteitstransmissie
- elektrienergia ülekanne
- elektriese kragtransmissie
- elektrik iletim hattı
- elektrik veriliş xətti
- elektrisk kraftoverføring
- Elektrizitätsübertragung
- elektros perdavimo linija
- elnett
- energia elektrikoaren transmisio
- kraftledning
- optimering af elnettet
- optimizarea rețelei electrice
- pembekalan kuasa elektrik
- prenos električne energije
- prenosová sústava
- prijenos električne energije
- přenos elektrické energie
- sieć elektroenergetyczna
- systema electricitatem subministrandi
- sähkönsiirto
- transmetimi i energjisë elektrike
- transmisi tenaga listrik
- transmisio de elektra energio
- transmisión de energía eléctrica
- transmisión de enerxía eléctrica
- transmissão de energia elétrica
- transport d'energia elèctrica
- transport d'énergie électrique
- transpò enèji elektrik
- trasmission d'energìa elétrica
- trasmissione di energia elettrica
- tresmisión d'enerxía llétrico
- ukufufukiswa kwamazuba
- átviteli hálózat
- βελτιστοποίηση του ηλεκτρικού δικτύου
- перадача электрычнай энэргіі
- передавання електроенергії
- передача электрической энергии
- пренос електричне енергије
- пренос на електрическа енергия
- цахилгаан эрчим хүч дамжуулах
- էլեկտրական էներգիայի հաղորդում
- מערכת הולכת חשמל
- انتقال انرژی الکتریکی
- د برېښنا انرژي خپرېدل
- نقل الكهرباء
- گواستنەوەی کارەبا
- विद्युत उर्जा पारेषण
- विद्युतशक्ति का प्रेषण
- বিদ্যুৎশক্তি সঞ্চালন
- மின்திறன் செலுத்தல்
- 輸電
- 輸電系統
- 输电系统
- 送電
- 송전
has_id_wikidata: Q200928
different_from:
- '[[_Standards/WikiData/WD~overhead_contact_line,110701|WD~overhead_contact_line,110701]]'
- '[[_Standards/WikiData/WD~Electric_transmission,96377104|WD~Electric_transmission,96377104]]'
described_by_source:
- '[[../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Small_Soviet_Encyclopedia,66386517|WD~Small_Soviet_Encyclopedia,66386517]]'
subclass_of:
- '[[_Standards/WikiData/WD~power_transmission,3242194|WD~power_transmission,3242194]]'
- '[[_Standards/WikiData/WD~electrical_engineering,43035|WD~electrical_engineering,43035]]'
instance_of:
- '[[../../../../WikiData/WD~economic_activity,8187769|WD~economic_activity,8187769]]'
- '[[_Standards/WikiData/WD~academic_discipline,11862829|WD~academic_discipline,11862829]]'
history_of_topic: '[[_Standards/WikiData/WD~history_of_electric_power_transmission,17030101|WD~history_of_electric_power_transmission,17030101]]'
uses:
- '[[_Standards/WikiData/WD~high_voltage_power_line,18562917|WD~high_voltage_power_line,18562917]]'
- '[[_Standards/WikiData/WD~energy_infrastructure,56397239|WD~energy_infrastructure,56397239]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q9610%20%28ben%29-Tahmid-%E0%A6%AC%E0%A6%BF%E0%A6%A6%E0%A7%8D%E0%A6%AF%E0%A7%81%E0%A7%8E%E0%A6%B6%E0%A6%95%E0%A7%8D%E0%A6%A4%E0%A6%BF%20%E0%A6%B8%E0%A6%9E%E0%A7%8D%E0%A6%9A%E0%A6%BE%E0%A6%B2%E0%A6%A8.wav
Nomenclature_for_Museum_Cataloging: 8799
OpenStreetMap_tag_or_key:
- Key:power
- Tag:usage=transmission
image: http://commons.wikimedia.org/wiki/Special:FilePath/Freileitung-Ried-1.jpg
Commons_category: Electric power transmission
described_at_URL: https://www.osha.gov/etools/electric-power/illustrated-glossary/transmission-lines
dv_has_:
  name_:
    af: elektriese kragtransmissie
    ar: نقل الكهرباء
    ast: tresmisión d'enerxía llétrico
    az: elektrik veriliş xətti
    be_tarask: перадача электрычнай энэргіі
    bg: пренос на електрическа енергия
    bn: বিদ্যুৎশক্তি সঞ্চালন
    ca: transport d'energia elèctrica
    ckb: گواستنەوەی کارەبا
    cs: přenos elektrické energie
    da: optimering af elnettet
    de: Elektrizitätsübertragung
    el: βελτιστοποίηση του ηλεκτρικού δικτύου
    en: electric power transmission
    eo: transmisio de elektra energio
    es: transmisión de energía eléctrica
    et: elektrienergia ülekanne
    eu: energia elektrikoaren transmisio
    fa: انتقال انرژی الکتریکی
    fi: sähkönsiirto
    fr: transport d'énergie électrique
    gl: transmisión de enerxía eléctrica
    he: מערכת הולכת חשמל
    hi: विद्युतशक्ति का प्रेषण
    hr: prijenos električne energije
    ht: transpò enèji elektrik
    hu: átviteli hálózat
    hy: էլեկտրական էներգիայի հաղորդում
    id: transmisi tenaga listrik
    it: trasmissione di energia elettrica
    ja: 送電
    ko: 송전
    la: systema electricitatem subministrandi
    lt: elektros perdavimo linija
    mn: цахилгаан эрчим хүч дамжуулах
    mr: विद्युत उर्जा पारेषण
    ms: pembekalan kuasa elektrik
    nb: elektrisk kraftoverføring
    nl: elektriciteitstransmissie
    nn: elnett
    pl: sieć elektroenergetyczna
    pms: trasmission d'energìa elétrica
    ps: د برېښنا انرژي خپرېدل
    pt: transmissão de energia elétrica
    ro: optimizarea rețelei electrice
    ru: передача электрической энергии
    sk: prenosová sústava
    sl: prenos električne energije
    sq: transmetimi i energjisë elektrike
    sr: пренос електричне енергије
    sv: kraftledning
    ta: மின்திறன் செலுத்தல்
    tr: elektrik iletim hattı
    uk: передавання електроенергії
    uz: elektr tokini uzatish
    wuu: 输电系统
    yue: 輸電
    zh: 輸電系統
    zh_cn: 输电系统
    zh_hans: 输电系统
    zh_hant: 輸電系統
    zh_hk: 輸電系統
    zh_sg: 输电系统
    zh_tw: 輸電系統
    zu: ukufufukiswa kwamazuba
---

# [[Electric_Power_Transmission]] 

#is_/different_from :: [[Electric_Power_Distribution]] 
#is_/same_as :: [[../../../../WikiData/WD~Electric_power_transmission,200928|WD~Electric_power_transmission,200928]] 

## #has_/text_of_/abstract 

> Electric power transmission is the bulk movement of electrical energy 
> from a generating site, such as a power plant, to an electrical substation. 
> 
> The interconnected lines that facilitate this movement form a transmission network. 
> This is distinct from the local wiring between high-voltage substations and customers, 
> which is typically referred to as electric power distribution. 
> The combined transmission and distribution network is part of electricity delivery, known as the electrical grid.
>
> Efficient long-distance transmission of electric power requires high voltages. This reduces the losses produced by strong currents. Transmission lines use either alternating current (AC) or direct current (DC). The voltage level is changed with transformers. The voltage is stepped up for transmission, then reduced for local distribution.
>
> A wide area synchronous grid, known as an interconnection in North America, directly connects generators delivering AC power with the same relative frequency to many consumers. North America has four major interconnections: Western, Eastern, Quebec and Texas. One grid connects most of continental Europe.
>
> Historically, transmission and distribution lines were often owned by the same company, but starting in the 1990s, many countries liberalized the regulation of the electricity market in ways that led to separate companies handling transmission and distribution.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Electric%20power%20transmission) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Electricity/Electric_Power_Distribution/Electric_Power_Transmission|Electric_Power_Transmission]] 

### #is_/same_as :: [[/_public/Science/Physics/Electricity/Electric_Power_Distribution/Electric_Power_Transmission.public|Electric_Power_Transmission.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Electricity/Electric_Power_Distribution/Electric_Power_Transmission.internal|Electric_Power_Transmission.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Electricity/Electric_Power_Distribution/Electric_Power_Transmission.protect|Electric_Power_Transmission.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Electricity/Electric_Power_Distribution/Electric_Power_Transmission.private|Electric_Power_Transmission.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Electricity/Electric_Power_Distribution/Electric_Power_Transmission.personal|Electric_Power_Transmission.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Electricity/Electric_Power_Distribution/Electric_Power_Transmission.secret|Electric_Power_Transmission.secret]] 

