---
aliases:
- chất dẫn điện
- conducteur électrique
- Conductor
- conductor electric
- conductor elèctric
- conductor eléctrico
- conductor llétricu
- conductrum
- condutor
- condutor eléctrico
- condutor elétrico
- conduttore elettrico
- dargludydd trydanol
- Electrical Conductor
- Electrical_Conductor
- elektr oʻtkazgichlar
- elektra konduktilo
- elektrický vodič
- elektriese geleier
- elektrijuht
- elektriksel iletken
- elektrischer Leiter
- elektrisk ledare
- elektrisk leder
- elektrisk leiar
- elektriskais vadītājs
- elektriski wodnik
- električni prevodnik
- električni provodnik
- električni vodič
- elektromos vezető
- elektros laidininkas
- eroale elektriko
- geleider
- Isixhobo sokuhamba kombane -
- keçirici materiallar
- kondiktè
- konduktor listrik
- konduktor na elektrikal
- laitere un isolatore
- Leiter
- pengalir elektrik
- penghantar listrik
- pinchikilla pusaq
- przewodnik elektryczny
- përçuesi elektrik
- rafleiðari
- ragihbar
- seoltóir leictreach
- siltewiş
- sähköjohde
- šleđgâjođetteijee
- ηλεκτρικός αγωγός
- електричен спроводник
- електрични проводник
- провідник
- проводник
- пыртӥз лэзись
- сілтеуіш
- токуадзæн
- цахилгаан дамжуулагч
- электрычны праваднік
- электрычныя праваднікі
- ярăш
- үткәргеч
- өткөргүч
- էլեկտրական հաղորդիչներ
- מוליך חשמלי
- الیکٹریکل کنڈیکٹر
- راگهبار
- رسانای الکتریکی
- سىلتەۋىش
- موصل كهربائي
- مُوَصِّل
- کئچیریجی مادهلر
- گەیەنەری کارەبایی
- चालक
- वाहक
- विद्युत चालक
- सुचालक
- তড়িৎ পরিবাহী
- ਬਿਜਲਈ ਚਾਲਕ
- ସୁପରିବାହୀ
- மின் வன்கடத்தி
- విద్యుత్ వాహకాలు
- ವಿದ್ಯುತ್ ವಾಹಕ
- വൈദ്യുത ചാലകം
- ตัวนำไฟฟ้า
- လျှပ်ကူးပစ္စည်း
- ၶိူင်ႈၾႆးၾလတ်းလႆႈ
- გამტარი
- 导体
- 導電體
- 導體
- 电气导体
- 電気伝導体
- 전기 전도체
has_id_wikidata: Q124291
made_from_material:
- '[[_Standards/WikiData/WD~electrolyte,162908|WD~electrolyte,162908]]'
- '[[_Standards/WikiData/WD~graphite,5309|WD~graphite,5309]]'
- '[[_Standards/WikiData/WD~metal,11426|WD~metal,11426]]'
opposite_of: '[[_Standards/WikiData/WD~insulator,178150|WD~insulator,178150]]'
has_characteristic:
- '[[_Standards/WikiData/WD~temperature_coefficient,898642|WD~temperature_coefficient,898642]]'
- '[[_Standards/WikiData/WD~current_voltage_characteristic,1893753|WD~current_voltage_characteristic,1893753]]'
- '[[_Standards/WikiData/WD~electrical_conductivity,4593291|WD~electrical_conductivity,4593291]]'
described_by_source:
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633|WD~Armenian_Soviet_Encyclopedia,_vol._6,124737633]]'
subclass_of: '[[_Standards/WikiData/WD~conductor,26717101|WD~conductor,26717101]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Stranded%20lamp%20wire.jpg
IEV_number: 121-12-02
Commons_category: Electrical conductors
dv_has_:
  name_:
    af: elektriese geleier
    ar: موصل كهربائي
    ast: conductor llétricu
    az: keçirici materiallar
    azb: کئچیریجی مادهلر
    be: электрычныя праваднікі
    be_tarask: электрычны праваднік
    bg: проводник
    bn: তড়িৎ পরিবাহী
    bs: električni vodič
    ca: conductor elèctric
    ckb: گەیەنەری کارەبایی
    cs: elektrický vodič
    cv: ярăш
    cy: dargludydd trydanol
    da: elektrisk leder
    de: elektrischer Leiter
    el: ηλεκτρικός αγωγός
    en: electrical conductor
    eo: elektra konduktilo
    es: conductor eléctrico
    et: elektrijuht
    eu: eroale elektriko
    fa: رسانای الکتریکی
    fi: sähköjohde
    fr: conducteur électrique
    ga: seoltóir leictreach
    gl: condutor eléctrico
    gor: penghantar listrik
    he: מוליך חשמלי
    hi: चालक
    hr: električni vodič
    hsb: elektriski wodnik
    ht: kondiktè
    hu: elektromos vezető
    hy: էլեկտրական հաղորդիչներ
    id: penghantar listrik
    is: rafleiðari
    it: conduttore elettrico
    ja: 電気伝導体
    ka: გამტარი
    kk: сілтеуіш
    kk_arab: سىلتەۋىش
    kk-cn: سىلتەۋىش
    kk_cyrl: сілтеуіш
    kk-kz: сілтеуіш
    kk_latn: siltewiş
    kk-tr: siltewiş
    kn: ವಿದ್ಯುತ್ ವಾಹಕ
    ko: 전기 전도체
    ku: ragihbar
    ku_arab: راگهبار
    ku_latn: ragihbar
    ky: өткөргүч
    la: conductrum
    lt: elektros laidininkas
    lv: elektriskais vadītājs
    mag: विद्युत चालक
    mk: електричен спроводник
    ml: വൈദ്യുത ചാലകം
    mn: цахилгаан дамжуулагч
    mr: वाहक
    ms: pengalir elektrik
    my: လျှပ်ကူးပစ္စည်း
    nb: elektrisk leder
    ne: सुचालक
    nl: geleider
    nn: elektrisk leiar
    or: ସୁପରିବାହୀ
    os: токуадзæн
    pa: ਬਿਜਲਈ ਚਾਲਕ
    pl: przewodnik elektryczny
    pms: condutor
    pnb: الیکٹریکل کنڈیکٹر
    pt: condutor elétrico
    qu: pinchikilla pusaq
    ro: conductor electric
    ru: проводник
    sco: conductor
    sh: električni vodič
    shn: ၶိူင်ႈၾႆးၾလတ်းလႆႈ
    sk: elektrický vodič
    sl: električni prevodnik
    smn: šleđgâjođetteijee
    sq: përçuesi elektrik
    sr: електрични проводник
    sr_ec: електрични проводник
    sr_el: električni provodnik
    stq: laitere un isolatore
    su: konduktor listrik
    sv: elektrisk ledare
    ta: மின் வன்கடத்தி
    te: విద్యుత్ వాహకాలు
    th: ตัวนำไฟฟ้า
    tl: konduktor na elektrikal
    tr: elektriksel iletken
    tt: үткәргеч
    tt_cyrl: үткәргеч
    udm: пыртӥз лэзись
    uk: провідник
    ur: مُوَصِّل
    uz: elektr oʻtkazgichlar
    vi: chất dẫn điện
    wuu: 电气导体
    xh: Isixhobo sokuhamba kombane -
    yue: 導電體
    zh: 導電體
    zh_cn: 导体
    zh_hans: 导体
    zh_hant: 導體
    zh_hk: 導體
    zh_sg: 导体
    zh_tw: 導體
---

# [[Electrical_Conductor]] 

#is_/same_as :: [[../../../../WikiData/WD~Electrical_conductor,124291|WD~Electrical_conductor,124291]] 

## #has_/text_of_/abstract 

> In physics and electrical engineering, a conductor is an object or type of material 
> that allows the flow of charge (electric current) in one or more directions. 
> Materials made of metal are common electrical conductors. 
> The flow of negatively charged electrons generates electric current, 
> positively charged holes, and positive or negative ions in some cases.
>
> In order for current to flow within a closed electrical circuit, one charged particle does not need to travel from the component producing the current (the current source) to those consuming it (the loads). Instead, the charged particle simply needs to nudge its neighbor a finite amount, who will nudge its neighbor, and on and on until a particle is nudged into the consumer, thus powering it. Essentially what is occurring is a long chain of momentum transfer between mobile charge carriers; the Drude model of conduction describes this process more rigorously. This momentum transfer model makes metal an ideal choice for a conductor; metals, characteristically, possess a delocalized sea of electrons which gives the electrons enough mobility to collide and thus affect a momentum transfer.
>
> As discussed above, electrons are the primary mover in metals; however, other devices such as the cationic electrolyte(s) of a battery, or the mobile protons of the proton conductor of a fuel cell rely on positive charge carriers. Insulators are non-conducting materials with few mobile charges that support only insignificant electric currents.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Electrical%20conductor) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Electricity/Electric_Power_Distribution/Electrical_Conductor|Electrical_Conductor]] 

### #is_/same_as :: [[/_public/Science/Physics/Electricity/Electric_Power_Distribution/Electrical_Conductor.public|Electrical_Conductor.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Electricity/Electric_Power_Distribution/Electrical_Conductor.internal|Electrical_Conductor.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Electricity/Electric_Power_Distribution/Electrical_Conductor.protect|Electrical_Conductor.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Electricity/Electric_Power_Distribution/Electrical_Conductor.private|Electrical_Conductor.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Electricity/Electric_Power_Distribution/Electrical_Conductor.personal|Electrical_Conductor.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Electricity/Electric_Power_Distribution/Electrical_Conductor.secret|Electrical_Conductor.secret]] 

