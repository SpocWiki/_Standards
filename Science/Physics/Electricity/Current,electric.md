---
aliases:
- .توك
- alektrėnė sraujė
- arus elektrik
- arus listrik
- aḍru amestrisiti
- cerrynt trydanol
- corent elétrica
- corente ełetrega
- corrent electric
- corrent elettrica
- corrent elèctric
- corrente elettrica
- corrente eléctrica
- corrente elétrica
- corrient electrica
- corriente eléctrica
- corriente llétrico
- courant
- courant électrique
- curent electric
- current
- Current,electric
- currente electric
- currenti elettrica
- daloy ng kuryente
- dawaanu mbëj
- dòng điện
- Electric current
- Electric_Current
- elektr togı
- elektr toki
- elektra korento
- elektra kurento
- elektresche Stroum
- elektrický proud
- elektrický prúd
- elektrieke stroom
- elektriese stroom
- elektrik akımı
- elektrik ceryanı
- elektrik cərəyanı
- elektrikong kuryente
- elektrine joksuz
- elektrischa Strom
- elektrische sjtroum
- elektrische stroom
- Elektrischer Strom
- elektrisk straum
- elektrisk ström
- elektrisk strøm
- elektriskā strāva
- elektrivool
- električna struja
- električni tok
- elektromos áram
- elektros srovė
- elrávdnji
- fluxus oneris electrici
- frosedh tredanek
- kaarentii elektirikii
- korronte elektriko
- kouran elektrik
- kurrent elettriku
- kuryente elektrisidad
- liâdggräuʹnnj
- mkondo wa umeme
- pinchikilla puriy
- prąd elektryczny
- rafstraumur
- red tredan
- rianaratra
- rryma elektrike
- rufuvha
- sirêma elektrîkê
- sruth leictreach
- Stroom
- sztrům
- sähkövirta
- tawa pi wawa linja
- tiān-liû
- uluzuba
- Ìwọ́ ìtanná
- šleđgâvirde
- ηλεκτρικό ρεύμα
- електрически ток
- електрична струја
- електричний струм
- сахилгаан гүйдэл
- ток
- цахилгаан гүйдэл
- электр агымы
- электр тогу
- электр тогы
- электрический ток
- электро ток
- электрычны ток
- էլեկտրական հոսանք
- זרם חשמלי
- עלעקטרישער שטראם
- الکتریک جریانی
- بجليءَ جو ڪرنٽ
- برقی رو
- برېښنايي جريان
- تيار كهربائي
- تەزووی کارەبایی
- جریان الکتریکی
- سارح د التريسينتي
- کرنٹ
- گریٛن
- विद्युत धारा
- विद्युत प्रवाह
- विद्युतीय बाहा
- তড়িৎ প্রবাহ
- বৈদ্যুতিক প্ৰবাহ
- ਚਲੰਤ ਬਿਜਲੀ
- ବିଦ୍ୟୁତ୍ ସ୍ରୋତ
- மின்னோட்டம்
- ఎలెక్ట్రిక్ కరెంట్
- ವಿದ್ಯುತ್ ಪ್ರವಾಹ
- വൈദ്യുതധാര
- กระแสไฟฟ้า
- လျှပ်စီး
- ელექტრული დენი
- 电流
- 電流
- 전류
has_id_wikidata: Q11651
Colon_Classification: C62
Commons_category: Electric current
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Small_Soviet_Encyclopedia,66386517|WD~Small_Soviet_Encyclopedia,66386517]]'
different_from:
- '[[_Standards/WikiData/WD~electric_current_density,234072|WD~electric_current_density,234072]]'
- '[[_Standards/WikiData/WD~flux_density,51720768|WD~flux_density,51720768]]'
- '[[_Standards/WikiData/WD~electric_current,29996|WD~electric_current,29996]]'
facet_of: '[[_Standards/WikiData/WD~electricity,12725|WD~electricity,12725]]'
has_part_s_: '[[_Standards/WikiData/WD~ion_current,6063423|WD~ion_current,6063423]]'
image: http://commons.wikimedia.org/wiki/Special:FilePath/Magnetic%20field%20of%20a%20steady%20current.svg
OmegaWiki_Defined_Meaning: 684678
permanent_duplicated_item: '[[_Standards/WikiData/WD~Q25681476,25681476|WD~Q25681476,25681476]]'
subclass_of: '[[_Standards/WikiData/WD~current,56298508|WD~current,56298508]]'
Wikidata_property: maximum current
dv_has_:
  name_:
    af: elektriese stroom
    an: corrient electrica
    ar: تيار كهربائي
    ary: سارح د التريسينتي
    as: বৈদ্যুতিক প্ৰবাহ
    ast: corriente llétrico
    az: elektrik cərəyanı
    azb: الکتریک جریانی
    ba: электр тогы
    bar: elektrischa Strom
    bcl: elektrikong kuryente
    be: электрычны ток
    be_tarask: электрычны ток
    bg: електрически ток
    bn: তড়িৎ প্রবাহ
    br: red tredan
    bs: električna struja
    bxr: сахилгаан гүйдэл
    ca: corrent elèctric
    ce: ток
    ckb: تەزووی کارەبایی
    crh: elektrik ceryanı
    cs: elektrický proud
    cv: электро ток
    cy: cerrynt trydanol
    da: elektrisk strøm
    de: elektrischer Strom
    el: ηλεκτρικό ρεύμα
    en: electric current
    eo: elektra kurento
    es: corriente eléctrica
    et: elektrivool
    eu: korronte elektriko
    fa: جریان الکتریکی
    fi: sähkövirta
    fr: courant électrique
    frr: Stroom
    ga: sruth leictreach
    gl: corrente eléctrica
    gor: arus listrik
    gsw: elektrischer Strom
    he: זרם חשמלי
    hi: विद्युत धारा
    hif: electric current
    hr: električna struja
    ht: kouran elektrik
    hu: elektromos áram
    hy: էլեկտրական հոսանք
    ia: currente electric
    id: arus listrik
    io: elektra korento
    is: rafstraumur
    it: corrente elettrica
    ja: 電流
    ka: ელექტრული დენი
    kaa: elektr togı
    kab: aḍru amestrisiti
    kk: электр тогы
    kn: ವಿದ್ಯುತ್ ಪ್ರವಾಹ
    ko: 전류
    ks: گریٛن
    ku: sirêma elektrîkê
    kw: frosedh tredanek
    ky: электр тогу
    la: fluxus oneris electrici
    lb: elektresche Stroum
    li: elektrische sjtroum
    lmo: corrent elettrica
    ln: courant
    lt: elektros srovė
    lv: elektriskā strāva
    lzh: 電流
    mag: विद्युत प्रवाह
    mg: rianaratra
    min: arus listrik
    mk: електрична струја
    ml: വൈദ്യുതധാര
    mn: цахилгаан гүйдэл
    mr: विद्युत धारा
    ms: arus elektrik
    mt: kurrent elettriku
    my: လျှပ်စီး
    nan: tiān-liû
    nb: elektrisk strøm
    ne: विद्युत धारा
    new: विद्युतीय बाहा
    nl: elektrische stroom
    nn: elektrisk straum
    oc: corrent electric
    om: kaarentii elektirikii
    or: ବିଦ୍ୟୁତ୍ ସ୍ରୋତ
    pa: ਚਲੰਤ ਬਿਜਲੀ
    pl: prąd elektryczny
    pms: corent elétrica
    pnb: کرنٹ
    ps: برېښنايي جريان
    pt: corrente elétrica
    qu: pinchikilla puriy
    ro: curent electric
    ru: электрический ток
    scn: currenti elettrica
    sco: current
    sd: بجليءَ جو ڪرنٽ
    se: elrávdnji
    sgs: alektrėnė sraujė
    sh: električna struja
    sk: elektrický prúd
    sl: električni tok
    smn: šleđgâvirde
    sms: liâdggräuʹnnj
    sn: rufuvha
    sq: rryma elektrike
    sr: електрична струја
    su: arus listrik
    sv: elektrisk ström
    sw: mkondo wa umeme
    szl: sztrům
    ta: மின்னோட்டம்
    te: ఎలెక్ట్రిక్ కరెంట్
    th: กระแสไฟฟ้า
    tl: daloy ng kuryente
    tok: tawa pi wawa linja
    tr: elektrik akımı
    tt: электр агымы
    tt_cyrl: электр агымы
    udm: электро ток
    ug: .توك
    uk: електричний струм
    ur: برقی رو
    uz: elektr toki
    vec: corente ełetrega
    vep: elektrine joksuz
    vi: dòng điện
    vls: elektrieke stroom
    war: kuryente elektrisidad
    wo: dawaanu mbëj
    wuu: 电流
    xmf: ელექტრული დენი
    yi: עלעקטרישער שטראם
    yo: Ìwọ́ ìtanná
    yue: 電流
    zh: 电流
    zh_hans: 电流
    zh_hant: 電流
    zh_tw: 電流
    zu: uluzuba
---

# [[Current,electric]]   

#is_/same_as :: [[../../../WikiData/WD~Electric_current,11651|WD~Electric_current,11651]] 

There are many Units for Current, but the SI-Unit is the [[../../../Dimension/Unit_of_Measure/SI-Unit/Unit~Ampere|Unit~Ampere]]. 


## #has_/text_of_/abstract 

> An electric current is a flow of charged particles, such as electrons or ions, 
> moving through an electrical conductor or space. 
> 
> It is defined as the net rate of flow of electric charge through a surface. 
> The moving particles are called charge carriers, 
> which may be one of several types of particles, depending on the conductor.  
> 
> In electric circuits the charge carriers are often electrons moving through a wire. 
> In semiconductors they can be electrons or holes. 
> In an electrolyte the charge carriers are ions, while in plasma, an ionized gas, they are ions and electrons.
>
> In the International System of Units (SI), electric current is expressed in units of ampere 
> (sometimes called an "amp", symbol A), which is equivalent to one coulomb per second. 
> The ampere is an SI base unit and electric current 
> is a base quantity in the International System of Quantities (ISQ). 
> Electric current is also known as amperage and is measured using a device called an ammeter.
>
> Electric currents create magnetic fields, 
> which are used in motors, generators, inductors, and transformers. 
> In ordinary conductors, they cause Joule heating, which creates light in incandescent light bulbs. 
> Time-varying currents emit electromagnetic waves, 
> which are used in telecommunications to broadcast information.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Electric%20current) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/Science/Physics/Electricity/Current,electric|Current,electric]] 

### #is_/same_as :: [[/_public/Science/Physics/Electricity/Current,electric.public|Current,electric.public]] 

### #is_/same_as :: [[/_internal/Science/Physics/Electricity/Current,electric.internal|Current,electric.internal]] 

### #is_/same_as :: [[/_protect/Science/Physics/Electricity/Current,electric.protect|Current,electric.protect]] 

### #is_/same_as :: [[/_private/Science/Physics/Electricity/Current,electric.private|Current,electric.private]] 

### #is_/same_as :: [[/_personal/Science/Physics/Electricity/Current,electric.personal|Current,electric.personal]] 

### #is_/same_as :: [[/_secret/Science/Physics/Electricity/Current,electric.secret|Current,electric.secret]] 

