# Safe fix script for is/ folder - processes recursively including subfolders
...
# 4. Display text matches actual filename
...
# Get all is_*.md files recursively
$files = Get-ChildItem -Path "." -Filter "is_*.md" -Recurse
$fixedCount = 0

Write-Host "`n=== Safe Fix: Plain Text -> Wiki-links (is/ folder) ===" -ForegroundColor Cyan
Write-Host "Processing files recursively, including subfolders" -ForegroundColor Cyan
    ...
    try {
        ...
        if ($content -match $absolutePathPattern) {
            ...
            foreach ($match in $matches) {
                ...
                $relPath = Find-ClassFile -className $className
                if ($relPath) {
                    $fileName = ($relPath -split '/')[-1]
                    $oldLink = $match.Value
                    $newLink = "[[$relPath|$fileName]]"
