---
aliases:
- Ajọfọ̀nàkò Àsìkò Káríayé
- Akud Amsideg Amaɣradi
- Am Uilíoch Comheagartha
- Amzer Hollvedel Kenurzhiet
- A̱mun Jen Swanta
- Bötendönya kileşterelgän waqıtı
- Coordinated Universal Time
- Coordinatit Universal Time
- egyezményes koordinált világidő
- Eşgüdümlü Evrensel Zaman
- Galah Universal Kakoordinasi
- gekoördineerde universele tyd
- Giờ Phối hợp Quốc tế
- Isuduwaha Waqtiga Caalamiga
- koordineeritud maailmaaeg
- koordinerad universell tid
- Koordiniade Wäidzeid
- koordinierte Weltzeit
- Koordinirano svjetsko vrijeme
- koordinoitu yleisaika
- Koordinovaný svetový čas
- Koordinovaný světový čas
- Koordinowany swětowy čas
- Koordinéiert Weltzäit
- Koordiněrowany swětowy cas
- Koördinearre Wrâldtiid
- Muda sanifu wa dunia
- Ora universală coordonată
- Panagpapada ti Sangalubongan nga Oras
- Pandaigdigang Batayan ng Oras
- Pankinábang Batayan kan Oras
- pasaulinis koordinuotasis laikas
- saskaņotais pasaules laiks
- Taimi Tuufaatasia Lautele
- Tempo coordinato universale
- Tempo Universal Coordenado
- Tempo Universal Coordinado
- Temps Universal Coordenat
- Temps Universal Coordinat
- temps universel coordonné
- Tempu Universal Koordiná
- Tenpo Coordenà Universałe
- tiempo universal coordinado
- Tiempo Universal Coordinato
- Tiempu Coordináu Universal
- Timp coordenât universâl
- Tlatēcpānalli cemānāhuaccāhuitl
- Traa Cho-chadjin Cho-oardit
- ténpo universâle cordinòu
- Universal tempore coordinate
- Universal Time Coordinate
- universale tempus coordinatum
- Universal_time
- univerzalni koordinirani čas
- uniwersalny czas koordynowany
- ura univirsali coordinata
- UTC
- UTC(Coordinated Universal Time)
- UTC(Coordinated_Universal_Time)
- Waktu Semesta Berkoordinat
- Waktu Universal Takoordinasi
- Waktu Universal Terkoordinasi
- Wektu Universal Kakoordinasi
- Àm Coitcheann Co-òrdanaichte
- Ára pavẽ omohendava
- Ümumdünya vaxtı
- Συγχρονισμένος Παγκόσμιος Χρόνος
- Бүткүл дүйнөлүк айкалыштырылган убакыт
- Вақти Ҳамоҳангсозии Ҷаҳонӣ
- всемирное координированное время
- Всесвітній координований час
- Дүниежүзілік үйлестірілген уақыт
- Координатжуулсан дэлхийн цаг
- Координирано универзално време
- Координирано универсално време
- Сусветны каардынаваны час
- Сусьветны каардынаваны час
- Համաշխարհային կոորդինացված ժամանակ
- זמן אוניברסלי מתואם
- التوقيت لعالمي لموحًد
- برابر عالمي وقت
- توقيت عالمي منسق
- دونیانین ایستاندارد زمانی
- ساعت هماهنگ جهانی
- عالمی وَقٕت
- ماس قەدەملىك دۇنياۋىي سائەت
- متناسق عالمی وقت
- همغږی نړیوال وخت
- کاتی گەردوونیی ھاوھەنگاو
- ޔޫ.ޓީ.ސީ.
- अन्तर्राष्ट्रिय प्रमाणिक समय
- कोऑर्डिनेटेड यूनिवर्सल टाइम
- जागतिक समन्वित वेळ
- समन्वित विश्व समय
- समन्वित सार्वत्रिक समय
- समन्वितसार्वत्रिकसमयः
- সর্বজনীন সমন্বিত সময়
- ਸੰਯੋਜਤ ਵਿਆਪਕ ਸਮਾਂ
- ஒருங்கிணைந்த அனைத்துலக நேரம்
- సార్వత్రిక సమన్వయ సమయం
- ಜಾಗತಿಕ ಸಮಯದ ನಿರ್ದೇಶಾಂಕ
- അന്താരാഷ്ട്രസമയക്രമം
- เวลาสากลเชิงพิกัด
- ເວລາສາກົນເຊີງພິກັດ
- მოსოფელიშ კოორდინირაფილი ბორჯი
- მსოფლიო კოორდინირებული დრო
- សម្របសម្រួលល្វែងម៉ោងសកល
- ᱚᱛᱥᱚᱝᱟᱱ ᱡᱮᱜᱮᱛᱟᱹᱨᱤ ᱚᱠᱛᱚ
- ⴰⴽⵓⴷ ⴰⵎⴰⴹⵍⴰⵏ ⴰⵎⵣⴷⴰⵢ
- 世界協調時間
- 世界標準時間
- 协调世界时
- 協定世界時
- 協調世界時
- 國際標準時間
- ꯌꯨꯅꯤꯚꯔꯁꯤꯌꯦꯜ ꯀꯣꯑꯣꯔꯗꯤꯅꯦꯇꯦꯗ ꯇꯥꯏꯝ
- 협정 세계시
has_id_wikidata: Q1536
based_on:
- '[[_Standards/WikiData/WD~International_Atomic_Time,119395|WD~International_Atomic_Time,119395]]'
- '[[_Standards/WikiData/WD~universal_time,874572|WD~universal_time,874572]]'
standards_body: '[[_Standards/WikiData/WD~International_Earth_Rotation_and_Reference_Systems_Service,530278|WD~International_Earth_Rotation_and_Reference_Systems_Service,530278]]'
said_to_be_the_same_as:
- '[[_Standards/WikiData/WD~Western_European_Time,843589|WD~Western_European_Time,843589]]'
- '[[_Standards/WikiData/WD~UTC_00_00,6574|WD~UTC_00_00,6574]]'
replaces: '[[_Standards/WikiData/WD~universal_time,874572|WD~universal_time,874572]]'
instance_of:
- '[[_Standards/WikiData/WD~time_standard,6313217|WD~time_standard,6313217]]'
- '[[_Standards/WikiData/WD~time_zone,12143|WD~time_zone,12143]]'
Stack_Exchange_tag: https://stackoverflow.com/tags/utc
Commons_category: Maps of time zones
short_name: UTC
dv_is_:
  same_as:
  - '[[../../../../../WikiData/WD~Coordinated_Universal_Time,1536|WD~Coordinated_Universal_Time,1536]]'
  - '[[UTC(Coordinated_Universal_Time)|UTC(Coordinated_Universal_Time)]]'
  - '[[/_public/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).public|UTC(Coordinated_Universal_Time).public]]'
  - '[[/_internal/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).internal|UTC(Coordinated_Universal_Time).internal]]'
  - '[[/_protect/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).protect|UTC(Coordinated_Universal_Time).protect]]'
  - '[[/_private/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).private|UTC(Coordinated_Universal_Time).private]]'
  - '[[/_personal/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).personal|UTC(Coordinated_Universal_Time).personal]]'
  - '[[/_secret/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).secret|UTC(Coordinated_Universal_Time).secret]]'
  - '[[UTC(Coordinated_Universal_Time)]]'
dv_is_same_as:
- '[[../../../../../WikiData/WD~Coordinated_Universal_Time,1536|WD~Coordinated_Universal_Time,1536]]'
- '[[UTC(Coordinated_Universal_Time)]]'
- '[[/_public/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).public|UTC(Coordinated_Universal_Time).public]]'
- '[[/_internal/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).internal|UTC(Coordinated_Universal_Time).internal]]'
- '[[/_protect/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).protect|UTC(Coordinated_Universal_Time).protect]]'
- '[[/_private/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).private|UTC(Coordinated_Universal_Time).private]]'
- '[[/_personal/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).personal|UTC(Coordinated_Universal_Time).personal]]'
- '[[/_secret/schema-org/Class/is_a_/Data_Type/Time_of_Day/UTC(Coordinated_Universal_Time).secret|UTC(Coordinated_Universal_Time).secret]]'
dv_has_:
  name_:
    af: gekoördineerde universele tyd
    an: Tiempo Universal Coordinato
    ar: توقيت عالمي منسق
    ary: التوقيت لعالمي لموحًد
    ast: Tiempu Coordináu Universal
    av: UTC
    az: Ümumdünya vaxtı
    azb: دونیانین ایستاندارد زمانی
    ban: Galah Universal Kakoordinasi
    bar: Koordiniade Wäidzeid
    bcl: Pankinábang Batayan kan Oras
    be: Сусветны каардынаваны час
    be_tarask: Сусьветны каардынаваны час
    bg: Координирано универсално време
    bho: कोऑर्डिनेटेड यूनिवर्सल टाइम
    bjn: Waktu Universal Takoordinasi
    blk: Coordinated Universal Time
    bn: সর্বজনীন সমন্বিত সময়
    br: Amzer Hollvedel Kenurzhiet
    bs: Koordinirano svjetsko vrijeme
    ca: Temps Universal Coordinat
    ckb: کاتی گەردوونیی ھاوھەنگاو
    crh: UTC
    cs: Koordinovaný světový čas
    cy: UTC
    da: UTC
    de: koordinierte Weltzeit
    de_ch: Koordinierte Weltzeit
    diq: UTC
    dsb: Koordiněrowany swětowy cas
    dv: ޔޫ.ޓީ.ސީ.
    el: Συγχρονισμένος Παγκόσμιος Χρόνος
    en: UTC
    en-us: Coordinated Universal Time
    eo: UTC
    es: tiempo universal coordinado
    et: koordineeritud maailmaaeg
    eu: UTC
    fa: ساعت هماهنگ جهانی
    fi: koordinoitu yleisaika
    fo: UTC
    fr: temps universel coordonné
    frr: UTC
    fur: Timp coordenât universâl
    fy: Koördinearre Wrâldtiid
    ga: Am Uilíoch Comheagartha
    gan: 世界標準時間
    gd: Àm Coitcheann Co-òrdanaichte
    gl: Tempo Universal Coordinado
    gn: Ára pavẽ omohendava
    gsw: UTC
    gv: Traa Cho-chadjin Cho-oardit
    he: זמן אוניברסלי מתואם
    hi: समन्वित सार्वत्रिक समय
    hr: Koordinirano svjetsko vrijeme
    hsb: Koordinowany swětowy čas
    hu: egyezményes koordinált világidő
    hy: Համաշխարհային կոորդինացված ժամանակ
    ia: Universal tempore coordinate
    id: Waktu Universal Terkoordinasi
    ig: Coordinated Universal Time
    ilo: Panagpapada ti Sangalubongan nga Oras
    inh: UTC
    io: UTC
    is: UTC
    it: Tempo coordinato universale
    ja: 協定世界時
    jv: Wektu Universal Kakoordinasi
    ka: მსოფლიო კოორდინირებული დრო
    kab: Akud Amsideg Amaɣradi
    kcg: A̱mun Jen Swanta
    kk: Дүниежүзілік үйлестірілген уақыт
    km: សម្របសម្រួលល្វែងម៉ោងសកល
    kn: ಜಾಗತಿಕ ಸಮಯದ ನಿರ್ದೇಶಾಂಕ
    ko: 협정 세계시
    ks: عالمی وَقٕت
    ku: UTC
    ky: Бүткүл дүйнөлүк айкалыштырылган убакыт
    la: universale tempus coordinatum
    lb: Koordinéiert Weltzäit
    lez: UTC
    li: UTC
    lij: ténpo universâle cordinòu
    lo: ເວລາສາກົນເຊີງພິກັດ
    lt: pasaulinis koordinuotasis laikas
    lv: saskaņotais pasaules laiks
    mai: अन्तर्राष्ट्रिय प्रमाणिक समय
    mg: Universal Time Coordinate
    mk: Координирано универзално време
    ml: അന്താരാഷ്ട്രസമയക്രമം
    mn: Координатжуулсан дэлхийн цаг
    mni: ꯌꯨꯅꯤꯚꯔꯁꯤꯌꯦꯜ ꯀꯣꯑꯣꯔꯗꯤꯅꯦꯇꯦꯗ ꯇꯥꯏꯝ
    mr: जागतिक समन्वित वेळ
    ms: Waktu Semesta Berkoordinat
    my: Coordinated Universal Time
    nah: Tlatēcpānalli cemānāhuaccāhuitl
    nan: UTC
    nb: UTC
    nds_nl: UTC
    ne: समन्वित विश्व समय
    nl: UTC
    nn: UTC
    nrm: UTC
    nso: Coordinated Universal Time
    oc: Temps Universal Coordenat
    pa: ਸੰਯੋਜਤ ਵਿਆਪਕ ਸਮਾਂ
    pap: Tempu Universal Koordiná
    pl: uniwersalny czas koordynowany
    pnb: متناسق عالمی وقت
    ps: همغږی نړیوال وخت
    pt: Tempo Universal Coordenado
    pt_br: Tempo Universal Coordenado
    ro: Ora universală coordonată
    ru: всемирное координированное время
    sa: समन्वितसार्वत्रिकसमयः
    sah: UTC
    sat: ᱚᱛᱥᱚᱝᱟᱱ ᱡᱮᱜᱮᱛᱟᱹᱨᱤ ᱚᱠᱛᱚ
    scn: ura univirsali coordinata
    sco: Coordinatit Universal Time
    sd: برابر عالمي وقت
    se: UTC
    sh: UTC
    shn: Coordinated Universal Time
    si: UTC
    sk: Koordinovaný svetový čas
    sl: univerzalni koordinirani čas
    sm: Taimi Tuufaatasia Lautele
    so: Isuduwaha Waqtiga Caalamiga
    sq: Coordinated Universal Time
    sr: Coordinated Universal Time
    sr_ec: Coordinated Universal Time
    stq: UTC
    su: UTC
    sv: koordinerad universell tid
    sw: Muda sanifu wa dunia
    ta: ஒருங்கிணைந்த அனைத்துலக நேரம்
    te: సార్వత్రిక సమన్వయ సమయం
    tg: Вақти Ҳамоҳангсозии Ҷаҳонӣ
    th: เวลาสากลเชิงพิกัด
    tl: Pandaigdigang Batayan ng Oras
    tr: Eşgüdümlü Evrensel Zaman
    tt: Bötendönya kileşterelgän waqıtı
    tyv: UTC
    ug: ماس قەدەملىك دۇنياۋىي سائەت
    uk: Всесвітній координований час
    ur: متناسق عالمی وقت
    uz: UTC
    vec: Tenpo Coordenà Universałe
    vep: UTC
    vi: Giờ Phối hợp Quốc tế
    vro: UTC
    wuu: 協調世界時
    xmf: მოსოფელიშ კოორდინირაფილი ბორჯი
    yo: Ajọfọ̀nàkò Àsìkò Káríayé
    yue: 國際標準時間
    zgh: ⴰⴽⵓⴷ ⴰⵎⴰⴹⵍⴰⵏ ⴰⵎⵣⴷⴰⵢ
    zh: 协调世界时
    zh_hans: 协调世界时
    zh_hk: 協調世界時
    zh_tw: 世界協調時間
    zu: Coordinated Universal Time
---

# [[UTC(Coordinated_Universal_Time)]] 

#is_/same_as :: [[../../../../../WikiData/WD~Coordinated_Universal_Time,1536|WD~Coordinated_Universal_Time,1536]] 

## #has_/text_of_/abstract 

> Coordinated Universal Time (UTC) is the **primary time standard** 
> globally used to regulate clocks and time. 
> 
> It establishes a contiguous reference for the current time, forming the basis for civil time and time zones. 
> UTC facilitates international communication, navigation, scientific research, and commerce.
>
> UTC has been widely embraced by most countries 
> and is the effective successor to Greenwich Mean Time (GMT). 
> 
> In specialised domains such as scientific research, navigation, and timekeeping, 
> other standards such as UT1 and International Atomic Time (TAI) are also used alongside UTC.
>
> UTC is based on TAI (International Atomic Time, abbreviated from its French name, temps atomique international), which is a **weighted average of hundreds of atomic clocks** worldwide. 
> UTC is **kept within about one second of mean solar time at 0° longitude** using leap Seconds, 
> the currently used prime meridian, and is **not adjusted for daylight saving time**.
>
> The coordination of time and frequency transmissions around the world began on 1 January 1960. UTC was first officially adopted as a standard in 1963 and "UTC" became the official abbreviation of Coordinated Universal Time in 1967. The current version of UTC is defined by the International Telecommunication Union.
>
> Since adoption, UTC has been adjusted several times, notably adding leap seconds starting in 1972. 
> Recent years have seen significant developments in the realm of UTC, 
> particularly in discussions about eliminating leap seconds from the timekeeping system 
> because leap seconds occasionally disrupt timekeeping systems worldwide. 
> 
> The General Conference on Weights and Measures adopted a resolution 
> to alter UTC with a new system that would eliminate leap seconds by 2035.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Coordinated%20Universal%20Time) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/schema-org/Class/is_a_/Data_Type/Time_Span/UTC(Coordinated_Universal_Time)|UTC(Coordinated_Universal_Time)]] 

### #is_/same_as :: [[/_public/schema-org/Class/is_a_/Data_Type/Time_Span/UTC(Coordinated_Universal_Time).public|UTC(Coordinated_Universal_Time).public]] 

### #is_/same_as :: [[/_internal/schema-org/Class/is_a_/Data_Type/Time_Span/UTC(Coordinated_Universal_Time).internal|UTC(Coordinated_Universal_Time).internal]] 

### #is_/same_as :: [[/_protect/schema-org/Class/is_a_/Data_Type/Time_Span/UTC(Coordinated_Universal_Time).protect|UTC(Coordinated_Universal_Time).protect]] 

### #is_/same_as :: [[/_private/schema-org/Class/is_a_/Data_Type/Time_Span/UTC(Coordinated_Universal_Time).private|UTC(Coordinated_Universal_Time).private]] 

### #is_/same_as :: [[/_personal/schema-org/Class/is_a_/Data_Type/Time_Span/UTC(Coordinated_Universal_Time).personal|UTC(Coordinated_Universal_Time).personal]] 

### #is_/same_as :: [[/_secret/schema-org/Class/is_a_/Data_Type/Time_Span/UTC(Coordinated_Universal_Time).secret|UTC(Coordinated_Universal_Time).secret]] 

