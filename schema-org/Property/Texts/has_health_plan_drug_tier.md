---
license: CC BY-SA 4.0
confidential: public
isDeleted: false
isReadOnly: false

#Obsidian well-known Keys
cssclasses: Predicate Text
publish: true

# Hugo Tags
type: Pred_Text

title: has_text_on_health_plan_drug_tier
linkTitle: has_text_on_health_plan_drug_tier

keywords: [health, plan, drug, tier]
layout: 
draft: false
publishDate:
expiryDate: 

tags:
- class/Property
- schema-org/Property/Text

aliases:
- health-plan-drug-tier
- health_plan_drug_tier
- healthPlanDrugTier
- has_text_on_health_plan_drug_tier
---

Predicate to describe the Text of HealthInsurancePlan, HealthPlanFormulary.

[ #is_/part_of :: pending: ]

Use it like this: 
- [ #has_/text_/on_/health_plan_drug_tier :: Text ] or 
- [ has_text_on_health_plan_drug_tier :: Text ] 

The tier(s) of drugs offered by this formulary or insurance plan.

Predicated describes that: 
[ #has_/domain  :: HealthInsurancePlan, HealthPlanFormulary ]
( #has_/name :: has_text_on_health_plan_drug_tier )
( #has_/range :: Text )

## Confidential Links & Embeds: 
- [[../../../../_public/schema.org/Predicate/Texts/has_health_plan_drug_tier.md|has_health_plan_drug_tier]] 
- [[../../../../_internal/schema.org/Predicate/Texts/has_health_plan_drug_tier.internal.md|has_health_plan_drug_tier.internal]] 
- [[../../../../_protect/schema.org/Predicate/Texts/has_health_plan_drug_tier.protect.md|has_health_plan_drug_tier.protect]] 
- [[../../../../_private/schema.org/Predicate/Texts/has_health_plan_drug_tier.private.md|has_health_plan_drug_tier.private]] 
- [[../../../../_personal/schema.org/Predicate/Texts/has_health_plan_drug_tier.personal.md|has_health_plan_drug_tier.personal]] 
- [[../../../../_secret/schema.org/Predicate/Texts/has_health_plan_drug_tier.secret.md|has_health_plan_drug_tier.secret]] 
