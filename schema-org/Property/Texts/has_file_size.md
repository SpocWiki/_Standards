---
license: CC BY-SA 4.0
confidential: public
isDeleted: false
isReadOnly: false

#Obsidian well-known Keys
cssclasses: Predicate Text
publish: true

# Hugo Tags
type: Pred_Text

title: has_text_of_file_size
linkTitle: has_text_of_file_size

keywords: [file, size]
layout: 
draft: false
publishDate:
expiryDate: 

tags:
- class/Property
- schema-org/Property/Text

aliases:
- file-size
- file_size
- fileSize
- has_text_of_file_size
---

Predicate to describe the Text of SoftwareApplication.

Use it like this: 
- [ #has_/text_of_/file_size :: Text ] or 
- [ has_text_of_file_size :: Text ] 

Size of the application / package (e.g. 18MB). In the absence of a unit (MB, KB etc.), KB will be assumed.

Predicated describes that: 
[ #has_/domain  :: SoftwareApplication ]
( #has_/name :: has_text_of_file_size )
( #has_/range :: Text )

## Confidential Links & Embeds: 
- [[../../../../_Standards/schema-org/Predicate/Texts/has_file_size.md|has_file_size]] 
- [[../../../../_internal/schema.org/Predicate/Texts/has_file_size.internal.md|has_file_size.internal]] 
- [[../../../../_protect/schema.org/Predicate/Texts/has_file_size.protect.md|has_file_size.protect]] 
- [[../../../../_private/schema.org/Predicate/Texts/has_file_size.private.md|has_file_size.private]] 
- [[../../../../_personal/schema.org/Predicate/Texts/has_file_size.personal.md|has_file_size.personal]] 
- [[../../../../_secret/schema.org/Predicate/Texts/has_file_size.secret.md|has_file_size.secret]] 
