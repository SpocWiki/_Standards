---
aliases:
  - direct-apply
  - direct_apply
  - directApply
  - is_direct_apply
  - is direct apply
confidential: public
cssclasses:
  - Predicate
  - Boolean
draft: false
expiryDate: ""
isDeleted: false
isReadOnly: false
keywords:
  - direct_apply
layout: ""
license: CC BY-SA 4.0
linkTitle: is_direct_apply
publish: true
publishDate: ""
tags:
  - class/Property
  - schema-org/Property/True
title: is_direct_apply
type: Pred_Bool
dv_is_:
  part_of: "pending:"
  same_as:
    - "[[is_direct_apply]]"
    - "[[/_public/schema-org/Property/Flags/is_direct_apply.public|is_direct_apply.public]]"
    - "[[/_internal/schema-org/Property/Flags/is_direct_apply.internal|is_direct_apply.internal]]"
    - "[[/_protect/schema-org/Property/Flags/is_direct_apply.protect|is_direct_apply.protect]]"
    - "[[/_private/schema-org/Property/Flags/is_direct_apply.private|is_direct_apply.private]]"
    - "[[/_personal/schema-org/Property/Flags/is_direct_apply.personal|is_direct_apply.personal]]"
    - "[[/_secret/schema-org/Property/Flags/is_direct_apply.secret|is_direct_apply.secret]]"
dv_is_direct_apply: Boolean
dv_has_:
  domain: JobPosting
  name: is_direct_apply
  range: Boolean
dv_is_part_of: "pending:"
dv_has_domain: JobPosting
dv_has_name: is_direct_apply
dv_has_range: Boolean
dv_is_same_as:
  - "[[is_direct_apply]]"
  - "[[/_public/schema-org/Property/Flags/is_direct_apply.public|is_direct_apply.public]]"
  - "[[/_internal/schema-org/Property/Flags/is_direct_apply.internal|is_direct_apply.internal]]"
  - "[[/_protect/schema-org/Property/Flags/is_direct_apply.protect|is_direct_apply.protect]]"
  - "[[/_private/schema-org/Property/Flags/is_direct_apply.private|is_direct_apply.private]]"
  - "[[/_personal/schema-org/Property/Flags/is_direct_apply.personal|is_direct_apply.personal]]"
  - "[[/_secret/schema-org/Property/Flags/is_direct_apply.secret|is_direct_apply.secret]]"
---

is_part_of = `=this.dv_is_part_of` 

Use these simple Tags to mark Instances as True or False: 
#is_/direct_apply 
#is_not_/direct_apply 

Or write it as a Triple: 
is_direct_apply = `=this.dv_is_direct_apply`  

Indicates whether an [[../../Class/is_a_/data_type/text/URL|URL]] that is associated with a [[JobPosting]] enables direct application for the job, via the posting website. A job posting is considered to have directApply of [[True]] if an application process for the specified job can be directly initiated via the url(s) given (noting that e.g. multiple internet domains might nevertheless be involved at an implementation level). A value of [[False]] is appropriate if there is no clear path to applying directly online for the specified job, navigating directly from the JobPosting url(s) supplied.

Predicate describes that: 

has_domain = `=this.dv_has_domain` 

has_name = `=this.dv_has_name` 
has_range = `=this.dv_has_range` 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/schema-org/Property/Flags/is_direct_apply|is_direct_apply]] 

### #is_/same_as :: [[/_public/schema-org/Property/Flags/is_direct_apply.public|is_direct_apply.public]] 

### #is_/same_as :: [[/_internal/schema-org/Property/Flags/is_direct_apply.internal|is_direct_apply.internal]] 

### #is_/same_as :: [[/_protect/schema-org/Property/Flags/is_direct_apply.protect|is_direct_apply.protect]] 

### #is_/same_as :: [[/_private/schema-org/Property/Flags/is_direct_apply.private|is_direct_apply.private]] 

### #is_/same_as :: [[/_personal/schema-org/Property/Flags/is_direct_apply.personal|is_direct_apply.personal]] 

### #is_/same_as :: [[/_secret/schema-org/Property/Flags/is_direct_apply.secret|is_direct_apply.secret]] 

