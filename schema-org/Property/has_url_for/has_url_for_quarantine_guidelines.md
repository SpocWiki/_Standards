---
aliases:
  - quarantine-guidelines
  - quarantine_guidelines
  - quarantineGuidelines
  - has_url_for_quarantine_guidelines
  - has url for quarantine guidelines
confidential: public
cssclasses:
  - Predicate
  - Url
draft: false
expiryDate: ""
isDeleted: false
isReadOnly: false
keywords:
  - quarantine_guidelines
layout: ""
license: CC BY-SA 4.0
linkTitle: has_url_for_quarantine_guidelines
publish: true
publishDate: ""
tags:
  - class/Property
  - schema-org/Property/Url
title: has_url_for_quarantine_guidelines
type: Predi_Url
dv_has_:
  url_for_:
    quarantine_guidelines: URL, WebContent
  domain: SpecialAnnouncement
dv_has_url_for_quarantine_guidelines: URL, WebContent
dv_is_:
  part_of: "pending:"
  same_as:
    - "[[/_Standards/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines|has_url_for_quarantine_guidelines]]"
    - "[[/_public/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.public|has_url_for_quarantine_guidelines.public]]"
    - "[[/_internal/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.internal|has_url_for_quarantine_guidelines.internal]]"
    - "[[/_protect/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.protect|has_url_for_quarantine_guidelines.protect]]"
    - "[[/_private/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.private|has_url_for_quarantine_guidelines.private]]"
    - "[[/_personal/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.personal|has_url_for_quarantine_guidelines.personal]]"
    - "[[/_secret/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.secret|has_url_for_quarantine_guidelines.secret]]"
dv_#has_:
  name: has_url_for_quarantine_guidelines
  range: URL, WebContent
dv_is_part_of: "pending:"
dv_has_domain: SpecialAnnouncement
dv_has_name: has_url_for_quarantine_guidelines
dv_has_range: URL, WebContent
dv_is_same_as:
  - "[[/_Standards/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines|has_url_for_quarantine_guidelines]]"
  - "[[/_public/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.public|has_url_for_quarantine_guidelines.public]]"
  - "[[/_internal/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.internal|has_url_for_quarantine_guidelines.internal]]"
  - "[[/_protect/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.protect|has_url_for_quarantine_guidelines.protect]]"
  - "[[/_private/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.private|has_url_for_quarantine_guidelines.private]]"
  - "[[/_personal/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.personal|has_url_for_quarantine_guidelines.personal]]"
  - "[[/_secret/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.secret|has_url_for_quarantine_guidelines.secret]]"
---

Predicate to specify the Url of SpecialAnnouncement.

is_part_of = `=this.dv_is_part_of` 

Use it like this: 
- has_url_for_quarantine_guidelines = `=this.dv_has_url_for_quarantine_guidelines`  
- has_url_for_quarantine_guidelines = `=this.dv_has_url_for_quarantine_guidelines`  

Guidelines about quarantine rules, e.g. in the context of a pandemic.

Predicate describes that: 

has_domain = `=this.dv_has_domain` 

has_name = `=this.dv_has_name` 
has_range = `=this.dv_has_range` 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines|has_url_for_quarantine_guidelines]] 

### #is_/same_as :: [[/_public/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.public|has_url_for_quarantine_guidelines.public]] 

### #is_/same_as :: [[/_internal/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.internal|has_url_for_quarantine_guidelines.internal]] 

### #is_/same_as :: [[/_protect/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.protect|has_url_for_quarantine_guidelines.protect]] 

### #is_/same_as :: [[/_private/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.private|has_url_for_quarantine_guidelines.private]] 

### #is_/same_as :: [[/_personal/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.personal|has_url_for_quarantine_guidelines.personal]] 

### #is_/same_as :: [[/_secret/schema-org/Property/has_url_for/has_url_for_quarantine_guidelines.secret|has_url_for_quarantine_guidelines.secret]] 

