---
aliases:
- Arag
- Asekkud
- bînîn
- golwg
- gweled
- görsel algı
- hwɛ
- ikusmen
- Koʻrish
- lukin
- látás
- nägemine
- näköaisti
- panhiling
- paningin
- penglihatan
- percepção visual
- persesion viziva
- radharc
- redze
- rega
- Rikuy
- shikimi
- sicht
- sigvermoë
- Sihþ
- sjón
- syn
- synssans
- synuppfattning
- Sī-kak
- thị giác
- têgihîştina bînînê
- vid
- Vidala percepto
- vidkapablo
- vision, sight, or eyesight
- visión
- vista
- Visual Perception
- Visual_Perception
- visuelle Wahrnehmung
- visus
- vizualno zaznavavanje
- vue
- văz
- wzrok
- zien
- zrak
- čulo vida
- Όραση
- босира
- вид
- зір
- зрение
- зрок
- күрү
- Көру мүшесі
- Көрүү сезими
- чуло вида
- տեսողություն
- זען
- ראייה
- البصر
- بصری آگہی
- بینایی
- حاسة البصر
- لید
- نظر
- दृश्य बोध
- दृष्टी
- দৃষ্টিগত প্রত্যক্ষণ
- ਨਜ਼ਰ
- చూపు
- ದೃಶ್ಯ ಗ್ರಹಿಕೆ
- കാഴ്ച
- การรับรู้ทางการเห็น
- 視知覺
- 視覚
- 視覺
- 视知觉
- 视觉
- 시각
has_id_wikidata: Q162668
subclass_of:
- '[[_Standards/WikiData/WD~perception,160402]]'
- '[[_Standards/WikiData/WD~sensory_perception_of_light_stimulus,14859561]]'
different_from:
- '[[_Standards/WikiData/WD~visual_system,558363]]'
- '[[_Standards/WikiData/WD~Zien,23091183]]'
- '[[_Standards/WikiData/WD~Vista,31257060]]'
described_by_source:
- '[[../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Dictionnaire_Infernal,1210353]]'
- "[[_Standards/WikiData/WD~The_New_Student's_Reference_Work,16082057]]"
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations_Lexikon,_4th_edition_(1885_1890),19219752]]'
said_to_be_the_same_as: '[[_Standards/WikiData/WD~Q760329,760329]]'
part_of:
- '[[_Standards/WikiData/WD~five_wits,1051623]]'
- '[[_Standards/WikiData/WD~psychology_terminology,77468620]]'
instance_of: '[[_Standards/WikiData/WD~biological_process,2996394]]'
PhilPapers_topic: vision
image: http://commons.wikimedia.org/wiki/Special:FilePath/Eye%20iris.jpg
Iconclass_notation: 31A31
Dewey_Decimal_Classification:
- 612.84
- 152.14
- 770.1
exact_match: http://purl.obolibrary.org/obo/GO_0007601
IEV_number: 845-22-082
UMLS_CUI: C0042830
MeSH_tree_code: F02.463.593.932
Colon_Classification: S:25
Commons_category: Visual perception
dv_has_:
  name_:
    af: sigvermoë
    ak: hwɛ
    ang: Sihþ
    ar: حاسة البصر
    arz: البصر
    ast: visión
    bcl: panhiling
    be: зрок
    be_tarask: зрок
    bg: Зрение
    bn: দৃষ্টিগত প্রত্যক্ষণ
    br: gweled
    bs: vid
    ca: vista
    ckb: بینایی
    cs: zrak
    cy: golwg
    da: synssans
    de: visuelle Wahrnehmung
    de_ch: Visuelle Wahrnehmung
    el: Όραση
    en: visual perception
    en_ca: visual perception
    en_gb: visual perception
    eo: vidkapablo
    es: visión
    et: nägemine
    eu: ikusmen
    fa: بینایی
    fi: näköaisti
    fr: vue
    ga: radharc
    gl: visión
    he: ראייה
    hi: दृश्य बोध
    hr: vid
    hu: látás
    hy: տեսողություն
    ia: vista
    id: penglihatan
    io: Vidala percepto
    is: sjón
    it: vista
    ja: 視覚
    kab: Asekkud
    kk: Көру мүшесі
    kn: ದೃಶ್ಯ ಗ್ರಹಿಕೆ
    ko: 시각
    ku: têgihîştina bînînê
    ku_latn: bînîn
    ky: Көрүү сезими
    la: visus
    lt: rega
    lv: redze
    mk: вид
    ml: കാഴ്ച
    mr: दृष्टी
    ms: penglihatan
    nan: Sī-kak
    nb: syn
    nl: zien
    nn: synssans
    oc: vista
    pa: ਨਜ਼ਰ
    pl: wzrok
    pnb: نظر
    ps: لید
    pt: percepção visual
    pt_br: percepção visual
    qu: Rikuy
    ro: văz
    ru: зрение
    sco: sicht
    sd: نظر
    sh: vid
    sk: zrak
    sl: vizualno zaznavavanje
    so: Arag
    sq: shikimi
    sr: чуло вида
    sr_ec: чуло вида
    sr_el: čulo vida
    sv: synuppfattning
    te: చూపు
    tg: босира
    th: การรับรู้ทางการเห็น
    tl: paningin
    tok: lukin
    tr: görsel algı
    tt: күрү
    tt_cyrl: күрү
    uk: зір
    ur: بصری آگہی
    uz: Koʻrish
    vec: persesion viziva
    vi: thị giác
    vro: Nägemine
    wuu: 视知觉
    yi: זען
    yue: 視覺
    zh: 視覺
    zh_cn: 视觉
    zh_hans: 视知觉
    zh_hant: 視覺
    zh_hk: 視知覺
    zh_sg: 视知觉
    zh_tw: 視知覺
---

# [[Visual_Perception]] 

#is_/same_as :: [[../../../../WikiData/WD~Visual_perception,162668|WD~Visual_perception,162668]]  

## #has_/text_of_/abstract 

> Visual perception is the ability to detect light 
> and use it to form an image of the surrounding environment. 
> 
> Photodetection without image formation is classified as [[Light_Sensing]]. 
> 
> In most vertebrates, visual perception can be enabled by 
> - photopic vision (daytime vision) or 
> - scotopic vision (night vision), 
> with most vertebrates having both. 
> 
> Visual perception detects light (photons) in the visible spectrum 
> reflected by objects in the environment or emitted by light sources. 
> 
> The visible range of light is defined by what is readily perceptible to humans, 
> though the visual perception of non-humans often extends beyond the visual spectrum. 
> 
> The resulting perception is also known as vision, sight, or eyesight 
> (adjectives visual, optical, and ocular, respectively). 
> 
> The various physiological components involved in vision are referred to 
> collectively as the visual system, and are the focus of much research in 
> linguistics, psychology, cognitive science, neuroscience, and molecular biology, 
> collectively referred to as vision science.
>
> Visual perception involves not only what we see 
> but also how our brain process information.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Visual%20perception) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Psychology/Consciousness/Perception/Visual_Perception|Visual_Perception]] 

### #is_/same_as :: [[/_public/bio/Psychology/Consciousness/Perception/Visual_Perception.public|Visual_Perception.public]] 

### #is_/same_as :: [[/_internal/bio/Psychology/Consciousness/Perception/Visual_Perception.internal|Visual_Perception.internal]] 

### #is_/same_as :: [[/_protect/bio/Psychology/Consciousness/Perception/Visual_Perception.protect|Visual_Perception.protect]] 

### #is_/same_as :: [[/_private/bio/Psychology/Consciousness/Perception/Visual_Perception.private|Visual_Perception.private]] 

### #is_/same_as :: [[/_personal/bio/Psychology/Consciousness/Perception/Visual_Perception.personal|Visual_Perception.personal]] 

### #is_/same_as :: [[/_secret/bio/Psychology/Consciousness/Perception/Visual_Perception.secret|Visual_Perception.secret]] 

