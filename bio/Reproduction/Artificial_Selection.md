---
aliases:
- Akllaspa mirachiy
- Aretus
- Artificial Selection
- Artificial_Selection
- Artipisyal na pagpili
- avel
- avl
- Chatishtirish
- Chọn lọc nhân tạo
- Hautespen artifizial
- Io̍k-chéng
- jalostus
- kunstig seleksjon
- kunstig selektion
- kunstmatige selectie
- Kynbætur
- Mbarështimi selektiv
- mesterséges szelekció
- pembiakbakaan terpilih
- selecció artificial
- selección artificial
- Selective breeding
- Seleición artificial
- selekcija
- Selekcïya ğılımı
- Seleksi buatan
- Seleksyon atifisyèl
- selekta bredado
- selezione artificiale
- seleção artificial
- sełesion artifisałe
- sztuczna selekcja
- Sèlèksi gawéan
- sélection artificielle
- Süni seçmə
- umetni izbor
- Umjetna selekcija
- umělý výběr
- Veivakasucumi digitaki
- Veštačka selekcija
- Yapay seçilim
- Zucht
- Τεχνητή επιλογή
- вештачка селекција
- Зориуд селекц
- Изкуствен отбор
- селекція
- селекция
- Селекция ғылымы
- сэлекцыя
- штучны адбор
- Արհեստական ընտրություն
- ברירה מלאכותית
- اصطفاء اصطناعي
- انتخاب مصنوعی
- زاوزێ پێ کردنی دەستکرد
- سەلەكتسىييا عىلىمى
- مصنوعي انتخاب
- छनौट प्रजनन
- वरणात्मक प्रजनन
- কৃত্রিম নির্বাচন
- ਚੋਣਵੀਂ ਨਸਲਕਸ਼ੀ
- தேர்ந்தெடுத்த இனப்பெருக்கம்
- การคัดเลือกพันธุ์
- ხელოვნური გადარჩევა
- 人工选择
- 人工選擇
- 人為選択
- 품종개량
has_id_wikidata: Q995745
subclass_of:
- '[[_Standards/WikiData/WD~intentional_human_activity,451967]]'
- '[[_Standards/WikiData/WD~biological_selection,1094654]]'
different_from:
- '[[_Standards/WikiData/WD~biological_selection,1094654]]'
- '[[_Standards/WikiData/WD~selection,12888920]]'
said_to_be_the_same_as:
- '[[_Standards/WikiData/WD~selective_breeding,4023882]]'
- '[[_Standards/WikiData/WD~Livestock_Rearing,9379111]]'
described_by_source: '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
opposite_of: '[[_Standards/WikiData/WD~natural_selection,43478]]'
OmegaWiki_Defined_Meaning: 808800
Commons_category: Breeding
image: http://commons.wikimedia.org/wiki/Special:FilePath/Cornselection.jpg
Krugosvet_article_archived_: biologiya/selektsiya
dv_has_:
  name_:
    an: Selección artificial
    ar: اصطفاء اصطناعي
    ast: Seleición artificial
    az: Süni seçmə
    ba: селекция
    be: штучны адбор
    be_tarask: сэлекцыя
    bg: Изкуствен отбор
    bn: কৃত্রিম নির্বাচন
    ca: selecció artificial
    ckb: زاوزێ پێ کردنی دەستکرد
    cs: umělý výběr
    da: kunstig selektion
    de: Zucht
    el: Τεχνητή επιλογή
    en: artificial selection
    en_ca: Artificial selection
    en_gb: artificial selection
    eo: selekta bredado
    es: selección artificial
    et: Aretus
    eu: Hautespen artifizial
    fa: انتخاب مصنوعی
    fi: jalostus
    fj: Veivakasucumi digitaki
    fr: sélection artificielle
    gl: selección artificial
    he: ברירה מלאכותית
    hi: वरणात्मक प्रजनन
    hr: Umjetna selekcija
    ht: Seleksyon atifisyèl
    hu: mesterséges szelekció
    hy: Արհեստական ընտրություն
    id: Seleksi buatan
    is: Kynbætur
    it: selezione artificiale
    ja: 人為選択
    jv: Sèlèksi gawéan
    ka: ხელოვნური გადარჩევა
    kk: Селекция ғылымы
    kk_arab: سەلەكتسىييا عىلىمى
    kk_cyrl: Селекция ғылымы
    kk_latn: Selekcïya ğılımı
    ko: 품종개량
    ky: Селекция
    lv: selekcija
    mk: вештачка селекција
    mn: Зориуд селекц
    ms: pembiakbakaan terpilih
    nan: Io̍k-chéng
    nb: avl
    ne: छनौट प्रजनन
    nl: kunstmatige selectie
    nn: kunstig seleksjon
    pa: ਚੋਣਵੀਂ ਨਸਲਕਸ਼ੀ
    pl: sztuczna selekcja
    pt: seleção artificial
    pt_br: seleção artificial
    qu: Akllaspa mirachiy
    ru: селекция
    sd: مصنوعي انتخاب
    sl: umetni izbor
    sq: Mbarështimi selektiv
    sr: Veštačka selekcija
    sv: avel
    ta: தேர்ந்தெடுத்த இனப்பெருக்கம்
    th: การคัดเลือกพันธุ์
    tl: Artipisyal na pagpili
    tr: Yapay seçilim
    tt: селекция
    uk: селекція
    uz: Chatishtirish
    vec: sełesion artifisałe
    vi: Chọn lọc nhân tạo
    wuu: 人工选择
    yue: 人工選擇
    zh: 人工選擇
    zh_hans: 人工选择
    zh_hant: 人工選擇
    zh_hk: 人工選擇
---
# [[Artificial_Selection]] 

#is_/same_as :: [[../../WikiData/WD~Artificial_selection,995745|WD~Artificial_selection,995745]] 

## #has_/text_of_/abstract 

> Selective breeding (also called artificial selection) is the process by which humans use animal breeding and plant breeding to selectively develop particular phenotypic traits (characteristics) by choosing which typically animal or plant males and females will sexually reproduce and have offspring together. Domesticated animals are known as breeds, normally bred by a professional breeder, while domesticated plants are known as varieties, cultigens, cultivars, or breeds. Two purebred animals of different breeds produce a crossbreed, and crossbred plants are called hybrids. Flowers, vegetables and fruit-trees may be bred by amateurs and commercial or non-commercial professionals: major crops are usually the provenance of the professionals.
>
> In animal breeding artificial selection is often combined with techniques such as inbreeding, linebreeding, and outcrossing. In plant breeding, similar methods are used. Charles Darwin discussed how selective breeding had been successful in producing change over time in his 1859 book, On the Origin of Species. Its first chapter discusses selective breeding and domestication of such animals as pigeons, cats, cattle, and dogs. Darwin used artificial selection as an analogy to propose and explain the theory of natural selection but distinguished the latter from the former as a separate process that is non-directed.
>
> The deliberate exploitation of selective breeding to produce desired results has become very common in agriculture and experimental biology.
>
> Selective breeding can be unintentional, for example, resulting from the process of human cultivation; and it may also produce unintended – desirable or undesirable – results. For example, in some grains, an increase in seed size may have resulted from certain ploughing practices rather than from the intentional selection of larger seeds. Most likely, there has been an interdependence between natural and artificial factors that have resulted in plant domestication.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Selective%20breeding) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Reproduction/Artificial_Selection|Artificial_Selection]] 

### #is_/same_as :: [[/_public/bio/Reproduction/Artificial_Selection.public|Artificial_Selection.public]] 

### #is_/same_as :: [[/_internal/bio/Reproduction/Artificial_Selection.internal|Artificial_Selection.internal]] 

### #is_/same_as :: [[/_protect/bio/Reproduction/Artificial_Selection.protect|Artificial_Selection.protect]] 

### #is_/same_as :: [[/_private/bio/Reproduction/Artificial_Selection.private|Artificial_Selection.private]] 

### #is_/same_as :: [[/_personal/bio/Reproduction/Artificial_Selection.personal|Artificial_Selection.personal]] 

### #is_/same_as :: [[/_secret/bio/Reproduction/Artificial_Selection.secret|Artificial_Selection.secret]] 

