---
aliases:
- imoibriú slabhrúil polaiméaráise
- PCR
- PCR (生物學)
- PCR(Polymerase Chain Reaction)
- PCR(Polymerase_Chain_Reaction)
- PGR
- Polimeraase-käätereaksjoon
- Polimerasaren kate-erreakzioa
- Polimerase kettingreaksiemetode
- Polimeraz zincir tepkimesi
- Polimeraz zəncir reaksiyası
- Polimeraza zanjiri reaktsiyasi
- polimeraza ĉen-reakcio
- Polimerazna lančana reakcija
- polimeráz-láncreakció
- polimerāzes ķēdes reakcija
- polymeraasiketjureaktio
- polymerase chain reaction
- Polymerase-Kettenreaktion
- polymerasekettingreactie
- polymerasekjedereaksjon
- polymeraskedjereaktion
- polymerázová reťazová reakcia
- polymerázová řetězová reakce
- Polümeraasi ahelreaktsioon
- Reaccion en cadena per la polimerasa
- reacció en cadena de la polimerasa
- reacción en cadea da polimerase
- Reacción en cadena de la polimerasa
- reacție de polimerizare în lanț
- Reakcja łańcuchowa polimerazy
- Reaksi berantai polimerase
- reazione a catena della polimerasi
- reação em cadeia da polimerase
- réaction en chaîne par polymérase
- Tasedmirt tanmezrit n Polimiraz
- tindak balas rantai polimerase
- To-thé-mûi liân-só hoán-èng
- verižna reakcija s polimerazo
- Αλυσιδωτή αντίδραση πολυμεράσης
- палімеразная ланцуговая рэакцыя
- палімэразная ланцуговая рэакцыя
- полімеразна ланцюгова реакція
- Полимеразды тізбекті реакция
- полимеразна верижна реакција
- Полимеразна верижна реакция
- Полимеразна ланчана реакција
- полимеразная цепная реакция
- Полимеразын гинжин урвал
- Պոլիմերազային շղթայական ռեակցիա
- تفاعل البوليميراز المتسلسل
- د پولیمیرېز ځنځیري تعامل
- مکـثـورخامری زنجیری تعامل
- واکنش زنجیرهای پلیمراز
- पॉलिमरेज़ चेन प्रतिक्रिया
- পলিমারেজ শৃঙ্খল বিক্রিয়া
- பாலிமரேசு தொடர் வினை
- వేదిక:వర్తమాన ఘటనలు
- പോളിമെറേയ്സ് ചെയിൻ റിയാക്ഷൻ
- පොලිමරේස් දාම ප්රතික්රියාව
- ปฏิกิริยาลูกโซ่พอลิเมอเรส
- პოლიმერაზული ჯაჭვური რეაქცია
- ポリメラーゼ連鎖反応
- 聚合酶鏈式反應
- 聚合酶链式反应
- 중합효소 연쇄 반응
has_id_wikidata: Q176996
part_of: '[[_Standards/WikiData/WD~medical_diagnosis,177719|WD~medical_diagnosis,177719]]'
subclass_of:
- '[[_Standards/WikiData/WD~nucleic_acid_test,348667|WD~nucleic_acid_test,348667]]'
- '[[_Standards/WikiData/WD~DNA_amplification,105364650|WD~DNA_amplification,105364650]]'
- '[[_Standards/WikiData/WD~RNA_amplification,106311245|WD~RNA_amplification,106311245]]'
instance_of: '[[_Standards/WikiData/WD~periodic_health_examination,1762591|WD~periodic_health_examination,1762591]]'
history_of_topic: '[[_Standards/WikiData/WD~history_of_polymerase_chain_reaction,5868383|WD~history_of_polymerase_chain_reaction,5868383]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_PCR,14401688|WD~Template_PCR,14401688]]'
short_name:
- PCR
- ПЛР
image:
- http://commons.wikimedia.org/wiki/Special:FilePath/Polymerase%20chain%20reaction-en.svg
- http://commons.wikimedia.org/wiki/Special:FilePath/Thermal%20cycler%20for%20PCR.jpg
MeSH_tree_code: E05.393.620.500
Krugosvet_article_archived_: nauka_i_tehnika/himiya/POLIMERAZNAYA_TSEPNAYA_REAKTSIYA.html
Commons_gallery: Polymerase chain reaction
Commons_category: Polymerase chain reaction
dv_has_:
  name_:
    af: Polimerase kettingreaksiemetode
    an: PCR
    ar: تفاعل البوليميراز المتسلسل
    ast: Reacción en cadena de la polimerasa
    az: Polimeraz zəncir reaksiyası
    be: палімеразная ланцуговая рэакцыя
    be_tarask: палімэразная ланцуговая рэакцыя
    bg: Полимеразна верижна реакция
    bn: পলিমারেজ শৃঙ্খল বিক্রিয়া
    bs: Polimerazna lančana reakcija
    ca: reacció en cadena de la polimerasa
    cs: polymerázová řetězová reakce
    da: PCR
    de: Polymerase-Kettenreaktion
    de_ch: Polymerase-Kettenreaktion
    el: Αλυσιδωτή αντίδραση πολυμεράσης
    en: polymerase chain reaction
    en_ca: Polymerase chain reaction
    en_gb: polymerase chain reaction
    eo: polimeraza ĉen-reakcio
    es: reacción en cadena de la polimerasa
    et: Polümeraasi ahelreaktsioon
    eu: Polimerasaren kate-erreakzioa
    fa: واکنش زنجیرهای پلیمراز
    fi: polymeraasiketjureaktio
    fr: réaction en chaîne par polymérase
    frr: Polimeraase-käätereaksjoon
    ga: imoibriú slabhrúil polaiméaráise
    gl: reacción en cadea da polimerase
    he: PCR
    hi: पॉलिमरेज़ चेन प्रतिक्रिया
    hr: PCR
    hu: polimeráz-láncreakció
    hy: Պոլիմերազային շղթայական ռեակցիա
    id: Reaksi berantai polimerase
    is: PCR
    it: reazione a catena della polimerasi
    ja: ポリメラーゼ連鎖反応
    ka: პოლიმერაზული ჯაჭვური რეაქცია
    kab: Tasedmirt tanmezrit n Polimiraz
    kk: Полимеразды тізбекті реакция
    ko: 중합효소 연쇄 반응
    lt: PGR
    lv: polimerāzes ķēdes reakcija
    mk: полимеразна верижна реакција
    ml: പോളിമെറേയ്സ് ചെയിൻ റിയാക്ഷൻ
    mn: Полимеразын гинжин урвал
    ms: tindak balas rantai polimerase
    nan: To-thé-mûi liân-só hoán-èng
    nb: PCR
    nl: polymerasekettingreactie
    nn: polymerasekjedereaksjon
    oc: Reaccion en cadena per la polimerasa
    pl: Reakcja łańcuchowa polimerazy
    ps: د پولیمیرېز ځنځیري تعامل
    pt: reação em cadeia da polimerase
    pt_br: Reação em cadeia da polimerase
    ro: reacție de polimerizare în lanț
    ru: полимеразная цепная реакция
    sh: Polimerazna lančana reakcija
    si: පොලිමරේස් දාම ප්රතික්රියාව
    sk: polymerázová reťazová reakcia
    sl: verižna reakcija s polimerazo
    sq: PCR
    sr: Полимеразна ланчана реакција
    sr_ec: Полимеразна ланчана реакција
    sr_el: Polimerazna lančana reakcija
    sv: polymeraskedjereaktion
    ta: பாலிமரேசு தொடர் வினை
    te: వేదిక:వర్తమాన ఘటనలు
    th: ปฏิกิริยาลูกโซ่พอลิเมอเรส
    tr: Polimeraz zincir tepkimesi
    uk: полімеразна ланцюгова реакція
    ur: مکـثـورخامری زنجیری تعامل
    uz: Polimeraza zanjiri reaktsiyasi
    vi: PCR
    wuu: 聚合酶链式反应
    yue: PCR (生物學)
    zh: 聚合酶链式反应
    zh_cn: 聚合酶链式反应
    zh_hans: 聚合酶链式反应
    zh_hant: 聚合酶鏈式反應
    zh_hk: 聚合酶鏈式反應
    zh_sg: 聚合酶链式反应
    zh_tw: 聚合酶鏈式反應
---

# [[PCR(Polymerase_Chain_Reaction)]] 

#is_/same_as :: [[_Standards/WikiData/WD~Polymerase_chain_reaction,176996|WD~Polymerase_chain_reaction,176996]] 

## #has_/text_of_/abstract 

> The polymerase chain reaction (PCR) is a  laboratory method widely used 
> to amplify copies of  specific DNA sequences rapidly, to enable detailed study. 
> 
> PCR was invented in 1983 by American biochemist Kary Mullis at Cetus Corporation. 
> Mullis and biochemist Michael Smith, 
> who had developed other essential ways of manipulating DNA, 
> were jointly awarded the Nobel Prize in Chemistry in 1993.
>
> PCR is fundamental to many of the procedures used in genetic testing, research, including analysis of ancient samples of DNA and identification of infectious agents. Using PCR, copies of very small amounts of DNA sequences are exponentially amplified in a series of cycles of temperature changes. PCR is now a common and often indispensable technique used in medical laboratory research for a broad variety of applications including biomedical research and forensic science.
>
> The majority of PCR methods rely on thermal cycling. Thermal cycling exposes reagents to repeated cycles of heating and cooling to permit different temperature-dependent reactions—specifically, DNA melting and enzyme-driven DNA replication. PCR employs two main reagents—primers (which are short single strand DNA fragments known as oligonucleotides that are a complementary sequence to the target DNA region) and a thermostable DNA polymerase. In the first step of PCR, the two strands of the DNA double helix are physically separated at a high temperature in a process called nucleic acid denaturation. In the second step, the temperature is lowered and the primers bind to the complementary sequences of DNA. The two DNA strands then become templates for DNA polymerase to enzymatically assemble a new DNA strand from free nucleotides, the building blocks of DNA. As PCR progresses, the DNA generated is itself used as a template for replication, setting in motion a chain reaction in which the original DNA template is exponentially amplified.
>
> Almost all PCR applications employ a heat-stable DNA polymerase, such as Taq polymerase, an enzyme originally isolated from the thermophilic bacterium Thermus aquaticus. If the polymerase used was heat-susceptible, it would denature under the high temperatures of the denaturation step. Before the use of Taq polymerase, DNA polymerase had to be manually added every cycle, which was a tedious and costly process.
>
> Applications of the technique include DNA cloning for sequencing, gene cloning and manipulation, gene mutagenesis; construction of DNA-based phylogenies, or functional analysis of genes; diagnosis and monitoring of genetic disorders; amplification of ancient DNA; analysis of genetic fingerprints for DNA profiling (for example, in forensic science and parentage testing); and detection of pathogens in nucleic acid tests for the diagnosis of infectious diseases.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Polymerase%20chain%20reaction) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Genetics/Nucleic_Acid/PCR(Polymerase_Chain_Reaction)|PCR(Polymerase_Chain_Reaction)]] 

### #is_/same_as :: [[/_public/bio/Genetics/Nucleic_Acid/PCR(Polymerase_Chain_Reaction).public|PCR(Polymerase_Chain_Reaction).public]] 

### #is_/same_as :: [[/_internal/bio/Genetics/Nucleic_Acid/PCR(Polymerase_Chain_Reaction).internal|PCR(Polymerase_Chain_Reaction).internal]] 

### #is_/same_as :: [[/_protect/bio/Genetics/Nucleic_Acid/PCR(Polymerase_Chain_Reaction).protect|PCR(Polymerase_Chain_Reaction).protect]] 

### #is_/same_as :: [[/_private/bio/Genetics/Nucleic_Acid/PCR(Polymerase_Chain_Reaction).private|PCR(Polymerase_Chain_Reaction).private]] 

### #is_/same_as :: [[/_personal/bio/Genetics/Nucleic_Acid/PCR(Polymerase_Chain_Reaction).personal|PCR(Polymerase_Chain_Reaction).personal]] 

### #is_/same_as :: [[/_secret/bio/Genetics/Nucleic_Acid/PCR(Polymerase_Chain_Reaction).secret|PCR(Polymerase_Chain_Reaction).secret]] 

