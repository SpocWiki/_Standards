---
aliases:
- Alamorana
- arvemecci
- arvevuovdi
- bosque lluvioso
- Bırrê varıti
- Dajin ruwan sama
- Dažďový les
- deštný les
- deževni gozd
- Driegnuosės medės
- Drėgnasis miškas
- esőerdő
- Fforest law
- floresta húmida
- foraoisí báistí
- Foresta pioval
- Foresta pluvial
- foresta pluviale
- forêt humide
- Guddefi chiuviali
- Hodíłchʼil Níłtsánígíí
- Hutan hujan
- Ihlathi Lemvula
- Kišna šuma
- Koadeier gleb
- Koswik law
- lasy deszczowe
- Lietus mežs
- Maulang gubat
- Mauran nga kagobaán
- Msitu wa mvua
- Muntaña tuqi suyu
- Oihan
- Paray sach'a-sach'a
- pluvarbaro
- Pluvoforesto
- pădure pluvială
- Raengerwaad
- Rainforest
- Regenwald
- regenwold
- Regenwoold
- regenwoud
- regnskog
- regnskov
- Regnskógur
- Reinwâld
- reënwoud
- rássjovuovdde
- Rütubətli ekvatorial meşələr
- rừng mưa
- sademetsä
- Selva
- Selva pluvial
- Selva pluviala
- Shara Tropikal
- Silva
- Silva pluvialis
- sååferge
- Utan ujan
- vihmamets
- Vosche chiovose
- Wana sabeh
- yağmur ormanı
- âʹbrrmeäʹcc
- Ú-lîm
- Ýagyş tokaýy
- Δάσος της βροχής
- Ардахтаах тыа
- дажджавы лес
- Доджовый лїс
- дождевой лес
- Дождовна шума
- Дощовий ліс
- Дъждовна гора
- кишна шума
- Сельва
- Ширэнгэн ой
- խոնավ անտառներ
- יער גשם
- רעגנוואלד
- برساتی جنگل
- جنگل بارانی
- دارستانی بارانی
- غابة مطيرة
- غابه مطيره
- ورښتي ځنګل
- یاغیش مئشهسی
- बरखाबन
- वर्षाबन
- वर्षावन
- অতিবৃষ্টি অরণ্য
- বৰ্ষাৰণ্য
- ਬਰਸਾਤੀ ਜੰਗਲ
- மழைக்காடு
- ಮಳೆಕಾಡು
- മഴക്കാട്
- වැසි වනාන්තර
- ป่าดิบชื้น
- မိုးသစ်တော
- წვიმის ტყეები
- 多雨林
- 雨林
- 우림
has_id_wikidata: Q9444
subclass_of: '[[_Standards/WikiData/WD~moist_forest,93734705|WD~moist_forest,93734705]]'
OmegaWiki_Defined_Meaning: 3101
U_S_National_Archives_Identifier: 10675925
Commons_category: Rainforests
image: http://commons.wikimedia.org/wiki/Special:FilePath/Taman-Negara.jpg
MeSH_tree_code:
- G16.500.275.157.437.500
- N06.230.124.343.500
subreddit:
- oddbobs
- Rainforest
IMDb_keyword: rainforest
Commons_gallery: Rainforest
dv_has_:
  name_:
    af: reënwoud
    an: Selva pluvial
    ar: غابة مطيرة
    arz: غابه مطيره
    as: বৰ্ষাৰণ্য
    ast: Selva
    ay: Muntaña tuqi suyu
    az: Rütubətli ekvatorial meşələr
    azb: یاغیش مئشهسی
    ba: Сельва
    ban: Wana sabeh
    be: Сельва
    be_tarask: дажджавы лес
    bg: Дъждовна гора
    bho: बरखाबन
    bjn: Hutan hujan
    bn: অতিবৃষ্টি অরণ্য
    br: Koadeier gleb
    bs: Kišna šuma
    ca: selva pluvial
    ckb: دارستانی بارانی
    cs: deštný les
    cy: Fforest law
    da: regnskov
    de: Regenwald
    diq: Bırrê varıti
    el: Δάσος της βροχής
    en: rainforest
    eo: pluvarbaro
    es: bosque lluvioso
    et: vihmamets
    eu: Oihan
    fa: جنگل بارانی
    fi: sademetsä
    fo: Regnskógur
    fr: forêt humide
    fy: Reinwâld
    ga: foraoisí báistí
    gl: Selva
    gsw: Regenwald
    ha: Dajin ruwan sama
    he: יער גשם
    hi: वर्षावन
    hif: Rainforest
    hr: kišna šuma
    hu: esőerdő
    hy: խոնավ անտառներ
    ia: Silva
    id: hutan hujan
    io: Pluvoforesto
    is: Regnskógur
    it: foresta pluviale
    ja: 多雨林
    ka: წვიმის ტყეები
    kk: Сельва
    kn: ಮಳೆಕಾಡು
    ko: 우림
    krc: Сельва
    kw: Koswik law
    la: Silva pluvialis
    lad: Shara Tropikal
    li: Raengerwaad
    lt: Drėgnasis miškas
    lv: Lietus mežs
    lzh: 雨林
    mg: Alamorana
    min: Utan ujan
    mk: Дождовна шума
    ml: മഴക്കാട്
    mn: Ширэнгэн ой
    mr: वर्षावन
    ms: Hutan hujan
    my: မိုးသစ်တော
    nan: Ú-lîm
    nb: regnskog
    nds: Regenwoold
    nds_nl: regenwold
    ne: वर्षाबन
    nl: regenwoud
    nn: regnskog
    nv: Hodíłchʼil Níłtsánígíí
    oc: Selva pluviala
    pa: ਬਰਸਾਤੀ ਜੰਗਲ
    pl: lasy deszczowe
    pms: Foresta pioval
    pnb: برساتی جنگل
    ps: ورښتي ځنګل
    pt: floresta húmida
    qu: Paray sach'a-sach'a
    ro: pădure pluvială
    roa-tara: Vosche chiovose
    ru: дождевой лес
    rue: Доджовый лїс
    sah: Ардахтаах тыа
    scn: Guddefi chiuviali
    se: arvevuovdi
    sgs: Driegnuosės medės
    sh: Kišna šuma
    si: වැසි වනාන්තර
    sk: Dažďový les
    sl: deževni gozd
    sma: sååferge
    smj: rássjovuovdde
    smn: arvemecci
    sms: âʹbrrmeäʹcc
    so: Rainforest
    sr: кишна шума
    sv: regnskog
    sw: Msitu wa mvua
    ta: மழைக்காடு
    th: ป่าดิบชื้น
    tk: Ýagyş tokaýy
    tl: Maulang gubat
    tr: yağmur ormanı
    uk: Дощовий ліс
    ur: برساتی جنگل
    vec: Foresta pluvial
    vi: rừng mưa
    war: Mauran nga kagobaán
    wuu: 雨林
    yi: רעגנוואלד
    yue: 雨林
    zh: 雨林
    zh_hans: 雨林
    zh_hant: 雨林
    zu: Ihlathi Lemvula
---

# [[Rainforest]] 

#is_/same_as :: [[_Standards/WikiData/WD~Rainforest,9444|WD~Rainforest,9444]] 

## #has_/text_of_/abstract 

> **Rainforest**s are forests characterized by a closed and continuous tree canopy, 
> moisture-dependent vegetation, the presence of epiphytes and lianas 
> and the absence of wildfire. 
> 
> Rainforests can be generally classified as tropical rainforests or temperate rainforests, 
> but other types have been described.
>
> Estimates vary from 40% to 75% of all biotic species being indigenous to the rainforests. 
> There may be many millions of species of plants, insects and microorganisms 
> still undiscovered in tropical rainforests. 
> 
> Tropical rainforests have been called the "jewels of the Earth" 
> and the "world's largest pharmacy", 
> because over one quarter of natural medicines have been discovered there.
>
> Rainforests as well as endemic rainforest species are rapidly disappearing 
> due to deforestation, the resulting habitat loss and pollution of the atmosphere.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Rainforest) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Ecology/Ecosystem/Rainforest|Rainforest]] 

### #is_/same_as :: [[/_public/bio/Ecology/Ecosystem/Rainforest.public|Rainforest.public]] 

### #is_/same_as :: [[/_internal/bio/Ecology/Ecosystem/Rainforest.internal|Rainforest.internal]] 

### #is_/same_as :: [[/_protect/bio/Ecology/Ecosystem/Rainforest.protect|Rainforest.protect]] 

### #is_/same_as :: [[/_private/bio/Ecology/Ecosystem/Rainforest.private|Rainforest.private]] 

### #is_/same_as :: [[/_personal/bio/Ecology/Ecosystem/Rainforest.personal|Rainforest.personal]] 

### #is_/same_as :: [[/_secret/bio/Ecology/Ecosystem/Rainforest.secret|Rainforest.secret]] 

