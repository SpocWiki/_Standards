---
aliases:
- barības ķēde
- besin zinciri
- Cadea trófica
- cadeia alimentar
- cadena alimentària
- cadena trófica
- Cadwyn fwyd
- catena alimentare
- Catina alimintari
- chaîne alimentaire
- Chia̍h-mi̍h báng
- chuỗi thức ăn
- Fiedselstring
- Food Chain
- foodchain
- Food_Chain
- Fæðukeðja
- fødekæde
- Hranidbeni lanac
- Jôdny lińcuch
- Kate trofiko
- Lanac ishrane
- Lanț trofic
- manĝoĉeno
- Mitybos grandinė
- Mtando chakula
- Nahrungskette
- nasin moku
- nri yinye
- Nutriva kateno
- näringskedja
- næringskjede
- potravinový reťazec
- potravní řetězec
- prehranjevalna veriga
- qida zənciri
- Rantai makanan
- ravintoketju
- Rezo alimantè
- slabhra bia
- Toitumisahel
- tápláléklánc
- voedselketen
- Voedselketting
- zincîra xurekan
- Zinxhiri ushqimor
- Łańcuch pokarmowy
- ǹsaŋ bìjɛk
- Τροφική αλυσίδα
- Ланац исхране
- Ланец на исхрана
- ланцуг харчавання
- пищевая цепь
- Трофикалык чынжырлар
- харчовий ланцюжок
- харчовы ланцужок
- Хранителна верига
- Қоректік деңгей
- Սննդային կապեր
- שרשרת מזון
- زنجیره غذایی
- زنجیرەی بژیو
- سلسلة غذائية
- غذائی زنجیر
- کھؠنہٕ ہانٛکل
- ކާބޯތަކެތީގެ އަންބަލަން
- अन्नसाखळी
- खाद्य शृंखला
- খাদ্য শৃঙ্খল
- ਖੁਰਾਕ ਲੜੀ
- உணவுச் சங்கிலி
- ఆహారపు గొలుసు
- ಆಹಾರ ಸರಪಳಿ
- โซ่อาหาร
- အစာကွင်းဆက်
- კვების ჯაჭვი
- 食物連鎖
- 食物鏈
- 食物链
- 먹이 사슬
has_id_wikidata: Q159462
discoverer_or_inventor:
- '[[_Standards/WikiData/WD~Al_Jahiz,293497|WD~Al_Jahiz,293497]]'
- '[[_Standards/WikiData/WD~Charles_Sutherland_Elton,1066200|WD~Charles_Sutherland_Elton,1066200]]'
has_part_s_:
- '[[_Standards/WikiData/WD~keystone_species,518808|WD~keystone_species,518808]]'
- '[[_Standards/WikiData/WD~trophic_level,1053008|WD~trophic_level,1053008]]'
manifestation_of:
- '[[_Standards/WikiData/WD~biological_organisation,803888|WD~biological_organisation,803888]]'
- '[[_Standards/WikiData/WD~biological_interaction,1823478|WD~biological_interaction,1823478]]'
part_of:
- '[[_Standards/WikiData/WD~food_web,1775153|WD~food_web,1775153]]'
- '[[_Standards/WikiData/WD~ecosystem,37813|WD~ecosystem,37813]]'
instance_of: '[[_Standards/WikiData/WD~biological_interaction,1823478|WD~biological_interaction,1823478]]'
described_by_source:
- '[[_Standards/WikiData/WD~Great_Soviet_Encyclopedia_(1969_1978),17378135|WD~Great_Soviet_Encyclopedia_(1969_1978),17378135]]'
- '[[_Standards/WikiData/WD~Belarusian_encyclopedia_(vol._15),24038862|WD~Belarusian_encyclopedia_(vol._15),24038862]]'
- '[[_Standards/WikiData/WD~Q60852062,60852062|WD~Q60852062,60852062]]'
studied_by:
- '[[_Standards/WikiData/WD~biology,420|WD~biology,420]]'
- '[[_Standards/WikiData/WD~ecology,7150|WD~ecology,7150]]'
- '[[_Standards/WikiData/WD~economics,8134|WD~economics,8134]]'
OmegaWiki_Defined_Meaning: 1507
image: http://commons.wikimedia.org/wiki/Special:FilePath/FoodWeb.svg
Commons_category: Food chain
MeSH_tree_code:
- G16.500.275.157.250
- N06.230.124.250
Wolfram_Language_entity_code: Entity["Concept", "FoodChain::9f855"]
dv_has_:
  name_:
    af: Voedselketting
    ar: سلسلة غذائية
    az: qida zənciri
    bas: ǹsaŋ bìjɛk
    be: ланцуг харчавання
    be_tarask: харчовы ланцужок
    bg: Хранителна верига
    bn: খাদ্য শৃঙ্খল
    ca: cadena alimentària
    ckb: زنجیرەی بژیو
    cs: potravní řetězec
    csb: Jôdny lińcuch
    cy: Cadwyn fwyd
    da: fødekæde
    de: Nahrungskette
    dv: ކާބޯތަކެތީގެ އަންބަލަން
    el: Τροφική αλυσίδα
    en: food chain
    en_ca: Food chain
    en_gb: food chain
    eo: manĝoĉeno
    es: cadena trófica
    et: Toitumisahel
    eu: Kate trofiko
    fa: زنجیره غذایی
    fi: ravintoketju
    fr: chaîne alimentaire
    fy: Fiedselstring
    ga: slabhra bia
    gl: Cadea trófica
    he: שרשרת מזון
    hi: खाद्य शृंखला
    hr: Hranidbeni lanac
    ht: Rezo alimantè
    hu: tápláléklánc
    hy: Սննդային կապեր
    id: Rantai makanan
    ig: nri yinye
    io: Nutriva kateno
    is: Fæðukeðja
    it: catena alimentare
    ja: 食物連鎖
    ka: კვების ჯაჭვი
    kk: Қоректік деңгей
    kn: ಆಹಾರ ಸರಪಳಿ
    ko: 먹이 사슬
    ks: کھؠنہٕ ہانٛکل
    ku: zincîra xurekan
    ku_latn: zincîra xurekan
    ky: Трофикалык чынжырлар
    lt: Mitybos grandinė
    lv: barības ķēde
    mk: Ланец на исхрана
    ml: foodchain
    mr: अन्नसाखळी
    ms: Rantai makanan
    my: အစာကွင်းဆက်
    nan: Chia̍h-mi̍h báng
    nb: næringskjede
    nl: voedselketen
    nn: næringskjede
    pa: ਖੁਰਾਕ ਲੜੀ
    pl: Łańcuch pokarmowy
    pt: cadeia alimentar
    pt_br: Cadeia alimentar
    ro: Lanț trofic
    ru: пищевая цепь
    scn: Catina alimintari
    sh: Hranidbeni lanac
    sk: potravinový reťazec
    sl: prehranjevalna veriga
    sq: Zinxhiri ushqimor
    sr: Ланац исхране
    sr_ec: Ланац исхране
    sr_el: Lanac ishrane
    sv: näringskedja
    sw: Mtando chakula
    ta: உணவுச் சங்கிலி
    te: ఆహారపు గొలుసు
    th: โซ่อาหาร
    tok: nasin moku
    tr: besin zinciri
    uk: харчовий ланцюжок
    ur: غذائی زنجیر
    vi: chuỗi thức ăn
    wuu: 食物链
    yue: 食物鏈
    zh: 食物鏈
    zh_cn: 食物链
    zh_hans: 食物链
    zh_hant: 食物鏈
    zh_hk: 食物鏈
    zh_sg: 食物链
    zh_tw: 食物鏈
---

# [[Food_Chain]] 

#is_/same_as :: [[../../../WikiData/WD~Food_chain,159462|WD~Food_chain,159462]]  

## #has_/text_of_/abstract 

> A food chain is a linear network of links in a food web, often 
> - beginning with an autotroph (such as grass or algae), also called a producer, 
> - and typically ending at 
> 	- an apex predator (such as grizzly bears or killer whales), 
> 	- detritivore (such as earthworms and woodlice), or 
> 	- decomposer (such as fungi or bacteria). 
> 
> A food web is distinct from a food chain. 
> A food chain illustrates the associations between organisms 
> according to the energy sources they consume in trophic levels, 
> and the most common way to quantify them is in length: 
> the number of links between a trophic consumer and the base of the chain.
>
> Studies of food chains are essential to many biological studies.
>
> Stability of the food chain is crucial for survival of most species. Removing even one component from the food chain could result in extinction or significant decreases in a species' probability of surviving. Many food chains and food webs contain a keystone species, a species that could directly affect the food chain and has a significant impact on the environment. The absence of a keystone species could destroy the balance of the entire food chain.
>
> The efficiency of a food chain depends on the energy first consumed by the primary producers. This energy then moves through the trophic levels.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Food%20chain) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Ecology/Ecosystem/Food_Chain|Food_Chain]] 

### #is_/same_as :: [[/_public/bio/Ecology/Ecosystem/Food_Chain.public|Food_Chain.public]] 

### #is_/same_as :: [[/_internal/bio/Ecology/Ecosystem/Food_Chain.internal|Food_Chain.internal]] 

### #is_/same_as :: [[/_protect/bio/Ecology/Ecosystem/Food_Chain.protect|Food_Chain.protect]] 

### #is_/same_as :: [[/_private/bio/Ecology/Ecosystem/Food_Chain.private|Food_Chain.private]] 

### #is_/same_as :: [[/_personal/bio/Ecology/Ecosystem/Food_Chain.personal|Food_Chain.personal]] 

### #is_/same_as :: [[/_secret/bio/Ecology/Ecosystem/Food_Chain.secret|Food_Chain.secret]] 

