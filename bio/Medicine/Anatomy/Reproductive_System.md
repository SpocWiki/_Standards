---
aliases:
- aparato reproductor
- aparatu reproductor
- aparelho reprodutor
- apparato genitale
- Ardivink-genel
- córas atáirgthe
- Fitaovam-pananahana
- forplantingssystem
- forplantningssystem
- fortplantningssystemet
- Habdhiska Taranka
- hệ sinh dục
- koendama zaûzê
- lisääntymiselimistö
- Lytiniai organai
- Mfumo wa uzazi
- Reproductive System
- Reproductive_System
- reprodukta sistemo
- Reproduktionssystem
- Reproduktiv organlar
- Reproduktiv sistem
- Reproduktivni sistem
- reproduktivni sitem
- Reproduktschoonssysteem
- reproduktsiya sisteması
- reproduktīvā sistēma
- rozmnožovacia sústava
- rozmnožovací soustava
- Seⁿ-si̍t hē-thóng
- sigimiselundkond
- Sistem pembiakan
- Sistem reproducător
- Sistem reproduksi
- sistem riprodhues
- Sistem réprodhuksi
- Sistema reproductivo
- sistema reproductor
- Sistema reproduktibo
- sistema reprodutor
- Sistema riprodutiv
- sistema ti reproduktibo
- Sistemang reproduktibo
- Sistèma reproductor
- spolni sustav
- system atgenhedlu
- Systema reproductionis
- systema reproductive
- système reproducteur
- szaporítórendszer
- Tete pehẽnguenguéra ñemoñarã
- Ugaltze-aparatu
- układ rozrodczy
- voortplantingssysteem
- voortplantingstelsel
- Æxlunarfæri
- üreme sistemi
- αναπαραγωγικό σύστημα
- Бэлгын системэ
- Жыныс органдары
- Нылпи ваёно сӧзнэт
- Полова система
- репродуктив система
- Репродуктиван система
- репродуктивен систем
- репродуктивна система
- репродуктивная система
- репродуктивни систем
- Репродуктивті жүйе
- Рэпрадуктыўная сістэма
- рэпрадукцыйная сыстэма
- Узвҳои таносул
- Ууһуур тиһик
- Яндзим
- վերարտադրողական համակարգ
- מערכת הרבייה
- פארמערן סיסטעם
- تؤرهمه سیستمی
- توليدي نظام
- تولیدی نظام
- جمن پربندھ
- جهاز تناسلي
- دستگاه تناسلی
- کۆئەندامی زاوزێ
- ނަސްލު އުފައްދާ ނިޒާމް
- प्रजनन तंत्र
- प्रजननसंस्था
- প্রজননতন্ত্র
- প্ৰজনন তন্ত্ৰ
- ਜਣਨ ਪ੍ਰਬੰਧ
- இனப்பெருக்கத் தொகுதி
- ಸಂತಾನೋತ್ಫತ್ತಿಯ ವ್ಯವಸ್ಥೆ
- പ്രത്യുത്പാദനേന്ദ്രിയ വ്യൂഹം
- ระบบสืบพันธุ์
- སྐྱེ་འཕེལ་མ་ལག་
- რეპროდუქციული სისტემა
- 生殖器系
- 生殖系統
- 生殖系统
- 생식계
has_id_wikidata: Q7895
Commons_category: Reproductive system
described_by_source: '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,_vol._5,124737632]]'
development_of_anatomical_structure: '[[_Standards/WikiData/WD~reproductive_system_development,21106555]]'
has_part_s_: '[[_Standards/WikiData/WD~gametangium,1479790]]'
has_part_s_of_the_class: '[[_Standards/WikiData/WD~sex_organ,4620674]]'
instance_of:
- '[[_Standards/WikiData/WD~anatomical_system_type,104027169]]'
- '[[_Standards/WikiData/WD~class_of_anatomical_entity,112826905]]'
part_of: '[[_Standards/WikiData/WD~genitourinary_system,4712977]]'
pronunciation_audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q1571%20%28mar%29-Vj18081991-%E0%A4%AA%E0%A5%8D%E0%A4%B0%E0%A4%9C%E0%A4%A8%E0%A4%A8%E0%A4%B8%E0%A4%82%E0%A4%B8%E0%A5%8D%E0%A4%A5%E0%A4%BE.wav
subclass_of:
- '[[_Standards/WikiData/WD~organ_system,188193]]'
- '[[_Standards/WikiData/WD~particular_anatomical_entity,112826975]]'
TA98_Latin_term: systemata genitalia
topic_s_main_category: '[[_Standards/WikiData/WD~Q9708900,9708900]]'
topic_s_main_template:
- '[[_Standards/WikiData/WD~Template_Female_reproductive_system,5614232]]'
- '[[_Standards/WikiData/WD~Q125733549,125733549]]'
dv_has_:
  name_:
    af: voortplantingstelsel
    an: Sistema reproductivo
    ar: جهاز تناسلي
    as: প্ৰজনন তন্ত্ৰ
    ast: aparatu reproductor
    az: Reproduktiv sistem
    azb: تؤرهمه سیستمی
    ba: Репродуктив система
    bcl: Sistemang reproduktibo
    be: Рэпрадуктыўная сістэма
    be_tarask: рэпрадукцыйная сыстэма
    bg: Полова система
    bn: প্রজননতন্ত্র
    bo: སྐྱེ་འཕེལ་མ་ལག་
    br: Ardivink-genel
    bs: Reproduktivni sistem
    bxr: Бэлгын системэ
    ca: sistema reproductor
    ce: Репродуктиван система
    ckb: کۆئەندامی زاوزێ
    crh: reproduktsiya sisteması
    cs: rozmnožovací soustava
    cy: system atgenhedlu
    da: forplantningssystem
    de: Reproduktionssystem
    dv: ނަސްލު އުފައްދާ ނިޒާމް
    el: αναπαραγωγικό σύστημα
    en: reproductive system
    en_gb: reproductive system
    eo: reprodukta sistemo
    es: aparato reproductor
    et: sigimiselundkond
    eu: Ugaltze-aparatu
    fa: دستگاه تناسلی
    fi: lisääntymiselimistö
    fr: système reproducteur
    ga: córas atáirgthe
    gl: sistema reprodutor
    gn: Tete pehẽnguenguéra ñemoñarã
    gor: Sistem reproduksi
    gsw: Reproduktionssystem
    he: מערכת הרבייה
    hi: प्रजनन तंत्र
    hif: reproductive system
    hr: spolni sustav
    hu: szaporítórendszer
    hy: վերարտադրողական համակարգ
    ia: systema reproductive
    id: sistem reproduksi
    ilo: sistema ti reproduktibo
    is: Æxlunarfæri
    it: apparato genitale
    ja: 生殖器系
    jv: Sistem réprodhuksi
    ka: რეპროდუქციული სისტემა
    kk: Репродуктивті жүйе
    kn: ಸಂತಾನೋತ್ಫತ್ತಿಯ ವ್ಯವಸ್ಥೆ
    knc: Reproductive system
    ko: 생식계
    ku_latn: koendama zaûzê
    kv: Яндзим
    ky: Жыныс органдары
    la: Systema reproductionis
    lt: Lytiniai organai
    lv: reproduktīvā sistēma
    mg: Fitaovam-pananahana
    mk: репродуктивен систем
    ml: പ്രത്യുത്പാദനേന്ദ്രിയ വ്യൂഹം
    mr: प्रजननसंस्था
    ms: Sistem pembiakan
    nan: Seⁿ-si̍t hē-thóng
    nb: forplantningssystem
    nds: Reproduktschoonssysteem
    nl: voortplantingssysteem
    nn: forplantingssystem
    oc: Sistèma reproductor
    pa: ਜਣਨ ਪ੍ਰਬੰਧ
    pl: układ rozrodczy
    pms: Sistema riprodutiv
    pnb: جمن پربندھ
    pt: aparelho reprodutor
    ro: Sistem reproducător
    ru: репродуктивная система
    sah: Ууһуур тиһик
    sd: توليدي نظام
    sh: Reproduktivni sistem
    sk: rozmnožovacia sústava
    sl: reproduktivni sitem
    so: Habdhiska Taranka
    sq: sistem riprodhues
    sr: репродуктивни систем
    sr_ec: репродуктивни систем
    sr_el: reproduktivni sistem
    sv: fortplantningssystemet
    sw: Mfumo wa uzazi
    ta: இனப்பெருக்கத் தொகுதி
    tg: Узвҳои таносул
    th: ระบบสืบพันธุ์
    tl: Sistemang reproduktibo
    tr: üreme sistemi
    tt: репродуктив система
    udm: Нылпи ваёно сӧзнэт
    uk: репродуктивна система
    ur: تولیدی نظام
    uz: Reproduktiv organlar
    vi: hệ sinh dục
    war: Sistema reproduktibo
    wuu: 生殖系统
    xmf: რეპროდუქციული სისტემა
    yi: פארמערן סיסטעם
    yue: 生殖系統
    zh: 生殖系统
---
# [[Reproductive_System]] 

#is_/same_as :: [[../../../WikiData/WD~Reproductive_system,7895|WD~Reproductive_system,7895]] 

## #has_/text_of_/abstract 

> The **Reproductive system** of an organism, also known as the genital system, 
> is the biological system made up of all the anatomical organs involved in sexual reproduction.  
> 
> Many non-living substances such as fluids, hormones, and pheromones 
> are also important accessories to the reproductive system. 
> 
> Unlike most organ systems, the sexes of differentiated species often have significant differences. 
> 
> These differences allow for a combination of genetic material between two individuals, 
> which allows for the possibility of greater genetic fitness of the offspring.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Reproductive%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Medicine/Anatomy/Reproductive_System|Reproductive_System]] 

### #is_/same_as :: [[/_public/bio/Medicine/Anatomy/Reproductive_System.public|Reproductive_System.public]] 

### #is_/same_as :: [[/_internal/bio/Medicine/Anatomy/Reproductive_System.internal|Reproductive_System.internal]] 

### #is_/same_as :: [[/_protect/bio/Medicine/Anatomy/Reproductive_System.protect|Reproductive_System.protect]] 

### #is_/same_as :: [[/_private/bio/Medicine/Anatomy/Reproductive_System.private|Reproductive_System.private]] 

### #is_/same_as :: [[/_personal/bio/Medicine/Anatomy/Reproductive_System.personal|Reproductive_System.personal]] 

### #is_/same_as :: [[/_secret/bio/Medicine/Anatomy/Reproductive_System.secret|Reproductive_System.secret]] 

