---
aliases:
- flara sistemo
- hid bilish aʼzolari
- Hiù-kak hē-thóng
- hệ khứu giác
- koendama bêhnkirinê
- Lyktarskynfæri
- Njušni sustav
- olfactory system
- Olfactory_System
- Olfaktorni sistem
- olfaktör sistem
- Riechbahn
- Sistem olfactiv
- Sistem pembauan
- Sistem penciuman
- sistema olfactiu
- sistema olfactivo
- sistema olfativo
- Sistema olfativu
- sistema olfattivo
- Sistemang pang-amoy
- system arogleuol
- Systema olfactorium
- système olfactif
- szaglórendszer
- Usaimen-sistema
- vohalna pot
- Čulo mirisa
- οσφρητικό σύστημα
- нюхавая сыстэма
- нюховий аналізатор
- обонятельная сенсорная система
- Հոտառական համակարգ
- מערכת ההרחה
- جهاز الشم
- دستگاه بویایی
- घ्राण तंत्र
- நுகர்ச்சி மண்டலம்
- ಘ್ರಾಣಮಂಡಲ
- ระบบรู้กลิ่น
- 嗅覚系
- 嗅覺系統
- 嗅觉系统
- 후각 기관
has_id_wikidata: Q1054094
instance_of: '[[_Standards/WikiData/WD~organ_system,188193|WD~organ_system,188193]]'
described_by_source:
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~New_Encyclopedic_Dictionary,19190511|WD~New_Encyclopedic_Dictionary,19190511]]'
different_from: '[[_Standards/WikiData/WD~sense_of_smell,1541064|WD~sense_of_smell,1541064]]'
facet_of: '[[_Standards/WikiData/WD~sense_of_smell,1541064|WD~sense_of_smell,1541064]]'
has_use: '[[_Standards/WikiData/WD~sense_of_smell,1541064|WD~sense_of_smell,1541064]]'
subclass_of: '[[_Standards/WikiData/WD~heterogeneous_anatomical_cluster,66561296|WD~heterogeneous_anatomical_cluster,66561296]]'
studied_by: '[[_Standards/WikiData/WD~rhinotopy,115756137|WD~rhinotopy,115756137]]'
Commons_category: Olfactory system
dv_has_:
  name_:
    ar: جهاز الشم
    ast: Sistema olfativu
    be_tarask: нюхавая сыстэма
    bs: Čulo mirisa
    ca: sistema olfactiu
    cy: system arogleuol
    de: Riechbahn
    el: οσφρητικό σύστημα
    en: olfactory system
    eo: flara sistemo
    es: sistema olfativo
    eu: Usaimen-sistema
    fa: دستگاه بویایی
    fr: système olfactif
    gl: sistema olfactivo
    he: מערכת ההרחה
    hi: घ्राण तंत्र
    hr: Njušni sustav
    hu: szaglórendszer
    hy: Հոտառական համակարգ
    id: Sistem penciuman
    is: Lyktarskynfæri
    it: sistema olfattivo
    ja: 嗅覚系
    kn: ಘ್ರಾಣಮಂಡಲ
    ko: 후각 기관
    ku: koendama bêhnkirinê
    la: Systema olfactorium
    ms: Sistem pembauan
    nan: Hiù-kak hē-thóng
    ro: Sistem olfactiv
    ru: обонятельная сенсорная система
    sh: Olfaktorni sistem
    sl: vohalna pot
    sr: Olfaktorni sistem
    ta: நுகர்ச்சி மண்டலம்
    th: ระบบรู้กลิ่น
    tl: Sistemang pang-amoy
    tr: olfaktör sistem
    uk: нюховий аналізатор
    uz: hid bilish aʼzolari
    vi: hệ khứu giác
    yue: 嗅覺系統
    zh: 嗅覺系統
    zh_hans: 嗅觉系统
    zh_hant: 嗅覺系統
    zh_hk: 嗅覺系統
---

# [[Olfactory_System]] 

#is_/same_as :: [[_Standards/WikiData/WD~Olfactory_system,1054094|WD~Olfactory_system,1054094]] 

## #has_/text_of_/abstract 

> The olfactory system is the sensory system used for the sense of smell (olfaction). 
> Olfaction is one of the special senses directly associated with specific organs. 
> 
> Most mammals and reptiles have a main olfactory system and an accessory olfactory system. 
> The main olfactory system detects airborne substances, 
> while the accessory system senses fluid-phase stimuli.
>
> The senses of smell and taste (gustatory system) are often referred to together 
> as the chemosensory system, 
> because they both give the brain information about the chemical composition of objects 
> through a process called transduction.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Olfactory%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Medicine/Anatomy/Sense/Olfactory_System|Olfactory_System]] 

### #is_/same_as :: [[/_public/bio/Medicine/Anatomy/Sense/Olfactory_System.public|Olfactory_System.public]] 

### #is_/same_as :: [[/_internal/bio/Medicine/Anatomy/Sense/Olfactory_System.internal|Olfactory_System.internal]] 

### #is_/same_as :: [[/_protect/bio/Medicine/Anatomy/Sense/Olfactory_System.protect|Olfactory_System.protect]] 

### #is_/same_as :: [[/_private/bio/Medicine/Anatomy/Sense/Olfactory_System.private|Olfactory_System.private]] 

### #is_/same_as :: [[/_personal/bio/Medicine/Anatomy/Sense/Olfactory_System.personal|Olfactory_System.personal]] 

### #is_/same_as :: [[/_secret/bio/Medicine/Anatomy/Sense/Olfactory_System.secret|Olfactory_System.secret]] 

