---
aliases:
- apparato tegumentario
- córas forchlúideach
- Ensengekera y'ekikugiro
- Goā-phoê hē-thóng
- Hệ vỏ bọc
- Integument
- Integumentair systeem
- Integumentary System
- Integumentary_System
- integumentinė sistema
- Integumentsystemet
- Integumentti
- koendama poşerê
- krycia sústava
- Krycí soustava
- kültakaró
- Pokrovni sistem
- pokrovni sustav
- Powłoka wspólna
- segorgānu sistēma
- Sirni Gogaa
- Sistem integumen
- Sistema integumentario
- Sistema integumentariu
- Sistema integumentaryo
- sistema tegumentar
- sistema tegumentari
- sistema tegumentario
- Sistemang integumentaryo
- sistemul tegumentar
- Sistèma integumentari
- System bilynol
- Systema integumentari
- Systema integumentarium
- système tégumentaire
- tegumenta sistemo
- Tegumentu-sistema
- Örtü sistemi
- örtüci sisteması
- Örtüji dokuma
- Örtük sistemi
- Þekjukerfið
- Καλυπτήριο σύστημα
- покривен систем
- Покривна система
- покровная система
- Покровни систем
- покрыўная сістэма
- покрыўная сыстэма
- Тән япмаһы системаһы
- מערכת הכסות
- انٹیگومنٹری پربندھ
- جهاز لحافي
- دستگاه پوششی
- پۆشەرە کۆئەندام
- अध्यावरणी तंत्र
- इन्तेगुमेन्तरी सिस्टम
- আচ্ছাদন তন্ত্র
- আচ্ছাদন তন্ত্ৰ
- புறவுறைத் தொகுதி
- ഇന്റഗ്യുമെന്ററി സിസ്റ്റം
- ශරීරාවරණ පද්ධතිය
- ระบบผิวหนัง
- 外皮系
- 外皮系統
- 表皮系統
- 表皮系统
- 피부계
has_id_wikidata: Q483213
part_of:
- '[[_Standards/WikiData/WD~organ_system,188193|WD~organ_system,188193]]'
- '[[_Standards/WikiData/WD~covering,23497974|WD~covering,23497974]]'
- '[[_Standards/WikiData/WD~human_body,23852|WD~human_body,23852]]'
subclass_of: '[[_Standards/WikiData/WD~organ_system,188193|WD~organ_system,188193]]'
instance_of:
- '[[_Standards/WikiData/WD~organ_system,188193|WD~organ_system,188193]]'
- '[[_Standards/WikiData/WD~class_of_anatomical_entity,112826905|WD~class_of_anatomical_entity,112826905]]'
maintained_by_WikiProject:
- '[[_Standards/WikiData/WD~WikiProject_Anatomy,8487304|WD~WikiProject_Anatomy,8487304]]'
- '[[_Standards/WikiData/WD~WikiProject_Animal_anatomy,21829301|WD~WikiProject_Animal_anatomy,21829301]]'
- '[[_Standards/WikiData/WD~WikiProject_Systems,21830857|WD~WikiProject_Systems,21830857]]'
MeSH_tree_code: A17
TA98_Latin_term: integumentum commune
Dewey_Decimal_Classification:
- 573.5
- 591.47
- 636.089279
image: http://commons.wikimedia.org/wiki/Special:FilePath/Skin%20blank.jpg
UMLS_CUI: C0037267
Wolfram_Language_entity_code: Entity["AnatomicalStructure", "IntegumentarySystem"]
Terminologia_Histologica: H3.12.00.0.00001
Commons_category: Integumentary system
dv_has_:
  name_:
    an: Sistema integumentario
    ar: جهاز لحافي
    as: আচ্ছাদন তন্ত্ৰ
    ast: Sistema integumentariu
    az: Örtük sistemi
    ba: Тән япмаһы системаһы
    be: покрыўная сістэма
    be_tarask: покрыўная сыстэма
    bg: Покривна система
    bn: আচ্ছাদন তন্ত্র
    bs: Pokrovni sistem
    ca: sistema tegumentari
    ckb: پۆشەرە کۆئەندام
    crh: örtüci sisteması
    cs: Krycí soustava
    cy: System bilynol
    de: Integument
    el: Καλυπτήριο σύστημα
    en: integumentary system
    eo: tegumenta sistemo
    es: sistema integumentario
    eu: Tegumentu-sistema
    fa: دستگاه پوششی
    fi: Integumentti
    fr: système tégumentaire
    ga: córas forchlúideach
    gl: sistema tegumentario
    gsw: Integument
    he: מערכת הכסות
    hi: अध्यावरणी तंत्र
    hr: pokrovni sustav
    hu: kültakaró
    ia: Systema integumentari
    id: Sistem integumen
    io: Tegumenta sistemo
    is: Þekjukerfið
    it: apparato tegumentario
    ja: 外皮系
    jv: Sistem integumen
    ko: 피부계
    ku_latn: koendama poşerê
    la: Systema integumentarium
    lg: Ensengekera y'ekikugiro
    li: Integumentair systeem
    lt: integumentinė sistema
    lv: segorgānu sistēma
    mk: покривен систем
    ml: ഇന്റഗ്യുമെന്ററി സിസ്റ്റം
    ms: Sistem integumen
    nan: Goā-phoê hē-thóng
    new: इन्तेगुमेन्तरी सिस्टम
    nl: integumentair systeem
    nn: integument
    oc: Sistèma integumentari
    om: Sirni Gogaa
    pl: Powłoka wspólna
    pms: Sistema tegumentari
    pnb: انٹیگومنٹری پربندھ
    pt: sistema tegumentar
    ro: sistemul tegumentar
    ru: покровная система
    sh: Pokrovni sistem
    si: ශරීරාවරණ පද්ධතිය
    sk: krycia sústava
    sl: integument
    sr: Покровни систем
    sr_ec: Покровни систем
    sr_el: Pokrovni sistem
    sv: Integumentsystemet
    ta: புறவுறைத் தொகுதி
    th: ระบบผิวหนัง
    tk: Örtüji dokuma
    tl: Sistemang integumentaryo
    tr: Örtü sistemi
    uk: покривна система
    vi: Hệ vỏ bọc
    war: Sistema integumentaryo
    wuu: 表皮系统
    yue: 外皮系統
    zh: 表皮系統
    zh_hans: 表皮系统
    zh_hant: 表皮系統
    zh_hk: 表皮系統
---

# [[Integumentary_System]] 

#is_/same_as :: [[WikiData/WD~Integumentary_system,483213|WD~Integumentary_system,483213]] 

## #has_/text_of_/abstract 

> The integumentary system is the set of organs forming the outermost layer of an animal's body, comprising the skin, hair, scales, feathers, hooves, claws, and nails. 
> It acts as a protective physical barrier 
> between the external environment and the internal environment. 
> Additionally, it maintains water balance, protects the deeper tissues, excretes waste, regulates body temperature, and contains the sensory receptors that detect pain, sensation, pressure, and temperature.
>
> The skin (integument) is a composite organ, made up of at least two major layers of tissue: 
> the outermost epidermis and the inner dermis, 
> separated by a basement membrane (comprising basal lamina and reticular lamina). 
> 
> The epidermis comprises five layers: the stratum corneum, stratum granulosum, stratum spinosum and stratum basale. Where the skin is thicker, such as in the palms and soles, there is an extra layer of skin between the stratum corneum and the stratum granulosum known as the stratum lucidum. The dermis comprises two sections, the papillary and reticular layers, and contains connective tissues, blood vessels, glands, follicles, hair roots, sensory nerve endings, and muscular tissue. Between the integument and the deep body musculature there is a transitional subcutaneous zone, the hypodermis.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Integumentary%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Medicine/Anatomy/Skin/Integumentary_System|Integumentary_System]] 

### #is_/same_as :: [[/_public/bio/Medicine/Anatomy/Skin/Integumentary_System.public|Integumentary_System.public]] 

### #is_/same_as :: [[/_internal/bio/Medicine/Anatomy/Skin/Integumentary_System.internal|Integumentary_System.internal]] 

### #is_/same_as :: [[/_protect/bio/Medicine/Anatomy/Skin/Integumentary_System.protect|Integumentary_System.protect]] 

### #is_/same_as :: [[/_private/bio/Medicine/Anatomy/Skin/Integumentary_System.private|Integumentary_System.private]] 

### #is_/same_as :: [[/_personal/bio/Medicine/Anatomy/Skin/Integumentary_System.personal|Integumentary_System.personal]] 

### #is_/same_as :: [[/_secret/bio/Medicine/Anatomy/Skin/Integumentary_System.secret|Integumentary_System.secret]] 

