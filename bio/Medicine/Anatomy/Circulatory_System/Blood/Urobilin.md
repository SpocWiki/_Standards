---
aliases:
has_id_wikidata: Q41353
subclass_of: "[[/_Standards/WikiData/WD~heterocyclic_compound,193430]]"
stereoisomer_of:
  - "[[/_Standards/WikiData/WD~(4S,10Z,16R)_phycourobilin,635700]]"
  - "[[/_Standards/WikiData/WD~(+)_urobilin,27282728]]"
subject_has_role: "[[/_Standards/WikiData/WD~primary_metabolite,3333419]]"
instance_of: "[[/_Standards/WikiData/WD~type_of_chemical_entity,113145171]]"
mass: 590.310435064
Commons_category: Urobilin
PubChem_CID: "6433298"
InChIKey: KDCCOOGTVSRCHX-UYMYUHGCSA-N
UNII: G0OTH73Y0M
InChI: InChI=1S/C33H42N4O6/c1-7-20-19(6)32(42)37-27(20)14-25-18(5)23(10-12-31(40)41)29(35-25)15-28-22(9-11-30(38)39)17(4)24(34-28)13-26-16(3)21(8-2)33(43)36-26/h15,26-27,35H,7-14H2,1-6H3,(H,36,43)(H,37,42)(H,38,39)(H,40,41)/b28-15-/t26-,27-/m0/s1
chemical_formula: C₃₃H₄₂N₄O₆
canonical_SMILES: CCC1=C(C(=O)NC1CC2=C(C(=C(N2)C=C3C(=C(C(=N3)CC4C(=C(C(=O)N4)CC)C)C)CCC(=O)O)CCC(=O)O)C)C
CAS_Registry_Number: 1856-98-0
EC_number: 217-456-3
Reaxys_registry_number: "3027217"
UMLS_CUI: C0042051
isomeric_SMILES: CCC1=C(C(=O)N[C@H]1CC2=C(C(=C(N2)/C=C\3/C(=C(C(=N3)C[C@H]4C(=C(C(=O)N4)CC)C)C)CCC(=O)O)CCC(=O)O)C)C
MeSH_tree_code:
  - D03.383.129.578.840.249.727
  - D03.633.400.909.249.727
  - D04.345.783.249.727
  - D23.767.193.727
DSSTOX_compound_identifier: DTXCID40883793
---

# [[Urobilin]] 

#can_/convert_from_ :: [[Bilirubin]] 
#is_/same_as :: [[WD~Urobilin,41353]] 

## #has_/text_of_/abstract 

> **Urobilin**, also known as urochrome, 
> is the chemical primarily responsible for the yellow color of urine. 
> It is a linear tetrapyrrole compound that, 
> along with the related colorless compound urobilinogen, 
> are degradation products of the cyclic tetrapyrrole heme.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Urobilin) 




