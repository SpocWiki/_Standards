---
aliases:
- ademhalingsstelsel
- Anagraw amesgunef
- Analachadh
- aomhaolingsstèlsel
- Aparato respiratorio
- Aparatu respiratoriu
- aparello respiratorio
- apparato respiratorio
- appareil respiratoire
- arnas aparatu
- asemhalingstelsel
- Atemtrakt
- Atemwege
- Atensysteem
- córas riospráide
- dem alw jüyesi
- dihala
- Dišni sistem
- dišni sustav
- dýchacia sústava
- dýchací soustava
- el sistema respiratorio
- elpošanas orgānu sistēma
- Hab Neefsi
- hengityselimistö
- hingamiselundkond
- Ho͘-khip hē-thóng
- hệ hô hấp
- Koendama henaseyê
- kvėpavimo sistema
- légzőrendszer
- Mfumo wa upumuaji
- mihanhan ilabu' nu nanum
- nafas organlari
- nefes sisteması
- Ootmungsapparat beim Mënsch
- Respirala sistemo
- respirasjonssystem
- respirationssystem
- Respiratorni sistem
- Respiratory System
- Respiratory_System
- saans le waala system
- Samana yawri
- Sirna hargansuu
- Sistem ambegan
- Sistem de respira
- Sistem panyabâ
- sistem pernafasan
- Sistem pernapasan
- sistem respirator
- sistema respiratori
- Sistema respiratorio
- Sistema respiratoriu
- Sistema respiratoryo
- Sistema respiratòri
- sistema respiratório
- sistema ti panaganges
- Sistemang respiratoryo
- Sistemi i frymëmarrjes
- sistèma respiratòri
- solunum sistemi
- spira sistemo
- system resbiradu
- Systema respiratorium
- układ oddechowy
- vuoiŋâmorgaaneh
- åndedrettssystem
- åndedrætssystem
- Öndunarfæri
- İnsanın tənəffüs sistemi
- αναπνευστικό σύστημα
- дем алу жүйесі
- дем алуу системасы
- дихальна система
- Дихателна система
- дишен систем
- дыхальная сістэма
- дыхальная сыстэма
- дыхательная система
- систем органа за дисање
- сулыш системасы
- Тыынар тиһик
- шокан сӧзнэт
- Һулыш алыу системаһы
- Շնչառական համակարգ
- אטעם-סיסטעם
- מערכת הנשימה
- انسان ۾ ساھ کڻڻ جو عمل
- تنفوس سیستمی
- جهاز تنفسي
- دستگاه تنفس
- ساييز غونډال
- ساہ لین پربندھ
- سيستم ڤرنفسن
- نظام تنفس
- نەپەسلىنىش سىستېمى
- کۆئەندامی ھەناسە
- ނޭވާލާ ނިޒާމް
- रेस्पिरेटरी सिस्टम
- श्वसन तंत्र
- श्वासप्रस्वास प्रणाली
- শ্বসনতন্ত্র
- শ্বাস তন্ত্ৰ
- ਸਾਹ ਪ੍ਰਣਾਲੀ
- மூச்சுத் தொகுதி
- శ్వాస వ్యవస్థ
- ಉಸಿರಾಟದ ವ್ಯವಸ್ಥೆ
- ശ്വസനേന്ദ്രിയവ്യൂഹം
- ශ්වසන පද්ධතිය
- ระบบหายใจ
- དབུགས་ལམ་མ་ལག་
- 呼吸器系
- 呼吸系統
- 呼吸系统
- 호흡 계통
has_id_wikidata: Q7891
Commons_category: Respiratory system
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
- '[[_Standards/WikiData/WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752|WD~Meyers_Konversations-Lexikon,_4th_edition_(1885_1890),19219752]]'
has_part_s_:
- '[[_Standards/WikiData/WD~nasal_cavity,156104|WD~nasal_cavity,156104]]'
- '[[_Standards/WikiData/WD~pharynx,174778|WD~pharynx,174778]]'
- '[[_Standards/WikiData/WD~trachea,175449|WD~trachea,175449]]'
- '[[_Standards/WikiData/WD~bronchus,181602|WD~bronchus,181602]]'
- '[[_Standards/WikiData/WD~respiratory_tract,13400765|WD~respiratory_tract,13400765]]'
- '[[_Standards/WikiData/WD~lung,7886|WD~lung,7886]]'
- '[[_Standards/WikiData/WD~larynx,9637|WD~larynx,9637]]'
Iconclass_notation: 49G612
image: http://commons.wikimedia.org/wiki/Special:FilePath/Respiratory%20system%20complete%20en.svg
instance_of:
- '[[_Standards/WikiData/WD~anatomical_system_type,104027169|WD~anatomical_system_type,104027169]]'
- '[[_Standards/WikiData/WD~class_of_anatomical_entity,112826905|WD~class_of_anatomical_entity,112826905]]'
Krugosvet_article: nauka_i_tehnika/biologiya/DIHANIYA_ORGANI.html
MeSH_tree_code: A04
part_of: '[[_Standards/WikiData/WD~circulatory_system,11068|WD~circulatory_system,11068]]'
produced_sound: '[[_Standards/WikiData/WD~respiratory_sound,779038|WD~respiratory_sound,779038]]'
subclass_of:
- '[[_Standards/WikiData/WD~organ_system,188193|WD~organ_system,188193]]'
- '[[_Standards/WikiData/WD~particular_anatomical_entity,112826975|WD~particular_anatomical_entity,112826975]]'
TA98_Latin_term: systema respiratorium
topic_s_main_template:
- '[[_Standards/WikiData/WD~Template_Lower_respiratory_system_anatomy,10522033|WD~Template_Lower_respiratory_system_anatomy,10522033]]'
- '[[_Standards/WikiData/WD~Q25720347,25720347|WD~Q25720347,25720347]]'
- '[[_Standards/WikiData/WD~Q125741688,125741688|WD~Q125741688,125741688]]'
UMLS_CUI: C0035237
WordLift_URL: http://data.medicalrecords.com/medicalrecords/healthwise/respiratory_system
dv_has_:
  name_:
    af: asemhalingstelsel
    an: Sistema respiratorio
    ar: جهاز تنفسي
    as: শ্বাস তন্ত্ৰ
    ast: Aparatu respiratoriu
    az: İnsanın tənəffüs sistemi
    azb: تنفوس سیستمی
    ba: Һулыш алыу системаһы
    bcl: Sistemang respiratoryo
    be: дыхальная сістэма
    be_tarask: дыхальная сыстэма
    bg: Дихателна система
    bn: শ্বসনতন্ত্র
    bo: དབུགས་ལམ་མ་ལག་
    bs: Respiratorni sistem
    ca: sistema respiratori
    ckb: کۆئەندامی ھەناسە
    crh: nefes sisteması
    cs: dýchací soustava
    cy: system resbiradu
    da: åndedrætssystem
    de: Atemwege
    de_ch: Atemtrakt
    dv: ނޭވާލާ ނިޒާމް
    el: αναπνευστικό σύστημα
    en: respiratory system
    en_ca: Respiratory system
    en_gb: respiratory system
    eo: spira sistemo
    es: aparato respiratorio
    es-419: el sistema respiratorio
    et: hingamiselundkond
    eu: arnas aparatu
    ext: Sistema respiratoriu
    fa: دستگاه تنفس
    fi: hengityselimistö
    fr: appareil respiratoire
    ga: córas riospráide
    gd: Analachadh
    gl: aparello respiratorio
    gsw: Atemtrakt
    he: מערכת הנשימה
    hi: श्वसन तंत्र
    hif: saans le waala system
    hr: dišni sustav
    hu: légzőrendszer
    hy: Շնչառական համակարգ
    id: Sistem pernapasan
    ilo: sistema ti panaganges
    io: Respirala sistemo
    is: Öndunarfæri
    it: apparato respiratorio
    ja: 呼吸器系
    jv: Sistem ambegan
    kab: Anagraw amesgunef
    kk: дем алу жүйесі
    kk_latn: dem alw jüyesi
    kn: ಉಸಿರಾಟದ ವ್ಯವಸ್ಥೆ
    knc: Respiratory system
    ko: 호흡 계통
    ku: Koendama henaseyê
    ku_latn: koendama henaseyê
    ky: дем алуу системасы
    la: Systema respiratorium
    lb: Ootmungsapparat beim Mënsch
    lfn: Sistem de respira
    li: aomhaolingsstèlsel
    lt: kvėpavimo sistema
    lv: elpošanas orgānu sistēma
    mad: Sistem panyabâ
    mk: дишен систем
    ml: ശ്വസനേന്ദ്രിയവ്യൂഹം
    ms: sistem pernafasan
    ms_arab: سيستم ڤرنفسن
    nan: Ho͘-khip hē-thóng
    nb: åndedrettssystem
    nds: Atensysteem
    ne: श्वासप्रस्वास प्रणाली
    new: रेस्पिरेटरी सिस्टम
    nl: ademhalingsstelsel
    nn: respirasjonssystem
    oc: sistèma respiratòri
    om: Sirna hargansuu
    pa: ਸਾਹ ਪ੍ਰਣਾਲੀ
    pam: Respiratory system
    pl: układ oddechowy
    pms: Sistema respiratòri
    pnb: ساہ لین پربندھ
    ps: ساييز غونډال
    pt: sistema respiratório
    pt_br: Sistema respiratório
    qu: Samana yawri
    ro: sistem respirator
    ru: дыхательная система
    sah: Тыынар тиһик
    sd: انسان ۾ ساھ کڻڻ جو عمل
    sh: Dišni sistem
    si: ශ්වසන පද්ධතිය
    sk: dýchacia sústava
    sl: dihala
    smn: vuoiŋâmorgaaneh
    so: Hab Neefsi
    sq: Sistemi i frymëmarrjes
    sr: систем органа за дисање
    sv: respirationssystem
    sw: Mfumo wa upumuaji
    szy: mihanhan ilabu' nu nanum
    ta: மூச்சுத் தொகுதி
    te: శ్వాస వ్యవస్థ
    th: ระบบหายใจ
    tl: Sistemang respiratoryo
    tr: solunum sistemi
    tt: сулыш системасы
    udm: шокан сӧзнэт
    ug: نەپەسلىنىش سىستېمى
    uk: дихальна система
    ur: نظام تنفس
    uz: nafas organlari
    vec: Aparato respiratorio
    vi: hệ hô hấp
    war: Sistema respiratoryo
    wuu: 呼吸系统
    yi: אטעם-סיסטעם
    yue: 呼吸系統
    zh: 呼吸系統
    zh_cn: 呼吸系统
    zh_hans: 呼吸系统
    zh_hant: 呼吸系統
    zh_hk: 呼吸系統
---

# [[Respiratory_System]] 

#is_/same_as :: [[../../../WikiData/WD~Respiratory_system,7891|WD~Respiratory_system,7891]] 

## #has_/text_of_/abstract 

> The **Respiratory system** (also respiratory apparatus, ventilatory system) is a biological system consisting of specific organs and structures used for gas exchange in animals and plants. The anatomy and physiology that make this happen varies greatly, depending on the size of the organism, the environment in which it lives and its evolutionary history. In land animals, the respiratory surface is internalized as linings of the lungs. Gas exchange in the lungs occurs in millions of small air sacs; in mammals and reptiles, these are called alveoli, and in birds, they are known as atria. These microscopic air sacs have a very rich blood supply, thus bringing the air into close contact with the blood. These air sacs communicate with the external environment via a system of airways, or hollow tubes, of which the largest is the trachea, which branches in the middle of the chest into the two main bronchi. These enter the lungs where they branch into progressively narrower secondary and tertiary bronchi that branch into numerous smaller tubes, the bronchioles. In birds, the bronchioles are termed parabronchi. It is the bronchioles, or parabronchi that generally open into the microscopic alveoli in mammals and atria in birds. Air has to be pumped from the environment into the alveoli or atria by the process of breathing which involves the muscles of respiration.
>
> In most fish, and a number of other aquatic animals (both vertebrates and invertebrates), the respiratory system consists of gills, which are either partially or completely external organs, bathed in the watery environment. This water flows over the gills by a variety of active or passive means. Gas exchange takes place in the gills which consist of thin or very flat filaments and lammellae which expose a very large surface area of highly vascularized tissue to the water.
>
> Other animals, such as insects, have respiratory systems with very simple anatomical features, and in amphibians, even the skin plays a vital role in gas exchange. Plants also have respiratory systems but the directionality of gas exchange can be opposite to that in animals. The respiratory system in plants includes anatomical features such as stomata, that are found in various parts of the plant.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Respiratory%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Medicine/Anatomy/Respiratory_System|Respiratory_System]] 

### #is_/same_as :: [[/_public/bio/Medicine/Anatomy/Respiratory_System.public|Respiratory_System.public]] 

### #is_/same_as :: [[/_internal/bio/Medicine/Anatomy/Respiratory_System.internal|Respiratory_System.internal]] 

### #is_/same_as :: [[/_protect/bio/Medicine/Anatomy/Respiratory_System.protect|Respiratory_System.protect]] 

### #is_/same_as :: [[/_private/bio/Medicine/Anatomy/Respiratory_System.private|Respiratory_System.private]] 

### #is_/same_as :: [[/_personal/bio/Medicine/Anatomy/Respiratory_System.personal|Respiratory_System.personal]] 

### #is_/same_as :: [[/_secret/bio/Medicine/Anatomy/Respiratory_System.secret|Respiratory_System.secret]] 

