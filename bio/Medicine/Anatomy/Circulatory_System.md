---
aliases:
- aparato circulatorio
- aparato sircolatorio
- aparèy kardyovaskilèr
- apparato circolatorio
- apparatu circulatoriu
- appareil cardiovasculaire
- asinsrites orgānu sistēma
- blodkredsløbet
- bloedsomloopstelsel
- bloedsomrin
- Bloodkreisloop
- Bludkreisluup
- Bluetkräislauf
- Bluttkreeslaf
- blóðrásarkerfi
- circulatory seestem
- Circulatory System
- Circulatory_System
- cirkulatorni sistem
- córas imsruthaithe
- dolaşım sistemi
- faiga tau faataamilosaga
- gwazhiadurezh
- habdhiska wareega dhiiga
- hart en vaatstelsel
- hjärt-kärlsystemet
- hệ tuần hoàn
- kardiovaskula sistemo
- Katta qon aylanish doirasi
- keringési rendszer
- kraujotakos sistema
- Kreislauf-System
- Kreislaufsystem
- krvotok
- krvožilni sustav
- mfumo wa mzunguko wa damu
- obehová sústava
- obtočila
- oběhová soustava
- qan dövranı sistemi
- Sinzvanz hidungj
- sirk'a llika
- sirkulasjonssystem
- sistem de sirculi
- sistem kardiovaskular
- sistem paredaran darah
- sistem peredaran
- sistema cardiovascular
- sistema circulatori
- sistema circulatorio
- sistema circulatoriu
- sistema circulatório
- sistema sircolatòri
- sistema sirkulatoryo
- sistema ti panagbalikuskos
- sistemang sirkulatoryo
- sistemi i qarkullimit të gjakut
- sistemul circulator
- sistèm sikilatwa fèmen
- sistèma circulatòri
- system gylchredol
- system kylghresek
- systema cardiovasculare
- systema circulatori
- sîstema gera xwînê
- sùn-fàn hì-thúng
- sûn-khoân hē-thóng
- südame-veresoonkond
- tuguy syryha ypykatu
- układ krwionośny człowieka
- verenkiertoelimet
- yürek-damar sisteması
- zirkulazio-aparatu
- καρδιαγγειακό σύστημα
- вир берган сӧзнэт
- йөрәк-ҡан тамырҙары системаһы
- кан әйләнеше системасы
- крвни систем
- кръвообращение
- крывяносная сыстэма
- сістэма кровазвароту
- система кровообігу
- циркулаторияб система
- циркуляторная система
- цусны эргэлтийн тогтолцоо
- шуһанай эрьелтэ
- қанайналым жүйесі
- արյան շրջանառության համակարգ
- בלוט צירקולאציע
- מחזור הדם
- ئايلىنىش سىستېمىسى
- انسان ۾ رت جي دوري جو سرشتو
- جهاز الدوران
- جهاز دورى
- د زړه او رگونو غونډال
- دستگاه گردش خون
- سرکولیٹری پربندھ
- سيستم ڤرايدرن
- قان دؤورانی
- قلبی وِ عائی نظام
- نِظامِ گَردِش خوٗن
- کۆئەندامی سووڕان
- ލޭދައުރުކުރާ ނިޒާމް
- परिसंचरण तंत्र
- मानवसञ्चारतन्त्रम्
- रक्तवहसंस्था
- সংবহন তন্ত্র
- সংবহন তন্ত্ৰ
- ਖੂਨ ਸੰਚਾਰ ਪ੍ਰਣਾਲੀ
- રુધિરાભિસરણ તંત્ર
- சுற்றோட்டத் தொகுதி
- ప్రసరణ వ్యవస్థ
- ರಕ್ತಪರಿಚಲನೆಯ ವ್ಯವಸ್ಥೆ
- രക്തചംക്രമണവ്യൂഹം
- පද්ධති-සංසරණ පද්ධතිය
- ระบบไหลเวียน
- འཁོར་སྐྱོད་མ་ལག་
- နှလုံး သွီးကြောစနစ်
- နှလုံး သွေးကြောစနစ်
- სისხლის მიმოქცევის სისტემა
- ប្រដាប់របត់ឈាម
- 循环系统
- 循環器系
- 循環系統
- ꯁꯔꯀꯨꯂꯦꯇꯔꯤ ꯁꯤꯁ꯭ꯇꯦꯝ
- 순환계통
has_id_wikidata: Q11068
Colon_Classification: L3
Commons_category: Cardiovascular system
described_by_source:
- '[[../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358|WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Encyclopædia_Britannica_11th_edition,867541|WD~Encyclopædia_Britannica_11th_edition,867541]]'
- "[[_Standards/WikiData/WD~The_New_Student's_Reference_Work,16082057|WD~The_New_Student's_Reference_Work,16082057]]"
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675|WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
development_of_anatomical_structure: '[[_Standards/WikiData/WD~circulatory_system_development,21428388|WD~circulatory_system_development,21428388]]'
has_characteristic:
- '[[_Standards/WikiData/WD~blood_circulation,14852003|WD~blood_circulation,14852003]]'
has_part_s_:
- '[[_Standards/WikiData/WD~cardiovascular_system,13168533|WD~cardiovascular_system,13168533]]'
- '[[_Standards/WikiData/WD~blood,7873|WD~blood,7873]]'
- '[[_Standards/WikiData/WD~respiratory_system,7891|WD~respiratory_system,7891]]'
has_part_s_of_the_class:
- '[[_Standards/WikiData/WD~open_circulatory_system,1912034|WD~open_circulatory_system,1912034]]'
- '[[_Standards/WikiData/WD~closed_circulatory_system,2874712|WD~closed_circulatory_system,2874712]]'
Iconclass_notation: 49G611
image: http://commons.wikimedia.org/wiki/Special:FilePath/Blutkreislauf.png
instance_of:
- '[[_Standards/WikiData/WD~anatomical_system_type,104027169|WD~anatomical_system_type,104027169]]'
- '[[_Standards/WikiData/WD~class_of_anatomical_entity,112826905|WD~class_of_anatomical_entity,112826905]]'
Krugosvet_article: biologiya/krovenosnaya-sistema
MeSH_tree_code:
- A07
said_to_be_the_same_as: '[[_Standards/WikiData/WD~cardiovascular_system,13168533|WD~cardiovascular_system,13168533]]'
subclass_of:
- '[[_Standards/WikiData/WD~organ_system,188193|WD~organ_system,188193]]'
- '[[_Standards/WikiData/WD~particular_anatomical_entity,112826975|WD~particular_anatomical_entity,112826975]]'
topic_s_main_template:
- '[[_Standards/WikiData/WD~Template_Arteries_and_veins,6308489|WD~Template_Arteries_and_veins,6308489]]'
- '[[_Standards/WikiData/WD~Q6853167,6853167|WD~Q6853167,6853167]]'
UMLS_CUI: C0007226
dv_has_:
  name_:
    af: bloedsomloopstelsel
    an: sistema circulatorio
    ar: جهاز الدوران
    arz: جهاز دورى
    as: সংবহন তন্ত্ৰ
    ast: sistema cardiovascular
    av: циркулаторияб система
    az: qan dövranı sistemi
    azb: قان دؤورانی
    ba: йөрәк-ҡан тамырҙары системаһы
    bcl: sistemang sirkulatoryo
    be: сістэма кровазвароту
    be_tarask: крывяносная сыстэма
    bg: кръвообращение
    bn: সংবহন তন্ত্র
    bo: འཁོར་སྐྱོད་མ་ལག་
    br: gwazhiadurezh
    bs: krvotok
    bxr: шуһанай эрьелтэ
    ca: sistema circulatori
    ckb: کۆئەندامی سووڕان
    crh: yürek-damar sisteması
    cs: oběhová soustava
    cy: system gylchredol
    da: blodkredsløbet
    de: Kreislaufsystem
    dv: ލޭދައުރުކުރާ ނިޒާމް
    el: καρδιαγγειακό σύστημα
    en: circulatory system
    eo: kardiovaskula sistemo
    es: aparato circulatorio
    et: südame-veresoonkond
    eu: zirkulazio-aparatu
    ext: sistema circulatoriu
    fa: دستگاه گردش خون
    fi: verenkiertoelimet
    fr: appareil cardiovasculaire
    frr: Bludkreisluup
    fy: bloedsomrin
    ga: córas imsruthaithe
    gcr: aparèy kardyovaskilèr
    gl: aparato circulatorio
    gn: tuguy syryha ypykatu
    gsw: Bluetkräislauf
    gu: રુધિરાભિસરણ તંત્ર
    hak: sùn-fàn hì-thúng
    he: מחזור הדם
    hi: परिसंचरण तंत्र
    hif: circulatory system
    hr: krvožilni sustav
    ht: sistèm sikilatwa fèmen
    hu: keringési rendszer
    hy: արյան շրջանառության համակարգ
    ia: systema circulatori
    id: sistem kardiovaskular
    ilo: sistema ti panagbalikuskos
    is: blóðrásarkerfi
    it: apparato circolatorio
    ja: 循環器系
    jv: sistem kardiovaskular
    ka: სისხლის მიმოქცევის სისტემა
    kk: қанайналым жүйесі
    km: ប្រដាប់របត់ឈាម
    kn: ರಕ್ತಪರಿಚಲನೆಯ ವ್ಯವಸ್ಥೆ
    ko: 순환계통
    ks: نِظامِ گَردِش خوٗن
    ku: sîstema gera xwînê
    kw: system kylghresek
    la: systema cardiovasculare
    lb: Bluttkreeslaf
    lfn: sistem de sirculi
    lt: kraujotakos sistema
    lv: asinsrites orgānu sistēma
    min: sistem paredaran darah
    ml: രക്തചംക്രമണവ്യൂഹം
    mn: цусны эргэлтийн тогтолцоо
    mni: ꯁꯔꯀꯨꯂꯦꯇꯔꯤ ꯁꯤꯁ꯭ꯇꯦꯝ
    mr: रक्तवहसंस्था
    ms: sistem peredaran
    ms_arab: سيستم ڤرايدرن
    my: နှလုံး သွေးကြောစနစ်
    nan: sûn-khoân hē-thóng
    nb: sirkulasjonssystem
    nds: Bloodkreisloop
    nl: hart en vaatstelsel
    nn: sirkulasjonssystem
    nup: Circulatory system
    oc: sistèma circulatòri
    pa: ਖੂਨ ਸੰਚਾਰ ਪ੍ਰਣਾਲੀ
    pam: circulatory system
    pl: układ krwionośny człowieka
    pms: sistema sircolatòri
    pnb: سرکولیٹری پربندھ
    ps: د زړه او رگونو غونډال
    pt: sistema circulatório
    qu: sirk'a llika
    rki: နှလုံး သွီးကြောစနစ်
    ro: sistemul circulator
    ru: циркуляторная система
    sa: मानवसञ्चारतन्त्रम्
    scn: apparatu circulatoriu
    sco: circulatory seestem
    sd: انسان ۾ رت جي دوري جو سرشتو
    sh: cirkulatorni sistem
    si: පද්ධති-සංසරණ පද්ධතිය
    sk: obehová sústava
    sl: obtočila
    sm: faiga tau faataamilosaga
    so: habdhiska wareega dhiiga
    sq: sistemi i qarkullimit të gjakut
    sr: крвни систем
    sr_ec: крвни систем
    sv: hjärt-kärlsystemet
    sw: mfumo wa mzunguko wa damu
    ta: சுற்றோட்டத் தொகுதி
    te: ప్రసరణ వ్యవస్థ
    th: ระบบไหลเวียน
    tl: sistemang sirkulatoryo
    tr: dolaşım sistemi
    tt: кан әйләнеше системасы
    udm: вир берган сӧзнэт
    ug: ئايلىنىش سىستېمىسى
    uk: система кровообігу
    ur: قلبی وِ عائی نظام
    uz: Katta qon aylanish doirasi
    vec: aparato sircolatorio
    vi: hệ tuần hoàn
    war: sistema sirkulatoryo
    wuu: 循环系统
    yi: בלוט צירקולאציע
    yue: 循環系統
    za: Sinzvanz hidungj
    zh: 循环系统
    zh_cn: 循环系统
    zh_hans: 循环系统
    zh_hant: 循環系統
---

# [[Circulatory_System]] 

#is_/same_as :: [[../../../WikiData/WD~Circulatory_system,11068|WD~Circulatory_system,11068]] 

## #has_/text_of_/abstract 

> In vertebrates, the **Circulatory system** is a system of organs that includes the heart, blood vessels, and blood which is circulated throughout the body. It includes the cardiovascular system, or vascular system, that consists of the heart and blood vessels (from Greek kardia meaning heart, and Latin vascula meaning vessels). The circulatory system has two divisions, a systemic circulation or circuit, and a pulmonary circulation or circuit. Some sources use the terms cardiovascular system and vascular system interchangeably with circulatory system.
>
> The network of blood vessels are the great vessels of the heart including large elastic arteries, and large veins; other arteries, smaller arterioles, capillaries that join with venules (small veins), and other veins. The circulatory system is closed in vertebrates, which means that the blood never leaves the network of blood vessels. Many invertebrates such as arthropods have an open circulatory system with a heart that pumps a hemolymph which returns via the body cavity rather than via blood vessels. Diploblasts such as sponges and comb jellies lack a circulatory system.
>
> Blood is a fluid consisting of plasma, red blood cells, white blood cells, and platelets; it is circulated around the body carrying oxygen and nutrients to the tissues and collecting and disposing of waste materials. Circulated nutrients include proteins and minerals and other components include  hemoglobin, hormones, and gases such as oxygen and  carbon dioxide. These substances provide nourishment, help the immune system to fight diseases, and help maintain homeostasis by stabilizing temperature and natural pH.
>
> In vertebrates, the lymphatic system is complementary to the circulatory system. 
> The lymphatic system carries excess plasma (filtered from the circulatory system capillaries as interstitial fluid between cells) away from the body tissues via accessory routes that return excess fluid back to blood circulation as lymph. 
> The lymphatic system is a subsystem that is essential for the functioning of the blood circulatory system; 
> without it the blood would become depleted of fluid.
>
> The lymphatic system also works with the immune system. The circulation of lymph takes much longer than that of blood and, unlike the closed (blood) circulatory system, the lymphatic system is an open system. Some sources describe it as a secondary circulatory system.
>
> The circulatory system can be affected by many cardiovascular diseases. Cardiologists are medical professionals which specialise in the heart, and cardiothoracic surgeons specialise in operating on the heart and its surrounding areas. Vascular surgeons focus on disorders of the blood vessels, and lymphatic vessels.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Circulatory%20system) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Medicine/Anatomy/Circulatory_System|Circulatory_System]] 

### #is_/same_as :: [[/_public/bio/Medicine/Anatomy/Circulatory_System.public|Circulatory_System.public]] 

### #is_/same_as :: [[/_internal/bio/Medicine/Anatomy/Circulatory_System.internal|Circulatory_System.internal]] 

### #is_/same_as :: [[/_protect/bio/Medicine/Anatomy/Circulatory_System.protect|Circulatory_System.protect]] 

### #is_/same_as :: [[/_private/bio/Medicine/Anatomy/Circulatory_System.private|Circulatory_System.private]] 

### #is_/same_as :: [[/_personal/bio/Medicine/Anatomy/Circulatory_System.personal|Circulatory_System.personal]] 

### #is_/same_as :: [[/_secret/bio/Medicine/Anatomy/Circulatory_System.secret|Circulatory_System.secret]] 

