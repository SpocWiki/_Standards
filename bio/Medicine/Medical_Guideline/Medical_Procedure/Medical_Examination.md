---
aliases:
- badanie fizykalne
- clinical examination
- esame obiettivo
- esploración física
- exame físico
- exame médico
- examen clinique
- examinare clinică
- exploració física
- Exploración fisica
- exploración física
- fizik muayene
- fizikalni pregled
- fyzikálne vyšetrenie
- fyzikální vyšetření
- khám sức khỏe
- klinički pregled
- korpa ekzameno
- körperliche Untersuchung
- lichamelijk onderzoek
- lääkärintarkastus
- medical checkup
- medical examination
- Medical_Examination
- medicinsk undersökning
- medisinsk undersøking
- miaketa fisiko
- orvosi vizsgálat
- pemeriksaan fisik
- pemeriksaan fizikal
- physical examination
- scrúdú fisiciúil
- sistematski pregled
- tervisekontroll
- σωματική εξέταση
- фізикальне обстеження
- фізычная дыягностыка
- физикальное обследование
- ֆիզիկալ զննում
- בדיקה גופנית
- آزمایش بالینی
- فحص سريري
- فيزيکي کتنه
- ശാരീരിക പരിശോധന
- การตรวจร่างกายทางการแพทย์
- 体格检查
- 身体診察
- 身體檢查
- 驗身
- 體格檢查
- 신체검사
has_id_wikidata: Q835153
has_use: '[[_Standards/WikiData/WD~medical_diagnosis,177719|WD~medical_diagnosis,177719]]'
part_of: '[[_Standards/WikiData/WD~health_risk_assessment,2284929|WD~health_risk_assessment,2284929]]'
subclass_of:
- '[[_Standards/WikiData/WD~medical_examination,4287917|WD~medical_examination,4287917]]'
- '[[_Standards/WikiData/WD~health_assessment,5690807|WD~health_assessment,5690807]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Medical_records_and_physical_exam,10541826|WD~Template_Medical_records_and_physical_exam,10541826]]'
disjoint_union_of: '[[_Standards/WikiData/WD~list_of_values_as_qualifiers,23766486|WD~list_of_values_as_qualifiers,23766486]]'
has_evaluation: '[[_Standards/WikiData/WD~physical_finding,69034079|WD~physical_finding,69034079]]'
Wikidata_property: medical examination
Commons_category: Physical examinations
image: http://commons.wikimedia.org/wiki/Special:FilePath/Attending%20surgeon%27s%20office%3B%20examination%20room%2C%20Washington%2C%20D.C.%20World%20War%201%20%281910s%29.jpg
equivalent_class: https://schema.org/PhysicalExam
UMLS_CUI:
- C0031809
- C0031811
MeSH_tree_code: E01.370.600
Library_of_Congress_Classification: RC76-RC76.5
dv_has_:
  name_:
    an: Exploración fisica
    ar: فحص سريري
    ast: esploración física
    be_tarask: фізычная дыягностыка
    bs: klinički pregled
    ca: exploració física
    cs: fyzikální vyšetření
    de: körperliche Untersuchung
    el: σωματική εξέταση
    en: physical examination
    en_gb: physical examination
    eo: korpa ekzameno
    es: exploración física
    et: tervisekontroll
    eu: miaketa fisiko
    fa: آزمایش بالینی
    fi: lääkärintarkastus
    fr: examen clinique
    ga: scrúdú fisiciúil
    gl: exame médico
    he: בדיקה גופנית
    hr: sistematski pregled
    hu: orvosi vizsgálat
    hy: ֆիզիկալ զննում
    id: pemeriksaan fisik
    it: esame obiettivo
    ja: 身体診察
    ko: 신체검사
    ml: ശാരീരിക പരിശോധന
    ms: pemeriksaan fizikal
    nl: lichamelijk onderzoek
    nn: medisinsk undersøking
    pl: badanie fizykalne
    ps: فيزيکي کتنه
    pt: exame físico
    ro: examinare clinică
    ru: физикальное обследование
    sk: fyzikálne vyšetrenie
    sl: fizikalni pregled
    sr: klinički pregled
    sv: medicinsk undersökning
    th: การตรวจร่างกายทางการแพทย์
    tr: fizik muayene
    uk: фізикальне обстеження
    vi: khám sức khỏe
    wuu: 体格检查
    yue: 驗身
    zh: 体格检查
    zh_hans: 体格检查
    zh_hant: 身體檢查
    zh_hk: 體格檢查
    zh_tw: 身體檢查
---

# [[Medical_Examination]] 

#is_/same_as :: [[_Standards/WikiData/WD~Physical_examination,835153|WD~Physical_examination,835153]] 

## #has_/text_of_/abstract 

> In a physical examination, medical examination, clinical examination, or medical checkup, 
> a medical practitioner examines a patient for any possible medical signs 
> or symptoms of a medical condition. 
> 
> It generally consists of a series of questions about the patient's medical history 
> followed by an examination based on the reported symptoms. 
> 
> Together, the medical history and the physical examination help to determine a diagnosis 
> and devise the treatment plan. 
> 
> These data then become part of the medical record.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Physical%20examination) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Medicine/Medical_Guideline/Medical_Procedure/Medical_Examination|Medical_Examination]] 

### #is_/same_as :: [[/_public/bio/Medicine/Medical_Guideline/Medical_Procedure/Medical_Examination.public|Medical_Examination.public]] 

### #is_/same_as :: [[/_internal/bio/Medicine/Medical_Guideline/Medical_Procedure/Medical_Examination.internal|Medical_Examination.internal]] 

### #is_/same_as :: [[/_protect/bio/Medicine/Medical_Guideline/Medical_Procedure/Medical_Examination.protect|Medical_Examination.protect]] 

### #is_/same_as :: [[/_private/bio/Medicine/Medical_Guideline/Medical_Procedure/Medical_Examination.private|Medical_Examination.private]] 

### #is_/same_as :: [[/_personal/bio/Medicine/Medical_Guideline/Medical_Procedure/Medical_Examination.personal|Medical_Examination.personal]] 

### #is_/same_as :: [[/_secret/bio/Medicine/Medical_Guideline/Medical_Procedure/Medical_Examination.secret|Medical_Examination.secret]] 

