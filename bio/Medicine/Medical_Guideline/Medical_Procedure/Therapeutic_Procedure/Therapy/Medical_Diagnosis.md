---
aliases:
- asennefru
- azri
- Cudursheeg
- diagnoos
- diagnos
- diagnose
- diagnose médica
- diagnosi
- diagnosi mèdica
- diagnosis
- diagnosis meddygol
- diagnosis medica
- diagnostic médical
- diagnostig
- Diagnoz
- diagnozė
- diagnóis
- diagnóza
- dijagnoza
- diàgnoze mèdega
- dukkako diagnoosos
- dálkkasdieđalaš diagnosa
- geneeskundige diagnose
- isiphumo sokuxilonga
- kumbondera chirwere
- lääketieteellinen diagnoosi
- medical diagnosis
- medicina diagnozo
- medicinska diagnoza
- medicinska dijagnoza
- medicīniskā diagnoze
- mediese diagnose
- orvosi diagnózis
- rozpoznanie
- taalkâstiõđlaž diagnoos
- talhâstieđâlâš diagnoos
- Tatenger médis
- ukuhlolwa kwesifo
- utambuzi
- veniyasîn
- xét nghiệm y khoa
- yareɛ mu nhwehw3 mu
- διάγνωση
- Дијагноза
- Диагноз
- диагноза
- дыягназ
- медицинска дијагноза
- медицинска диагноза
- медицинский диагноз
- медичний діагноз
- ташхис
- ախտանուն
- אבחנה
- تشخيص
- تشخیص
- تشخیص پزشکی
- تَشخیٖص (طِب)
- دۆزینەوە
- طبي تشخیص
- निदान
- রোগ নির্ণয়
- ਰੋਗ-ਪਛਾਣ
- அறுதியிடல்
- ರೋಗನಿದಾನ
- രോഗനിർണയം
- การวินิจฉัยทางการแพทย์
- დიაგნოზი
- 睇症
- 診断行為
- 診斷
- 诊断
- 진단
has_id_wikidata: Q177719
described_by_source:
- '[[../../../../../../WikiData/WD~Brockhaus_and_Efron_Encyclopedic_Dictionary,602358]]'
- '[[_Standards/WikiData/WD~Armenian_Soviet_Encyclopedia,2657718]]'
- '[[_Standards/WikiData/WD~Small_Brockhaus_and_Efron_Encyclopedic_Dictionary,19180675]]'
has_cause: '[[_Standards/WikiData/WD~medical_diagnostics,9206884]]'
topic_has_template: '[[_Standards/WikiData/WD~Template_Infobox_diagnostic,12201161]]'
subclass_of: '[[_Standards/WikiData/WD~diagnosis,16644043]]'
Wikidata_property:
- medical examination
- positive diagnostic predictor for
Library_of_Congress_Classification: RC71-RC78.7
Commons_category: Diagnosis
UMLS_CUI: C0525034
dv_has_:
  name_:
    af: mediese diagnose
    ar: تشخيص
    ban: Tatenger médis
    be: Дыягназ
    be-tarask: дыягназ
    bg: медицинска диагноза
    bn: রোগ নির্ণয়
    br: diagnostig
    bs: dijagnoza
    ca: diagnosi mèdica
    ckb: دۆزینەوە
    cs: diagnóza
    cy: diagnosis meddygol
    da: diagnose
    dag: Diagnosis
    de: Diagnose
    de-ch: Diagnose
    el: διάγνωση
    en: medical diagnosis
    en-ca: medical diagnosis
    en-gb: medical diagnosis
    eo: medicina diagnozo
    es: Diagnosis
    et: diagnoos
    eu: diagnosi
    fa: تشخیص پزشکی
    fi: lääketieteellinen diagnoosi
    fr: diagnostic médical
    ga: diagnóis
    gl: diagnose médica
    gsw: Diagnose
    he: אבחנה
    hi: निदान
    hr: medicinska dijagnoza
    hu: orvosi diagnózis
    hy: ախտանուն
    id: diagnosis
    it: diagnosi
    ja: 診断行為
    ka: დიაგნოზი
    kab: asennefru
    kk: Диагноз
    kn: ರೋಗನಿದಾನ
    ko: 진단
    ks: تَشخیٖص (طِب)
    ku: veniyasîn
    ky: диагноз
    la: diagnosis medica
    lb: Diagnos
    lt: diagnozė
    lv: medicīniskā diagnoze
    mhr: диагноз
    mk: Дијагноза
    ml: രോഗനിർണയം
    ms: diagnosis
    nb: diagnose
    nl: geneeskundige diagnose
    nn: diagnose
    pa: ਰੋਗ-ਪਛਾਣ
    pl: rozpoznanie
    ps: طبي تشخیص
    rmf: dukkako diagnoosos
    ru: медицинский диагноз
    rue: диагноза
    sco: medical diagnosis
    se: dálkkasdieđalaš diagnosa
    sh: dijagnoza
    shi: azri
    sk: diagnóza
    sl: medicinska diagnoza
    smn: talhâstieđâlâš diagnoos
    sms: taalkâstiõđlaž diagnoos
    sn: kumbondera chirwere
    so: Cudursheeg
    sr: медицинска дијагноза
    sv: diagnos
    sw: utambuzi
    ta: அறுதியிடல்
    tg: ташхис
    th: การวินิจฉัยทางการแพทย์
    tt: диагноз
    tw: yareɛ mu nhwehw3 mu
    uk: медичний діагноз
    ur: تشخیص
    uz: Diagnoz
    vec: diàgnoze mèdega
    vi: xét nghiệm y khoa
    wuu: 诊断
    xh: isiphumo sokuxilonga
    yue: 睇症
    zh: 诊断
    zh-cn: 诊断
    zh-hans: 诊断
    zh-hant: 診斷
    zh-hk: 診斷
    zh-sg: 诊断
    zh-tw: 診斷
    zu: ukuhlolwa kwesifo
---
# [[Medical_Diagnosis]] 

#is_/same_as :: [[WD~Medical_diagnosis,177719]] 

## #has_/text_of_/abstract 

> Medical diagnosis (abbreviated Dx, Dx, or Ds) is the process of determining 
> which disease or condition explains a person's symptoms and signs. 
> 
> It is most often referred to as a diagnosis with the medical context being implicit. 
> The information required for a diagnosis is typically collected from a history and physical examination of the person seeking medical care. Often, one or more diagnostic procedures, such as medical tests, are also done during the process. Sometimes the posthumous diagnosis is considered a kind of medical diagnosis.
>
> Diagnosis is often challenging because many signs and symptoms are nonspecific. For example, redness of the skin (erythema), by itself, is a sign of many disorders and thus does not tell the healthcare professional what is wrong. Thus differential diagnosis, in which several possible explanations are compared and contrasted, must be performed. This involves the correlation of various pieces of information followed by the recognition and differentiation of patterns. Occasionally the process is made easy by a sign or symptom (or a group of several) that is pathognomonic.
>
> Diagnosis is a major component of the procedure of a doctor's visit. From the point of view of statistics, the diagnostic procedure involves classification tests.
>
> [Wikipedia](https://en.wikipedia.org/wiki/Medical%20diagnosis) 


## Confidential Links & Embeds: 

### #is_/same_as :: [[/_Standards/bio/Medicine/Medical_Guideline/Medical_Procedure/Therapeutic_Procedure/Therapy/Medical_Diagnosis|Medical_Diagnosis]] 

### #is_/same_as :: [[/_public/bio/Medicine/Medical_Guideline/Medical_Procedure/Therapeutic_Procedure/Therapy/Medical_Diagnosis.public|Medical_Diagnosis.public]] 

### #is_/same_as :: [[/_internal/bio/Medicine/Medical_Guideline/Medical_Procedure/Therapeutic_Procedure/Therapy/Medical_Diagnosis.internal|Medical_Diagnosis.internal]] 

### #is_/same_as :: [[/_protect/bio/Medicine/Medical_Guideline/Medical_Procedure/Therapeutic_Procedure/Therapy/Medical_Diagnosis.protect|Medical_Diagnosis.protect]] 

### #is_/same_as :: [[/_private/bio/Medicine/Medical_Guideline/Medical_Procedure/Therapeutic_Procedure/Therapy/Medical_Diagnosis.private|Medical_Diagnosis.private]] 

### #is_/same_as :: [[/_personal/bio/Medicine/Medical_Guideline/Medical_Procedure/Therapeutic_Procedure/Therapy/Medical_Diagnosis.personal|Medical_Diagnosis.personal]] 

### #is_/same_as :: [[/_secret/bio/Medicine/Medical_Guideline/Medical_Procedure/Therapeutic_Procedure/Therapy/Medical_Diagnosis.secret|Medical_Diagnosis.secret]] 

